    ...
    <script>
        $(document).ready(function() {
            ...
            function sendSMS() {
                ...
                // Send SMS via AJAX
                $.ajax({
                    url: '{{ route("kayit.action") }}', // Your register route that sends SMS
                    method: 'POST',
                    data: formData,
            ...
            $('#nextBtn').on('click', function(e) {
                ...
                if (currentStep < 2) {
                    ...
                    if (validateStep(currentStep)) {
                        currentStep++;
                        showStep(currentStep);
                        // Eğer 3. adıma geçiliyorsa ve SMS henüz gönderilmediyse, SMS'i otomatik gönder
                        if (currentStep === 2 && !smsSent) {
                            if (validateStep(1)) { // 2. adımı (firma bilgileri) doğrula
                                sendSMS();
                            } else {
                                // Eğer 2. adım doğrulanamazsa, 3. adıma geçişi engelle
                                currentStep--; // Adımı geri al
                                showStep(currentStep);
                            }
                        }
                    }
                    ...
                    // SMS step
                    if (!smsSent) {
                        // Eğer SMS gönderilmediyse (örneğin kullanıcı geri gelip tekrar bu adıma geldiyse)
                        if (validateStep(1)) { // 2. adımı (firma bilgileri) doğrula
                            sendSMS();
                        }
                    } else {
            ...
            $('#prevBtn').on('click', function() {
                if (currentStep > 0) {
                    ...
                    // If going back from SMS step, reset SMS state
                    if (currentStep < 2) {
                        clearInterval(countdownInterval);
                        $('#countdownTimer').hide();
                        smsSent = false;
