...
<!-- reCAPTCHA script tag'ına callback ekleyin -->
<script src="https://www.google.com/recaptcha/enterprise.js?onload=onRecaptchaLoad" async defer></script>    
...
<body>
    ...
    // Form reset yerine sadece step'i sıfırla
    currentStep = 0;
    ...
    // Eğer plan seçilmemişse default planı yükle
    if (!$('#subscription_plan').val()) {
        loadSubscriptionPlans();
    }
}

            // Eğer tamamen sıfırlamak istiyorsanız, ayrı bir fonksiyon oluşturun:
function resetRegisterForm() {
    ...
    // Planları yeniden yükle ve default planı seç
    loadSubscriptionPlans();
}
...
function loadSubscriptionPlans() {
    ...
    planInfo.show();
        ...
        // reCAPTCHA'yı form genişliğine uyarlamak için JavaScript
$(document).ready(function() {
    // reCAPTCHA yüklendikten sonra boyutlandır
    setTimeout(function() {
        resizeRecaptcha();
    }, 1000);
    
    // Sayfa yeniden boyutlandırıldığında da ayarla
...
function resizeRecaptcha() {
    const formWidth = $('#loginForm').width();
    const recaptchaContainer = $('#loginForm .g-recaptcha');
    
    if (recaptchaContainer.length > 0) {
        // Form genişliğine göre reCAPTCHA'yı ölçeklendir
        let scale = 1;
        const standardWidth = 304; // Google reCAPTCHA standart genişliği
        
        if (formWidth < standardWidth) {
            scale = formWidth / standardWidth;
        }
...
// reCAPTCHA callback fonksiyonu (isteğe bağlı)
