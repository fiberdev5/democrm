...
function showFieldError(fieldName, message) {
    // name attribute bazlı arama
    const field = $('[name="'+fieldName+'"]');
    field.addClass('is-invalid');
    // .server-error yoksa ekle
    let $err = field.closest('.mb-3').find('.server-error');
    if (!$err.length) {
        $err = $('<div class="text-danger server-error mt-2"></div>');
        // input'tan sonra göster
        field.after($err);
    }
    $err.text(message);
}
    ...
    $.ajax({
        url: '{{ route("kayit.action") }}', // mevcut register route'un
        method: 'POST',
        data: formData,
        success: function(response) {
            // Sunucu validasyonu başarılı ve SMS gönderildi
            smsSent = true;
            $('#nextBtn').text('Doğrula ve Kaydı Tamamla');
            $('#countdownTimer').show();
            startCountdown();
        ...
        error: function(xhr) {
            // Validation hataları 422 ile döner (Laravel)
            if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                ...
                toastr.error('Sunucuda bir hata oluştu. Lütfen tekrar deneyin.');
            }
    ...
    if (currentStep < 2) {
        // client-side required kontrol
        if (!validateStep(currentStep)) return;

        // Eğer 2. adımdan (index 1) SMS adımına geçilecekse:
        if (currentStep === 1) {
            // server validation ve SMS gönderimini yap, başarılıysa adımı ilerlet
            sendSMS(function(success) {
                if (success) {
                    currentStep++;
                    showStep(currentStep);
                } else {
                    // Hata varsa adım değişmez (hata mesajları inputların yanında gösteriliyor)
                }
            });
        } else {
            // normal ilerleme (1. adımdan 2. adıma gibi)
            currentStep++;
            showStep(currentStep);
        }
    } else if (currentStep === 2) {
        // SMS adımı: eğer sms daha önce gönderilmemişse tekrar gönder, aksi halde doğrula
        if (!smsSent) {
            if (validateStep(1)) {
                sendSMS(function(success) {
                    if (success) {
                        // zaten sms adımındayız; kullanıcı kutusuna kodu girecek
                    }
                });
            }
        } else {
...
// prev ve reset fonksiyonlarında sunucu hatalarını temizle
