    ...
    <script>
        $(document).ready(function() {
            ...
            function showPlanInfo(planData) {
                ...
                // Function to display limit value
                function formatLimitValue(value) {
                    return value == -1 ? 'Sınırsız' : value;
                }
                
                // Plan limits
                featuresHtml += `<span class="plan-feature"><strong>Kullanıcı:</strong> ${formatLimitValue(planData.users)}</span>`;
                featuresHtml += `<span class="plan-feature"><strong>Bayi:</strong> ${formatLimitValue(planData.dealers)}</span>`;
                featuresHtml += `<span class="plan-feature"><strong>Stok:</strong> ${formatLimitValue(planData.stocks)}</span>`;
                featuresHtml += `<span class="plan-feature"><strong>Konsinye:</strong> ${formatLimitValue(planData.konsinye)}</span>`;
                
                // Plan features
                if (planData.tickets) {
                    featuresHtml += `<span class="plan-feature">✓ Ticket Sistemi</span>`;
                }
                if (planData.basic_reports) {
                    featuresHtml += `<span class="plan-feature">✓ Temel Raporlar</span>`;
                }
                if (planData.inventory) {
                    featuresHtml += `<span class="plan-feature">✓ Envanter Yönetimi</span>`;
                }
