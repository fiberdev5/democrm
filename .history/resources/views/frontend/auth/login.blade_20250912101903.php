    ...
    <script>
        $(document).ready(function() {
            ...
            function loadSubscriptionPlans() {
                $.ajax({
                    url: '{{ route("get.subscription.plans") }}', // Bu route'u oluşturmanız gerekiyor
                    method: 'GET',
                    success: function(response) {
                        ...
                        response.plans.forEach(function(plan) {
                            const option = $('<option></option>')
                                ...
                                .data('konsinye', plan.limits.konsinye || '0')
                            ...
                            // Set default plan (lowest price)
                            if (!defaultPlan || parseFloat(plan.price) < parseFloat(defaultPlan.price)) {
                                defaultPlan = plan;
                            }
                        });
                        
                        // Select default plan if no session plan
           ...
           // Mevcut showPlanInfo fonksiyonunuzu bu şekilde güncelleyin
function showPlanInfo(planData) {
    ...
    function formatLimitValue(value) {
        return value == -1 ? 'Sınırsız' : value.toLocaleString('tr-TR');
    }
    ...
    // Create modern features HTML
    const featuresHtml = `
        <div class="plan-info-container">
            <!-- Plan Header -->
            <div class="plan-header">
                <h5 class="plan-name">${planData.name} Planı</h5>
                <div class="plan-price">${formatPrice(planData.price, planData.billing_cycle)}</div>
            </div>

            <!-- Plan Limits Grid -->
            <div class="plan-features-grid">
                <div class="feature-item">
                    <div class="feature-icon ${planData.users == -1 ? 'unlimited' : 'limit'}">
                        ${planData.users == -1 ? '∞' : planData.users}
                    </div>
                    <div class="feature-text">
                        <div class="feature-label">Kullanıcı</div>
                        <div class="feature-value">${formatLimitValue(planData.users)} kullanıcı hesabı</div>
                    </div>
                </div>

                <div class="feature-item">
                    <div class="feature-icon ${planData.dealers == -1 ? 'unlimited' : 'limit'}">
                        ${planData.dealers == -1 ? '∞' : planData.dealers}
                    </div>
                    <div class="feature-text">
                        <div class="feature-label">Bayi</div>
                        <div class="feature-value">${formatLimitValue(planData.dealers)} bayi kaydı</div>
                    </div>
                </div>

                <div class="feature-item">
                    <div class="feature-icon ${planData.stocks == -1 ? 'unlimited' : 'limit'}">
                        ${planData.stocks == -1 ? '∞' : planData.stocks}
                    </div>
                    <div class="feature-text">
                        <div class="feature-label">Stok</div>
                        <div class="feature-value">${formatLimitValue(planData.stocks)} ürün kalemi</div>
                    </div>
                </div>

                <div class="feature-item">
                    <div class="feature-icon ${planData.konsinye == -1 ? 'unlimited' : 'limit'}">
                        ${planData.konsinye == -1 ? '∞' : planData.konsinye}
                    </div>
                    <div class="feature-text">
                        <div class="feature-label">Konsinye</div>
                        <div class="feature-value">${formatLimitValue(planData.konsinye)} konsinye kalemi</div>
                    </div>
                </div>
            </div>
            ...
            <div class="features-section">
                <div class="features-title">Dahil Olan Özellikler</div>
                <div class="feature-list">
                    ${planData.tickets === 'true' || planData.tickets === true ? '<span class="feature-badge">Ticket Sistemi</span>' : ''}
                    ${planData.basic_reports === 'true' || planData.basic_reports === true ? '<span class="feature-badge">Temel Raporlar</span>' : ''}
                    ${planData.inventory === 'true' || planData.inventory === true ? '<span class="feature-badge">Stok Yönetimi</span>' : ''}
                </div>
            </div>

            <!-- Info Tip -->
            <div class="info-tip">
                Bu plan iş ihtiyaçlarınıza uygun mu? İhtiyaçlarınız değiştiğinde planınızı kolayca yükseltebilirsiniz.
            </div>
        </div>
    `;
    
    featuresDiv.html(featuresHtml);
    planInfo.show();
}
