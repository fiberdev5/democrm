...
<section class="py-16 bg-gray-50">
  <div class="demo container-xxl">
      ...
      @foreach($pricing as $i => $price)
        ...
        <div class="relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition transform hover:-translate-y-2 p-8 flex flex-col text-center pricing-card">
          ...
          <div class="text-4xl font-extrabold text-gray-900 mb-4">
            ₺ {{ number_format($plan->price) }} <span class="text-base text-gray-500 font-normal">/yıllık</span> <span class="text-[11px] sm:text-xs bg-bls-teal-50 text-bls-success-700 px-2 py-0.5 rounded-xl ml-2">%<!-- -->30<!-- --> Kazanın</span>
          </div>
    ...
    <div class="flex items-center">
      ...
      <span class="font-medium">{{ $price->limits['users'] ?? '3' }} Kullanıcı</span>
    </div>
  </div>

          
          <a href="{{ route('plan.select', $price->id) }}" class="inline-block w-full py-2 rounded-full bg-gradient-to-r from-orange-500 to-[#f9b233] text-white font-semibold transition hover:opacity-90">
            Satın Al
          </a>
