...
document.addEventListener("DOMContentLoaded", function () {
  ...
  toggleButtons.forEach(function (btn) {
    btn.addEventListener("click", function () {
      const currentCard = this.closest(".relative"); // Ana kart div'ini seç
      const currentDescription = currentCard.querySelector(".price-description");

      // Tıkladığım butonun durumu ne olursa olsun, önce TÜM kartları kapat
      toggleButtons.forEach(function (otherBtn) {
        const otherCard = otherBtn.closest(".relative");
        const otherDescription = otherCard.querySelector(".price-description");

        // Eğer tıkladığım kart, döngüdeki kart değilse, onu kesinlikle kapat
        if (currentCard !== otherCard) {
          otherDescription.classList.add("hidden");
          otherBtn.textContent = "Detayları Gör";
        }
      });

      // Şimdi sadece tıkladığım kartın durumunu değiştir
