...
<section class="py-16 bg-gray-50">
  ...
  <!-- Detay içeriği -->
  <div class="price-description max-h-0 overflow-hidden transition-all duration-300 text-gray-600">
    <div class="pt-4">
      {!! $price->description !!}
    </div>
  </div>

  <a href="#" class="inline-block w-full py-3 mt-4 rounded-full bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-semibold transition hover:opacity-90">
    Sign Up
  </a>
</div>
      @endforeach
  ...
  document.querySelectorAll(".toggle-btn").forEach(btn => {
    btn.addEventListener("click", function () {
      ...
      if (desc.classList.contains("max-h-0")) {
        ...
        desc.classList.add("max-h-96"); // yeterli yüksekliği ver
