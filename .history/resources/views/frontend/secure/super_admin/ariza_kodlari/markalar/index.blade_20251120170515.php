            ...
            <div class="modal-body">
                <form id="markaEkle" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="marka" class="form-control marka" placeholder="Örn: BMW" required>
                    </div>
                    <div class="form-group">
                        <label>Logo Yükle</label>
                        <input type="file" name="resim" class="form-control-file">
                        <small class="text-muted d-block mt-1" style="font-size: 11px">Sadece jpg, png.</small>
                    </div>
                    <div class="text-right">
                        <!-- İsteğe bağlı iptal butonu -->
...
{{-- Marka Düzenle Modal (Bu AJAX ile dolacak, çünkü veri çekmesi lazım) --}}
...
$(document).ready(function(){
    ...
    /** 1. Marka Ekle Butonu (AJAX KALDIRILDI -> HIZLANDI) **/
    // Artık sunucudan veri beklemiyor, direkt açılıyor.
    $('.markaEkleBtn').click(function(){
        // Formu temizle (önceki yazı kalmasın)
        $('#markaEkle')[0].reset(); 
        // Modalı aç
        $('#markaEkleModal').modal('show'); 
    });

    /** 2. Marka Düzenle Modal (Burada AJAX şart) **/
    $(document).on('click', '.markaDuzenleBtn', function(){
        let id = $(this).data('id');
        
        // Modal açılırken kullanıcıya bilgi ver
        $('#markaDuzenleModal .modal-body').html('<div class="text-center p-3"><i class="fa fa-spinner fa-spin"></i> Veriler getiriliyor...</div>');
        $('#markaDuzenleModal').modal('show');

        // Veriyi çek ve modal içine bas
        $.get("/super-admin/marka-duzenle/" + id, function(data){
            // Gelen data sadece form inputları olmalı, modal-header vs. içermemeli
            $('#markaDuzenleModal .modal-body').html(data);
        });
    });

    /** 3. Marka Ekle Form Submit **/
    $(document).on("submit", "#markaEkle", function(e){
        ...
        $.ajax({
            url: "{{ route('super.admin.markalar.store') }}", // Route'un doğru olduğundan emin olun
            type: "POST",
