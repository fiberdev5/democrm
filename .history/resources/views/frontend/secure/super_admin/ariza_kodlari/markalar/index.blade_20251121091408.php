            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <button type="button" class="btn btn-success btn-sm markaEkleBtn">
...
<style>
    ...
    #markaTable_wrapper .dataTables_length {
        float: left;
    }
...
$(document).ready(function(){
    ...
    table = $('#markaTable').DataTable({
        ...
        "order": [[0, "asc"]],
        "paging": false, // Sayfalamayı kapat
        "info": false, // "Showing 1 to 10 of 50 entries" yazısını kapat
        "responsive": true,
        "dom": 'frti', // Sadece filter (arama), table, info kalsın
        "columnDefs": [
    ...
    $(document).on('change', '.custom-file-input', function() {
        ...
        // Önizleme göster (Ekle modalı için)
    ...
    $(document).on('click', '.markaDuzenleBtn', function(){
        ...
        // Mevcut resmi göster
        if(resim) {
            $('#mevcutResim img').attr('src', '{{ asset("upload") }}/' + resim);
            $('#mevcutResim').show();
        } else {
            $('#mevcutResim').hide();
        }
        
        // Custom file label'ı resetle
        $('.custom-file-label').removeClass('selected').html('Dosya Seçin');
    ...
    $(document).on('submit', '#markaDuzenleForm', function(e){
        ...
        $.ajax({
            url: "/super-admin/marka-guncelle/" + id,
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
