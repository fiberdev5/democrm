...
$(document).ready(function() {
    ...
    // 1. MARKA EKLEME BUTONU (Sayfa içinde sabit olduğu için click çalışır ama garanti olsun diye on kullanıyoruz)
    $(document).on('click', '.markaEkleBtn', function() {
        // Modal içeriğini temizle ve yükleniyor yaz
        ...
        $.ajax({
            ...
            error: function(xhr) {
                console.log(xhr);
                alert("Form yüklenirken hata oluştu. Console (F12) kontrol edin.");
            }
        });
    });
    
    // 2. MARKA DÜZENLEME BUTONU (Tablo içinde olduğu için document.on ZORUNLUDUR)
    $(document).on('click', '.markaDuzenleBtn', function() {
        var id = $(this).data('id');
        
        // Laravel route'unu JS içinde düzgün oluşturmak için:
        var url = "{{ route('super.admin.markalar.edit', ':id') }}";
        ...
        $.ajax({
            ...
            error: function(xhr) {
                console.log(xhr);
                alert("Düzenleme formu yüklenemedi. ID: " + id);
            }
        });
    });
    
    // 3. MARKA SİLME İŞLEMİ
    $(document).on('click', '.markaSil', function() {
        if(confirm("Silmek istediğinizden emin misiniz?")) {
            var id = $(this).data('id');
            var url = "{{ route('super.admin.markalar.destroy', ':id') }}";
            url = url.replace(':id', id);

            $.ajax({
                url: url,
                method: "POST", // Laravel delete metodu genellikle DELETE ister ama POST + _method ile de olur
                data: {
                    _token: "{{ csrf_token() }}",
                    _method: "DELETE" // Eğer route resource ise DELETE methodu gerekir
                },
                success: function(data) {
                    alert(data.message);
                    location.reload();
