...
function toggleUnifiedDetails() {
    ...
    if (isVisible) {
        ...
        // Detayları açarken depolama bilgilerini yükle
        loadStorageDetailsForUnified();
    }
}
...
function loadStorageDetailsForUnified() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                // Servis fotoğrafları
                const serviceCount = response.details.service_photos?.count || 0;
                document.getElementById('servicePhotosCount').textContent = `${serviceCount} dosya`;
                
                // Stok fotoğrafları
                const stockCount = response.details.stock_photos?.count || 0;
                document.getElementById('stockPhotosCount').textContent = `${stockCount} dosya`;
                
                // Diğer dosyalar
                const otherDetails = response.details.other_files?.breakdown || {};
