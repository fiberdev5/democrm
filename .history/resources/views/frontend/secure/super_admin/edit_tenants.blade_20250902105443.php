      ...
      <div class="card-footer bg-light d-flex justify-content-between align-items-center p-3">
        ...
        <div>
          ...
          <form action="{{ route('super.admin.tenant.toggle.status', [$tenant->id,$tenant->id]) }}" method="POST" style="display: inline;">
            ...
            <button type="submit" class="btn btn-danger btn-sm toggle-status">Pasif Yap</button>
        @else
            <!-- Firma pasifse aktif et butonu -->
            <button type="submit" class="btn btn-success btn-sm toggle-status">Aktif Et</button>
        @endif
          </form>
...
$(document).on('click', '.toggle-status', function () {
    let button = $(this);
    let tenantId = button.data('id');

    $.ajax({
        url: '/super-admin/tenants/' + tenantId + '/toggle-status',
        type: 'POST',
        data: {
            _token: '{{ csrf_token() }}'
        },
