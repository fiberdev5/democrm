...
<div class="card-body p-3">
  ...
  <div class="subscription-card mt-3">
    ...
    @if($isTrialFromTenant || ($currentSubscription && $currentSubscription->plansubs))
          ...
          <div class="subscription-info-item">
            ...
            <div class="subscription-value">
              ...
              @elseif($currentSubscription->status === 'trial')
                <span class="status-badge status-trial">Deneme</span>
              @else
                <span class="status-badge status-other">{{ ucfirst($currentSubscription->status) }}</span>
              @endif
            </div>
          </div>
          <div class="subscription-info-item">
            ...
            <div class="subscription-value">
              ...
              @else
                <span class="price-amount">{{ $currentSubscription->plansubs->getFormattedPrice() }}</span>
              @endif
            </div>
          </div>
          <div class="subscription-info-item">
      ...
      <div class="package-features">
        <div class="features-grid">
          ...
          @if(!$isTrialFromTenant && isset($currentSubscription->plansubs->limits['storage_gb']))
            <div class="feature-item storage-feature">
              <div class="feature-label">Depolama</div>
              <div class="feature-value">
                @if($currentSubscription->plansubs->limits['storage_gb'] == -1)
                  <div class="storage-info">
                    <span class="storage-usage">{{ $storageInfo['current_usage_formatted'] ?? '0 B' }}</span>
                    <span class="storage-limit">/ Sınırsız</span>
                  </div>
                ...
                @else
                  ...
                  <div class="storage-info">
                    <span class="storage-usage">{{ $storageInfo['current_usage_formatted'] ?? '0 B' }}</span>
                    <span class="storage-limit">/ {{ number_format($totalGB, 0) }} GB</span>
                    @if($extraStorage > 0)
      ...
      <!-- Depolama Detayları Butonu -->
      @if(isset($storageInfo))
      <div class="storage-details-section">
        <button class="storage-details-btn" onclick="showStorageDetails()">
                  ...
                  <tr>
                    ...
                    <td>
                      @if($subscription->status === 'active')
                        <span class="history-status status-active">Aktif</span>
                      @elseif($subscription->status === 'trial')
                        <span class="history-status status-trial">Deneme</span>
                      @elseif($subscription->status === 'expired')
                        <span class="history-status status-expired">Süresi Dolmuş</span>
                      @else
                        <span class="history-status status-other">{{ ucfirst($subscription->status) }}</span>
                      @endif
                    </td>
