...
<style>
/* Modal tablo düzeltmeleri */
.modal-lg {
    max-width: 90% !important; /* Modal genişliğini artır */
}

/* Tablo hizalama düzeltmeleri */
.tenant-users-table {
    margin: 0;
    width: 100%;
...
/* Sütun genişlikleri */
.tenant-users-table .col-user {
    width: 35%;
}

.tenant-users-table .col-roles {
    width: 25%;
    text-align: center;
}

.tenant-users-table .col-status {
    width: 20%;
    text-align: center;
}

.tenant-users-table .col-actions {
    width: 20%;
    text-align: center;
}
...
<div class="modal fade" id="tenantUsersModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body p-0" id="tenantUsersContent">
...
<div class="modal fade" id="impersonationModal" tabindex="-1" aria-hidden="true" style="background: rgba(0, 0, 0, 0.7); padding-top:100px;">
    ...
    function renderTenantUsers(users, tenantId) {
        ...
        var html = `
            <div class="table-responsive">
                <table class="table tenant-users-table mb-0">
                    <thead>
                        <tr>
        ...
        users.forEach(function(user) {
            var statusBadge = user.is_active ? 
                '<span class="badge bg-success status-badge"><i class="mdi mdi-check-circle me-1"></i>Aktif</span>' : 
                '<span class="badge bg-secondary status-badge"><i class="mdi mdi-pause-circle me-1"></i>Pasif</span>';
            ...
            if (user.can_be_impersonated && user.is_active) {
                actions = `<button class="btn btn-outline-danger btn-sm impersonate-btn impersonate-user-btn" 
                                  data-user-id="${user.user_id}" 
                                  data-user-name="${user.name}"
                                  data-company-name="${$('#modalTenantName').text()}"
                                  title="Bu kullanıcı olarak giriş yap">
                              <i class="fas fa-user-secret me-1"></i>Giriş
                          </button>`;
            } else {
            ...
            if (user.ayrilma_tarihi) {
                userInfo += `<div class="user-exit-date">Çıkış: ${user.ayrilma_tarihi}</div>`;
            }
            userInfo += '</div>';

            html += `<tr>
                <td class="col-user">${userInfo}</td>
                <td class="col-roles">${rolesBadges || '<span class="text-muted">Rol yok</span>'}</td>
        ...
        html += `<div class="mt-3 mx-3 mb-3 p-3 bg-light rounded">
            <div class="row text-center">
                <div class="col-4">
                    <div class="fw-bold text-primary fs-4">${users.length}</div>
                    <small class="text-muted">Toplam Kullanıcı</small>
                </div>
                <div class="col-4">
                    <div class="fw-bold text-success fs-4">${activeCount}</div>
                    <small class="text-muted">Aktif Kullanıcı</small>
                </div>
                <div class="col-4">
                    <div class="fw-bold text-danger fs-4">${impersonatableCount}</div>
