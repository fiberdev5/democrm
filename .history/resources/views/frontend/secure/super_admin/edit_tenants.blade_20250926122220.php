...
<div class="storage-section">
    <div class="section-header">
        ...
        @php
            // Mevcut $storageInfo array'inde bulunan verileri kullan
            $storageUsagePercent = isset($storageInfo['usage_percentage']) ? floatval($storageInfo['usage_percentage']) : 0;
            
            // Eğer usage_percentage 0 ise, manuel hesapla
            if ($storageUsagePercent <= 0) {
                ...
                preg_match('/([0-9,\.]+)\s*([A-Z]+)/', $usageFormatted, $usageMatches);
                preg_match('/([0-9,\.]+)\s*([A-Z]+)/', $limitFormatted, $limitMatches);
                
                if (!empty($usageMatches) && !empty($limitMatches)) {
                    $usageValue = floatval(str_replace(',', '', $usageMatches[1]));
                    $usageUnit = strtoupper($usageMatches[2]);
                    $limitValue = floatval(str_replace(',', '', $limitMatches[1]));
                    $limitUnit = strtoupper($limitMatches[2]);
                    
                    // Byte cinsine çevir - inline olarak
                    $usageBytes = $usageValue;
                    switch ($usageUnit) {
                        case 'KB': $usageBytes = $usageValue * 1024; break;
                        case 'MB': $usageBytes = $usageValue * 1024 * 1024; break;
                        case 'GB': $usageBytes = $usageValue * 1024 * 1024 * 1024; break;
                        case 'TB': $usageBytes = $usageValue * 1024 * 1024 * 1024 * 1024; break;
                    }
                    
                    $limitBytes = $limitValue;
                    switch ($limitUnit) {
                        case 'KB': $limitBytes = $limitValue * 1024; break;
                        case 'MB': $limitBytes = $limitValue * 1024 * 1024; break;
                        case 'GB': $limitBytes = $limitValue * 1024 * 1024 * 1024; break;
                        case 'TB': $limitBytes = $limitValue * 1024 * 1024 * 1024 * 1024; break;
                    }
                    
                    if ($limitBytes > 0) {
                        $storageUsagePercent = ($usageBytes / $limitBytes) * 100;
                        $storageUsagePercent = min(100, round($storageUsagePercent, 1));
                    }
                }
            }
            
            // Görüntüleme için formatted değerleri kullan
    ...
    <div class="storage-progress-compact">
        <div class="storage-stats">
            <div class="stat">
                <span class="value">{{ number_format($storageUsagePercent, 1) }}%</span>
