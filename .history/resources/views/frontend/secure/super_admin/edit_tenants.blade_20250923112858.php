...
<div class="tab-pane fade" id="service-stats">
  <div class="service-stats-container">
    <!-- Toplam Servis Göstergesi -->
    <div class="service-total-display">
      <div class="service-total-label">Toplam Servis</div>
      <div class="service-total-value">{{ $topServisSayisi ?? 0 }}</div>
    </div>

    <!-- Period Navigation -->
    <div class="service-period-navigation">
      @foreach($periodStats as $key => $period)
        <button class="service-period-btn {{ $key === 'bugun' ? 'period-active' : '' }}" 
                data-period="{{ $key }}" 
                data-target="#period-{{ $key }}">
          <span class="service-period-label">{{ $period['label'] }}</span>
          <span class="service-period-count">{{ $period['toplam'] }}</span>
        </button>
      @endforeach
    </div>
    ...
    <div class="service-stats-grid">
      @foreach($periodStats as $key => $period)
        <div class="service-period-content {{ $key === 'bugun' ? 'active' : '' }}" id="period-{{ $key }}">
          <!-- Markalar -->
          <div class="service-stat-panel">
            <div class="service-stat-header">
              <h6 class="service-stat-title">
            ...
            <div class="service-stat-content">
              @forelse($period['markalar'] as $marka)
                <div class="service-stat-item">
                  <span class="service-stat-label">{{ $marka->marka }}</span>
                  <span class="service-stat-badge">{{ $marka->sayi }}</span>
                </div>
              @empty
                <div class="service-empty-state">
                  <i class="fas fa-tags service-empty-icon"></i>
                  <div>Kayıt yok</div>
                </div>
              @endforelse
            </div>
          ...
          <div class="service-stat-panel">
            ...
            <div class="service-stat-content">
              @forelse($period['turler'] as $tur)
                <div class="service-stat-item">
                  <span class="service-stat-label">{{ $tur->cihaz }}</span>
                  <span class="service-stat-badge">{{ $tur->sayi }}</span>
                </div>
              @empty
                <div class="service-empty-state">
                  <i class="fas fa-mobile-alt service-empty-icon"></i>
                  <div>Kayıt yok</div>
                </div>
              @endforelse
            </div>
          ...
          <div class="service-stat-panel">
            ...
            <div class="service-stat-content">
              @forelse($period['kaynaklar'] as $kaynak)
                <div class="service-stat-item">
                  <span class="service-stat-label">{{ $kaynak->kaynak }}</span>
                  <span class="service-stat-badge">{{ $kaynak->sayi }}</span>
                </div>
              @empty
                <div class="service-empty-state">
                  <i class="fas fa-route service-empty-icon"></i>
                  <div>Kayıt yok</div>
                </div>
              @endforelse
            </div>
          ...
          <div class="service-stat-panel">
            ...
            <div class="service-stat-content">
              @forelse($period['operatorler'] as $operator)
                <div class="service-stat-item">
                  <span class="service-stat-label">{{ $operator->name }}</span>
                  <span class="service-stat-badge">{{ $operator->sayi }}</span>
                </div>
              @empty
                <div class="service-empty-state">
                  <i class="fas fa-users service-empty-icon"></i>
                  <div>Kayıt yok</div>
                </div>
              @endforelse
            </div>
...
// Servis istatistikleri period switching - mevcut kodunuzun yerine
$(document).off('click', '.service-period-btn').on('click', '.service-period-btn', function(e) {
  ...
  // Button navigation
  $('.service-period-btn').removeClass('period-active');
  $this.addClass('period-active');
  
  // Content switching
  $('.service-period-content').removeClass('active').hide();
  $(`#period-${period}`).addClass('active').show();
  
  // Smooth transition effect
  $('.service-stat-panel').css('opacity', '0.6');
  setTimeout(function() {
    $('.service-stat-panel').css('opacity', '1');
  }, 200);
});
...
@media (max-width: 768px) {
  ...
  /* Servis İstatistikleri Özel Stiller */
        ...
        .service-period-btn:hover::before {
            left: 100%;
        }

        .service-period-btn.period-active {
        ...
        .service-stat-content::-webkit-scrollbar {
            width: 6px;
        }

        .service-stat-content::-webkit-scrollbar-track {
            background: #f8f9fa;
        }

        .service-stat-content::-webkit-scrollbar-thumb {
            background: linear-gradient(135deg, #667eea, #764ba2);
            border-radius: 3px;
        }
