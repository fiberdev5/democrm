    ...
    <div class="modal-content">
      ...
      <div class="modal-body" id="paymentDetailContent">
        ...
        <div id="paymentDetailError" style="display: none;" class="text-center py-4">
          ...
          <button class="btn btn-outline-primary" onclick="retryLoadPaymentDetail()">
            <i class="fas fa-sync-alt me-1"></i>Tekrar Dene
          </button>
        </div>
      </div>
...
window.PaymentModule = (function() {
  ...
  function loadPaymentInfo(tenantId) {
    ...
    fetch(`/super-admin/tenant/${tenantId}/payments`, {
      method: 'GET',
      headers: {
    ...
    .then(data => {
      if (data.success) {
        ...
        console.error('API yanıt hatası:', data.message);
        showPaymentError();
      }
    })
    .catch(error => {
  ...
  // Ödeme detayını göster - Ana fonksiyon
  function showPaymentDetail(type, paymentId) {
    console.log('Ödeme detayı isteniyor:', type, paymentId, 'Tenant ID:', getCurrentTenantId());
    
    // Tenant ID'yi al
    const tenantId = getCurrentTenantId();
    
    if (!tenantId || !paymentId) {
      console.error('Geçersiz parametreler - Tenant ID:', tenantId, 'Payment ID:', paymentId);
      showPaymentDetailError('Geçersiz parametreler');
      return;
    }
    ...
    showPaymentDetailLoading();
    
    // AJAX ile detayları getir
    fetchPaymentDetail(tenantId, type, paymentId);
  }

  // AJAX ile ödeme detayını getir
  function fetchPaymentDetail(tenantId, type, paymentId) {
    // CSRF token'ı al
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
    
    fetch(`/super-admin/tenant/${tenantId}/payment-detail/${type}/${paymentId}`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest',
        'X-CSRF-TOKEN': csrfToken,
    ...
    .then(data => {
      console.log('Ödeme detayı başarılı:', data);
      
      if (data.success && data.payment) {
        displayPaymentDetail(data.payment);
      } else {
        showPaymentDetailError(data.message || 'Ödeme detayı alınamadı');
      }
    })
    .catch(error => {
      console.error('Ödeme detayı hatası:', error);
      showPaymentDetailError('Ödeme detayı yüklenirken bir hata oluştu: ' + error.message);
    });
  }

  // Tenant ID'yi al - Güvenilir yöntem
  function getCurrentTenantId() {
    ...
    // Modal'dan al
    const modalTenantId = document.getElementById('current-tenant-id')?.value;
    if (modalTenantId && modalTenantId !== 'null' && modalTenantId !== '') {
      currentTenantId = modalTenantId;
      return currentTenantId;
    }
    
    // URL'den al (eğer edit sayfasındaysak)
    const urlMatch = window.location.pathname.match(/\/tenant\/(\d+)/);
    if (urlMatch && urlMatch[1]) {
      currentTenantId = urlMatch[1];
      return currentTenantId;
    }
    
    console.warn('Tenant ID bulunamadı');
    return null;
  }

  // UI durumları - Payment Loading
  ...
  // UI durumları - Payment Detail Modal
  function showPaymentDetailLoading() {
    const loading = document.getElementById('paymentDetailLoading');
    const data = document.getElementById('paymentDetailData');
    const error = document.getElementById('paymentDetailError');
  ...
  function showPaymentDetailError(message = 'Bir hata oluştu') {
    const loading = document.getElementById('paymentDetailLoading');
    const data = document.getElementById('paymentDetailData');
    const error = document.getElementById('paymentDetailError');
  ...
  function displayPaymentDetail(payment) {
    ...
    const dataDiv = document.getElementById('paymentDetailData');
    
    if (dataDiv) {
      dataDiv.innerHTML = detailHtml;
    }
  ...
  function updatePaymentSummary(summary) {
    const elements = {
      ...
      'summary-refunded': summary.refunded || '0'
    };
  ...
  function createActionButtons(payment) {
    ...
    if (payment.invoice_path) {
        ...
        buttons += `<a href="${invoPath}" class="btn btn-outline-primary btn-sm me-1" title="Faturayı Görüntüle" target="_blank">
                      <i class="fas fa-file-pdf"></i>
                    </a>`;
    }
    
    // Detay butonu - düzeltildi
  ...
  function getTypeBadge(type, label) {
    ...
    return `<span class="badge bg-secondary bg-opacity-10 text-secondary">${label || type}</span>`;
  }
  ...
  function setCurrentTenantId(tenantId) {
    ...
    console.log('Tenant ID set edildi:', tenantId);
  }
  
  // Tekrar deneme fonksiyonu
  function retryLoadPaymentDetail() {
    if (currentPaymentType && currentPaymentId) {
      showPaymentDetail(currentPaymentType, currentPaymentId);
    } else {
      showPaymentDetailError('Tekrar denemek için gerekli bilgiler mevcut değil');
    }
  }
  ...
  function initEventHandlers() {
    ...
    // Modal açıldığında tenant ID'yi al
    document.addEventListener('show.bs.modal', function(event) {
      if (event.target.id === 'editTenantModal') {
        const button = event.relatedTarget;
        if (button) {
          const tenantId = button.getAttribute('data-bs-id') || button.getAttribute('data-id');
          if (tenantId && tenantId !== 'null' && tenantId !== null) {
            setCurrentTenantId(tenantId);
            const hiddenInput = document.getElementById('current-tenant-id');
            if (hiddenInput) {
    ...
    document.addEventListener('shown.bs.tab', function(event) {
      if (event.target.id === 'payment-info-tab') {
        ...
        if (tenantId && tenantId !== 'null' && tenantId !== null) {
          ...
          console.error('Tenant ID hala geçersiz:', tenantId);
          showPaymentError();
        }
      }
    });
    ...
    document.addEventListener('change', function(event) {
      if (event.target.id === 'status-filter' || event.target.id === 'type-filter') {
        applyFilters();
      }
    });
    ...
    document.addEventListener('click', function(event) {
      if (event.target.matches('.nav-link[data-period]') || event.target.closest('.nav-link[data-period]')) {
        event.preventDefault();
        
        const button = event.target.closest('.nav-link[data-period]') || event.target;
        const targetSelector = button.getAttribute('data-target');
        
        if (targetSelector) {
          // Tab navigation
          document.querySelectorAll('.nav-link[data-period]').forEach(link => {
            link.classList.remove('active');
          });
          button.classList.add('active');
          
          // Tab content
          document.querySelectorAll('.tab-pane[id^="period-"]').forEach(pane => {
  ...
  return {
    ...
    retryLoadPaymentDetail: retryLoadPaymentDetail,
...
window.showPaymentDetail = function(type, paymentId) {
  PaymentModule.showPaymentDetail(type, paymentId);
};

window.retryLoadPaymentDetail = function() {
  PaymentModule.retryLoadPaymentDetail();
};

// Document ready event
document.addEventListener('DOMContentLoaded', function() {
  PaymentModule.init();
  console.log('PaymentModule hazır ve kullanıma uygun');
});
</script>
