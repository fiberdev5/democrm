        ...
        <div class="card">
          <div class="card-header sayfaBaslik d-flex justify-content-between align-items-center">
            <div>
              ...
              Super Admin - Tüm Firmalar
            </div>
            <div class="badge bg-warning">
              <i class="fas fa-crown me-1"></i>Super Admin Modu
            </div>
          </div>
          <div class="card-body">
            <table id="datatableTenants" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
              ...
              <thead class="title">
                <tr>
                  ...
                  <th data-priority="2">Firma Adı</th>
                  <th>Telefon</th>
                  <th>Adres</th>
                  <th>Durum</th>
                  <th data-priority="1" style="width: 120px;">İşlemler</th>
                </tr>
              </thead>
              <tbody>
...
$(document).ready(function () {
  var table = $('#datatableTenants').DataTable({
      ...
      ajax: {
        ...
        data: function(data) {
          ...
          data.status = $('#tenantStatus').val(); // Changed from 'tip' to 'status'
          data.il = $('#countrySelect').val();
          data.ilce = $('#citySelect').val();
        }
      },
      'columns': [
        { data: 'id', name: 'id' }, // Added name for server-side sorting
        { data: 'name', name: 'firma_adi' }, // Corrected name for sorting
        { data: 'tel', name: 'tel1' }, // Corrected name for sorting
        { data: 'address', name: 'adres' }, // Corrected name for sorting
        { data: 'durum', name: 'status' }, // Corrected name for sorting
  ...
  // Changed from '#musteriTipi' to '#tenantStatus'
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '.impersonate-tenant-owner', function() {
        ...
        $('#confirmImpersonationBtn').off('click').on('click', function() {
            var reason = $('#impersonationReason').val() || 'Super Admin tarafından firma yönetimi';
    ...
    $(document).on('click', '.impersonate-user-btn', function() {
        ...
        $('#confirmImpersonationBtn').off('click').on('click', function() {
            var reason = $('#impersonationReason').val() || 'Super Admin tarafından kullanıcı desteği';
    ...
    function renderTenantUsers(users, tenantId) {
        ...
        users.forEach(function(user) {
            var statusBadge = user.is_active ? 
                ...
                '<span class="badge bg-secondary"><i class="mdi mdi-pause-circle me-1"></i>Pasif</span>';
            ...
            if (user.can_be_impersonated && user.is_active) {
                actions = `<button class="btn btn-warning btn-sm impersonate-user-btn" 
        ...
        html += `<div class="mt-3 p-3 bg-light rounded">
            <div class="row text-center">
                ...
                <div class="col-4">
                    <div class="fw-bold text-success">${activeCount}</div>
                    <small class="text-muted">Aktif Kullanıcı</small>
                </div>
                <div class="col-4">
                    <div class="fw-bold text-warning">${impersonatableCount}</div>
                    <small class="text-muted">İmpersonate Edilebilir</small>
                </div>
            </div>
        </div>`;
