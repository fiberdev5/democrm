  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.success) {
        ...
        // Toplam
        // Toplam - yüzde bilgisiyle birlikte
let totalSize = response.storage_info.current_usage_formatted || '0 B';
let limitSize = response.storage_info.limit_formatted || '0 GB';
let usagePercent = 0;

// Yüzde hesapla (basit version)
if (response.storage_info.usage_percentage && response.storage_info.usage_percentage > 0.001) {
  usagePercent = response.storage_info.usage_percentage.toFixed(2);
} else {
  // Manuel hesapla - basit MB/GB örneği için
  if (totalSize.includes('MB') && limitSize.includes('GB')) {
    const usageMB = parseFloat(totalSize.replace(' MB', ''));
    const limitGB = parseFloat(limitSize.replace(' GB', ''));
    usagePercent = ((usageMB / 1024) / limitGB * 100).toFixed(2);
  }
}
...
storageHtml += `
  <div class="storage-item total">
    ...
    <div class="storage-size">${totalSize} / ${limitSize} (${usagePercent}%)</div>
  </div>
</div>`;
