...
$(document).off('shown.bs.tab', '#payment-info-tab').on('shown.bs.tab', '#payment-info-tab', function () {
  // Farklı yollarla tenant ID'yi bulmaya çalış
  let tenantId = null;
  
  // 1. Modal'daki aktif button'dan al
  const activeButton = $(this);
  tenantId = activeButton.closest('.modal').find('[data-bs-id]').first().data('bs-id');
  
  // 2. Eğer bulunamadıysa, modal content'ten al
  if (!tenantId) {
    tenantId = $('#editTenantModal').find('[data-bs-id]').first().data('bs-id');
  }
  
  // 3. Son çare olarak global değişkenden al
  if (!tenantId) {
    tenantId = window.currentTenantId;
  }
  
  console.log('Bulunan tenant ID:', tenantId);
  
  if (tenantId && tenantId !== PaymentModule.currentTenantId) {
    PaymentModule.currentTenantId = tenantId;
    PaymentModule.loadPaymentInfo(tenantId);
  } else if (!tenantId) {
    console.error('Tenant ID bulunamadı!');
  }
});

// Modal açıldığında tenant ID'yi sakla
$(document).off('shown.bs.modal', '#editTenantModal').on('shown.bs.modal', '#editTenantModal', function() {
  const tenantId = $('.editTenant').data('bs-id');
  console.log('Modal açıldı, tenant ID:', tenantId);
  
  if (tenantId) {
    window.currentTenantId = tenantId;
    PaymentModule.currentTenantId = tenantId;
    
    // Eğer payment tab aktifse hemen yükle
