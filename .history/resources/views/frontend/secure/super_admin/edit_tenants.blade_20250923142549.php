  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.success && response.details) {
        ...
        // Toplam
        let totalSize = '0 B';
        if (response.details.service_photos && response.details.stock_photos && response.details.other_files) {
          // Toplam hesaplama gerekirse backend'den gelsin
          totalSize = response.total_size_formatted || '0 B';
        }
...
// Sayfa yüklendiğinde depolama bilgilerini yükle
$(document).ready(function() {
  // Kısa bir gecikme ile yükle (DOM hazır olsun diye)
  setTimeout(function() {
    loadStorageDetails();
  }, 500);
});

// Modal açıldığında da yükle (tenant ID değişmiş olabilir)
