  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.success) {
        ...
        // Toplam - PHP kodundaki mantığı kullan
        let totalSize = response.storage_info.current_usage_formatted || '0 B';
        let totalGB = response.storage_info.total_limit_gb || 0;
        ...
        if (totalGB === -1) {
          ...
          limitText = `${Math.floor(totalGB)} GB`;
          if (extraStorage > 0) {
            limitText += ` (+${Math.floor(extraStorage)} GB ek)`;
          }
        }
        ...
        storageHtml += `
          <div class="storage-item total">
            ...
            <div class="storage-size">${totalSize} / ${limitText}</div>
          </div>
        </div>`;
