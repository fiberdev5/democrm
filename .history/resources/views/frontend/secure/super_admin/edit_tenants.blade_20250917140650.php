...
<div class="card-header bg-primary text-white p-3">
  <div class="d-flex align-items-start justify-content-between">
    <div class="d-flex flex-column">
      <h4 class="mb-2">{{ $tenant->firma_adi }}</h4>
      @if($tenant->status == 1)
    ...
    <div class="d-flex align-items-center gap-2">
      <form action="{{ route('super.admin.tenant.toggle.status', [$tenant->id,$tenant->id]) }}" method="POST" style="display: inline;">
        @csrf
        @if($tenant->status == 1)
          <button type="submit" class="btn btn-outline-light btn-sm">Pasif Yap</button>
        @else
          <button type="submit" class="btn btn-outline-light btn-sm">Aktif Et</button>
        @endif
      </form>
...
<!-- Footer -->
<div class="card-footer bg-light d-flex justify-content-center align-items-center p-3">
  <div class="text-center">
    <small class="text-muted d-block">Toplam Servis Sayısı</small>
    <div class="fw-bold text-primary h5 mb-0">{{ $topServisSayisi ?? 0 }}</div>
  </div>
</div>
