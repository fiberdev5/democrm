    ...
    @if($isTrialFromTenant || ($currentSubscription && $currentSubscription->plansubs))
      <div class="subscription-content">
        <div class="row">
          ...
          <!-- Sağ Taraf: Depolama Detayları -->
          @if(isset($storageInfo))
          <div class="col-md-4">
            <div class="storage-details-card">
              <h6 class="storage-card-title">Depolama Detayları</h6>
              ...
              <!-- Statik fallback (eğer AJAX çalışmazsa) -->
              <div class="storage-breakdown-list d-none" id="static-storage-info">
                @if(isset($storageInfo['current_usage_formatted']))
                  <div class="storage-item total">
                    <div class="storage-type">Toplam Kullanım</div>
                    <div class="storage-size">{{ $storageInfo['current_usage_formatted'] }}</div>
                  </div>
                @endif
              </div>
            </div>
...
// PaymentModule içindeki loadStorageDetails fonksiyonunu global yap
function loadStorageDetails() {
  ...
  $.ajax({
    ...
    method: 'GET',
    success: function(response) {
      console.log('Storage Response:', response); // Debug için
      
      if (response.success && response.details) {
        let storageHtml = '<div class="storage-breakdown-list">';
        
        // Servis Fotoğrafları
        if (response.details.service_photos) {
          storageHtml += `
            <div class="storage-item">
              <div class="storage-type">Servis Fotoğrafları</div>
              <div class="storage-size">
                ${response.details.service_photos.total_size_formatted}
                <small class="d-block text-muted">${response.details.service_photos.count} adet</small>
              </div>
            </div>`;
        }
        
        // Stok Fotoğrafları
        if (response.details.stock_photos) {
          storageHtml += `
            <div class="storage-item">
              <div class="storage-type">Stok Fotoğrafları</div>
              <div class="storage-size">
                ${response.details.stock_photos.total_size_formatted}
                <small class="d-block text-muted">${response.details.stock_photos.count} adet</small>
              </div>
            </div>`;
        }
        
        // Diğer Dosyalar
        if (response.details.other_files) {
          storageHtml += `
            <div class="storage-item">
              <div class="storage-type">Diğer Dosyalar</div>
              <div class="storage-size">
                ${response.details.other_files.total_size_formatted}
                <small class="d-block text-muted">${response.details.other_files.total_count} adet</small>
              </div>
            </div>`;
        }
        
        // Toplam - Backend'den gelen toplam kullanımı kullan
        let totalSize = response.total_size_formatted || 
                       response.details.total_size_formatted || 
                       '0 B';
        ...
        console.log('No storage details found'); // Debug için
        container.html(`
          <div class="storage-breakdown-list">
            <div class="storage-item">
              <div class="storage-type">Depolama Bilgisi</div>
              <div class="storage-size">Bulunamadı</div>
            </div>
          </div>
        `);
      }
    ...
    error: function(xhr, status, error) {
      console.error('Storage AJAX Error:', xhr.responseText); // Debug için
      
      container.html(`
        <div class="text-center py-3">
          <small class="text-muted">Depolama bilgileri yüklenemedi</small>
          <button class="btn btn-link btn-sm p-0 d-block" onclick="loadStorageDetails()">
            <small>Tekrar dene</small>
          </button>
        </div>
      `);
      
      // Hata durumunda statik bilgiyi göster
      $('#static-storage-info').removeClass('d-none');
    }
  });
}

// Document ready event - depolama detaylarını otomatik yükle
...
// Modal her açıldığında depolama bilgilerini yenile
...
// Tab değiştiğinde de yükle
$('#customer-subscription-tab').on('shown.bs.tab', function() {
  setTimeout(function() {
    if ($('#storage-breakdown-container').length > 0) {
      loadStorageDetails();
    }
...
/* Subscription Details - Alt Alta Liste */
