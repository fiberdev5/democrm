...
<div class="tab-pane fade" id="service-stats">
  <div class="border rounded p-3 service-stats-container"> <!-- p-2 yerine p-3 ve yeni sınıf -->
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h6 class="mb-0 fw-semibold text-dark">Servis İstatistikleri</h6>
      <div class="text-end">
        <small class="text-muted d-block">Toplam Servis</small>
        <span class="fw-bold text-primary fs-5">{{ $topServisSayisi ?? 0 }}</span>
      </div>
    </div>

    <!-- Tab Navigation - Period Seçimi -->
    <div class="nav nav-pills period-nav mb-3" role="tablist"> <!-- period-nav sınıfı ekledik -->
      @foreach($periodStats as $key => $period)
        <button class="nav-link {{ $key === 'bugun' ? 'active' : '' }}" 
          ...
          <div class="text-center">
            <span class="d-block fw-medium">{{ $period['label'] }}</span>
            <span class="badge bg-light text-dark mt-1">{{ $period['toplam'] }}</span> <!-- Badge rengi değişti -->
          </div>
        </button>
      @endforeach
    </div>
    ...
    <div class="tab-content">
      @foreach($periodStats as $key => $period)
        <div class="tab-pane fade {{ $key === 'bugun' ? 'show active' : '' }}" 
             id="period-{{ $key }}">
          <div class="row g-2">
            <!-- Markalar -->
            <div class="col-lg-3 col-md-6">
              <div class="card h-100 shadow-sm border-0"> <!-- Daha sade kart -->
                <div class="card-header bg-light py-2"> <!-- py-1 yerine py-2 -->
                  <small class="fw-semibold text-dark">Markalar</small>
                </div>
                <div class="card-body p-2"> <!-- p-1 yerine p-2 -->
                  @forelse($period['markalar'] as $marka)
                    <div class="d-flex justify-content-between align-items-center py-1 border-bottom-dashed"> <!-- Yeni sınıf -->
                      <small class="text-muted">{{ $marka->marka }}</small>
                      <span class="badge bg-secondary-subtle text-secondary">{{ $marka->sayi }}</span> <!-- Yeni badge stili -->
                    </div>
                  @empty
                    <small class="text-muted fst-italic">Kayıt yok</small>
                  @endforelse
                </div>
              </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6">
              <div class="card h-100 shadow-sm border-0">
                <div class="card-header bg-light py-2">
                  <small class="fw-semibold text-dark">Cihaz Türleri</small>
                </div>
                <div class="card-body p-2">
                  @forelse($period['turler'] as $tur)
                    <div class="d-flex justify-content-between align-items-center py-1 border-bottom-dashed">
                      <small class="text-muted">{{ $tur->cihaz }}</small>
                      <span class="badge bg-secondary-subtle text-secondary">{{ $tur->sayi }}</span>
                    </div>
                  @empty
                    <small class="text-muted fst-italic">Kayıt yok</small>
                  @endforelse
            ...
            <div class="col-lg-3 col-md-6">
              <div class="card h-100 shadow-sm border-0">
                <div class="card-header bg-light py-2">
                  <small class="fw-semibold text-dark">Kaynaklar</small>
                </div>
                <div class="card-body p-2">
                  @forelse($period['kaynaklar'] as $kaynak)
                    <div class="d-flex justify-content-between align-items-center py-1 border-bottom-dashed">
                      <small class="text-muted">{{ $kaynak->kaynak }}</small>
                      <span class="badge bg-secondary-subtle text-secondary">{{ $kaynak->sayi }}</span>
                    </div>
                  @empty
                    <small class="text-muted fst-italic">Kayıt yok</small>
                  @endforelse
            ...
            <div class="col-lg-3 col-md-6">
              <div class="card h-100 shadow-sm border-0">
                <div class="card-header bg-light py-2">
                  <small class="fw-semibold text-dark">Personeller</small>
                </div>
                <div class="card-body p-2">
                  @forelse($period['operatorler'] as $operator)
                    <div class="d-flex justify-content-between align-items-center py-1 border-bottom-dashed">
                      <small class="text-muted">{{ $operator->name }}</small>
                      <span class="badge bg-secondary-subtle text-secondary">{{ $operator->sayi }}</span>
                    </div>
                  @empty
                    <small class="text-muted fst-italic">Kayıt yok</small>
                  @endforelse
