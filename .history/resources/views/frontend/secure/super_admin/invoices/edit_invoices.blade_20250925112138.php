...
<script>
// Mevcut form submit kodunu bu şekilde değiştirin
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data === false) {
        ...
        // Başarı mesajını modal içinde göster (modal kapatmadan)
        var successHtml = '<div class="alert alert-success alert-dismissible fade show" style="margin-bottom: 15px;">' +
                         '<strong>Başarılı:</strong> Fatura başarıyla güncellendi!' +
                         '<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
        
        // Eğer önceki mesaj varsa kaldır
        $('#editInvo .alert').remove();
        
        // Form'un en üstüne başarı mesajını ekle
        $('#editInvo').prepend(successHtml);
        ...
        // Modal KAPANMASIN - bu satırları kaldırdık:
        // $('#editInvoiceModal').modal('hide');
      }
    },
    error: function(xhr, status, error) {
      // Hata mesajını modal içinde göster
      var errorHtml = '<div class="alert alert-danger alert-dismissible fade show" style="margin-bottom: 15px;">' +
                     '<strong>Hata:</strong> Güncelleme başarısız! ' + error +
                     '<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
      
      // Eğer önceki mesaj varsa kaldır
      $('#editInvo .alert').remove();
      
      // Form'un en üstüne hata mesajını ekle
      $('#editInvo').prepend(errorHtml);
