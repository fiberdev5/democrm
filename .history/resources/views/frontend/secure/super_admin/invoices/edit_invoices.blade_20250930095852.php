            ...
            <div class="col-md-8 rw2">
              @if($invoice_id->faturaPdf != null)
                ...
                <div style="margin-bottom: 5px;">
                  <a href="{{asset($invoice_id->faturaPdf)}}" target="_blank" style="color: #28a745; text-decoration: none;">
                    <i class="fas fa-file-pdf"></i> Yüklü Dosya
                  </a>
                  <button type="button" class="btn btn-outline-danger btn-sm eArsivSil" data-id="{{$invoice_id->id}}" style="margin-left: 10px;">
              ...
              <!-- Her zaman göster -->
              <input type="file" class="form-control" name="pdf" id="pdfFileInput" accept=".pdf,.jpg,.jpeg,.png" {{$invoice_id->faturaPdf == null ? 'required' : ''}}>
              <small class="text-muted">PDF, JPG, JPEG veya PNG formatında yükleyebilirsiniz</small>
            </div>
          </div>
        </div>
      </div>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data && data.status === 'success') {
                    ...
                    // Butonları gizle ve input göster
                    $('.btnWrap').html(
                        '<input type="file" class="form-control" name="pdf" id="pdfFileInput" accept=".pdf,.jpg,.jpeg,.png" required>'
                    );
...
$(document).ready(function() {
    ...
    $('#pdfFileInput').on('change', function() {
        ...
        if (fileInput.files.length > 0) {
            ...
            uploadBtn.removeClass('btn-outline-success').addClass('btn-secondary');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Sil butonunu göster
                        if ($('.eArsivSil').length === 0) {
                            $('.btnWrap').append(
                                '<a href="" class="btn btn-outline-danger btn-sm mobilBtn mbuton1 eArsivSil" data-id="' + invoiceId + '" title="Sil">' +
    ...
    function resetUploadButton() {
        var uploadBtn = $('#uploadPdfBtn');
        uploadBtn.html('<i class="fas fa-upload"></i> Dosya Seç');
        uploadBtn.prop('disabled', false);
        uploadBtn.removeClass('btn-secondary').addClass('btn-outline-success');
    }
    
    // Buton durumlarını güncelleme fonksiyonu
    function updateButtonStates() {
        // Sayfa yüklendiğinde gerekirse burada kontroller yapılabilir
    }
});
</script>
