...
$('#bekleyenOdemelerBtn').on('click', function() {
    ...
    $.ajax({
        ...
        success: function(firmalar) {
            ...
            $('#bekleyenOdemelerBtn').html('<i class="fas fa-filter"></i> Bekleyen');
            
            if (firmalar.length === 0) {
                firmaListesiGoster([]);
                const liste = $('#firmaListesi');
                liste.html('<li class="list-group-item" style="background: #fff3cd; color: #856404;">Faturası oluşturulmamış ödemesi olan firma bulunamadı</li>');
                liste.show();
            } else {
