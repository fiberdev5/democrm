...
$(document).ready(function(){
    $('#editInvo').on('click', '.invoic_e', function(e){
        ...
        $.ajax({
            ...
            console.log(data);
            if ($.trim(data) === "-1") {
                // Başarısız durumda sayfayı yenile
                window.location.reload(true);
            } else {
                // Başarılı durumda - Modal açık kalsın, sadece mesaj göster
                alert("E-Arşiv belgesi başarıyla güncellendi");
                ...
                // Modal içeriğini yenile (isteğe bağlı)
                // $('#InvoiceModal .modal-body').html(data);
                
                // MODAL'I KAPATMA - Bu satırları kaldırdık/yorum yaptık
                // $('#InvoiceModal').modal('hide');
                // $('#editInvoiceModal').modal('hide');
            }
        }).fail(function(xhr, status, error) {
            console.error('E-Arşiv yükleme hatası:', error);
            alert("E-Arşiv yükleme başarısız: " + error);
            // Hata durumunda da modal açık kalabilir
        });
    });
});
</script>
