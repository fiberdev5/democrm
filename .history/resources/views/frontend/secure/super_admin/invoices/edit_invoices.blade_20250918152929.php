  ...
  <div class="card f2">
     <div class="card-header">
       <div style="display: flex; justify-content: space-between; align-items: center;">
         ...
         <button type="button" class="btn btn-sm btn-outline-primary musteriDuzenle">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="musteriDuzenleModalLabel">Müşteri Bilgilerini Düzenle</h5>
      ...
      <div class="modal-body">
        <form id="musteriDuzenleForm">
          <div class="row">
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="modalFirmaAdi" value="{{ $invoice_id->tenant->firma_adi }}" required>
              </div>
            </div>
            <div class="col-md-6">
              <div class="mb-3">
                <label class="form-label">Telefon</label>
                <input type="text" class="form-control" id="modalTel1" value="{{ $invoice_id->tenant->tel1 }}">
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="modalIl" value="{{ $invoice_id->tenant->il }}">
              </div>
            </div>
            <div class="col-md-6">
              <div class="mb-3">
                <label class="form-label">İlçe</label>
                <input type="text" class="form-control" id="modalIlce" value="{{ $invoice_id->tenant->ilce }}">
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="modalVergiNo" value="{{ $invoice_id->tenant->vergiNo }}">
              </div>
            </div>
            <div class="col-md-6">
              <div class="mb-3">
                <label class="form-label">Vergi Dairesi</label>
                <input type="text" class="form-control" id="modalVergiDairesi" value="{{ $invoice_id->tenant->vergiDairesi }}">
              </div>
            </div>
          </div>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control" rows="3" id="modalAdres">{{ $invoice_id->tenant->adres }}</textarea>
          </div>
        </form>
      </div>
...
$(document).ready(function() {
    // Modal açma eventi
    $('.musteriDuzenle').on('click', function(e) {
        e.preventDefault();
        console.log('Modal açılıyor...'); // Debug için
        $('#musteriDuzenleModal').modal('show');
    });

    // Müşteri kaydetme eventi
    $('.musteriKaydet').click(function() {
        var firmaAdi = $('#modalFirmaAdi').val();
        var tel1 = $('#modalTel1').val();
        var il = $('#modalIl').val();
        var ilce = $('#modalIlce').val();
        var vergiNo = $('#modalVergiNo').val();
        var vergiDairesi = $('#modalVergiDairesi').val();
        var adres = $('#modalAdres').val();
        ...
        // Ana formdaki görünümü güncelle
        $('#seciliMusteri').html(
            '<div style="display: flex; justify-content: space-between; align-items: flex-start;">' +
              '<div style="flex: 1;">' +
        ...
        $('input[name="vergiNo"]').val(vergiNo);
        $('input[name="vergiDairesi"]').val(vergiDairesi);
        $('input[name="tel1"]').val(tel1);
        $('input[name="il"]').val(il);
        $('input[name="ilce"]').val(ilce);
        $('textarea[name="adres"]').val(adres);
