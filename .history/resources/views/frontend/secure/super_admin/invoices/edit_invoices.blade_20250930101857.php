...
$('#editInvo').on('click', '.eArsivSil', function(e) {
    e.preventDefault();
    if (confirm("Bu e-faturayı silmek istediğinizden emin misiniz?")) {
            ...
            success: function(data) {
                if (data && data.status === 'success') {
                    ...
                    $('#fileUploadContainer').html(
                        ...
                        '<div id="fileSelectArea">' +
                        '<button type="button" class="btn btn-link p-0" id="selectFileBtn" ' +
                        'onclick="document.getElementById(\'pdfFileInput\').click()" ' +
                        'style="color: #6c757d; text-decoration: none; font-size: 13px;">' +
                        '<i class="fas fa-file-upload"></i> Dosya Seç</button></div>'
                    );
                    
                    // Event listener'ı yeniden ekle
                    $('#pdfFileInput').off('change').on('change', function() { /* yukarıdaki kod */ });
