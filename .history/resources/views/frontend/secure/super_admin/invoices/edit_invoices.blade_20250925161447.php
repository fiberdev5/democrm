...
<div class="row form-group" style="border:0">
  ...
  <div class="col-md-8 rw2">
    <div class="btnWrap">
      ...
      <a href="#" target="_blank" class="btn btn-outline-warning btn-sm mobilBtn mbuton1 d-none" id="viewPdfBtn" title="Görüntüle">
        <i class="far fa-eye"></i>
      </a>
      @else
      <a href="{{asset($invoice_id->faturaPdf)}}" target="_blank" class="btn btn-outline-warning btn-sm mobilBtn mbuton1" id="viewPdfBtn" title="Görüntüle">
        <i class="far fa-eye"></i>
      </a>
      @endif
      ...
      <div style="display: inline-block; position: relative;">
        ...
        <button type="button" 
                ...
                title="Yükle"
                @if($invoice_id->faturaPdf != null) disabled @endif>
          <i class="fas fa-upload"></i>
        </button>
      </div>
      
      <!-- Sil butonu -->
      <a href="" class="btn btn-outline-danger btn-sm mobilBtn mbuton1 eArsivSil" data-id="{{$invoice_id->id}}" title="Sil">
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data && data.status === 'success') {
                    ...
                    uploadBtn.html('<i class="fas fa-upload"></i> Yükle');
                    uploadBtn.removeClass('btn-uploaded').addClass('btn-upload-active');
...
$(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Görüntüle butonunu aktif et ve URL'yi güncelle
                        ...
                        // Upload butonunu "Yüklendi" durumuna getir
                        uploadBtn.html('<i class="fas fa-check"></i> Yüklendi');
                        uploadBtn.removeClass('btn-secondary').addClass('btn-uploaded');
                        uploadBtn.prop('disabled', true);
                        ...
                        // Başarısız durumda butonu eski haline getir
                        resetUploadButton();
                    }
                },
                error: function(xhr, status, error) {
    ...
    function resetUploadButton() {
        ...
        uploadBtn.html('<i class="fas fa-upload"></i> Yükle');
        uploadBtn.prop('disabled', false);
        uploadBtn.removeClass('btn-secondary btn-uploaded').addClass('btn-upload-active');
    }
    ...
    // Eğer PDF varsa upload butonunu devre dışı bırak
    if (!viewBtn.hasClass('d-none')) {
        uploadBtn.find('i').removeClass('fas fa-upload').addClass('fas fa-check');
        uploadBtn.removeClass('btn-outline-success').addClass('btn-uploaded');
        uploadBtn.prop('disabled', true);
    } else {
        uploadBtn.find('i').removeClass('fas fa-check').addClass('fas fa-upload');
