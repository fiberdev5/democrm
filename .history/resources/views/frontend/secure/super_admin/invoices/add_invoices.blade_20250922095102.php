      ...
      <div class="row form-group">
        <div class="col-12">
          ...
          <div id="odemeListesi" style="display: none;">
            <h6>Fatura Oluşturulacak Ödeme:</h6>
...
<script type="text/javascript">
  ...
  function loadCompletedPayments(tenantId) {
    ...
    $.ajax({
      ...
      error: function() {
        $('#odemeYukleniyor').hide();
        $('#odemeSecenekleri').html('<div class="alert alert-danger">Ödemeler yüklenirken hata oluştu.</div>');
        $('#odemeListesi').show();
      }
    });
  }
  ...
  function autoFillFormFromPayment(amount, description) {
    ...
    // Fiyat olarak ödeme tutarını ayarla
    $('.fiyat0').val(amount.toFixed(2));
    
    // Tutar olarak da aynı tutarı ayarla  
    $('.tutar0').val(amount.toFixed(2));
    
    // Toplamları hesapla
    $('.toplam').val(amount.toFixed(2));
    $('.araToplam').val(amount.toFixed(2));
    
    // KDV'yi 0 yap
    $('.kdvTutar').val('0');  // KDV oranını 0 yap
    $('.kdv').val('0.00');    // KDV tutarını 0 yap
    $('.genelToplam').val(amount.toFixed(2)); // Genel toplam = ana tutar (KDV yok)
  }
...
$(document).click(function(e) {
    ...
    $('#addInvo').submit(function (event) {
      ...
      if (!$('#selectedPaymentId').val() || !$('#selectedPaymentType').val()) {
        alert('Lütfen bir ödeme seçin.');
        formIsValid = false;
      }
      ...
      if (!formIsValid) {
        ...
        if (!$('#selectedPaymentId').val()) {
          alert('Lütfen zorunlu alanları doldurun ve bir ödeme seçin.');
        } else {
