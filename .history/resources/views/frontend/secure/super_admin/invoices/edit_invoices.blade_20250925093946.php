              ...
              <div id="secilenOzemeler" class="selected-payments-summary">
                @if($invoice_id->payment_details)
                  ...
                  @foreach($existingPayments as $index => $payment)
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 4px 0; border-bottom: 1px solid #dee2e6;">
                      <div style="flex: 1;">{{ $index + 1 }}. {{ $payment['description'] ?? 'Ödeme' }}</div>
                      <div style="font-weight: 600;">{{ $payment['amount'] ?? 0 }} TL</div>
                      <div style="margin-left: 10px; cursor: pointer; color: #dc3545; font-size: 20px; padding: 0 5px;" onclick="removePaymentFromSummary({{ $index }})" title="Kaldır">×</div>
                    </div>
                  @endforeach
                @else
...
<script type="text/javascript">
  ...
  function updateSelectedPaymentsSummary() {
    ...
    selectedPayments.forEach(function(payment, index) {
        ...
        html += '<div style="margin-left: 10px; cursor: pointer; color: #dc3545; font-size: 20px; padding: 0 5px;" onclick="removePaymentFromList(' + index + ')" title="Kaldır">×</div>';
        html += '</div>';
    });
