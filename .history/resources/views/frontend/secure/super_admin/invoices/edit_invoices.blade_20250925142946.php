    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data === false) {
                ...
                // Başarılı güncelleme - MODAL KAPATILMASIN
                alert(data.message || "Fatura güncellendi");
                $('#datatableInvoice').DataTable().ajax.reload();
                
                // MODAL KAPATMA KODLARINI KALDIRIYORUZ
                // $('#editInvoiceModal').modal('hide'); // Bu satırı kaldır
                // $('.modal').modal('hide'); // Bu satırı kaldır
        ...
        error: function(xhr, status, error) {
            ...
            // Hata durumunda da modal kapatma
            // $('#editInvoiceModal').modal('hide'); // İsteğe bağlı
        }
    });
});
// E-Arşiv yükleme için AYRI bir handler ekleyin
$(document).on('click', '.invoic_e', function(e) {
    e.preventDefault();
    var invoiceId = $(this).attr("data-bs-id");
    
    // E-Arşiv yükleme modalını aç (InvoiceModal)
    $.ajax({
...
// E-Arşiv yükleme tamamlandığında SADECE InvoiceModal'ı kapat
$(document).on('submit', '#InvoiceModal form', function(e) {
    ...
    $.ajax({
        ...
        processData: false,
        success: function(response) {
            alert('E-Arşiv belgesi başarıyla yüklendi');
            
            // SADECE e-arşiv modalını kapat, ana edit modalı açık kalsın
            $('#InvoiceModal').modal('hide');
            
            // Ana edit modalındaki PDF görüntüleme linkini güncelle
            if (response.pdf_url) {
                $('#editInvoiceModal .btn-warning[href*="asset"]').attr('href', response.pdf_url).removeClass('d-none');
            }
        },
        error: function() {
...
// Eğer modallar iç içe açılıyorsa, backdrop ayarını değiştirin
$('#InvoiceModal').modal({
    backdrop: 'static', // Ana modalın kapanmasını engeller
    keyboard: false
});
</script>