...
$(document).ready(function(){
    $('#editInvo').on('click', '.invoic_e', function(e){
        ...
        // E-Arşiv yükleme modalını aç (modal HTML'ini yükle)
        $.ajax({
            ...
            success: function(data) {
                if ($.trim(data) === "-1") {
                    ...
                    // E-Arşiv modal'ını aç
                    $('#InvoiceModal').modal('show');
                    $('#InvoiceModal .modal-body').html(data);
                }
            },
            error: function(xhr, status, error) {
                alert("E-Arşiv modalı açılırken hata oluştu: " + error);
            }
        });
    });
});

window.onEarshivUploadSuccess = function(invoiceId, message) {
    // Başarı mesajı göster
    alert(message || "E-Arşiv belgesi başarıyla güncellendi");
    ...
    // E-Arşiv modalını kapat
    $('#InvoiceModal').modal('hide');
    
    // Ana edit modalının içeriğini yenile (açık kalması için)
    refreshMainEditModal(invoiceId);
}
// Ana edit modalını yenile
function refreshMainEditModal(invoiceId) {
    $.ajax({
        url: '{{ route("super.admin.invoices.edit", "") }}/' + invoiceId,
        type: 'GET',
        success: function(data) {
            // Ana edit modalının içeriğini güncelle
            $('#editInvoiceModal .modal-body').html(data);
            
            // Ana edit modalı zaten açıksa, açık kalacak
            // Eğer kapalıysa tekrar aç
            if (!$('#editInvoiceModal').hasClass('show')) {
                $('#editInvoiceModal').modal('show');
            }
        },
        error: function() {
            console.log('Ana modal içerik yenileme hatası');
        }
    });
}
</script>
