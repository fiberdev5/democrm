...
function firmaAra(aramaMetni) {
    $.ajax({
        url: '{{ route("super.admin.firma.ara") }}', // Bu route'u oluşturun
        type: 'POST',
        data: {
            arama: aramaMetni,
            _token: '{{ csrf_token() }}'
...
function firmaListesiGoster(firmalar) {
    ...
    if (firmalar.length === 0) {
        ...
        firmalar.forEach(function(firma) {
            const item = $(`
                <li class="list-group-item" style="cursor: pointer;" onclick="firmasec(${firma.id}, '${firma.firma_adi}', '${firma.tel1}', '${firma.tel2}', '${firma.il}', '${firma.ilce}', '${firma.adres}', '${firma.vergi_no}', '${firma.vergi_dairesi}')">
...
function firmaSecç(id, firmaAdi, tel1, tel2, il, ilce, adres, vergiNo, vergiDairesi) {
