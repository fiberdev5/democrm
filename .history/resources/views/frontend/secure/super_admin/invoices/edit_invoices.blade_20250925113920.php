...
$(document).ready(function(){
    $('#editInvo').on('click', '.invoic_e', function(e){
        ...
        $.ajax({
            ...
            success: function(data) { // 'done' yerine 'success' kullanmak daha yaygındır
                if ($.trim(data) === "-1") {
                    window.location.reload(true); // Hata durumunda sayfayı yenile
                } else {
                    $('#InvoiceModal').modal('show');
                    $('#InvoiceModal .modal-body').html(data);

                    // E-arşivin mevcut durumuna göre butonları ayarla
                    var faturaPdfUrl = $('#InvoiceModal .modal-body').find('.eArsivGosterBtn').attr('href'); // Varsayalım ki modal içinde bir .eArsivGosterBtn var
                    if (faturaPdfUrl && faturaPdfUrl !== '#') {
                        $('#InvoiceModal .modal-body').find('.eArsivGosterBtn').removeClass('d-none');
                        $('#InvoiceModal .modal-body').find('.eArsivSil').show(); // Sil butonunu göster
                    } else {
                        $('#InvoiceModal .modal-body').find('.eArsivGosterBtn').addClass('d-none');
                        $('#InvoiceModal .modal-body').find('.eArsivSil').hide(); // Sil butonunu gizle
                    }
                }
            },
            error: function() {
                alert("Fatura detayı yüklenirken bir hata oluştu.");
            }
        });
    });
});
...
$(document).ready(function() {
    $('#editInvo').on('click', '.eArsivSil', function(e) {
        ...
        if (confirmDelete) {
            ...
            var $thisButton = $(this); // Sil butonunu referans al
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data) {
                        alert("E-fatura başarıyla silindi.");
                        $('#datatableInvoice').DataTable().ajax.reload(); // Ana tabloyu güncelle

                        // Modal içindeki ilgili alanları güncelle
                        // "Görüntüle" butonunu gizle
                        $thisButton.prev('.btn-warning').addClass('d-none').attr('href', '#'); 
                        // "Sil" butonunu kaldır (veya pasifleştir)
                        $thisButton.remove(); 
                        // "Düzenle" butonunu (invoic_e class'ına sahip olanı) da kaldırabilir veya pasifleştirebilirsiniz,
                        // eğer e-arşiv silindikten sonra düzenlenmemesi gerekiyorsa.
                        // $thisButton.closest('.btnWrap').find('.invoic_e').remove();

                        // Fatura numarasını veya e-arşivle ilgili diğer bilgileri temizle
                        // Eğer fatura numarasının E-arşiv belgesi silindiğinde temizlenmesi gerekiyorsa:
                        // $('.faturaNumarasi').val(''); 

                        // Başka bir modal açıksa kapatma komutlarını kaldırın
                        // $('#InvoiceModal').modal('hide'); // Bu satırı yorum satırı yapın veya silin
                        // $('#editInvoiceModal').modal('hide'); // Bu satırı yorum satırı yapın veya silin
                    } else {
