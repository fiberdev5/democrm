...
<form method="post" id="editInvo" action="{{ route('super.admin.invoices.update')}}" enctype="multipart/form-data" class="needs-validation" novalidate>
  ...
  <div class="card f2">
    ...
    <!-- Sol sütun: Firma Seçimi -->
    <div class="col-md-6 d-flex flex-column gap-2 border-end" style="padding-right: 15px;">
        <div class="row form-group">
           <div class="col-md-4 rw1"><label><span class="firmaAdiSpan">Firma Adı</span> <span style="font-weight: bold; color: red;">*</span></label></div>
           <div class="col-md-8 rw2">
             <select name="firma_id" class="form-select firmaSecim" required>
               <option value="">Firma Seçiniz</option>
               @foreach($tenants as $tenant)
                 <option value="{{$tenant->id}}" 
                         data-firma-adi="{{$tenant->firma_adi}}"
                         data-telefon="{{$tenant->telefon}}"
                         data-il="{{$tenant->il}}"
                         data-ilce="{{$tenant->ilce}}"
                         data-adres="{{$tenant->adres}}"
                         data-vergi-no="{{$tenant->vergi_no}}"
                         data-vergi-dairesi="{{$tenant->vergi_dairesi}}"
    ...
    <div class="col-md-6 d-flex flex-column gap-1" style="padding-left: 15px;">
        <span><strong>{{ $invoice_id->tenant->firma_adi }}</strong></span>
        <span>{{ $invoice_id->tenant?->adres }} {{ $invoice_id->tenant?->ilce }}/{{ $invoice_id->tenant?->il }}</span>

        @if(!empty($invoice_id->tenant?->telefon))
            <span>TEL: {{ $invoice_id->tenant?->telefon }}</span>
        @endif

        @if(!empty($invoice_id->tenant?->vergi_no) || !empty($invoice_id->tenant?->vergi_dairesi))
            <span>VERGİ NO/DAİRESİ: {{ $invoice_id->tenant?->vergi_no }}/{{ $invoice_id->tenant?->vergi_dairesi }}</span>
        @endif
    </div>
</div>
     </div>
...
function formatToTwoDecimalPlaces(value) {
  ...
  $(document).ready(function (e) {
    ...
    // Firma seçimi değiştiğinde bilgileri güncelle  
    $('.firmaSecim').change(function() {
      var selectedOption = $(this).find('option:selected');
      if (selectedOption.val()) {
        // Sayfayı yeniden yükle veya AJAX ile bilgileri güncelle
        location.reload();
      }
    });
  });
...
<script>
  ...
  $.ajax({
    ...
    $('.indirim').on('keyup', function() {
      ...
      $(".araToplam").val(formatToTwoDecimalPlaces(araToplam));
      $(".genelToplam").val(formatToTwoDecimalPlaces(gen