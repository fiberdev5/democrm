     ...
     <div class="card-body">
        <div class="row">
           <div class="col-12">
                       ...
                       <div style="flex: 1;">
                         ...
                         <div style="font-size: 13px; color: #6c757d; line-height: 1.5;">
                           @if(!empty($invoice_id->tenant?->telefon))
                             <div style="margin-bottom: 3px;">📞 Telefon: {{ $invoice_id->tenant->telefon }}</div>
                           @endif
                           ...
                           @if(!empty($invoice_id->tenant?->vergi_no) || !empty($invoice_id->tenant?->vergi_dairesi))
                             <div style="margin-bottom: 3px;">🏢 Vergi: {{ $invoice_id->tenant->vergi_no }} {{ $invoice_id->tenant->vergi_dairesi ? ' - ' . $invoice_id->tenant->vergi_dairesi : '' }}</div>
                           @endif
              ...
              <input type="hidden" name="vergiNo" value="{{ $invoice_id->tenant->vergi_no }}" class="vergiNo">
              <input type="hidden" name="vergiDairesi" value="{{ $invoice_id->tenant->vergi_dairesi }}" class="vergiDairesi">
              <input type="hidden" name="tel1" value="{{ $invoice_id->tenant->telefon }}" class="tel1">
  ...
  $(document).ready(function() {
    ...
    $('.musteriKaydet').click(function() {
      ...
      if (selectedOption.val()) {
        ...
        var telefon = selectedOption.data('telefon') || '';
        var il = selectedOption.data('il') || '';
        var ilce = selectedOption.data('ilce') || '';
        var adres = selectedOption.data('adres') || '';
        var vergiNo = selectedOption.data('vergi-no') || '';
        var vergiDairesi = selectedOption.data('vergi-dairesi') || '';
        ...
        $('#seciliMusteri').html(
          '<div style="display: flex; justify-content: space-between; align-items: flex-start;">' +
            '<div style="flex: 1;">' +
              ...
              '<div style="font-size: 13px; color: #6c757d; line-height: 1.5;">' +
                (telefon ? '<div style="margin-bottom: 3px;">📞 Telefon: ' + telefon + '</div>' : '') +
