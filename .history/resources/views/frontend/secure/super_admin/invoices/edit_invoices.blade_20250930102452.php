  ...
  <div class="col-md-8 rw2">
    <div id="fileUploadContainer">
      ...
      @if($invoice_id->faturaPdf != null)
        ...
        @endphp
        <div id="uploadedFileArea" style="display: flex; align-items: center; gap: 8px;">
          <a href="{{asset($invoice_id->faturaPdf)}}" target="_blank" 
             id="uploadedFileLink"
             style="color: #495057; text-decoration: none; font-size: 13px;"
             title="{{$fileName}} - Görüntülemek için tıklayın">
            <i class="fas fa-file-pdf" style="color: #dc3545;"></i> {{$displayName}}
          </a>
          <button type="button" class="btn btn-link p-0 eArsivSil" data-id="{{$invoice_id->id}}" 
                  style="color: #dc3545; font-size: 18px; line-height: 1;" title="Sil">
      ...
      @else
        ...
        <div id="fileSelectArea">
          <button type="button" 
                  ...
                  style="color: #6c757d; text-decoration: none; font-size: 13px;">
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data && data.status === 'success') {
                    ...
                    // Yüklenen dosya alanını gizle
                    $('#uploadedFileArea').remove();
                    
                    // Dosya seç butonunu göster
                    $('#fileUploadContainer').html(
                        '<input type="file" id="pdfFileInput" name="pdf" accept=".pdf,.jpg,.jpeg,.png" style="display: none;" data-invoice-id="' + id + '">' +
                        '<div id="fileSelectArea">' +
                        '<button type="button" ' +
                        'class="btn btn-link p-0" ' +
                        'id="selectFileBtn" ' +
                    ...
                    // File input event listener'ı yeniden ekle
...
// File input event listener'ını yeniden başlatma fonksiyonu
function reinitializeFileInput() {
    $('#pdfFileInput').off('change').on('change', function() {
        var fileInput = this;
        ...
        if (fileInput.files.length > 0) {
            ...
            $('#fileSelectArea').html('<span style="color: #6c757d; font-size: 13px;"><i class="fas fa-spinner fa-spin"></i> Yükleniyor...</span>');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        var displayName = fileName.length > 25 ? fileName.substring(0, 22) + '...' : fileName;
                        ...
                        $('#fileUploadContainer').append(
                            ...
                            '<i class="fas fa-file-pdf" style="color: #dc3545;"></i> ' + displayName +
                            '</a>' +
                            '<button type="button" class="btn btn-link p-0 eArsivSil" data-id="' + invoiceId + '" ' +
                            'style="color: #dc3545; font-size: 18px; line-height: 1;" title="Sil">' +
                            '<i class="fas fa-times"></i>' +
                            '</button>' +
                            '</div>'
...
$(document).ready(function() {
 ...
 // PDF dosya seçimi değiştiğinde otomatik yükleme
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Dosya seç alanını gizle ve yüklenen dosya alanını göster
                        $('#fileSelectArea').hide();
                        $('#fileUploadContainer').append(
                            '<div id="uploadedFileArea" style="display: flex; align-items: center; gap: 8px;">' +
                            '<a href="' + fileUrl + '" target="_blank" ' +
    ...
    // Dosya seç alanını eski haline getir
    function resetFileSelect() {
        $('#fileSelectArea').html(
            '<button type="button" ' +
            'class="btn btn-link p-0" ' +
