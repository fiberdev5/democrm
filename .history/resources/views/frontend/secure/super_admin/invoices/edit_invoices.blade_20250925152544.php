...
<script type="text/javascript">
// E-Arşiv işlemi - Modal kapanmasını engelle
$(document).ready(function(){
    $('#editInvo').on('click', '.invoic_e', function(e){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                console.log(data);
                if ($.trim(data) === "-1") {
                    alert("E-Arşiv işlemi başarısız!");
                    // Modal'ı KAPATMA, sadece uyarı ver
                } else {
                    // E-Arşiv modal'ını aç (ana modal açık kalacak)
                    $('#InvoiceModal').modal('show');
                    $('#InvoiceModal .modal-body').html(data);
                    
                    // Ana modal'ın z-index'ini artır ki üstte kalsın
                    $('#editInvoiceModal').css('z-index', '1040');
                    $('#InvoiceModal').css('z-index', '1050');
                }
            },
            error: function(xhr, status, error) {
                // Button'u eski haline getir
                button.html(originalHtml).prop('disabled', false);
                alert("E-Arşiv yüklenirken hata oluştu: " + error);
            }
        });
    });
});

// E-Arşiv modal kapandığında ana modal'a geri dön
$(document).on('hidden.bs.modal', '#InvoiceModal', function () {
    // Ana modal'ı tekrar görünür yap
    $('#editInvoiceModal').css('z-index', '1050');
    
    // Ana modal'ın içeriğini yenile (e-arşiv durumu güncellensin)
    var invoiceId = $('#editInvo input[name="id"]').val();
    if (invoiceId) {
        refreshModalContent(invoiceId);
    }
});

// Modal yenileme fonksiyonunu güncelle
function refreshModalContent(invoiceId) {
    // Loading göstergesi
    $('#editInvoiceModal .modal-body').prepend(
        '<div id="refreshLoader" style="text-align: center; padding: 10px; background: rgba(0,0,0,0.1); position: absolute; top: 0; left: 0; right: 0; z-index: 9999;">' +
        '<i class="fas fa-spinner fa-spin"></i> İçerik güncelleniyor...' +
        '</div>'
    );
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            ...
            console.error('Modal yenilenemedi:', error);
        }
    });
}
</script>
...
<script>
// E-Arşiv silme işlemi - Modal kapanmasını engelle
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data && data.status === 'success') {
                    ...
                    // Modal içeriğini yenile - MODAL KAPANMASIN
...
<script>
// Ana form submit işlemi - Modal kapanmasını engelle
$('#editInvo').on('submit', function(e) {
    ...
    $.ajax({
        ...
        processData: false,
        success: function(data) {
            // Submit button'u eski haline getir
            submitButton.val(originalValue).prop('disabled', false);
            
            console.log('Form Response:', data);
            
            if (data === false) {
                // Sadece uyarı ver, modal'ı kapatma
                alert("Kaydetme işlemi başarısız!");
            } else if (data && data.status === 'success') {
                // Başarılı güncelleme - Modal AÇIK KALACAK
                alert(data.message || "Fatura güncellendi");
                
                // DataTable'ı güncelle (eğer varsa)
                if (typeof $('#datatableInvoice').DataTable === 'function') {
                    $('#datatableInvoice').DataTable().ajax.reload();
                }
...
<style>
/* Ek CSS - Modal kapanma sorunu için gerekli stiller */
.border-success {
...
/* Form geçiş efektleri */
#editInvo {
    transition: border-color 0.3s ease, box-shadow 0.3s ease;
}

#editInvo.border-success {
    box-shadow: 0 0 10px rgba(40, 167, 69, 0.3);
}

#editInvo.border-danger {
