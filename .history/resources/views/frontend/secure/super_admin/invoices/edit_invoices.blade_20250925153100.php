        ...
        if (confirmDelete) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log('Response:', data); // Debug için ekle
                    
                    // Controller'dan gelen response formatını kontrol et
                    if (data && data.status === 'success') {
                        $('#datatableInvoice').DataTable().ajax.reload();
                        
                        // E-Arşiv butonlarını gizle
                        $('.btn-warning[href*="' + id + '"]').addClass('d-none');
