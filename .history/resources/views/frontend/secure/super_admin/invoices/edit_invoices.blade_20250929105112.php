    ...
    // PDF dosya seçimi değiştiğinde otomatik yükleme
      $('#pdfFileInput').on('change', function() {
        ...
        if (fileInput.files.length > 0) {
            ...
            // Dosya seçildi mesajını göster
            $('#selectedFileName').text(file.name);
            $('#fileSelectedStatus').show();
            $('#noFileWarning').hide();
            ...
            $('#fileSelectedStatus').html('<i class="fas fa-spinner fa-spin"></i> Yükleniyor...');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Sayfayı yenile veya dinamik olarak güncelle
                        location.reload();
