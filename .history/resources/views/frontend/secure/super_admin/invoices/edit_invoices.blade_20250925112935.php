...
/* E-fatura modal z-index */
#InvoiceModal {
    z-index: 1060; /* Ana modal'dan yüksek olmalı */
}
            ...
            <div style="margin-top: 10px; padding: 10px; background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 4px;">
              ...
              <div style="font-size: 12px; margin-top: 5px;">
                ...
                @foreach($existingPayments as $payment)
                  <div>• {{ $payment['description'] ?? 'Ödeme' }} - {{ $payment['amount'] ?? 0 }} TL</div>
                @endforeach
              </div>
            </div>
          @endif
...
<script type="text/javascript">
  ...
  function updateSelectedPaymentsSummary() {
    ...
    selectedPayments.forEach(function(payment, index) {
        ...
        html += '<div style="margin-left: 10px; cursor: pointer; color: #dc3545;" onclick="removePayment(\'' + payment.key + '\')" title="Kaldır">×</div>';
        html += '</div>';
    });
    ...
    $('#editInvo').on('click', '.invoic_e', function(e){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $(e.target).prop('disabled', false).html('<i class="fas fa-edit"></i>');
                
                if ($.trim(data) === "-1") {
                    window.location.reload(true);
                } else {
                    // E-fatura modal'ını göster (ana modal üzerine)
                    // Not: Bu modal'ın kapatılma eventini dinliyoruz
                    $('#InvoiceModal').modal('show');
                    $('#InvoiceModal .modal-body').html(data);
                    
                    // E-fatura modal'ı kapatıldığında ana modal'ın içeriğini yenile
                    $('#InvoiceModal').off('hidden.bs.modal.refresh').on('hidden.bs.modal.refresh', function() {
                        reloadModalContent(id);
                    });
                }
            },
            error: function() {
                $(e.target).prop('disabled', false).html('<i class="fas fa-edit"></i>');
                showModalAlert('error', 'E-fatura bilgileri yüklenirken hata oluştu.');
            }
        });
    });
});

// E-fatura modal'ı kapatıldığında ana modal'ı güncelle
$(document).on('hidden.bs.modal', '#InvoiceModal', function (e) {
    // Ana modal hala açık mı kontrol et
    if ($('#editInvoiceModal').hasClass('show')) {
        // Ana modal'ın içeriğini yenile
        var invoiceId = $('input[name="id"]').val();
        if (invoiceId) {
            reloadModalContent(invoiceId);
        }
    }
});
</script>
    ...
    $('#editInvo').on('click', '.eArsivSil', function(e) {
        ...
        if (confirmDelete) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data) {
                        // Ana modal'ı kapatmıyoruz, sadece içeriği güncelliyoruz
                        
                        // 1. DataTable'ı güncelle (arka planda)
                        if (typeof $('#datatableInvoice').DataTable !== 'undefined') {
                            $('#datatableInvoice').DataTable().ajax.reload(null, false);
                        }
                        
                        // 2. Modal içeriğini yeniden yükle
                        reloadModalContent(id);
                        
                        // Başarı mesajı
                        showModalAlert('success', 'E-Arşiv başarıyla silindi.');
                        
                    } else {
                        showModalAlert('error', 'Silme işlemi başarısız oldu.');
                        $button.prop('disabled', false).text('Sil');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('E-Arşiv silme hatası:', xhr.responseText);
                    showModalAlert('error', 'Silme işlemi sırasında hata oluştu.');
                    $button.prop('disabled', false).text('Sil');
                }
            });
        }
...
// Modal içeriğini yeniden yükle
function reloadModalContent(invoiceId) {
    $.ajax({
        url: "{{ route('super.admin.invoices.show', '') }}/" + invoiceId,
        type: 'GET',
        beforeSend: function() {
            // Yükleme göstergesi ekle
            $('#editInvoiceModal .modal-body').prepend(
                '<div id="modal-loading" class="text-center p-3">' +
                '<div class="loading-spinner"></div>' +
                '<span style="margin-left: 10px;">Güncelleniyor...</span>' +
                '</div>'
            );
        },
        success: function(data) {
            if ($.trim(data) === "-1") {
                // Hata durumunda ana modal'ı kapat
        ...
        error: function() {
            ...
            showModalAlert('error', 'İçerik güncellenirken hata oluştu.');
        }
    });
}
...
function showModalAlert(type, message) {
    ...
    var alertIcon = type === 'success' ? '✓' : '✗';
    
    var alertHtml = '<div class="alert ' + alertClass + ' alert-dismissible fade show modal-alert" role="alert" style="margin-bottom: 15px;">' +
    ...
    // Yeni alert'i modal body'nin başına ekle
    $('#editInvoiceModal .modal-body').prepend(alertHtml);
    
    // 3 saniye sonra otomatik kaldır
    setTimeout(function() {
        $('.modal-alert').fadeOut(300, function() {
            $(this).remove();
        });
    }, 3000);
}
...
$('#editInvo').on('submit', function(e) {
    ...
    submitButton.prop('disabled', true).val('Kaydediliyor...');
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data === false) {
                ...
                showModalAlert('success', 'Fatura başarıyla güncellendi.');
        ...
        error: function(xhr, status, error) {
            console.error('Form güncelleme hatası:', xhr.responseText);
            showModalAlert('error', 'Güncelleme başarısız! Lütfen tekrar deneyin.');
