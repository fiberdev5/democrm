...
<script type="text/javascript">
  // Global değişkenler
  window.selectedPayments = []; // Şu anda seçili ödemeler
  window.originalPayments = []; // Sayfa ilk yüklendiğinde bağlı olan orijinal ödemeler
  window.originalTotals = {}; // Orijinal fatura tutarları
  ...
  // Çoklu ödeme seçimi fonksiyonu (GÜNCELLENDİ)
  window.selectPayment = function(paymentId, paymentType, amount, description) {
    ...
    // Form verilerini SADECE DEĞİŞİKLİKLERE GÖRE güncelle
    updateFormFromChanges();

    // Kaldırılan ödemeler için listeyi yenile
    refreshPaymentList();
  }

  // YENİ FONKSİYON: Ödeme listesini yenile (kaldırılan ödemeler yukarı çıksın)
function refreshPaymentList() {
    var paymentContainer = $('#odemeSecenekleri');
    var paymentItems = paymentContainer.find('.payment-item').detach().sort(function(a, b) {
        var aSelected = $(a).hasClass('selected');
        var bSelected = $(b).hasClass('selected');
        
        // SEÇİLİ OLANLAR YUKARIDA OLSUN (düzeltildi)
        if (aSelected && !bSelected) return -1;  // a yukarı
        if (!aSelected && bSelected) return 1;   // b yukarı
        return 0; // eşit
    });
    
    paymentContainer.append(paymentItems);
}
  // Değişikliklere göre form güncelleme fonksiyonu (YENİ)
  function updateFormFromChanges() {
    ...
    // Eğer hiç seçili ödeme yoksa, formu sıfırla
    if (selectedPayments.length === 0) {
        ...
        $('.toplamYazi').attr('placeholder', 'Tüm ödemeler kaldırıldı');
        return;
    }
    
    // Değişiklik analizi
    var addedPayments = selectedPayments.filter(function(selected) {
        return !originalPayments.some(function(original) {
            return original.key === selected.key;
        });
    });
    
    var removedPayments = originalPayments.filter(function(original) {
        return !selectedPayments.some(function(selected) {
            return selected.key === original.key;
        });
    });
    
    console.log('Eklenen ödemeler:', addedPayments);
    console.log('Kaldırılan ödemeler:', removedPayments);
    
    // Tüm seçili ödemeler için form inputları oluştur
    selectedPayments.forEach(function(payment, index) {
        ...
        // KDV dahil tutarı KDV hariç tutara çevir (backend için)
        ...
        // Ürün bilgileri (backend KDV hariç bekliyor)
    ...
    // Tutarları güncelle - SADECE DEĞİŞİKLİK VARSA
    if (addedPayments.length > 0 || removedPayments.length > 0) {
        calculateNewTotals(addedPayments, removedPayments);
    }
    
    // Toplam yazısını güncelle
    if (selectedPayments.length > 0) {
        var changeText = '';
        if (addedPayments.length > 0) {
            changeText += addedPayments.length + ' eklenen';
        }
        if (removedPayments.length > 0) {
            if (changeText) changeText += ', ';
            changeText += removedPayments.length + ' kaldırılan';
        }
        if (!changeText) changeText = 'mevcut';
        
        $('.toplamYazi').attr('placeholder', selectedPayments.length + ' adet ödeme (' + changeText + ')');
    }
}

  // Yeni tutarları hesaplama fonksiyonu (YENİ)
  function calculateNewTotals(addedPayments, removedPayments) {
    var kdvOrani = parseFloat($('.kdvTutar').val()) || 20;
    var kdvOraniFaktor = (100 + kdvOrani) / 100;
    
    // Eğer hiç seçili ödeme yoksa, tamamen sıfırla
    if (selectedPayments.length === 0) {
        $('.toplam').val('0.00');
        $('.indirim').val('0.00');
        $('.araToplam').val('0.00');
    ...
    // Mevcut toplamdan başla
    var currentToplam = originalTotals.toplam;
    
    // Kaldırılan ödemeleri çıkar
    removedPayments.forEach(function(payment) {
        var kdvHaricTutar = payment.amount / kdvOraniFaktor;
        currentToplam -= kdvHaricTutar;
    });
    
    // Eklenen ödemeleri ekle
    addedPayments.forEach(function(payment) {
        var kdvHaricTutar = payment.amount / kdvOraniFaktor;
        currentToplam += kdvHaricTutar;
    });
    
    // Negatif değer kontrolü
    if (currentToplam < 0) currentToplam = 0;
    
    // Form alanlarını güncelle
    $('.toplam').val(currentToplam.toFixed(2));
    kdvHesapla(currentToplam);
}


  // Seçili ödemeler özetini güncelle (GÜNCELLENDİ)
  function updateSelectedPaymentsSummary() {
    var summaryDiv = $('#secilenOzemeler');
    var totalAmount = 0;
    
    if (selectedPayments.length === 0) {
        summaryDiv.html('<div style="color: #6c757d; font-style: italic;">Tüm ödemeler kaldırıldı - Fatura tutarı sıfırlanacak</div>');
        return;
    }
    
    var html = '';
    selectedPayments.forEach(function(payment, index) {
        totalAmount += payment.amount; // KDV dahil tutar
        
        // Bu ödeme orijinal ödemelerden mi yoksa yeni mi?
        var isOriginal = originalPayments.some(function(original) {
            return original.key === payment.key;
        });
        
        var badgeClass = isOriginal ? 'payment-badge original' : 'payment-badge new';
        var badgeText = isOriginal ? 'ESKİ' : 'YENİ';
        
        html += '<div style="display: flex; justify-content: space-between; align-items: center; padding: 6px 0; border-bottom: 1px solid #dee2e6;">';
        html += '<div style="flex: 1;">';
        html += '<span class="' + badgeClass + '">' + badgeText + '</span>';
        html += (index + 1) + '. ' + payment.description;
        html += '</div>';
        // KDV DAHİL TUTAR GÖSTERİLİYOR (değişiklik burada)
        html += '<div style="font-weight: 600; margin-right: 10px;">' + payment.amount + ' TL <small style="color: #6c757d;">(KDV Dahil)</small></div>';
        html += '<div style="cursor: pointer; color: #dc3545; font-size: 18px; padding: 0 5px;" onclick="removePaymentFromSummary(' + index + ')" title="Kaldır">×</div>';
        html += '</div>';
    });
    
    // Toplam KDV dahil tutar göster
    html += '<div style="margin-top: 10px; padding-top: 10px; border-top: 2px solid #dee2e6; font-weight: bold; text-align: right;">';
    html += 'Toplam KDV Dahil: ' + totalAmount.toFixed(2) + ' TL';
    html += '</div>';
...
function loadCompletedPayments(tenantId) {
    ...
    $.ajax({
      ...
      success: function(payments) {
        ...
        if (payments.length === 0) {
          ...
          // AJAX yükleme tamamlandıktan SONRA listeyi sırala
          setTimeout(function() {
            refreshPaymentList();
          }, 100);
        }
  ...
  // Özetten ödeme kaldırma fonksiyonu (GÜNCELLENDİ)
window.removePaymentFromSummary = function(index) {
    if (selectedPayments[index]) {
      ...
      // YENİ EKLENDİ: Liste sıralamasını yenile
      refreshPaymentList();
    }
}


  // Sayfa yüklendiğinde orijinal ödemeleri kaydet
$(document).ready(function() {
    // Orijinal fatura tutarlarını kaydet
    originalTotals = {
        toplam: parseFloat($('.toplam').val()) || 0,
        indirim: parseFloat($('.indirim').val()) || 0,
        kdvTutar: parseFloat($('.kdvTutar').val()) || 20,
        kdv: parseFloat($('.kdv').val()) || 0,
        genelToplam: parseFloat($('.genelToplam').val()) || 0
    };
    
    // Mevcut bağlı ödemeleri originalPayments ve selectedPayments array'lerine ekle
    @if($invoice_id->payment_details)
      ...
      existingPayments.forEach(function(payment, index) {
        ...
        originalPayments.push(paymentObj);
        selectedPayments.push({...paymentObj}); // Kopyasını selectedPayments'a da ekle
      });
    ...
    if (firmaId) {
      // AJAX yükleme 500ms sonra başlasın (DOM hazır olsun)
      setTimeout(function() {
...
<script>
// Form gönderme validasyonu (GÜNCELLENDİ)
$('#editInvo').on('submit', function(e) {
    e.preventDefault();

    // Değişiklik kontrolü
    var hasChanges = selectedPayments.length !== originalPayments.length ||
                    !selectedPayments.every(function(selected) {
                        return originalPayments.some(function(original) {
                            return original.key === selected.key;
                        });
                    });
    ...
    if (selectedPayments.length === 0) {
        if (!confirm('Hiçbir ödeme seçilmemiş. Fatura tutarı sıfır olacak. Devam etmek istiyor musunuz?')) {
            return;
        }
    } else if (hasChanges) {
        if (!confirm('Ödeme değişiklikleri yapıldı. Fatura tutarları güncellenecek. Devam etmek istiyor musunuz?')) {
            return;
        }
    }

    // Normal form validasyonu
