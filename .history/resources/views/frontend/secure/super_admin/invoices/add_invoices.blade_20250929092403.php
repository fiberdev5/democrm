...
#firmaListesi .list-group-item:hover {
...
<div class="row form-group">
    ...
    <div class="col-md-9 rw2">
        <div style="display: flex; gap: 8px; margin-bottom: 5px;">
            <input type="text" id="firmaArama" class="form-control" placeholder="Firma adı yazın..." autocomplete="off" style="flex: 1;">
            <button type="button" id="bekleyenOdemelerBtn" class="btn btn-sm" style="background: #5dade2; color: white; white-space: nowrap; padding: 6px 12px; border: none;" title="Faturası oluşturulmamış ödemesi olan firmaları göster">
    <i class="fas fa-file-invoice"></i> Bekleyen Ödemeler
</button>
        </div>
   ...
   function firmaListesiGoster(firmalar) {
    ...
    if (firmalar.length === 0) {
        ...
        firmalar.forEach(function(firma) {
            ...
            // Bekleyen ödeme badge'i (eğer varsa)
            var pendingBadge = '';
...
$('#bekleyenOdemelerBtn').on('click', function() {
    ...
    $.ajax({
        ...
        success: function(firmalar) {
            ...
            $('#bekleyenOdemelerBtn').html('<i class="fas fa-file-invoice"></i> Bekleyen Ödemeler');
            
            if (firmalar.length === 0) {
                const liste = $('#firmaListesi');
                liste.empty();
                liste.html('<li class="list-group-item" style="background: #fff3cd; color: #856404; padding: 10px;">Faturası oluşturulmamış ödemesi olan firma bulunamadı</li>');
                liste.show();
            } else {
                firmaListesiGoster(firmalar);
            }
        },
        error: function() {
            $('#bekleyenOdemelerBtn').prop('disabled', false);
