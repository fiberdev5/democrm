...
<div class="row">
    <div class="col-12">
        <div class="ticket-header">
            <div class="row align-items-md-start"> {{-- align-items-md-start olarak değiştirildi --}}
                <div class="col-md-8"> {{-- Genişlik ayarı --}}
                    <h3 class="mb-3 text-dark">{{ $ticket->subject }}</h3>
                    
                    <!-- Firma ve Kullanıcı Bilgisi -->
                    <div class="row g-3 mb-3"> {{-- Boşluk ve düzen için g-3 eklendi --}}
                        <div class="col-sm-6"> {{-- Her iki bilgi kartı eşit ve düzenli görünsün diye --}}
                            <div class="card h-80 shadow-sm border-0"> {{-- Kart stili eklendi --}}
                                <div class="card-body p-2 d-flex align-items-center">
                                    <i class="fas fa-building text-primary me-3"></i> {{-- İkon boyutu büyütüldü --}}
                                    <div>
                        ...
                        <div class="col-sm-6"> {{-- Her iki bilgi kartı eşit ve düzenli görünsün diye --}}
                            <div class="card h-100 shadow-sm border-0"> {{-- Kart stili eklendi --}}
                                <div class="card-body p-2 d-flex align-items-center">
                                    <i class="fas fa-user text-primary me-3 "></i> {{-- İkon boyutu büyütüldü --}}
                                    <div>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        @php
                            $priorityColors = [
                                'acil' => 'secondary', // Tüm renkler secondary olarak bırakıldı
                                'kritik' => 'secondary',
                                'yuksek' => 'secondary',
                                'orta' => 'secondary',
                                'dusuk' => 'secondary'
                        ...
                        @switch($ticket->category)
                            @case('teknik_sorun')
                                <span class="badge status-badge-large bg-secondary"> {{-- Tüm renkler secondary olarak bırakıldı --}}
                ...
                <!-- Sağ Taraf: Buton + Tarihler -->
                <div class="col-md-4 text-md-end d-flex flex-column justify-content-start align-items-md-end mt-3 mt-md-0"> {{-- Hizalama ve düzenleme --}}
                    <!-- Buton en üste -->
                    <div class="mb-3"> {{-- Buton ile tarih kartı arasına boşluk --}}
                        @if($ticket->status == 'kapali')
                            <form action="{{ route('super.admin.destek.reopen', $ticket->id) }}" 
                                  method="POST" class="d-inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn btn-success btn-sm w-100 w-md-auto" {{-- Genişlik ayarı --}}
                        ...
                        @else
                            <form action="{{ route('super.admin.destek.close', $ticket->id) }}" 
                                ...
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn btn-secondary btn-sm w-100 w-md-auto" {{-- Genişlik ayarı --}}
                    ...
                    <!-- Tarih Kartı -->
                    <div class="card border-0 bg-white shadow-sm w-100 w-md-auto"> {{-- Kart stili ve genişlik ayarı --}}
