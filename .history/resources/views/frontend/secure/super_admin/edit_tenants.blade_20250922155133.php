    ...
    <div class="text-center p-1 rounded">
      ...
      <div class="fw-semibold">
        @if($currentSubscription->plansubs->limits['storage_gb'] == -1)
          @php
            $usedGB = isset($storageInfo['used_bytes']) ? round($storageInfo['used_bytes'] / (1024 * 1024 * 1024), 2) : 0;
          @endphp
          {{ $usedGB }} GB / Sınırsız
        ...
        @else
          @php
            $usedGB = isset($storageInfo['used_bytes']) ? round($storageInfo['used_bytes'] / (1024 * 1024 * 1024), 2) : 0;
            $totalGB = $currentSubscription->plansubs->limits['storage_gb'];
            $percentage = $totalGB > 0 ? round(($usedGB / $totalGB) * 100, 1) : 0;
          @endphp
          {{ $usedGB }} / {{ $totalGB }} GB
          <br>
          <small class="text-muted">({{ $percentage }}% dolu)</small>
          @if($percentage > 80)
