...
// Global depolama fonksiyonu (PaymentModule dışında) - GÜNCELLENMIŞ
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.success && response.details) {
        ...
        // Servis Fotoğrafları - Sadece sayı
        if (response.details.service_photos) {
          storageHtml += `
            <div class="storage-item">
              <div class="storage-type">Servis Fotoğrafları</div>
              <div class="storage-size">
                ${response.details.service_photos.count} adet
              </div>
            </div>`;
        }
        
        // Stok Fotoğrafları - Sadece sayı (Sadece bu çalışıyor dediğin için)
        if (response.details.stock_photos) {
          storageHtml += `
            <div class="storage-item">
              <div class="storage-type">Stok Fotoğrafları</div>
              <div class="storage-size">
                ${response.details.stock_photos.count} adet
              </div>
            </div>`;
        }
        
        // Diğer Dosyalar - Sadece sayı
        if (response.details.other_files) {
          storageHtml += `
            <div class="storage-item">
              <div class="storage-type">Diğer Dosyalar</div>
              <div class="storage-size">
                ${response.details.other_files.total_count} adet
              </div>
            </div>`;
        }
        
        // Toplam - Backend'den gelen total_size_formatted kullan
        let totalSize = response.total_size_formatted || '0 B';
        ...
        console.log('No storage details found');
        container.html(`
