  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.success) {
        ...
        // Servis Fotoğrafları
        storageHtml += '<div class="storage-item">';
        storageHtml += '<div class="storage-type">Servis Fotoğrafları</div>';
        storageHtml += '<div class="storage-size">' + (response.details.service_photos.count) + ' adet</div>';
        storageHtml += '</div>';
        
        // Stok Fotoğrafları
        storageHtml += '<div class="storage-item">';
        storageHtml += '<div class="storage-type">Stok Fotoğrafları</div>';
        storageHtml += '<div class="storage-size">' + (response.details.stock_photos.count) + ' adet</div>';
        storageHtml += '</div>';
        
        // Diğer Dosyalar
        storageHtml += '<div class="storage-item">';
        storageHtml += '<div class="storage-type">Diğer Dosyalar</div>';
        storageHtml += '<div class="storage-size">' + (response.details.other_files.total_count) + ' adet</div>';
        storageHtml += '</div>';
        
        // Toplam
        let totalSize = response.storage_info.current_usage_formatted || '0 B';
        let totalGB = response.storage_info.total_limit_gb || 10;
        let extraStorage = response.storage_info.extra_storage_gb || 0;
        
        let limitText = totalGB + ' GB';
        if (extraStorage > 0) {
          limitText += ' (+' + extraStorage + ' GB ek)';
        }
        
        storageHtml += '<div class="storage-item total">';
        storageHtml += '<div class="storage-type">Toplam Kullanım</div>';
        storageHtml += '<div class="storage-size">' + totalSize + ' / ' + limitText + '</div>';
        storageHtml += '</div>';
        storageHtml += '</div>';
        ...
        container.html('<div class="text-center py-3"><small class="text-muted text-danger">Depolama bilgileri yüklenemedi</small></div>');
      }
    },
    error: function(xhr, status, error) {
      console.error('Storage AJAX Error:', xhr.responseText);
      container.html('<div class="text-center py-3"><small class="text-muted text-danger">Hata oluştu</small></div>');
    }
  });
}
