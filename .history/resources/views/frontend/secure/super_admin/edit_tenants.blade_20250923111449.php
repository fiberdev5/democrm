  ...
  <div class="border rounded p-2">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      @foreach($periodStats as $key => $period)
        <button class="nav-link {{ $key === 'bugun' ? 'active' : '' }} position-relative" 
                data-period="{{ $key }}" 
                data-target="#period-{{ $key }}"
                type="button">
          <div class="text-center">
            <div class="fw-medium">{{ $period['label'] }}</div>
            <div class="mt-1">
              <span class="badge bg-white bg-opacity-25 text-white rounded-pill px-2">{{ $period['toplam'] }}</span>
            </div>
          </div>
        </button>
      @endforeach
...
/* Servis İstatistikleri Tab Navigation Düzenlemeleri */
#service-stats .nav-pills .nav-link {
...
#service-stats .nav-pills .nav-link:hover {
  background-color: rgba(13, 110, 253, 0.1);
  transform: translateY(-2px);
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
}

#service-stats .nav-pills .nav-link.active {
  background: linear-gradient(135deg, #0d6efd, #0056b3);
  color: white;
  box-shadow: 0 4px 12px rgba(13, 110, 253, 0.3);
}

#service-stats .nav-pills .nav-link.active .badge {
  background-color: rgba(255, 255, 255, 0.3) !important;
  color: white !important;
  border: 1px solid rgba(255, 255, 255, 0.5);
}

#service-stats .nav-pills .nav-link:not(.active) .badge {
  background-color: #f8f9fa;
  color: #6c757d;
  border: 1px solid #dee2e6;
}

/* Toplam servis sayısı stili */
#service-stats .fs-5 {
...
@media (max-width: 768px) {
  ...
  #service-stats .d-flex.justify-content-between {
    flex-direction: column;
    text-align: center;
  }
  
  #service-stats .text-end {
