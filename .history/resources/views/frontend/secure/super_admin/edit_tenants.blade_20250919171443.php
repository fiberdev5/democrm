...
window.PaymentModule = (function() {
  ...
  function showPaymentDetail(type, paymentId) {
    ...
    // Modal'ı backdrop olmadan aç (önemli değişiklik)
  const modal = new bootstrap.Modal(document.getElementById('paymentDetailModal'), {
    backdrop: 'static', // veya false
    keyboard: true
  });
  modal.show();
    ...
    $.ajax({
      // showPaymentDetail fonksiyonundaki AJAX URL'ini şu şekilde değiştirin:
url: `/super-admin/tenant/${currentTenantId}/payment/${type}/${paymentId}`,
  ...
  // Payment detail modal kapandığında backdrop temizliği yap
  $('#paymentDetailModal').on('hidden.bs.modal', function () {
    // Bootstrap backdrop'larını temizle
    ...
    // Ana modal'ın backdrop'ını geri ekle
    setTimeout(function() {
      if ($('#editTenantModal').hasClass('show')) {
        $('body').addClass('modal-open');
        // Gerekirse scrollbar padding'ini yeniden hesapla
  ...
  // Ana modal kapandığında tam temizlik
 ...
 return {
  init: function() {
    initEventHandlers();
    initModalEventHandlers(); // Yeni eklenen
  },
