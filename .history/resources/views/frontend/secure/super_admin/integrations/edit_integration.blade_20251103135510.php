...
<!-- API Fields Örnekleri Modal - FORM DIŞINDA (Aynı modal create'teki ile aynı) -->
...
// Logo önizleme (sadece edit formunda)
...
// Modal açma fonksiyonu - DÜZELTİLMİŞ
function openApiFieldsModal() {
    // Mevcut açık modal varsa backdrop sayısını sakla
    ...
    // Modal açıldıktan sonra z-index düzeltmesi
    setTimeout(function() {
        var modalBackdrops = $('.modal-backdrop');
        if (modalBackdrops.length > 1) {
            // İlk backdrop (ana modal) için z-index
            $(modalBackdrops[0]).css('z-index', '1050');
            // İkinci backdrop (API fields modal) için z-index
            $(modalBackdrops[1]).css('z-index', '1060');
        }
        // API Fields modalının z-index'ini ayarla
        $('#apiFieldsModal').css('z-index', '1061');
    }, 100);
}

// Modal kapatma event'i - backdrop temizliği
$('#apiFieldsModal').on('hidden.bs.modal', function (e) {
    // Eğer arkada başka modal varsa backdrop'u temizle
    if ($('.modal.show').length > 0) {
        $('body').addClass('modal-open');
        
        // Fazla backdrop'ları temizle
...
$(document).ready(function () {
    // Sayfa yüklendiğinde mevcut JSON'u validate et (sadece edit formunda)
    if ($('#api_fields').val().trim()) {
        validateJSON($('#api_fields')[0]);
    }
    
    // Form submit - CREATE İÇİN
