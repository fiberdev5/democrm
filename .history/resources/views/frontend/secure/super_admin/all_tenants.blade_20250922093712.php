...
// Modal kapatma işlemini daha güvenli hale getirin
$("#editTenantModal").on("hidden.bs.modal", function() {
    setTimeout(function() {
        // Başka bir modal açık değilse ve body'de modal-open class'ı yoksa temizle
        if (!$('.modal.show').length && !$('body').hasClass('modal-open')) {
            $('#editTenantModal .modal-body').html("");
        }
    }, 100); // Daha kısa timeout
});

// Payment detail modal için de benzer handling
$('#paymentDetailModal').on('hidden.bs.modal', function() {
    // Ana modal'ın backdrop'unu restore et
