          ...
          <div class="col-md-4">
            <div class="storage-details-card">
              ...
              <div class="storage-breakdown-list">
                @if(isset($storageInfo['service_photos_size_formatted']))
                  <div class="storage-item">
                    <div class="storage-type">Servis Fotoğrafları</div>
                    <div class="storage-size">{{ $storageInfo['service_photos_size_formatted'] }}</div>
                  </div>
                @endif
                @if(isset($storageInfo['stock_photos_size_formatted']))
                  <div class="storage-item">
                    <div class="storage-type">Stok Fotoğrafları</div>
                    <div class="storage-size">{{ $storageInfo['stock_photos_size_formatted'] }}</div>
                  </div>
                @endif
                @if(isset($storageInfo['other_files_size_formatted']))
                  <div class="storage-item">
                    <div class="storage-type">Diğer Dosyalar</div>
                    <div class="storage-size">{{ $storageInfo['other_files_size_formatted'] }}</div>
                  </div>
                @endif
                
                <!-- Eğer formatlı veriler yoksa, alternatif alan adları deneyelim -->
                @if(!isset($storageInfo['service_photos_size_formatted']) && isset($storageInfo['breakdown']))
                  @foreach($storageInfo['breakdown'] as $type => $size)
                    <div class="storage-item">
                      <div class="storage-type">
                        @if($type === 'service_photos')
                          Servis Fotoğrafları
                        @elseif($type === 'stock_photos')
                          Stok Fotoğrafları
                        @elseif($type === 'other_files')
                          Diğer Dosyalar
                        @else
                ...
                <!-- Debug için storageInfo içeriğini gösterelim -->
                @if(config('app.debug'))
                  <div class="storage-item debug">
                    <div class="storage-type">Debug - Storage Keys</div>
                    <div class="storage-size">{{ implode(', ', array_keys($storageInfo)) }}</div>
                  </div>
                @endif
                ...
                <div class="storage-item total">
                  ...
                  <div class="storage-size">{{ $storageInfo['current_usage_formatted'] ?? $storageInfo['total_usage_formatted'] ?? '0 B' }}</div>
                </div>
              </div>
            </div>
          </div>
          @else
          <!-- Debug için storageInfo yoksa bilgi gösterelim -->
          @if(config('app.debug'))
          <div class="col-md-4">
            <div class="storage-details-card">
              <h6 class="storage-card-title">Debug - Depolama Bilgisi Yok</h6>
              <div class="storage-breakdown-list">
                <div class="storage-item">
                  <div class="storage-type">StorageInfo Status</div>
                  <div class="storage-size">{{ isset($storageInfo) ? 'Var ama boş' : 'Tanımlı değil' }}</div>
                </div>
                @if(isset($storageInfo))
                  <div class="storage-item">
                    <div class="storage-type">StorageInfo Type</div>
                    <div class="storage-size">{{ gettype($storageInfo) }}</div>
                  </div>
                @endif
              </div>
            </div>
