                ...
                <div class="card-body p-3">
                  <div class="row align-items-center">
                    <div class="col-md-3">
                      <div class="d-flex align-items-center">
                        <div class="icon bg-light text-primary rounded-circle d-flex justify-content-center align-items-center me-3" style="width:50px; height:50px;">
                          <i class="{{ $tenant->activeSubscription->plansubs->getIconClass() }}"></i>
                        </div>
                        <div>
                          <h5 class="mb-0">{{ $tenant->activeSubscription->plansubs->name }}</h5>
                          <small class="text-muted">{{ $tenant->activeSubscription->plansubs->getBillingCycleText() }}</small>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="col-md-2 text-center">
                      <div class="mb-1">
                        {!! $tenant->activeSubscription->getStatusBadge() !!}
                      </div>
                      <small class="text-muted">Durum</small>
                    </div>
                    
                    <div class="col-md-2 text-center">
                      <div class="fw-bold text-primary fs-5">{{ $tenant->activeSubscription->plansubs->getFormattedPrice() }}</div>
                      <small class="text-muted">Fiyat</small>
                    </div>
                    
                    <div class="col-md-2 text-center">
                      @if($tenant->activeSubscription->isOnTrial())
                        <div class="fw-bold text-info">{{ $tenant->activeSubscription->getRemainingDays() }} gün</div>
                        <small class="text-muted">Deneme kaldı</small>
                      @elseif($tenant->activeSubscription->isActive())
                        <div class="fw-bold text-success">{{ $tenant->activeSubscription->getRemainingDays() }} gün</div>
                    ...
                    <div class="col-md-3 text-center">
                      <div class="fw-bold">{{ $tenant->activeSubscription->ends_at->format('d.m.Y') }}</div>
                      <small class="text-muted">Bitiş Tarihi</small>
                    </div>
                  </div>
                  ...
                  <div class="mt-3">
                    ...
                    <div class="row g-2">
                      @if($tenant->activeSubscription->plansubs->limits)
                        @foreach($tenant->activeSubscription->plansubs->limits as $key => $limit)
                          <div class="col-md-3">
                            <div class="small">
                              <strong>{{ ucfirst(str_replace('_', ' ', $key)) }}:</strong>
                              {{ $tenant->activeSubscription->plansubs->getFeatureText($key) }}
                            </div>
                          </div>
                        @endforeach
                      @endif
