...
<script>
  $(document).ready(function() {
   ...
   function startImpersonation(userId, reason) {
        $('#confirmImpersonationBtn').prop('disabled', true).html(`
        ...
        .fail(function(xhr) {
            console.error('Impersonation hatası:', xhr.responseText);
            var error = xhr.responseJSON?.message || 'Bir hata oluştu';
            showNotification(error, 'danger');
        })
        .always(function() {
            $('#confirmImpersonationBtn').prop('disabled', false).html(`
                <i class="fas fa-user-secret me-1"></i>Giriş Yap
            `);
        });
    }
