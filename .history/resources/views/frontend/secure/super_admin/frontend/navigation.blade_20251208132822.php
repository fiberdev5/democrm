...
// NAVBAR FORM - Güncelle
$('#navbarForm').on('submit', function(e) {
    e.preventDefault();
    
    // FormData kullan (image upload için)
    const formData = new FormData();
    const logoFile = $('#navbar_logo_file')[0].files[0];
    
    // Mevcut menu_items'ı koru
    const existingNavbar = {!! json_encode($navbar->content ?? []) !!};
    
    const content = {
        logo: $('#navbar_logo_current').val(), // Mevcut logo path
        menu_items: existingNavbar.menu_items || [],
        login_button: {
            text: $('#login_text').val(),
            icon: $('#login_icon').val(),
            url: $('#login_url').val(),
    ...
    // Eğer yeni logo yüklenmişse ekle
    if (logoFile) {
    ...
    $.ajax({
        ...
        success: function(response) {
            toastr.success('Navbar ayarları güncellendi');
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-2 menu-url-field">
                        <label class="form-label">URL</label>
                        <input type="text" class="form-control menu-url" placeholder="/">
                    </div>
                </div>
...
// NAVBAR FORM - Güncelle (menüleri dahil et)
$('#navbarForm').on('submit', function(e) {
    e.preventDefault();
    ...
    $('.menu-item').each(function() {
        const type = $(this).find('.menu-type').val();
        const item = {
            title: $(this).find('.menu-title').val(),
            type: type
        };
        
        if (type === 'link') {
            item.url = $(this).find('.menu-url').val();
        } else {
            // Dropdown items'ı parse et
            const dropdownText = $(this).find('.dropdown-items').val();
            const items = [];
