...
<div class="tab-pane fade" id="service-stats">
  <div class="service-stats-wrapper">
    <!-- Toplam Servis -->
    <div class="service-total-info">
      <div class="service-total-label">Toplam Servis</div>
      <div class="service-total-number">{{ $topServisSayisi ?? 0 }}</div>
    </div>

    <!-- Period Tabs -->
    <div class="service-period-tabs">
      @foreach($periodStats as $key => $period)
        <button class="service-period-item {{ $key === 'bugun' ? 'period-selected' : '' }}" 
                data-period="{{ $key }}"
                data-target="#period-{{ $key }}">
          <span class="service-period-title">{{ $period['label'] }}</span>
          <span class="service-period-value">{{ $period['toplam'] }}</span>
        </button>
      @endforeach
    </div>
    ...
    <div class="service-content-grid">
      ...
      <div class="service-data-box">
        <div class="service-data-header">
          <h6 class="service-data-title">
            <i class="fas fa-tags"></i>
            Markalar
          </h6>
        </div>
        <div class="service-data-list">
          @forelse($periodStats['bugun']['markalar'] as $marka)
            <div class="service-data-row">
              <span class="service-data-name">{{ $marka->marka }}</span>
              <span class="service-data-count">{{ $marka->sayi }}</span>
            </div>
          @empty
      ...
      <div class="service-data-box">
        ...
        <div class="service-data-list">
          @forelse($periodStats['bugun']['turler'] as $tur)
            <div class="service-data-row">
              <span class="service-data-name">{{ $tur->cihaz }}</span>
              <span class="service-data-count">{{ $tur->sayi }}</span>
            </div>
          @empty
            <div class="service-empty-message">
              <i class="fas fa-mobile-alt"></i>
      ...
      <div class="service-data-box">
        ...
        <div class="service-data-list">
          @forelse($periodStats['bugun']['kaynaklar'] as $kaynak)
            <div class="service-data-row">
              <span class="service-data-name">{{ $kaynak->kaynak }}</span>
              <span class="service-data-count">{{ $kaynak->sayi }}</span>
            </div>
          @empty
            <div class="service-empty-message">
              <i class="fas fa-route"></i>
      ...
      <div class="service-data-box">
        ...
        <div class="service-data-list">
          @forelse($periodStats['bugun']['operatorler'] as $operator)
            <div class="service-data-row">
              <span class="service-data-name">{{ $operator->name }}</span>
              <span class="service-data-count">{{ $operator->sayi }}</span>
            </div>
          @empty
            <div class="service-empty-message">
              <i class="fas fa-users"></i>
...
// Servis period switching - basit versiyon
$(document).off('click', '.service-period-item').on('click', '.service-period-item', function(e) {
  e.preventDefault();
  
  var period = $(this).data('period');
  
  // Tab navigation
  $('.service-period-item').removeClass('period-selected');
  $(this).addClass('period-selected');
  
  // Veriyi dinamik olarak yükleme kısmını burada ekleyebilirsiniz
  console.log('Period değişti:', period);
});
