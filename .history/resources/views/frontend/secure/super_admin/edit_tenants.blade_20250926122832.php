...
<div class="storage-section">
    <div class="section-header">
        ...
        @php
            // ===========================================
            // DEBUG BLOKU - Geçici olarak ekleyin
            // ===========================================
            echo "<div style='background: #ff4444; color: white; padding: 10px; margin: 10px 0; font-size: 12px; border-radius: 5px;'>";
            echo "<strong>🔍 DEBUG - StorageInfo İçeriği:</strong><br>";
            
            if(isset($storageInfo) && is_array($storageInfo)) {
                echo "<pre style='background: rgba(255,255,255,0.2); padding: 10px; margin: 5px 0; border-radius: 3px; font-size: 11px;'>";
                foreach($storageInfo as $key => $value) {
                    echo $key . " => " . (is_string($value) || is_numeric($value) ? $value : gettype($value)) . "\n";
                }
                echo "</pre>";
            } else {
                echo "<strong>❌ storageInfo bulunamadı veya array değil!</strong><br>";
                echo "Type: " . gettype($storageInfo ?? null) . "<br>";
                echo "İçerik: " . json_encode($storageInfo ?? null);
            }
            echo "</div>";
            ...
            // Debug: Backend'den gelen yüzde
            echo "<div style='background: #4444ff; color: white; padding: 8px; margin: 5px 0; border-radius: 3px;'>";
            echo "🔢 Backend Usage Percentage: <strong>" . $storageUsagePercent . "%</strong>";
            echo "</div>";
            ...
            if ($storageUsagePercent <= 0) {
                ...
                // Debug: String değerler
                echo "<div style='background: #44aa44; color: white; padding: 8px; margin: 5px 0; border-radius: 3px;'>";
                echo "📊 Manuel Hesaplama Başladı:<br>";
                echo "• Usage String: '<strong>" . $usageFormatted . "</strong>'<br>";
                echo "• Limit String: '<strong>" . $limitFormatted . "</strong>'";
                echo "</div>";
                
                // Tüm durumları kapsayan hesaplama
                $usageBytes = 0;
                $limitBytes = 0;
                
                // Kullanılan alan hesabı
                if (strpos($usageFormatted, 'KB') !== false) {
                    $usageValue = floatval(str_replace(['KB', ' '], '', $usageFormatted));
                    $usageBytes = $usageValue * 1024;
                    echo "<div style='background: #aa44aa; color: white; padding: 5px; margin: 2px 0;'>📈 KB: " . $usageValue . " → " . number_format($usageBytes) . " bytes</div>";
                } elseif (strpos($usageFormatted, 'MB') !== false) {
                    $usageValue = floatval(str_replace(['MB', ' '], '', $usageFormatted));
                    $usageBytes = $usageValue * 1024 * 1024;
                    echo "<div style='background: #aa44aa; color: white; padding: 5px; margin: 2px 0;'>📈 MB: " . $usageValue . " → " . number_format($usageBytes) . " bytes</div>";
                } elseif (strpos($usageFormatted, 'GB') !== false) {
                    $usageValue = floatval(str_replace(['GB', ' '], '', $usageFormatted));
                    $usageBytes = $usageValue * 1024 * 1024 * 1024;
                    echo "<div style='background: #aa44aa; color: white; padding: 5px; margin: 2px 0;'>📈 GB: " . $usageValue . " → " . number_format($usageBytes) . " bytes</div>";
                } else {
                    $usageBytes = floatval(str_replace([' ', 'B'], '', $usageFormatted));
                    echo "<div style='background: #aa44aa; color: white; padding: 5px; margin: 2px 0;'>📈 B: " . $usageBytes . " bytes</div>";
                }
                
                // Limit hesabı
                if (strpos($limitFormatted, 'KB') !== false) {
                    $limitValue = floatval(str_replace(['KB', ' '], '', $limitFormatted));
                    $limitBytes = $limitValue * 1024;
                    echo "<div style='background: #aa8844; color: white; padding: 5px; margin: 2px 0;'>📊 Limit KB: " . $limitValue . " → " . number_format($limitBytes) . " bytes</div>";
                } elseif (strpos($limitFormatted, 'MB') !== false) {
                    $limitValue = floatval(str_replace(['MB', ' '], '', $limitFormatted));
                    $limitBytes = $limitValue * 1024 * 1024;
                    echo "<div style='background: #aa8844; color: white; padding: 5px; margin: 2px 0;'>📊 Limit MB: " . $limitValue . " → " . number_format($limitBytes) . " bytes</div>";
                } elseif (strpos($limitFormatted, 'GB') !== false) {
                    $limitValue = floatval(str_replace(['GB', ' '], '', $limitFormatted));
                    $limitBytes = $limitValue * 1024 * 1024 * 1024;
                    echo "<div style='background: #aa8844; color: white; padding: 5px; margin: 2px 0;'>📊 Limit GB: " . $limitValue . " → " . number_format($limitBytes) . " bytes</div>";
                } else {
                    $limitBytes = floatval(str_replace([' ', 'B'], '', $limitFormatted));
                    echo "<div style='background: #aa8844; color: white; padding: 5px; margin: 2px 0;'>📊 Limit B: " . $limitBytes . " bytes</div>";
                }
                
                // Yüzde hesapla
                if ($limitBytes > 0) {
                    $oldPercent = $storageUsagePercent;
                    $storageUsagePercent = ($usageBytes / $limitBytes) * 100;
                    $storageUsagePercent = round($storageUsagePercent, 3);
                    
                    echo "<div style='background: #ff8844; color: white; padding: 8px; margin: 5px 0; border-radius: 3px; font-weight: bold;'>";
                    echo "🎯 SONUÇ: " . number_format($usageBytes) . " / " . number_format($limitBytes) . " = <strong>" . $storageUsagePercent . "%</strong><br>";
                    echo "Eski: " . $oldPercent . "% → Yeni: " . $storageUsagePercent . "%";
                    echo "</div>";
                } else {
                    echo "<div style='background: #ff4444; color: white; padding: 8px; margin: 5px 0;'>❌ Limit bytes sıfır!</div>";
                }
            } else {
                echo "<div style='background: #888888; color: white; padding: 8px; margin: 5px 0;'>ℹ️ Backend'den yüzde geldi, manuel hesaplama yapılmadı</div>";
            }
    ...
    <div class="storage-progress-compact">
        <div class="storage-stats">
            <div class="stat">
                <span class="value">{{ number_format($storageUsagePercent, 2) }}%</span>
        ...
        <div class="progress-bar-simple">
            <div class="progress-fill 
                ...
                @else
                    bg-success
                @endif" style="width: {{ min($storageUsagePercent, 100) }}%">
            </div>
        </div>
