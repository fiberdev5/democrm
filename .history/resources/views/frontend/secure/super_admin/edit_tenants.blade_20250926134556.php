...
function loadStorageDetails() {
  ...
  console.log('Storage Info:', response.storage_info);
  console.log('Total Limit GB:', response.storage_info?.total_limit_gb);
  console.log('Current Usage:', response.storage_info?.current_usage_formatted);
  
  // Geçici olarak sadece bu bilgileri göster
  if (response.success) {
    container.html(`
      <div class="storage-breakdown-list">
        <div class="storage-item">
          <div class="storage-type">Debug Test</div>
          <div class="storage-size">Usage: ${response.storage_info?.current_usage_formatted || 'YOK'}</div>
        </div>
        <div class="storage-item">
          <div class="storage-type">Debug Test 2</div>
          <div class="storage-size">Limit: ${response.storage_info?.total_limit_gb || 'YOK'} GB</div>
        </div>
      </div>
    `);
  }
