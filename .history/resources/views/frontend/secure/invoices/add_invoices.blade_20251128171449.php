...
<script>
  $(document).ready(function (e) {
    ...
    function kdvHesapla(toplam){
      var indirim = Number($(".indirim").val()) || 0; // Boşsa 0
      var kdvTutar = Number($(".kdvTutar").val()) || 20; // Boşsa 20
      var tevkifatOrani = Number($(".tevkifatOrani").val()) || 0; // Boşsa 0
    ...
    $('.kdvTutar').on('keyup', function() {
      // Eğer henüz ürün eklenmemişse hesaplama yapma
      if (sonucToplam === 0) {
        return;
      }
    ...
    $('.tevkifatOrani').on('change', function() {
      // Eğer henüz ürün eklenmemişse hesaplama yapma
      if (sonucToplam === 0) {
        // Sadece tevkifat tutarını sıfırla
        $(".tevkifatTutari").val(0);
        return;
      }
