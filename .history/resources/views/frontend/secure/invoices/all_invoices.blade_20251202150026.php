    ...
    $('#datatableInvoice').on('click', '.checkFormalizationStatus', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    const typeText = response.type === 'e-invoice' ? 'e-Fatura' : 'e-Arşiv';
                    let message = `Tür: ${typeText}\nDurum: ${response.status_text}`;
                    
                    if (response.pdf_url) {
                        message += '\n\nPDF hazır! İndirmek ister misiniz?';
                        if (confirm(message)) {
                    ...
                    alert('❌ ' + (response.message || 'Durum kontrol edilemedi'));
                }
            },
            error: function(xhr) {
                alert('❌ Durum kontrol edilemedi');
            },
            complete: function() {
                $btn.prop('disabled', false).html(originalHtml);
            }
