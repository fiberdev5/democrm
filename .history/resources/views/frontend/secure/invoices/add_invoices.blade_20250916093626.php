            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.storage_warning) {
                        setTimeout(function() {
                            showNotification(response.storage_warning, 'warning');
                        }, 1000);
                    }
            ...
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    if (errorResponse && errorResponse.error_type === 'storage_limit_exceeded') {
                        ...
                        var errorMessages = Object.values(errorResponse.errors).flat();
                        showNotification(errorMessages.join(' '), 'error');
                    } else {
                        showNotification('Form doğrulama hatası', 'error');
                    }
                } else {
