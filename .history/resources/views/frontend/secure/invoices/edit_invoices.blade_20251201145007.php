...
$(document).ready(function() {
    ...
    function loadRemainingAmount(invoiceId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    const remaining = parseFloat(response.remaining_amount || 0).toFixed(2);
                    $('#remainingAmount').text(remaining);
                    $('#paymentAmount').attr('max', remaining);
                    
                    // ⭐ Eğer kalan tutar 0 ise uyarı göster
                    if (parseFloat(remaining) <= 0) {
                        $('#remainingAmount').parent().html(
                            '<span class="text-success"><i class="fas fa-check-circle"></i> Fatura tamamen ödendi</span>'
                        );
                    }
                } else {
                    $('#remainingAmount').text('Hesaplanamadı');
                }
            },
            error: function(xhr) {
                if (xhr.status === 404) {
                    $('#remainingAmount').text('Fatura bulunamadı');
                    alert('Bu fatura Paraşüt\'te bulunamadı.');
                } else {
                    $('#remainingAmount').text('Hata');
                }
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // ⭐ Sayfayı yenile - Böylece "Tahsilat Ekle" butonu kaybolur
                    window.location.reload();
                    
                    // Veya DataTable varsa:
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.payments.length > 0) {
                    ...
                    html += '<thead><tr><th>Tarih</th><th>Tutar</th><th>Açıklama</th><th>Tahsilat ID</th></tr></thead><tbody>';
                    ...
                    // ⭐ Veritabanında kayıtlı ID'ler
                    if (response.stored_payment_ids && response.stored_payment_ids.length > 0) {
                        html += `<div class="alert alert-info mt-2">
                            <small><strong>Veritabanında Kayıtlı:</strong> ${response.stored_payment_ids.join(', ')}</small>
                        </div>`;
                    }
