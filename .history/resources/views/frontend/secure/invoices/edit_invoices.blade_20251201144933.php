...
<div class="row form-group" style="border:0">
    <div class="col-md-4 rw1"><label>Tahsilat İşlemleri</label></div>
    <div class="col-md-8 rw2">
        {{-- ⭐ TAHSİLAT YOKSA: Tahsilat Ekle + Tahsilatları Gör --}}
        @if(!$invoice_id->has_payment)
            ...
            <small class="text-muted d-block mt-1">
                <i class="fas fa-info-circle"></i> Henüz tahsilat yapılmamış
            </small>
        @else
            {{-- ⭐ TAHSİLAT VARSA: Sadece Tahsilatları Gör --}}
            <button type="button" class="btn btn-info btn-sm tahsilatlariGorBtn" data-invoice-id="{{$invoice_id->id}}">
                <i class="fas fa-list"></i> Tahsilatları Gör
            </button>
            <small class="text-success d-block mt-1">
                <i class="fas fa-check-circle"></i> Tahsilat yapılmış 
        ...
        @if($invoice_id->parasutInvoiceId)
            <small class="text-success d-block mt-1">
                <i class="fas fa-check-circle"></i> Paraşüt ID: {{$invoice_id->parasutInvoiceId}}
            </small>
        @else
            <small class="text-warning d-block mt-1">
                <i class="fas fa-exclamation-circle"></i> Paraşüt ID bulunamadı
            </small>
        @endif
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="tahsilatForm">
                    ...
                    <input type="hidden" id="parasutInvoiceId" name="parasut_invoice_id">
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Kalan Tutar: <span id="remainingAmount">-</span> TL</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="paymentDescription" name="description" rows="2" placeholder="Ödeme açıklaması"></textarea>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    $('.tahsilatEkleBtn').on('click', function() {
        ...
        const parasutId = $(this).data('parasut-id');
        
        $('#invoiceId').val(invoiceId);
        $('#parasutInvoiceId').val(parasutId);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.accounts) {
                    parasutAccounts = response.accounts;
            ...
            error: function() {
                ...
                alert('Hesaplar yüklenemedi. Lütfen tekrar deneyin.');
            }
        });
    }
    ...
    function loadRemainingAmount(invoiceId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    $('#remainingAmount').text(parseFloat(response.remaining_amount || 0).toFixed(2));
                    $('#paymentAmount').attr('max', response.remaining_amount);
                }
            }
        });
    }
    ...
    $('#tahsilatKaydetBtn').on('click', function() {
        ...
        const formData = {
            ...
            parasut_invoice_id: $('#parasutInvoiceId').val(),
        ...
        $.ajax({
            ...
            complete: function() {
                $('#tahsilatKaydetBtn').prop('disabled', false).html('<i class="fas fa-save"></i> Kaydet');
            }
        });
    });
    ...
    $('.tahsilatlariGorBtn').on('click', function() {
        ...
        $('#paymentsListContainer').html('<div class="text-center"><div class="spinner-border text-primary" role="status"></div></div>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.payments.length > 0) {
                    ...
                    html += '<thead><tr><th>Tarih</th><th>Tutar</th><th>Açıklama</th></tr></thead><tbody>';
                    ...
                    html += `<div class="alert alert-info mt-3">
                        <strong>Kalan Tutar:</strong> ${parseFloat(response.remaining_amount || 0).toFixed(2)} TL
                    </div>`;
            ...
            error: function() {
                $('#paymentsListContainer').html('<div class="alert alert-danger">Tahsilatlar yüklenemedi.</div>');
            }
        });
    });
});
