     ...
     <div class="card-body">
        <div class="row">
           <div class="col-sm-6 s1">
              ...
              <div class="kisaMusteriBil" style="font-size: 14px; display: flex; flex-wrap: wrap; gap: 15px;">
    <span><strong>{{ $invoice_id->customer->adSoyad }}</strong></span>
    @if(!empty($invoice_id->customer?->tcNo))
        <span>TC: 11111111111</span>
    @endif
    <span>{{ $invoice_id->customer?->adres }}</span>
    @if(!empty($invoice_id->customer?->vergiNo))
        <span>VERGİ NO: 121212</span>
    @endif
    @if(!empty($invoice_id->customer?->vergiDairesi))
        <span>VERGİ DAİRESİ: Bursa</span>
    @endif
</div>
