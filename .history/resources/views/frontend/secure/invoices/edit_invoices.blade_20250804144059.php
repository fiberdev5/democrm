           ...
           <div class="col-sm-6 s1">
              <div class="row form-group">
                ...
                <div class="col-md-8 rw2 d-flex flex-wrap align-items-center gap-2">
                    ...
                    <a href="{{ url($firma->id . '/servisler') }}" target="_blank" class="servisiAc btn btn-outline-danger px-2 py-1"style="font-size: 13px; line-height: 1.3;">Servisi Aç</a>
                </div>
              </div>
              <div class="row form-group">
  ...
  <script>
    $(document).ready(function () {
        $('#search').on('input', function () {
            let servisId = $(this).val().trim();
            let baseUrl = "{{ url($firma->id . '/servisler') }}";

            if (servisId) {
                $('.servisiAc').attr('href', baseUrl + '?did=' + encodeURIComponent(servisId));
            } else {
                $('.servisiAc').attr('href', baseUrl);
            }
        });

        // Sayfa yüklendiğinde de doğru linki ayarla (varsa değer)
