    ...
    <div class="col-md-8 col-sm-6">
        @if($hasParasutIntegration)
            <div class="d-flex flex-wrap align-items-center gap-2" id="tahsilatButtonsContainer">
                {{-- ✅ Backend'den gelen duruma göre göster --}}
                @if($paymentInfo['status'] === 'paid')
                    {{-- Tamamen ödendi - buton gizli --}}
                    <button type="button" class="btn btn-success btn-sm tahsilatEkleBtn d-none" data-invoice-id="{{$invoice_id->id}}">
                        <i class="fas fa-plus"></i> Tahsilat Ekle
                    </button>
                @elseif($paymentInfo['status'] === 'not_sent')
                    {{-- Henüz Paraşüt'e gönderilmemiş --}}
                    <button type="button" class="btn btn-success btn-sm tahsilatEkleBtn" data-invoice-id="{{$invoice_id->id}}" disabled title="Önce faturayı Paraşüt'e gönderin">
                        <i class="fas fa-plus"></i> Tahsilat Ekle
                    </button>
                @else
                    {{-- Gönderilebilir durumda --}}
                    <button type="button" class="btn btn-success btn-sm tahsilatEkleBtn" data-invoice-id="{{$invoice_id->id}}">
                        <i class="fas fa-plus"></i> Tahsilat Ekle
                    </button>
                @endif
                
                {{-- Tahsilatları Gör Butonu --}}
                @if($invoice_id->has_payment && $paymentInfo['payment_count'] > 0)
                    <button type="button" class="btn btn-info btn-sm tahsilatlariGorBtn" data-invoice-id="{{$invoice_id->id}}">
                        <i class="fas fa-list"></i> Tahsilatları Gör ({{$paymentInfo['payment_count']}} adet)
                    </button>
                @endif
                
                {{-- ✅ Durum bilgisi - Backend'den geldi --}}
                <small class="text-muted" id="paymentStatusText">
                    @if($paymentInfo['status'] === 'paid')
                        <i class="fas fa-check-circle text-success"></i> Fatura tamamen ödendi
                    @elseif($paymentInfo['status'] === 'partial')
                        <i class="fas fa-exclamation-circle text-warning"></i> Kısmi ödeme yapılmış - Kalan: <strong>{{ number_format($paymentInfo['remaining_amount'], 2) }} TL</strong>
                    @elseif($paymentInfo['status'] === 'sent')
                        <i class="fas fa-info-circle text-muted"></i> Henüz tahsilat yapılmamış
                    @elseif($paymentInfo['status'] === 'not_sent')
                        <i class="fas fa-info-circle text-info"></i> Henüz Paraşüt'e gönderilmemiş
                    @else
...
<script>
$(document).ready(function() {
    let remainingAmountGlobal = {{ $paymentInfo['remaining_amount'] ?? 0 }};
    let accountsCache = null; // ✅ Hesapları cache'le
    let isLoadingPayments = false; // ✅ Çift yüklemeyi önle

    // ✅ SADECE 'unknown' durumundaysa AJAX çağır
    ...
    // ✅ Sayfa yüklendiğinde kalan tutarı kontrol et (Debounced)
    let checkRemainingTimeout;
    function debouncedCheckRemaining() {
        clearTimeout(checkRemainingTimeout);
        checkRemainingTimeout = setTimeout(checkRemainingAmountOnLoad, 300);
    }

    debouncedCheckRemaining();

    function checkRemainingAmountOnLoad() {
        const invoiceId = $('.tahsilatEkleBtn').data('invoice-id');
        if (!invoiceId || isLoadingPayments) return;
    ...
    $('.tahsilatEkleBtn').on('click', function() {
        ...
        // ✅ Hesaplar cache'deyse direkt kullan
        if (accountsCache) {
            populateAccounts(accountsCache);
        } else {
            loadParasutAccounts();
        }
    ...
    // ✅ Hesapları cache'leyerek yükle
    function loadParasutAccounts() {
        const firmaId = {{ $firma->id }};
        
        // Loading göster
        $('#accountId').html('<option value="">Yükleniyor...</option>');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.accounts) {
                    accountsCache = response.accounts; // ✅ Cache'e kaydet
    ...
    // ✅ Kalan tutarı yükle (Optimistic UI)
    function loadRemainingAmount(invoiceId) {
        const firmaId = {{ $firma->id }};
        
        // ✅ Optimistic: Yükleniyor göster
        $('#remainingAmountDisplay').text('Hesaplanıyor...');
        $('#tahsilatKaydetBtn').prop('disabled', true);
    ...
    $('#tahsilatKaydetBtn').on('click', function() {
        ...
        // ✅ Optimistic UI: Butonu devre dışı bırak
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (typeof $('#datatableInvoice').DataTable === 'function') {
                        $('#datatableInvoice').DataTable().ajax.reload(null, false); // ✅ false = sayfada kal
                    }
                    
                    // ✅ Edit modalını yenile
                    reloadEditModal(formData.invoice_id, firmaId);
                } else {
