              ...
              <select class="form-select tevkifatKodu" name="tevkifatKodu">
                ...
                @if(isset($tevkifatKodlari))
                  @foreach($tevkifatKodlari as $kod)
                    <option value="{{$kod->id}}">{{$kod->kodu}} - {{$kod->adi}}</option>
                  @endforeach
                @endif
              </select>
            </div>
...
<script>
 $(document).ready(function (e) {
    ...
    // Sayfa ilk yüklendiğinde tevkifat tutarını sıfırla
    $(".tevkifatTutari").val('0.00');
    ...
    function kdvHesapla(toplam){
      ...
      var kdvTutar = Number($(".kdvTutar").val()) || 20;
      var tevkifatOrani = Number($(".tevkifatOrani").val()) || 0;
      ...
      if (tevkifatOrani > 0) {
        ...
        // TEVKİFAT ORANI 0 İSE TEVKİFAT TUTARI DA 0 OLMALI
        tevkifatHesapla = 0;
      ...
      // HER İKİ TEVKİFAT ALANI İÇİN AYRI SET ET
      $('input.tevkifatTutari[disabled]').val(tevkifatHesapla.toFixed(2));
      $('input[type="hidden"][name="tevkifatTutari"]').val(tevkifatHesapla.toFixed(2));
    ...
    $('.kdvTutar').on('keyup', function() {
      ...
      if (tevkifatOrani > 0) {
        ...
        tevkifatHesapla = 0; // EKSİK OLAN KISIM
        genelToplam = araToplam + kdv;
      }
      ...
      $(".araToplam").val(araToplam.toFixed(2));
      $(".kdv").val(kdv);
      $(".tevkifatTutari[disabled]").val(tevkifatHesapla);
      $("input[type='hidden'].tevkifatTutari").val(tevkifatHesapla);
      $(".genelToplam").val(genelToplam);
    });
    ...
    // Tevkifat oranı değiştiğinde hesaplama
    $('.tevkifatOrani').on('change', function() {
      var tevkifatOrani = Number($(this).val()) || 0;
      
      // Eğer sonucToplam 0 ise, tevkifat tutarını sıfırla
      if (sonucToplam === 0) {
        $('input.tevkifatTutari[disabled]').val('0.00');
        $('input[type="hidden"][name="tevkifatTutari"]').val('0.00');
        return;
      }
      ...
      if (tevkifatOrani > 0) {
        ...
        tevkifatHesapla = 0; // AÇIKÇA 0 OLARAK BELİRT
        genelToplam = araToplam + kdv;
      }
