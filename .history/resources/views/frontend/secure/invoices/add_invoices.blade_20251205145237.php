...
$(document).ready(function() {
    ...
    function hesaplaGenelToplam() {
        ...
        // ✅ Düzeltilmiş seçici - index bazlı class kullanımı
        var satirSayisi = $form.find('.satirBody .miktar').length;
        ...
        var kdvOrani = parseFloat($form.find(".kdvTutar").val()) || 20;
        ...
        $form.find(".kdv").val(kdvTutari.toFixed(2));
        $form.find(".tevkifatTutari").val(tevkifatTutari.toFixed(2));
        $form.find(".genelToplam").val(genelToplam.toFixed(2));
        
        console.log('Hesaplama:', { toplam, kdvOrani, kdvTutari, tevkifatOrani, tevkifatTutari, genelToplam });
    }
    
    // ⭐ Satır değerleri değiştiğinde (delegated event - yeni eklenen satırlar için de çalışır)
    $form.on('keyup change', '.miktar, .fiyat', function() {
        // İlgili satırın tutarını hesapla
        var $row = $(this).closest('.row');
        var miktar = parseFloat($row.find('.miktar').val()) || 0;
        var fiyat = parseFloat($row.find('.fiyat').val()) || 0;
    ...
    $form.find('.kdvKodu').on('change', function() {
        ...
        console.log('KDV Kodu değişti:', { kod: $(this).val(), oran: kdvOrani });
    ...
    $form.find('.kdvTutar').on('keyup change', function() {
        hesaplaGenelToplam();
    });
    ...
    $form.find('.tevkifatKodu').on('change', function() {
        ...
        console.log('Tevkifat Kodu değişti:', { kod: $(this).val(), oran: tevkifatOrani });
        
        // ✅ Oranı güncelle
        $tevkifatOraniSelect.val(tevkifatOrani);
        
        if (tevkifatOrani > 0 && currentOran !== tevkifatOrani) {
            Swal.fire({
