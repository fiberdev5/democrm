                ...
                <div class="col-md-8 rw2">
                    <div class="btnWrap">
                        ...
                        @elseif($invoice_id->formalization_status === 'pending')
                            <button type="button" class="btn btn-warning btn-block btn-sm checkPdfBtn" data-invoice-id="{{ $invoice_id->id }}">
...
$(document).ready(function() {
    ...
    $form.find('.tevkifatKodu').on('change', function() {
        ...
        if (tevkifatOrani > 0) {
            // Tevkifat kodu seçildi
            if (currentOran !== tevkifatOrani) {
                // Oranlar uyuşmuyor, uyarı göster ve otomatik ayarla
                Swal.fire({
                ...
                $tevkifatOraniSelect.val(tevkifatOrani).trigger('change');
            }
        } else {
            // Tevkifat kodu kaldırıldı
            $tevkifatOraniSelect.val(0).trigger('change');
        }
    });
    
    // ⭐ Tevkifat Oranı manuel değiştirildiğinde uyarı
    $form.find('.tevkifatOrani').on('change', function() {
        ...
        if (selectedKodu) {
            ...
            if (selectedOran !== koduOrani && selectedOran > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $(this).val(koduOrani).trigger('change');
                        hesaplaGenelToplam();
                    }
                });
            }
    ...
    $form.find('.kdvKodu').on('change', function() {
        ...
        if (kdvOrani !== undefined && kdvOrani !== '') {
            const currentKdv = parseInt($kdvTutarInput.val()) || 20;
            
            if (parseInt(kdvOrani) !== currentKdv) {
                Swal.fire({
                    title: 'KDV Oranı Güncellendi',
                    html: `Seçilen KDV istisna kodu için KDV oranı <strong>%${kdvOrani}</strong> olarak ayarlandı.`,
                    icon: 'info',
                    confirmButtonText: 'Tamam'
                });
                
                $kdvTutarInput.val(kdvOrani);
                hesaplaGenelToplam();
            }
        }
    });
    ...
    function hesaplaGenelToplam() {
        var toplam = parseFloat($form.find(".toplam").val()) || 0;
        var indirim = parseFloat($form.find(".indirim").val()) || 0;
        var kdvTutar = parseFloat($form.find(".kdvTutar").val()) || 20;
        ...
        $form.find(".kdv").val(kdv);
        $form.find(".tevkifatTutari").val(tevkifatHesapla);
        $form.find(".tevkifatTutariGoster").val(tevkifatHesapla);
        $form.find(".genelToplam").val(genelToplam);
    }
    ...
    $form.on('submit', function(e) {
        ...
        // Tevkifat oranı var ama kod yoksa uyar
