...
<script>
$(document).ready(function() {
    // ⭐ PDF/Durum Kontrol Butonu - İyileştirilmiş Versiyon
    $(document).on('click', '.checkPdfBtn', function() {
        ...
        $.ajax({
            ...
            type: 'GET',
            timeout: 15000, // AJAX timeout
            success: function(response) {
                // ✅ Timeout'u iptal et (başarılı response geldi)
                ...
                if (response.success) {
                    ...
                    if (response.status === 'sent' && response.pdf_url) {
                        ...
                        Swal.fire({
                            ...
                            html: `
                                ...
                                <p class="text-muted" style="font-size: 12px; margin-top: 10px;">
                                    <i class="fas fa-clock"></i> PDF son kullanma: ${response.pdf_expires_at ? new Date(response.pdf_expires_at).toLocaleString('tr-TR') : 'Belirtilmemiş'}
                                </p>
                            `,
                            icon: 'success',
                            showCancelButton: true,
                        ...
                        Swal.fire({
                            ...
                            html: `
                                <p>Fatura resmileştirildi ancak PDF henüz hazırlanıyor.</p>
                                ${response.invoice_number ? `<p><strong>Fatura No:</strong> ${response.invoice_number}</p>` : ''}
                                <p class="text-muted">Lütfen 30-60 saniye sonra tekrar deneyin.</p>
                            `,
                            icon: 'info',
                        ...
                        Swal.fire({
                            ...
                            html: `
                                <p>Resmileştirme işlemi henüz tamamlanmadı.</p>
                                <p class="text-muted">Paraşüt'te işlem yapılıyor, lütfen 1-2 dakika sonra tekrar kontrol edin.</p>
                            `,
                            icon: 'warning',
                            confirmButtonText: 'Tamam'
                        }).then(() => {
                        ...
                        Swal.fire({
                            title: 'Resmileştirme Hatası',
                            html: `
                                <p>Paraşüt'te fatura resmileştirilemedi.</p>
                                ${response.raw_status ? `<p class="text-muted"><small>Durum: ${response.raw_status}</small></p>` : ''}
                                <p class="text-danger">Lütfen Paraşüt panelinden kontrol edin.</p>
                            `,
                            icon: 'error',
                            showCancelButton: true,
                            confirmButtonText: 'Sayfayı Yenile',
                        ...
                        Swal.fire({
                            ...
                            text: 'Fatura durumu belirlenemedi.',
                            icon: 'question',
                            showCancelButton: true,
                            confirmButtonText: 'Sayfayı Yenile',
                            cancelButtonText: 'Kapat'
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <p>${response.message || 'Fatura durumu kontrol edilemedi.'}</p>
                            <p class="text-muted">Paraşüt bağlantısında sorun olabilir.</p>
                        `,
                        icon: 'error',
                        showCancelButton: true,
                        confirmButtonText: 'Sayfayı Yenile',
            ...
            error: function(xhr, status, error) {
                ...
                console.error('PDF kontrol hatası:', {
                    status: xhr.status,
                ...
                if (xhr.status === 0) {
                    ...
                    errorTitle = 'Fatura Bulunamadı';
                    errorMessage = 'Bu fatura Paraşüt\'te bulunamadı. Önce faturayı resmileştirmeniz gerekebilir.';
                } else if (xhr.status === 401 || xhr.status === 403) {
                    errorTitle = 'Yetkilendirme Hatası';
                    errorMessage = 'Paraşüt entegrasyonu yetkilendirmesi başarısız. Ayarlarınızı kontrol edin.';
                } else if (xhr.status === 500) {
                    errorTitle = 'Sunucu Hatası';
                    errorMessage = 'Paraşüt sunucusunda bir hata oluştu. Lütfen daha sonra tekrar deneyin.';
                } else if (xhr.status === 503 || xhr.status === 504) {
                    errorTitle = 'Servis Erişilemez';
                    errorMessage = 'Paraşüt servisi şu anda erişilemez durumda. Lütfen daha sonra tekrar deneyin.';
                } else if (status === 'timeout') {
                    errorTitle = 'Zaman Aşımı';
                    errorMessage = 'İstek çok uzun sürdü. Paraşüt sunucusu yanıt vermiyor.';
                } else if (status === 'parsererror') {
                    errorTitle = 'Veri Hatası';
                    errorMessage = 'Sunucudan gelen yanıt işlenemedi.';
                } else {
                    errorTitle = 'Bilinmeyen Hata';
                    errorMessage = xhr.responseJSON?.message || 'Durum kontrol edilemedi. Lütfen tekrar deneyin.';
                }
                ...
                Swal.fire({
                    ...
                    html: `
                        ...
                        <div class="text-muted" style="font-size: 12px; margin-top: 10px;">
                            <p>Hata Kodu: ${xhr.status || 'N/A'}</p>
                            ${error ? `<p>Detay: ${error}</p>` : ''}
                        </div>
                        <div class="alert alert-warning mt-3" style="font-size: 13px;">
                            <i class="fas fa-info-circle"></i> 
                            ${xhr.status === 404 ? 'Faturayı önce Paraşüt\'e gönderin ve resmileştirin.' : 'Paraşüt\'ten manuel kontrol yapabilirsiniz.'}
                        </div>
                    `,
                    icon: 'error',
                    showCancelButton: showReloadButton,
                    confirmButtonText: showReloadButton ? '<i class="fas fa-sync"></i> Sayfayı Yenile' : 'Tamam',
                    cancelButtonText: 'Kapat',
                    confirmButtonColor: '#dc3545',
                    allowOutsideClick: false
                }).then((result) => {
                    if (result.isConfirmed && showReloadButton) {
                        reloadEditModal(invoiceId, firmaId);
                    } else {
                        button.prop('disabled', false).html(originalHtml);
                    }
            ...
            complete: function() {
                // ✅ İstek tamamlandığında (başarılı veya başarısız) timeout'u temizle
                clearTimeout(timeoutId);
            }
        });
    });
    ...
    function reloadEditModal(invoiceId, firmaId) {
        ...
        Swal.fire({
            title: 'Yükleniyor...',
            html: 'Fatura bilgileri güncelleniyor',
            allowOutsideClick: false,
        ...
        $.ajax({
            ...
            error: function(xhr) {
                Swal.fire({
                    ...
                    text: 'Fatura bilgileri yüklenemedi. Sayfayı tamamen yenilemek ister misiniz?',
                    icon: 'error',
                    showCancelButton: true,
                    confirmButtonText: 'Sayfayı Yenile',
                    cancelButtonText: 'Kapat',
