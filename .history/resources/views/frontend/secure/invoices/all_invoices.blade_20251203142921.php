...
$(document).ready(function() {
    ...
    $('#datatableInvoice').on('click', '.checkFormalizationStatus', function() {
        const invoiceId = $(this).data('invoice-id');
    const button = $(this);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // ✅ PDF varsa özel dialog
                if (response.pdf_url && response.status === 'sent') {
                    Swal.fire({
                        title: 'Resmileştirme Durumu',
                        html: `
                            <div class="text-start">
                                <p><strong>Durum:</strong> <span class="text-success">${response.status_text}</span></p>
                                <p><strong>Tip:</strong> ${typeText}</p>
                                ${response.invoice_number ? `<p><strong>Fatura No:</strong> ${response.invoice_number}</p>` : ''}
                                <p><strong>PDF:</strong> <span class="text-success">Hazır</span></p>
                                ${response.pdf_expires_at ? `<p class="small text-muted">URL Son Kullanma: ${new Date(response.pdf_expires_at).toLocaleString('tr-TR')}</p>` : ''}
                            </div>
                        `,
                        icon: 'success',
                    ...
                    // PDF henüz yok veya pending
                    let iconType = 'info';
                    let message = `<div class="text-start">
                        <p><strong>Durum:</strong> ${response.status_text}</p>
                        <p><strong>Tip:</strong> ${typeText}</p>`;
                    
                    if (response.status === 'pending') {
                        message += `<p class="text-warning">PDF henüz hazırlanıyor, lütfen birkaç dakika sonra tekrar kontrol edin.</p>`;
                        iconType = 'warning';
                    } else if (response.status === 'error') {
                        message += `<p class="text-danger">Resmileştirmede hata oluştu.</p>`;
                        iconType = 'error';
                    }
