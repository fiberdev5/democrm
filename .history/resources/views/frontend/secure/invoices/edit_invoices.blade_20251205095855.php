      ...
      <div class="card card-invoices f3 h-100">
        <div class="card-body card-invoices-body">
          ...
          <div class="row form-group" style="border:0">
            ...
            <div class="col-md-8 rw2">
              <select class="form-select tevkifatKodu" name="tevkifatKodu" disabled>
          ...
          <div class="row form-group" style="border:0">
            ...
            <div class="col-md-8 rw2">
              <input type="text" name="faturaAciklama" class="form-control faturaAciklama" disabled value="{{$invoice_id->faturaAciklama ?? ''}}">
            </div>
          </div>
    ...
    <div class="col-md-8 rw2">
        <div class="btnWrap">
            ...
            @if($invoice_id->formalization_status === 'sent')
                {{-- PDF URL varsa görüntüle butonu --}}
                @if(!empty($invoice_id->formalization_pdf_url))
                    <a href="{{ $invoice_id->formalization_pdf_url }}" target="_blank" class="btn btn-success btn-sm">
                        <i class="far fa-file-pdf"></i> PDF'i Görüntüle
                    </a>
                    @if($invoice_id->formalization_pdf_expires_at)
                        <small class="text-muted d-block mt-1">
                            <i class="fas fa-clock"></i> 
                            Son Kullanma: {{ \Carbon\Carbon::parse($invoice_id->formalization_pdf_expires_at)->format('d.m.Y H:i') }}
                        </small>
                    @endif
                @else
                    {{-- PDF henüz hazır değilse durum kontrol butonu --}}
                    <button type="button" class="btn btn-info btn-sm checkPdfBtn" data-invoice-id="{{ $invoice_id->id }}">
                        <i class="fas fa-sync-alt"></i> PDF Durumunu Kontrol Et
                    </button>
                    <small class="text-muted d-block mt-1">
                        <i class="fas fa-info-circle"></i> PDF henüz hazırlanıyor
                    </small>
                @endif
            @elseif($invoice_id->formalization_status === 'pending')
                <button type="button" class="btn btn-warning btn-sm checkPdfBtn" data-invoice-id="{{ $invoice_id->id }}">
                    <i class="fas fa-sync-alt"></i> Durumu Kontrol Et
                </button>
                <small class="text-muted d-block mt-1">
                    <i class="fas fa-info-circle"></i> İşlem devam ediyor...
                </small>
            @elseif($invoice_id->formalization_status === 'error')
                <small class="text-danger d-block">
                    <i class="fas fa-exclamation-triangle"></i> 
                    {{ $invoice_id->formalization_error ?? 'Resmileştirme hatası' }}
                </small>
            @endif
        </div>
    </div>
      ...
      <div class="card card-invoices f4 h-100">
        <div class="card-body card-invoices-body" style="padding:17px 5px">
          <div class="row form-group">
            ...
            <div class="col-md-8 rw2 custom-rw2"><input type="text" onkeyup="sayiKontrol(this)" name="toplam" value="{{$invoice_id->toplam}}" autocomplete="off" class="form-control toplam" disabled required></div>
          </div>
          ...
          <div class="row form-group">
            ...
            <div class="col-md-2 rw2 col-6"><input type="text" disabled onkeyup="sayiKontrol(this)" name="kdvTutar" autocomplete="off" class="form-control kdvTutar" value="{{$invoice_id->kdvTutar ?? 20}}"></div>
            <div class="col-md-8 rw2 custom-rw2 col-6"><input type="text" disabled onkeyup="sayiKontrol(this)" name="kdv" class="form-control kdv" value="{{$invoice_id->kdv}}"></div>
          </div>
          ...
          <div class="row form-group">
            ...
            <div class="col-md-2 col-6 rw2">
              <select class="form-select tevkifatOrani" name="tevkifatOrani" disabled>
            ...
            <div class="col-md-4 custom-rw2 col-6 rw2">
              <input type="text" class="form-control tevkifatTutari" disabled value="{{number_format($invoice_id->tevkifatTutari ?? 0, 2, '.', '')}}" disabled>
          ...
          <div class="row form-group" style="padding-bottom: 0">
            ...
            <div class="col-md-8 rw2 custom-rw2"><input type="text" disabled onkeyup="sayiKontrol(this)" name="genelToplam" value="{{$invoice_id->genelToplam}}" autocomplete="off" class="form-control genelToplam" required></div>
          </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="tahsilatForm">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Kasa/Banka Hesabı <span class="text-danger">*</span></label>
                        <select class="form-select" id="accountId" name="account_id" required>
                            <option value="">Yükleniyor...</option>
                        </select>
                    </div>

                    {{-- ⭐ Tutar gösterimi - Sadece görüntüleme, değiştirilemez --}}
                    <div class="mb-3">
                        <label class="form-label">Tahsilat Tutarı</label>
                        <div class="alert alert-info mb-0">
                            <i class="fas fa-info-circle"></i> 
                            Faturanın kalan tutarının <strong>TAMAMINI</strong> tahsilat olarak ekleyeceksiniz:
                            <h4 class="mt-2 mb-0"><span id="remainingAmountDisplay">-</span> TL</h4>
                        </div>
                        <small class="text-muted">Kısmi ödeme kabul edilmez, kalan tutarın tamamı tahsil edilecektir.</small>
                    </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success" id="tahsilatKaydetBtn">
...
<div class="modal fade" id="tahsilatlariGorModal" tabindex="-1">
...
<script>
$(document).ready(function() {
    let remainingAmountGlobal = 0; // Global değişken olarak kalan tutar

    // ⭐ Sayfa yüklendiğinde kalan tutarı kontrol et
    ...
    $('.tahsilatEkleBtn').on('click', function() {
    ...
    function loadRemainingAmount(invoiceId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    const remaining = parseFloat(response.remaining_amount || 0);
                    remainingAmountGlobal = remaining; // Global değişkene kaydet
    ...
    // ⭐ Tahsilat Kaydet - Tutar parametresi yok
    $('#tahsilatKaydetBtn').on('click', function() {
    const form = $('#tahsilatForm');
    
    // Sadece account_id ve date zorunlu
    if (!$('#accountId').val()) {
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log('Tahsilat response:', response); // ✅ DEBUG
            
            try {
                if (response.success) {
                    // ✅ amount_paid kontrolü
                ...
                console.error('Success callback hatası:', e);
                alert('Tahsilat eklendi ancak sayfa yenilenemedi. Lütfen sayfayı manuel olarak yenileyin.');
                $('#tahsilatModal').modal('hide');
            }
        },
        error: function(xhr, status, error) {
            console.error('AJAX Hatası:', xhr.responseText); // ✅ DEBUG
    ...
    // Tahsilatları Gör
    $('.tahsilatlariGorBtn').on('click', function() {
        const invoiceId = $(this).data('invoice-id');
        const firmaId = {{ $firma->id }};
    ...
    // ⭐ TAHSİLAT SİLME İŞLEMİ
    $(document).on('click', '.deleteTahsilatBtn', function() {
        const invoiceId = $(this).data('invoice-id');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Tahsilatları Gör modalını kapat
...
<script>
$(document).ready(function() {
    // ⭐ PDF Durumunu Kontrol Et Butonu
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.pdf_url) {
                    // PDF hazır - Sayfayı yenile
                    Swal.fire({
                        title: 'PDF Hazır!',
                        text: 'PDF başarıyla oluşturuldu. Sayfa yenilenecek.',
                        icon: 'success',
                        timer: 2000,
                    ...
                    Swal.fire({
                        title: 'PDF Henüz Hazır Değil',
                        text: 'PDF oluşturma işlemi devam ediyor. Lütfen birkaç dakika sonra tekrar deneyin.',
                        icon: 'info'
                    });
