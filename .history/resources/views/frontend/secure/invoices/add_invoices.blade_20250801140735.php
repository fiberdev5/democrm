...
<script>
    ...
    // SERVİS ID -> Servisleri ara ve listele
    $('.servisid').on("input", function () {
        var servisId = $(this).val();
        
        // Servisi Aç linkini güncelle
        $(".servisiAc").attr("href", "/servisler.php?sid=" + servisId);
        ...
        if (servisId.length >= 2) {
            $.ajax({
                url: "{{ route('servis.musteri.getir', $firma->id) }}",
                type: "POST",
                data: {
                    servisId: servisId,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(response) {
                    ...
                    if (response.success && response.data && response.data.length > 0) {
                        ...
                        // Servis bulunamadı
                        $('#result').append(`
                            <li class="list-group-item text-center no-click" style="color: #6c757d; font-style: italic; cursor: default;">
                                <i class="fas fa-search" style="margin-right: 5px;"></i>
                                "${servisId}" ID'li servis bulunamadı
                            </li>
                        `);
                    }
                },
                error: function(xhr, status, error) {
                    ...
                    $('#result').append(`
                        <li class="list-group-item text-center no-click" style="color: #dc3545; background-color: #f8d7da; border-color: #f5c6cb; cursor: default;">
    ...
    $('#result').on('click', 'li', function () {
        // Tıklanamaz öğeler için işlem yapma
