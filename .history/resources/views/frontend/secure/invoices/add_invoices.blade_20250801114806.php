...
<script>
  ...
  function turkceKucukHarfeDonustur(text) {
    if (!text) return '';
    return text.replace(/Ğ/g, 'ğ').replace(/Ü/g, 'ü').replace(/Ş/g, 'ş')
               .replace(/İ/g, 'i').replace(/Ö/g, 'ö').replace(/Ç/g, 'ç')
               .toLowerCase();
  }
  ...
  $(document).ready(function () {
    ...
    // SERVİS ID -> Servisi Aç
    $('.servisid').on("input", function () {
      $(".servisiAc").attr("href", "/servisler.php?sid=" + this.value);
    });
  ...
  $('#result').html('<li>Yükleniyor...</li>');

  $.post({
    url: "{{ route('fatura.musteri.getir', $firma->id) }}",
    data: { faturaMusteriGetir: searchField },
    dataType: 'json',
  ...
  // İlçe, il yüklendikten sonra set edilmeli
  setTimeout(() => {
  ...
  $('#result').html('');
    ...
    // Formu göndermeden önce zorunlu alanları kontrol et
