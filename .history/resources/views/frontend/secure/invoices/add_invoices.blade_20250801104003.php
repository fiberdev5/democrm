<form method="post" id="addInvo" action="{{ route('store.invoices')}}" enctype="multipart/form-data" class="needs-validation" novalidate>
  @csrf
  <div class="card f5">
            ...
            <div class="col-md-4 rw2">
              <select class="form-control paraBirimi" name="paraBirimi">
                @foreach($currencies as $currency)
                  <option value="{{$currency->id}}">{{$currency->baslik}} ({{$currency->aciklama}})</option>
                @endforeach
              </select>
            </div>
          </div>         
