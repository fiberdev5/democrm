...
<script>
$(document).ready(function() {
    // ⭐ Sayfa yüklendiğinde kalan tutarı kontrol et ve butonları ayarla
    ...
    function updatePaymentButtons(remainingAmount, paymentCount) {
        ...
        if (remainingAmount <= 0) {
            // Tamamen ödendi - Tahsilat Ekle butonunu gizle
            $tahsilatBtn.hide();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#paymentAmount').attr('max', remaining).val('');
    ...
    $('#tahsilatKaydetBtn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    alert('Tahsilat başarıyla eklendi!\n\nKalan Tutar: ' + response.remaining_amount.toFixed(2) + ' TL');
                    $('#tahsilatModal').modal('hide');
                ...
                if (response.success && response.payments.length > 0) {
                    ...
                    if (remaining > 0) {
                        html += `<div class="alert alert-warning mt-3">
                            ...
                            <div class="mt-2">
                                <small>Fatura henüz tamamen ödenmemiş. Kalan tutar için yeni tahsilat ekleyebilirsiniz.</small>
                            </div>
                        </div>`;
                    } else {
