...
<div class="row cardRow1 mb-1 fatura-mobil-add">
  ...
  <div class="card card-invoices f4 h-100">
    ...
    <div class="col-md-2 rw2 col-6">
        {{-- KDV Oranı - Kod ile değişeceği için disabled/readonly OLMAMALI --}}
        <input type="text" name="kdvTutar" class="form-control kdvTutar" value="{{ $invoice_id->kdvTutar ?? 20 }}" {{ $invoice_id->formalized ? 'readonly' : '' }}>
    </div>
    <div class="col-md-8 rw2 custom-rw2 col-6">
        {{-- KDV Tutarı - Otomatik hesaplanıyor --}}
        <input type="text" name="kdv" class="form-control kdv" value="{{ $invoice_id->kdv }}" readonly>
    </div>
</div>
...
$(document).ready(function() {
    ...
    function hesaplaGenelToplam() {
        ...
        $form.find('.miktar').each(function(index) {
            ...
            sonucToplam += tutar;
            $form.find(".tutar").eq(index).val(tutar.toFixed(2));
        });
