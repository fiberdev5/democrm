...
@section('user')
  <link href="{{ asset('frontend/css/invoices/all_invoices.css') }}" rel="stylesheet" type="text/css" />
...
<script>
$(document).ready(function() {
    // ⭐ MODAL STACKING YÖNETİMİ
    let editModalWasOpen = false;
    
    // Tahsilat modalları açıldığında
    $('#tahsilatModal, #tahsilatlariGorModal').on('show.bs.modal', function() {
        editModalWasOpen = $('#editInvoiceModal').hasClass('show');
        
        // Edit modalın z-index'ini düşür
        $('#editInvoiceModal').css('z-index', 1040);
        
        // Tahsilat modalının z-index'ini yükselt
        $(this).css('z-index', 1060);
        
        // Backdrop düzenlemesi
        setTimeout(() => {
            $('.modal-backdrop').not(':last').css('z-index', 1039);
            $('.modal-backdrop:last').css('z-index', 1059);
        }, 10);
    });

    // Tahsilat modalları kapandığında
    $('#tahsilatModal, #tahsilatlariGorModal').on('hidden.bs.modal', function() {
        // Edit modalı geri getir
        if (editModalWasOpen) {
            $('#editInvoiceModal').css('z-index', 1055);
            $('.modal-backdrop').css('z-index', 1050);
            $('body').addClass('modal-open'); // Body'yi modal-open tut
    ...
    // Edit modal tamamen kapandığında içeriği temizle
    $('#editInvoiceModal').on('hidden.bs.modal', function() {
        // Sadece tahsilat modalları kapalıysa temizle
        if (!$('#tahsilatModal').hasClass('show') && !$('#tahsilatlariGorModal').hasClass('show')) {
            $('#editInvoiceModal .modal-body').html("");
        }
    });
});
