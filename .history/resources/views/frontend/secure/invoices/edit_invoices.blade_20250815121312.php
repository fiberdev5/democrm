...
<form method="post" id="editInvo" action="{{ route('update.invoices', $firma->id)}}" enctype="multipart/form-data" class="needs-validation" novalidate>
  ...
  <div class="card f2">
    ...
    <span class="badge bg-light text-dark">{{ $invoice_id->customer->adSoyad }}</span>
    @if(!empty($invoice_id->customer?->tcNo))
        <span class="badge bg-light text-dark">TC: 11111111111</span>
    @endif
    @if(!empty($invoice_id->customer?->vergiNo))
        <span class="badge bg-light text-dark">VERGİ NO: 121212</span>
    @endif
</div>
