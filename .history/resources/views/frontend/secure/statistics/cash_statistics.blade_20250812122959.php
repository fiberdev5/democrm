                    ...
                    <div class="col-sm-6 giderTablosu">
                        <div class="row">
                            <div class="sol col-sm-6">
                                  <ul>
        {{-- Eski 'odemeTuruAll' döngüsü yerine bu gelecek --}}
        @foreach($giderListesi as $item)
            <li class="gider">
                <div class="renk" style="background:{{ $item['renk'] }}"></div>
                <div class="adi">{{ $item['adi'] }}</div>
                <div class="para">{{ number_format($item['deger'], 0, ',', '.') }} TL</div>
            </li>
        @endforeach
        <li class="gider">
...
myChart = new Chart(ctx, {
    type: 'pie',
    data: {
        // Eski değişkenler yerine yenileri
        labels: {!! $grafikEtiketleri !!},
        datasets: [{
            data: {!! $grafikDegerleri !!},
