...
@section('user')
   <link href="{{ asset('frontend/css/statistics/stock_statistics.css') }}" rel="stylesheet" type="text/css" />
...
<script>
    $(document).ready(function () {
        // Tarih değişkenlerini başlangıçta tanımla
        let start_date = moment().subtract(1, 'months');
        let end_date = moment();

        // 1. ADIM: DataTable'ı ÖNCE tanımla.
        // Bu, 'table' değişkenini oluşturur, böylece diğer fonksiyonlar ona güvenle erişebilir.
        var table = $('#datatablePersonelDepoStats').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    d._token = "{{ csrf_token() }}";
                    // AJAX isteği gönderilirken tarihleri her zaman Daterangepicker'dan al.
            ...
            columns: [
                ...
                {
                    data: 'toplam_adet',
                    name: 'toplam_adet', // Sıralama için name eklemek önemlidir.
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        var url = "{{ url($tenant_id . '/stoklar') }}?personel=" + row.user_id + "&from_date=" + from_date + "&to_date=" + to_date;
        ...
        // 2. ADIM: DataTable tanımlandıktan SONRA Daterangepicker'ı başlat.
        ...
        // "Uygula" butonuna basıldığında tabloyu yeniden yüklemek için olay dinleyici ekle.
        $('#daterange').on('apply.daterangepicker', function(ev, picker) {
            // Sunucu taraflı olduğu için veriyi yeniden çekmek amacıyla ajax.reload() kullanılır.
            table.ajax.reload();
        });

        // 3. ADIM: Hızlı filtre fonksiyonunu ve olaylarını en sona ekle.
        // Bu fonksiyon artık var olan 'table' değişkenine sorunsuzca erişebilir.
        ...
        $('#lastYear').click(function () {
            updateRange(moment().subtract(1, 'year'), moment());
        });
        $('#lastMonth').click(function () {
            updateRange(moment().subtract(1, 'month'), moment());
        });
        $('#lastWeek').click(function () {
            updateRange(moment().subtract(7, 'days'), moment());
        });
...
<script>
    // Bu ikinci script bloğu olduğu gibi kalabilir, çünkü ana tablo mantığından bağımsızdır.
    $(document).ready(function () {
