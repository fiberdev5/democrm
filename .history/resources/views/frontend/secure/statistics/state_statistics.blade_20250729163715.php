...
$(document).ready(function () {
    ...
    $('#yesterday').click(function() {
        updateRange(moment().subtract(1, 'days').startOf('day'), moment().subtract(1, 'days').endOf('day'));
    });
    $('#today').click(function() {
        updateRange(today.startOf('day'), today.endOf('day'));
    });
    ...
    var table = $('#datatableStateStats').DataTable({
        ...
        language: {
            ...
            sInfo: "Toplam _TOTAL_ durumdan _START_ ile _END_ arası gösteriliyor", // Detaylı bilgi eklendi
            sInfoEmpty: "Kayıt yok",
            sInfoFiltered: "(Toplam _MAX_ durumdan filtrelendi)", // Filtreleme bilgisi
            sSearch: "Durum Ara:",
            sZeroRecords: "Eşleşen kayıt bulunamadı",
            sLengthMenu: "Sayfada _MENU_ kayıt göster", // Sayfa başına kayıt sayısı
            sProcessing: "Yükleniyor..." // Yükleme metni
        },
        ajax: {
            url: "{{ route('state.statistics', $tenant_id) }}",
            data: function(d) { // 'data' yerine 'd' kullanılması daha yaygın
        ...
        columns: [
            { 
                ...
                render: function(data, type, row) {
                    // row.durum_slug ve row.durum_id kontrolcüden geliyor
                    let statusClass = `status-${row.durum_slug}`; 
                    let iconText = data.charAt(0).toUpperCase(); // Durum adının ilk harfi

                    return `
                        <div class="d-flex align-items-center">
                            <div class="status-icon ${statusClass}">${iconText}</div>
                            <div>${data}</div>
                        </div>
                    `;
                }
            ...
            {
                data: null, // Bu sütun doğrudan bir veri kaynağına bağlı değil, render ile oluşturuluyor
                orderable: false,
                ...
                render: function(data, type, row) {
                    ...
                    // row.durum_id kontrolcüden geliyor
                    let url = `{{ url($tenant_id . '/servisler') }}?state_id=${row.durum_id}&state_date1=${from}&state_date2=${to}`;
        ...
        dom: '<"top"lfip<"clear">rt<"bottom"i><"clear">', // Arama kutusu ve sayfa bilgisi üste alındı, sayfalama alta alındı
        lengthMenu: [ [10, 25, 50, 100, -1], [10, 25, 50, 100, "Tümü"] ] // Varsayılan 10 kayıt da eklendi
    });

    // Arama kutusu için özel bir placeholder eklemek isterseniz
    $('.dataTables_filter input').attr('placeholder', 'Durum adı ile ara...');
    $('.dataTables_filter label').contents().filter(function(){
        return this.nodeType === 3; // Text nodes
    }).remove(); // "Durum Ara:" yazısını kaldırırız, çünkü placeholder yeterli

    // Tarih aralığı dropdown'ı dışarı tıklayınca kapanmasın
