        ...
        <div class="card">
            <div class="card-header sayfaBaslik d-flex justify-content-between align-items-center">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu p-3 dropdown-menu-end" style="min-width: 320px;">
                        <form id="filtreForm">
                            {{-- Cihaz Türü --}}
                            <div class="row mb-3">
                                ...
                                <div class="col-8">
                                    <select class="form-select" name="cihazTur" id="cihazTur">
                                        ...
                                        {{-- $cihazTurleri değişkeni controller'dan gelmeli --}}
                            ...
                            <div class="row mb-3">
                                <div class="col-12 text-end tarih-butonlari">
                            ...
                            <button type="button" class="btn btn-primary btn-sm w-100" id="araBtn">
...
<style>
/* Önceki stilleri koruyun, sadece gerekli olanları güncelleyin */
.dropdown-menu {
    border: 1px solid #dee2e6;
    box-shadow: 0 0.5rem 1rem rgba(0, 0, 0, 0.15);
    min-width: 320px !important; /* Resimdeki menü genişliğine benzer */
    min-height: 220px;
    padding: 15px;
    overflow-y: auto;
    /* Sağdan açılması için */
    right: 0; 
    left: auto;
}
...
.dropdown-menu form .row {
    align-items: center; /* Label ve inputları aynı hizada tutar */
}
.dropdown-menu form .col-4 {
    text-align: right; /* Label'ları sağa yaslar */
    padding-right: 10px; /* Biraz boşluk bırakır */
}
</style>
...
$(document).ready(function() {
    ...
    $('.tarih-btn').click(function() {
        ...
        if (isYesterday) {
            ...
            tari2 = moment().subtract(1, 'days').format('DD/MM/YYYY');
        } else if (days === 0) {
            // Bugün
            tarih1 = moment().format('DD/MM/YYYY');
            tari2 = moment().format('DD/MM/YYYY');
        } else {
    ...
    function loadTechnicianStatistics() {
        ...
        const cihazTur = $('#cihazTur').val(); // Cihaz türü filtresi
        ...
        $.ajax({
            ...
            data: {
                ...
                cihazTur: cihazTur // Cihaz türü parametresi
            },
            ...
            error: function(xhr) {
                showError('Bir hata oluştu: ' + xhr.responseText);
            },
            complete: function() {
                $('#loadingDiv').hide();
    ...
    function buildStatisticsTable(data) {
        ...
        if (data.length === 0) {
            html = `<tr><td colspan="9" class="text-center">Belirtilen kriterlere göre teknisyen verisi bulunamadı.</td></tr>`;
        } else {
        ...
        // DataTable initialize (eğer zaten initialize edilmişse destroy et, sonra yeniden oluştur)
    ...
    $(document).on('click', '.teknisyen-detay-btn', function() {
        ...
        if ($this.hasClass('clicked')) {
            ...
            // Örnek bir AJAX çağrısı (gerçek veriyi kendi endpoint'inizden çekmelisiniz)
            /*
            $.ajax({
                url: '/{{ $tenant_id }}/teknisyen-detaylari/' + teknisyenId,
                method: 'GET',
                success: function(response) {
                    if (response.success) {
                        $('.detay-satir').html(`
                            <td colspan="9">
                                <div class="p-3">
                                    <h6>Teknisyen Detay Bilgileri: ${response.data.teknisyen_adı}</h6>
                                    <p>${response.data.detay_aciklama}</p>
                                    <p><small class="text-muted">Bu veriler dinamik olarak yüklendi.</small></p>
                                </div>
                            </td>
                        `);
                    } else {
                        $('.detay-satir').html(`<td colspan="9"><div class="p-3 text-danger">Detay bilgileri yüklenirken bir hata oluştu: ${response.message}</div></td>`);
                    }
                },
                error: function() {
                    $('.detay-satir').html(`<td colspan="9"><div class="p-3 text-danger">Detay bilgileri yüklenirken bir ağ hatası oluştu.</div></td>`);
                }
            });
            */
            // Şimdilik sadece demo metin gösterelim
            setTimeout(function() {
                $('.detay-satir').html(`
                    <td colspan="9">
                        <div class="p-3">
                ...
                `);
            }, 1000); // 1 saniye sonra demo içeriği göster
        }
    });
