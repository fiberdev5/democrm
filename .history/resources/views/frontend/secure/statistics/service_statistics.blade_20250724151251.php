        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="collapse" id="filterPanel">
                <div class="card-body bg-light">
                    <form id="istatistikAra" action="{{ route('service.statistics') }}" method="get">
                        <div class="row g-3">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="date" name="tarih1" class="form-control" 
                                    value="{{ isset($request) ? \Carbon\Carbon::createFromFormat('d/m/Y', $request->tarih1)->format('Y-m-d') : date('Y-m-d') }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Bitiş Tarihi</label>
                                <input type="date" name="tarih2" class="form-control" 
                                    value="{{ isset($request) ? \Carbon\Carbon::createFromFormat('d/m/Y', $request->tarih2)->format('Y-m-d') : date('Y-m-d') }}">
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-12">
                    ...
                    <div class="card shadow-sm">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                ...
                                <span class="badge bg-primary fs-6">Toplam: {{ $statistics['toplam'] }}</span>
                            </div>
                ...
                @foreach($periodStats as $key => $period)
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="heading{{ $key }}">
                            <button class="accordion-button {{ $key !== 'bugun' ? 'collapsed' : '' }}" type="button" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Service Chart
    @if(isset($chartData) || isset($statistics['chartData']))
    const chartData = @json($statistics['chartData'] ?? $chartData ?? []);
    ...
    // Hourly Chart
    @if(isset($hourlyData) || isset($statistics['hourlyData']))
    const hourlyData = @json($statistics['hourlyData'] ?? $hourlyData ?? []);
