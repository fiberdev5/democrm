...
myChart2 = new Chart(ctx2, {
    ...
    options: {
        ...
        legend: { display: false }, // Liste olduğu için legend gizlendi
        responsive: true,
        maintainAspectRatio: false, // Oranı koruma kapatıldı
    },
});
</script>

<script>
// Daterangepicker ayarları (Değişiklik yok)
...
<script>
// Filtreleme butonları ve fonksiyon çağrıları (Değişiklik yok)
$(document).ready(function () {
    var today = moment();
...
/*
    YÖNLENDİRME: 
    Aşağıdaki 'filterData' fonksiyonu, eski yapıdaki elemanları güncelliyordu.
    Yeni yapıda gelir listesi dinamik olduğu için, bu fonksiyonun 'filterGiderData' gibi çalışması,
    yani backend'den bir HTML alıp '.gelirTablosu .sol' içeriğini güncellemesi daha doğru olacaktır.
    Mevcut haliyle sadece listedeki para birimlerini güncelleyecektir.
    Backend tarafında '/gelir-tablo/getir' rotasının HTML döndürecek şekilde güncellenmesi önerilir.
*/
function filterData(startDate, endDate) {
    $.ajax({
        ...
        success: function(response) {
            // Bu kısım backend'den gelen yanıta göre düzenlenmeli.
            // Örnek olarak değerleri güncelliyoruz:
...
<style>
/* Stil kodunda değişiklik yapmaya gerek yok, mevcut hali yeni yapıya uygun. */
.gider {
    display: flex;
    align-items: center;
    margin-bottom: 5px;
