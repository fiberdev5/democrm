...
@section('user')
  ...
  <script>
    $(document).ready(function () {
        ...
        // 1. ADIM: Daterangepicker'ı DataTable'dan ÖNCE başlat.
        // Bu, DataTable'ın ilk AJAX çağrısı yaptığında 'startDate' hatasını önler.
        ...
        // 2. ADIM: DataTable'ı tanımla.
        // Artık AJAX çağrısındaki data fonksiyonu, daterangepicker'ı bulabileceği için hata vermeyecektir.
        var table = $('#datatablePersonelDepoStats').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    d._token = "{{ csrf_token() }}";
                    // Bu satır artık hata vermez çünkü daterangepicker yukarıda başlatıldı.
            ...
            columns: [
                { data: 'personel_name', name: 'name' /* ... render ... */ },
                { data: 'toplam_adet', name: 'toplam_adet' /* ... render ... */ },
                { data: null, name: 'action', orderable: false, searchable: false /* ... render ... */ }
            ],
            // ... Diğer DataTable ayarlarınız (dom, lengthMenu, initComplete, vb.) buraya gelecek ...
        ...
        // 3. ADIM: DataTable tanımlandıktan SONRA olay dinleyicilerini ekle.
        // 'table' değişkeni artık mevcut ve kullanılabilir durumda.
        $('#daterange').on('apply.daterangepicker', function(ev, picker) {
            table.ajax.reload();
        });

        // Hızlı filtre fonksiyonu ve olayları
        ...
        $('#lastYear').click(() => updateRange(moment().subtract(1, 'year'), moment()));
        $('#lastMonth').click(() => updateRange(moment().subtract(1, 'month'), moment()));
        $('#lastWeek').click(() => updateRange(moment().subtract(7, 'days'), moment()));
        $('#yesterday').click(() => {
...
<script>
    // Bu ikinci script bloğu bağımsız olduğu için olduğu gibi kalabilir.
    $(document).ready(function () {
