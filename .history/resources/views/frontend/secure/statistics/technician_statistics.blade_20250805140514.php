...
$(document).on('click', '.teknisyen-detay-btn', function() {
    ...
    const $this = $(this);
    const currentScrollPosition = window.scrollY; // Mevcut kaydırma konumunu sakla
    
    // Önceki detay satırlarını kapat
    // Bu kısım sayfa kaydırma sorununa neden olabilir.
    // Sadece farklı bir butona tıklandığında eski detayı kapatmalıyız.
    const isAlreadyOpen = $this.hasClass('clicked');
    ...
    if (!isAlreadyOpen) {
        ...
        loadTechnicianDetail(teknisyenId, currentScrollPosition); // Scroll pozisyonunu fonksiyona geçir
    } else {
        // Eğer zaten açıksa, kapatma işlemi tamamlandıktan sonra
        // herhangi bir işlem yapmaya gerek yok.
    }
});

function loadTechnicianDetail(teknisyenId, scrollPosition) {
    const tarih1 = $('#tarih1').val();
    const tarih2 = $('#tarih2').val();
    const cihazTur = $('#cihazTur').val();
    ...
    $.ajax({
        ...
        complete: function() {
            // Detay yüklendikten sonra kaydırma konumunu geri yükle
...
const teknisyenId = data.teknisyen_id || data.id; // API'den gelen ID'yi kullanın
                ...
                <div class="col-md-4">
                    <div class="card shadow-sm">
                        <div class="card-body">
                            ...
                            <canvas id="tamamlananChart_${teknisyenId}" height="120"></canvas>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card shadow-sm">
                        <div class="card-body">
                            ...
                            <canvas id="iptalChart_${teknisyenId}" height="120"></canvas>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card shadow-sm">
                        <div class="card-body">
                            ...
                            <canvas id="gelirChart_${teknisyenId}" height="120"></canvas>
                        </div>
                    </div>
                </div>
            </div>
...
// drawDetailCharts fonksiyonu
setTimeout(() => {
    drawDetailCharts(data, teknisyenId);
}, 100);
}
...
function drawDetailCharts(data) {
    const chartOptions = {
        ...
        animation: {
        duration: 0 // animasyonu kapat, kayma ihtimali azalır
    },
        plugins: {
    ...
    // Tamamlanan Servisler Grafiği
     const tamamlananCtx = document.getElementById(`tamamlananChart_${teknisyenId}`);
    if (tamamlananCtx) {
    ...
    const iptalCtx = document.getElementById(`iptalChart_${teknisyenId}`);
    if (iptalCtx) {
        new Chart(iptalCtx, {
            type: 'line',
            data: {
    ...
    // Gelir Grafiği
    const gelirCtx = document.getElementById(`gelirChart_${teknisyenId}`);
    if (gelirCtx) {
        new Chart(gelirCtx, {
            type: 'line',
            data: {
