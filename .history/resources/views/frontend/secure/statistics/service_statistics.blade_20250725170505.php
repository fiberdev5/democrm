...
<div class="page-content servis-istatistik">
    ...
    <div class="card-header">
        <ul class="nav nav-tabs card-header-tabs" id="periodTab" role="tablist">
            @foreach($periodStats as $key => $period)
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ $key === 'bugun' ? 'active' : '' }}" 
                            ...
                            aria-selected="{{ $key === 'bugun' ? 'true' : 'false' }}">
                        <i class="fas fa-calendar-day me-1"></i>
                        {{ $period['label'] }}: <span class="badge bg-primary ms-1">{{ $period['toplam'] }}</span>
                    </button>
                </li>
            @endforeach
        </ul>
