...
/ SORUN 2: Anket sonuçları kısayol butonları - her seferinde yeni moment nesnesi oluştur
    $('#lastMonthResults').click(function() {
        updateResultsRange(moment().subtract(1, 'month'), moment()); // today yerine moment()
    });
    $('#last15DaysResults').click(function() {
        updateResultsRange(moment().subtract(15, 'days'), moment());
    });
    $('#lastWeekResults').click(function() {
        updateResultsRange(moment().subtract(7, 'days'), moment());
    });
    $('#yesterdayResults').click(function() {
        let yesterday = moment().subtract(1, 'days');
        updateResultsRange(yesterday.clone(), yesterday.clone()); // clone() kullan
    });
    $('#todayResults').click(function() {
        let today = moment();
        updateResultsRange(today.clone(), today.clone()); // clone() kullan
    });

    // SORUN 3: Personel istatistikleri kısayol butonları - aynı düzeltmeleri uygula
    $('#lastMonth').click(function() {
        updateRange(moment().subtract(1, 'month'), moment());
    });
    $('#last15Days').click(function() {
        updateRange(moment().subtract(15, 'days'), moment());
    });
    ...
    $('#yesterday').click(function() {
        let yesterday = moment().subtract(1, 'days');
        updateRange(yesterday.clone(), yesterday.clone());
    });
    $('#today').click(function() {
        let today = moment();
        updateRange(today.clone(), today.clone());
    });
