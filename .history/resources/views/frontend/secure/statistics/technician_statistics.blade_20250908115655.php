                ...
                <div class="dropdown-menu teknisyen-dropdown-menu p-3 dropdown-menu-end" style="min-width: 300px;">
                        <form id="filtreForm">
                           ...
                           <div class="row mb-3">
                                ...
                                <div class="col-8">
                                    <input type="text" name="tarih1" id="tarih1" class="form-control datepickerStatistic mb-2" ...>
                                    <input type="text" name="tarih2" id="tarih2" class="form-control datepickerStatistic mb-2" ...>
                                </div>
                            </div>
...
$(document).ready(function() {
    ...
    // Geri kalan tüm fonksiyonlarınız (buildStatisticsTable, loadTechnicianDetail, vs.)
    // HİÇBİR DEĞİŞİKLİK YAPILMADAN AYNEN BURAYA GELECEK.
    // ...
    // ... (Mevcut kodunuzdaki buildStatisticsTable ve diğer tüm fonksiyonları buraya yapıştırın)
    // ...
            ...
            data.forEach(function(teknisyen) {
                html += `
                    <tr class="teknisyen-detay-btn" data-teknisyen-id="${teknisyen.id || 'unknown'}">
                        <td><strong>${teknisyen.name || 'Bilinmeyen'}</strong><i class="fas fa-chevron-right detay-icon"></i></td>
        ...
        setTimeout(function() {
            try {
                dataTable = $('#teknisyenTable').DataTable({
                    ...
                    "order": [[7, 'desc']], "destroy": true,
                    "language": { "sEmptyTable": "Herhangi bir teknisyen verisi bulunamadı." },
                    "columnDefs": [{ "orderable": false, "targets": 0 }]
                });
            } catch (error) { console.error('DataTable oluşturma hatası:', error); }
        }, 100);
    }
    // ... Diğer tüm fonksiyonlarınız (loadTechnicianDetail, renderTechnicianDetail, vs.) buraya...
...
$(document).on('click', '.teknisyen-detay-btn', function() {
    ...
    $('.detay-satir').remove();
    $('.teknisyen-detay-btn').not($this).removeClass('clicked'); // Kendisi hariç diğerlerini kapat
