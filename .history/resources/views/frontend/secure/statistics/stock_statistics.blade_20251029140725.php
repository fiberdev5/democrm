    ...
    $(document).ready(function () {
        ...
        var table = $('#datatablePersonelDepoStats').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    d._token = "{{ csrf_token() }}";
                    // Tarih parametrelerini her AJAX isteğinde Daterangepicker'dan al
            ...
            columns: [
                {
                    data: 'personel_name',
                    name: 'name', // Sunucu taraflı sıralama ve arama için önemlidir
                    render: function (data, type, row) {
                        // Güvenlik için gelen HTML'i temizle
                ...
                {
                    data: 'toplam_adet',
                    name: 'toplam_adet', // Sunucu taraflı sıralama için name alanı eklenmeli
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function (data, type, row) {
                        // URL oluştururken tarihleri tekrar daterangepicker'dan al
            ...
            "initComplete": function (settings, json) {
                ...
                // 'card-header' yerine 'card-body' veya uygun bir yere eklemek daha iyi olabilir
                // Mevcut yapıda 'card-header' doğru yer gibi duruyor.
                $('.card-header').append(flexContainer);
                $('.searchWrap').css({ visibility: 'visible', opacity: 1 });
            }
        });

        // *** EKLENEN KISIM: Daterangepicker'da "Uygula" butonuna basıldığında tabloyu yenile ***
        $('#daterange').on('apply.daterangepicker', function(ev, picker) {
            table.ajax.reload();
        });

        // Hızlı tarih filtreleri için fonksiyon
        function updateRange(start, end) {
            $('#daterange').data('daterangepicker').setStartDate(start);
            $('#daterange').data('daterangepicker').setEndDate(end);
            // Daterangepicker'ı güncelledikten sonra tabloyu yeniden yükle
            table.ajax.reload();
        }

        // Hızlı tarih filtre butonlarının olay dinleyicileri
...
{{-- İkinci script bloğu aynı kalabilir --}}
<script>
    $(document).ready(function () {
