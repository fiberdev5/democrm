            ...
            <div class="card-body">
                <div class="row align-items-center">
                    ...
                    <div class="col-sm-6 gelirTablosu">
                         <h5 class="text-center mb-3">Gelir Tablosu</h5>
                        <div class="row">
                            <div class="col-sm-6">
                                <ul style="list-style: none; padding-left: 0;">    
                                    ...
                                    <li class="gelir-item">
                                     ...
                                     <li class="gelir-item" style="border-top: 1px solid #eee; margin-top:10px; padding-top:10px;">
                            ...
                            <div class="col-sm-6">
                                <!-- YENİ: Sabit boyutlu sarmalayıcı -->
                    ...
                    <div class="col-sm-6 giderTablosu">
                        <h5 class="text-center mb-3">Gider Tablosu</h5>
                        <div class="row">
                                ...
                                <ul style="list-style: none; padding-left: 0;">    
                                    @foreach($odemeTuruAll as $key => $value )
                                        @php 
                                            ...
                                            $renkler = ['#8BC34A', '#FF5722', '#00BCD4', '#E91E63', '#2196F3', '#FFC107', '#673AB7', '#009688', '#F44336', '#3F51B5', '#FF9800', '#9C27B0', '#4CAF50'];
                                    ...
                                    @endforeach
                                    <li class="gider" style="border-top: 1px solid #eee; margin-top:10px; padding-top:10px;">
                                        <div class="renk" style="background: #000"></div>
                                        <div class="adi">Toplam</div>
...
<!-- ... Diğer script'leriniz (AJAX, Daterangepicker vb.) buraya gelecek ... -->
<script>
...
function filterData(startDate, endDate) {
    $.ajax({
        ...
        success: function(response) {
            $(".gelirNakit").text(new Intl.NumberFormat('tr-TR').format(response.nakit) + " TL");
            $(".gelirEft").text(new Intl.NumberFormat('tr-TR').format(response.eft) + " TL");
            $(".gelirKart").text(new Intl.NumberFormat('tr-TR').format(response.kart) + " TL");
            $(".gelirToplam").text(new Intl.NumberFormat('tr-TR').format(response.toplam) + " TL");
        },
...
function filterGiderData(startDate, endDate) {
    $.ajax({
        ...
        data: { startDate: startDate, endDate: endDate, _token: "{{ csrf_token() }}" },
        success: function(response) {
            $(".giderTablosu .sol ul").html(response.html);
        },
        error: function(xhr, status, error) { console.error(error); }
    });
}
...
.gider .renk, .gelir-item .renk {
    width: 15px;
    height: 15px;
    border-radius: 3px;
    margin-right: 10px;
}
.gider .adi, .gelir-item .adi {
    flex: 1;
    font-size: 14px;
}
.gider .para, .gelir-item .para {
    font-weight: bold;
    font-size: 14px;
}
...
.chart-container {
    position: relative;
    height: 220px; /* Bu değeri artırıp azaltarak grafik boyutunu ayarlayabilirsiniz */
    width: 100%;
}
</style>
