...
<div class="page-content servis-istatistik">
    <div class="container-fluid">
        ...
        @endif
        <!-- Dinamik Charts Section -->
<div class="row mt-4">
    <!-- Dinamik Servis Sayıları Grafiği -->
    <div class="col-lg-8">
        <div class="card shadow-sm servisSayilariChart">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <h5 class="mb-0"><i class="fas fa-clock me-2"></i>Servis Sayıları</h5>
                </div>
            ...
            <div class="card-body">
                <div class="tab-content">
                    <div id="gun7" class="tab-pane fade show active">
                        <canvas id="myAreaChart" height="200"></canvas>
                    </div>
                    <div id="gun15" class="tab-pane fade">
                        <canvas id="myAreaChart2" height="200"></canvas>
                    </div>
                    <div id="gun30" class="tab-pane fade">
                        <canvas id="myAreaChart3" height="200"></canvas>
                    </div>
                </div>
            </div>
        </div>
     ...
     <!-- Dinamik Saatlik Dağılım -->
            <div class="col-lg-4">
                <div class="card shadow-sm servisSaatleriChart">
                    <div class="card-header">
                        <div class="d-flex flex-wrap align-items-center gap-2">
                            <input type="date" name="saatTarih" class="form-control form-control-sm saatTarih" style="max-width: 150px;" value="{{ date('Y-m-d') }}">
                            <button class="btn btn-outline-primary btn-sm btn7Gun">7 Gün</button>
                            <button class="btn btn-outline-primary btn-sm btn15Gun">15 Gün</button>
                            <button class="btn btn-outline-primary btn-sm btn30Gun">30 Gün</button>
                        </div>
                    </div>
                    <div class="card-body">
                        <canvas id="saatArea" height="300"></canvas>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let hourlyChartInstance = null; // Saatlik chart instance
    ...
    function loadChart(days, canvasId, colors) {
        fetch(`/statistics/${tenantId}/chart-data?days=${days}`)
    ...
    function loadHourlyChart(type, date = null) {
        let url = `/statistics/${tenantId}/hourly-data?type=${type}`;
        if (date) {
            url += `&date=${date}`;
        }
        ...
        fetch(url)
            .then(response => response.json())
            .then(data => {
                const ctx = document.getElementById('saatArea').getContext('2d');
    ...
    document.querySelector('.saatTarih').addEventListener('change', function() {
        ...
        document.querySelectorAll('.btn7Gun, .btn15Gun, .btn30Gun').forEach(btn => {
        ...
        loadHourlyChart('today', formattedDate);
    });

    // 7 Gün butonu
    document.querySelector('.btn7Gun').addEventListener('click', function() {
        // Active state'leri güncelle
        document.querySelectorAll('.btn7Gun, .btn15Gun, .btn30Gun').forEach(btn => {
            btn.classList.remove('active', 'btn-primary');
            btn.classList.add('btn-outline-primary');
        });
        ...
        loadHourlyChart('7days');
    });

    // 15 Gün butonu
    document.querySelector('.btn15Gun').addEventListener('click', function() {
        // Active state'leri güncelle
        document.querySelectorAll('.btn7Gun, .btn15Gun, .btn30Gun').forEach(btn => {
            btn.classList.remove('active', 'btn-primary');
            btn.classList.add('btn-outline-primary');
        ...
        loadHourlyChart('15days');
    });

    // 30 Gün butonu
    document.querySelector('.btn30Gun').addEventListener('click', function() {
        // Active state'leri güncelle
        document.querySelectorAll('.btn7Gun, .btn15Gun, .btn30Gun').forEach(btn => {
            btn.classList.remove('active', 'btn-primary');
            btn.classList.add('btn-outline-primary');
