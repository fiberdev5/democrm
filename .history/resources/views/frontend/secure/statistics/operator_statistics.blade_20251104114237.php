...
@section('user')
    ...
    <style>
        #datatableOperatorStats_wrapper .dataTables_filter {
...
/* DataTable wrapper'ın üst kısmındaki boşluğu sıfırla */
.dataTables_wrapper .row:first-child {
    margin: 0 !important;
    min-height: 0 !important;
}

/* Eğer DataTable üstte bir div oluşturuyorsa onu da gizle */
#datatableOperatorStats_wrapper > .row:first-child {
    display: none !important;
}

/* Header'a taşındıktan sonra göster */
...
">Operatör İstatistikleri</span>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#datatableOperatorStats').on('preInit.dt', function() {
        // DataTable wrapper'ını bul ve ilk row'u gizle
        setTimeout(function() {
            $('#datatableOperatorStats_wrapper > .row:first-child').hide();
        }, 0);
    });
            ...
            // İlk row'u kesin gizle
            $('#datatableOperatorStats_wrapper > .row:first-child').hide();
            
            var searchContainer = $('#datatableOperatorStats_filter');
            var searchInput = searchContainer.find('input');
            var filterWrapper = $('.searchWrap');
            var flexContainer = $('<div class="d-flex justify-content-end align-items-center gap-2"></div>'); 
