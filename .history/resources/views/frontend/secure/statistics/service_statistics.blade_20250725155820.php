...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadChart(days, canvasId, colors) {
        fetch(`/${tenantId}/chart-data?days=${days}`)
            ...
            .catch(error => console.error('Chart yüklenirken hata:', error));
    }
    ...
    function loadHourlyChart(type, canvasId, colors, date = null) {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                const labels = Array.from({ length: 24 }, (_, i) => String(i + 1).padStart(2, '0'));
            ...
            .catch(error => console.error('Saatlik chart yüklenirken hata:', error));
    }

    const chartColors = {
        7:   { background: "rgba(255,165,0,0.2)", border: "rgba(255,165,0,0.7)", point: "rgba(255,165,0,1)" },
        15:  { background: "rgba(255,0,0,0.2)", border: "rgba(255,0,0,0.7)", point: "rgba(255,0,0,1)" },
        30:  { background: "rgba(84,177,47,0.2)", border: "rgba(84,177,47,0.7)", point: "rgba(84,177,47,1)" }
    };

    const hourlyColors = {
        '7days':  { background: "rgba(255,0,0,0.2)", border: "rgba(255,0,0,0.7)", point: "rgba(255,0,0,1)" },
        '15days': { background: "rgba(2,117,216,0.2)", border: "rgba(2,117,216,1)", point: "rgba(2,117,216,1)" },
        '30days': { background: "rgba(255,165,0,0.2)", border: "rgba(255,165,0,0.7)", point: "rgba(255,165,0,1)" }
    };

    loadChart(7, 'myAreaChart', chartColors[7]);
    loadHourlyChart('7days', 'saatArea7', hourlyColors['7days']);

    // Tab'lara tıklayınca grafik yükle
    document.querySelectorAll('.chart-tab').forEach(tab => {
        tab.addEventListener('shown.bs.tab', function () {
            const days = parseInt(this.dataset.days);
            const canvasId = days === 7 ? 'myAreaChart' : days === 15 ? 'myAreaChart2' : 'myAreaChart3';
    ...
    // Tarih input değiştiğinde güncelle
    const dateInput = document.querySelector('.saatTarih');
    if (dateInput) {
    ...
    // ⛔️ Hızlı tarih seçiminde dropdown kapanmasın
    document.querySelectorAll('.quick-date').forEach(btn => {
        btn.addEventListener('click', function (e) {
            e.stopPropagation(); // 🔒 Dropdown kapanmasını engeller
            const start = this.dataset.start;
            const end = this.dataset.end;

            document.querySelector('input[name="tarih1"]').value = start;
            document.querySelector('input[name="tarih2"]').value = end;
        });
    });

    // 🔒 Dropdown menünün tamamında kapanmayı engelle (inputa tıklanınca da kapanmasın)
    const dropdown = document.querySelector('.dropdown-menu.servisDrop');
    if (dropdown) {
