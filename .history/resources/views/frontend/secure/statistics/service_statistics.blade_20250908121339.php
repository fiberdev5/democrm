        ...
        @include('frontend.secure.statistics.statistics_menu', ['tenant_id' => $tenant_id])
           <!-- Modern Header Card -->
            ...
            <div class="card shadow-sm mb-4 istatistik-card">
                ...
                <div class="btn-group mb-0" id="genelIstatistikFilterContainer">
                        ...
                        <form id="istatistikAra" action="{{ route('statistics', $tenant_id) }}" method="get">
                            ...
                            <div class="d-flex align-items-center mb-2">
                                ...
                                <div class="flex-grow-1">
                                    <select name="personeller" class="form-select form-select-sm">
                            ...
                            <div class="d-flex align-items-center mb-3">
                                <label class="form-label me-2 mb-0" style="width: 120px;">Tarih Aralığı</label>
                                <div class="flex-grow-1">
                                    {{-- DÜZELTME: Başlangıç değeri value attribute'u ile eklendi --}}
                                    <input type="text" id="tarihAraligiGenel" class="form-control form-control-sm" style="background:#fff;" 
                                           value="{{ \Carbon\Carbon::parse($request->tarih1 ?? now()->subMonth())->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($request->tarih2 ?? now())->format('d/m/Y') }}">
                                </div>
                            </div>
                            <input type="hidden" name="tarih1" id="tarih1_hidden">
                            <input type="hidden" name="tarih2" id="tarih2_hidden">
                            {{-- Hızlı filtre ve Ara --}}
                            <div>
                                <div class="tarihAraligi mt-2 mb-2 d-flex justify-content-center flex-wrap gap-1">
                                    {{-- DÜZELTME: Tüm butonlara type="button" eklendi --}}
                                    <button type="button" id="lastYear" class="btn btn-sm btn-outline-secondary">Son 1 Yıl</button>
                                    <button type="button" id="lastMonth" class="btn btn-sm btn-outline-secondary">Son 1 Ay</button>
                                    <button type="button" id="lastWeek" class="btn btn-sm btn-outline-secondary">Son 7 Gün</button>
                                    <button type="button" id="yesterday" class="btn btn-sm btn-outline-secondary">Dün</button>
                                    <button type="button" id="today" class="btn btn-sm btn-outline-secondary">Bugün</button>
                                </div>
...
$(document).ready(function () {
    // --- DROPDOWN KAPANMA SORUNU İÇİN ÇÖZÜM BAŞLANGICI ---
    let preventDropdownHide = false;
    $('#genelIstatistikFilterContainer').on('hide.bs.dropdown', function(e) {
    ...
    $('#genelIstatistikFilterContainer').find('#tarihAraligiGenel').on('focus mousedown', function() {
        preventDropdownHide = true;
    });
    $('#genelIstatistikFilterContainer').find('.tarihAraligi button').on('mousedown', function() {
        preventDropdownHide = true;
    });
    $('#tarihAraligiGenel').on('apply.daterangepicker cancel.daterangepicker hide.daterangepicker', function() {
        preventDropdownHide = false;
    });
    // --- DROPDOWN KAPANMA SORUNU İÇİN ÇÖZÜM BİTİŞİ ---

    // Daterangepicker'ı başlat
    $('#tarihAraligiGenel').daterangepicker({
        // PHP'den gelen veya varsayılan tarihleri ayarla
        startDate: moment('{{ \Carbon\Carbon::parse($request->tarih1 ?? now()->subMonth())->format("Y-m-d") }}'),
        endDate: moment('{{ \Carbon\Carbon::parse($request->tarih2 ?? now())->format("Y-m-d") }}'),
        locale: {
            format: 'DD/MM/YYYY',
            separator: ' - ',
            applyLabel: 'Uygula',
    ...
    // Hızlı tarih butonlarının işlevselliği
    const picker = $('#tarihAraligiGenel').data('daterangepicker');
    ...
    // Form gönderilmeden hemen önce gizli input'ları doldur
    $('#istatistikAra').on('submit', function() {
        // Daterangepicker'dan başlangıç ve bitiş tarihlerini al
        const startDate = picker.startDate.format('YYYY-MM-DD');
        const endDate = picker.endDate.format('YYYY-MM-DD');
        
        // Gizli input'ların değerlerini ayarla
        $('#tarih1_hidden').val(startDate);
        $('#tarih2_hidden').val(endDate);

        // Form normal şekilde gönderilebilir
        return true;
    });
});
</script>
