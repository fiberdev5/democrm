...
<div class="page-content servis-istatistik" id="surveyStats">
    <div class="container-fluid">
        ...
        <!-- Personel Bazında Anket İstatistikleri - AYRI FİLTRELER İLE -->
                                ...
                                <div class="dropdown-menu p-3" style="min-width: 200px;">
                                    <div class="item">
                                        <div class="row mb-3">
                                            <label class="col-sm-4">Cihaz Türü:</label>
                                            <div class="col-sm-8">
                                                <select id="deviceTypePersonnel" class="form-select form-select-sm">
                                                    <option value="">Hepsi</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row mb-3">
                                            <label class="col-sm-4">Tarih Aralığı:</label>
                                            <div class="col-sm-8">
                                                <input id="daterangePersonnel" class="form-control form-control-sm tarih-araligi" />
                                                <div class="tarihAraligi mt-2 mb-2">
                                                    <button id="lastMonthPersonnel" class="btn btn-sm btn-secondary me-1">Son 1 Ay</button>
                                                    <button id="last15DaysPersonnel" class="btn btn-sm btn-secondary me-1">Son 15 Gün</button>
                                                    <button id="lastWeekPersonnel" class="btn btn-sm btn-secondary me-1">Son 7 Gün</button>
                                                    <button id="yesterdayPersonnel" class="btn btn-sm btn-secondary me-1">Dün</button>
                                                    <button id="todayPersonnel" class="btn btn-sm btn-secondary">Bugün</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
$(document).ready(function () {
    ...
    // Anket sonuçları filtre değişiklikleri
    ...
    // ============= PERSONEL İSTATİSTİKLERİ KISMI =============
    
    // Personel istatistikleri için ayrı tarih seçici
    $('#daterangePersonnel').daterangepicker({
        startDate: start_date,
        endDate: end_date,
        locale: {
            format: 'DD-MM-YYYY',
    ...
    // Personel istatistikleri kısayol butonları - Ayrı ID'ler ile
    $('#lastMonthPersonnel').click(function() {
        updatePersonnelRange(moment().subtract(1, 'month'), today);
    });
    $('#last15DaysPersonnel').click(function() {
        updatePersonnelRange(moment().subtract(15, 'days'), today);
    });
    $('#lastWeekPersonnel').click(function() {
        updatePersonnelRange(moment().subtract(7, 'days'), today);
    });
    $('#yesterdayPersonnel').click(function() {
        updatePersonnelRange(moment().subtract(1, 'days'), moment().subtract(1, 'days'));
    });
    $('#todayPersonnel').click(function() {
        updatePersonnelRange(today, today);
    });

    function updatePersonnelRange(start, end) {
        $('#daterangePersonnel').data('daterangepicker').setStartDate(start);
        $('#daterangePersonnel').data('daterangepicker').setEndDate(end);
        loadSurveyStatistics();
    }

    // Personel filtre değişikliği
    $('#deviceTypePersonnel').change(function() {
        loadSurveyStatistics();
    });

    // Personel bazında anket istatistiklerini yükle
    function loadSurveyStatistics() {
        var fromDate = $('#daterangePersonnel').data('daterangepicker').startDate.format('YYYY-MM-DD');
        var toDate = $('#daterangePersonnel').data('daterangepicker').endDate.format('YYYY-MM-DD');
        var deviceTypeId = $('#deviceTypePersonnel').val();
        ...
        $.ajax({
            ...
            success: function(response) {
                // Cihaz türleri dropdown'ını doldur - Personel için ayrı
                if ($('#deviceTypePersonnel option').length <= 1) {
                    response.deviceTypes.forEach(function(deviceType) {
                        $('#deviceTypePersonnel').append('<option value="' + deviceType.id + '">' + deviceType.cihaz + '</option>');
                    });
                }
    ...
    function updatePersonnelTable(personnelStats) {
        ...
        statsArray.forEach(function(personnel) {
            // Detay butonu için parametreleri hazırla - Güncellenmiş
            var from_date = $('#daterangePersonnel').data('daterangepicker').startDate.format('YYYY-MM-DD');
            var to_date = $('#daterangePersonnel').data('daterangepicker').endDate.format('YYYY-MM-DD');
            let deviceType = document.getElementById("deviceTypePersonnel").value;
