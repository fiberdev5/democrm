                            ...
                            <table id="datatableSurveyStats" class="table table-hover mb-0">
                                <thead class="title">
                                    <tr>
                                        ...
                                        <th style="width: 130px"><i class="fas fa-check-circle me-2"></i>Sonlanan Servisler</th>
                                        <th style="width: 130px"><i class="fas fa-poll me-2"></i>Yapılan Anketler</th>
                            ...
                            <div class="col-md-6">
                                <div class="card bg-light">
                                    <div class="card-body text-center">
                                        <h5 class="card-title">Toplam Tamamlanan Servis</h5>
                                        <h3 class="text-primary" id="totalCompletedServices">0</h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="card bg-light">
                                    <div class="card-body text-center">
                                        <h5 class="card-title">Yapılan Toplam Anket</h5>
                                        <h3 class="text-success" id="totalSurveyedServices">0</h3>
                                    </div>
                                </div>
                            </div>
                        </div>
...
$(document).ready(function () {
    let today = moment();
    let start_date = moment().subtract(1, 'day'); // Varsayılan dün
    let end_date = moment().subtract(1, 'day');   // Varsayılan dün
    ...
    function loadSurveyStatistics() {
        ...
        $.ajax({
            url: "{{ route('survey.statistics', $tenant_id) }}",
            method: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                from_date: fromDate,
            ...
            success: function (response) {
                ...
                if (Object.keys(response.groupedSurveys).length > 0) {
                    Object.values(response.groupedSurveys).forEach(function(personel) {
                        ...
                        tableHtml += '<td><a href="' + detailUrl + '" target="_blank" class="btn btn-primary btn-sm" style="font-size:13px;padding:1px">Anketleri Göster</a></td>';
                        tableHtml += '</tr>';
                    });
                } else {
