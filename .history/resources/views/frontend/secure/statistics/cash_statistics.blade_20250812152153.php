                    ...
                    <div class="card-header">
                        ...
                        <div class="dropdown float-end">
                            ...
                            <div class="dropdown-menu p-3" style="width: 300px;">
                                ...
                                <button type="button" class="btn btn-success btn-sm w-100" id="filterButton">ARA</button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                                    ...
                                    <div class="sol col-sm-6" id="giderListesiWrapper">
                                         <ul class="summary-list">    
                                            @foreach($odemeTuruAll as $key => $value )
                                                @php 
                                                    $colorIndex = $loop->index % count($renkler);
...
.dropdown-menu {
    /* Bootstrap 5'in dropdown'larının tıklayınca kapanmasını engellemek için */
...
var giderChart = new Chart(ctxGider, {
    ...
    data: {
        ...
        datasets: [{
            ...
            backgroundColor: {!! json_encode(array_values($renkler)) !!},
            borderWidth: 0,
        }],
    }
});
...
$(document).ready(function () {
    ...
    // Hızlı Tarih Değiştirme Butonları
    ...
    // Bootstrap dropdown'un içindeki elementlere tıklanınca kapanmasını engelle
...
function fetchKasaVerileri(startDate, endDate) {
    ...
    // 1. Gelir Tablosu Verilerini Getir
    $.ajax({
    ...
    // 2. Gider Tablosu Verilerini Getir (HTML olarak)
    $.ajax({
    ...
    // 3. Gelir Grafiği Verilerini Getir
    $.ajax({
    ...
    // 4. Gider Grafiği Verilerini Getir
    $.ajax({
        url: `/${tenant_id}/gider-grafik/getir`,
        method: 'POST',
        data: { startDate: startDate, endDate: endDate, _token: token },
        success: function(response) {
            if(response.giderler && response.labels && response.colors) {
                giderChart.data.labels = response.labels;
                giderChart.data.datasets[0].data = response.giderler;
                giderChart.data.datasets[0].backgroundColor = response.colors;
                giderChart.update();
            }
        }
    });
