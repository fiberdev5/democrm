...
var table = $('#datatableOperatorStats').DataTable({
    ...
    initComplete: function() {
        // Mobil görünümde başlıkları değiştir - SADECE BİR KEZ
        if ($(window).width() <= 767) {
            $('#datatableOperatorStats thead th').eq(0).text('Operatör');
            $('#datatableOperatorStats thead th').eq(1).text('Toplam');
            $('#datatableOperatorStats thead th').eq(2).text('');
        }
    },
