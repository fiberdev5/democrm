...
@section('user')
    <link href="{{ asset('frontend/css/statistics/state_statistics.css') }}" rel="stylesheet" type="text/css" />
    ...
    // Yeni bir div oluşturup arama ve filtreyi içine koyun.
    // Bu div'i zaten card-header'ın içine ekliyoruz.
    var flexContainer = $('<div class="d-flex justify-content-end align-items-center"></div>');

    // Arama kutusunu doğrudan flexContainer içine taşıyın
    // Bu, searchContainer'ın wrapper'ını taşır
    flexContainer.append(searchContainer);
    // Filtre butonunu da flexContainer içine taşıyın
    flexContainer.append(filterWrapper);

    // card-header'a bu flexContainer'ı ekleyin
    // DataTable'ın filtre wrapper'ı artık burada değil, bu nedenle append yerine prepend veya belirli bir yere insert kullanmak daha iyi olabilir.
    // Eğer card-header içinde sadece başlık ve bu kontrol grubu olacaksa, başlığın altına eklemek mantıklı.
    $('.card-header').append(flexContainer);

    // searchWrap'ın görünürlüğünü CSS ile değil, doğrudan HTML'de style="visibility: visible;" ile veya JS ile en erken zamanda ayarlamak daha iyidir.
    // Ya da bu kısmı tamamen kaldırıp CSS'den yönetin.
    // Eğer illa JS ile görünür yapacaksanız, DOM'a ekledikten hemen sonra yapın.
    // filterWrapper.css({ visibility: 'visible', opacity: 1 }); // Bu satırı initComplete dışına taşıyabilirsiniz
    // Eğer .searchWrap başlangıçta <div class="searchWrap float-end" style="visibility: visible; opacity: 1;"> olarak başlarsa, bu JS'e gerek kalmaz.
}
                            });
                        });
    </script>
