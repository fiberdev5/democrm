...
<script>
// Moment.js kütüphanesinin bu script bloğundan önce yüklendiğinden emin olun!
// Örnek: <script src="https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.29.1/moment.min.js"></script>

$(document).ready(function () {
    // Tarih input alanlarını seçiyoruz
    const tarih1Input = document.querySelector('input[name="tarih1"]');
    const tarih2Input = document.querySelector('input[name="tarih2"]');

    // Hızlı tarih butonları için olay dinleyicileri ekliyoruz
    document.getElementById('lastYear').addEventListener('click', function(e) {
        e.preventDefault(); // Formun veya butonun varsayılan submit davranışını engelle
        const today = moment();
    ...
    // "Ara" butonuna özel bir JavaScript olayı eklemenize gerek yok
    // çünkü zaten <button type="submit"> olduğu için varsayılan olarak formu gönderecektir.
    // Eğer "Ara" butonuna ek bir işlem eklemeden formun gönderilmesini istiyorsanız,
    // aşağıdaki gibi bir event listener eklemenize gerek yoktur.
    // Ancak, form ID'niz 'istatistikAra' ve butonunuz form içinde olduğu sürece çalışır.
    
    // Eğer formun otomatik gönderilmesini engelleyip sadece bu "Ara" butonuyla göndermek isterseniz:
    // document.getElementById('istatistikAra').addEventListener('submit', function(e) {
    //     // Burada herhangi bir ek doğrulama veya işlem yapabilirsiniz
    //     // e.preventDefault(); // Formun normal submitini engellemek isterseniz
    //     // this.submit(); // Sonra manuel olarak göndermek isterseniz
    // });
});
</script>
