...
$(document).ready(function () {
    ...
    // Anket Sonuçları filtre dropdown'ının kapanmasını engelle
$('.searchWrap .dropdown-menu').on('click', function(e) {
    ...
    if ($(e.target).closest('.daterangepicker').length || $(e.target).is('#daterangeResults')) {
        e.stopPropagation();
    }
});

// Personel bazında anket istatistikleri filtre dropdown'ının kapanmasını engelle (önceki cevabımdaki kod)
$('#personel_anket .dropdown-menu').on('click', function(e) {
    if ($(e.target).closest('.daterangepicker').length || $(e.target).is('#daterange')) { // #daterange id'si için de kontrol
        e.stopPropagation();
    }
});
