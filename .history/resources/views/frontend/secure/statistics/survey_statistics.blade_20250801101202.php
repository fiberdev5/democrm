...
<script>
// Blade dosyasındaki JavaScript kısmını tamamen değiştirin

<script>
// TENANT_ID'yi Blade'den JavaScript'e aktarıyoruz
const TENANT_ID = "{{ $tenant_id }}";
let surveyChart;
let filtreleDropdownActive = false; // Dropdown durumunu takip etmek için
...
$(document).ready(function () {
    ...
    // Dropdown'ların durumunu kontrol et
    $('.filtrele').on('click', function(e) {
        e.stopPropagation();
        const dropdown = $(this).next('.dropdown-menu');
        
        // Eğer bu dropdown kapalıysa aç, açıksa kapat
        if (dropdown.hasClass('show')) {
            dropdown.removeClass('show');
            filtreleDropdownActive = false;
        } else {
            // Önce tüm dropdownları kapat
            $('.dropdown-menu').removeClass('show');
            // Sonra bu dropdown'ı aç
            dropdown.addClass('show');
            filtreleDropdownActive = true;
        }
    });
    ...
    // Dropdown içerisine tıklandığında kapanmasını engelle
    ...
    function updatePersonnelTable(personnelStats) {
        ...
        statsArray.forEach(function(personnel) {
            ...
            var detailUrl = "{{ url($tenant_id . '/servisler') }}" + 
                          "?operator_id=" + personnel.personel_id + 
                          "&opeator_istatistik_tarih1=" + from_date + 
                          "&opeator_istatistik_tarih2=" + to_date;
