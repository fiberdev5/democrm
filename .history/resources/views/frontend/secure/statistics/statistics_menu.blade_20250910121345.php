...
<style>
    /* Menü Konteyneri: Tek satır ve yatay kaydırma için düzenlendi */
    .statistics-menu {
        display: flex;        /* Butonları yan yana dizer */
        flex-wrap: nowrap;      /* Butonların alt satıra inmesini engeller */
        overflow-x: auto;     /* Ekrana sığmazsa yatay kaydırma çubuğu ekler */
        gap: 1rem;            /* Butonlar arasına boşluk bırakır */
        padding-bottom: 1rem; /* Kaydırma çubuğunun içeriğe yapışmasını önler */
        -webkit-overflow-scrolling: touch; /* iOS'ta akıcı kaydırma için */
        scrollbar-width: thin; /* Firefox için kaydırma çubuğunu inceltir */
    }

    /* Kaydırma çubuğu için stil (isteğe bağlı ama şık durur) */
    ...
    .statistic-btn {
        flex-shrink: 0;         /* Butonların daralmamasını, orijinal boyutunu korumasını sağlar */
        min-width: 150px;       /* Her butonun minimum genişliği (isteğe bağlı) */
        margin-bottom: 0 !important; /* HTML'deki mb-2'yi ezmek için */

        /* ----- Mevcut stilleriniz buradan devam ediyor ----- */
    ...
    .statistic-btn i {
        font-size: 1.5rem; /* İkon boyutunu biraz büyüttüm */
        margin-bottom: 0.5rem; /* Yazı ile arasına boşluk ekledim */
        z-index: 2;
        position: relative;
    }

    .statistic-btn span, .statistic-btn { /* Buton yazısını span içine almasanız bile çalışması için */
        font-size: 0.8rem;
        font-weight: 600;
        z-index: 2;
        position: relative;
    ...
    /*
      ÖNEMLİ NOT:
      Eski kodunuzdaki @media (max-width: ...) ile başlayan
      ve .statistics-menu { grid-template-columns: ... } içeren
      tüm blokları SİLİN.
      Yukarıdaki yeni .statistics-menu stili bu blokların yerini almıştır.
    */
</style>