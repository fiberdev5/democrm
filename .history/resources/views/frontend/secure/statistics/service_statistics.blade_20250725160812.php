...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Yardımcı fonksiyon: DD.MM.YYYY formatını YYYY-MM-DD formatına çevirir
    function convertDateFormat(dateString) {
        ...
        return dateString; // Zaten doğru formatta ise aynen döndür
    }

    // Hızlı tarih butonları tarih inputlarını günceller
    document.querySelectorAll('.quick-date').forEach(btn => {
        btn.addEventListener('click', function (e) {
            ...
            // Tarihleri YYYY-MM-DD formatına çevirerek atama yapın
            document.querySelector('input[name="tarih1"]').value = convertDateFormat(start);
            document.querySelector('input[name="tarih2"]').value = convertDateFormat(end);
        });
    });

    // Dropdown içindeki tıklamaları menünün kapanmasını engelle
    ...
    // Ayrıca, varsayılan olarak Carbon ile atanan tarihlerin de doğru formatta olduğundan emin olun.
    // Laravel tarafında aşağıdaki gibi atandığından emin olun:
    // value="{{ \Carbon\Carbon::parse($request->tarih1 ?? now()->subDays(29))->format('Y-m-d') }}"
    // Bu kısım zaten doğru gibi görünüyor, ek bir düzeltmeye gerek yok.
