                ...
                <div class="table-modern">
                    <div class="card-header">
                        ...
                        <div class="searchWrap float-end">
                            <div class="btn-group mb-2" id="resultsFilterDropdownContainer"> {{-- Added a more specific ID --}}
            ...
            <div class="col-12">
                <div class="table-modern">
                    <div class="card-header">
                        ...
                        <div class="searchWrap float-end" id="personnelFilterDropdownContainer"> {{-- Added a more specific ID --}}
...
$(document).ready(function () {
    ...
    // --- YENİ VE DAHA SAĞLAM ÇÖZÜM BAŞLANGICI ---

    // Bu bayraklar, daterangepicker veya kısa yol butonlarına tıklandığında dropdown'ın kapanmasını engellemek için kullanılır.
    let isClickFromDaterangepicker = false;

    // Her iki dropdown için de ortak bir fonksiyon yazalım
    function preventDropdownClose(dropdownContainerId) {
        ...
        $dropdownContainer.find('.tarihAraligi button').on('click', function(e) {
            ...
            isClickFromDaterangepicker = true; // Bayrağı ayarla
            // Kısa yol butonları için update fonksiyonları burada çağrılmalı veya zaten event handlerları içinde çağrılıyor olmalı
        });
        ...
        $(document).on('mousedown', function(e) {
            if ($(e.target).closest('.daterangepicker').length) {
                ...
                // Eğer daterangepicker dışında bir yere tıklandıysa, bayrağı sıfırla
                // Ancak bu, dropdown'ın kapanmasına izin vereceği için dikkatli kullanılmalı
                // Sadece daterangepicker'ın kapanması durumunda dropdown'ın da kapanması isteniyorsa bu kısım faydalı olabilir.
                // Şimdilik sadece daterangepicker içinde tıklama olduğunda true yapıp bırakalım.
                // isClickFromDaterangepicker = false; // Bu satır sorun yaratabilir, şimdilik yorumda kalsın
            }
        });
    ...
    // --- YENİ VE DAHA SAĞLAM ÇÖZÜM BİTİŞİ ---

    // Anket sonuçları kısayol butonları - e.stopPropagation() bu fonksiyon içinden kaldırıldı
    $('#lastMonthResults').click(function() { updateResultsRange(moment().subtract(1, 'month'), today); });
    $('#last15DaysResults').click(function() { updateResultsRange(moment().subtract(15, 'days'), today); });
    $('#lastWeekResults').click(function() { updateResultsRange(moment().subtract(7, 'days'), today); });
    $('#yesterdayResults').click(function() { updateResultsRange(moment().subtract(1, 'days'), moment().subtract(1, 'days')); });
    $('#todayResults').click(function() { updateResultsRange(today, today); });

    // Personel istatistikleri kısayol butonları - e.stopPropagation() bu fonksiyon içinden kaldırıldı
