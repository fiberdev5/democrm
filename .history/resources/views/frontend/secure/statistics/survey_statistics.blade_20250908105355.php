                ...
                <div class="table-modern">
                    <div class="card-header">
                        ...
                        <div class="searchWrap float-end">
                            <div class="btn-group mb-2" id="resultsFilterDropdown"> {{-- Added ID here --}}
                                <button class="btn btn-dark btn-sm dropdown-toggle filtrele" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    Filtrele <i class="mdi mdi-chevron-down"></i>
                                </button>
                                 <div class="dropdown-menu p-3" style="min-width: 200px;" data-bs-auto-close="outside">
                                    <div class="item">
                                        <div class="row mb-3">
            ...
            <div class="col-12">
                <div class="table-modern">
                    <div class="card-header">
                        ...
                        <div class="searchWrap float-end" id="personnelFilterDropdown"> {{-- Added ID here --}}
                            <div class="btn-group mb-2">
                                <button class="btn btn-dark btn-sm dropdown-toggle filtrele" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    Filtrele <i class="mdi mdi-chevron-down"></i>
                                </button>
                                 <div class="dropdown-menu dropdown-menu-end p-3" style="min-width: 300px;" data-bs-auto-close="outside">
                                    <div class="item">
                                        <div class="row mb-3">
                                            <label class="col-sm-4">Cihaz Türü:</label>
                                            <div class="col-sm-8">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-8">
                                                ...
                                                <div class="tarihAraligi mt-2 mb-2">
                                                    ...
                                                    <button class="btn btn-sm btn-secondary me-1 btnLastMonth">Son 1 Ay</button>
                                                    <button class="btn btn-sm btn-secondary me-1 btnLast15Days">Son 15 Gün</button>
                                                    <button class="btn btn-sm btn-secondary me-1 btnLastWeek">Son 7 Gün</button>
                                                    <button class="btn btn-sm btn-secondary me-1 btnYesterday">Dün</button>
...
$(document).ready(function () {
    ...
    // --- YENİ ÇÖZÜM BAŞLANGICI ---

    // Genel click event handler'ı ile Bootstrap'ın kapanma davranışını yakala ve kontrol et
    $(document).on('click', function (e) {
        const $target = $(e.target);

        // Check for Survey Results filter dropdown
        const $resultsDropdownMenu = $('#resultsFilterDropdown .dropdown-menu');
        const $resultsDropdownToggle = $('#resultsFilterDropdown .dropdown-toggle');

        // Check for Personnel Stats filter dropdown
        const $personnelDropdownMenu = $('#personnelFilterDropdown .dropdown-menu');
        const $personnelDropdownToggle = $('#personnelFilterDropdown .dropdown-toggle');

        // Condition 1: Click is INSIDE the dropdown menu itself (excluding the toggle)
        const isClickInsideResultsDropdownMenu = $target.closest($resultsDropdownMenu).length && !$target.closest($resultsDropdownToggle).length;
        const isClickInsidePersonnelDropdownMenu = $target.closest($personnelDropdownMenu).length && !$target.closest($personnelDropdownToggle).length;

        // Condition 2: Click is INSIDE a daterangepicker element
        const isClickInsideDaterangePicker = $target.closest('.daterangepicker').length;

        // Condition 3: Click is on one of the custom date shortcut buttons
        const isClickOnCustomDateButton = $target.closest('.tarihAraligi button').length;

        // If the click is inside a dropdown menu (but not its toggle), or inside the daterangepicker, or on a custom button
        if (isClickInsideResultsDropdownMenu || isClickInsidePersonnelDropdownMenu || isClickInsideDaterangePicker || isClickOnCustomDateButton) {
            e.stopPropagation(); // Prevent Bootstrap from closing the dropdown
        }
    });

    // Ensure clicks on the date range inputs themselves don't close the dropdown
    $('#daterangeResults, #daterange').on('click', function(e) {
        e.stopPropagation();
    });

    // Custom date range buttons also need to stop propagation to prevent dropdown close
    // And then trigger the update functions
    $('#lastMonthResults').click(function(e) { e.stopPropagation(); updateResultsRange(moment().subtract(1, 'month'), today); });
    $('#last15DaysResults').click(function(e) { e.stopPropagation(); updateResultsRange(moment().subtract(15, 'days'), today); });
    $('#lastWeekResults').click(function(e) { e.stopPropagation(); updateResultsRange(moment().subtract(7, 'days'), today); });
    $('#yesterdayResults').click(function(e) { e.stopPropagation(); updateResultsRange(moment().subtract(1, 'days'), moment().subtract(1, 'days')); });
    $('#todayResults').click(function(e) { e.stopPropagation(); updateResultsRange(today, today); });

    $('#personnelFilterDropdown .btnLastMonth').click(function(e) { e.stopPropagation(); updateRange(moment().subtract(1, 'month'), today); });
    $('#personnelFilterDropdown .btnLast15Days').click(function(e) { e.stopPropagation(); updateRange(moment().subtract(15, 'days'), today); });
    $('#personnelFilterDropdown .btnLastWeek').click(function(e) { e.stopPropagation(); updateRange(moment().subtract(7, 'days'), today); });
    $('#personnelFilterDropdown .btnYesterday').click(function(e) { e.stopPropagation(); updateRange(moment().subtract(1, 'days'), moment().subtract(1, 'days')); });
    $('#personnelFilterDropdown .btnToday').click(function(e) { e.stopPropagation(); updateRange(today, today); });
