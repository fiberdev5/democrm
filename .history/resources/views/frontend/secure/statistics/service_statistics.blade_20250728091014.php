...
<div class="page-content servis-istatistik">
    <div class="container-fluid">
        ...
        @else
            <!-- Tab-style Period Statistics -->
            <div class="card shadow-sm">
                <!-- Tab Navigation -->
                <div class="period-tabs d-flex flex-wrap bg-light border-bottom">
                    @foreach($periodStats as $key => $period)
                        <button class="period-tab flex-fill text-center border-0 bg-transparent py-3 px-4 {{ $key === 'bugun' ? 'active' : '' }}" 
                                data-period="{{ $key }}" 
                                style="position: relative; transition: all 0.3s ease; color: #6c757d; font-weight: 500;">
                            <i class="fas fa-calendar-day me-2"></i>
                            {{ $period['label'] }}
                            <span class="badge bg-primary ms-2">{{ $period['toplam'] }}</span>
                        </button>
                    @endforeach
                </div>
                ...
                <div class="tab-content-wrapper">
                    @foreach($periodStats as $key => $period)
                        <div class="tab-pane-content {{ $key === 'bugun' ? 'active' : '' }}" 
                             data-content="{{ $key }}" 
                             style="display: {{ $key === 'bugun' ? 'block' : 'none' }};">
                            <div class="p-4">
                                <div class="row g-4">
                                    <!-- Markalar -->
                                    <div class="col-lg-3 col-md-6">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    periodTabs.forEach(tab => {
        tab.addEventListener('click', function() {
            ...
            // Show target content
            const targetContent = document.querySelector(`[data-content="${targetPeriod}"]`);
            if (targetContent) {
