...
$(document).ready(function () {
    ...
    statsArray.forEach(function(personnel) {
        let detailUrl = `/secure/${TENANT_ID}/personel-anket-detay/${personnel.personel_id}`;

        let row = `<tr>
            <td>${personnel.adsoyad}</td>
            <td>${personnel.servisler.length}</td>
            <td>${personnel.servisler.length}</td>
            <td>
...
.searchWrap {
    position: relative;
    z-index: 1050; /* Bootstrap modal z-index'ine yakın yüksek değer */
}

.card {
    overflow: visible !important; /* dropdown dışarı taşabilsin */
}
...
.dropdown-menu {
    left: auto !important;
    right: 0; !important;
    transform: translateX(-100%); /* Sola kaydırır */
}
