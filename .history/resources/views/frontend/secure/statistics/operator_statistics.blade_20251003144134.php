...
<style>
    /* Specific styles for small screens to align search and filter button */
    @media (max-width: 991.98px) { /* Targeting Bootstrap's 'lg' breakpoint for consistency */
        .dataTables_filter {
            display: flex; /* Use flexbox to align items horizontally */
            align-items: center; /* Vertically align items in the middle */
            margin-bottom: 15px; /* Add some space below the filter row */
        ...
        .dataTables_filter input {
            ...
            border-top-right-radius: 0; /* Make input edges flush with button */
            border-bottom-right-radius: 0;
        }

        .dataTables_filter .searchWrap {
            flex-shrink: 0; /* Prevent the filter button from shrinking */
            margin-top: 0 !important; /* Override any default margin */
            margin-bottom: 0 !important; /* Override any default margin */
        }

        /* Adjust the dropdown menu's position to avoid being cut off */
        .dataTables_filter .dropdown-menu {
            left: auto !important; /* Reset left positioning */
            right: 0 !important; /* Align dropdown to the right of the button */
        }

        /* If the input-group class is added by JS, ensure it works correctly */
        .dataTables_filter.input-group {
            /* No specific styles needed here, flexbox on .dataTables_filter handles it */
        }
    }

    /* Your existing specific styles for smaller mobile screens (if still needed, adjust breakpoint) */
    @media (max-width: 767px) {
        /* You might not need these anymore with the .dataTables_filter flexbox setup */
        /* Test this to see if the above @media (max-width: 991.98px) covers it */
        /* .searchWrap{margin-top: 0px !important;} */
        /* .pageDetail .searchWrap{width: auto !important;} */ /* Let flexbox manage width */
        /* .pageDetail .searchWrap{margin-bottom: 0px !important;} */
        /* div.dataTables_filter input{margin-left: 0 !important;} */
    }

    /* Avatar styling for the operator name column */
    .avatar {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        width: 32px; /* Or whatever size you prefer */
        height: 32px; /* Or whatever size you prefer */
        border-radius: 50%;
            ...
            <div class="col-12">
                ...
                <div class="table-modern">
                    ...
                    <div class="card-body">
                        ...
                        {{-- This wrapper will be moved by JS --}}
...
$(document).ready(function () {
    ...
    var table = $('#datatableOperatorStats').DataTable({
        ...
        initComplete: function(settings, json) {
            ...
            // Mobilde filtre butonunu search yanına taşı
            if (window.matchMedia("(max-width: 991.98px)").matches) {
                var mobileFilterWrapper = $('.d-lg-none.searchWrap').detach(); // Detach to ensure it's moved
                searchContainer.append(mobileFilterWrapper);
                searchContainer.addClass('input-group');
                // Removed specific border-radius adjustment as the new CSS handles it better within flexbox
            }
            
            // Filtreleri görünür yap
            $('.searchWrap').css({ visibility: 'visible', opacity: 1 });
        }
    });

    // Example for datatableOffer (if you have another one)
    // You would replicate the same initComplete logic for it
    /*
    var tableOffer = $('#datatableOffer').DataTable({
        ...
        initComplete: function(settings, json) {
            ...
            if (window.matchMedia("(max-width: 991.98px)").matches) {
                var mobileFilterWrapper = $('.d-lg-none.searchWrap').detach(); // Assuming you have a similar mobile filter for offers
                searchContainer.append(mobileFilterWrapper);
                searchContainer.addClass('input-group');
            }
            $('.searchWrap').css({ visibility: 'visible', opacity: 1 });
