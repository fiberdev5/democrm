            ...
            <div class="col-12">
                <div class="table-modern">
                    <div class="card-header">
                        ...
                        <div class="searchWrap float-end" id="personel_anket">
                            <div class="btn-group mb-2">
...
$(document).ready(function () {
    ...
    // ANKET SONUÇLARI filtre dropdown'ının kapanmasını engelle
    $('.searchWrap .dropdown-menu').on('click', function(e) {
        // Eğer tıklama datepicker'ın kendisine veya içindeki bir elemente geliyorsa
        // olayın yayılmasını durdur (stopPropagation)
        // daterangepicker'ın oluşturduğu ana kapsayıcı genellikle 'daterangepicker' sınıfına sahiptir.
        if ($(e.target).closest('.daterangepicker').length || $(e.target).is('#daterangeResults') || $(e.target).closest('.tarihAraligi button').length) {
            e.stopPropagation();
        }
    });

    // PERSONEL BAZINDA ANKET İSTATİSTİKLERİ filtre dropdown'ının kapanmasını engelle
    $('#personel_anket .dropdown-menu').on('click', function(e) {
        // Aynı şekilde daterangepicker'ın kendi elementleri için de kontrol yapıyoruz
        if ($(e.target).closest('.daterangepicker').length || $(e.target).is('#daterange') || $(e.target).closest('.tarihAraligi button').length) { 
            e.stopPropagation();
        }
    });
    
    // Bootstrap 5'in dropdown kapanma davranışını değiştirmek için bir genel çözüm
    // Bu, dropdown'un, içinde bulunan datepicker'dan gelen olaylara tepki vermesini engeller.
    $(document).on('click', function(e) {
        var $target = $(e.target);
        // Anket Sonuçları için dropdown kontrolü
        if ($target.closest('.searchWrap .dropdown-menu').length) {
            // Eğer tıklanan element, .searchWrap içindeki dropdown menünün bir parçasıysa,
            // ve bu bir datepicker elementi veya kısayol butonu değilse, normalde kapanmasını engelleriz.
            // Ancak asıl sorun datepicker kendi elemanları dışına tıklandığında kapanması
            // bu yüzden burada daha çok datepicker'ın kendisi dışındaki tıklamaları ele alıyoruz
            if ($target.closest('.daterangepicker').length || $target.closest('.tarihAraligi button').length || $target.is('#daterangeResults')) {
                e.stopPropagation();
            }
        }
        // Personel Bazında Anket İstatistikleri için dropdown kontrolü
        if ($target.closest('#personel_anket .dropdown-menu').length) {
             if ($target.closest('.daterangepicker').length || $target.closest('.tarihAraligi button').length || $target.is('#daterange')) {
                e.stopPropagation();
            }
        }
    });
    ...
    $('#personel_anket .btnLastMonth').click(function() {
        updateRange(moment().subtract(1, 'month'), today);
    });
    $('#personel_anket .btnLast15Days').click(function() {
        updateRange(moment().subtract(15, 'days'), today);
    });
    $('#personel_anket .btnLastWeek').click(function() {
        updateRange(moment().subtract(7, 'days'), today);
    });
    $('#personel_anket .btnYesterday').click(function() {
        updateRange(moment().subtract(1, 'days'), moment().subtract(1, 'days'));
    });
    $('#personel_anket .btnToday').click(function() {
