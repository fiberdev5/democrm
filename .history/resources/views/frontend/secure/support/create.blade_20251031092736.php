...
// Form submit koruması
$('#supportTicketForm').submit(function(event) {
    // Token kontrolü - zaten gönderiliyorsa engelle
    if (formSubmitting) {
        event.preventDefault();
        alert('Destek talebi gönderiliyor, lütfen bekleyin...');
        return false;
    }
    
    // Form validasyonu (mevcut required alanlar tarayıcı tarafından kontrol ediliyor)
    // Ekstra kontroller gerekirse buraya eklenebilir
    
    // Butonu disable et ve işlemi başlat
    formSubmitting = true;
    $(this).find('button[type="submit"]').prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-2"></i> Gönderiliyor...');
    
    // 5 saniye sonra yeniden aktif et (sunucu yanıtı gelmezse)
    setTimeout(function() {
        $('#formToken').val(generateToken());
        formSubmitting = false;
        $('#supportTicketForm button[type="submit"]')
            .prop('disabled', false)
            .html('<i class="fas fa-paper-plane me-2"></i> Destek Talebi Gönder');
    }, 5000);
