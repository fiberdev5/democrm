...
$(document).ready(function() {
    ...
    function parseCustomDate(dateStr, timeStr) {
        ...
        if (dateParts.length === 3) {
            ...
            var month = parseInt(dateParts[1]) - 1; // ay 0-bazlı
    ...
    var table = $('#supportTicketsTable').DataTable({
        ...
        order: [[0, 'desc']], // Başlangıç sıralaması
        
        columnDefs: [
            {
                targets: [5, 6], // Tarih sütunları
                render: function(data, type, row, meta) {
                    if (type === 'sort' || type === 'display') {
                        if (meta.col === 5) {
                            // Oluşturma tarihi sütunu
                            var $cell = $(data);
                            var dateText = $cell.find('.fw-medium').length ? $cell.find('.fw-medium').text() : $cell.text();
                            var timeText = $cell.find('.text-muted').length ? $cell.find('.text-muted').text() : '';
                            
                            if (type === 'sort') {
                                return parseCustomDate(dateText, timeText).getTime();
                            }
                        } else if (meta.col === 6) {
                            // Son yanıt tarihi sütunu
                            var $cell = $(data);
                            var dateText = $cell.find('.fw-medium').length ? $cell.find('.fw-medium').text() : $cell.text();
                            var timeText = $cell.find('.text-muted').length ? $cell.find('.text-muted').text() : '';
                            
                            if (type === 'sort') {
                                // Son yanıt tarihi ile oluşturma tarihini karşılaştır
                                var currentRow = table.row(meta.row).node();
                                var $currentRow = $(currentRow);
                                
                                // Oluşturma tarihi
                                var createdDateText = $currentRow.find('td:eq(5) .fw-medium').text();
                                var createdTimeText = $currentRow.find('td:eq(5) .text-muted').text();
                                var createdDate = parseCustomDate(createdDateText, createdTimeText);
                                
                                // Son yanıt tarihi
                                var lastReplyDate = parseCustomDate(dateText, timeText);
                                
                                // En güncel olanı döndür
                                return Math.max(createdDate.getTime(), lastReplyDate.getTime());
                            }
                        }
                    }
                    return data;
        ...
        // DOM yapısını düzelt - sayfalama için
        dom: '<"row"<"col-sm-6"l><"col-sm-6"f>>rt<"row"<"col-sm-6"i><"col-sm-6"p>>',
        lengthMenu: [ [25, 50, 100], [25, 50, 100] ],
        
        // Sayfa yüklendikten sonra son yanıt tarihine göre sırala
