...
function getPaytrToken() {
    const paytrData = @json($paytrData);
    
    // PayTR token isteği için form data hazırla
    const formData = new FormData();
    
    Object.keys(paytrData).forEach(key => {
        formData.append(key, paytrData[key]);
    });
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            iframe.src = 'https://www.paytr.com/odeme/api/get-token' + data.token;
            ...
            showError('PayTR Hatası', data.reason || 'Ödeme sayfası oluşturulamadı.');
        }
    })
    .catch(error => {
        console.error('PayTR API Error:', error);
        showError('Bağlantı Hatası', 'PayTR API\'ye bağlanılamadı.');
    });
}
...
function showError(title, message) {
    document.getElementById('paytr-iframe-container').innerHTML = `
        <div class="alert alert-danger text-center">
            ...
            <h5>${title}</h5>
            <p>${message}</p>
            <button onclick="getPaytrToken()" class="btn btn-primary me-2">
