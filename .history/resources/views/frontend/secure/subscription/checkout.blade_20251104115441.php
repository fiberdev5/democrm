    ...
    <form action="{{ route('subscription.process', [$tenant_id, $planid]) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div class="space-y-2">
          ...
          <!-- Kurumsal Alanları -->
          <div id="kurumsal-fields" class="space-y-2" style="display:none;">
            <div class="grid grid-cols-3 gap-1 items-center">
          ...
          <div class="grid grid-cols-3 gap-1 items-start">
            ...
            <textarea name="address" rows="2" class="col-span-2 border rounded-lg p-2 focus:ring-2 focus:ring-orange-400">{{ old('address', $tenant->adres ?? '') }}</textarea>
          </div>
        </div>
      </div>
