...
<div class="page-content">
  <div class="container-fluid py-8">
    ...
    <form action="{{ route('subscription.process', [$tenant_id, $planid]) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-2">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6">
        <h3 class="text-lg font-semibold text-gray-700 mb-4">Fatura Bilgileri (Kurumsal)</h3>

        <!-- Form alanları -->
        <div class="space-y-3">
          <div class="flex gap-2 items-center">
            <label class="text-sm font-medium text-gray-600 w-32">Firma Adı</label>
          ...
          <!-- Kurumsal Alanları (her zaman görünür) -->
          <div class="space-y-3">
            <div class="flex gap-2 items-center">
              <label class="text-sm font-medium text-gray-600 w-32">Vergi Dairesi</label>
              <input type="text" name="tax_office" class="flex-1 border rounded-lg p-2 focus:ring-2 focus:ring-orange-400" value="{{ old('tax_office', $tenant->vergiDairesi ?? '') }}" required>
            </div>

            <div class="flex gap-2 items-center">
              <label class="text-sm font-medium text-gray-600 w-32">Vergi Numarası</label>
              <input type="text" name="tax_number" class="flex-1 border rounded-lg p-2 focus:ring-2 focus:ring-orange-400" value="{{ old('tax_number', $tenant->vergiNo ?? '') }}" required>
            </div>
          </div>
          ...
          <div class="flex gap-2 items-start">
            ...
            <textarea name="address" rows="3" class="flex-1 border rounded-lg p-2 focus:ring-2 focus:ring-orange-400" required>{{ old('address', $tenant->adres ?? '') }}</textarea>
          </div>
        </div>
