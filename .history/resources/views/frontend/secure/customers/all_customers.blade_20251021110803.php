...
function getFilterInfoForPrint() {
  ...
  var table = $('#datatableCustomer').DataTable({
      ...
      dom: 'B<"top"f>rt<"bottom"i<"float-end"lp>><"clear">',
       // YENİ: Buttons yapılandırması
    buttons: [{
      extend: 'print',
      text: 'Yazdır',
      autoPrint: true,
      
      // YENİ: Başlık ve mesajları sıfırlayarak tekrarı önle
      title: '', // DataTables'ın otomatik başlık eklemesini engelle
      messageTop: '', // DataTables'ın otomatik üst mesaj eklemesini engelle
      ...
      customize: function (win) {
        ...
        // 1. Adım: Mevcut tüm stilleri temizle
        $(win.document.head).find('style, link').remove();

        // 2. Adım: Yeni yazdırma düzeni için özel CSS stilleri ekle
        $(win.document.head).append(
          '<style>' +
          'body { font-family: sans-serif; margin: 25px; font-size: 12px; }' +
          '.print-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #666; padding-bottom: 10px; margin-bottom: 25px; font-size: 14px;}' +
          '.print-title { text-align: left; font-size: 20px; font-weight: bold; margin-bottom: 15px; }' +
          'table { width: 100%; border-collapse: collapse; }' +
          'table th, table td { border-top: 1px solid #ddd; border-bottom: 1px solid #ddd; padding: 9px; text-align: left; vertical-align: top; }' + // Dikey hizalama eklendi
          'table th { background-color: #f8f8f8; font-weight: bold; }' +
          '.print-footer { margin-top: 15px; text-align: left; font-size: 12px; border-top: 1px solid #ccc; padding-top: 10px; }' +
          '</style>'
        );

        // 3. Adım: Gerekli bilgileri al
        ...
        // 4. Adım: DataTables'ın otomatik eklediği başlığı (h1) kaldır (önlem olarak)
        $(win.document.body).find('h1').remove();

        // 5. Adım: Sayfanın en başına özel başlığı (Tarih ve Firma Adı) ekle
        ...
        // 6. Adım: Tablodan önce "Tüm Müşteriler" başlığını ekle
        var title = '<div class="print-title">Tüm Müşteriler</div>';
        $(win.document.body).find('table').before(title);
        
        // YENİ 7. Adım: Hücrelerdeki "Ad Soyad:", "Telefon:" gibi gereksiz metinleri temizle
        $(win.document.body).find('table tbody tr').each(function() {
            // Sütun indeksleri: 1->Ad Soyad, 2->Telefon, 3->Adres
            var adSoyadCell = $(this).find('td:eq(1)');
            var telefonCell = $(this).find('td:eq(2)');
            var adresCell = $(this).find('td:eq(3)');

            // Hücrelerin metinlerini al, istenmeyen kısımları sil ve geri yaz
            adSoyadCell.text( adSoyadCell.text().replace('Ad Soyad:', '').trim() );
            telefonCell.text( telefonCell.text().replace('Telefon:', '').trim() );
            adresCell.text( adresCell.text().replace('Adres:', '').trim() );
        });

        // 8. Adım: Tablonun altına alt bilgiyi ekle
