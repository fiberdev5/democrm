 ...
 // Edit Customer Modal - Buton click event'i
  ...
  var table = $('#datatableCustomer').DataTable({
    ...
    buttons: [{
      ...
      exportOptions: {
        stripHtml: false,
        columns: [0, 1, 2, 3] // Sadece ID, Ad Soyad, Telefon, Adres
      },
      customize: function (win) {
        // Başlık bilgileri
        var filterInfo = getFilterInfoForPrint();
        ...
        $(win.document.body).find('h1')
          .css('font-size', '17px')
          .html('{{ $firma->firma_adi ?? "Firma Adı" }} - Müşteri Listesi');
        
        // Tablo stilleri
        $(win.document.body).find('table')
          .addClass('musteriDataTableYazdirTable')
          .css({
            'width': '100%',
            'border-collapse': 'collapse'
          });
        ...
        // Bilgi satırını ekle
        var infoRow = '<tr><td colspan="4" style="border: 1px solid #ddd; padding: 10px; background-color: #f9f9f9;">' +
                      '<strong>Filtre Bilgileri:</strong><br>' +
                      filterInfo +
                      '<br><strong>Listelenen Müşteri Sayısı:</strong> ' + totalRecords + 
                      '<br><strong>Yazdırma Tarihi:</strong> ' + dateNow +
                      '</td></tr>';
  ...
  function getFilterInfo() {
    ...
    if (musteriTipi) {
        ...
        filters.push(`<strong>Müşteri Tipi:</strong> ${tipText}`);
    }
    
    var il = $('#countrySelect').val();
    if (il) {
        var ilText = $('#countrySelect option:selected').text();
        filters.push(`<strong>İl:</strong> ${ilText}`);
    }
    
    var ilce = $('#citySelect').val();
    if (ilce) {
        var ilceText = $('#citySelect option:selected').text();
        filters.push(`<strong>İlçe:</strong> ${ilceText}`);
    }
    
    var dateRange = $('#daterangeCustomer').val();
    if (dateRange) {
        filters.push(`<strong>Tarih Aralığı:</strong> ${dateRange}`);
    }
    
    var searchTerm = $('input[type="search"]').val();
    if (searchTerm) {
        filters.push(`<strong>Arama:</strong> ${searchTerm}`);
    }
    
    if (filters.length > 0) {
        return '<p><strong>Uygulanan Filtreler:</strong><br>' + filters.join('<br>') + '</p>';
    }
  ...
  function printCustomerTable() {
    ...
    if (data.length === 0) {
        alert('Yazdırılacak müşteri bulunamadı!');
        return;
    }
    ...
    printWindow.document.write('<html><head><title>Müşteri Listesi</title>');
    ...
    printWindow.document.write(`
        ...
        .print-footer { margin-top: 20px; text-align: center; font-size: 12px; border-top: 1px solid #ddd; padding-top: 10px; }
    ...
    printWindow.document.write(`
        <div class="print-header">
            ...
            <p><strong>Firma:</strong> {{ $firma->firma_adi ?? 'Firma Adı' }}</p>
            <p><strong>Yazdırma Tarihi:</strong> ${moment().format('DD-MM-YYYY HH:mm')}</p>
            ${filterInfo}
            <p><strong>Toplam Müşteri:</strong> ${data.length}</p>
        </div>
        
        <button class="no-print" onclick="window.print()" style="margin-bottom: 10px; padding: 10px 20px; cursor: pointer;">Yazdır</button>
    ...
    data.each(function(customer, index) {
        var name = $('<div>').html(customer.name).text();
        var tel = $('<div>').html(customer.tel).text();
        var address = $('<div>').html(customer.address).text();
    ...
    printWindow.document.write(`
        ...
        <div class="print-footer">
            <p>Bu rapor ${moment().format('DD-MM-YYYY HH:mm')} tarihinde oluşturulmuştur.</p>
        </div>
    `);
