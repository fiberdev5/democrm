...
$('#datatableCustomer tbody').on('click', 'tr', function(e) {
    ...
    // Eğer bir butona veya linke tıklanmışsa, hiçbir şey yapma
    if ($target.is('a, button, i') || $target.closest('a, button').length > 0) {
        return;
    }
    
    // Action sütununa tıklanmışsa, hiçbir şey yapma
    if ($target.closest('td').index() === 4) {
        return;
    }
