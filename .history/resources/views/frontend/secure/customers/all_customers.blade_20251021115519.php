...
buttons: [{
  ...
  customize: function (win) {
    ...
    $(win.document.head).append(
      '<style>' +
      'body { margin: 10px; font-family: sans-serif; font-size: 11px; }' +
      '.print-header { display: flex; justify-content: space-between; align-items: center; padding-bottom: 10px; margin-bottom: 25px; }' +
      '.print-title { text-align: left; font-size: 18px; font-weight: bold; margin-bottom: 20px; }' +
      ...
      '.print-footer { margin-top: 15px; text-align: left; border-top: 1px solid #ccc; padding-top: 10px; }' +
      // Sayfa numarası stili - @media print DIŞINDA
      '.page-number-container { text-align: center; margin-top: 20px; font-size: 10px; color: #666; }' +
      '@page { ' +
      '  margin: 10mm; ' +
      '  @bottom-center { content: "Sayfa " counter(page) " / " counter(pages); font-size: 10px; }' +
      '}' +
      '@media print { ' +
      '  thead { display: table-header-group; }' +
      '  tbody { page-break-inside: avoid; }' +
      '  a, a:link, a:visited { color: #000 !important; text-decoration: none !important; }' +
      '  .page-number-container { display: block; }' +
      '}' +
      '</style>'
    );
    
    // 3. Gerekli bilgileri al
    ...
    // 4. DataTables'ın otomatik eklediği başlığı kaldır
    $(win.document.body).find('h1').remove();
    
    // 5. Sayfanın en başına tarih ve firma adını ekle
    ...
    // 6. "Tüm Müşteriler" başlığını ekle
    ...
    // 8. SAYFA NUMARASINI EKLE - footer'dan sonra
    var pageNumber = '<div class="page-number-container">' +
                     '  <span class="pageNumber"></span>' +
                     '</div>';
    $(win.document.body).append(pageNumber);
  }
}],
