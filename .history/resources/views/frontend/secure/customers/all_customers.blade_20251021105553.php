...
buttons: [{
  ...
  autoPrint: true,
  exportOptions: {
    columns: [0, 1, 2, 3], // Tüm kolonları al (ID dahil)
    format: {
      body: function (data, row, column, node) {
        // HTML taglerini temizle, sadece text al
  ...
  customize: function (win) {
    ...
    $(win.document.body).find('table').before(
      ...
      '<strong>Filtre Bilgileri:</strong><br>' + filterInfo +
    ...
    // ID kolonunu gizle (ilk kolon)
    $(win.document.body).find('table th:first-child, table td:first-child').hide();
  }
}],
