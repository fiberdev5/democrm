...
$('#datatableCustomer tbody').on('click', 'tr', function(e) {
    ...
    if (id) {
        ...
        $.ajax({
            url: "/" + firma_id + "/musteri/duzenle/" + id,
            beforeSend: function() {
                // Minimal loading (opsiyonel)
                $('#editCustomerModal .modal-body').html('<div class="text-center p-2"><small>Yükleniyor...</small></div>');
            }
        }).done(function(data) {
            if ($.trim(data) === "-1") {
                window.location.reload(true);
...
// Satır tıklama event'i mobilde sadece yazının üzerine gelince açılıyordu modal
// Boşluğa tıklayınca da açılsın diye eklendi
// $('#datatableCustomer tbody').on('click', 'tr', function(e) {
//     // Eğer action sütununa (butonlara) tıklanmışsa, satır tıklamasını engelle
//     if ($(e.target).closest('td').index() === 4 || $(e.target).closest('a').hasClass('btn')) {
//         return;
//     }
