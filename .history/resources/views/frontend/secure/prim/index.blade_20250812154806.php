...
<script>
  ...
  // Modal yönetimi için özel fonksiyonlar
$(document).ready(function() {
  // Detay modal açıldığında
  $('#gunlukDetayModal').on('show.bs.modal', function () {
    // Ana modal'ın z-index'ini düşür
    $('#primModal').css('z-index', 1040);
    $(this).css('z-index', 1050);
  });

  // Detay modal kapandığında
  $('#gunlukDetayModal').on('hidden.bs.modal', function () {
    // Ana modal'ın z-index'ini geri yükselt
    $('#primModal').css('z-index', 1050);
    
    // Ana modal hala açıksa backdrop'u koru
    if ($('#primModal').hasClass('show')) {
      $('body').addClass('modal-open');
      // Eğer backdrop yoksa tekrar ekle
      if ($('.modal-backdrop').length === 0) {
        $('<div class="modal-backdrop fade show"></div>').appendTo('body');
      }
    }
    
    // Detay modal içeriğini temizle
    $('#gunlukDetayContent').html('');
  });

  // Ana modal kapandığında tüm içeriği temizle
  $('#primModal').on('hidden.bs.modal', function () {
    $(this).find('.modal-body').html('Yükleniyor...');
    // Tüm modal-backdrop'ları temizle
