...
<script>
// Bu fonksiyon, tüm DOM yüklendikten sonra çalışır.
document.addEventListener('DOMContentLoaded', function () {

    console.log("Sayfa yüklendi. Sekme script'i başlatılıyor.");

    // Tüm sekme butonlarını ve içerik alanlarını seçelim.
    const periodTabs = document.querySelectorAll('.period-tab-compact');
    const periodContents = document.querySelectorAll('.period-accordion .collapse');

    // Eğer buton bulunamadıysa konsola bilgi verelim.
    if (periodTabs.length === 0) {
        console.warn("UYARI: '.period-tab-compact' sınıfına sahip hiçbir buton bulunamadı.");
        return; // Fonksiyonu durdur.
    }
    console.log(periodTabs.length + " adet sekme butonu bulundu.");

    // Her bir butona tıklama olayı ekleyelim.
    periodTabs.forEach(tab => {
        tab.addEventListener('click', function (event) {
            
            // Tarayıcının varsayılan davranışını engelle (gerekli olmayabilir ama iyi bir pratiktir)
            event.preventDefault(); 
            
            console.log("Butona tıklandı:", this.textContent.trim());

            // --- 1. Butonların Aktif Durumunu Yönet ---
            periodTabs.forEach(item => item.classList.remove('active'));
            this.classList.add('active');
            console.log("Aktif sınıfı ayarlandı.");

            // --- 2. İçeriklerin Görünürlüğünü Yönet ---
            const targetSelector = this.getAttribute('data-target');
            if (!targetSelector) {
                console.error("HATA: Tıklanan butonun 'data-target' özelliği yok!", this);
                return;
            }
            console.log("Hedef seçici (selector):", targetSelector);

            // Önce tüm içerikleri gizleyelim
            periodContents.forEach(content => {
                // Eğer Bootstrap'in kendi JS'i varsa, bu daha güvenli bir gizleme yöntemidir.
                // Ama biz manuel yapıyoruz, o yüzden sadece 'show' sınıfını kaldırıyoruz.
                content.classList.remove('show');
            });
            console.log("Tüm içeriklerin 'show' sınıfı kaldırıldı.");

            const targetContent = document.querySelector(targetSelector);
            if (targetContent) {
                targetContent.classList.add('show');
                console.log("Hedef içerik bulundu ve 'show' sınıfı eklendi:", targetContent);
            } else {
                console.error("HATA: Hedef içerik bulunamadı! Seçici:", targetSelector);
            }
        });
    });
});
