<!-- Tenant Detay Modal -->
      ...
      <div class="card-footer bg-light d-flex justify-content-between align-items-center p-3">
        ...
        <div>
          ...
          <form action="{{ route('tenant.changeStatus', $tenant->id) }}" method="POST" style="display: inline;">
              @csrf <!-- Laravel Güvenlik Token'ı -->
              
              @if($tenant->status == 1)
                  <!-- Firma zaten aktifse, pasif yapma butonu göster -->
                  <button type="submit" class="btn btn-warning btn-sm">Pasif Yap</button>
              @else
                  <!-- Firma aktif değilse, aktif etme butonu göster -->
