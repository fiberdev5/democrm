...
document.addEventListener('DOMContentLoaded', function () {
    // Tüm sekme butonlarını seçelim
    const periodTabs = document.querySelectorAll('.period-tab-compact');
    
    // Tüm içerik alanlarını seçelim (accordion içindeki)
    const periodContents = document.querySelectorAll('.period-accordion .collapse');

    // Her bir sekme butonuna tıklama olayı ekleyelim
    periodTabs.forEach(function(tab) {
        tab.addEventListener('click', function () {
            
            // --- Butonların Aktif Durumunu Yönet ---
            // Önce tüm butonlardaki 'active' sınıfını kaldıralım
            periodTabs.forEach(function(item) {
                item.classList.remove('active');
            });
            // Sadece tıklanan bu butona 'active' sınıfını ekleyelim
            this.classList.add('active');

            
            // --- İçeriklerin Görünürlüğünü Yönet ---
            // Hedef içeriğin ID'sini butonun 'data-target' özelliğinden alalım
            const targetId = this.getAttribute('data-target');
            const targetContent = document.querySelector(targetId);

            // Önce tüm içerik alanlarındaki 'show' sınıfını kaldıralım (gizleyelim)
            periodContents.forEach(function(content) {
                content.classList.remove('show');
            });

            // Sadece hedeflenen içeriğe 'show' sınıfını ekleyerek görünür yapalım
            if (targetContent) {
