...
<body data-topbar="dark">
    ...
    <div id="layout-wrapper">
        ...
        <div class="main-content">
            @if (session('warning'))
                <div id="trial-warning-notification" class="alert-notification warning">
                    <span>{{ session('warning') }}</span>
                    <button type="button" class="close-notification" onclick="this.parentElement.style.display='none';">&times;</button>
                </div>
            @endif
            @yield('user')
            <!-- End Page-content -->
        ...
        @endif
         @if(session('warning'))
            toastr.options = {
                ...
                "progressBar": true, // İlerleme çubuğu göster (kapanmadığı için pek anlamı yok ama estetik)
                "positionClass": "toast-top-right", // Sağ üst köşe
                "timeOut": "0", // Otomatik kapanma süresi (0 = süresiz açık kalır)
                "extendedTimeOut": "0", // Kullanıcı etkileşimi sonrası uzatılmış süre (0 = süresiz)
                "tapToDismiss": false // Bildirime tıklayarak kapanmasını engeller
            };
