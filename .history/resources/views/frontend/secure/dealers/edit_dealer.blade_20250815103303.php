<form method="post" id="editBayi" action="{{ route('update.dealer', [$firma->id, $bayi->user_id]) }}" enctype="multipart/form-data">
  ...
  <div class="row ayrilmaTarihi">
      ...
      <div class="col-sm-8">
          <input name="ayrilmaTarihi" class="form-control datepicker ayrilmaTarihi" type="date" value="{{$bayi->ayrilmaTarihi}}" style="border: 1px solid #ced4da;"required>
      </div>
  </div>
  ...
  <div class="row">
    ...
    <div class="col-sm-8">
      <input name="vergiNo" class="form-control" type="text" value="{{ $bayi->vergiNo }}">
    </div>
  </div>
  ...
  <div class="row">
    ...
    <div class="col-sm-8">
      <input name="vergiDairesi" class="form-control" type="text" value="{{ $bayi->vergiDairesi }}">
    </div>
  </div>
  ...
  <div class="row">
    ...
    <div class="col-sm-8">
      @if($bayi->belgePdf && file_exists(public_path('storage/' . $bayi->belgePdf)))
        <div class="mb-2">
          <small class="text-success">Mevcut belge:</small>
          @php
            $fileExtension = pathinfo($bayi->belgePdf, PATHINFO_EXTENSION);
          @endphp
          
          @if(in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png']))
            <div class="mt-1">
              <img src="{{ asset('storage/' . $bayi->belgePdf) }}" style="max-width: 100px; max-height: 100px; border: 1px solid #ddd;">
              <a href="{{ asset('storage/' . $bayi->belgePdf) }}" target="_blank" class="btn btn-sm btn-outline-primary ms-2">
          ...
          @else
            <div class="mt-1">
              ...
              <a href="{{ asset('storage/' . $bayi->belgePdf) }}" target="_blank" class="btn btn-sm btn-outline-danger">
                <i class="fas fa-eye"></i> PDF Görüntüle
              </a>
            </div>
          @endif
        </div>
      ...
      <input name="belgePdf" class="form-control mt-2" type="file" accept=".pdf,.jpg,.jpeg,.png,.svg">
      <small class="text-muted">Yeni belge yüklerseniz eskisi değiştirilir. PDF, JPG, PNG, SVG kabul edilir.</small>
    </div>
  </div>
    ...
    <div class="row">
      ...
      <div class="col-sm-8">
        <input name="tel" id="tel" class="form-control phone" type="text" value="{{ $bayi->tel }}">
      </div>
    </div>
