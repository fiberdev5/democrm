<form method="post" id="editBayi" action="{{ route('update.dealer', [$firma->id, $bayi->user_id]) }}" enctype="multipart/form-data">
  ...
  <div class="row">
    ...
    <div class="col-sm-8">
      @if($bayi->belgePdf)
        @php
          $belgeler = is_array($bayi->belgePdf) ? $bayi->belgePdf : json_decode($bayi->belgePdf, true) ?: [$bayi->belgePdf];
        @endphp
        
        <div class="mb-2">
          <small class="text-success">Mevcut belgeler:</small>
          @foreach($belgeler as $belge)
            @if(file_exists(public_path('storage/' . $belge)))
              @php
              ...
              <div class="mt-1 p-2 border rounded">
                @if(in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png']))
                  <img src="{{ asset('storage/' . $belge) }}" style="max-width: 80px; max-height: 80px; border: 1px solid #ddd;">
                  <a href="{{ asset('storage/' . $belge) }}" target="_blank" class="btn btn-sm btn-outline-primary ms-2">
                    <i class="fas fa-eye"></i> Görüntüle
                  </a>
                @else
                  <i class="fas fa-file-pdf text-danger"></i> 
                  <a href="{{ asset('storage/' . $belge) }}" target="_blank" class="btn btn-sm btn-outline-danger">
