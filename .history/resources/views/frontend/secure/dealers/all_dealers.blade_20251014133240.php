...
@section('user')
            ...
            <div class="card-body">
              <table id="datatableBayi" class="table table-bordered dt-responsive nowrap"
                ...
                @if(is_null($dealerLimit) || $dealerLimit == -1 || $dealerAll < $dealerLimit)
                  <a data-bs-toggle="modal" data-bs-target="#addBayiModal" class="btn btn-success btn-sm addBayi"><i
                      class="fas fa-plus"></i><span>Bayi Ekle</span></a>
                @else
                  <a data-bs-toggle="modal" data-bs-target="#addBayiModal" class="btn btn-success btn-sm addBayi"
                    disabled="disabled" style="pointer-events: none;opacity: .4;cursor: default;"><i
                      class="fas fa-plus"></i><span>Bayi Ekle</span></a>
                  <span class="text-muted ms-2" style="position:absolute;left: 80px;top: 47px;">
  ...
  <script type="text/javascript">
    ...
    // Mobilde sadece yazının üzerine gelince açılıyordu modal
    // Boşluğa tıklayınca da açılsın diye eklendi
    $('#datatableBayi tbody').on('click', 'tr', function(e) {
      var $target = $(e.target);
      
      // Sadece butonlara tıklanmışsa engelle
      if ($target.closest('.btn').length > 0 || $target.closest('td').index() === 4) {
        return;
      }
