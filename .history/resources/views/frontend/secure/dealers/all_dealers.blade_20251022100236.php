...
<div class="dealer-buttons-container">
  ...
  <a href="javascript:void(0);" class="btn btn-primary btn-sm printDealers">
    <i class="fas fa-print"></i><span>Yazdır</span>
  </a>
</div>

@if(!is_null($dealerLimit) && $dealerLimit != -1 && $dealerAll >= $dealerLimit)
  <span class="text-muted dealer-limit-warning">
    <i class="fas fa-info-circle me-1"></i>Aboneliğinize göre bayi limiti doldu (maks: {{ $dealerLimit }})
  </span>
@endif
...
$(document).ready(function() {
  $('.printDealers').on('click', function(e) {
    ...
    var durumText = $('#durum option:selected').text();
    ...
    var html = `
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          table th { font-weight: bold; color: #333; }
      ...
      <body>
        ...
        <div class="info-section">
          <div class="info-item"><strong>Yazdırma Tarihi:</strong> ${currentDate}</div>
          <div class="info-item"><strong>Toplam Bayi:</strong> ${data.length}</div>
          <div class="info-item"><strong>Durum Filtresi:</strong> ${durumText}</div>
        </div>
    ...
    if (data.length > 0) {
      data.each(function(row, index) {
        ...
        var name = $('<div>').html(row.name).text();
        var grup = $('<div>').html(row.grup).text();
        var tel = $('<div>').html(row.tel).text();
        var address = $('<div>').html(row.address).text();
        var status = $('<div>').html(row.status).text();
      ...
      html += '<tr><td colspan="6" style="text-align: center; padding: 20px;">Kayıt bulunamadı</td></tr>';
    }
    ...
    html += `
        ...
        <div class="footer">
          <p>Bu rapor {{ $firma->name }} tarafından ${currentDate} tarihinde oluşturulmuştur.</p>
        </div>
