<form method="post" id="editBayi" action="{{ route('update.dealer', [$firma->id, $bayi->user_id]) }}" enctype="multipart/form-data">
 ...
 <div class="row">
    ...
    <div class="col-sm-8">
      @php
        // belgeler JSON formatında olduğu için decode ediyoruz. Hata almamak için boş bir dizi varsayıyoruz.
        $belgeler = json_decode($bayi->belgePdf, true) ?: [];
      @endphp

      @if(!empty($belgeler))
        <div class="mb-2">
          <small class="text-success">Mevcut belgeler:</small>
          {{-- Her bir belge için döngü oluşturuyoruz --}}
          @foreach($belgeler as $index => $belgePath)
            @php
              $fileExtension = pathinfo($belgePath, PATHINFO_EXTENSION);
              // ShowDealerDocument rotasına link veriyoruz.
              $viewUrl = route('dealers.document.show', ['tenant_id' => $firma->id, 'user_id' => $bayi->user_id, 'document_index' => $index]);
            @endphp
            
            @if(in_array(strtolower($fileExtension), ['jpg', 'jpeg', 'png', 'svg']))
              <div class="mt-2 d-flex align-items-center">
                <img src="{{ $viewUrl }}" style="width: 80px; height: 80px; object-fit: cover; border: 1px solid #ddd; margin-right: 10px;">
            ...
            @else
              <div class="mt-2">
                <i class="fas fa-file-pdf text-danger fa-2x" style="margin-right: 10px;"></i>
                <a href="{{ $viewUrl }}" target="_blank" class="btn btn-sm btn-outline-danger">
                  <i class="fas fa-eye"></i> Belgeyi Görüntüle ({{ $index + 1 }})
                </a>
              </div>
            @endif
          @endforeach
      ...
      {{-- 'name' özelliğini dizi olarak değiştiriyoruz ve 'multiple' ekliyoruz --}}
      <input name="belgePdf[]" class="form-control mt-3" type="file" accept=".pdf,.jpg,.jpeg,.png,.svg" multiple>
      <small class="text-muted">Yeni belge yüklerseniz eskileri silinir. En fazla 2 adet yükleyebilirsiniz.</small>
    </div>
</div>
