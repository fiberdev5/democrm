...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-info"id="saveStockInfoBtn" form="editStockUnifiedForm">Ürün Bilgilerini Kaydet</button>
</div>
<!-- Hareket Ekle Modal -->
  ...
  <div class="modal-dialog">
    <form id="hareketEkleForm" method="POST" action="{{ route('store.stock.action', request()->route('tenant_id')) }}">
      ...
      <div class="modal-content">
        ...
        <div id="tedarikciSelectDiv">  <!-- Kapsayıcı div eklendi -->
        ...
        type="button" id="addNewSupplierBtn" style="min-width: 27px; height:27px;">
  +
</button>
...
$(document).ready(function() {
   ...
   $('#editStockUnifiedForm').on('submit', function(e) {
    ...
    $form.find('.invalid-feedback').text(''); // Alan hata mesajlarını temizle

    $.ajax({
        url: url,
        method: 'POST', // Blade içinde @method('PUT') veya @method('PATCH') olmalı
        data: formData,
        beforeSend: function() {
        ...
        success: function(response) {
            ...
            // Başlıkta ürün adı varsa onu da güncelleyelim
    ...
    // Ana modal içeriğini geçici olarak karartabilir veya bir yükleniyor animasyonu gösterebiliriz (isteğe bağlı)
    var modalContent = $('#editStockModal .modal-content');

    $.ajax({
        // EditStock metodunu çağıran URL'i burada oluşturuyoruz.
     ...
     $('#hareketEkleForm').on('submit', function(e) {
        e.preventDefault(); // Formun normal gönderimini ve sayfa yenilenmesini engelle

        var form = $(this);
        var url = form.attr('action');
        var method = form.attr('method');
        var data = form.serialize(); // Form verilerini topla
        ...
        $.ajax({
            ...
            success: function(response) {
            // 1. Hareket ekleme modal'ını kapat ve formu temizle
            $('#hareketEkleModal').modal('hide');
            form[0].reset();

            // 2. Ana modal'ı güncellemek için gerekli ID'leri al
            var stockId = $('#modalStokId').val();
            var tenantId = "{{ request()->route('tenant_id') }}";

            // 3. Ana modal'ın içeriğini yenilemek için fonksiyonumuzu çağır
            refreshStockDetails(stockId, tenantId);
            
            // 4. Kullanıcıya başarı mesajını göster (tercihen alert yerine daha modern bir toast notification ile)
            // Şimdilik alert ile devam edelim.
            alert(response.message);
        },
                    error: function(xhr) {
                        // Sunucudan bir hata dönerse
                        var errorMessage = 'Bir hata oluştu.';
                        // Validasyon hatası varsa, onu göster
                        if (xhr.responseJSON && xhr.responseJSON.message) {
                            errorMessage = xhr.responseJSON.message;
                        } else if (xhr.responseJSON && xhr.responseJSON.errors) {
                            // İlk validasyon hatasını al
                            errorMessage = Object.values(xhr.responseJSON.errors)[0][0];
                        }
                        alert(errorMessage);
                    },
                    complete: function() {
                        // İstek tamamlandığında butonu eski haline getir
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.status === 'success') {
                    // Sayfayı yenilemek yerine, sadece ilgili satırı tablodan kaldır
