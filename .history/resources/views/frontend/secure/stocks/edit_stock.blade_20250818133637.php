...
<div class="modal-body">
    ...
    <div class="row">
        <!-- ============================================= -->
        <!-- ============== SOL SÜTUN (GÜNCELLENMİŞ VE BİRLEŞTİRİLMİŞ) ============== -->
        <!-- ============================================= -->
        <div class="col-lg-5">
            
            <div class="card h-100">
                <div class="card-header">
                    <h5><i class="bi bi-pencil-square"></i> Ürün Detayları ve Yönetimi</h5>
                </div>
                <div class="card-body py-2 px-3 scrollable-card-body-left">
                    ...
                    <form method="POST" id="editStockUnifiedForm" action="{{ route('update.stock', [$firma->id, $stock->id]) }}">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Markalar</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Cihaz Türleri</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Kategori</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Raf</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Ürün Adı</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Ürün Kodu</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row mb-1 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Satış Fiyatı</label>
                            <div class="col-sm-8">
                                <div class="input-group input-group-sm">
                                    <input name="fiyat" type="number" min="0" step="0.01" class="form-control" placeholder="Fiyat" value="{{ $stock->fiyat }}">
                                    <select name="fiyatBirim" class="form-select" style="max-width: 70px;">
                        ...
                        <div class="row mb-2 align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Açıklama</label>
                            <div class="col-sm-8">
                        ...
                        <div class="row align-items-center">
                            <label class="col-sm-4 col-form-label-sm">Stok Durumu</label>
                            <div class="col-sm-8">
                    ...
                    <!-- Fotoğraflar Bölümü -->
                    <h5 class="mb-2"><i class="bi bi-images"></i> Fotoğraflar</h5>
                    ...
                    <div class="row imgBox mt-2">
                        ...
                        @else
                            <div class="col-12">
                                <div class="alert alert-secondary text-center p-2">Henüz fotoğraf eklenmemiş.</div>
                            </div>
                        @endif
                    </div>
            ...
            <div class="card mb-3">
                <div class="card-header ch1">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <select class="form-select-sm islemSec d-inline-block" name="islemSec" style="width: auto;">
    ...
    <form id="hareketEkleForm" method="POST" action="{{ route('store.stock.action', request()->route('tenant_id')) }}">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-2" id="tedarikciSelectDiv">
...
<!-- İKİNCİ MODAL YANLIŞLIKLA AYNI ID İLE EKLENMİŞTİ, BU BLOK KALDIRILDI -->

<style>
/* Sağ taraftaki KARTLAR için kaydırma alanı yüksekliği */
.scrollable-card-body {
    max-height: 280px; 
    overflow-y: auto;
}
/* SOL taraftaki TEK KART için kaydırma alanı yüksekliği (Sağdaki iki kartın toplamına yakın) */
.scrollable-card-body-left {
    max-height: 620px; /* Bu değeri kendi ekranınıza göre ayarlayabilirsiniz */
    overflow-y: auto;
}

/* Tablolardaki başlıkları sabitler */
.scrollable-card-body thead th,
.scrollable-card-body-left thead th {
    position: sticky; 
    top: 0; 
...
.toplam-header-row {
    ...
    position: sticky;
    top: 35px; /* th yüksekliğine göre ayarlayın */
    z-index: 1;
}
</style>
...
$(document).ready(function() {
    ...
    // Ürün kodu maskesi (Eğer bu kütüphaneyi kullanmıyorsanız bu satırı kaldırabilirsiniz)
    if($.fn.mask) {
    ...
    $('#editStockUnifiedForm').submit(function(e){
        ...
        if(urunKodu.length > 0 && urunKodu.length !== 13){
            e.preventDefault();
            Swal.fire('Uyarı', 'Ürün kodu tam 13 haneli olmalıdır!', 'warning');
    ...
    $('.islemSec').on('change', function () {       
        ...
        // Görünen satırlar üzerinden toplamları yeniden hesapla
        card.find('table tbody tr:visible').each(function() {
            var islemTipi = $(this).find('.tdNumber').text().trim().split(',')[1];
    ...
    $('#hareketEkleModal select[name="islem"]').on('change', function() {       
        var val = $(this).val();             
        if (val == '1') { // Alış         
            $('#fiyatInputDiv').show();         
            $('#fiyatInputDiv input').prop('required', true);          
            $('#personelSelectDiv').addClass('d-none');         
            $('#personelSelectDiv select').prop('required', false).val('');          
            $('#tedarikciSelectDiv').show();       
            ...
            $('#tedarikciSelectDiv').hide();
            $('#tedarikciSelectDiv select').val('');
        }     
    }).trigger('change');   
    ...
    $('#customFile').on("change", function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                $(".imgLoad").hide();
                $('#customFile').val('');
                // Eğer "fotoğraf yok" uyarısı varsa onu kaldır
                if($('.imgBox .alert').length) {
                    $('.imgBox').empty();
                }
                $('.imgBox').prepend(`<div class="col-4 col-md-3 stn mb-2" data-id="${res.id}"><img src="${res.resim_yolu}" class="img-fluid border rounded"><button class="btn btn-danger btn-sm w-100 stokFotoSil mt-1 py-0" data-id="${res.id}"><i class="fas fa-trash-alt"></i></button></div>`);
            },
            error: function (xhr) {
                $(".imgLoad").hide();
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                fotoDiv.fadeOut(300, function () { 
                    $(this).remove(); 
                    // Eğer hiç fotoğraf kalmadıysa uyarıyı göster
                    if($('.imgBox .stn').length === 0) {
                        $('.imgBox').html('<div class="col-12"><div class="alert alert-secondary text-center p-2">Henüz fotoğraf eklenmemiş.</div></div>');
                    }
                });
            },
            error: function (xhr) {
