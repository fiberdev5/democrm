      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <button type="button" class="btn btn-warning btn-sm" id="customPrintBtn">
...
$(document).ready(function () {
  var table = $('#datatableStock').DataTable({
      ...
      // DOM düzeni - sadece temel layout, butonlar ayrı yerde
      ...
      buttons: [
          {
              ...
              className: 'btn btn-warning btn-sm d-none', // Gizli tut
              title: 'Stok Raporu',
              exportOptions: {
                  columns: [0, 1, 2, 3, 4, 5, 6, 7] // Eylem sütunu hariç
              },
              customize: function ( win ) {
                  $(win.document.body).find('table').addClass('compact').css('font-size', 'inherit');
                  $(win.document.body).find('h1').css('text-align', 'center');
                  
                  // Toplam değerleri ekleme
                  var toplamAdetText = $('#toplamAdet').text();
                  var toplamFiyatText = $('#toplamFiyat').text();
                  $(win.document.body).append('<div style="font-size: 14px; margin-top: 20px;">' +
                                                '<div><strong>Toplam Adet:</strong> ' + toplamAdetText + '</div>' +
                                                '<div><strong>Toplam Değer:</strong> ' + toplamFiyatText + '</div>' + 
                                              '</div>');
              }
          }
      ],
      drawCallback: function(settings) {
        ...
        // Yazdır butonunu özel konuma taşı
        if ($('#printButtonContainer').is(':empty')) {
          var printButton = '<button type="button" class="btn btn-warning btn-sm" id="customPrintBtn">' +
                           '<i class="fas fa-print"></i> Yazdır</button>';
          $('#printButtonContainer').html(printButton);
        }
      },
      lengthMenu: [ [25, 50, 100, -1], [25, 50, 100, "Tümü"] ]
  });

  // Filtreler değiştiğinde tabloyu yeniden çiz - seçici düzeltildi
  $('#raf, #marka, #cihaz, #personel').change(function() {
      table.draw();
  });

  // Özel yazdır butonuna tıklama olayı
  $(document).on('click', '#customPrintBtn', function() {
