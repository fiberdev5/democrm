...
// Bu kod bloğunu, mevcut '#openEditModalBtn' click event'inizin yerine koyun.
$(document).on('click', '#openEditModalBtn', function() {
    // Düzenleme formunun yükleneceği URL'yi al
    var url = $(this).data('url');

    // Stok Ekleme modalını bul
    var addStockModal = $('#addStockModal'); // <-- EĞER FARKLIYSA BU ID'Yİ DEĞİŞTİRİN

    // Stok Ekleme modalı tamamen kapandıktan sonra SADECE BİR KEZ çalışacak bir olay dinleyicisi ekle.
    addStockModal.one('hidden.bs.modal', function () {
        // AJAX isteği ile düzenleme formunun HTML içeriğini al
        $.ajax({
            url: url,
            type: 'GET',
            success: function(res) {
                if(res.html) {
                    // Gelen HTML'i düzenleme modalının içine yerleştir
                    $('#editStockModal .modal-body').html(res.html);
                    // Şimdi düzenleme modalını göster
    ...
    // Stok Ekleme modalını kapatma işlemini başlat.
    // Yukarıdaki 'hidden.bs.modal' olayı, bu işlem bittikten sonra tetiklenecek.
