...
$(document).ready(function () {
    ...
    // Form submit - DÜZELTİLDİ
    $('#addStock').submit(function(event) {
        // Önce tüm zorunlu alanları kontrol et
        var isValid = true;
        ...
        // Ürün kodu kontrolü - sadece dolu ise kontrol et
        var urunKoduInput = $('input[name="urunKodu"]');
        if (urunKoduInput.val() && urunKoduInput.val().trim() !== '') {
...
$(document).ready(function () {
    ...
    // Edit modal açma - DÜZELTİLDİ
    $(document).on('click', '#openEditModalBtn', function(e) {
        e.preventDefault(); // Sayfa yenilenmesini engelle
        var url = $(this).data('url');
        
        if (!url) {
            alert('Düzenleme URL\'si bulunamadı.');
            return;
        }

        // Loading göster
        $('#editStockModal .modal-body').html('<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Yükleniyor...</div>');
        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.html) {
                    ...
                    // Eğer direkt HTML döndürülüyorsa
                    $('#editStockModal .modal-body').html(res);
                } else {
            ...
            error: function(xhr, status, error) {
                ...
                console.error('Response:', xhr.responseText);
                $('#editStockModal .modal-body').html('<div class="alert alert-danger">Düzenleme formu yüklenirken hata oluştu: ' + error + '</div>');
            }
        });
    });

    // Modal davranışları - DÜZELTİLDİ
    $('#editStockModal').on('show.bs.modal', function () {
        // Ana formu gizlemek yerine sadece z-index ayarla
    ...
    // Diğer modal işlemleri (Marka, Cihaz, Kategori, Raf ekleme)
