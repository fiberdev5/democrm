    ...
    $('#customFile').on("change", function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // Yükleme başarılı olunca formu gizle
                $('#stokFotoEkle').hide();
            },
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                fotoDiv.fadeOut(300, function () { 
                    $(this).remove(); 
                    // --- YENİ EKLENEN KOD ---
                    // Silme başarılı olunca formu tekrar göster
                    // Formun HTML'ini yeniden oluşturup eklemek en temiz yöntem olabilir.
                    if ($('#stokFotoEkle').length === 0) {
                         const formHtml = `
                            <form method="POST" id="stokFotoEkle" enctype="multipart/form-data">
                                @csrf
                                <div class="mb-2">
                        ...
                        // Formu, imgBox'tan önce veya uygun bir yere ekleyin
                        $('.imgBox').parent().find('hr').before(formHtml);
                    } else {
                        $('#stokFotoEkle').show();
                    }
                    // --- KOD SONU ---
