        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addBrandForm" action="{{ route('store.brand.ajax', $firma->id) }}">
                    @csrf
                    <input type="hidden" name="form_token" id="brandFormToken" value="">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addDeviceTypeForm" action="{{ route('store.device.type.ajax', $firma->id) }}">
                    @csrf
                    <input type="hidden" name="form_token" id="deviceFormToken" value="">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addCategoryForm" action="{{ route('store.category.ajax', $firma->id) }}">
                    @csrf
                    <input type="hidden" name="form_token" id="categoryFormToken" value="">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addShelfForm" action="{{ route('store.shelf.ajax', $firma->id) }}">
                    @csrf
                    <input type="hidden" name="form_token" id="shelfFormToken" value="">
...
$(document).ready(function () {
    ...
    // Sayfa yüklendiğinde token'ları oluştur
  ...
  // TOKEN SİSTEMLİ SUB MODAL KURULUMU
    function setupSubModalWithToken(buttonSelector, modalId, formId, tokenId, selectName, successMessage, submittingFlag) {
        // Modal açma
        $(document).on('click', buttonSelector, function () {
            // Yeni token oluştur
            $(tokenId).val(generateToken());
    ...
    // Sub modalları token ile kurulum
    setupSubModalWithToken('#addNewBrandBtn', '#addBrandModal', '#addBrandForm', '#brandFormToken', 'marka_id', 'Marka başarıyla eklendi.', 'brandFormSubmitting');
    setupSubModalWithToken('#addNewDeviceTypeBtn', '#addDeviceTypeModal', '#addDeviceTypeForm', '#deviceFormToken', 'cihaz_id', 'Cihaz Türü başarıyla eklendi.', 'deviceFormSubmitting');
    setupSubModalWithToken('#addNewCategoryBtn', '#addCategoryModal', '#addCategoryForm', '#categoryFormToken', 'urunKategori', 'Kategori başarıyla eklendi.', 'categoryFormSubmitting');
    setupSubModalWithToken('#addNewShelfBtn', '#addShelfModal', '#addShelfForm', '#shelfFormToken', 'raf_id', 'Raf başarıyla eklendi.', 'shelfFormSubmitting');
...
$(document).ready(function () {
    ...
    var tenantId = "{{ $firma->id }}";
    var mainFormId = '#addStock';


    // Form submit - Mevcut validation ile birleştir
    $(mainFormId).submit(function(event) {
        // Token kontrolü
        if (formSubmitting) {
            event.preventDefault();
