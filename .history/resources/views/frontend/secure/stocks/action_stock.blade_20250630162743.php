...
      @elseif($stokIslem->islem == 2)
                    @php
                      $service = \App\Models\Service::find($stokIslem->servisid);
                      $userWhoUsed = \App\Models\User::find($stokIslem->personel); // StokAction'daki personel ID'sini kullan
                    @endphp
                    Servis: {{ $service->service_code ?? 'Bilinmiyor' }}
                    @if($userWhoUsed)
                      (Kullanan: {{ $userWhoUsed->name }})
                    @endif
                  @elseif($stokIslem->islem == 3)
                    {{ $perSec->name ?? '' }}
                  @endif
