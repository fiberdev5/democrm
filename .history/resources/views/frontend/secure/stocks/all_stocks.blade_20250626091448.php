...
{{-- DataTables Buttons için gerekli CSS ve JS dosyalarını head'e ekle --}}
@push('styles')
            ...
            <table id="datatableStock" class="table table-bordered dt-responsive nowrap" style="width: 100%;">
              <thead class="title">
                <tr>
                  ...
                  <th>Raf</th>
                  <th>Marka / Cihaz</th>
                  <th class="no-print" style="width: 96px;">Düzenle</th>
                </tr>
              </thead>
              <tbody></tbody>
            </table>
...
{{-- Gerekli JavaScript kütüphanelerini yükle --}}
...
<script>
// DataTable - Scriptsler tamamen yüklendikten sonra çalışması için setTimeout kullan
$(document).ready(function () {
  
  // Kütüphanelerin yüklenmesini bekle
  setTimeout(function() {
    
    // DataTables ve Buttons'ın yüklenip yüklenmediğini kontrol et
    if (typeof $.fn.dataTable === 'undefined') {
      console.error('DataTables yüklenmedi!');
      return;
    }
    
    if (typeof $.fn.dataTable.Buttons === 'undefined') {
      console.error('DataTables Buttons yüklenmedi!');
      return;
    }
        ...
        buttons: [
          {
            ...
            className: 'btn btn-primary btn-sm d-none', // Gizli buton
    ...
    $('.stockprintButton').click(function(e) {
      ...
      try {
        if (table && table.button) {
          ...
          console.error('DataTable buttons bulunamadı!');
        }
      } catch(err) {
  ...
  }, 1000); // 1 saniye bekle (kütüphanelerin yüklenmesi için)
...
/* Düzenle sütunu yazdırmada gizle */
.no-print {
  /* Bu class yazdırmada gizlenecek */
}
</style>
