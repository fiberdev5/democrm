...
// Modal kapanma olayını sadece alt modal yoksa çalıştır
  $("#addStockModal").on("hidden.bs.modal", function(){
    // Eğer alt modal açık değilse içeriği temizle
    if (!isSubModalOpen) {
      $(this).find(".modal-body").html("");
    }
  });

  // Alt modal'lar açıldığında bayrak set et
  $('#addBrandModal, #addDeviceTypeModal, #addCategoryModal, #addShelfModal').on('show.bs.modal', function(){
    isSubModalOpen = true;
  });

  // Alt modal'lar kapandığında bayrak sıfırla ve ana modal'ı geri aç
  $('#addBrandModal, #addDeviceTypeModal, #addCategoryModal, #addShelfModal').on('hidden.bs.modal', function(){
    isSubModalOpen = false;
    
    // Ana modal hala görünür durumda mı kontrol et, değilse tekrar aç
    if (!$('#addStockModal').is(':visible')) {
      $('#addStockModal').modal('show');
    } else {
      // Modal açıksa sadece body scroll'unu düzelt
  ...
  $('#datatableStock').on('click', '.editStock', function(){
    ...
    var modal = $('#editStockModal'); // Modal elementini seçelim
    
    // 1. Modal boyutunu büyüt ve içeriği temizle
