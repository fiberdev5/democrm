...
<script>
  ...
  $(document).ready(function () {
    $('.islemSec').on('change', function () {
        ...
        // Üst kısımdaki toplam bilgiyi güncelle
        var toplamBilgi = card.find('.toplam-bilgi');
        if (goruntulenenSatir > 0) {
            toplamBilgi.show();
            toplamBilgi.find('.toplam-adet').text(toplamAdet);
            toplamBilgi.find('.toplam-tutar').text(toplamFiyat.toFixed(2));
        } else {
            toplamBilgi.hide();
        }
    });
