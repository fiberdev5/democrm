...
<div class="row mb-2">
  ...
  <div class="row mb-2">
    ...
    <div class="col-sm-8">
      <input name="urunKodu" type="text" class="form-control @error('urunKodu') is-invalid @enderror" required value="{{ old('urunKodu') }}">
 <div id="urunKoduError" class="text-danger mt-1" style="display:none; font-size: 0.9em;">
      Ürün kodu tam 13 haneli olmalıdır!
    </div>
@error('urunKodu')
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const urunKoduError = document.getElementById('urunKoduError');

    urunKoduInput.addEventListener('input', function () {
      const val = this.value.trim();
      if (val.length === 13) {
        urunKoduError.style.display = 'none';
        this.classList.remove('is-invalid');
      } else {
        urunKoduError.style.display = 'block';
