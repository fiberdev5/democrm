   ...
   $('#addStock').submit(function(event) {
    ...
    form.find('input[required], select[required]').each(function() {
      if (!$(this).val()) {
        ...
        missingFields.push(label || $(this).attr('name'));
      }
    });
    ...
    // AJAX kontrolü: aynı ürün adı var mı?
    $.ajax({
      url: form.attr('action'), // formun action'ı Laravel route'u olmalı
      method: 'POST',
      data: form.serialize(),
      success: function(res) {
        // AJAX'tan json gelirse ve aynı ürün varsa
        if (res.status === 'exists') {
          if (confirm(res.message + '\n\nDüzenleme sayfasına gitmek ister misiniz?')) {
            // yönlendir
            window.location.href = `/${tenant_id}/stok/duzenle/${res.stock_id}`;

          }
        } else {
          // aynı ürün yoksa formu submit et (normal şekilde)
          form.off('submit').submit(); // infinite loop'u engelle
        }
      },
      error: function(xhr) {
        // eğer json değilse (redirect ise) form normal çalışsın
