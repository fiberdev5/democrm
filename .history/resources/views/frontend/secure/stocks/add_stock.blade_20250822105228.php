 ...
 <div class="row mb-1 align-items-center">
    ...
    <div class="col-sm-9">
        <div class="input-group">
            <select name="marka_id" class="form-select" required style="width: calc(100% - 42px);">
                <option value="" selected disabled>- Seçiniz -</option>
                @foreach($markalar as $marka)
                    <option value="{{ $marka->id }}">{{ $marka->marka }}</option>
                @endforeach
            </select>
            <button class="btn btn-success" type="button" id="addNewBrandBtn" style="width: 42px;">+</button>
        </div>
    </div>
</div>
    ...
    function initializeSelect2(selector, placeholder, url) {
        $(selector).select2({
            ...
            ajax: {
                url: url,
                dataType: 'json',
                delay: 250, // Kullanıcı yazmayı bıraktıktan sonra arama yapmak için bekleme süresi
                data: function (params) {
                    return {
                        q: params.term // Arama metnini 'q' parametresi olarak gönder
                    };
                },
