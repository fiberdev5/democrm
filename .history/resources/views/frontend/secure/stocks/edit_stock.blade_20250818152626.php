...
$(document).ready(function() {
      // 'hareketEkleModal' (ikinci modal) açılmak üzereyken...
    $('#hareketEkleModal').on('show.bs.modal', function () {
        // Halihazırda ekranda olan ilk modalın backdrop'ını buluyoruz.
        // ve z-index'ini, ilk modalın hemen üzerine çıkacak şekilde ayarlıyoruz.
        // Bootstrap'in varsayılan modal z-index'i 1055'tir. Biz onu 1056 yapacağız.
        $('.hareketEkleModal-backdrop.show').css('z-index', 1056);
    });

    // 'hareketEkleModal' (ikinci modal) kapandıktan sonra...
    $('#hareketEkleModal').on('hidden.bs.modal', function () {
        // Backdrop'ın z-index'ini eski haline (CSS'teki varsayılan değerine) döndürüyoruz.
        // Böylece ilk modal tekrar normal şekilde görünür.
        $('.modal-backdrop.show').css('z-index', ''); // Inline stili kaldırır

        // İlk modal hala görünür durumdaysa, sayfanın kaydırma çubuğunu
        // korumak için body'ye 'modal-open' sınıfını tekrar ekliyoruz.
        if ($('#editStockModal').length > 0) {
