...
$(document).ready(function(){
  ...
  // Stok ekleme modalı kapandığında
  $('#addStockModal').on('hidden.bs.modal', function () {
    // Modal içeriğini temizle
    $(this).find('.modal-body').html('Yükleniyor...');
    
    // DataTable'ı yeniden yükle ve event'leri yeniden bağla
    setTimeout(function() {
      if (typeof window.stockTable !== 'undefined') {
        window.stockTable.ajax.reload(function() {
          // Reload tamamlandıktan sonra event'leri yeniden bağla
          bindEditEvents();
        }, false);
      }
    }, 500);
  ...
  // Edit event'lerini bağlama fonksiyonu
  function bindEditEvents() {
    // Önce mevcut event'leri kaldır
    $('.editStock').off('click.editStock');
    
    // Yeni event'leri bağla
    $('.editStock').on('click.editStock', function(e){
      e.preventDefault();
      e.stopPropagation();
      ...
      console.log('Edit clicked for ID:', id); // Debug için
      
      // Modal boyutunu ayarla ve içeriği temizle
      ...
      $.ajax({
        ...
        error: function(xhr, status, error) {
          console.log('AJAX Error:', status, error); // Debug için
          modal.find('.modal-content').html(`
            <div class="modal-header">
            ...
            <div class="modal-body">
              <div class="alert alert-danger">
                İçerik yüklenirken bir hata oluştu.<br>
                Hata: ${error}
              </div>
            </div>
          `);
  ...
  // Edit modalı kapandığında temizlik
...
// DataTable Script
$(document).ready(function () {
  window.stockTable = $('#datatableStock').DataTable({
      processing: true,
      serverSide: true,
      ajax: {
      ...
      drawCallback: function(settings) {
        ...
        // HER ÇİZİMDE EVENT'LERİ YENİDEN BAĞLA
        bindEditEvents();
      },
      lengthMenu: [ [25, 50, 100, -1], [25, 50, 100, "Tümü"] ]
  });

  // İlk yükleme için event'leri bağla
  bindEditEvents();
  ...
  $('#printButton').on('click', function () {
    window.stockTable.button(0).trigger();
  });
});

// Global fonksiyon tanımı (DataTable callback'lerinde kullanılabilmesi için)
function bindEditEvents() {
  // Önce mevcut event'leri kaldır
  $('.editStock').off('click.editStock');
