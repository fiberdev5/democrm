...
.faded-background {
    ...
    opacity: 0.5;
    /* Yüksek z-index değeriyle diğer öğelerin önüne çıkmasını sağla */
    position: relative; 
    z-index: 1040; 
}
...
$(document).on('click', '.hareketEkleBtn', function() {     
    ...
    $('#editStockContent').addClass('faded-background'); 

    // Hareket ekle modalını göster
    $('#hareketEkleModal').modal('show');   
});
...
$('#hareketEkleModal').on('hidden.bs.modal', function () {
    ...
    $('#editStockContent').removeClass('faded-background');

    // Bootstrap'in body'den modal-open sınıfını kaldırmasını engellemek için
    // Bu, ana modalın scrollbarının kaybolmasını önler.
