      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <a data-bs-toggle="modal" data-bs-target="#addKonsinyeModal" class="btn btn-success btn-sm addKonsinye">
              <i class="fas fa-plus"></i> <span>Cihaz Ekle</span>
            </a>
            <a data-bs-toggle="modal" data-bs-target="#konsinyePrintModal" class="btn btn-warning btn-sm konsinyePrintButton">
            ...
            <table id="datatableKonsinye" class="table table-bordered dt-responsive nowrap" style="width: 100%;">
              <thead class="title">
                <tr>
...
<!-- Modallar -->
<div id="addKonsinyeModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="addKonsinyeModalLabel" aria-hidden="true">
  <div class="modal-dialog ">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="addKonsinyeModalLabel">Cihaz Ekle</h6>
...
<div id="editKonsinyeModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="editKonsinyeModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="editKonsinyeModalLabel">Cihaz Düzenle</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">Yükleniyor...</div>
    </div>
  ...
  $(".addKonsinye").click(function(){
    $.ajax({
      ...
      if($.trim(data) === "-1"){
        ...
        $('#addKonsinyeModal').modal('show');
        $('#addKonsinyeModal .modal-body').html(data);
      }
    });
  });

  $("#addKonsinyeModal").on("hidden.bs.modal", function(){
    $(this).find(".modal-body").html("");
  });

  $('#datatableKonsinye').on('click', '.editKonsinye', function(){
    var id = $(this).data('id');
    $.ajax({
      ...
      if($.trim(data.html) === "-1"){
        ...
        $('#editKonsinyeModal').modal('show');
        $('#editKonsinyeModal .modal-body').html(data.html);
        $('#editKonsinyeModalLabel').text(data.urunAdi);
      }
    });
  });

  $("#editKonsinyeModal").on("hidden.bs.modal", function(){
    $(this).find(".modal-body").html("");
    $('#editKonsinyeModalLabel').text("Cihaz Düzenle");
  });
});
</script>
...
$(document).ready(function () {
  var table = $('#datatableKonsinye').DataTable({
      ...
      serverSide: true,
      ajax: {
        url: "{{ route('konsinye-cihazlar.data', $firma->id) }}",
        data: function (d) {
          d.raf = $('#raf').val();
          d.marka = $('#marka').val();
          d.cihaz = $('#cihaz').val();
