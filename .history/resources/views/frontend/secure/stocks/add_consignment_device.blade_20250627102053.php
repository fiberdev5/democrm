...
$(document).ready(function () {
                ...
                success: function (res) {
                    if (res.exists) {
                        var warningHtml = '<div class="alert alert-warning mt-2">' +
                            ...
                            <button type="button" id="openEditModalBtn" data-url="{{ $edit_url }}" class="btn btn-sm btn-primary ms-2">Ürünü Düzenle</button> +
                            '</div>';
    ...
    $(document).on('click', '#openEditModalBtn', function() {
        ...
        e.preventDefault(); // submit olmasını engelle
        $.ajax({
            url: url,
            type: 'GET',
            success: function(res) {
    ...
    // Modal göster/gizle sırasında form görünürlüğü
