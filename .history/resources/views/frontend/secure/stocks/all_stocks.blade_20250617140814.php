...
$(document).ready(function () {
  ...
  var table = $('#datatableStock').DataTable({
      ...
      serverSide: true,
      ajax: {
        url: "{{ route('stocks', $firma->id) }}",  // Laravel route, controller'da tanımla
        data: function (d) {
          d.search = $('input[type="search"]').val();
          // İstersen burada ekstra filtreler ekleyebilirsin, örn:
      ...
      columns: [
        { data: 'id', name: 'id' },
        { data: 'created_at', name: 'created_at' },         // Tarih
        { data: 'urunAdi', name: 'urunAdi' },               // Ürün Adı
        { data: 'urunKodu', name: 'urunKodu' },             // Ürün Kodu
        { data: 'fiyat', name: 'fiyat' },                   // Fiyat
        { data: 'adet', name: 'adet' },                     // Adet (stokta mevcut)
        { data: 'raf_adi', name: 'raf_adi' },               // Raf (depo adı)
        { data: 'marka_cihaz', name: 'marka_cihaz' },       // Marka / Cihaz birleşik olarak dönecek
        { data: 'action', orderable: false, searchable: false }  // Düzenle / Sil butonları
      ],
      order: [[0, 'desc']],
      language: {
        // Dil ayarlarını kendine göre ekleyebilirsin
        sEmptyTable: "Tabloda herhangi bir veri mevcut değil",
        sInfo: "Toplam _TOTAL_ kayıt gösteriliyor",
        sSearch: "Ara:",
        oPaginate: {
          sFirst: "İlk",
          sLast: "Son",
