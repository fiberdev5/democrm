...
<div class="row mb-3">
  <label class="col-sm-4">Ürün Kodu ve Barkod</label>
  <div class="col-sm-8">
    @if($stock->urunKodu)
      {{ DNS1D::getBarcodePNG($stock->urunKodu, 'C128') }}
      <p style="font-size: 13px">{{ $stock->urunKodu }}</p>

      <a href="{{ route('stok.barkod.pdf', [$firma->id, $stock->id]) }}" target="_blank" class="btn btn-outline-secondary btn-sm mt-2">
        Barkodu PDF Olarak Aç
      </a>
    @else
      <p class="text-muted">Ürün kodu tanımlı değil.</p>
    @endif
  </div>
</div>
...
<div class="row mb-3">
  ...
  <div class="col-sm-8 d-flex gap-3">
    <div class="border rounded p-2 flex-fill text-center bg-light">
      <strong>Toplam Stok:</strong> {{ $kalanStok }} adet
    </div>
    <div class="border rounded p-2 flex-fill text-center bg-light">
      <strong>Personel Stok:</strong> {{ $personelAdet }} adet
    </div>
  </div>
</div>
