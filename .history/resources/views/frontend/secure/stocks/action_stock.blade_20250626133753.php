...
<div class="col-7 text-end">
  ...
  <select class="form-control-select islemSec d-inline-block" name="islemSec" style="width: auto; min-width: 150px;">
    ...
    <option value="2">Personel'e Gönder</option>
    <option value="4">Serviste Kullanım</option>
  </select>
...
$(document).ready(function () {
    $('.islemSec').on('change', function () {
        ...
        var rows = card.find('table tbody tr').not('.toplam-satir');
        
        var toplamAdet = 0;
        var toplamFiyat = 0;
        var goruntulenenSatir = 0;

        rows.each(function () {
            var tdNumber = $(this).find('.tdNumber').text().trim();
            var shouldShow = (selected == 0) || tdNumber.endsWith(',' + selected);

            if (shouldShow) {
                $(this).show();
                goruntulenenSatir++;
                
                // Adet ve fiyat hesapla
        ...
        card.find('.toplam-satir').remove();
        
        // Yeni toplam satırı ekle
        if (goruntulenenSatir > 0) {
            var toplamSatir = `
                <tr class="toplam-satir" style="background-color: #e9ecef; font-weight: bold; border-top: 3px solid #007bff;">
                    <td style="display:none;"></td>
                    <td></td>
                    <td></td>
                    <td style="text-align: right; color: #007bff;">TOPLAM:</td>
                    <td style="color: #28a745; font-size: 14px;">${toplamAdet}</td>
                    <td style="color: #dc3545; font-size: 14px;">${toplamFiyat.toFixed(2)} TL</td>
                    <td></td>
                </tr>
            `;
            
            card.find('table tbody').append(toplamSatir);
        }
    });
