<ul class="nav nav-pills" role="tablist" style="margin-bottom: 5px;">
  ...
  <li class="nav-item"><a class="nav-link" data-bs-toggle="pill" href="#tab3" data-id="{{ $consignmentDevice->id }}" role="tab">Personel Stokları</a></li>
  <li class="nav-item"><a class="nav-link" data-bs-toggle="pill" href="#tab4" data-id="{{ $consignmentDevice->id }}" role="tab">Fotoğraflar</a></li>
</ul>
  ...
  <div id="tab1" class="tab-pane active" style="padding: 0">
    <form method="POST" id="editConsignmentDevice" action="{{ route('update.consignment.device', [$firma->id, $consignmentDevice->id]) }}">
        ...
        <div class="col-sm-8">
          <select name="urunKategori" class="form-select" required>
            ...
            @foreach($kategoriler as $kategori)
              <option value="{{ $kategori->id }}" {{ $consignmentDevice->urunKategori == $kategori->id ? 'selected' : '' }}>
                {{ $kategori->kategori }}
              </option>
            @endforeach
          </select>
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-8">
          <input type="text" name="urunKodu" class="form-control" value="{{ $consignmentDevice->urunKodu }}" required maxlength="13" minlength="13">
          @error('urunKodu')
