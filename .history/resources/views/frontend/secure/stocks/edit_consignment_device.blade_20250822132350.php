          ...
          <!-- 1. KART: ÜRÜN BİLGİLERİ -->
            <div class="card mb-3">
                ...
                <div class="card-body p-1">
                    ...
                    <form method="POST" id="editConsignmentUnifiedForm" action="{{ route('update.consignment', [$firma->id, $stock->id]) }}">
                        @csrf
                        {{-- Marka --}}
                        <div class="row g-1 align-items-center">
                            <label class="col-sm-3 col-form-label col-form-label-sm">Marka<span class="text-danger">*</span></label>
                            <div class="col-sm-9">
                                <select name="marka_id" class="form-select form-select-sm select2-ajax" data-url="{{ route('search.brands', $firma->id) }}" required>
                                    @if($stock->marka)
                                    <option value="{{ $stock->stok_marka }}" selected>{{ $stock->marka->marka }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        ...
                        {{-- Cihaz Türü --}}
                        <div class="row g-1 align-items-center">
                            <label class="col-sm-3 col-form-label col-form-label-sm">Cihaz Türü<span class="text-danger">*</span></label>
                            <div class="col-sm-9">
                                <select name="cihaz_id" class="form-select form-select-sm select2-ajax" data-url="{{ route('search.devices', $firma->id) }}" required>
                                     @if($stock->cihaz)
                                    <option value="{{ $stock->stok_cihaz }}" selected>{{ $stock->cihaz->cihaz }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        ...
                        {{-- Kategori --}}
                        <div class="row g-1 align-items-center">
                            <label class="col-sm-3 col-form-label col-form-label-sm">Kategori<span class="text-danger">*</span></label>
                            <div class="col-sm-9">
                                <select name="urunKategori" class="form-select form-select-sm select2-ajax" data-url="{{ route('search.categories', $firma->id) }}" required>
                                    @if($stock->kategori)
                                    <option value="{{ $stock->urunKategori }}" selected>{{ $stock->kategori->kategori }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        ...
                        {{-- Raf --}}
                        <div class="row g-1 align-items-center">
                            <label class="col-sm-3 col-form-label col-form-label-sm">Raf<span class="text-danger">*</span></label>
                            <div class="col-sm-9">
                               <select name="raf_id" class="form-select form-select-sm select2-ajax" data-url="{{ route('search.shelves', $firma->id) }}" required>
                                     @if($stock->raf)
                                    <option value="{{ $stock->urunDepo }}" selected>{{ $stock->raf->raf_adi }}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        ...
                        {{-- Ürün Kodu --}}
                        <div class="row g-1 align-items-center">
                            <label class="col-sm-3 col-form-label col-form-label-sm">Ürün Kodu<span class="text-danger">*</span></label>
                            <div class="col-sm-9">
                                <input type="text" name="urunKodu" class="form-control form-control-sm" value="{{ $stock->urunKodu }}" required>
                            </div>
                        </div>
                        ...
                        <div class="row g-1 align-items-center">
                            ...
                            <div class="col-sm-9">
                                <div class="alert alert-secondary py-1 px-2 mb-0">
                                    <small><strong>Depodaki Stok:</strong> {{ $kalanStok }} Adet</small>
                                </div>
                            </div>
                        </div>
                    </form>
...
<div class="modal fade" id="hareketEkleModal" tabindex="-1" aria-hidden="true" style="padding-top: 70px; background: rgba(0,0,0,0.5);">
  <div class="modal-dialog">
    <form id="hareketEkleForm" method="POST" action="{{ route('store.consignment.action', $firma->id) }}">
      @csrf
...
@include('frontend.secure.stocks.partials.add_supplier_modal')
...
$(document).ready(function() {
    ...
    $('#editConsignmentUnifiedForm').on('submit', function(e) {
        e.preventDefault();
        var $form = $(this), $submitButton = $('#saveConsignmentInfoBtn'), $messageDiv = $('#updateConsignmentFormMessages');
        $.ajax({
            url: $form.attr('action'),
            method: 'POST',
            data: $form.serialize(),
            beforeSend: () => $submitButton.prop('disabled', true).text('Kaydediliyor...'),
            success: function(response) {
                $messageDiv.html('<div class="alert alert-success py-1 px-2">' + response.message + '</div>');
                $('#editConsignmentModalTitle').text('Konsinye Cihaz Detayları [' + $form.find('input[name="urunAdi"]').val() + ']');
                setTimeout(() => $messageDiv.fadeOut('slow', function() { $(this).html('').show(); }), 3000);
            },
            error: function(jqXHR) {
                var errorMessage = (jqXHR.responseJSON && jqXHR.responseJSON.message) ? jqXHR.responseJSON.message : 'Bir hata oluştu.';
                $messageDiv.html('<div class="alert alert-danger py-1 px-2">' + errorMessage + '</div>');
            },
            complete: () => $submitButton.prop('disabled', false).text('Ürün Bilgilerini Kaydet')
        });
    });

    // Modal içeriğini yenileme fonksiyonu
    function refreshConsignmentDetails(stockId, tenantId) {
        $.get('/' + tenantId + '/konsinye/duzenle/' + stockId, function(response) {
            if (response.html) {
                $('#editConsignmentModal .modal-content').html(response.html);
                $('input[name="urunKodu"]').mask('0000000000000', { placeholder: '_____________' });
            }
        }).fail(() => alert('Detaylar yenilenirken bir hata oluştu.'));
    }
    ...
    $('#hareketEkleForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                form[0].reset();
                refreshConsignmentDetails($('#modalStokId').val(), "{{ $firma->id }}");
                alert(response.message);
            },
            error: (xhr) => alert((xhr.responseJSON && xhr.responseJSON.message) || 'Bir hata oluştu.'),
            complete: () => submitButton.prop('disabled', false).text('Kaydet')
        });
    });
    ...
    $('select[name="islemSec"]').on('change', function () {
        var selected = $(this).val(), $card = $(this).closest('.card');
        var toplamAdet = 0, toplamFiyat = 0;
        $card.find('table tbody tr').each(function () {
            var tdNumber = $(this).find('.tdNumber').text();
            var isVisible = (selected == 0 || tdNumber.endsWith(',' + selected));
            $(this).toggle(isVisible);
            if(isVisible && selected != 0){
                toplamAdet += parseInt($(this).find('td').eq(4).text()) || 0;
                toplamFiyat += parseFloat($(this).find('td').eq(5).text().replace(' TL', '')) || 0;
            }
        });
    ...
    $(document).on('click', '.stokSilBtn', function (e) {
        ...
        var id = $(this).data('id'), $button = $(this), $row = $button.closest('tr');
        $.ajax({
            url: '/{{ $firma->id }}/konsinye-hareket-sil/' + id,
            method: 'POST',
            data: { _token: '{{ csrf_token() }}', _method: 'DELETE' },
            success: function(res) {
                alert(res.message);
                if (res.status === 'success') $row.fadeOut(400, () => $row.remove());
            },
            error: () => alert('Silme işlemi sırasında hata oluştu.')
        });
    });
    ...
    $(document).on('change', '#customFile', function () {
        ...
        $.ajax({
            url: "/{{ $firma->id }}/konsinye-foto-ekle",
            method: "POST", data: formData, contentType: false, processData: false,
            beforeSend: () => $(".imgLoad").show(),
            success: function (res) {
                $('.imgBox').html(`<div class="col-4 col-md-3 stn mb-2" data-id="${res.id}"><img src="${res.resim_yolu}" class="img-fluid border rounded"><button class="btn btn-danger btn-sm w-100 stokFotoSil mt-1 py-0" data-id="${res.id}"><i class="fas fa-trash-alt"></i></button></div>`);
                $('#noPhotos, #uploadFormContainer').hide();
                $('#photoLimitWarning').show();
            },
            error: (xhr) => alert(xhr.responseJSON ? xhr.responseJSON.message : "Yükleme başarısız."),
            complete: () => { $(".imgLoad").hide(); $('#customFile').val(''); }
        });
    });
    ...
    $(document).on('click', '.stokFotoSil', function (e) {
        ...
        var id = $(this).data('id'), $fotoDiv = $('.stn[data-id="' + id + '"]');
        $.post("/{{ $firma->id }}/konsinye-foto-sil", { _token: "{{ csrf_token() }}", id: id }, function(res) {
            alert(res.message);
    ...
    // Tedarikçi Ekleme
    $('#addNewSupplierBtn').on('click', () => $('#addSupplierModal').modal('show'));
    $('#addSupplierModal').on('hidden.bs.modal', () => { if ($('#hareketEkleModal').is(':visible')) $('body').addClass('modal-open'); });
    $('#addStokSupplier').on('submit', function(e) {
        ...
        }).fail(() => alert("Tedarikçi eklenemedi."));
    });
});
</script>