...
$(document).ready(function () {   
  ...
  // Silme formunu ajax ile göndermek için
    $(document).on('submit', '.stock-delete-form', function(e) {
        e.preventDefault();
        
        var form = $(this);
        var url = form.attr('action');
        var row = form.closest('tr');
        var islem = row.find('.tdNumber').text().split(',')[1]; // İşlem türünü al
        var stokId = form.find('input[name="stok_id"]').val(); // Stok ID'sini al
        ...
        $.ajax({
            ...
            complete: function() {
                ...
                form.find('button[type="submit"]').html('<i class="fas fa-trash-alt"></i>');
            }
        });
    });
