...
<script>
// Bu script etiketini mevcut kodunuzla birleştirin veya ayrı bir yere koyun.
// Önemli olan $(document).on('click', '#openEditModalBtn', ...); olayının sadece bir kere tanımlanmasıdır.

$(document).on('click', '#openEditModalBtn', function() {
    var url = $(this).data('url');
    
    // Stok ekleme modalının ID'si. Eğer bu form bir modal içindeyse,
    // o modalın ID'sini buraya yazmalısınız.
    // Örneğin: var addStockModal = $('#yeniStokEkleModal');
    var addStockModal = $('#addStockModal'); // <-- BURANIN DOĞRU OLDUĞUNDAN EMİN OLUN

    // addStockModal tamamen kapandıktan sonra SADECE BİR KEZ çalışacak bir olay dinleyici ekle
    addStockModal.one('hidden.bs.modal', function () {
        // Artık eski modal kapandığına göre, AJAX isteğini yapıp yenisini açabiliriz.
        $.ajax({
            url: url,
            type: 'GET',
            success: function(res) {
                if (res.html) {
                    // Yeni modalın içeriğini doldur ve göster
    ...
    // Kapatma işlemini başlat. Yukarıdaki olay dinleyici bu işlem bittikten sonra tetiklenecek.
