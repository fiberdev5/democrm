...
<script>
 ...
 // Hareket Ekle modalını aç
    $(document).on('click', '.hareketEkleBtn', function () {
        ...
        // Hareketleri filtrele - Mobil uyumlu
$(document).on('change click touchend', '.islemSec', function () {
    var $select = $(this);
    var selected = $select.val();
    
    // Aynı değer için tekrar çalışmasını önle
    if ($select.data('lastVal') === selected) return;
    $select.data('lastVal', selected);
    
    // Modal içindeki card'ı bul
    var $modal = $select.closest('.modal');
    var card = $modal.find('.card').has('table');
    var rows = card.find('table tbody tr');
    var summaryRow = card.find('.toplam-header-row');

    // Eğer stok hareketi yoksa (boş mesaj satırı varsa) hiçbir şey yapma
    var emptyRow = rows.filter(function() {
    ...
    rows.each(function () {
        ...
        // Toplam header satırını atla
        if ($row.hasClass('toplam-header-row')) return;
        
        var tdNumber = $row.find('.tdNumber').text().trim();
        var currentBg = $row.css('background-color');
        ...
        if (selected == 0) {
            ...
            if (tdNumber.endsWith(',' + selected)) {
                ...
                var fiyat = parseFloat($row.find('td').eq(5).text().replace(' TL', '').trim()) || 0;
                toplamAdet += adet;
    ...
    // Mobil için force repaint
    setTimeout(function() {
        card.find('table').css('opacity', '0.99');
        setTimeout(function() {
            card.find('table').css('opacity', '1');
        }, 50);
    }, 10);
});
