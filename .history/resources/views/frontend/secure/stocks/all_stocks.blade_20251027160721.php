...
@section('user')
  ...
  border-left: 4px solid #0d6efd; /* Vurgu için sol tarafta mavi şerit */
  padding: 1rem;
  border-radius: 0.375rem; /* Bootstrap'in standart border-radius'u */
  display: flex;
  align-items: center;
  font-size: 0.9rem;
}

.stock-limit-callout a {
  text-decoration: none;
}

.stock-limit-callout a:hover {
...
@if(!is_null($stockLimit) && $stockLimit != -1 && $stockAll >= $stockLimit)
  {{-- Özel CSS ile tasarlanmış modern bir uyarı kutusu --}}
  <div class="stock-limit-callout mt-2">
    <i class="fas fa-info-circle text-primary"></i>
    <span class="ms-2">
      Stok limitiniz doldu (Maks: {{ $stockLimit }}). 
      <a href="{{ route('abonelikler', ['tenant_id' => $firma->id]) }}" class="fw-bold">
        Daha yüksek bir plana geçin.
      </a>
    </span>
  </div>
@endif
