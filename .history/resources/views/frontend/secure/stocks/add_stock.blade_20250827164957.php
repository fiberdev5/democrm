...
$(document).ready(function () {
    ...
    var isProductNameDuplicate = false; // Ürün adı tekrarını kontrol eden bayrak
    ...
    $(mainFormSelector + ' input[name="urunKodu"]').mask('000-0000000000', {
        placeholder: '___-__________',
        translation: {
            '0': {pattern: /[0-9]/}
        }
    }).on('input', function() {
        let cleanValue = $(this).cleanVal(); // Masked inputtan sadece rakamları al
        
        // Görsel geri bildirim
        if (cleanValue.length === 13) {
            $(this).removeClass('is-invalid').addClass('is-valid');
            $(this).next('small').text('Ürün kodu geçerli.'); // Başarılı mesajı
        } else {
            $(this).removeClass('is-valid').addClass('is-invalid');
            $(this).next('small').text('Ürün kodu 13 haneli olmalıdır (tireler dahil).'); // Hata mesajı
        }
    });

    // Ürün Adı Kontrolü (AJAX ile veritabanında var mı kontrolü)
    var checkProductNameTimeout;
    $(mainFormSelector + ' input[name="urunAdi"]').on('input', function () {
        clearTimeout(checkProductNameTimeout);
        var urunAdi = $(this).val().trim();
        $('#urunAdiUyari').html(''); // Uyarıyı temizle
        isProductNameDuplicate = false; // Bayrağı sıfırla

        if (urunAdi.length < 3) return; // 3 karakterden kısa ise kontrol yapma

        checkProductNameTimeout = setTimeout(function () {
            $.ajax({
                ...
                success: function (res) {
                    if (res.exists) {
                        isProductNameDuplicate = true; // Tekrar var bayrağını true yap
                ...
                error: function(xhr) {
                    console.error("Ürün adı kontrolü sırasında hata oluştu:", xhr.responseText);
                    // Kullanıcıya bir hata mesajı gösterebilirsiniz
                }
            });
        }, 600); // 600ms bekleme süresi
    });
