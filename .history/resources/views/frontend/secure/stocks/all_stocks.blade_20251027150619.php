...
@if(!is_null($stockLimit) && $stockLimit != -1 && $stockAll >= $stockLimit)
  <div class="bg-warning bg-opacity-10 border-start border-warning border-4 rounded p-3 mt-3 mb-0">
    <div class="d-flex align-items-center justify-content-between">
      <div>
        <i class="fas fa-info-circle text-warning me-2"></i>
        <span class="text-dark">
          <strong>Limit doldu:</strong> {{ $stockAll }}/{{ $stockLimit }} stok kartı
        </span>
      </div>
      <a href="{{ route('abonelikler', ['tenant_id' => $firma->id]) }}" 
         class="btn btn-outline-warning btn-sm">
        Planı Yükselt <i class="fas fa-arrow-right ms-1"></i>
      </a>
    </div>
  </div>
@endif
