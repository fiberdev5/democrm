...
@php
  // Toplam giriş - çıkış hareketlerinden kalan adet
  ...
  // Personel stok hareketlerinden alınan toplam adet
  $personelAdeti = \App\Models\PersonelStock::where('stokId', $stock->id)->sum('adet');

  // Depodaki ürün = kalan stok - personel stokları
  $depoAdeti = $kalanStok - $personelAdeti;
@endphp
  ...
  <div class="col-md-6">
    <div class="border rounded p-2 bg-light text-center">
      <strong>Toplam Stok</strong><br>
      <span class="fs-5 text-primary">{{ $kalanStok }}</span> adet
    </div>
  </div>
  <div class="col-md-6">
    <div class="border rounded p-2 bg-light text-center">
      <strong>Personelde</strong><br>
      <span class="fs-5 text-warning">{{ $personelAdet }}</span> adet
    </div>
  </div>
</div>
