...
.stock-limit-alert .limit-text {
  color: #856404;
  font-weight: 500;
  margin-right: 6px;
}

.stock-limit-alert .upgrade-link {
  color: #0d6efd;
  font-weight: 600;
...
.stock-limit-alert .upgrade-link:hover {
...
@if(!is_null($stockLimit) && $stockLimit != -1 && $stockAll >= $stockLimit)
  <div class="stock-limit-alert">
    <i class="fas fa-exclamation-triangle"></i>
    <span class="limit-text">
      Stok limitine ulaşıldı 
      <span class="stock-count-badge">{{ $stockAll }}/{{ $stockLimit }}</span>
    </span>
    <a href="{{ route('abonelikler', ['tenant_id' => $firma->id]) }}" class="upgrade-link">
...
@elseif(!is_null($stockLimit) && $stockLimit != -1 && ($stockLimit - $stockAll) <= 5 && ($stockLimit - $stockAll) > 0)
  <div class="stock-limit-alert" style="border-left-color: #17a2b8; background: linear-gradient(135deg, #d1ecf1 0%, #e7f6f8 100%);">
    <i class="fas fa-info-circle" style="color: #138496;"></i>
    <span class="limit-text" style="color: #0c5460;">
      Limite yaklaşıyorsunuz
      <span class="stock-count-badge" style="color: #138496; border-color: rgba(19, 132, 150, 0.2);">{{ $stockAll }}/{{ $stockLimit }}</span>
    </span>
  </div>
@endif
