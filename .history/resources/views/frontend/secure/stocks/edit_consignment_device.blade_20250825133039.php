                        ...
                        <form method="POST" id="consignmentFotoEkle" enctype="multipart/form-data">
                            ...
                            <div class="mb-2">
                                <input type="file" class="form-control form-control-sm" 
                                    name="resim" id="customFile" 
                                    accept="image/jpeg,image/png"
                                    data-tenant-id="{{ $firma->id }}">
                    ...
                    <div class="row imgBox">
                        @foreach($photos as $foto)
                            <div class="col-4 col-md-3 stn mb-2" data-id="{{ $foto->id }}">
                                ...
                                <button class="btn btn-danger btn-sm w-100 stokFotoSil mt-1 py-0" 
                                        data-id="{{ $foto->id }}"
                                        data-tenant-id="{{ $firma->id }}">
                            ...
                            <tbody>
                                ...
                                @else
                                    ...
                                    <tr style="{{ $renk }}">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-danger btn-sm stokSilBtn" 
  ...
  $(document).ready(function() {
    ...
    $('#hareketEkleForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Hareket ekleme modal'ını kapat ve formu temizle
                $('#hareketEkleModal').modal('hide');
                form[0].reset();

                // Ana modal'ı güncellemek için gerekli ID'leri al
                var stockId = $('#modalStokId').val();
                var tenantId = "{{ $firma->id ?? '' }}";

                // Ana modal'ın içeriğini yenilemek için fonksiyonumuzu çağır
    ...
    // İç içe modal probleminden dolayı sayfada hala bir ".modal-backdrop" kalmışsa,
    // onu manuel olarak kaldır. Bu, "karanlık ekran" sorununu çözer.
    $('.modal-backdrop').remove();
    
    // Her ihtimale karşı body'deki sınıfı da temizleyelim.
