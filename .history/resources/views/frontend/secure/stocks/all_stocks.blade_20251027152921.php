...
.stock-limit-alert-custom i {
  font-size: 16px;
  color: #ff9800;
  margin-right: 8px;
}

.stock-limit-alert-custom .limit-text-custom {
  color: #856404;
  font-weight: 500;
  margin-right: 6px;
}

.stock-limit-alert-custom .upgrade-link-custom {
  color: #0d6efd;
  font-weight: 600;
...
.stock-limit-alert-custom .upgrade-link-custom:hover {
  background: rgba(13, 110, 253, 0.1);
  text-decoration: none;
}

.stock-count-badge-custom {
  background: #fff;
  padding: 2px 8px;
  border-radius: 4px;
  font-weight: 600;
  color: #d97706;
  margin: 0 4px;
  border: 1px solid rgba(217, 118, 6, 0.2);
}
  </style>
...
@if(!is_null($stockLimit) && $stockLimit != -1 && $stockAll >= $stockLimit)
  <div class="stock-limit-alert-custom">
    <i class="fas fa-exclamation-triangle"></i>
    <span class="limit-text-custom">
      Stok limitine ulaşıldı 
      <span class="stock-count-badge-custom">{{ $stockAll }}/{{ $stockLimit }}</span>
    </span>
    <a href="{{ route('abonelikler', ['tenant_id' => $firma->id]) }}" class="upgrade-link-custom">
      <i class="fas fa-arrow-up me-1"></i>Planı Yükselt
    </a>
  </div>
@elseif(!is_null($stockLimit) && $stockLimit != -1 && ($stockLimit - $stockAll) <= 5 && ($stockLimit - $stockAll) > 0)
  <div class="stock-limit-alert-custom" style="border-left-color: #17a2b8; background: linear-gradient(135deg, #d1ecf1 0%, #e7f6f8 100%);">
    <i class="fas fa-info-circle" style="color: #138496;"></i>
    <span class="limit-text-custom" style="color: #0c5460;">
      Limite yaklaşıyorsunuz
      <span class="stock-count-badge-custom" style="color: #138496; border-color: rgba(19, 132, 150, 0.2);">{{ $stockAll }}/{{ $stockLimit }}</span>
    </span>
  </div>
@endif
