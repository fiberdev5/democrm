              ...
              <tr style="{{ $renk }}">
                ...
                <td>
                  @if($stokIslem->islem == 1)
                    {{ $stokIslem->tedarikci ?? '-' }}
                  @elseif($stokIslem->islem == 2)
                    Servis: {{ $stokIslem->servisid }} ({{ $teknisyenAdi }})
                  @elseif($stokIslem->islem == 3)
                    {{ $perSec->name ?? '' }}
                  @endif
                </td>
 ...
 $(document).ready(function () {
  $('.islemSec').on('change', function () {
    ...
    rows.each(function () {
      ...
      // Toplam sadece filtreli işlemlerde hesaplanacak
      if (selected != 0 && tdNumber.endsWith(',' + selected)) {
    ...
    // 🔽 TAM BU SATIRDAN SONRA GÖSTER/GİZLE KONTROLÜ BURAYA
    if (selected == 0) {
      card.find('.toplam-header-row').hide();
      card.find('.toplam-bilgi').hide();
    } else {
      card.find('.toplam-header-row').show();
      card.find('.toplam-bilgi').show();
      card.find('.toplam-adet-header').text(toplamAdet + ' Adet');
      card.find('.toplam-fiyat-header').text(toplamFiyat.toFixed(2) + ' TL');
    }
  });

  // Sayfa yüklendiğinde otomatik tetiklenir
