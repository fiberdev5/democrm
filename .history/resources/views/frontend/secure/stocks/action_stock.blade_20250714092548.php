              ...
              <tr style="{{ $renk }}">
                ...
                <td>
                  @if($stokIslem->islem == 1)
                    {{ $stokIslem->tedarikci ?? '-' }}
                  @elseif($stokIslem->islem == 2)
<a href="{{ route('all.services', [$firma->id, 'sid' => $stokIslem->servisid]) }}"
   target="_blank"
   class="servis-modal-trigger"
   data-servisid="{{ $stokIslem->servisid }}"
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Servis Düzenle</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Kapat"></button>
      </div>
      <div class="modal-body">
        <!-- AJAX içeriği burada yüklenecek -->
      </div>
    </div>
  </div>
</div>
...
<script>
  $(document).on('click', '.servis-modal-trigger', function () {
    let servisId = $(this).data('servisid');
    let firma_id = $(this).data('firma_id');
    ...
    $.ajax({
      ...
      success: function (data) {
        if ($.trim(data) === "-1") {
          alert("Servis bulunamadı veya yetkiniz yok.");
        } else {
          $('#editServiceModal').modal('show');
          $('#editServiceModal .modal-body').html(data);
        }
      },
      error: function () {
        alert("Servis bilgileri alınırken hata oluştu.");
      }
    });
  });
</script>
