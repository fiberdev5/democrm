...
$(document).on('show.bs.modal', function (e) {
        // e.target, o an açılan modal'ın kendisidir.
        var triggeredModal = $(e.target);

        // Eğer açılan modal, bizim #addConsignmentDevice formumuzu içeriyorsa...
        if (triggeredModal.find('#addConsignmentDevice').length > 0) {
            
            // ...ve daha önce başlatılmadıysa...
            if (!triggeredModal.data('select2-initialized')) {
                console.log('Konsinye formu modalı açıldı, Select2 başlatılıyor...');
                
                // Select2'leri BAŞLAT!
                initializeSelect2('#addConsignmentDevice select[name="marka_id"]', 'Marka ara...', "/" + tenantId + "/search-brands");
                initializeSelect2('#addConsignmentDevice select[name="cihaz_id"]', 'Cihaz türü ara...', "/" + tenantId + "/search-devices");
                initializeSelect2('#addConsignmentDevice select[name="raf_id"]', 'Raf ara...', "/" + tenantId + "/search-shelves");
                
                // Bu modal için başlatma işleminin yapıldığını işaretle.
                triggeredModal.data('select2-initialized', true);
            }
        }
    });
