          ...
          <tbody>
            ...
            @foreach($stokHareketleri as $stokIslem)
              ...
              <tr style="{{ $renk }}">
                ...
                <td>
                  <button type="button" class="btn btn-danger btn-sm btn-delete-stock" data-id="{{ $stokIslem->id }}">
...
$(document).on('click', '.btn-delete-stock', function () {
    ...
    const tenantId = '{{ request()->route("tenant_id") }}';

    $.ajax({
        url: `/${tenantId}/stoklar/delete/${stokId}`,
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
            _method: 'DELETE'
        ...
        success: function (response) {
            if (response.status === 'warning') {
                ...
                // Serviste kullanılmamışsa gerçek silme işlemi başlat
                Swal.fire({
                    title: 'Emin misiniz?',
                    text: 'Bu stok hareketi silinecek!',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $('<form>', {
                            method: 'POST',
                            action: `/${tenantId}/stoklar/delete/${stokId}`
                        })
                        .append($('<input>', {
                            type: 'hidden',
                            name: '_token',
