        ...
        <div class="col-sm-8">
          <select name="marka_id" class="form-control form-select" required>
            ...
            @foreach($markalar as $marka)
              <option value="{{ $marka->id }}" {{ $consignmentDevice->stok_marka == $marka->id ? 'selected' : '' }}>
                {{ $marka->marka }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-sm-8">
          <select name="cihaz_id" class="form-control form-select" required>
            ...
            @foreach($cihazlar as $cihaz)
              <option value="{{ $cihaz->id }}" {{ $consignmentDevice->stok_cihaz == $cihaz->id ? 'selected' : '' }}>
                {{ $cihaz->cihaz }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-sm-8">
          <select name="raf_id" class="form-control form-select" required>
            ...
            @foreach($rafListesi as $raf)
              <option value="{{ $raf->id }}" {{ $consignmentDevice->urunDepo == $raf->id ? 'selected' : '' }}>
                {{ $raf->raf_adi }}
              </option>
            @endforeach
          </select>
    ...
    @enderror
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-8">
          <div class="row g-2">
            <div class="col-8">
              <input name="fiyat" type="number" min="0" step="0.01" class="form-control" value="{{ $consignmentDevice->fiyat }}" >
            </div>
            <div class="col-4">
      ...
      @php
        $toplamGiris = \App\Models\StockAction::where('stokId', $consignmentDevice->id)->whereIn('islem', [1, 4])->sum('adet');
        $toplamCikis = \App\Models\StockAction::where('stokId', $consignmentDevice->id)->where('islem', 2)->sum('adet');
