      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <a data-bs-toggle="modal" data-bs-target="#addConsigmentModal" class="btn btn-success btn-sm addConsigment">
              <i class="fas fa-plus"></i> <span>Cihaz Ekle</span>
            </a>
            <a data-bs-toggle="modal" data-bs-target="#consigmentPrintModal" class="btn btn-warning btn-sm consigmentPrintButton">
            ...
            <table id="datatableConsigment" class="table table-bordered dt-responsive nowrap" style="width: 100%;">
              <thead class="title">
                <tr>
...
<!-- Modallar -->
<div id="addConsigmentModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="addConsigmentModalLabel" aria-hidden="true">
  <div class="modal-dialog ">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="addConsigmentModalLabel">Cihaz Ekle</h6>
...
<div id="editConsigmentModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="editConsigmentModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title" id="editConsigmentModalLabel">Cihaz Düzenle</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">Yükleniyor...</div>
    </div>
  ...
  $(".addConsigment").click(function(){
    $.ajax({
      ...
      if($.trim(data) === "-1"){
        ...
        $('#addConsigmentModal').modal('show');
        $('#addConsigmentModal .modal-body').html(data);
      }
    });
  });

  $("#addConsigmentModal").on("hidden.bs.modal", function(){
    $(this).find(".modal-body").html("");
  });

  $('#datatableConsigment').on('click', '.editConsigment', function(){
    var id = $(this).data('id');
    $.ajax({
      url: "/"+firma_id+"/konsinye-cihaz/duzenle/"+id,
      dataType: 'json'
    }).done(function(data){
      if($.trim(data.html) === "-1"){
        location.reload(true);
      } else {
        $('#editConsigmentModal').modal('show');
        $('#editConsigmentModal .modal-body').html(data.html);
        $('#editConsigmentModalLabel').text(data.urunAdi);
      }
    });
  });

  $("#editConsigmentModal").on("hidden.bs.modal", function(){
    $(this).find(".modal-body").html("");
    $('#editConsigmentModalLabel').text("Cihaz Düzenle");
  });
});
</script>
...
<script>
$(document).ready(function () {
  var table = $('#datatableConsigment').DataTable({
      processing: true,
      serverSide: true,
      ajax: {
