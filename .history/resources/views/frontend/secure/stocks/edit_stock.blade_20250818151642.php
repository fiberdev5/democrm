...
$(document).ready(function() {
     // "Hareket Ekle" modalı açılmak üzereyken tetiklenir.
    $(document).on('show.bs.modal', '#hareketEkleModal', function () {
      // O anda zaten açık olan ve görünen bir modal varsa (.modal.show), onu bul ve karart.
      // Bu, arkadaki ana "Stok Detayları" modalını hedefleyecektir.
      $('.modal.show').addClass('modal-dimmed');
    });

    // "Hareket Ekle" modalı tamamen kapandıktan sonra tetiklenir.
    $(document).on('hidden.bs.modal', '#hareketEkleModal', function () {
      // Karartılmış olan modalı bul ve normal haline geri döndür.
      $('.modal-dimmed').removeClass('modal-dimmed');
    });
    // Ürün kodu maskesi
