...
<div class="stock-buttons-container">
  ...
  @else
    <a class="btn btn-success btn-sm addStock disabled" tabindex="-1" aria-disabled="true">
      <i class="fas fa-plus"></i><span>Stok Kartı Ekle</span>
    </a>
  @endif
...
@if(!is_null($stockLimit) && $stockLimit != -1 && $stockAll >= $stockLimit)
  <div class="alert alert-warning d-inline-flex align-items-center mt-2 py-2 px-3 border-0 shadow-sm" role="alert">
    <i class="fas fa-exclamation-triangle me-2"></i>
    <span>
      Stok limiti doldu (<strong>{{ $stockLimit }}</strong> adet) • 
      <a href="{{ route('abonelikler', ['tenant_id' => $firma->id]) }}" class="alert-link fw-semibold text-decoration-none">
        Planınızı yükseltin <i class="fas fa-arrow-right ms-1"></i>
      </a>
    </span>
  </div>
@endif
