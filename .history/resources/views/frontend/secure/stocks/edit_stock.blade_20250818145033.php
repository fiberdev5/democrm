...
<style>
  /* İkinci modal açıldığında ilk modala uygulanacak stil */
#editStockModal.modal-behind .modal-content {
    opacity: 0.5; /* Modalı yarı saydam yap */
    filter: blur(2px); /* Arkadaki modala hafif bir bulanıklık efekti ekle (isteğe bağlı) */
    transition: opacity 0.3s ease, filter 0.3s ease; /* Efektin yumuşak olmasını sağla */
}

/* İkinci modalın (hareket ekle) z-index'ini artırarak
   her zaman ilk modalın ve onun backdrop'unun üzerinde olmasını sağla */
#hareketEkleModal {
    z-index: 1060; /* Bootstrap'in varsayılan modal z-index'i 1050'dir */
}

/* İkinci modalın backdrop'unun z-index'ini de ilk modalın üzerinde olacak şekilde ayarla */
#hareketEkleModal + .modal-backdrop {
    z-index: 1059;
}
...
$(document).ready(function() {
    ...
    $('#hareketEkleModal').on('hidden.bs.modal', function (e) {
     ...
     var editStockModal = $('#editStockModal');
    var hareketEkleModal = $('#hareketEkleModal');

    // 'hareketEkleModal' gösterilmeye başladığında...
    hareketEkleModal.on('show.bs.modal', function () {
        // Arkadaki 'editStockModal'a stil sınıfını ekle
        editStockModal.addClass('modal-behind');
    });

    // 'hareketEkleModal' tamamen gizlendiğinde...
    hareketEkleModal.on('hidden.bs.modal', function () {
        // Arkadaki 'editStockModal'dan stil sınıfını kaldır
        editStockModal.removeClass('modal-behind');
        
        // Bootstrap'in bazen unuttuğu scrollbar problemini önlemek için
        // eğer ilk modal hala görünürse, body'ye modal-open sınıfını tekrar ekle.
        // (Not: Zaten bu kontrolü yapan bir kodunuz var, bu bir pekiştirme)
        if (editStockModal.is(':visible')) {
            $('body').addClass('modal-open');
        }
    });
