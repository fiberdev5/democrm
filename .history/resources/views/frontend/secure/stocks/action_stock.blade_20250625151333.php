            ...
            @foreach($stokHareketleri as $stokIslem)
              @php
               ...
               if ($stokIslem->islem == 1) {
                  ...
                  $toplam -= $stokIslem->adet; // Personel'e gönderide adet azalıyor
                }
              @endphp
...
<script>
  ...
  $(document).ready(function () {
    $('.islemSec').on('change', function () {
      ...
      var card = $(this).closest('.card'); // sadece ilgili kartta çalışacak
...
<script>
  $(document).ready(function() {
    $('select[name="islem"]').on('change', function() {
      var val = $(this).val();
      if(val == '2') { // Personel'e Gönder seçilince fiyat gizle
        $('#fiyatInputDiv').hide();
        $('input[name="fiyat"]').prop('required', false).val('');
      } else {
        $('#fiyatInputDiv').show();
        $('input[name="fiyat"]').prop('required', true);
      }
    });
  });
</script>
