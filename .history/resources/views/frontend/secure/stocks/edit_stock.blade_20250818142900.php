...
.modal-backdrop-darken {
    ...
    pointer-events: none; /* Arkadaki modal'a tıklanmasını engeller */
    z-index: 1056; /* İkinci modal'ın backdrop'unun üzerinde olmalı */
}
...
$(document).ready(function() {
    ...
    var hareketEkleModal = document.getElementById('hareketEkleModal');
    var editStockModal = document.getElementById('editStockModal'); // Ana modal'ı ID ile seçelim. Bunun için ana modal div'ine id="editStockModal" eklemeniz gerekebilir.

    // Eğer ana modal'ınızın bir ID'si yoksa, aşağıdaki gibi bir ID ekleyin:
    // <div class="modal-dialog modal-xl" id="editStockModal">
    // Veya jQuery ile sınıfından yakalayın.
    var $editStockModal = $('.modal-header:contains("Stok Detayları ve Yönetimi")').closest('.modal-content');


    // 'hareketEkleModal' açılmak üzereyken...
    $(hareketEkleModal).on('show.bs.modal', function () {
        // Arka plandaki 'editStockModal'ın içeriğine karartma sınıfını ekle
        $editStockModal.addClass('modal-backdrop-darken');
    });

    // 'hareketEkleModal' tamamen kapandığında...
    $(hareketEkleModal).on('hidden.bs.modal', function () {
        // Arka plandaki 'editStockModal'dan karartma sınıfını kaldır
        $editStockModal.removeClass('modal-backdrop-darken');
        
        // Ana modal'ın hala görünür olup olmadığını kontrol et ve body'e 'modal-open' sınıfını geri ekle
        // Bu, Bootstrap'in ikinci modal kapanınca scroll bar'ı geri getirmesini engeller.
