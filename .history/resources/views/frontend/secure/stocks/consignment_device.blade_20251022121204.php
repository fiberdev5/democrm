...
$(document).ready(function () {
  ...
  var table = $('#datatableConsignment').DataTable({
      ...
      order: [[0, 'desc']],
dom: 'B<"top"f>rt<"bottom"i<"float-end"lp>><"clear">', // 'B' eklendi
buttons: [{
    extend: 'print',
    text: 'Yazdır',
    className: 'd-none', // Butonu gizliyoruz, kendi özel butonumuzla tetikleyeceğiz
    autoPrint: true,
    exportOptions: {
      columns: [0, 1, 2, 3, 4, 5, 6, 7], // Yazdırılacak sütun indeksleri
      format: {
        body: function (data, row, column, node) {
          // Veri temizleme işlemi (Stok sayfasındaki ile aynı)
          if (data === null || data === undefined) return '';
          if (typeof data === 'object') data = $(data).text();
          data = String(data);
          // Olası HTML etiketlerini veya responsive başlıkları temizle
          data = data.replace(/ID:|Tarih:|Ürün Adı:|Ürün Kodu:|Fiyat:|Adet:|Raf:|Marka \/ Cihaz:/gi, '');
          return data.trim();
        }
      }
    },
    customize: function (win) {
      // Stok sayfasındaki yazdırma stili aynen alındı
      ...
      // Başlık ekle (Konsinye için uyarlandı)
      var title = '<div class="print-title">Konsinye Cihazlar</div>';
      $(win.document.body).find('table').before(title);
      
      // Footer ekle (Konsinye için uyarlandı)
      var footer = '<div class="print-footer">' +
                  '  <span>Listelenen Konsinye Cihaz Sayısı: ' + totalRecords + ' - Tarih: ' + moment().format('DD/MM/YYYY') + '</span>' +
                  '</div>';
