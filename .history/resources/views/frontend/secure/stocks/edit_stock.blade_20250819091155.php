          ...
          <!-- 1. KART: ÜRÜN BİLGİLERİ -->
                ...
                <div class="card-body p-2">
                    <form method="POST" id="editStockUnifiedForm" action="{{ route('update.stock', [$firma->id, $stock->id]) }}">
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Markalar</label>
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Cihaz Türleri</label>
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Kategori</label>
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Raf</label>
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Ürün Adı</label>
                            <input type="text" name="urunAdi" class="form-control form-control-sm" value="{{ $stock->urunAdi }}">
                        </div>

                        <div class="form-group-compact">
                            <label class="form-label-compact">Ürün Kodu</label>
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Satış Fiyatı</label>
                        ...
                        <div class="form-group-compact">
                            <label class="form-label-compact">Açıklama</label>
                        ...
                        @endphp
                        <div class="form-group-compact">
                            <label class="form-label-compact">Stok Durumu</label>
            ...
            <div class="card">
                <div class="card-header">
                    <h5><i class="bi bi-images"></i> Fotoğraf</h5>
                </div>
                <div class="card-body">
                    @if(count($photos) == 0)
                        <form method="POST" id="stokFotoEkle" enctype="multipart/form-data">
                            ...
                            <div class="mb-2">
                                ...
                                <small class="text-muted">Sadece 1 fotoğraf yükleyebilirsiniz (JPG/PNG, max 5MB)</small>
                            </div>
                        ...
                        @if(count($photos) > 0)
                            @foreach($photos as $foto)
                                <div class="text-center mb-2">
                                    <img src="{{ Storage::url($foto->resimyol) }}" class="img-fluid border rounded" style="max-width: 100%; max-height: 200px;">
                                    <br>
                                    <button class="btn btn-danger btn-sm stokFotoSil mt-2" data-id="{{ $foto->id }}">
                                        <i class="fas fa-trash-alt"></i> Fotoğrafı Sil
                                    </button>
                                </div>
                            @endforeach
                        @else
                            <div class="text-center text-muted no-photo-message">
                                <i class="bi bi-image" style="font-size: 3rem; opacity: 0.3;"></i>
                                <p>Mevcut fotoğraf bulunmamaktadır</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="card-body scrollable-card-body p-0">
                    ...
                    @else
                        <div class="text-center text-muted py-4">
                            ...
                            <p>Stok hareketi bulunmamaktadır</p>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="card-body scrollable-card-body p-0">
                    ...
                    @else
                        <div class="text-center text-muted py-4">
                            ...
                            <p>Personel stoğu bulunmamaktadır</p>
                        </div>
                    @endif
                </div>
            </div>
      ...
      <div class="modal-content">
        ...
        <div class="mb-2">
            ...
            <div class="input-group">
              <select name="tedarikci" class="form-control select2-tedarikci">
                <option value="">Tedarikçi ara...</option>
                @foreach(\App\Models\StockSupplier::orderBy('tedarikci')->take(5)->get() as $tedarikci)
        ...
        <div class="mb-2 d-none" id="personelSelectDiv">
          ...
          <select name="personel" class="form-control select2-personel">
            <option value="">Personel ara...</option>
            @foreach(\App\Models\User::where('tenant_id', request()->route('tenant_id'))->orderBy('name')->take(5)->get() as $personel)
...
$(document).ready(function() {
    ...
    // Select2 başlatma - Tedarikçi seçimi
    $('.select2-tedarikci').select2({
        ...
        ajax: {
            url: function() {
                return '/{{ request()->route("tenant_id") }}/tedarikci-ara';
            },
            dataType: 'json',
            delay: 250,
    ...
    // Select2 başlatma - Personel seçimi
    $('.select2-personel').select2({
        ...
        ajax: {
            url: function() {
                return '/{{ request()->route("tenant_id") }}/personel-ara';
            },
            dataType: 'json',
            delay: 250,
            data: function (params) {
    ...
    // Fotoğraf seçildiğinde otomatik yükle - Sadece fotoğraf yoksa
    $('#customFile').on("change", function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                $(".imgLoad").hide();
                $('#customFile').val('');
                // Fotoğraf yüklendikten sonra formu gizle ve fotoğrafı göster
                $('#stokFotoEkle').hide();
                $('.no-photo-message').hide();
                $('.imgBox').html(`
                    <div class="text-center mb-2">
                        <img src="${res.resim_yolu}" class="img-fluid border rounded" style="max-width: 100%; max-height: 200px;">
                        <br>
                        <button class="btn btn-danger btn-sm stokFotoSil mt-2" data-id="${res.id}">
                            <i class="fas fa-trash-alt"></i> Fotoğrafı Sil
                        </button>
                    </div>
                `);
    ...
    $(document).on('click', '.stokFotoSil', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                alert(res.message);
                // Fotoğraf silindikten sonra upload formunu tekrar göster
                $('.imgBox').html(`
                    <div class="text-center text-muted no-photo-message">
                        <i class="bi bi-image" style="font-size: 3rem; opacity: 0.3;"></i>
                        <p>Mevcut fotoğraf bulunmamaktadır</p>
        ...
        if (this.checkValidity() === false) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Yeni tedarikçiyi Select2'ye ekle
                    var newOption = new Option(response.tedarikci, response.id, true, true);
                    $('.select2-tedarikci').append(newOption).trigger('change');
