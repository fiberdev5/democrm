                                    ...
                                    <tr style="{{ $renk }}">
                                        ...
                                        <td>
                                            ...
                                            @elseif($stokIslem->islem == 2)
                                                <a href="{{ route('all.services', [$firma->id, 'did' => $stokIslem->servisid]) }}" target="_blank" style="color:red!important;"class="link-stock">Servis: {{ $stokIslem->servisid }}</a> ({{ $teknisyenAdi }})
...
$(document).ready(function() {
    ...
    // Hareket Ekle modalını aç ve Select2'yi başlat
$(document).on('click', '.hareketEkleBtn', function() {
    $('#modalStokId').val($(this).data('stokid'));

    // Select2'yi AJAX arama özelliği ile başlat
    $('#tedarikciSelect').select2({
        ...
        ajax: {
            // Rota URL'sini depo stoklarındaki gibi dinamik hale getiriyoruz
            url: '/{{ $firma->id }}/search-suppliers',
            dataType: 'json',
...
// Hareket Ekle modalı kapandığında Select2'yi temizle
$('#hareketEkleModal').on('hidden.bs.modal', function () {
    // Select2'yi yok et, böylece tekrar açıldığında sorun çıkarmaz
    $('#tedarikciSelect').select2('destroy');
    $('#tedarikciSelect').val(null).trigger('change'); // Seçimi temizle
    
    // Arkadaki ana modal'ın scrollbar sorununu düzeltmek için
   ...
   // geldiğini kontrol etmemizi sağlar.
$(document).on('submit', '#addStokSupplier', function(e) {
    e.preventDefault();
    var form = $(this);
    
    $.post(form.attr('action'), form.serialize(), function(response) {
        // Sunucu yanıtında 'text' yerine 'tedarikci' geliyor olabilir, kontrol edelim.
        // Select2 için 'text' propertysi beklenir.
        if (response && response.id && (response.text || response.tedarikci)) {
            // Select2'nin beklediği format {id: 1, text: 'Tedarikçi Adı'} şeklindedir.
            var newOption = new Option(response.text || response.tedarikci, response.id, true, true);
            
            $('#tedarikciSelect').append(newOption).trigger('change');
            
            form[0].reset();
            $('#addSupplierModal').modal('hide');
        } else {
             alert("Tedarikçi eklendi ancak yanıt alınamadı. Lütfen listeyi kontrol edin.");
             $('#addSupplierModal').modal('hide');
        }
    }).fail(function() {
        alert("Tedarikçi eklenemedi. Bir hata oluştu.");
    });
});
});
</script>