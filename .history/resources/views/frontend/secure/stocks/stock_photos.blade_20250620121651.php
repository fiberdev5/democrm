<form method="POST" id="stokFotoEkle" enctype="multipart/form-data">
    ...
    <!-- Önizleme resmi (ilk yüklendiğinde görünür) -->
    <div class="mb-3">
        <img 
            id="resimGoster" 
            src="{{ isset($firstPhoto) ? $firstPhoto->resimyol : '' }}" 
            alt="Resim Önizleme" 
            style="max-width: 200px; border: 1px solid #ccc; padding: 5px;"
            @if(!isset($firstPhoto) || !$firstPhoto->resimyol) onerror="this.style.display='none';" @endif
        >
    </div>
...
$(document).ready(function () {
    ...
    $('#customFile').on("change", function () {
        ...
        reader.onload = function(e) {
            $('#resimGoster').attr('src', e.target.result).show();
        }
        ...
        $.ajax({
            ...
            success: function (res) {
                $(".imgLoad").hide();
                $('#customFile').val('');
                // Önizlemeyi gizle ya da istediğin gibi bırakabilirsin
                $('#resimGoster').hide();
...
$(document).on('click', '.stokFotoSil', function (e) {
    ...
    $.ajax({
        ...
        success: function (res) {
            alert(res.message); // Veya kendi notification sistemini kullanabilirsin
