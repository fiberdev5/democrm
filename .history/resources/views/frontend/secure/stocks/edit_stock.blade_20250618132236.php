  ...
  <div class="row mb-2">
    ...
    <div class="col-sm-8">
      <select name="marka_id" class="form-select" required>
        ...
        @foreach($markalar as $brand)
          <option value="{{ $brand->id }}" {{ $stock->stok_marka == $brand->id ? 'selected' : '' }}>
            {{ $brand->brandName }}
          </option>
        @endforeach
      </select>
  ...
  <div class="row mb-2">
    ...
    <div class="col-sm-8">
      <select name="cihaz_id" class="form-select" required>
        <option value="" disabled>Seçiniz</option>
        @foreach($cihazlar as $type)
          <option value="{{ $type->id }}" {{ $stock->stok_cihaz == $type->id ? 'selected' : '' }}>
            {{ $type->deviceType }}
          </option>
        @endforeach
      </select>
