...
<script>
  function deleteStock(tenantId, stockId) {
    $.ajax({
        url: `/your-route/${tenantId}/delete-stock/${stockId}`, // controller rotanı ayarla
        type: 'DELETE', // veya 'POST' + _method='DELETE' (route ayarına göre)
        data: {
            _token: $('meta[name="csrf-token"]').attr('content') // CSRF token
        },
        success: function(response) {
            if (response.status === 'success') {
                alert(response.message); // veya SweetAlert, toastr vs
                // Mesela tabloyu yeniden yükle veya sayfayı yenile
                location.reload();
