  ...
  <div class="mt-2 toplam-bilgi" style="display: none;">
    ...
    <div class="card-body" style="padding: 0;">
      <div class="table-responsive">
        <div id="ajax-alert-container" class="mt-3">
          ...
          <tbody>
            ...
            @foreach($stokHareketleri as $stokIslem)
              ...
              <tr style="{{ $renk }}">
                ...
                <td>
                  <button type="button" class="btn btn-danger btn-sm delete-stock-action-btn"
        data-id="{{ $stokIslem->id }}"
        data-tenant-id="{{ request()->route('tenant_id') }}">
    ...
    rows.each(function () {         
      ...
      if (selected == 0) {           
        ...
        if (tdNumber.endsWith(',' + selected)) {             
          ...
          var fiyatText = $(this).find('td').eq(5).text().replace(' TL', '').trim();
          fiyatText = fiyatText.replace(/\./g, '').replace(',', '.');
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.delete-stock-action-btn', function(e) {
            e.preventDefault(); // Varsayılan form gönderimini engelle

            const stockActionId = $(this).data('id');
            const tenantId = $(this).data('tenant-id'); // HTML'den tenant_id'yi alıyoruz

            // URL'yi kendi rota yapınıza göre oluşturun.
            // Örneğin: /1/stok-haraket-sil/634
            const deleteUrl = `/${tenantId}/stok-haraket-sil/${stockActionId}`;

            // Kullanıcıdan onay al
            if (!confirm('Silmek istediğinize emin misiniz?')) {
                return; // Onaylanmazsa işlemi durdur
            }

            // AJAX isteği gönderme
            $.ajax({
                url: deleteUrl,
                type: 'DELETE', // Laravel'de DELETE metodu kullanılır
                data: {
                    _token: '{{ csrf_token() }}' // CSRF token'ı gönderilmesi zorunlu
                },
                success: function(response) {
                    if (response.status === 'success') {
                        // Başarılı olursa satırı tablodan kaldır
                        $(`.delete-stock-action-btn[data-id="${stockActionId}"]`).closest('tr').remove();
                        // Toplamları güncelle (mevcut filtreye göre)
                        $('.islemSec').trigger('change');
                    }
                    // Uyarı mesajını göster
                    displayAjaxAlert(response.message, response.alert_type);
                },
                error: function(xhr) {
                    // Hata durumunda
                    const errorResponse = xhr.responseJSON;
                    if (errorResponse && errorResponse.message) {
                        displayAjaxAlert(errorResponse.message, errorResponse.alert_type || 'danger');
                    } else {
                        displayAjaxAlert('Beklenmeyen bir hata oluştu.', 'danger');
                    }
                }
            });
        });

        // AJAX uyarı mesajlarını göstermek için yardımcı fonksiyon (daha önceki yanıttan)
        function displayAjaxAlert(message, type) {
            const container = $('#ajax-alert-container');
            container.empty(); // Önceki uyarıyı temizle
