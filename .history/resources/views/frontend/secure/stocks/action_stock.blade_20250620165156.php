...
// İşlem seçimi değiştiğinde filtrele
  $('.islemSec').on('change', function() {
    var islem = $(this).val(); // seçili işlem değeri
    var firma_id = {{ $tenant_id }};  // tenant_id
    var stock_id = {{ $stock->id }};  // stok id

    var url = "/" + firma_id + "/stok-haraketleri/" + stock_id;
    if (islem && islem != 0) {
      url += "?islem=" + islem;
    }
    ...
    $.ajax({
      ...
      if($.trim(data) === "-1") {
        ...
        $('.card-body tbody').html(data);  // Tablo gövdesini güncelle
      }
    });
  });
});
