...
$(document).ready(function() {
    ...
    // Hareket Ekle modalını aç - EKSIK OLAN KOD
    ...
    // Form Submit - AJAX
    $(document).on('submit', '#hareketEkleForm', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    // Kısa bir delay ile modal'ı kapat
                    setTimeout(function() {
                    ...
                    showErrorMessage(response.message || 'Bir hata oluştu.');
                }
            },
            error: function(xhr, status, error) {
                ...
                if (status === 'timeout') {
                    ...
                    errorMessage = 'Doğrulama hatası. Lütfen bilgileri kontrol edin.';
                }
                
                showErrorMessage(errorMessage);
            },
            complete: function() {
                // Buton ve form elemanlarını tekrar aktif et
    ...
    function resetHareketForm() {
        ...
        $('select[name="islem"]').val('1').trigger('change');
    }

    // Modal içeriğini yenileme fonksiyonu (gelişmiş)
    function reloadModalContent() {
        var currentStockId = $('#modalStokId').val();
        var tenantId = '{{ $tenant_id }}';
        
        if (!currentStockId) {
            console.warn('Stok ID bulunamadı');
            return;
        }
        
        // Loading göstergesi ekle
        var $modalBody = $('#editStockModal .modal-body');
        var originalContent = $modalBody.html();
        
        $modalBody.html('<div class="text-center py-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Yükleniyor...</span></div><p class="mt-2">İçerik güncelleniyor...</p></div>');
        
        $.ajax({
            url: '/' + tenantId + '/edit-stock/' + currentStockId,
            method: 'GET',
            success: function(response) {
                if (response.html) {
                    // Kısa bir delay ile yeni içeriği göster (animasyon için)
                    setTimeout(function() {
                        $('#editStockModal .modal-content').html(response.html);
                        
                        // Mevcut form değerlerini koru (eğer gerekiyorsa)
                        // Bu kısım ihtiyaca göre özelleştirilebilir
                        
                    }, 300);
                } else {
                    throw new Error('HTML içerik alınamadı');
                }
            },
            error: function(xhr, status, error) {
                console.error('Modal içerik yenilenemedi:', error);
                
                // Orijinal içeriği geri yükle
                $modalBody.html(originalContent);
                
                // Kullanıcıya hata bildirimi
                showErrorMessage('İçerik güncellenemedi. Sayfa yenilenecek.');
    ...
    // Başarılı mesaj gösterme fonksiyonu
    function showSuccessMessage(message) {
        // Mevcut alert yerine daha hoş bir bildirim kullanabilirsiniz
        // Örnek: Toastr, SweetAlert vb.
        
        // Basit bir success alert
    ...
    // Modal kapanma olayını handle et
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.status === 'success') {
                        reloadModalContent(); // Sayfa yenilemek yerine modal içeriğini yenile
                    } else {
