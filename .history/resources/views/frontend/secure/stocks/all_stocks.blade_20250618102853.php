              ...
              <thead class="title">
                ...
                <tfoot>
                  <tr>
                    ...
                    <th id="toplamFiyatFooter"></th>
                    <th id="toplamAdetFooter"></th>
                    <th colspan="3"></th>
                  </tr>
                </tfoot>
              <tbody>
...
$(document).ready(function () {
  var table = $('#datatableStock').DataTable({
      ...
      // Yardımcı fonksiyon: sayıya çevir
      var parseFloatSafe = function (i) {
        return typeof i === 'string' ?
          parseFloat(i.replace(/[\₺,]/g, '').replace(/\s/g, '')) || 0 :
          typeof i === 'number' ? i : 0;
      };
      ...
      data.forEach(function (rowData) {
        var fiyat = parseFloatSafe(rowData.fiyat);
        var adet = parseFloatSafe(rowData.adet);

        toplamFiyat += fiyat * adet;
        toplamAdet += adet;
      });

      // Footer hücrelerine yaz
      $(api.column(4).footer()).html(toplamFiyat.toFixed(2) + " ₺");
      $(api.column(5).footer()).html(toplamAdet + " adet");
    }
