...
<div class="page-content">
  ...
  <div class="col-auto">
    <select id="filterPersonel" class="form-select form-select-sm">
  ...
  <div class="col-auto">
    <select id="filterRaf" class="form-select form-select-sm">
      <option value="">Raf Seçiniz</option>
      @foreach($rafListesi as $r)
        <option value="{{ $r->id }}">{{ $r->raf_adi }}</option>
      @endforeach
    </select>
  </div>
  <div class="col-auto">
    <select id="filterMarka" class="form-select form-select-sm">
  ...
  <div class="col-auto">
    <select id="filterCihaz" class="form-select form-select-sm">
      <option value="">Cihaz Seçiniz</option>
      @foreach($cihazlar as $c)
        <option value="{{ $c->id }}">{{ $c->cihaz }}</option>
      @endforeach
    </select>
  </div>
  <div class="col-auto">
    <select id="filterStokDurum" class="form-select form-select-sm">
      <option value="">Stok Durumu</option>
      <option value="1">Stokta Olanlar</option>
      <option value="0">Stokta Olmayanlar</option>
    </select>
  </div>
  <div class="col-auto">
    <button id="btnFilterReset" class="btn btn-secondary btn-sm">Temizle</button>
  </div>
</div>
  ...
  var table = $('#datatableStock').DataTable({
      ...
      ajax: {
        url: "{{ route('stocks', $firma->id) }}",
          data: function (d) {
          d.personel_id = $('#filterPersonel').val();
          d.raf_id = $('#filterRaf').val();
          d.marka_id = $('#filterMarka').val();
          d.cihaz_id = $('#filterCihaz').val();
          d.stok_durum = $('#filterStokDurum').val();
        }
      },
      columns: [
        { data: 'id', name: 'id' },
      ...
      lengthMenu: [ [25, 50, 100, -1], [25, 50, 100, "Tümü"] ]
        ...
        // Filtre değiştiğinde tabloyu yenile
  $('#filterPersonel, #filterRaf, #filterMarka, #filterCihaz, #filterStokDurum').change(function() {
    table.draw();
  });

  // Temizle butonu
  $('#btnFilterReset').click(function() {
    $('#filterPersonel, #filterRaf, #filterMarka, #filterCihaz, #filterStokDurum').val('');
    table.draw();
  });
});
</script>
