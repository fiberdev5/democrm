...
<style>
.modal-dimmed {
    opacity: 0.5; /* Opaklığı %50'ye düşür */
    pointer-events: none; /* Üzerindeki tıklama ve mouse olaylarını devre dışı bırak */
    transition: opacity .15s linear; /* Geçişi yumuşat */
}
...
$(document).ready(function() {
   // "Hareket Ekle" modalı açılırken ana modalı solgunlaştır
    $(document).on('show.bs.modal', '#hareketEkleModal', function () {
      // #editStockModal'ın ID'si sabit olduğu için doğrudan seçebiliriz.
      // Not: Ana modalın ID'si #editStockModal değilse, burayı güncellemelisiniz.
      // Sizin kodunuzda ana modal'ın ID'si bulunmuyor, ancak div'i seçmek için
      // bu ID'yi kullanıyoruz. Eğer ana modal'ın ID'si farklıysa onu kullanın.
      // Genellikle bu modal'ı içeren ana div'e #editStockModal ID'si verilir.
      $('#editStockModal').addClass('modal-dimmed');
    });

    // "Hareket Ekle" modalı kapandığında ana modalı normale döndür
    $(document).on('hidden.bs.modal', '#hareketEkleModal', function () {
      $('#editStockModal').removeClass('modal-dimmed');
    });
