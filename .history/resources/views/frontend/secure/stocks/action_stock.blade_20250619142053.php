...
<div class="modal fade" id="hareketEkleModal" tabindex="-1" aria-labelledby="hareketEkleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <form method="post" id="hareketEkleForm" enctype="multipart/form-data">
        @csrf
...
<script>
  $(document).ready(function(){
    ...
    // Form submit - Ajax ile gönderim
    $('#hareketEkleForm').submit(function (e) {
      e.preventDefault();
      var formData = $(this).serialize();
      var stokId = $('#stok_id').val();

      // tenant_id'yi blade'den js'ye alabilirsin veya URL'de ekle
      // Örnek: /{tenant_id}/stok-hareketi-ekle
      var tenantId = '{{ $tenant_id }}';

      $.ajax({
        url: '/' + tenantId + '/stok-hareketi-ekle',  // Route buraya bakacak
        method: 'POST',
        data: formData,
        success: function (response) {
          $('#hareketEkleModal').modal('hide');
          alert('Hareket başarıyla eklendi.');
          location.reload(); // Sayfayı yenile veya tabloyu ajax ile yenile
        },
        error: function (xhr) {
