...
<div class="modal-body">
    ...
    <div class="row align-items-center">
        <!-- Başlık Kısmı: Mobilde tam satır, masaüstünde sola yaslı -->
        <div class="col-12 col-md-6 mb-2 mb-md-0">
            <h5 class="mb-0"><i class="bi bi-arrows-move"></i> Stok Hareketleri</h5>
        </div>
        
        <!-- Buton ve Select Kısmı -->
        <div class="col-12 col-md-6 d-flex justify-content-end gap-2">
            <!-- Buton -->
            <button type="button" class="btn btn-success btn-sm hareketEkleBtn flex-shrink-0"
                data-stokid="{{ $stock->id }}" data-tenant-id="{{ $firma->id }}">
                 <i class="fas fa-plus"></i> <span class="d-none d-sm-inline">Hareket Ekle</span>
            </button>
            
            <!-- SELECT: form-select sınıfı eklendi, id verildi -->
            <!-- Mobilde rahat tıklanması için style="width: auto" yerine flex-grow mobilde kullanıldı -->
            <select id="hareketFiltreSelect" class="form-select form-select-sm islemSec" name="islemSec" style="max-width: 160px;">
                <option value="0">Hepsi</option>
                <option value="1">Alış</option>
                <option value="4">İade (Geri Alma)</option>
...
<script>
 ...
 // Hareket Ekle modalını aç
    $(document).on('click', '.hareketEkleBtn', function () {
        ...
        // Sadece 'change' kullanıyoruz, 'input' mobilde select için gereksiz tetiklenmelere yol açabilir
$(document).on('change', '#hareketFiltreSelect', function () {
    var selected = $(this).val(); // Seçilen değer (Örn: "1")
    var card = $(this).closest('.card-stock'); // En yakın kartı bul
    var rows = card.find('table tbody tr'); // Satırları bul
    var summaryRow = card.find('.toplam-header-row'); // Toplam satırı
    ...
    rows.each(function () {
        // Satır içindeki gizli işlem kodunu al
        var $row = $(this);
        // Mobilde satır yapısı değişmediği sürece bu çalışır. 
        // Boşlukları (newline, space) tamamen temizliyoruz.
        var hiddenText = $row.find('.tdNumber').text().replace(/\s+/g, ''); 
        
        // hiddenText örneği: "0,1" veya "0,4"
        ...
        if (selected == "0") {
            ...
            // "0,1" metni ",1" ile bitiyor mu kontrolü
            if (hiddenText.indexOf(',' + selected) !== -1) {
                shouldShow = true;
            }
        }

        if (shouldShow) {
            $row.show(); // Satırı göster
            // Hesaplama yap
            var adet = parseInt($row.find('td').eq(4).text()) || 0;
            // Fiyat temizliği: ' TL' ve boşlukları sil, virgülü noktaya çevir (varsa)
            var hamFiyat = $row.find('td').eq(5).text().replace(' TL', '').trim();
            var fiyat = parseFloat(hamFiyat) || 0;
            
            toplamAdet += adet;
            toplamFiyat += fiyat * adet;
        } else {
            $row.hide(); // Satırı gizle
        }
    });
