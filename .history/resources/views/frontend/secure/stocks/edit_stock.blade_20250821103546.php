...
$(document).ready(function() {
    ...
    // Modal kapandığında Select2'yi imha et ve event listener'ları temizle
    $('#hareketEkleModal').on('hidden.bs.modal', function () {
        $('#tedarikciSelect').select2('destroy');
        $('#personelSelect').select2('destroy');
        
        // Modal içindeki tüm event listener'ları temizle
        $(this).find('*').off('.modalEvents');
    ...
    // Modal açıldığında event listener'ları yeniden bağla
    $('#hareketEkleModal').on('shown.bs.modal', function () {
        // Önce mevcut event listener'ları temizle
        $('select[name="islem"]').off('change.modalEvents');
        
        // Hareket ekleme modalındaki formun davranışları - namespace ile
        $('select[name="islem"]').on('change.modalEvents', function() {
            var val = $(this).val();
    ...
    // Stok hareketini sil - DÜZELTME: $(document).on kullanarak event delegation
    $(document).on('click', '.stokSilBtn', function () {
        var id = $(this).data('id');
        var tenant_id = "{{ request()->route('tenant_id') }}";
        var $button = $(this); // Butonu referans olarak sakla
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.status === 'success') {
                        ...
                        $button.prop('disabled', false); // Hata durumunda butonu tekrar aktif et
                    }
                },
                error: function () { 
    ...
    // Fotoğrafı sil - DÜZELTME: Event delegation ve buton devre dışı bırakma
