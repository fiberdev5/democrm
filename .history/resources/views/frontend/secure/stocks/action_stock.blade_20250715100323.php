          ...
          <tbody>
            ...
            @foreach($stokHareketleri as $stokIslem)
              ...
              <tr style="{{ $renk }}">
                ...
                <td>
<button type="button"
    class="btn btn-danger btn-sm stokSilBtn"
...
$(document).on('click', '.stokSilBtn', function () {
    ...
    Swal.fire({
        title: 'Emin misiniz?',
        text: "Bu stok hareketini silmek istiyor musunuz?",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/'+tenant_id+'/stok-hareket-sil/' + id,
                method: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function (res) {
                    if (res.status === 'success') {
                        ...
                        Swal.fire('İşlem Engellendi', res.message, 'warning');
                    }
                },
                error: function () {
