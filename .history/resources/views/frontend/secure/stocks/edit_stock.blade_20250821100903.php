...
<div class="card">
    ...
    <div class="card-body">
        {{-- Fotoğraf yükleme formu - Sadece fotoğraf yoksa göster --}}
        <form method="POST" id="stokFotoEkle" enctype="multipart/form-data" style="display: {{ $photos->isEmpty() ? 'block' : 'none' }};">
            @csrf
            <div class="mb-2">
        ...
        <div class="row imgBox">
            @foreach($photos as $foto)
                <div class="col-4 col-md-3 stn mb-2" data-id="{{ $foto->id }}">
                    ...
                    <button class="btn btn-danger btn-sm w-100 stokFotoSil mt-1 py-0" data-id="{{ $foto->id }}">
...
$('select[name="islem"]').on('change', function() {
   ...
   // Fotoğraf seçildiğinde otomatik yükle - EVENT DELEGATION
            ...
            success: function (res) {
                ...
                $('.imgBox').prepend(`
                    <div class="col-4 col-md-3 stn mb-2" data-id="${res.id}">
                        <img src="${res.resim_yolu}" class="img-fluid border rounded" style="width: 100%;">
                ...
                // "Henüz fotoğraf yüklenmemiş" mesajını gizle
                $('#noPhotos').hide();
                
                // File input'u gizle (çünkü artık fotoğraf var)
                $('#stokFotoEkle').hide();
            },
            error: function (xhr) {
                $(".imgLoad").hide();
    ...
    // Fotoğrafı sil - EVENT DELEGATION İLE DÜZELTİLDİ
    $(document).on('click', '.stokFotoSil', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                console.log('Delete response:', res); // Debug için
                ...
                if (res.alert_type === 'success') {
                    ...
                    fotoDiv.fadeOut(300, function () { 
                        ...
                        // Eğer hiç fotoğraf kalmadıysa
                        if ($('.imgBox .stn').length === 0) {
                            // File input'u tekrar göster
                            $('#stokFotoEkle').show();
                            // "Henüz fotoğraf yüklenmemiş" mesajını göster
                            $('#noPhotos').show();
                        }
                    });
                } else {
            ...
            error: function (xhr) {
                console.error('Delete error:', xhr); // Debug için
