...
$(document).ready(function () {
    ...
    function initializeSelect2(selector, placeholder, url) {
        // Select2'nin zaten başlatılıp başlatılmadığını kontrol et
        // Sadece başlatılmışsa 'destroy' et.
        if ($(selector).data('select2')) { // <-- Bu kontrol daha güvenilirdir
    ...
    // Select2'leri başlatan genel fonksiyon
    function initializeAllSelect2s() {
        ...
        initializeSelect2('select[name="urunKategori"]', 'Kategori ara...', '/' + tenantId + '/search-categories');
        initializeSelect2('select[name="raf_id"]', 'Raf ara...', '/' + tenantId + '/search-shelves');
    }

    // Sayfa ilk yüklendiğinde Select2'leri başlat
    initializeAllSelect2s();
    ...
    // Düzenleme modalı kapatıldığında ana modaldaki Select2'leri yeniden başlat
    ...
    $(document).on('input', 'input[name="adet"], input[name="fiyat"], input[name="fiyatBirim"]', hesaplaFiyat);

    // Ürün kodu mask - event delegation kullan
    $(document).on('input', 'input[name="urunKodu"]', function() {
    ...
    // Form submit - event delegation kullan
    $(document).on('submit', '#addStock', function(event) {
    ...
    var checkTimeout;
    $(document).on('input', 'input[name="urunAdi"]', function () {
    ...
    $(document).on('submit', '#addBrandForm', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('select[name="marka_id"]').append(newOption).trigger('change');
    ...
    $(document).on('submit', '#addDeviceTypeForm', function(e) {
        e.preventDefault(); 
        var form = $(this);  
        $.ajax({
            url: form.attr('action'),
            ...
            success: function(response) {
                var newOption = new Option(response.text, response.id, true, true);
                $('select[name="cihaz_id"]').append(newOption).trigger('change');
    ...
    $(document).on('submit', '#addCategoryForm', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                var newOption = new Option(response.text, response.id, true, true);
                $('select[name="urunKategori"]').append(newOption).trigger('change');
    ...
    $(document).on('submit', '#addShelfForm', function(e) {
        e.preventDefault(); 
        var form = $(this);  
        $.ajax({
            url: form.attr('action'),
            ...
            success: function(response) {
                var newOption = new Option(response.text, response.id, true, true);
                $('select[name="raf_id"]').append(newOption).trigger('change');
