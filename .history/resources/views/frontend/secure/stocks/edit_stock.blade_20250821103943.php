...
$(document).ready(function() {
    ...
    // Global değişken - silme işlemlerini takip etmek için
    window.deletingInProgress = false;

    // Stok hareketini sil - ONE() kullanarak her seferinde tek event bağla
    $(document).on('click', '.stokSilBtn', function () {
        // Eğer silme işlemi devam ediyorsa, yeni tıklamaları engelle
        ...
        if (confirm("Stok hareketini silmek istediğinize emin misiniz?")) {
            window.deletingInProgress = true; // Silme başladı
            ...
            $.ajax({
                ...
                success: function (res) {
                    window.deletingInProgress = false; // Silme tamamlandı
                    alert(res.message);
                ...
                error: function () { 
                    window.deletingInProgress = false; // Hata durumunda da sıfırla
...
$(document).on('change', 'select[name="islem"]', function() {
    ...
    // Global değişken - fotoğraf silme işlemlerini takip etmek için
    window.photoDeleteInProgress = false;

    // Fotoğrafı sil - Global flag ile kontrol
    $(document).on('click', '.stokFotoSil', function (e) {
        e.preventDefault();
        
        // Eğer silme işlemi devam ediyorsa, yeni tıklamaları engelle
        if (window.photoDeleteInProgress) return;
        
        if (!confirm("Fotoğraf silinsin mi?")) return;

        window.photoDeleteInProgress = true; // Silme başladı
        ...
        $.ajax({
            ...
            success: function (res) {
                window.photoDeleteInProgress = false; // Silme tamamlandı
                alert(res.message);
                fotoDiv.fadeOut(300, function () { 
                    $(this).remove();
                    // Eğer hiç fotoğraf kalmadıysa "henüz fotoğraf yüklenmemiş" mesajını ve dosya seçme alanını göster
                    if ($('.imgBox .stn').length === 0) {
                        $('#noPhotos').show();
                        // Dosya seçme formunu da tekrar göster
                        if ($('#stokFotoEkle').is(':hidden')) {
                            $('#stokFotoEkle').show();
                        }
                    }
                });
            ...
            error: function (xhr) {
                window.photoDeleteInProgress = false; // Hata durumunda da sıfırla
