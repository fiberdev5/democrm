...
<form method="POST" id="editStock" action="{{ route('update.stock', [$firma->id, $stock->id]) }}">
 ...
 <div id="tab2" class="tab-pane fade" style="padding: 0;">
  <div class="card" style="margin-bottom: 3px;">
    <div class="card-header ch1" style="padding: 3px 0;">
      <div class="row" style="margin-left: -10px;margin-right: -10px;">
        <div class="col-5">
          <button type="button" class="btn btn-success btn-sm hareketEkleBtn" data-id="{{ $stock->id }}" @if($kalanGun < 0) disabled @endif>Hareket Ekle</button>
        </div>
  ...
  <div class="table-responsive" style="margin: 0;">
    <table class="table table-hover table-striped filterTable" width="100%" cellspacing="0" style="margin: 0;">
      <thead class="title">
        <tr>
          <th style="padding: 5px 10px;font-size: 12px;">Tarih</th>
          <th style="padding: 5px 10px;font-size: 12px;">İşlem</th>
          <th style="padding: 5px 10px;font-size: 12px;" class="thBaslik">No</th>
      ...
      <tbody>
        ...
        @foreach($stokHareketleri as $stokIslem)
          @php
            $tarihSaat = explode(' ', $stokIslem->tarih);
            ...
            if($stokIslem->islem == 1){
              ...
              $perSec = $perKasa ? \App\Models\Personel::find($perKasa->pid) : \App\Models\Personel::find($stokIslem->personel);
            }
          @endphp
            ...
            <td style="vertical-align: middle; font-size: 11px; padding: 0 10px; font-weight: 500;">
              ...
              @elseif($stokIslem->islem == 2)
                @if(in_array(1, $grup_izinler ?? []))
                  <a href="{{ url('servisler/'.$stokIslem->servisid) }}" target="_blank">Servis: {{ $stokIslem->servisid }} ({{ $stokIslem->adsoyad }})</a>
                @else
                  Servis: {{ $stokIslem->servisid }} ({{ $stokIslem->adsoyad }})
                @endif
              @elseif($stokIslem->islem == 3)
                {{ $perSec->adsoyad ?? '' }}
              @endif
