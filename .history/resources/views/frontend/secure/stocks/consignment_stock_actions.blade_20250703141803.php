        ...
        <tbody>
          ...
          @foreach($stokHareketleri as $stokIslem)
            @php
             ...
             }elseif($stokIslem->islem == 2){ // Bu bloğu ekleyin
              $islem = "Serviste Kullanım";
              $renk = 'background-color: rgb(255, 182, 193);'; // Örneğin, açık kırmızı
          }
            ...
            <tr style="{{ $renk }}">
              ...
              <td>{{ $islem }}</td>
              <td>
                @if($stokIslem->islem == 4)
                  {{  $stokIslem->musteri->adSoyad ?? '-' }}
                @endif
              </td>
    ...
    <form id="hareketEkleForm" method="POST" action="{{ route('store.consignment.stock.action', request()->route('tenant_id')) }}">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3" id="musteriGroup">
            <label>Müşteri Seç</label>
            <select name="servisid" class="form-control">
              <option value="">Seçiniz</option>
              @foreach(\App\Models\Customer::all() as $musteri)
