...
$(document).ready(function(){
  ...
  var table; // Global tablo değişkeni
  ...
  $(document).on('click', '.editStock', function(){
    ...
    $.ajax({
      ...
      if($.trim(data.html) === "-1"){
        ...
        $('#editStockModalLabel').text(data.urunAdi || 'Stok Düzenle');
      }
    }).fail(function(){
      alert('Bir hata oluştu. Lütfen tekrar deneyin.');
    });
  ...
  table = $('#datatableStock').DataTable({
    ...
    drawCallback: function(settings){
      ...
      if(json && json.toplamAdet !== undefined && json.toplamFiyat !== undefined){
    ...
    pageLength: 25,
    stateSave: false, // Cache kapatıldı
    searching: true
  });

  // Filtre dropdownları değişince tablonun yeniden çizilmesi
  ...
  $('#clearFilters').click(function(){
    $('#raf, #marka, #cihaz, #personel').val('');
    table.draw();
  });

  // Tablo reload fonksiyonu (modal kapatıldığında kullanmak için)
  window.reloadStockTable = function() {
    if (table) {
      table.ajax.reload(null, false); // false = sayfa sıfırlanmasın
    }
  };
});
</script>
