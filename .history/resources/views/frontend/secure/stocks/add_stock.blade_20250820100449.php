  ...
  <div class="row mb-2">
    ...
    <div class="col-sm-8">
      <div class="input-group">
        ...
        <button class="btn btn-success" type="button"  id="addDeviceTypeBtn">+</button>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="addDeviceTypeModal" tabindex="-1" aria-hidden="true" style="background: rgba(0, 0, 0, 0.7); padding-top:100px;">
    <div class="modal-dialog modal-sm ">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Yeni Cihaz Türü Ekle</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="addDeviceTypeForm" action="{{ route('store.device.type.ajax', $firma->id) }}">
                    @csrf
                    <div class="row mb-3"><label class="col-sm-4">Cihaz:<span class="text-danger">*</span></label><div class="col-sm-8"><input name="marka" class="form-control" type="text" required></div></div>
                    <div class="row">
                        <div class="col-sm-12 text-end">
                            <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">İptal</button>
                            <input type="submit" class="btn btn-info btn-sm" value="Kaydet">
...
$(document).ready(function () {
    ...
    $(document).on('click', '#addDeviceTypeBtn', function () {
        // Bootstrap'in JavaScript API'sini kullanarak yeni bir modal örneği oluştur
        var brandModal = new bootstrap.Modal(document.getElementById('addBrandModal'));
        // Modalı göster
        brandModal.show();
