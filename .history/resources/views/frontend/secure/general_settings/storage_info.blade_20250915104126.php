...
<div class="storage-widget" id="storageWidget">
    ...
    <div class="storage-actions">
        @if($storageInfo['warning_threshold'])
            <a href="{{ route('subscription.plans') }}" class="btn btn-upgrade">
                <i class="fas fa-arrow-up"></i>
                <span>Planı Yükselt</span>
            </a>
        @endif
        
        <button class="btn btn-manage" onclick="openFileManager()">
...
function refreshStorageInfo() {
    ...
    fetch(`/tenant/{{ auth()->user()->tenant->id }}/storage/info`)
...
function loadStorageDetails() {
    ...
    fetch(`/tenant/{{ auth()->user()->tenant->id }}/storage/details`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function updateDetailedInfo(details) {
    // Update file counts (implement based on your API response)
    document.getElementById('servicePhotosCount').textContent = details.service_photos?.count || '0';
    document.getElementById('stockPhotosCount').textContent = details.stock_photos?.count || '0';
    document.getElementById('otherFilesCount').textContent = details.other_files?.count || '0';
}

function openFileManager() {
    // Implement file manager modal or redirect
    window.location.href = '/files/manage';
}
