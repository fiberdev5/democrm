 ...
 $(document).ready(function () {
    ...
    function loadData(url, tabId) {
        ...
        $.ajax({
            ...
            $(tabId).html('<div class="alert alert-danger">Veriler yüklenemedi.</div>');
        }).always(function() {
    ...
    // Dropdown-item'lar için düzeltilmiş click olayı
    $('.kasaSubMenu .dropdown-item').on('click', function (e) {
        e.preventDefault();
        e.stopPropagation();

        var tabMap = {
            "nav1": "/{{$firma->id}}/firma-bilgileri", // nav1 de ekledim
            "nav2": "/{{$firma->id}}/sms-ayarlari",
            "nav3": "/{{$firma->id}}/cihaz-markalari",
            "nav4": "/{{$firma->id}}/cihaz-turleri",
            "nav5": "/{{$firma->id}}/garanti-sureleri",
        ...
        // Sadece tıklanan item'a active class'ı ekle
    ...
    $('.kasaSubMenu .dropdown-toggle').on('click', function(e) {
        e.preventDefault();
    ...
    // Dropdown'un kapanmasını engelle (sadece içeriğe tıklandığında)
