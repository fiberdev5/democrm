@php
    ...
    $anketPersonel = $anket?->personelUser ?? $servis->personelUser ?? null;
@endphp
...
<form method="POST" id="surveyForm" action="{{ route('survey.store', ['tenant_id' => $tenant_id, 'servisId' => $servis->id]) }}">
    ...
    <div class="col-lg-12 rw1">
        ...
        <input type="text" readonly class="form-control" value="{{ $anketPersonel->name ?? 'Belirtilmemiş' }}">
        <input type="hidden" name="personel" value="{{ $anketPersonel->personel ?? '' }}">
    </div>
    @endif
