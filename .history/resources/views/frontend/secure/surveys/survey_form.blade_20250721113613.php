@php
    ...
    $anketiYapilanPersonel = null;
    if ($anket && $anket->personelUser) {
        $anketiYapilanPersonel = $anket->personelUser;
    } elseif (isset($servisiYapanPersonelId)) {
        // SurveyCreate metodundan gelen ID ile kullanıcıyı çek
        $anketiYapilanPersonel = \App\Models\User::find($servisiYapanPersonelId);
    }
@endphp
...
<form method="POST" id="surveyForm" action="{{ route('survey.store', ['tenant_id' => $tenant_id, 'servisId' => $servis->id]) }}">
    ...
    <div class="col-lg-12 rw1">
        ...
        <input type="hidden" name="personel" value="{{ $anketiYapilanPersonel->name ?? '' }}">
    </div>
    @endif
