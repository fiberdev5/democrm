...
<div class="modal fade" id="surveySuccessModal" tabindex="-1" aria-labelledby="surveySuccessModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="surveySuccessModalLabel">Web Sitesi Mesajı</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body text-center">
        <p id="modalMessage" class="fw-bold fs-5"></p>
      </div>
...
<script>
// sayiKontrol fonksiyonu eksikti, ekliyoruz.
    ...
    $('#surveyForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Eğer Laravel'den 'success: true' gelirse
                if (response.success) {
                    $('#modalMessage').text(response.message); // Modal mesajını ayarla
                    var surveyModal = new bootstrap.Modal(document.getElementById('surveySuccessModal'));
                    surveyModal.show(); // Modalı göster

                    // Modalı kapattıktan sonra sayfayı yenilemek isterseniz:
                    $('#surveySuccessModal').on('hidden.bs.modal', function (e) {
                        location.reload(); // Modalı kapattıktan sonra sayfayı yenile
                    });
                } else {
