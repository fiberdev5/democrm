            ...
            <div class="row">
                <div class="col-lg-4 sol">
                    <button data-toggle="collapse" data-target="#planlamaSearch" 
                            class="btn btn-primary btn-sm btn-block planlamaSearchBtn">
                            ...
                            <div class="card-header" style="padding: 5px;">
                                <form id="filtreForm">
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <div class="col-md-8 rw2">
                                            <input type="text" name="tarih1" class="form-control datepicker tarih1" readonly 
                                                   value="{{ now()->subMonth()->format('d/m/Y') }}" style="background:#fff;margin-bottom: 3px;">
                                            <input type="text" name="tarih2" class="form-control datepicker tarih2" readonly 
                                                   value="{{ now()->format('d/m/Y') }}" style="background:#fff;margin-bottom: 2px;">
                                            
                                            <div class="tarihAraliklari" style="margin-top: 5px">
                                                <button type="button" class="btn btn-primary btn-sm tarihDegistirBtn" 
                                                        data-tarih1="{{ now()->subYear()->format('d/m/Y') }}" 
                                                        data-tarih2="{{ now()->format('d/m/Y') }}" 
                                                        style="padding: 0 5px">Son 1 Yıl</button>
                                                <button type="button" class="btn btn-primary btn-sm tarihDegistirBtn" 
                                                        data-tarih1="{{ now()->subMonth()->format('d/m/Y') }}" 
                                                        data-tarih2="{{ now()->format('d/m/Y') }}" 
                                                        style="padding: 0 5px">Son 1 Ay</button>
                                                <button type="button" class="btn btn-primary btn-sm tarihDegistirBtn" 
                                                        data-tarih1="{{ now()->subDays(7)->format('d/m/Y') }}" 
                                                        data-tarih2="{{ now()->format('d/m/Y') }}" 
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <div class="col-md-8 rw2">
                                            <select class="form-control bolgeler" id="ilce" name="bolgeler[]" multiple style="height: 170px">
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12" style="padding: 0 5px">
                                        <button type="button" class="btn btn-block btn-primary btn-sm topluSmsListele">Listele</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
$(document).ready(function () {
    ...
    function ilceGetir(il) {
        $("#ilce").attr("disabled", false).html('<option value="0" selected>HEPSİ</option>');
        
        $.ajax({
            url: "{{ route('toplu-sms.ilce-getir') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                ilceSecimId: il
    ...
    $(".topluSmsListele").click(function(){
        ...
        // Tarihleri formatla
        data.tarih1 = data.tarih1.replace(/\//g, '-');
        data.tarih2 = data.tarih2.replace(/\//g, '-');

        $(".servisListe").html("<img src='{{ asset('images/ajax_load.gif') }}'>");

        $.ajax({
            url: "{{ route('toplu-sms.listele') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                ...data
