@php
    ...
    $debug['firma_id'] = Auth::user()->firma_id ?? 'NULL';
    
    // Query 1: whereHas ile kontrol
    $query1 = \App\Models\IntegrationPurchase::where('tenant_id', 28)
    ...
    $debug['query1_sql'] = $query1->toSql();
    $debug['query1_exists'] = $query1->exists();
    $debug['query1_count'] = $query1->count();
    
    // Query 2: Direkt integration_id ile
    $query2 = \App\Models\IntegrationPurchase::where('tenant_id', 28)
        ->where('integration_id', 10)
        ->where('status', 'completed')
        ->where('is_active', true);
        
    $debug['query2_exists'] = $query2->exists();
    $debug['query2_record'] = $query2->first();
    
    // Query 3: Tüm şartları kaldırarak
    $query3 = \App\Models\IntegrationPurchase::where('tenant_id', 28)
        ->where('integration_id', 10);
        
    $debug['query3_exists'] = $query3->exists();
    $debug['query3_record'] = $query3->first();
    
    // Sonuç
    $verimorActive = $query1->exists();
    $debug['verimor_active'] = $verimorActive;
    
    dump($debug);
@endphp
<!-- Debug sonuçlarını göster -->
<div style="background: #f0f0f0; padding: 15px; margin: 10px; border: 2px solid #333;">
    <h3>VERIMOR DEBUG</h3>
