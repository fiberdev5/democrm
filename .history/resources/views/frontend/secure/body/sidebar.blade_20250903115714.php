     ...
     <div id="sidebar-menu">
       <ul class="metismenu list-unstyled" id="side-menu" style="padding-top: 2px;"> 
             @if(Auth::user()->isSuperAdmin())
              ...
              {{-- Destek Talepleri menüsü --}}
<li class="nav-item">
    <a href="#sidebarSupport" class="nav-link" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="sidebarSupport">
        <i class="fas fa-life-ring"></i>
        @php $openTicketsCount = \App\Models\SupportTicket::where('status', 'acik')->count(); @endphp
        @if($openTicketsCount > 0)
            <span class="badge rounded-pill bg-danger float-end">{{ $openTicketsCount }}</span>
        @endif
        <span data-key="t-support">Destek Talepleri</span>
    </a>
    <div class="collapse menu-dropdown {{ request()->routeIs('super.admin.destek.*') ? 'show' : '' }}" id="sidebarSupport">
        <ul class="nav nav-sm flex-column">
            <li class="nav-item">
                <a href="{{ route('super.admin.destek.dashboard') }}" class="nav-link {{ request()->routeIs('super.admin.destek.dashboard') ? 'active' : '' }}" data-key="t-support-dashboard"> Dashboard </a>
            </li>
            <li class="nav-item">
                <a href="{{ route('super.admin.destek.index') }}" class="nav-link {{ request()->routeIs('super.admin.destek.index') && !request()->has('status') && !request()->has('priority') ? 'active' : '' }}" data-key="t-all-tickets"> Tüm Talepler </a>
            </li>
            <li class="nav-item">
                <a href="{{ route('super.admin.destek.index', ['status' => 'acik']) }}" class="nav-link {{ request()->fullUrlIs(route('super.admin.destek.index', ['status' => 'acik'])) ? 'active' : '' }}" data-key="t-open-tickets"> Açık Talepler </a>
            </li>
            <li class="nav-item">
                <a href="{{ route('super.admin.destek.index', ['priority' => 'yuksek']) }}" class="nav-link {{ request()->fullUrlIs(route('super.admin.destek.index', ['priority' => 'yuksek'])) ? 'active' : '' }}" data-key="t-urgent-tickets"> Acil Talepler </a>
            </li>
        </ul>
    </div>
</li>
