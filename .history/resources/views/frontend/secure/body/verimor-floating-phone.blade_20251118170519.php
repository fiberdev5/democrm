@php
    ...
    if (Auth::check() && Auth::user()->firma_id) {
        // DOĞRUDAN SORGU - whereHas kullanma
        $purchase = \App\Models\IntegrationPurchase::where('tenant_id', Auth::user()->firma_id)
            ->where('integration_id', 10) // Verimor Santral
            ->where('status', 'completed')
            ->where('is_active', 1)
            ->first();
        
        $verimorActive = $purchase !== null;
    }
@endphp
    ...
    function loadWebPhone() {
        $.ajax({
            ...
            error: function(xhr) {
                ...
                $('#phoneLoading').html('<div class="text-danger"><i class="fas fa-times"></i><p>Yüklenemedi: ' + (xhr.responseJSON?.message || 'Bilinmeyen hata') + '</p></div>');
            }
        });
    }
