...
$('.consignment-checkbox:checked').each(function() {
    // Stage ID'sini almanın farklı bir yolu
    // Bu, hidden input'un formu içinde olduğunu varsayar
    const stageId = $(this).closest('form').find(`input[name="soru[${$(this).attr('name').match(/\[(\d+)\]/)[1]}]"]`).attr('name').match(/\[(\d+)\]/)[1];
    
    // Eğer hidden input'un tam adı her zaman aynı paternde ise (soru[id])
    // `$(this).closest('div.col-lg-12').find('input[name^="soru["]');`
    // veya daha doğrudan:
    const hiddenInput = $(this).closest('.myKonsinyeList').nextAll('input[type="hidden"][name^="soru["]').first();


    if (hiddenInput.length > 0) {
        const nameAttr = hiddenInput.attr('name');
        if (nameAttr) {
            const matchResult = nameAttr.match(/\[(\d+)\]/);
            if (matchResult && matchResult[1]) {
                ...
                console.error('Stage ID bulunamadı:', nameAttr);
            }
        } else {
            console.error('Name attribute bulunamadı');
        }
    } else {
        console.error('Hidden input bulunamadı');
    }
});
