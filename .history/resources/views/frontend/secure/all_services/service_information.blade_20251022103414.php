...
<input type="hidden" name="acil" class="acil" value="{{$service_id->acil !== 0 ? '1' : '0'}}"/>    
        </div>
...
$(document).ready(function (e) {  
    // Acil butonuna tıklandığında hidden input değerini güncelle
    $(".servisAcilBtn").click(function() {
        var checkbox = $(this).find('input[type="checkbox"]');
        var isChecked = !checkbox.is(':checked'); // Toggle olacağı için tersini al
        
        // Checkbox durumunu değiştir
        checkbox.prop('checked', isChecked);
        
        // Hidden input değerini güncelle
        $("#servisDuzenle .acil").val(isChecked ? "1" : "0");
        
        // Görsel güncelleme için (eğer gerekiyorsa)
        if(isChecked) {
    ...
    $("#servisDuzenle").on('submit', function (e) {
        ...
        // Form verilerine servis ID'yi ekle
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                var errorMessage = "Servis güncellenirken hatayla karşılaşıldı.";
