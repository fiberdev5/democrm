...
<script type="text/javascript">
  $(document).ready(function (e) {
    ...
    // Servis Düzenle formunun submit işlemi
    $("#servisDuzenle").on('submit', (function (e) {
      ...
      $.ajax({
        ...
        success: function (data) {
            ...
            loadServiceHistory({{ $service_id->id }}); // **ÖNEMLİ: Servis güncellendiğinde geçmişi de yenile**
            $('.nav1').trigger('click'); // İlgili sekmeyi tetikle
        },
        error: function (xhr, status, error) {
          alert("Servis güncellenirken hatayla karşılaşıldı.");
          console.error("Servis Güncelleme Hatası:", xhr.responseText);
        }
      });
    }));
  });
