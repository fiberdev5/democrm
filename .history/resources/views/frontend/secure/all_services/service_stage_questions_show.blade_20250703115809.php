...
<form method="post" id="servisPlanKaydet" action="{{ route('save.service.plan', $firma->id) }}" class="col-sm-6" style="margin: 0 auto;padding:10px;">
    @csrf
    @foreach($stage_questions as $question)
        {{-- [Parca] veya [Konsinye Cihaz] soru türü için bu blok çalışsın --}}
        @if($question->soruTuru == '[Parca]' || $question->soruTuru == '[Konsinye Cihaz]')
            <div class="row form-group">
                <div class="col-lg-12">
                    <label>{{ $question->soru }}</label>
                    
                    {{-- Dinamik olarak gösterilecek stok listesi --}}
                    <div class="parcalar-listesi myParcaList" style="width:100%">
                        <input id="urunAraInput_{{ $question->id }}" type="text" class="form-control mb-2" autocomplete="off" placeholder="Ürün adı veya ürün kodu ara...">
                        
                        @php $say = 0; @endphp

                        {{-- Personel Stokları Listesi --}}
                        @if($question->soruTuru == '[Parca]') {{-- Eğer bu soru Personel Stoğu ile ilgiliyse --}}
                            ...
                            @forelse($stoklar as $stok)
                                ...
                                @if($stokSec && $stokAdet > 0)
                                    @php $say++; @endphp
                                    <div class="checkbox stock-item" style="padding:3px 0;" data-product-name="{{ $stokSec->urunAdi ?? 'N/A' }}" data-product-code="{{ $stokSec->urunKodu ?? '' }}">
                                        <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                                            <input type="checkbox" name="parca[{{ $question->id }}][]" value="{{ $stokId }}" 
                                        ...
                                        <input type="number" name="adet[{{ $question->id }}][{{ $stokId }}]" value="1" min="1" max="{{ $stokAdet }}" class="form-control quantity-input" autocomplete="off" style="width: 40px;display: inline-block;text-align:center; display:none;">
                                    </div>
                                @endif
                            @empty
                        ...
                        {{-- Konsinye Cihazlar Listesi --}}
                        @elseif($question->soruTuru == '[Konsinye Cihaz]') {{-- Eğer bu soru Konsinye Cihaz ile ilgiliyse --}}
                            <p>Toplam Konsinye Cihaz Sayısı: {{ $toplamConsignmentAdedi }}</p>
                            @forelse($consignmentDevices as $device)
                                @if($device->current_stock_quantity > 0) {{-- Sadece stoğu olanları göster --}}
                                    @php $say++; @endphp
                                    <div class="checkbox stock-item" style="padding:3px 0;" data-product-name="{{ $device->name ?? 'N/A' }}" data-product-code="{{ $device->urunKodu ?? '' }}">
                                        <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                                            <input type="checkbox" name="parca[{{ $question->id }}][]" value="C_{{ $device->id }}" {{-- 'C_' öneki --}}
                                        ...
                                        <input type="number" name="adet[{{ $question->id }}][C_{{ $device->id }}]" value="1" min="1" max="{{ $device->current_stock_quantity }}" class="form-control quantity-input" autocomplete="off" style="width: 40px;display: inline-block;text-align:center; display:none;">
                                    </div>
                                @endif
                            @empty
        ...
        @else
            {{-- Diğer mevcut soru türleri (Açıklama, Grup, Tarih, Saat, Araç, Fiyat, Teklif, Bayi) --}}
            <div class="row form-group">
...
$(document).ready(function() {
    ...
    $('[id^="urunAraInput_"]').on('keyup', function() {
    ...
    $('.stock-checkbox').on('change', function() {
        ...
        if ($(this).is(':checked')) {
            ...
            quantityInput.hide();
            quantityInput.val(1); // Kutudan işaret kaldırılınca adeti 1 yap
        }
    });
    ...
    // ... Mevcut servisPlanKaydet submit handler kodunuz ...
    $('#servisPlanKaydet').on('submit', function(e) {
        ...
        // Eğer seçili parçalar varsa, bunları formData'ya ekle
        // Bu kısım, her iki stok türü için de geçerli olacak şekilde güncellendi
        $('.stock-checkbox:checked').each(function() {
            const questionId = $(this).attr('name').match(/parca\[(\d+)\]/)[1];
            const stockValue = $(this).val(); // Bu, normal stok ID'si veya 'C_KonsinyeID' olacak
            const quantity = $(this).closest('.stock-item').find('.quantity-input').val();
            
            if (quantity && parseInt(quantity) > 0) {
                 formData.append('parca[' + questionId + '][]', stockValue);
                 formData.append('adet[' + questionId + '][' + stockValue + ']', quantity);
            }
        });
