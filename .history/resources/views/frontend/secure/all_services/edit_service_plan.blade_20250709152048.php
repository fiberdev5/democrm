...
$('.consignment-checkbox:checked').each(function() {
    // Önce elementi bulalım
    const hiddenInput = $(this).closest('.form-group').find('input[name^="soru["]');
    
    // Element var mı kontrol edelim
    if (hiddenInput.length > 0) {
        const nameAttr = hiddenInput.attr('name');
        
        // name attribute var mı kontrol edelim
        if (nameAttr) {
            const matchResult = nameAttr.match(/\[(\d+)\]/);
            
            // match sonucu var mı kontrol edelim
