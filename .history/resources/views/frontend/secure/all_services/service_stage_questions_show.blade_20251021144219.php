...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @elseif(str_contains($stage->cevapTuru, 'Grup'))
        @php
            // Birden fazla grup var mı kontrol et (virgül ile ayrılmış)
            $isMultipleGroups = str_contains($stage->cevapTuru, ',');
        @endphp
        
        @if($isMultipleGroups && str_contains($stage->cevapTuru, '[Grup-4]') && str_contains($stage->cevapTuru, '[Grup-5]'))
            ...
            @php
                ...
                $teknisyenYardimcilari = App\Models\User::where('tenant_id', $firma->id)
                    ->where('status', '1')
                    ->whereHas('roles', function($query) {
                        $query->where('name', 'Teknisyen Yardımcısı'); // Rol adını kontrol edin
                    })
                    ->orderBy('name', 'asc')
                    ->get();
            @endphp
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label>Teknisyen <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label>Teknisyen Yardımcısı <small class="text-muted">(Opsiyonel)</small></label>
