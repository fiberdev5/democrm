...
@section('user')
 <!-- <script type="text/javascript" src="https://cdn.jsdelivr.net/jquery/latest/jquery.min.js"></script>-->
  ...
  <style>
    ...
    /* Sayfa yüklenirken tabloyu tamamen gizle */
table#datatableService {
    display: none !important;
}

/* DataTable wrapper oluşturulduğunda göster */
div.dataTables_wrapper table#datatableService {
...
<script type="text/javascript">
  ...
  // Filtre butonunu BUL ama TAŞIMA
  var filterWrapper = $('.searchWrap.kullanici_teknisyenfiltre').first();

  // Flexbox container oluştur
  var flexContainer = $('<div class="d-flex align-items-center w-100 gap-2"></div>');

  // "Search:" metnini kaldır
  ...
  // SADECE arama kutusunu ekle
  flexContainer.append(searchContainer);
  
  // Filtre butonunu KLONLAMA, sadece referans ver
  // Eğer filtre butonu zaten sayfada varsa, onu kullan
  if (filterWrapper.length > 0 && filterWrapper.parent().attr('id') !== 'datatableService_wrapper') {
    // Filtre butonu DataTable dışındaysa, klonla
    flexContainer.append(filterWrapper.clone(true));
  } else {
    // Filtre butonu yoksa veya içerdeyse, orijinalini kullan
  ...
  // Görünür yap
  $('#datatableService_wrapper').addClass('ready');
        }
      });
