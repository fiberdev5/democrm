...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        // cevapTuru'ndan grup numaralarını çıkar
        preg_match_all('/Grup-(\d+)/', $stage->cevapTuru, $matches);
        $gruplar = $matches[1] ?? [];
        
        $personel = collect();
        
        // Admin grubu için (Grup-0)
        if (in_array('0', $gruplar)) {
            $adminPersonel = App\Models\User::where('tenant_id', $firma->id)
                ...
                ->orderBy('name', 'asc')
                ->get();
            $personel = $personel->merge($adminPersonel);
        }
        
        // Teknisyen grubu için (Grup-3, Grup-4)
        if (array_intersect(['3', '4'], $gruplar)) {
            $teknisyenPersonel = App\Models\User::where('tenant_id', $firma->id)
                ...
                ->orderBy('name', 'asc')
                ->get();
            $personel = $personel->merge($teknisyenPersonel);
        }
        
        // Atölye Ustası grubu için (Grup-4, Grup-5)
        if (array_intersect(['4', '5'], $gruplar)) {
            $atolyeUstasiPersonel = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
                ->whereHas('roles', function($query) {
                    $query->where('name', ['Atölye Ustası','Teknisyen']);
                })
                ->orderBy('name', 'asc')
                ->get();
            $personel = $personel->merge($atolyeUstasiPersonel);
        }
        
        // Duplicate kayıtları temizle
        $personel = $personel->unique('user_id');
    @endphp
