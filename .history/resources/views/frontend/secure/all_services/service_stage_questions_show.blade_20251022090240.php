...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @elseif(str_contains($stage->cevapTuru, 'Grup'))
        @php
            ...
            // İlk grup required olacak
            $isFirst = true;
        @endphp
        
        @foreach($grupKodlari as $grupKodu)
            @php
                $roller = [];
                $grupLabel = '';
                ...
                if (in_array($grupKodu, [261, 262])) {
                    ...
                    $grupLabel = 'Atölye Ustası';
                } elseif ($grupKodu == 4) {
                    $roller = ['Teknisyen'];
                    $grupLabel = 'Teknisyen';
                } elseif ($grupKodu == 5) {
                    $roller = ['Teknisyen Yardımcısı']; // veya ['Yardımcı Teknisyen'] eğer rol adı farklıysa
                    $grupLabel = 'Teknisyen Yardımcısı';
                }
            ...
            @if($personeller->count())
                <div class="form-group mb-3">
                    @if($grupLabel)
                        <label class="form-label">{{ $grupLabel }}</label>
                    @endif
                    <select class="form-control" 
                            name="soru[{{ $stage->id }}][grup_{{ $grupKodu }}]" 
                            @if($isFirst) required @endif>
            ...
            @else
                <p>{{ $grupLabel ?: "Grup-$grupKodu" }} için personel bulunamadı.</p>
            @endif
