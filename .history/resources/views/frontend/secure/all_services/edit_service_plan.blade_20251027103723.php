...
$("#servisPlanGuncelle").on('submit', function(e) {
    ...
    // ÖNEMLİ: Tüm stok checkbox verilerini manuel olarak temizle
    // Çünkü FormData otomatik olarak checkbox'ları ekliyor
    $('.stock-checkbox').each(function() {
        const stockId = $(this).val();
        formData.delete(`stokCheck${stockId}`);
        formData.delete(`stokAdet${stockId}`);
    });

    $('.consignment-checkbox').each(function() {
        const stockId = $(this).val();
        formData.delete(`konsinyeCheck${stockId}`);
        formData.delete(`konsinyeAdet${stockId}`);
    });

    // Şimdi sadece işaretli olanları ekle
    $('.stock-checkbox:checked').each(function() {
        const parcaDropdown = $(this).closest('.myParcaList');
        const stageId = parcaDropdown.data('soru-id');
    ...
    // Seçili "Konsinye Cihaz" ürünlerini FormData'ya ekle
    ...
    // Debug: FormData içeriğini kontrol et (isteğe bağlı)
