    ...
    <div class="row form-group">
        <div class="col-lg-12">
            ...
            <div class="parcalar-dropdown myParcaList" data-soru-id="{{ $plan->id }}" style="width:100%">
                ...
                {{-- Düzeltme burada! $stok zaten bir PersonelStock kaydı --}}
                @forelse($stoklar as $personelStokKaydi) {{-- Değişken adını değiştirdim, daha açıklayıcı olsun --}}
                    @php
                        // Artık $personelStokKaydi direkt PersonelStock objesi
                        // PersonelStock modelindeki 'stok' ilişkisi sayesinde Stock objesine erişebiliriz
                        $stok = $personelStokKaydi->stok; 
                        $stokId = $stok->id;
                        $stokAdet = $personelStokKaydi->adet; // Adet zaten PersonelStock objesinde
        ...
        <div class="col-lg-8">
            @php
                ...
                if ($plan->cevap) {
                    ...
                    if (count($cevapParts) >= 2) {
                        ...
                        // Seçili cihazın adını ve mevcut stok miktarını direkt Stock modelinden al
            ...
            @if($currentConsignmentId)
                <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9; border-radius: 8px;">
                    ...
                    <p>Bu cihazdan mevcut toplam stok: {{ $selectedConsignmentDeviceCurrentStock }}</p>
                </div>
            @else
                <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9; border-radius: 8px;">
                    <p>Henüz atanmış bir konsinye cihaz yok.</p>
                </div>
            @endif

            {{-- Konsinye cihaz seçimi dropdown'u ve adet girişi alanı artık burada olmayacak --}}
            {{-- Çünkü EditServicePlan sadece toplam adedi gönderiyor ve detaylı liste yok. --}}
            {{-- Eğer yeni bir konsinye cihaz seçimi gerekiyorsa, bunu farklı bir mekanizma (örneğin bir modal) ile yapmalısınız. --}}
            
            {{-- Sadece toplam konsinye adet bilgisini gösterebiliriz (EditServicePlan'dan gelen). --}}
            <p>Toplam Müşteri Konsinye Cihaz Adedi: **{{ $toplamConsignmentAdedi }}**</p>

            {{-- Gizli input - form submit'te bu değer gönderilecek --}}
            {{-- Bu input'un value'su, yukarıdaki PHP bloğunda hesaplanan $plan->cevap değeri olmalı. --}}
            <input type="hidden"
                   name="soru{{ $plan->id }}"
