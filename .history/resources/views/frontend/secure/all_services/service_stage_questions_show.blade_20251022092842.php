...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    <input type="hidden" name="soru[{{ $stage->id }}]" class="form-control" value="Konsinye Cihaz"/>
                ...
                @else
                    <div class="col-lg-12" > 
                       ...
                       @if(str_contains($stage->cevapTuru, 'Grup'))
    @php
        // Teknisyenler için sorgu
        ...
        // Teknisyen ve Teknisyen Yardımcıları için sorgu
        $teknisyenVeYardimcilari = App\Models\User::where('tenant_id', $firma->id)
            ->where('status', '1')
            ->whereHas('roles', function($query) {
    ...
    {{-- Teknisyen Select Kutusu --}}
    <div class="form-group">
        <label for="teknisyen-{{ $stage->id }}">Teknisyen Seçimi:</label>
        @if($teknisyenler->count())
            <select class="form-control" name="teknisyen_soru[{{ $stage->id }}]" id="teknisyen-{{ $stage->id }}" required>
        ...
        @else
            <p>Teknisyen rolüne sahip personel bulunamadı.</p>
        @endif
    </div>

    {{-- Teknisyen Yardımcısı Select Kutusu --}}
    <div class="form-group mt-3">
        <label for="yardimci-teknisyen-{{ $stage->id }}">Teknisyen Yardımcısı Seçimi (İsteğe Bağlı):</label>
        @if($teknisyenVeYardimcilari->count())
            <select class="form-control" name="yardimci_teknisyen_soru[{{ $stage->id }}]" id="yardimci-teknisyen-{{ $stage->id }}">
                <option value="">-Seçiniz-</option>
                @foreach($teknisyenVeYardimcilari as $personel)
                    <option value="{{ $personel->user_id }}">{{ $personel->name }}</option>
                @endforeach
            </select>
        @else
            <p>Teknisyen veya Teknisyen Yardımcısı rolüne sahip personel bulunamadı.</p>
        @endif
    </div>
@endif
