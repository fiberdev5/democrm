                ...
                <div class="row form-group">
                    ...
                    <div class="col-lg-8">
                        @php
                            ...
                            // Stored 'cevap' string'ini ayrıştır (sadece C_ID---Adet formatına odaklan)
...
<script>
    $(document).ready(function(e) {
        ...
        // Konsinye adet giriş alanı değiştiğinde
        ...
        $('.consignment-device-select').each(function() {
            ...
            if (selectedOption.val()) { // Eğer bir cihaz seçiliyse (edit modunda genelde seçili gelir)
                ...
                updateConsignmentCombinedValue(selectElement); // İlk yüklemede gizli input'u güncelle
            } else {
                // Seçili cihaz yoksa adet alanını gizle
                quantityGroup.hide();
                quantityInput.val(1).removeAttr('max');
                availableQuantityText.text('(Mevcut: 0)');
                updateConsignmentCombinedValue(selectElement); // Seçim yoksa gizli input'u boşalt
            }
        });

        // Konsinye cihazlar için gizli input değerini güncelleyen fonksiyon
        function updateConsignmentCombinedValue(selectElement) {
            ...
            if (selectedOption.val()) { // Bir cihaz seçildiyse
                ...
                if (isNaN(quantity) || quantity < 1) {
                    ...
                    quantity = available;
                    quantityInput.val(quantity); // Adeti mevcut stoğa düşür
                }

                combinedValueInput.val(`C_${deviceId}---${quantity}`);
            } else {
                combinedValueInput.val(''); // Cihaz seçimi kaldırıldıysa boşalt
            }
        }
