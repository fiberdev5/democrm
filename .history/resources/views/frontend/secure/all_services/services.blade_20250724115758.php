...
<script>
  ...
  $(document).on('submit', '#gelenCagriArama', function (e) {
    e.preventDefault();
    activeFilterType = 'gelen-cagrilar';
    activeFilters = formToObj($(this));
    table.draw(); // datatable’ı güncelle
    $('#servisRaporlaModal').modal('hide');
  });
...
<div id="gelenCagriRaporu" style="display:none;">
    @include('frontend.secure.incoming_calls.all_calls')
</div>

<script>
    function raporlamaFiltreDegisti(filterType) {
        if (filterType === 'gelen-cagrilar') {
            $('#gelenCagriRaporu').show();
            // Diğer tabloları gizle
        } else {
            $('#gelenCagriRaporu').hide();
        }
    }

    // Örneğin filtre değiştiğinde
    $('#filtreSelect').change(function() {
        raporlamaFiltreDegisti($(this).val());
    });
</script>
<script>
 $(document).ready(function () {
