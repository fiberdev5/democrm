...
<script>
  $(function () {
    ...
    $('#assignBtn').click(function () {
      ...
      // 1. Modal'ı "Yükleniyor..." durumuyla hazırla ve göster
      $('#servisPersonelAtamaModal .modal-body').html('<div class="text-center p-3"><i class="fa fa-spinner fa-spin fa-2x"></i><p>Form yükleniyor…</p></div>');
      $('#servisPersonelAtamaModal').modal('show');

      // 2. AJAX ile form içeriğini getir
      $.get("{{ route('service.plan.form', $firma->id) }}", { servisidler, gelenDurum, gidenDurum })
      .done(function(html) {
        // Başarılı olursa içeriği modal'a bas
        $('#servisPersonelAtamaModal .modal-body').html(html);
      })
      .fail(function(jqXHR, textStatus, errorThrown) {
        // Başarısız olursa konsola detaylı hata yaz ve kullanıcıya mesaj göster
        console.error("Planlama formu yükleme hatası:", textStatus, errorThrown, jqXHR.responseText);
        const errorMessage = '<div class="alert alert-danger">Form yüklenemedi. Sunucu hatası veya ağ sorunu olabilir. Lütfen tekrar deneyin.</div>';
        $('#servisPersonelAtamaModal .modal-body').html(errorMessage);
      });
    });
  });
</script>
