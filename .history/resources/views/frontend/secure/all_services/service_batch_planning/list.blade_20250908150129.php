...
$(document).on('click', '.personelServisDuzenle', function(e){
    ...
    console.log('Tıklama gerçekleşti, AJAX başlatılıyor...');
    
    // ÖNCE modal'ı aç, SONRA içeriği yükle
    $('#personelServisDuzenleModal .modal-title').html(name+" ("+id+")");
    $('#personelServisDuzenleModal .modal-body').html('<div class="text-center p-3"><i class="fa fa-spinner fa-spin"></i> Yükleniyor...</div>');
    $('#personelServisDuzenleModal').modal('show');
    
    console.log('Modal açıldı, AJAX çağrısı yapılıyor...');
    ...
    $.ajax({
        ...
        $('#personelServisDuzenleModal .modal-body').html('<div class="alert alert-danger">Hata oluştu</div>');
    });

    $('#personelServisDuzenleModal').on('hide.bs.modal', function(e) {
    e.preventDefault(); // Kapanmayı engelle
