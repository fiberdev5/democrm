...
<script>
  $(document).ready(function() {
    ...
    function checkAndRestoreServiceList() {
      // Açık modal varsa veya modal açılma sürecindeyse bekle
        if ($('.modal.show').length > 0 || $('.modal').hasClass('fade')) {
            return;
        }
    ...
    $(document).on('hidden.bs.modal', '.modal', function() {
      ...
      var excludedModals = ['servisTopluPlanlaModal', 'personelServisDuzenleModal', 'servisPersonelAtamaModal'];
      if (modalId && !excludedModals.includes(modalId)) {
        setTimeout(function() {
          checkAndRestoreServiceList();
        }, 500);
      }
