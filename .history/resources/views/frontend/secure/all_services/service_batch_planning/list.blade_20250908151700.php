...
<div id="personelServisDuzenleModal" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex='-1' style="padding-top: 20px;background: rgba(0, 0, 0, 0.50);">  
...
$(document).ready(function(){
    $(".personelServisDuzenle").click( function(e){
        ...
        // Modalın body'sini temizleyip yükleniyor mesajı gösterelim
        $('#personelServisDuzenleModal .modal-title').html(name + " (" + id + ")");
        $('#personelServisDuzenleModal .modal-body').html('Yükleniyor...');
        $('#personelServisDuzenleModal').modal('show'); // Modalı önce gösterelim
        ...
        $.ajax({
            url: "/" + firma_id + "/servis/duzenle/" + id
                ...
                // İçerik yüklendikten sonra modalı tekrar göstermek gerekmez, zaten açık.
                // Eğer modal içeriği içinde bir sorun varsa bu satırdan sonra olabilir.
            ...
            $('#personelServisDuzenleModal .modal-body').html('<div class="alert alert-danger">Servis detayları yüklenirken bir hata oluştu. Lütfen konsolu kontrol edin.</div>');
        });
    });
});
</script>
