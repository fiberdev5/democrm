...
<script type="text/javascript">
 $(document).ready(function(e) {
    ...
    $("#topluPlanKaydet").on('submit', function (e) {
        e.preventDefault(); // Form'un normal submit davranışını engelle
        
        var formData = new FormData(this);
        
        // Submit butonunu devre dışı bırak (çift gönderim önlemi)
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            dataType: 'json', // JSON response beklediğimizi belirt
            success: function (data) {
                // Başarılı mesajı göster
                if (data.success) {
                    alert(data.success);
                    
                    // Modal'ı kapat ve içeriğini temizle
            ...
            complete: function() {
                // İşlem tamamlandıktan sonra submit butonunu tekrar aktif et
                $("#topluPlanKaydet").find('input[type="submit"]').prop('disabled', false);
            }
        });
    });
});
