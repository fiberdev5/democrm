...
<div class="row form-group">
    <div class="col-lg-12">
        <label>{{ $soru->soru }}</label>
                ...
                @if(!empty($kullanilanParcalarArray))
                    <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9;">
                        <p style="font-weight: bold; margin-bottom: 5px;">Kullanılan Parçalar:</p>
                        <ul>
        ...
        <div class="parcalar-dropdown myParcaList" data-soru-id="{{ $plan->id }}" style="width:100%">
            {{-- Eski @php $say = 0; @endphp satırını buradan kaldırın --}}
            <p>Toplam Stok Sayısı: {{ $stoklar->count() }}</p>

            @forelse($stoklar as $stok)
                @php
                    // Bu kısımlar doğru çalışıyor, olduğu gibi kalsın
                    ...
                    $stokSec = $stok; // Zaten Stock objesi, tekrar sorgulamaya gerek yok.
                ...
                @if($stokSec && $stokAdet > 0)
                    {{-- Eski @php $say++; @endphp satırını buradan kaldırın, artık gerekli değil --}}
                ...
                @endif
            @empty
                {{-- Bu 'empty' bloğu, eğer $stoklar koleksiyonu tamamen boşsa çalışır --}}
                <p style="color:orange;">Stok verisi bulunamadı.</p>
            @endforelse

            {{-- Buradaki $say kontrolünü kaldırın ve yerine $stoklar->isEmpty() kontrolünü kullanın --}}
            @if($stoklar->isEmpty()) 
