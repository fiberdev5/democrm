  ...
  $(document).ready(function () {
    ...
    var table = $('#datatableService').DataTable({
      ...
      createdRow: function (row, data) {
          ...
          // 🔔 ACİL ALARM GÖRSELİ EKLEME
          if (data.acil == 1) {
              const $cihazTd = $('td', row).eq(2); // 4. sütun = cihaz sütunu
              const alarmIcon = "<img src='{{asset('frontend/img/alarm.gif')}}' style='width:12px;height:12px;margin-left:4px;margin-top:-2px' title='Acil Servis'>";
              $cihazTd.append(alarmIcon);
          }
