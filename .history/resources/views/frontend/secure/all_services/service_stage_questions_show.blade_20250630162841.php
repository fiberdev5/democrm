{{-- Normal Servis Formu --}}
{{-- Tek bir form olduğundan emin olun, eğer aşağıda bir tane daha varsa onu silin. --}}
  ...
  @foreach($stage_questions as $stage)
      @if($stage->cevapTuru == "[Parca]")
        <div class="row form-group">
          <div class="col-lg-12">
            <label>{{ $stage->soru }}</label> {{-- Parça seçimi için soru --}}
            ...
            <div class="parcalar-dropdown myParcaList" style="width:100%">
              ...
              @foreach($stoklar as $stok)
                @if($stok->adet > 0)
                  @php
                    $stokSec = App\Models\Stock::where('firma_id', $firma->id)->where('id', $stok->stokid)->first();
                    $say++;
                  @endphp
                  <div class="checkbox" style="padding:3px 0;">
                    <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                      {{-- Checkbox'ın name'ini stokCheck[stokid] olarak değiştiriyoruz --}}
                      <input type="checkbox" name="stokCheck{{ $stok->stokid }}" value="on" style="position: relative; top:2px; margin-right:3px;text-transform: capitalize;">
                      {{ $stokSec->urunAdi }} (Mevcut: {{ $stok->adet }})
                    </label>
                    {{-- Parça adeti için input. name'ini stokAdet[stokid] olarak değiştiriyoruz. max özelliği ile mevcut stoktan fazlasını girmeyi engelle. --}}
                    <input type="number" name="stokAdet{{ $stok->stokid }}" value="1" min="1" max="{{ $stok->adet }}" class="form-control" autocomplete="off" style="width: 40px;display: inline-block;text-align:center;">
                  </div>
                @endif
              @endforeach
          ...
          {{-- Bu gizli input sadece "[Parca]" türünde bir sorunun olduğunu belirtir. --}}
          <input type="hidden" name="soru[{{ $stage->id }}]" class="form-control" value="Parca"/>
        </div>
      @else
        {{-- Diğer soru türleri olduğu gibi kalabilir --}}
        <div class="row form-group">
...
{{-- Sadece bir tane AJAX submit script bloğu bırakın --}}
<script>
$(document).ready(function() {
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.status === 'success') {
          ...
          // Bu fonksiyonların doğru çalıştığından ve sayfada tanımlı olduğundan emin olun
          ...
          // Modalı kapatmak veya formu gizlemek gibi işlemler
          $('#servisPlanKaydet').hide(); // Veya modalı kapatabilirsiniz
  ...
  // Stok ara inputu için örnek JavaScript (eğer varsa)
