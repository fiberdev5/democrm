                ...
                @foreach($planCevaplar as $plan)
                    ...
                    @elseif($soru->cevapTuru == "[Konsinye Cihaz]")
                        @php
                            ...
                            if ($plan->cevap) {
                                // Yeni formatı ayrıştır: ID:35-Adet:2-Ad:Cihaz Adı
                                if (preg_match('/ID:(\d+)-Adet:(\d+)-Ad:(.*)/', $plan->cevap, $matches)) {
                                    $seciliCihazId = $matches[1];
                                    $seciliAdet = $matches[2];
                                    $seciliCihazAdi = $matches[3];
                                }
                            }
                            // Eğer cevapta ID varsa, cihazı consignmentDevices koleksiyonunda bulun
                            $seciliDevice = $consignmentDevices->where('id', $seciliCihazId)->first();
                            if ($seciliDevice) {
                                $seciliCihazAdi = $seciliDevice->urunAdi ?? $seciliDevice->urun_adi ?? 'N/A';
                            }
                        @endphp
                        ...
                        <div class="row form-group">
                            <div class="col-lg-12">
                                ...
                                <input type="hidden" name="parca[{{ $plan->id }}][consignment_name]"
                                        ...
                                        value="{{ $seciliCihazAdi }}"> {{-- Buraya çekilen adı atıyoruz --}}
