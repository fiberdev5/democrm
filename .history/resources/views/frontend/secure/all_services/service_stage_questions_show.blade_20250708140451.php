    ...
    @foreach($stage_questions as $stage)
        ...
        <div class="row form-group">
            <div class="col-lg-12">
              ...
              @elseif($stage->cevapTuru == "[Konsinye]")
                <div class="col-lg-8" style="margin-left: -15px;">
                    <select class="form-control consignment-device-select" name="konsinye[{{ $stage->id }}]" data-stage-id="{{ $stage->id }}">
                        <option value="">- Konsinye Cihaz Seçiniz -</option>
                        @foreach($konsinyeCihazlar as $cihaz)
                            <option value="{{ $cihaz->id }}" data-product-name="{{ $cihaz->urunAdi }}" data-available="{{ $cihaz->adet }}">
                                {{ $cihaz->urunAdi }} ({{ $cihaz->urunKodu }}) - Mevcut: {{ $cihaz->adet }}
                            </option>
                        @endforeach
                    </select>

                    {{-- Adet girişi --}}
                    <div class="form-group consignment-quantity-group mt-2" style="display:none;">
                        <input type="number" class="form-control quantity-input" name="konsinyeAdet[{{ $stage->id }}]" min="1" value="1" style="width: 100px;" />
                        <small class="text-muted available-quantity-text">Mevcut adet: <span></span></small>
                    </div>

                    <input type="hidden" name="soru[{{ $stage->id }}]" value="Konsinye" />
                </div>
                {{-- Diğer Cevap Türleri --}}
                @else
                    <div class="col-lg-8" style="margin-left: -15px;"> 
...
$('.consignment-device-select').on('change', function() {
    ...
    var group = $select.closest('.form-group').find('.consignment-quantity-group');
    var input = group.find('.quantity-input');
    var availableText = group.find('.available-quantity-text span');

    if (selectedValue) {
        group.show();
        input.attr('max', availableStock).val(1);
        availableText.text(availableStock);
    } else {
        group.hide();
        input.val(1).removeAttr('max');
        availableText.text('');
    }
});
// Seçilen konsinye cihazları FormData'ya ekle
$('.consignment-device-select').each(function() {
    var stageId = $(this).data('stage-id');
    var selectedValue = $(this).val();
    if (selectedValue) {
        var adet = $(this).closest('.form-group').find('.consignment-quantity-group .quantity-input').val();
        formData.append(`konsinye[${stageId}]`, selectedValue);
        formData.append(`konsinyeAdet[${stageId}]`, adet);
    }
});
