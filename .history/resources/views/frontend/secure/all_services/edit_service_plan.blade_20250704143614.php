    ...
    <div class="row form-group">
        ...
        <div class="col-lg-8">
            @php
                ...
                // Check if a consignment device is already assigned to this plan answer
                if ($plan->cevap) {
                    // Parse the stored answer format (e.g., "C_123---5")
                    list($currentConsignmentIdWithPrefix, $currentConsignmentQuantity) = array_pad(explode('---', $plan->cevap), 2, 0);
                    $currentConsignmentId = str_replace('C_', '', $currentConsignmentIdWithPrefix); // Extract the actual ID

                    // Try to find the selected device within the detailedConsignmentStocks passed from the controller
                    $selectedDevice = $detailedConsignmentStocks->firstWhere('id', $currentConsignmentId);
                    if ($selectedDevice) {
                        // If found, use its pre-calculated name
                        $currentConsignmentDeviceName = $selectedDevice->urunAdi ?? $selectedDevice->urun_adi ?? 'Cihaz Adı Bulunamadı';
                    } else {
                        // If not found in the filtered list (e.g., stock became 0 or category changed),
                        // try to find the original device by ID to get its name
                ...
                // If a current consignment ID exists, determine its available stock
                if ($currentConsignmentId) {
                    // First, try to get the stock from the detailed list passed from the controller
                    ...
                    if ($selectedDeviceInView) {
                        ...
                        // Fallback: If the device is not in the detailed list (e.g., due to filtering or zero stock),
                        // re-calculate its current stock directly from StockActions.
                        $girisAdet = App\Models\StockAction::where('stokId', $currentConsignmentId)
                            ->whereIn('islem', [1, 4]) // Incoming operations
                            ->sum('adet');
                        $cikisAdet = App\Models\StockAction::where('stokId', $currentConsignmentId)
                            ->where('islem', 2) // Outgoing operations
                            ->sum('adet');
                        $selectedConsignmentDeviceCurrentStock = max(0, $girisAdet - $cikisAdet);
                    }
                }
            @endphp
            {{-- Display currently assigned device if any --}}
            @if($currentConsignmentId)
                <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9; border-radius: 8px;">
                    <p class="text-gray-700"><strong>Halihazırda Atanan Cihaz:</strong> {{ $currentConsignmentDeviceName }} (Adet: {{ $currentConsignmentQuantity }})</p>
                </div>
            @endif

            {{-- Dropdown for selecting a new consignment device --}}
            <select id="consignment-device-select-{{ $plan->id }}" class="form-control consignment-device-select rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" data-plan-id="{{ $plan->id }}">
                <option value="">-- Konsinye Cihaz Seçiniz --</option>
                {{-- Loop through available detailed consignment stocks --}}
                ...
                {{-- If the currently assigned device is not in the filtered list (e.g., out of stock or category changed), display it as a disabled option --}}
                @if($currentConsignmentId && !$detailedConsignmentStocks->contains('id', $currentConsignmentId))
                    @php
                        $originalSelectedDevice = App\Models\Stock::find($currentConsignmentId);
                    @endphp
                    @if($originalSelectedDevice)
                        <option value="C_{{ $originalSelectedDevice->id }}"
                                data-available="{{ $selectedConsignmentDeviceCurrentStock }}" 
                                data-product-name="{{ $originalSelectedDevice->urunAdi ?? $originalSelectedDevice->urun_adi ?? 'N/A' }}"
                                data-product-code="{{ $originalSelectedDevice->urunKodu ?? '' }}"
                                selected disabled style="color: grey;">
                            {{ $originalSelectedDevice->urunAdi ?? $originalSelectedDevice->urun_adi ?? 'Cihaz Adı Bulunamadı' }} (Mevcut: {{ $selectedConsignmentDeviceCurrentStock }})
                        </option>
                    @endif
                @endif
            </select>
            {{-- Quantity input for the selected consignment device --}}
            <div class="form-group consignment-quantity-group" style="margin-top: 10px; {{ $currentConsignmentId ? 'display:block;' : 'display:none;' }}">
                <label for="consignment-quantity-edit-{{ $plan->id }}" class="block text-sm font-medium text-gray-700">Adet:</label>
                <input type="number"
                        id="consignment-quantity-edit-{{ $plan->id }}"
                        value="{{ $currentConsignmentQuantity }}"
                        min="1"
                        max="{{ $selectedConsignmentDeviceCurrentStock }}"
                        class="form-control quantity-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        autocomplete="off"
                        style="width: 80px; display:inline-block;">
                <span class="text-sm text-gray-500 available-quantity-text">(Mevcut: {{ $selectedConsignmentDeviceCurrentStock }})</span>
            </div>
            {{-- This hidden input will now store the combined value (C_ID---QUANTITY) --}}
            <input type="hidden" name="soru{{ $plan->id }}" id="consignment-combined-value-{{ $plan->id }}" value="{{ $plan->cevap ?? '' }}"/>
        </div>
    </div>
...
<script>
$(document).ready(function(e) {
     // Konsinye Cihaz seçimi değiştiğinde adet alanını ve max değerini güncelle
    $(document).on('change', '.consignment-device-select', function() {
        var selectedOption = $(this).find('option:selected');
        // selectedOption.data('available') artık doğrudan PHP'den gelen current_stock_quantity'yi kullanacak
        ...
        if (selectedOption.val()) {
            ...
            quantityInput.val(1); // Yeni bir cihaz seçildiğinde adedi 1'e sıfırla
    ...
    // Konsinye Cihaz adet input'unun max değer kontrolü
    $(document).on('change keyup', '.consignment-quantity-group .quantity-input', function() {
    ...
    // Sayfa yüklendiğinde konsinye cihaz için başlangıç kontrolü
    $('.consignment-device-select').each(function() {
        var selectedOption = $(this).find('option:selected');
        if (selectedOption.val()) {
            // data-available özniteliği doğru current_stock_quantity değerini içerecek
            var availableQuantity = selectedOption.data('available'); 
            var quantityGroup = $(this).closest('.col-lg-8').find('.consignment-quantity-group');
            var quantityInput = quantityGroup.find('.quantity-input');
            var availableQuantityText = quantityGroup.find('.available-quantity-text');
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const selectElement = document.getElementById(`consignment-device-select-${planId}`);
            const quantityInput = document.getElementById(`consignment-quantity-edit-${planId}`);
            const combinedValueInput = document.getElementById(`consignment-combined-value-${planId}`);
            const quantityGroup = selectElement.nextElementSibling; // Assuming quantity group is right after select
            const availableQuantityText = quantityInput.nextElementSibling;
            ...
            function updateCombinedValue() {
                ...
                if (deviceId) {
                    combinedValueInput.value = `${deviceId}---${quantity}`;
                    // Show quantity group if a device is selected
                    quantityGroup.style.display = 'block';
                } else {
                    combinedValueInput.value = ''; // Clear if no device is selected
                    // Hide quantity group if no device is selected
            ...
            function updateQuantityMax() {
                const selectedOption = selectElement.options[selectElement.selectedIndex];
                const available = selectedOption.dataset.available ? parseInt(selectedOption.dataset.available) : 0;
                quantityInput.setAttribute('max', available);
                availableQuantityText.textContent = `(Mevcut: ${available})`;

                // Adjust current quantity if it exceeds available
                if (parseInt(quantityInput.value) > available) {
                    quantityInput.value = available > 0 ? 1 : 0; // Reset to 1 or 0 if available
                }
                if (available === 0 && selectElement.value !== '') {
                    // If selected device has 0 available, disable quantity input and show warning
                    quantityInput.disabled = true;
                    quantityInput.value = 0;
                    // You might want to add a visual cue here, e.g., change border color
