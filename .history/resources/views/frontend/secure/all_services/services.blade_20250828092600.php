...
/* Tablet ekranlarda kompakt düzen */
@media (min-width: 577px) and (max-width: 991px) {
    .service-buttons .btn-sm {
        font-size: 0.75rem;
        padding: 0.25rem 0.5rem;
    }
}

/* Filtre dropdown'ını sağa yasla */
.filter-section {
    margin-left: auto;
}
            ...
            <div class="service-buttons">
              @if(auth()->user()->can('Tüm Servisleri Görebilir'))
                <a class="btn btn-success btn-sm addService" data-bs-toggle="modal" data-bs-target="#addServiceModal">
                  <i class="fas fa-plus"></i><span> Servis Ekle</span>
                </a> 
                <a type="button" class="btn btn-success btn-sm gelenCagriButon" data-bs-toggle="modal" data-bs-target="#gelenCagriModal">
                  <div class="text">Gelen Çağrılar <i data-toggle="tooltip" title="Gereksiz çağrıları kaydetmek için kullanılır." class="fas fa-info-circle"></i></div>
                </a>
                <button type="button" class="btn btn-danger btn-sm servisPlanlaBtn">
                  <div class="text">Servis Planlama <i data-toggle="tooltip" title="Toplu servis yönlendirmeleri yapmak için kullanılır." class="fas fa-info-circle"></i></div>
                </button>
                <button type="button" class="btn btn-primary btn-sm servisRaporlaModalBtn" data-toggle="modal" data-target="#servisRaporlaModal">
                  Raporlar
                </button>
                <button type="button" class="btn btn-primary btn-sm anketModalBtn" data-toggle="modal" data-target="#anketModal">
                  Anketler
                </button>
                
                @if(Auth::check() && Auth::user()->hasAnyRole(['Operatör']))
                  <button type="button" class="btn btn-primary btn-sm kullaniciPrimGoster" data-toggle="modal" data-target="#kullaniciPrimModal">
                    Primlerim
                  </button>
                @else
                  <button type="button" class="btn btn-primary btn-sm primModalBtn" data-toggle="modal" data-target="#primModal">
                    Primler
                  </button>
                @endif
              @endif
              ...
              @if(auth()->user()->hasAnyRole(['Teknisyen', 'Teknisyen Yardımcısı', 'Atölye Ustası', 'Atölye Çırak']))
                <button type="button" class="btn btn-primary btn-sm teknisyenDepoGoster" data-toggle="modal" data-target="#teknisyenDepoModal">
                  Depo
                </button>
              @endif
