...
@if($soru->cevapTuru == "[Parca]")
    <div class="row form-group">
        <div class="col-lg-12">
            ...
            @endphp
            {{-- ÖNCEKİ SEÇİLİ PARÇALARI SAKLAMAK İÇİN HIDDEN INPUT EKLEYİN --}}
            <input type="hidden" 
                   name="onceki_parcalar_{{ $plan->id }}" 
                   value="{{ $plan->cevap }}" 
                   class="onceki-parcalar-hidden">
...
$(document).ready(function(e) {
    ...
    // Önceden seçili parçaları sakla
    let oncekiSeciliParcalar = {};
    
    // Sayfa yüklendiğinde önceki seçimleri kaydet
    $('.myParcaList').each(function() {
        const stageId = $(this).data('soru-id');
        oncekiSeciliParcalar[stageId] = [];
        ...
        $(this).find('.stock-checkbox:checked').each(function() {
            ...
            oncekiSeciliParcalar[stageId].push({
                id: stockId,
                quantity: quantity
            });
        });
...
// Konsinye Cihaz checkbox değiştiğinde adet girişini göster/gizle ve maksimumu ayarla
    ...
    // Form Submit
     $("#servisPlanGuncelle").on('submit', function(e) {
        ...
        // ÖNCEKİ PARÇALARI KORUMA LOJİĞİ
        $('.myParcaList').each(function() {
            const stageId = $(this).data('soru-id');
            const oncekiParcalar = oncekiSeciliParcalar[stageId] || [];
            
            // Şu anda işaretli olan parçaları topla
            const suankiSeciliParcalar = [];
            $(this).find('.stock-checkbox:checked').each(function() {
                const stockId = $(this).val();
                const quantity = $(this).closest('.stock-item').find('.quantity-input').val();
                suankiSeciliParcalar.push({
                    id: stockId,
                    quantity: quantity
                });
            });
            
            // Eğer kullanıcı hiç parça işaretlemediyse, önceki parçaları koru
            if (suankiSeciliParcalar.length === 0 && oncekiParcalar.length > 0) {
                // Önceki parçaları FormData'ya ekle
                oncekiParcalar.forEach(function(parca) {
                    formData.append(`stokCheck${parca.id}`, 'on');
                    formData.append(`stokAdet${parca.id}`, parca.quantity);
                });
            } else {
                // Kullanıcı değişiklik yaptıysa, yeni seçimleri ekle
                suankiSeciliParcalar.forEach(function(parca) {
                    formData.append(`stokCheck${parca.id}`, 'on');
                    formData.append(`stokAdet${parca.id}`, parca.quantity);
                });
            }
        ...
        // Konsinye cihazlar için aynı mantık
        ...
        // ESKI TEMİZLEME KODLARINI KALDIRIN - ARTIK GEREKSİZ
        // $('.stock-checkbox:not(:checked)').each... kodunu silin
        // $('.consignment-checkbox:not(:checked)').each... kodunu silin
