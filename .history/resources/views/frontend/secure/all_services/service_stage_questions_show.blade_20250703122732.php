...
<div class="row form-group">
   ...
   @elseif($stage->cevapTuru == "[Konsinye Cihaz]")
                        @php $konsinye_say = 0; @endphp {{-- Bu sayacı Konsinye bölümü için kullanacağız --}}
                        <p>Toplam Konsinye Cihaz Stoğu: {{ $toplamConsignmentAdedi }}</p> {{-- İfadeyi daha açık hale getirdim --}}
                        ...
                        @forelse($consignmentDevices as $device)
                            @if($device->current_stock_quantity > 0)
                                ...
                                <div class="checkbox stock-item" style="padding:3px 0;" 
                                     ...
                                     data-product-name="{{ $device->urunAdi ?? $device->urun_adi ?? 'N/A' }}"> {{-- urunAdi veya urun_adi kullanıldı --}}
                                    <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                                        ...
                                        {{ $device->urunAdi ?? $device->urun_adi ?? 'Cihaz Adı Bulunamadı' }} (Mevcut: {{ $device->current_stock_quantity }}) {{-- urunAdi veya urun_adi kullanıldı --}}
                                    </label>
