...
// Önceden seçilmiş parça ID'lerini bul
$('.myParcaList').each(function() {
    const $list = $(this);
    $list.find('.stock-item').each(function() {
        const $checkbox = $(this).find('.stock-checkbox');
        const $qty = $(this).find('.quantity-input');
        const stokId = $checkbox.val();
        const adet = $qty.val();

        // Eğer checkbox DOM'da checked olmasa bile, kullanıcı dokunmamışsa backend'e gönder
        if ($checkbox.attr('checked') && !$checkbox.is(':checked')) {
            formData.append(`stokCheck${stokId}`, 'on');
            formData.append(`stokAdet${stokId}`, adet);
        }
    });
});
