...
$(document).on('submit', '#gelenCagriArama', function (e) {
    e.preventDefault();
    const filters = formToObj($(this));

    if (table) {
        table.destroy(); // varsa önce yok et
    }

    table = $('#gelenCagriTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/gelen-cagrilar-datatable',
            data: filters // burada tarih gibi filtreleri gönderiyoruz
        },
        columns: [
            { data: 'created_at', title: 'Tarih' },
            { data: 'kayitAlan', title: 'Kayıt Alan' },
            { data: 'servis_kaynagi', title: 'Servis Kaynağı' },
            { data: 'marka', title: 'Marka' },
