...
{{-- Grup Kontrolü Başlangıcı --}}
@elseif(str_contains($stage->cevapTuru, 'Grup'))
    @php
        preg_match('/\[Grup-(\d+)\]/', $stage->cevapTuru, $matches);
        $grupKodu = $matches[1] ?? null;

        // Sadece ana teknisyen grubunu işleme al (örneğin 5). 
        // Yardımcı teknisyen grubu (4) ayrıca bir dropdown oluşturmasın.
        $anaTeknisyenGrupKodu = 5; 
    @endphp

    @if ($grupKodu == $anaTeknisyenGrupKodu)
        @php
            // Tüm teknisyenleri bir kez alalım
            $roller = ['Teknisyen', 'Yardımcı Teknisyen']; // Gerekirse rolleri genişletin
        ...
        @if($personeller->count())
            {{-- 1. Ana Teknisyen (Zorunlu Alan) --}}
            <div class="form-group">
                <label for="teknisyen_secimi">Teknisyen <span style="color:red;">*</span></label>
                <select id="teknisyen_secimi" class="form-control" name="soru[{{ $stage->id }}]" required>
            ...
            {{-- 2. Yardımcı Teknisyen (İsteğe Bağlı Alan) --}}
            {{-- ÖNEMLİ: name attribute'ü farklı olmalı! --}}
            <div class="form-group" style="margin-top: 15px;">
                <label for="yardimci_teknisyen_secimi">Yardımcı Teknisyen (İsteğe Bağlı)</label>
                <select id="yardimci_teknisyen_secimi" class="form-control" name="yardimci_teknisyen[{{ $stage->id }}]">
                    <option value="">-Yardımcı Teknisyen Seçiniz-</option>
                    @foreach($personeller as $personel)
                        <option value="{{ $personel->user_id }}">{{ $personel->name }}</option>
                    @endforeach
                </select>
    ...
    @endif {{-- if $grupKodu == $anaTeknisyenGrupKodu end --}}
