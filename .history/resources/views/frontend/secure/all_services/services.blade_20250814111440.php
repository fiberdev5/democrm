...
$(document).ready(function(){
  $('#datatableService').on('click', '.servisi-sonlandir', function(e){
    e.preventDefault();
    
    if ($(this).is(':disabled')) {
      return false; // Eğer radio button disable ise işlem yapma
    }
    
    var servis_id = $(this).attr("data-servis-id");
    var firma_id = {{$firma->id}};
    
    if(confirm('Bu servisi sonlandırmak istediğinizden emin misiniz?')) {
      $.ajax({
        url: "/" + firma_id + "/servis/sonlandir/" + servis_id,
        type: "POST",
        data: {
          "_token": "{{ csrf_token() }}"
        },
        success: function(response) {
          if(response.success) {
            alert('Servis başarıyla sonlandırıldı.');
            $('#datatableService').DataTable().draw(); // Tabloyu yenile
          } else {
            alert('Servis sonlandırılırken bir hata oluştu: ' + response.message);
          }
        },
        error: function(xhr) {
          alert('Servis sonlandırılırken bir hata oluştu.');
