...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @elseif(str_contains($stage->cevapTuru, 'Grup'))
        @php
            // Birden fazla grup varsa hepsini yakala
            ...
            $grupIsimleri = [
                ...
                261 => 'Atölye Ustası',
                262 => 'Atölye Ustası Yardımcısı',
            ];
        @endphp

        @foreach($grupKodlari as $grupKodu)
            @php
                // Grup koduna göre rol belirle
                $roller = [];
                if (in_array($grupKodu, [261, 262])) {
                    $roller = ['Atölye Ustası'];
                } elseif (in_array($grupKodu, [4])) {
                    $roller = ['Teknisyen'];
                } elseif (in_array($grupKodu, [5])) {
            ...
            <div class="mb-2">
                <label>
                    {{ $grupIsimleri[$grupKodu] ?? 'Personel' }}
                </label>

                @if($personeller->count())
                    <select class="form-control" name="soru[{{ $stage->id }}][{{ $grupKodu }}]" required>
