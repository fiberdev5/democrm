...
@elseif($soru->cevapTuru == "[Konsinye Cihaz]")
    <div class="row form-group">
        ...
        <div class="col-lg-8">
            @php
                ...
                if ($plan->cevap) {
                    ...
                    if ($selectedDevice) {
                        ...
                        // Cihaz detailedConsignmentStocks içinde yoksa, doğrudan Stock modelinden ara
                ...
                if ($currentConsignmentId) {
                    ...
                    if ($selectedDeviceInView) {
                        ...
                        // Eğer detaylı stoklarda yoksa, stok hareketlerinden manuel hesapla
            ...
            {{-- Halihazırda Atanan Cihaz bilgisini göster --}}
            @if($currentConsignmentId)
                <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9;">
            ...
            <select name="soru{{ $plan->id }}" class="form-control consignment-device-select" data-plan-id="{{ $plan->id }}">
                {{-- İlk seçenek: Eğer currentConsignmentId boşsa, bu seçeneği otomatik seçili yap --}}
                <option value="" {{ !$currentConsignmentId ? 'selected' : '' }}>-- Konsinye Cihaz Seçiniz --</option>

                {{-- Mevcut stoktaki konsinye cihazları listele --}}
                @foreach($detailedConsignmentStocks as $device)
                    <option value="C_{{ $device->id }}"
                            ...
                            {{ $currentConsignmentId == $device->id ? 'selected' : '' }}> {{-- currentConsignmentId ile eşleşiyorsa seçili yap --}}
                        {{ $device->urunAdi ?? $device->urun_adi ?? 'Cihaz Adı Bulunamadı' }} (Mevcut: {{ $device->current_stock_quantity }})
                    </option>
                @endforeach
            
                {{-- Eğer plana atanmış bir cihaz varsa ama bu cihaz mevcut detailedConsignmentStocks listesinde yoksa (örneğin stoğu bitmişse),
                     o cihazı listeye gri olarak ve seçili ekle --}}
                @if($currentConsignmentId && !$detailedConsignmentStocks->contains('id', $currentConsignmentId))
                    @php
                        // Cihazı doğrudan Stock modelinden çek
                    ...
                    @if($originalSelectedDevice)
                        <option value="C_{{ $originalSelectedDevice->id }}"
                                ...
                                selected style="color: grey;"> {{-- Bu seçeneği otomatik olarak seçili yap ve gri göster --}}
