            ...
            <tbody>
              @foreach($deleted_services as $item)
                <tr data-id="{{$item->id}}">
                  ...
                  <td><div class="mobileTitle">Telefon:</div>{{$item->serviskaynak->kaynak}}</td>
                  <td><div class="mobileTitle">Marka:</div>{{$item->brand->marka}}</td>
                  <td><div class="mobileTitle">Açıklama:</div>{{$item->ariza}}</td>
                  <td><div class="mobileTitle">Personel:</div>{{$item->kayit_alan->name}}</td>
                  <td class="tabloBtn">
                    <a href="javascript:void(0);"  class="btn btn-danger btn-sm mobilBtn deleteService" data-bs-id="{{$item->id}}" title="Sil"><i class="fas fa-trash-alt"></i> <span> Sil</span></a>
                  </td>
                </tr>
              @endforeach
            </tbody>
