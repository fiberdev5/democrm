<ul class="nav nav-pills nav-justified" role="tablist" style="margin-bottom: 5px">
  ...
  {{-- <li class="nav-item" style="font-size: 14px;"><a class="nav-link nav6" data-bs-toggle="pill" href="#tab6" data-id="{{ $service_id->id }}" role="tab"><i class="fas fa-coins"></i><span>Raporlar</span></a></li> --}}
</ul>
...
<script type="text/javascript">
  $(document).ready(function (e) {
    ...
    $.ajax({
      url: "/"+ firma_id + "/servis-bilgileri/tum/{{ $service_id->id }}"
    }).done(function(data) {
      if($.trim(data)==="-1"){
...
<script type="text/javascript">
  $(".nav1").click(function(){
    ...
    if(id){
      $.ajax({
        url: "/" + firma_id + "/servis-bilgileri/tum/" + id
      }).done(function(data) {
        if($.trim(data)==="-1"){
          window.location.reload(true);
        }else{
