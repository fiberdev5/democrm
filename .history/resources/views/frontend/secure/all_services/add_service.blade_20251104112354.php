...
$(document).ready(function() {
    ...
    $('#servisEkle input, #servisEkle select, #servisEkle textarea').on('change input', function() {
        formChanged = true;
    });
    ...
    $('.servisModal').on('hide.bs.modal', function(e) {
        // Eğer form submit ediliyorsa veya form değişmemişse direkt kapat
        if (isSubmitting || !formChanged) {
            return true;
        }
        
        // Form değişmişse onay iste
        if (!confirm('Kapatmak istediğinizden emin misiniz? Yaptığınız değişiklikler kaybolacak.')) {
            e.preventDefault();
            return false;
        }
        
        // Tamam'a basıldıysa formu sıfırla
        formChanged = false;
    });
    
    // Sayfadan ayrılırken de uyar
