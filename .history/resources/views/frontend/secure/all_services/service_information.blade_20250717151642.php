  ...
  <div class="card-header" style="padding: 3px 5px;">
    <div class="row">
      ...
      <div class="col-sm-11" style="text-align: right;">
        <a href="#" id="openCustomerSurveyModal" class="btn btn-warning btn-sm servisMusteriAnketiBtn" data-id="{{ $service_id->id }}" data-customer-id="{{ $service_id->musteri->id }}">Müşteri Anketi</a>
...
<div class="modal fade" id="customerSurveyModal" tabindex="-1" aria-labelledby="customerSurveyModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="customerSurveyModalLabel">Müşteri Anketi - <span id="surveyCustomerName"></span></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div id="surveyFormContent">Yükleniyor...</div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Kapat</button>
        <button type="button" class="btn btn-primary" id="saveSurveyBtn">Kaydet</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        // "Müşteri Anketi" butonuna tıklama olayı
        $('#openCustomerSurveyModal').on('click', function(e) {
            e.preventDefault(); // Sayfa yenilemesini engelle

            var serviceId = $(this).data('id'); // Servis ID'si
            var customerId = $(this).data('customer-id'); // Müşteri ID'si (opsiyonel, gerekirse)
            var customerName = "{{ $service_id->musteri->adSoyad }}"; // Müşteri Adı Soyadı

            // Modal başlığını güncelle
            $('#surveyCustomerName').text(customerName);

            // Anket formunu yüklemek için Ajax çağrısı yap
            // Şimdilik boş bir form yükleyeceğiz, sonraki adımlarda dinamik hale getireceğiz.
            loadSurveyForm(serviceId, 'personel', customerId); // 'personel' veya 'bayi' tipi şimdilik sabit
            
            // Modalı göster
            $('#customerSurveyModal').modal('show');
        });

        // Anket formunu Ajax ile yükleyecek fonksiyon
        function loadSurveyForm(serviceId, type, targetId) {
            var firma_id = {{ $firma->id }}; // Mevcut firma ID'si
            var url = "/" + firma_id + "/servis-anketi/form/" + serviceId; // Anket formu için yeni bir route tanımlayacağız

            // Eski PHP kodunuzda `getid` ve `tip` değişkenleri vardı.
            // Bu değişkenleri Ajax isteğine ekleyebiliriz.
            // Örneğin, anketin hangi personel veya bayi için yapıldığı:
            // Şu an için `targetId` olarak `customerId` kullanılıyor, ancak bu `personel` veya `bayi` ID'si olmalı.
            // Personel seçimi için ayrı bir dropdown veya input alanı gerekebilir.
            // Şimdilik 'personel' tipi ve `service_id->users->id` kullanarak operatör için anket formu getirelim.
            // Eğer anketin yapıldığı personel servis operatörü değilse, bu kısım güncellenmeli.
            var anketYapilanTargetId = "{{ $service_id->users->id }}"; // Servisin operatör ID'si
            var anketTipi = "personel"; // Şu an için sabit 'personel' tipi
            ...
            $.ajax({
                ...
                data: {
                    tip: anketTipi,
                    id: anketYapilanTargetId // Anket yapılacak personelin ID'si
                },
                success: function(data) {
                    if ($.trim(data) === "-1") {
                        alert("İzin hatası veya geçersiz servis ID'si!");
                        window.location.reload(true);
                    } else {
                        $('#surveyFormContent').html(data); // Modala formu yerleştir
                    }
                },
                error: function(xhr, status, error) {
                    console.error("Anket formu yüklenirken bir hata oluştu: " + error);
                    $('#surveyFormContent').html('<p class="text-danger">Anket formu yüklenemedi. Lütfen tekrar deneyin.</p>');
                }
            });
        }
    });
