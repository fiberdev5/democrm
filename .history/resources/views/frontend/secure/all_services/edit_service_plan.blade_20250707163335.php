...
$(document).ready(function(e) {
    ...
    $(document).on('change', '.consignment-device-select', function() {
        console.log('Konsinye cihaz seçimi değişti');
        updateConsignmentCombinedValue($(this));
    });

    // Konsinye cihaz quantity input'u değiştiğinde
    $(document).on('input change', '.quantity-input', function() {
        console.log('Konsinye cihaz adedi değişti');
        const quantityInput = $(this);
        const selectElement = quantityInput.closest('.col-lg-8, .col-lg-12').find('.consignment-device-select');
        updateConsignmentCombinedValue(selectElement);
    });

    // Konsinye cihaz değerlerini güncelleyen fonksiyon
    function updateConsignmentCombinedValue(selectElement) {
        ...
        console.log('updateConsignmentCombinedValue çalıştı');
        console.log('Seçili option:', selectedOption.val());
        console.log('combinedValueInput element:', combinedValueInput.length);
        ...
        if (selectedOption.val() && selectedOption.val() !== '') {
            ...
            // Quantity'yi mevcut stok sınırında tut
            ...
            const newValue = `${deviceName}: C_${deviceId}---${currentQuantity}`;
            combinedValueInput.val(newValue);
            
            console.log('Konsinye cihaz değeri güncellendi:', newValue);
            console.log('Hidden input ID:', combinedValueInput.attr('id'));
            console.log('Hidden input name:', combinedValueInput.attr('name'));
        } else {
            quantityGroup.hide();
            ...
            deviceNameInput.val('');
            console.log('Konsinye cihaz seçimi temizlendi');
        }
    }

    // Sayfa yüklendiğinde her bir konsinye cihaz seçimini ayarla
    console.log('Sayfa yüklendi, konsinye cihaz seçimleri ayarlanıyor...');
    $('.consignment-device-select').each(function(index) {
        console.log('Konsinye cihaz seçimi ayarlanıyor:', index);
        
        // Mevcut seçili değeri kontrol et
        const currentValue = $(this).val();
        console.log('Mevcut seçili değer:', currentValue);
        
        // Eğer seçili bir değer varsa, quantity group'u göster
