...
$('.consignment-checkbox:checked').each(function() {
    ...
    if (nameAttr) {
        // Checkbox'ın name özniteliğinden stageId'yi al
        const matchResult = nameAttr.match(/konsinye_cihaz\[(\d+)\]/);
        ...
        if (matchResult && matchResult[1]) {
            ...
            console.error('Stage ID bulunamadı veya name formatı hatalı:', nameAttr);
        }
    } else {
        console.error('Name attribute bulunamadı (consignment-checkbox üzerinde)');
    }
});
