...
$(document).ready(function() {
    ...
    $('#addServiceModal').off('hide.bs.modal').on('hide.bs.modal', function(e) {
        // Eğer form submit ediliyorsa direkt kapat
        if (isSubmitting) {
            isSubmitting = false;
            confirmShown = false;
            return true;
        ...
        // Eğer confirm zaten gösteriliyorsa, tekrar gösterme
        if (confirmShown) {
            return false;
        }
        ...
        if (confirm('Kapatmak istediğinizden emin misiniz?')) {
            // Tamam'a basıldı - modalı kapat
            confirmShown = false;
            isSubmitting = false;
            $('#addServiceModal').modal('hide');
        } else {
            // İptal'e basıldı - açık kal
            confirmShown = false;
        }
    ...
    // Modal tamamen kapandığında flag'leri resetle
