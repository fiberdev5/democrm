...
$(document).ready(function() {
    ...
    $(".servisAcilBtn").on('click', function(e) {
        ...
        if (checkbox.is(':checked')) {
            ...
            checkmark.show();
            hiddenInput.val('1');
            $(this).css('background', '#dc3545'); // Kırmızı renk (acil)
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    toastr.success('Servis başarıyla güncellendi.');
                    ...
                    toastr.error('Güncelleme başarısız: ' + (data.message || ''));
                }
            },
            error: function(xhr, status, error) {
    ...
    // Otomatik kaydetme fonksiyonu (opsiyonel)
    function autoSaveAcil() {
        var formData = new FormData();
        formData.append('acil', $('#acilHiddenInput').val());
        formData.append('servisid', '{{$service_id->id}}');
        formData.append('cihazModel', $('.cihazModel').val() || '{{$service_id->cihazModel}}');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    toastr.success('Acil durumu güncellendi.');
                }
            },
            error: function(xhr) {
                console.error("Acil güncelleme hatası:", xhr.responseJSON);
            }
        });
    }
});
