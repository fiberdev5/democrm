...
<script>
  ...
  $(document).on('submit', '#yapilanAnketler', function (e) {
    e.preventDefault();
    var formData = {
        filterType: 'yapilananketler',
        filters: {
            yapilan_personel: $('select[name="yapilan_personel"]').val(),
            bayiler: $('select[name="bayiler"]').val(), // ✅ Bayi parametresi
            yapilananket_tarih1: $('input[name="yapilananket_tarih1"]').val(),
            yapilananket_tarih2: $('input[name="yapilananket_tarih2"]').val()
        }
    };
    table.draw(); // datatable’ı güncelle
  ...
  $(document).on('submit', '#yapilmayanAnketler', function (e) {
    e.preventDefault();
       var formData = {
        filterType: 'yapilmayanAnketler', 
        filters: {
            yapilmayan_personel: $('select[name="yapilmayan_personel"]').val(),
            bayiler: $('select[name="bayiler"]').val(), // ✅ Bayi parametresi
            yapilmayananket_tarih1: $('input[name="yapilmayananket_tarih1"]').val(),
            yapilmayananket_tarih2: $('input[name="yapilmayananket_tarih2"]').val()
        }
    };
    table.draw(); // datatable’ı güncelle
    $('#anketModal').modal('hide');
