...
$(document).ready(function(e) {
    ...
    // Tüm işaretli checkbox'ların adet input'larını göster
    ...
    $('.consignment-checkbox:checked').each(function() {
        $(this).closest('.stock-item').find('.consignment-quantity-input').show();
    });
...
// Konsinye Cihaz checkbox değiştiğinde adet girişini göster/gizle ve maksimumu ayarla
  ...
  $("#servisPlanGuncelle").on('submit', function(e) {
    ...
    // PARÇA SEÇİMLERİNİ EKLE
    // Tüm parça checkbox'larını tara (işaretli olsun olmasın)
    $('.myParcaList').each(function() {
        const stageId = $(this).data('soru-id');
        const selectedParts = []; // Bu stage için seçili parçaları topla
        
        // Bu dropdown içindeki tüm işaretli checkbox'ları bul
        ...
        // Bu stage için seçim yapılmadıysa boş değer gönder
        if (selectedParts.length === 0 && stageId) {
            formData.append(`soru${stageId}`, '');
        }
    });

    // KONSİNYE CİHAZ SEÇİMLERİNİ EKLE
    $('.myKonsinyeList').each(function() {
        ...
        $(this).find('.consignment-checkbox:checked').each(function() {
            ...
            selectedConsignments.push(`${stockId}---${quantity}`);
        });
    });

    // İşaretlenmemiş checkbox'ların verilerini temizle
