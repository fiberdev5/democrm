    ...
    @forelse($consignmentDevices as $device)
        @if($device->current_stock_quantity > 0)
            ...
            <div class="checkbox stock-item" style="padding:3px 0;" 
                 ...
                 data-product-name="{{ $device->urunAdi ?? $device->urun_adi ?? 'N/A' }}"> {{-- BURAYI KONTROL EDİN --}}
                <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                    <input type="checkbox" name="parca[{{ $stage->id }}][C_{{ $device->id }}]" 
                           ...
                           style="position: relative; top:2px; margin-right:3px;">
                    {{ $device->urunAdi ?? $device->urun_adi ?? 'Cihaz Adı Bulunamadı' }} (Mevcut: {{ $device->current_stock_quantity }}) {{-- BURAYI KONTROL EDİN --}}
                </label>
                <input type="number" name="adet[{{ $stage->id }}][C_{{ $device->id }}]" {{-- C_ ön ekini adet inputuna da ekledik --}}
