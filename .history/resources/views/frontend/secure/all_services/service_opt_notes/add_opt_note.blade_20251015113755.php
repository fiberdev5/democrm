...
success: function (data) {
  if (data.success) {
    ...
    // DataTables'ı yeniden başlatma/yeniden yükleme güvenliği
    if ($.fn.DataTable.isDataTable('#datatableService')) {
        $('#datatableService').DataTable().ajax.reload();
    } else {
        // Eğer tablo henüz DataTables olarak başlatılmamışsa, başlatın.
        // Bu kısım genellikle sayfa yüklendiğinde bir kez yapılır,
        // ancak emin olmak için buraya da eklenebilir.
        $('#datatableService').DataTable({ /* Buraya DataTables seçeneklerinizi ekleyin */ });
    }
