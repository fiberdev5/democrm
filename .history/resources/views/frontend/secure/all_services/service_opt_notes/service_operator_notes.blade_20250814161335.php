...
<script>
  ...
  const count = $('#operatorNotuTablo tbody tr').length;
  const noDataDiv = `<div id="noOperatorNotu" class="text-center text-muted" style="padding: 20px;">
    <i class="fas fa-user-tie fa-2x mb-2" style="color: #ddd;"></i>
    <p style="font-size: 13px; color: #6c757d; margin: 0;">Henüz operatör notu eklenmemiş</p>
  </div>`;

  if (count === 0) {
    $('#operatorNotuTablo').closest('.table-responsive').hide();
    $('#noOperatorNotu').length === 0 ? $('.card-body').append(noDataDiv) : $('#noOperatorNotu').show();
  } else {
    $('#noOperatorNotu').hide();
