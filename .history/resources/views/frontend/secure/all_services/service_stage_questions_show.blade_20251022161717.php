                ...
                @else
                    <div class="col-lg-12"> 
                        ...
                        @elseif(str_contains($stage->cevapTuru, 'Grup'))
                            @php
                                // Tüm grup kodlarını yakala (virgülle ayrılmış olabilir)
                                ...
                                // Rolleri belirle
                                if (count(array_intersect($grupKodlari, ['261', '262'])) > 0) {
                                    $roller = ['Atölye Ustası'];
                                } elseif (in_array('5', $grupKodlari)) {
                                    // Grup-5 varsa (Yardımcı Teknisyen)
                                    $roller = ['Teknisyen', 'Yardımcı Teknisyen'];
                                } elseif (in_array('4', $grupKodlari)) {
                                ...
                                // Zorunluluk kontrolü: Grup-5 içeriyorsa zorunlu DEĞİL, içermiyorsa zorunlu
                                $isRequired = !in_array('5', $grupKodlari);
                            @endphp

                            @if($personeller->count())
                                <select class="form-control" name="soru[{{ $stage->id }}]" {{ $isRequired ? 'required' : '' }}>
