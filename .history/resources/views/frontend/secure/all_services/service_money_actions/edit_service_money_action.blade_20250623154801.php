            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" id="servisOdemeDuzenle">
                        ...
                        @if(auth()->user()->grup == "242" || auth()->user()->yetki == "1")
                            <div class="row form-group mb-3">
                                <div class="col-lg-5"><label>Tarih <span class="text-danger">*</span></label></div>
                                <div class="col-lg-7">
                                    <input type="text" name="tarih" class="form-control datepicker" readonly 
                                           value="{{ \Carbon\Carbon::parse($servisPara->tarih)->format('d/m/Y H') }}" 
                                    ...
                                    <select class="form-control personeller" name="personeller">
                                        @foreach ($personeller as $personel)
                                            <option value="{{ $personel->id }}" 
                                                {{ $servisPara->pid == $personel->id ? 'selected' : '' }}>
                                                {{ $personel->adsoyad }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="text-center mt-3">
                            ...
                            <a href="{{ route('payments.index') }}" class="btn btn-secondary btn-sm">İptal</a>
                        </div>
                    </form>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $("#servisOdemeDuzenle").on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('payments.update', $servisPara->id) }}",
            type: "POST",
