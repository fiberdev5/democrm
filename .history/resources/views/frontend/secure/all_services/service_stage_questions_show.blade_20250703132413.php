                ...
                @if($stage->cevapTuru == "[Parca]")
                    ...
                    <div class="parcalar-dropdown myParcaList" style="width:100%">
                        ...
                        @forelse($stoklar as $stok)
                            ...
                            @endif
                        @empty
                            {{-- Boş bırakılabilir, aşağıdaki if ile mesaj gösterilecek --}}
                        @endforelse
...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    <div class="col-lg-12"> {{-- Bu div, dıştaki form-group'un col-lg-12'si ile aynı seviyede olacak --}}
        ...
        <select name="parca[{{ $stage->id }}][consignment_select]" 
                ...
                required> {{-- Seçim zorunlu hale getirildi --}}
            <option value="">-- Konsinye Cihaz Seçiniz --</option>
            @foreach($consignmentDevices as $device)
                @if($device->current_stock_quantity > 0)
                    {{-- Option değeri C_ ön ekli, böylece arka uçta konsinye olduğu anlaşılır --}}
                    {{-- data nitelikleri JavaScript için stok ve ürün kodunu taşır --}}
        ...
        {{-- Adet inputu, seçime göre görünür olacak --}}
        ...
        @if($toplamConsignmentAdedi == 0) {{-- Toplam konsinye adedi 0 ise uyarı göster --}}
    ...
    <input type="hidden" name="soru[{{ $stage->id }}]" class="form-control" value="Konsinye Cihaz"/>
                ...
                {{-- Diğer Cevap Türleri --}}
                @else
                    <div class="col-lg-8" style="margin-left: -15px;"> {{-- col-lg-8'i buraya taşıdık --}}
...
$(document).ready(function() {
  $('#servisPlanKaydet').on('submit', function(e) {
    ...
    $('.stock-checkbox:checked').each(function() {
            const full_name = $(this).attr('name'); // Örn: parca[64][C_271] veya parca[64][123]
            const value = $(this).val(); // Örn: C_271 veya 123
            const quantityInput = $(this).closest('.stock-item').find('.quantity-input');
            const quantity = quantityInput.val();

            // Sadece seçili ve adeti girilmiş olanları FormData'ya ekle
            if (quantity && parseInt(quantity) > 0) {
                // formData.append() ile array formatında ekleme
                // name="parca[soru_id][stok_id_or_C_id]" olduğu için doğrudan ekleyebiliriz
                // Laravel request'i bunu parse edecektir.
