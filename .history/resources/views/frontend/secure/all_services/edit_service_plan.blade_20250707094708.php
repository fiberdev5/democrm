...
{{-- Düzenleme Formu - Konsinye Cihaz Bölümü Eklentisi --}}
 ...
 <div class="row form-group">
     ...
     <div class="col-lg-8">
         ...
         <select name="parca[{{ $plan->soruid }}][consignment_select]"
                 class="form-control consignment-device-select"
                 data-stage-id="{{ $plan->soruid }}">
             ...
             @foreach($consignmentDevices as $device)
                 @if($device->current_stock_quantity > 0)
                     <option value="C_{{ $device->id }}"
                             ...
                             {{ ($plan->cevap == 'C_' . $device->id) ? 'selected' : '' }}>
                         {{ $device->urunAdi ?? $device->urun_adi ?? 'Cihaz Adı Bulunamadı' }} (Mevcut: {{ $device->current_stock_quantity }})
                     </option>
                 @endif
             @endforeach
         </select>
         <div class="form-group consignment-quantity-group" style="margin-top: 10px; {{ ($plan->cevap && strpos($plan->cevap, 'C_') === 0) ? '' : 'display:none;' }}">
             <label for="consignment-quantity-{{ $plan->soruid }}">Adet:</label>
             <input type="number"
                    name="adet[{{ $plan->soruid }}][selected_consignment_quantity]"
                    id="consignment-quantity-{{ $plan->soruid }}"
                    value="{{ ($plan->detay && isset(json_decode($plan->detay, true)['adet'])) ? json_decode($plan->detay, true)['adet'] : 1 }}"
                    min="1"
                    class="form-control quantity-input"
                    autocomplete="off"
                    style="width: 80px; display:inline-block;">
             <span class="text-muted available-quantity-text">(Mevcut: {{ $consignmentDevices->firstWhere('id', str_replace('C_', '', $plan->cevap ?? '0'))?->current_stock_quantity ?? 0 }})</span>
         </div>
         {{--Konsinye cihaz adını tutacak input --}}
         <input type="hidden" name="parca[{{ $plan->soruid }}][consignment_name]" class="consignment-device-name-input" value="{{ $consignmentDevices->firstWhere('id', str_replace('C_', '', $plan->cevap ?? '0'))?->urunAdi ?? $consignmentDevices->firstWhere('id', str_replace('C_', '', $plan->cevap ?? '0'))?->urun_adi ?? '' }}"/>
...
<script>
    ...
    // Düzenleme formunda konsinye cihaz dropdown değiştiğinde adet girişini yönet
    $(document).on('change', '.consignment-device-select', function() {
        const selectedOption = $(this).find('option:selected');
        const quantityGroup = $(this).closest('.col-lg-8').find('.consignment-quantity-group');
        ...
        const consignmentNameInput = $(this).closest('.col-lg-8').find('.consignment-device-name-input');
    ...
    // Düzenleme formunda adet girişlerine sadece sayı girilmesini sağla ve min/max kontrolü yap
