...
<script>
 ...
 $('#operatorArama').on('submit', function(e) {
  e.preventDefault();

  activeFilterType = 'operator';
  activeFilters = $(this).serializeArray().reduce((acc, field) => {
    acc[field.name] = field.value;
    return acc;
  }, {});

  console.log('operator filters:', activeFilters);
  console.log('filterType:', activeFilterType);

  table.draw(); // datatable’ı yeniden çağır
});

  $('#teknisyenArama').on('submit', function (e) {
      e.preventDefault();
      activeFilterType = 'technician';
      activeFilters    = serializeToObj($(this));
      table.draw();
  });
