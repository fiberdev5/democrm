            ...
            <tbody>
              @foreach($deleted_services as $item)
                <tr data-id="{{$item->id}}">
                  ...
                  <td class="tabloBtn">
                    <a href="{{ route('restore.service', [$firma->id, $item->id]) }}"  class="btn btn-danger btn-sm mobilBtn restoreService" data-bs-id="{{$item->id}}" title="Sil"> <label style="margin-bottom: 0;"> Geri Al</label></a>
                  </td>
                </tr>
              @endforeach
            </tbody>
