...
$(document).ready(function() {
    ...
    $(document).on('change', '.consignment-device-select', function() {
        ...
        if (selectedOption.val()) { // Bir cihaz seçildiyse
            ...
            // product-name data'sı varsa onu kullan, yoksa metinden ayır
    ...
    // Bu fonksiyonu hem personel stoğu hem de genel parça araması için kullanabiliriz.
    ...
    $(document).on('keyup', '.urunAraInput', function() {
        // .urunAraInput'un next sibling'i olan .myParcaList'in içindeki .checkbox'ları filtrele
        filterProductList($(this), '.myParcaList .checkbox');
    });

    // Ürün Arama (Personel Stoğu)
    $('#urunAraInput').on('keyup', function() {
        // Sadece bu form grubu içindeki .myParcaList'teki .stock-item'leri filtrele
        filterProductList($(this), '.myParcaList .stock-item');
    });
    ...
    $('#servisPlanKaydet').on('submit', function(e) {
        ...
        $('.stock-checkbox:checked').each(function() {
            ...
            const stageId = stageIdMatch ? stageIdMatch[1] : null; // null kontrolü eklendi
        ...
        $('.consignment-device-select').each(function() {
            ...
            if (selectedValue) { // Eğer bir cihaz seçildiyse
                ...
                var productName = $(this).find('option:selected').data('product-name') || $(this).find('option:selected').text().split(' (Mevcut:')[0]; // Cihaz adını al

                // parca[stage_id][consignment_id] formatı için
                ...
                // Cihaz adını backend'e göndermek için (Yeni eklendi)
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    if (typeof loadServiceHistory === 'function') {
                        // Laravel Blade'den gelen {{ $service_id->id }} değerinin JavaScript'e doğru aktarıldığından emin olun
                        // Örneğin, bir data attribute ile HTML'ye basılabilir: <div id="service-data" data-service-id="{{ $service_id->id }}"></div>
                        // Ve burada $('#service-data').data('service-id') şeklinde okunabilir.
                        // Şimdilik doğrudan bırakıldı, Blade tarafından işlenmesi bekleniyor.
    ...
    // Konsinye cihaz adet giriş grubunu gizle (eğer halihazırda seçili bir cihaz yoksa)
    $('.consignment-device-select').each(function() {
        if (!$(this).val()) {
            $(this).closest('.col-lg-8, .col-lg-12').find('.consignment-quantity-group').hide();
        }
    });
});
</script>