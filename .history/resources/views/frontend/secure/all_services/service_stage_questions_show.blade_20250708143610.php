            ...
            <div class="col-lg-12">
                ...
                @elseif($stage->cevapTuru == "[Konsinye Cihaz]")
                    ...
                    <div class="konsinye-dropdown myKonsinyeList" style="width:100%">
                        ...
                        {{-- Buradaki if/else koşulunu kaldırıyoruz, çünkü konsinyeCihazlar zaten filtrelenmiş gelecek --}}
                        @forelse($konsinyeCihazlar as $konsinyeCihaz)
                            @php
                                $konsinyeId = $konsinyeCihaz->id; // Controller'dan gelen $konsinyeCihaz nesnesinde direkt id var
                                $konsinyeAdet = $konsinyeCihaz->adet ?? $konsinyeCihaz->quantity ?? 0;
                                // Burada App\Models\Stock::where... ile tekrar sorgulama YAPMIYORUZ!
                            @endphp

                            {{-- Sadece adet > 0 kontrolü kaldı, çünkü controller'da zaten filtrelendi
                                Ama tekrar kontrol etmek isteyebilirsiniz, o zaman bu satır kalır. --}}
                            @if($konsinyeAdet > 0)
                                @php $konsinye_say++; @endphp
                                <div class="checkbox stock-item" style="padding:3px 0;" data-product-code="{{ $konsinyeCihaz->urunKodu ?? '' }}" data-product-name="{{ $konsinyeCihaz->urunAdi ?? $konsinyeCihaz->urun_adi ?? 'N/A' }}">
                                    <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                                    ...
                                    <input type="number" name="konsinye_adet[{{ $stage->id }}][{{ $konsinyeId }}]"
                                                ...
                                                class="form-control quantity-input consignment-quantity-input" autocomplete="off"
                                                style="width: 40px;display: inline-block;text-align:center; display:none;">
                                </div>
                            @endif
                        @empty
