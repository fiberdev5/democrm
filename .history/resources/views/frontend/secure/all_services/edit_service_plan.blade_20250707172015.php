                    ...
                    @elseif($soru->cevapTuru == "[Konsinye Cihaz]")
                        <div class="row form-group">
                            ...
                            <div class="col-lg-8">
                                @php
                                    ...
                                    if ($plan->cevap) {
                                        ...
                                        if (strpos($plan->cevap, ': C_') !== false) {
                                            ...
                                            $currentConsignmentName = trim($mainParts[0]); // Trim whitespace
                                        ...
                                        elseif (str_starts_with($plan->cevap, 'C_')) {
                                            ...
                                            // Eski format ise, cihaz adını veritabanından almaya çalış
                                            if ($currentConsignmentId && !$currentConsignmentName) {
                                                $tempDevice = App\Models\Stock::find($currentConsignmentId);
                                                if ($tempDevice) {
                                                    $currentConsignmentName = $tempDevice->urunAdi ?? $tempDevice->urun_adi ?? 'Cihaz Adı Bulunamadı';
                                                }
                                            }
                                        }
                                        // Fallback: Eğer sadece cihaz adı saklanmışsa (örn: "ECAKombi")
                                        if (!$currentConsignmentId && $plan->cevap && !str_contains($plan->cevap, '---')) {
                                            $foundDeviceByName = $tumConsinyeCihazlar->first(function ($device) use ($plan) {
                                                return ($device->urunAdi ?? $device->urun_adi) === $plan->cevap;
                                            });

                                            if ($foundDeviceByName) {
                                                $currentConsignmentId = $foundDeviceByName->id;
                                                $currentConsignmentName = $foundDeviceByName->urunAdi ?? $foundDeviceByName->urun_adi;
                                                $currentConsignmentQuantity = 1; // Eğer sadece isim varsa adeti 1 varsay
                                            }
                                        }
                                        ...
                                        if ($currentConsignmentId) {
                                            ...
                                            if ($originalDevice) {
                                                // Cihaz adı hala ayarlanmamışsa veritabanından al
                                                ...
                                                $cikisAdet = App::Models\StockAction::where('stokId', $currentConsignmentId)
                                                    ->where('islem', 2)
                                                    ->where('planId', $plan->planid) // Bu planın kendi kullanımı
                                                    ->sum('adet');
                                    ...
                                    // Debugging output for PHP variables (Kaldırılabilir, sadece hata ayıklama içindir)
                                    // echo "<script>";
                                ...
                                {{-- Adet giriş alanı --}}
                                <div class="consignment-quantity-group" id="consignmentQuantityGroup_{{ $plan->id }}" style="display: {{ ($currentConsignmentId && $currentConsignmentQuantity > 0) ? 'block' : 'none' }}; margin-top: 10px;">
                                    <label>Adet:</label>
                                    <input type="number"
...
{{-- Custom Message Modal/Div --}}
<div id="customMessageModal" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); background-color:#fff; padding:20px; border:1px solid #ccc; box-shadow:0 0 10px rgba(0,0,0,0.2); z-index:1000;">
    <p id="customMessageText"></p>
    <button onclick="hideCustomMessage()">Kapat</button>
</div>

<script>
// Custom message display functions
function showCustomMessage(message, type = 'success') {
    var messageDiv = $('#customMessageModal');
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error) {
                    ...
                    // Başarılı olursa, loadServiceHistory'yi çağır veya sayfayı yenile
                    if (typeof loadServiceHistory === 'function' && data.servis_id) {
                        ...
                        // Sayfa yenilemeyi geciktirerek kullanıcının mesajı görmesini sağlayın
                        setTimeout(function() {
                            location.reload();
            ...
            error: function(xhr, status, error) {
                ...
                try {
                    ...
                    if (jsonResponse.error) {
                        ...
                        errorMsg = jsonResponse.message; // Laravel'in varsayılan hata mesajı
                    }
                } catch (e) {
                    errorMsg = xhr.responseText;
                }
                showCustomMessage('Plan güncellenirken bir hata oluştu: ' + errorMsg, 'error');
            }
        });
    });
    ...
    function updateConsignmentCombinedValue($selectElement) {
        ...
        if (selectedValue && quantity > 0) {
            var combinedValue = productName + ': ' + selectedValue + '---' + quantity;
    ...
    $('.consignment-device-select').each(function() {
        ...
        updateConsignmentCombinedValue($select); // Başlangıçta da hidden inputu doğru değere ayarla
    });
});
</script>