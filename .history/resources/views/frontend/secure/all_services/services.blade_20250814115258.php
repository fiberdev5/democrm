...
$(document).ready(function() {
    // DataTables dinamik olarak satırları yüklediği için 'on' metodu ile event delegation kullanıyoruz.
    // Bu, sonradan eklenen butonların da çalışmasını sağlar.
    $('#datatableService').on('click', '.servisi-sonlandir-btn', function() {
        var button = $(this); // Tıklanan butonu bir değişkene atıyoruz.

        // Butonun içindeki data-* etiketlerinden gerekli bilgileri alıyoruz.
        var servisId = button.data('servis-id');
        var gelenIslemId = button.data('gelen-islem-id');
        var gidenIslemId = button.data('giden-islem-id');
        var firmaId = '{{ $firma->id }}'; // Blade içinden firma (tenant) ID'sini alıyoruz.

        // Kullanıcıdan onay alıyoruz.
        if (!confirm('Bu servisi sonlandırmak istediğinizden emin misiniz?')) {
            return; // Eğer kullanıcı "İptal" derse işlemi durdur.
        }

        // Kullanıcının tekrar tıklamasını önlemek için butonu pasif hale getirip bir "yükleniyor" ikonu gösteriyoruz.
        button.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> İşleniyor...');

        $.ajax({
            // DİKKAT: Buradaki URL'i kendi projenizdeki SaveServicePlan rotasıyla değiştirin.
            // Bu rota, 'SaveServicePlan' fonksiyonunu tetiklemelidir.
            url: `/${firmaId}/services/save-plan`,
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}', // Laravel CSRF koruması için gereklidir.
                servis: servisId,
                // PHP tarafı gelenIslem'i JSON olarak bekliyordu, bu yüzden formatı koruyoruz.
                gelenIslem: JSON.stringify({ id: gelenIslemId }), 
                gidenIslem: gidenIslemId
            },
            success: function(response) {
                // PHP'den dönen cevabı kontrol ediyoruz.
                if (response.status === 'success') {
                    // İşlem başarılıysa DataTables'ı yeniden çizdirerek güncel halini gösteriyoruz.
                    // (null, false) parametreleri tablonun mevcut sayfasında kalmasını sağlar.
                    $('#datatableService').DataTable().ajax.reload(null, false);
                    
                    // İsteğe bağlı: Kullanıcıya 'toastr' gibi bir kütüphaneyle daha şık bir bildirim gösterebilirsiniz.
                    // toastr.success('Servis başarıyla sonlandırıldı.');
                } else {
                    // Eğer işlem başarılı değilse (örn: stok hatası), PHP'den gelen mesajı gösteriyoruz.
                    alert('Hata: ' + response.message);
                    // Butonu tekrar aktif hale getiriyoruz.
                    button.prop('disabled', false).html('<i class="fas fa-check-circle"></i> Sonlandır');
                }
            },
            error: function(xhr) {
                // Sunucu tarafında bir hata oluşursa (örn: 500 Internal Server Error) kullanıcıyı bilgilendiriyoruz.
                alert('Servis sonlandırılırken bir sunucu hatası oluştu.');
                console.error('AJAX Hatası:', xhr.responseText); // Hatanın detayını konsola yazdırıyoruz.
                 // Butonu tekrar aktif hale getiriyoruz.
                button.prop('disabled', false).html('<i class="fas fa-check-circle"></i> Sonlandır');
            }
        });
