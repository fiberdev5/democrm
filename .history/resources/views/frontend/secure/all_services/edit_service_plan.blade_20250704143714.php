    ...
    <div class="row form-group">
        ...
        <div class="col-lg-8">
            @php
                ...
                // DD 1: Check the raw value of $plan->cevap
                // dd($plan->cevap); 
                ...
                if ($plan->cevap) {
                    ...
                    // DD 2: Check the extracted $currentConsignmentId and $currentConsignmentQuantity
                    // dd(['currentConsignmentId' => $currentConsignmentId, 'currentConsignmentQuantity' => $currentConsignmentQuantity]);
                ...
                // DD 3: Check if the current ID is found in the detailedConsignmentStocks collection
                // dd(['currentConsignmentId' => $currentConsignmentId, 'detailedConsignmentStocks_contains' => $detailedConsignmentStocks->contains('id', $currentConsignmentId), 'detailedConsignmentStocks_firstWhere' => $detailedConsignmentStocks->firstWhere('id', $currentConsignmentId)]);
                // DD 4: Check the entire $detailedConsignmentStocks collection (use with caution, can be very large)
                // dd($detailedConsignmentStocks->pluck('id', 'urunAdi')); // Shows IDs and names for easier inspection
            @endphp
            ...
            <select id="consignment-device-select-{{ $plan->id }}" class="form-control consignment-device-select rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" data-plan-id="{{ $plan->id }}">
                ...
                @foreach($detailedConsignmentStocks as $device)
                    <option value="C_{{ $device->id }}"
                            ...
                            {{-- DD 5: Check the condition for 'selected' attribute for each option --}}
                            {{-- {{ dd(['option_id' => $device->id, 'currentConsignmentId' => $currentConsignmentId, 'is_selected' => ($currentConsignmentId == $device->id)]) }} --}}
