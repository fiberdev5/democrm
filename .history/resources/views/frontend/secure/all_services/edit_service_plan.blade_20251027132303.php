...
// Konsinye Cihaz checkbox değiştiğinde adet girişini göster/gizle ve maksimumu ayarla
    ...
    // Form Submit
    // Form Submit - YENİ VE BASİTLEŞTİRİLMİŞ VERSİYON
    $("#servisPlanGuncelle").on('submit', function(e) {
        e.preventDefault();

        // 1. FormData nesnesini formun mevcut durumundan oluştur.
        // Bu, işaretli checkbox'lar ve değerleri de dahil olmak üzere
        // tüm aktif form elemanlarını otomatik olarak alır.
        const formData = new FormData(this);

        // 2. İşaretlenmemiş checkbox'lara ait adet input'larının gönderilmesini engelle.
        // En temiz yol, göndermeden hemen önce bunları devre dışı bırakmaktır.
        // Devre dışı bırakılan (disabled) elemanlar FormData'ya dahil edilmez.
        $(this).find('.stock-checkbox:not(:checked)').closest('.stock-item').find('.quantity-input').prop('disabled', true);
        $(this).find('.consignment-checkbox:not(:checked)').closest('.stock-item').find('.consignment-quantity-input').prop('disabled', true);

        // 3. Form verisini, devre dışı bırakma işleminden sonra tekrar oluştur.
        // Bu, sadece gönderilmesi gereken verilerin dahil edilmesini sağlar.
        const finalFormData = new FormData(this);

        // 4. AJAX isteğinden sonra formun tekrar kullanılabilmesi için
        // devre dışı bıraktığımız input'ları tekrar etkinleştirelim.
        $(this).find('.quantity-input, .consignment-quantity-input').prop('disabled', false);


        $.ajax({
            url: $(this).attr('action'),
            type: "POST",
            data: finalFormData, // Sadece bu temizlenmiş veriyi gönder
            contentType: false,
            cache: false,
            processData: false,
            ...
            complete: function() {
                // Her ihtimale karşı, hata durumunda bile input'ları tekrar etkinleştir.
                $("#servisPlanGuncelle").find('.quantity-input, .consignment-quantity-input').prop('disabled', false);
            }
        });
    });
