...
<div class="servisModal teknisyenServisModal">
    <form method="POST" id="servisDuzenle" action="{{ route('servis.update', $servis->id) }}">
        @csrf
        @method('PUT')
                    ...
                    <div class="col-sm-12" style="text-align: left;">
                        <label style="text-align: left;width: auto;display: inline-block;margin: 0;">
                            ...
                            <span style="background: #ec0000;border: 1px solid #ce0000;color: #fff;padding: 0px 5px;border-radius: 3px;margin-left: 5px;max-width: 215px">
                                {{ $servis->servisKaynak->kaynak ?? 'Belirtilmemiş' }}
                            </span>
                        </label>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <span>
                            <a href="https://www.google.com/maps?daddr={{ $servis->musteri->adres }} {{ $servis->musteri->ilce }}/{{ $servis->musteri->il }}" style="color:red">
                                {{ $servis->musteri->adres }} {{ $servis->musteri->ilce }}/{{ $servis->musteri->il }}
                            </a>
                        </span>
                        ...
                        @if($konsinyeCihaz)
                            <span>
                                <label>Konsinye Cihaz: </label>
                                {{ $konsinyeCihaz->urunAdi }}
                            </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <span class="cihazName">
                            {{ strtoupper($servis->cihazMarka->marka . ' - ' . $servis->cihazTuru->cihaz . ' - ' . $servis->cihazAriza) }}
                        </span>
                        ...
                        <span style="margin:0">
                            ...
                            <input type="text" name="cihazModel" class="form-control cihazModel" 
                                   ...
                                   style="display:inline-block;width:calc(100% - 147px);padding:3px 5px;">
                                ...
                                @foreach($eskiIslemler as $eskiIslem)
                                    <tr>
                                        <td class="kayitTarihiCS" style="vertical-align: middle;width: 100px; font-size: 11px; padding: 0 5px;">
                                            {{ $eskiIslem->tarih->format('d/m/Y') }}<br>
                                            {{ $eskiIslem->tarih->format('H:i') }}
                                        </td>
                                        <td style="vertical-align: middle;font-size: 11px; padding: 0 5px;">
                                            {{ $eskiIslem->personel->adsoyad }}
                                        </td>
            ...
            <div class="card-header" style="padding: 3px 5px;">
                <div class="row">
                    <div class="col-sm-12" style="text-align: right;">
                        <button type="button" class="btn btn-warning btn-sm servisYaziKopyala">
                            Fiş Yazdır
                        </button>
                        <button type="button" class="btn btn-primary btn-sm servisGuncelleBtn">
                            Servis Güncelle
...
$(document).ready(function() {
    ...
    $('.servisGuncelleBtn').click(function() {
        ...
        $.ajax({
            url: '{{ route("servis.update", $servis->id) }}',
            type: 'PUT',
            data: formData,
    ...
    $('.altAsamalar').change(function() {
        ...
        if (selectedValue) {
            $.ajax({
                url: '{{ route("servis.addProcess", $servis->id) }}',
                type: 'POST',
                data: {
...
.servisAcilLabel input:checked + .checkmark {
    background: #007bff;
    border-color: #007bff;
    color: white;
}
