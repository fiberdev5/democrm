...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        // Tüm grup kodlarını parse et
        preg_match_all('/\[Grup-(\d+)\]/', $stage->cevapTuru, $matches);
        $grupKodlari = $matches[1] ?? [];
    @endphp

    @if($stage->id == 24 && in_array('4', $grupKodlari))
        @php
            // Sadece Teknisyen rolüne sahip kullanıcılar
    ...
    @if($stage->id == 25 && (in_array('4', $grupKodlari) || in_array('5', $grupKodlari)))
        @php
            // Teknisyen veya Teknisyen Yardımcısı rolüne sahip kullanıcılar
            $yardimcilar = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
                ->whereHas('roles', function($query) {
        ...
        <div class="form-group">
            <label>Yardımcı Teknisyen</label>
            <select class="form-control" name="soru[{{ $stage->id }}]">
                <option value="">-Seçiniz (Opsiyonel)-</option>
                @foreach($yardimcilar as $personel)
                    <option value="{{ $personel->user_id }}">{{ $personel->name }}</option>
                @endforeach
            </select>
        </div>
