...
$(document).ready(function() {
    ...
    // Form submit
    $('#servisEkle').submit(function(event) {
        // Eğer form zaten gönderiliyorsa engelle
        if (formSubmitting) {
            event.preventDefault();
            return false;
        }
        ...
        // Form gönderildikten sonra yeni token oluştur (sayfa yenilenmezse diye)
        setTimeout(function() {
            $('#formToken').val(generateToken());
            formSubmitting = false;
            $submitBtn.prop('disabled', false).val('Servis Kaydet');
            $('.gonderBtnWrap').show();
            $('.yukleniyor').hide();
        }, 3000);
