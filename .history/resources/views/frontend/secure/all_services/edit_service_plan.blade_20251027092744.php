...
// Konsinye Cihaz checkbox değiştiğinde adet girişini göster/gizle ve maksimumu ayarla
    ...
    $('.myParcaList').each(function() {
        ...
        // Backend'in beklediği formatta birleştir ve formData'ya ekle
        // Bu işlem, formdaki <input type="hidden" name="soru{ID}" value="Parca"> alanının değerini günceller.
        formData.set(`soru${soruId}`, parcaCevaplari.join(', '));
    });

    // 2. Konsinye Cihaz verisini hazırla
    $('.myKonsinyeList').each(function() {
        const konsinyeCevaplari = [];
        const soruId = $(this).closest('.form-group').find('input[type=hidden][value="Konsinye Cihaz"]').attr('name').replace('soru', '');
