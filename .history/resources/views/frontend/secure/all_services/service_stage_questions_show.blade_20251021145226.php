...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        ...
        $isMultipleGroups = str_contains($stage->cevapTuru, ',');
        
        // Teknisyen ve Teknisyen Yardımcısı kombinasyonu mu kontrol et
        $isTeknisyenCombo = str_contains($stage->cevapTuru, '[Grup-4]') && str_contains($stage->cevapTuru, '[Grup-5]');
    @endphp
    
    @if($isTeknisyenCombo)
        {{-- Teknisyen ve Teknisyen Yardımcısı için ayrı selectler --}}
        @php
            $teknisyenler = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
            ...
            $teknisyenYardimcilari = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
                ->whereHas('roles', function($query) {
                    $query->where('name', 'Teknisyen Yardımcısı');
                })
                ->orderBy('name', 'asc')
                ->get();
        @endphp
        ...
        <div class="row">
            <div class="col-md-6">
                <label style="font-size: 12px;">Teknisyen <span class="text-danger">*</span></label>
                @if($teknisyenler->count())
                    <select class="form-control" name="soru[{{ $stage->id }}][teknisyen]" required>
                ...
                @else
                    <p class="text-danger">Teknisyen bulunamadı.</p>
                @endif
            </div>
            
            <div class="col-md-6">
                <label style="font-size: 12px;">Teknisyen Yardımcısı <small class="text-muted">(Opsiyonel)</small></label>
                @if($teknisyenYardimcilari->count())
                    <select class="form-control" name="soru[{{ $stage->id }}][yardimci]">
                        <option value="">-Seçiniz-</option>
                        @foreach($teknisyenYardimcilari as $personel)
                            <option value="{{ $personel->user_id }}">{{ $personel->name }}</option>
                        @endforeach
                    </select>
                @else
                    <p class="text-muted">Teknisyen yardımcısı bulunamadı.</p>
                @endif
            </div>
        </div>
    @else
        {{-- Tek grup için mevcut kod (değişmeden kalacak) --}}
        @php
