...
$(document).ready(function() {
    ...
    $(document).on('change', '.consignment-device-select', function() {
        ...
        const consignmentNameInput = $(this).closest('.col-lg-12').find('.consignment-device-name-input'); // Eklenen hidden input
    ...
    $('#servisPlanKaydet').on('submit', function(e) {
        ...
        // Bu kısım, checkbox'ların name="" özelliği "parca[stage_id][stock_id]" olduğu için
        // zaten doğrudan FormData'ya eklenecektir. Bu döngüye gerek kalmayabilir
        // ancak emin olmak için burada tutabiliriz. Laravel'in checkbox değerini
        // sadece işaretliyse göndermesi yaygın bir davranıştır.
        ...
        // Sadece gizli inputların ve selectin isimlerinin doğru olduğundan emin olmalıyız.
        $('.consignment-device-select').each(function() {
            ...
            if (selectedValue) { // Eğer bir cihaz seçildiyse
                ...
                var productName = $(this).find('option:selected').data('product-name'); // Cihaz adını al

                // `parca[stage_id][consignment_select]` zaten select elementinin adı, bu doğrudan gönderilir.
                // Sadece hidden inputlar için manuel ekleme gerekebilir.
                // Eğer Blade tarafındaki hidden input'un adı `parca[{{ $stage->id }}][consignment_name]` ise,
                // bu da otomatik gönderilir. Sadece `adet` alanı için manuel eklemeyi kontrol edelim.

                // Önceki stok kontrolü için kullandığımız isimle uyumlu hale getirelim:
                // `parca[{{ $stage->id }}][consignment_id]` ve `adet[{{ $stage->id }}][selected_consignment_quantity]`
                formData.append('parca[' + stageId + '][consignment_id]', selectedValue.replace('C_', '')); // Sadece ID'yi gönder
                formData.append('adet[' + stageId + '][selected_consignment_quantity]', quantity); // Adet
            }
        });
