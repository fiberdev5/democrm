...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        ...
        // Teknisyen Yardımcısı seçimi için personeller (hem teknisyen hem yardımcı)
    ...
    {{-- Teknisyen Yardımcısı Seçimi (Opsiyonel) --}}
    <div class="form-group mb-3">
    ...
    {{-- Personel bulunamadığı durumlar için uyarılar --}}
    @if($teknisyenler->isEmpty())
        <div class="alert alert-warning">Teknisyen rolüne sahip personel bulunamadı.</div>
    @endif
    
    @if($teknisyenYardimcilari->isEmpty())
        <div class="alert alert-warning">Teknisyen veya Teknisyen Yardımcısı rolüne sahip personel bulunamadı.</div>
    @endif
