...
<script>
    $(document).ready(function(e) {
        ...
        $("#servisPlanGuncelle").on('submit', function(e) {
            e.preventDefault();
// Konsinye cihaz verilerini submit öncesi hazırla
$('.consignment-device-select').each(function () {
    ...
    var nameInput = $(this).closest('.col-lg-12, .col-lg-8').find('.consignment-device-name-input'); // ikisini de kapsar

    // Formatı düzelt: örnek => C_53---2
    if (selectedValue) {
        selectedValue = String(selectedValue).trim();

        if (!selectedValue.startsWith('C_')) {
            // Eksik prefix varsa düzelt
            selectedValue = 'C_' + selectedValue.replace(/^C?_?/, '');
        }

        // Gizli inputlara değerleri ata
        hiddenInput.val(selectedValue + '---' + quantity);
        nameInput.val(productName); // konsinye cihaz adı
    } else {
        hiddenInput.val('');
        nameInput.val('');
    }
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('change', '.consignment-device-select', function() {
            ...
            var selectedValue = selectedOption.val(); // e.g., C_53
            ...
            if (selectedValue) {
                ...
                quantityInput.val(1); // Cihaz seçildiğinde adeti 1 olarak ayarla
                availableQuantityText.text('(Mevcut: ' + availableQuantity + ')');

                // Hidden input'u hemen güncelle
                hiddenInput.val(selectedValue + '---' + quantityInput.val());
            } else {
                // Cihaz seçimi kaldırıldı
                quantityGroup.slideUp();
                quantityInput.val(0); // Adeti sıfırla
                availableQuantityText.text('(Mevcut: 0)');
                hiddenInput.val(''); // Hidden input'u temizle
            }
        });
        ...
        $(document).on('change keyup', '.quantity-input', function() {
            ...
            var selectedValue = dropdown.val(); // C_ID formatında olmalı
            var quantity = parseInt($(this).val()); // Tam sayıya çevir
            var max = parseInt($(this).attr('max'));

            // Adet kontrolü ve düzeltme
            ...
            if (selectedValue) {
                ...
                // Eğer dropdown'da bir seçim yoksa hidden input'u da temizle
                $('#consignment-combined-value-' + planId).val('');
            }
        });

        // Sayfa yüklendiğinde mevcut seçimleri ve adetleri kontrol et ve gizli inputu güncelle
        // Bu kısım, mevcut kayıt düzenlenirken adet girişinin doğru görünmesini sağlar
        $('.consignment-device-select').each(function() {
            var planId = $(this).data('plan-id');
            var selectedOption = $(this).find('option:selected');
            var quantityGroup = $(this).closest('.col-lg-8').find('.consignment-quantity-group');
            ...
            if (selectedOption.val()) { // Eğer bir cihaz seçili ise
                var availableQuantity = selectedOption.data('available');
                var currentQuantity = parseInt(quantityInput.val());

                quantityGroup.show(); // Adet girişini göster
                quantityInput.attr('max', availableQuantity); // Max değeri ayarla
                availableQuantityText.text('(Mevcut: ' + availableQuantity + ')');

                // Mevcut adet, mevcut stoktan fazlaysa veya 1'den azsa düzelt
                ...
                // Sayfa yüklendiğinde hidden input'un doğru değeri içermesini sağla
                hiddenInput.val(selectedOption.val() + '---' + quantityInput.val());

            } else {
                quantityGroup.hide(); // Seçili cihaz yoksa gizle
                hiddenInput.val(''); // Seçili cihaz yoksa hidden inputu temizle
            }
        });
    });
</script>