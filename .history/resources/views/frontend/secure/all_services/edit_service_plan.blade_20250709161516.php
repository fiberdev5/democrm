...
// Konsinye cihaz: Önce tüm konsinye input'larını FormData'dan temizle
for (var pair of formData.entries()) {
    if (pair[0].startsWith('konsinye_cihaz') || pair[0].startsWith('konsinye_adet')) {
...
$('.consignment-checkbox:checked').each(function() {
    ...
    const matchResult = nameAttr.match(/konsinye_cihaz\[(\d+)\]\[(\d+)\]/);

    if (matchResult && matchResult[1] && matchResult[2]) {
        const stageId = matchResult[1];
        const konsinyeId = matchResult[2];
        const quantityInput = $(this).closest('.stock-item').find('.consignment-quantity-input');
        const quantity = quantityInput.val();

        formData.append(`konsinye_cihaz[${stageId}][${konsinyeId}]`, konsinyeId);
        formData.append(`konsinye_adet[${stageId}][${konsinyeId}]`, quantity);
    }
});
