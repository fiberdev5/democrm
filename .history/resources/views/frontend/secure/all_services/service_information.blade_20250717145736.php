  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="surveyModalLabel">Müşteri Memnuniyet Anketi</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Kapat"></button>
      </div>
      <div class="modal-body">
        <div id="surveyModalContent">Yükleniyor...</div>
      </div>
    </div>
  </div>
</div>
...
$('.servisMusteriAnketiBtn').on('click', function(e) {
    e.preventDefault();
    var id = $(this).data('id'); // servis ID
    var firma_id = {{ $firma->id }};
    
    $('#surveyModal').modal('show');
    $('#surveyModalContent').html("Yükleniyor...");

    $.ajax({
        url: '/' + firma_id + '/anket-formu/' + id,
        type: 'GET',
        success: function(response) {
            $('#surveyModalContent').html(response);
        },
        error: function() {
            $('#surveyModalContent').html("Anket yüklenirken hata oluştu.");
        }
    });
});
