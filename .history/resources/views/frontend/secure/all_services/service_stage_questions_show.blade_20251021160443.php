...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        // Cevap türündeki tüm grup kodlarını çek
        preg_match_all('/\[Grup-(\d+)\]/', $stage->cevapTuru, $matches);
        $grupKodlari = $matches[1] ?? [];

        $teknisyenRolleri = ['Teknisyen']; // Grup-4 için
        $teknisyenYardimcisiRolleri = ['Teknisyen Yardımcısı']; // Grup-5 için (eğer böyle bir rolünüz varsa)

        $personellerTeknisyen = collect();
        $personellerTeknisyenYardimcisi = collect();

        // Eğer grup kodları arasında 4 varsa, Teknisyenleri çek
        if (in_array('4', $grupKodlari)) {
            $personellerTeknisyen = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
                ->whereHas('roles', function($query) use ($teknisyenRolleri) {
                    $query->whereIn('name', $teknisyenRolleri);
                })
                ->orderBy('name', 'asc')
                ->get();
        }

        // Eğer grup kodları arasında 5 varsa, Teknisyen Yardımcılarını çek
        if (in_array('5', $grupKodlari)) {
            $personellerTeknisyenYardimcisi = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
                ->whereHas('roles', function($query) use ($teknisyenYardimcisiRolleri) {
                    $query->whereIn('name', $teknisyenYardimcisiRolleri);
                })
                ->orderBy('name', 'asc')
                ->get();
        }

        // Genel Grup kodları için (örneğin Atölye Ustası)
        $digerGrupKodlari = array_diff($grupKodlari, ['4', '5']);
        $personellerDigerGrup = collect();

        if (!empty($digerGrupKodlari)) {
            $rollerDigerGrup = [];
            if (in_array('261', $digerGrupKodlari) || in_array('262', $digerGrupKodlari)) {
                $rollerDigerGrup[] = 'Atölye Ustası';
            }
            // Diğer grup kodlarına göre rolleri buraya ekleyebilirsiniz.

            if (!empty($rollerDigerGrup)) {
                $personellerDigerGrup = App\Models\User::where('tenant_id', $firma->id)
                    ->where('status', '1')
                    ->whereHas('roles', function($query) use ($rollerDigerGrup) {
                        $query->whereIn('name', $rollerDigerGrup);
                    })
                    ->orderBy('name', 'asc')
                    ->get();
            }
    ...
    @if($personellerTeknisyen->count())
        <div class="form-group mb-3">
            <label for="teknisyen_soru_{{ $stage->id }}">Teknisyen Seçin:</label>
            <select class="form-control" name="soru[{{ $stage->id }}][teknisyen]" id="teknisyen_soru_{{ $stage->id }}" @if(in_array('4', $grupKodlari)) required @endif>
                <option value="">-Seçiniz-</option>
                @foreach($personellerTeknisyen as $personel)
    ...
    @if($personellerTeknisyenYardimcisi->count())
        <div class="form-group mb-3">
            <label for="teknisyen_yardimcisi_soru_{{ $stage->id }}">Teknisyen Yardımcısı Seçin:</label>
            <select class="form-control" name="soru[{{ $stage->id }}][teknisyen_yardimcisi]" id="teknisyen_yardimcisi_soru_{{ $stage->id }}" @if(in_array('5', $grupKodlari)) required @endif>
                <option value="">-Seçiniz-</option>
                @foreach($personellerTeknisyenYardimcisi as $personel)
                    <option value="{{ $personel->user_id }}">{{ $personel->name }}</option>
                @endforeach
            </select>
        </div>
    ...
    @if($personellerDigerGrup->count())
        <div class="form-group mb-3">
            <label for="diger_grup_soru_{{ $stage->id }}">Diğer Personel Seçin:</label>
            <select class="form-control" name="soru[{{ $stage->id }}][diger_grup]" id="diger_grup_soru_{{ $stage->id }}">
                <option value="">-Seçiniz-</option>
                @foreach($personellerDigerGrup as $personel)
                    <option value="{{ $personel->user_id }}">{{ $personel->name }}</option>
                @endforeach
            </select>
        </div>
    ...
    @if(!$personellerTeknisyen->count() && !$personellerTeknisyenYardimcisi->count() && !$personellerDigerGrup->count())
        <p>Bu gruba/gruplara ait personel bulunamadı.</p>
    @endif
