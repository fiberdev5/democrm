...
<div class="servisAsamalari">
  <div class="card card3">
    ...
    <span>Acil</span>
    <input type="checkbox" style="display: none;" {{$service_id->acil == 1 ? 'checked' : ''}}>
    <div class="checkmark"><i class="fas fa-check"></i></div>
</label>
<input type="hidden" name="acil" class="acil" value="{{$service_id->acil == 1 ? '1' : '0'}}"/>

<!-- Form içinde servis ID'yi ekleyin (eğer yoksa) -->
...
<script type="text/javascript">
$(document).ready(function() {
    // Sayfa yüklendiğinde acil durumunu kontrol et ve görünümü ayarla
    function updateAcilButton() {
        var checkbox = $('.servisAcilBtn input[type="checkbox"]');
        var checkmark = $('.servisAcilBtn .checkmark');
        
        if (checkbox.is(':checked')) {
            checkmark.css('opacity', '1');
            $('.servisAcilBtn').css('background', '#dc3545'); // Kırmızı
        } else {
            checkmark.css('opacity', '1');
            $('.servisAcilBtn').css('background', '#343a40'); // Gri
        }
    }
    ...
    $(".servisAcilBtn").on('click', function(e) {
        ...
        console.log('Acil yeni değer:', hiddenInput.val());
    });
    ...
    $(".servisGuncelleBtn").click(function(e) {
        ...
        // Submit etmeden önce acil değerini kontrol et
        var acilValue = $('.servisAcilBtn input[type="checkbox"]').is(':checked') ? '1' : '0';
        $('.acil').val(acilValue);
    ...
    $("#servisDuzenle").on('submit', function(e) {
        ...
        console.log('Acil değeri:', formData.get('acil'));
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                if (xhr.status === 422) {
                    var errors = xhr.responseJSON.messages || xhr.responseJSON.errors;
                    if (errors) {
