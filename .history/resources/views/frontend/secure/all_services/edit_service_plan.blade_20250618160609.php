@extends('layouts.app')

@section('content')
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <form method="POST" id="servisPlanGuncelle" action="{{ route('service-plan.update') }}">
                @csrf
                <input type="hidden" name="planid" value="{{ $servisPlan->id }}">
                <input type="hidden" name="tenant_id" value="{{ $tenant_id }}">

                {{-- İşlemi Yapan Personel Seçimi --}}
                @if($kullanici->grup == "242" || $kullanici->yetki == "1")
                <div class="row form-group">
                    <div class="col-lg-4">
                        <label>İşlemi Yapan</label>
                    </div>
                    <div class="col-lg-8">
                        <select name="planIslemiYapan" class="form-control planIslemiYapan">
                            @foreach($personellerAll as $personel)
                                <option value="{{ $personel->id }}" 
                                    {{ $personel->id == $servisPlan->pid ? 'selected' : '' }}>
                                    {{ $personel->adsoyad }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                </div>
                @endif

                {{-- Plan Cevapları --}}
                @foreach($planCevaplar as $plan)
                    @php
                        $soru = App\Models\ServisAsamaSoru::find($plan->soruid);
                    @endphp

                    @if($soru->cevap == "[Parca]")
                        {{-- Parça Seçimi --}}
                        <div class="row form-group">
                            <div class="col-lg-12">
                                @if($servisPlan->gidenIslem == "259")
                                    {{-- Depo Parça Listesi --}}
                                    @include('partials.depo_parca_listesi', [
                                        'stoklar' => $stoklar,
                                        'plan' => $plan,
                                        'tenant_id' => $tenant_id
                                    ])
                                @else
                                    {{-- Uyumlu Parça Listesi --}}
                                    @include('partials.uyumlu_parca_listesi', [
                                        'servis' => $servis,
                                        'plan' => $plan,
                                        'tenant_id' => $tenant_id
                                    ])
                                @endif
                            </div>
                        </div>
                    @else
                        {{-- Diğer Soru Tipleri --}}
                        <div class="row form-group">
                            <div class="col-lg-4">
                                <label>{{ $soru->soru }}</label>
                            </div>
                            <div class="col-lg-8">
                                @if($soru->cevap == "[Aciklama]")
                                    <input type="text" name="soru{{ $plan->id }}" 
                                           class="form-control" value="{{ $plan->cevap }}">

                                @elseif(strpos($soru->cevap, 'Grup') !== false)
                                    {{-- Grup Seçimi --}}
                                    @if(strpos($soru->cevap, 'Grup-0') !== false)
                                        <select class="form-control" name="soru{{ $plan->id }}">
                                            @foreach($personellerAll as $personel)
                                                <option value="{{ $personel->id }}" 
                                                    {{ $plan->cevap == $personel->id ? 'selected' : '' }}>
                                                    {{ $personel->adsoyad }}
                                                </option>
                                            @endforeach
                                        </select>
                                    @else
                                        {{-- Belirli Grup Personelleri --}}
                                        @php
                                            $gruplar = [];
                                            $soruGruplar = explode(', ', $soru->cevap);
                                            foreach($soruGruplar as $grup) {
                                                $grupId = substr(explode('-', $grup)[1], 0, -1);
                                                $gruplar[] = $grupId;
                                            }
                                            $grupPersoneller = App\Models\Personel::where('kid', $tenant_id)
                                                ->where('yetki', '0')
                                                ->where('durum', '1')
                                                ->whereIn('grup', $gruplar)
                                                ->orderBy('adsoyad', 'ASC')
                                                ->get();
                                        @endphp
                                        <select class="form-control" name="soru{{ $plan->id }}">
                                            <option value="">-Seçiniz-</option>
                                            @foreach($grupPersoneller as $personel)
                                                <option value="{{ $personel->id }}" 
                                                    {{ $plan->cevap == $personel->id ? 'selected' : '' }}>
                                                    {{ $personel->adsoyad }}
                                                </option>
                                            @endforeach
                                        </select>
                                    @endif

                                @elseif($soru->cevap == "[Tarih]")
                                    <input type="text" name="soru{{ $plan->id }}" 
                                           class="form-control datepicker" readonly 
                                           value="{{ $plan->cevap }}" style="background:#fff;">

                                @elseif($soru->cevap == "[Saat]")
                                    <select class="form-control" name="soru{{ $plan->id }}">
                                        @php
                                            $saatler = [
                                                '08:00-10:00', '09:00-11:00', '10:00-12:00', '11:00-13:00',
                                                '12:00-14:00', '13:00-15:00', '14:00-16:00', '15:00-17:00',
                                                '16:00-18:00', '17:00-19:00', '18:00-20:00', '19:00-21:00',
                                                '20:00-22:00', '21:00-23:00'
                                            ];
                                        @endphp
                                        @foreach($saatler as $saat)
                                            <option value="{{ $saat }}" 
                                                {{ $plan->cevap == $saat ? 'selected' : '' }}>
                                                {{ $saat }}
                                            </option>
                                        @endforeach
                                    </select>

                                @elseif($soru->cevap == "[Arac]")
                                    @php
                                        $araclar = App\Models\ServisArac::where('kid', $tenant_id)
                                            ->where('durum', '1')
                                            ->orderBy('id', 'ASC')
                                            ->get();
                                    @endphp
                                    <select class="form-control" name="soru{{ $plan->id }}">
                                        @foreach($araclar as $arac)
                                            <option value="{{ $arac->id }}" 
                                                {{ $plan->cevap == $arac->id ? 'selected' : '' }}>
                                                {{ $arac->arac }}
                                            </option>
                                        @endforeach
                                    </select>

                                @elseif($soru->cevap == "[Fiyat]")
                                    <input type="number" name="soru{{ $plan->id }}" 
                                           class="form-control" value="{{ $plan->cevap }}">

                                @elseif($soru->cevap == "[Teklif]")
                                    <input type="number" name="soru{{ $plan->id }}" 
                                           class="form-control" value="{{ $plan->cevap }}">
                                    <span style="font-size: 12px; color: red; font-weight: 500; margin: 0; padding: 0; display: block;">
                                        Bu alan sadece teklif vermek için kullanılır.
                                    </span>

                                @elseif($soru->cevap == "[Bayi]")
                                    @php
                                        $bayiler = App\Models\Personel::where('grup', 258)
                                            ->where('durum', '1')
                                            ->where('kid', $tenant_id)
                                            ->orderBy('id', 'ASC')
                                            ->get();
                                    @endphp
                                    <select class="form-control" name="soru{{ $plan->id }}">
                                        @foreach($bayiler as $bayi)
                                            <option value="{{ $bayi->id }}" 
                                                {{ $plan->cevap == $bayi->id ? 'selected' : '' }}>
                                                {{ $bayi->adsoyad }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="eskiBayi" value="{{ $plan->cevap }}">
                                @endif
                            </div>
                        </div>
                    @endif
                @endforeach

                {{-- Form Butonları --}}
                <div class="row">
                    <div class="col-lg-12" style="text-align: center; margin-bottom: 0px; margin-top: 5px;">
                        <input type="submit" class="btn btn-primary btn-sm" value="Güncelle">
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>
$(document).ready(function(e) {
    // Datepicker
    $('.datepicker').datepicker({
        language: 'tr',
        autoclose: true,
    });

    // Form Submit
    $("#servisPlanGuncelle").on('submit', function(e) {
        e.preventDefault();
        
        $.ajax({
            url: $(this).attr('action'),
            type: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
            processData: false,
            success: function(data) {
                if(data.error) {
                    alert(data.error);
                } else {
                    alert('Plan başarıyla güncellendi');
                    $('#servisPlanDuzenleModal').modal('hide');
                    $('.nav1').trigger('click');
                    if(typeof(servislerTable) != "undefined" && servislerTable !== null) {
                        servislerTable.draw();
                    }
                    if(typeof(raporlarTable) != "undefined" && raporlarTable !== null) {
                        raporlarTable.draw();
                    }
                }
            },
            error: function(e) {
                alert("Hata: " + e.responseText);
            }
        });
    });

    // Parça arama
    $('#urunAraInput').keyup(function() {
        var value = $(this).val().toLowerCase();
        $(".myParcaList .checkbox").filter(function() {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
    });

    // Dropdown tıklama engelleme
    $(document).on('click', '.parcalar-dropdown', function(e) {
        e.stopPropagation();
    });
});
</script>
@endpush