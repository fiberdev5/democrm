...
<form method="post" id="servisPlanKaydet" action="{{ route('save.service.plan', $firma->id) }}" class="col-sm-6" style="margin: 0 auto;padding:10px;">
    ...
    @foreach($stage_questions as $stage)
                            ...
                            @if($stokSec && $stokAdet > 0)
                                ...
                                <div class="checkbox" style="padding:3px 0;" data-product-code="{{ $stokSec->urunKodu ?? '' }}">
                                    <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                                        <input type="checkbox" name="personelStokCheck[{{ $stokId }}]" value="on" style="position: relative; top:2px; margin-right:3px;">
                                        {{ $stokSec->urunAdi ?? $stokSec->urun_adi ?? 'Ürün Adı Bulunamadı' }} (Mevcut: {{ $stokAdet }})
                                    </label>
                                    <input type="number" name="personelStokAdet[{{ $stokId }}]" value="1" min="1" max="{{ $stokAdet }}" class="form-control" autocomplete="off" style="width: 40px;display: inline-block;text-align:center;">
                                </div>
                            @endif
                        @empty
                        @endforelse
        ...
        @elseif($stage->cevapTuru == "[KonsinyeCihaz]") {{-- BURADA ELSEIF KULLANIYORUZ --}}
            <div class="row form-group">
                <div class="col-lg-12">
                    <label>{{ $stage->soru }}</label>
                    <input id="konsinyeCihazAraInput" type="text" class="form-control" data-id="" autocomplete="off" autofocus="on" placeholder="Cihaz adı veya kodu">
                    <div class="parcalar-dropdown myKonsinyeCihazList" style="width:100%">
                        @php $say = 0; @endphp

                        <p>Toplam Konsinye Cihaz Stoğu: {{ $toplamKonsinyeCihazAdedi }}</p>

                        @forelse($konsinyeCihazlar as $cihaz)
                            @php
                                $cihazId = $cihaz->id;
                                // Bu konsinye cihaz için kalan stoğu hesapla
                                $girisAdet = App\Models\StockAction::where('stokId', $cihazId)
                                                                    ->whereIn('islem', [1, 4])
                                                                    ->sum('adet');
                                $cikisAdet = App\Models\StockAction::where('stokId', $cihazId)
                            ...
                            @if($kalanStok > 0)
                                @php $say++; @endphp
                                <div class="checkbox" style="padding:3px 0;" data-product-code="{{ $cihaz->urunKodu ?? '' }}">
                                    <label style="width: calc(100% - 40px);display: inline-block;text-transform: capitalize;">
                                        <input type="checkbox" name="selectedConsignmentDevices[{{ $cihazId }}]" value="{{ $cihazId }}" style="position: relative; top:2px; margin-right:3px;">
                                        {{ $cihaz->urunAdi ?? 'Cihaz Adı Bulunamadı' }} (Mevcut: {{ $kalanStok }})
                                    </label>
                                    <input type="number" name="consignmentDeviceQuantities[{{ $cihazId }}]" value="1" min="1" max="{{ $kalanStok }}" class="form-control" autocomplete="off" style="width: 40px;display: inline-block;text-align:center;">
                                </div>
                            @endif
                        @empty
                            {{-- <label style="color:red">Uyumlu Konsinye Cihaz Bulunamadı veya Stokta Yok.</label> --}}
                        @endforelse
                ...
                <input type="hidden" name="soru[{{ $stage->id }}]" class="form-control" value="KonsinyeCihaz"/>
            </div>
        @else {{-- Diğer cevap türleri için else bloğu --}}
            <div class="row form-group">
        ...
        @endif {{-- Her bir @if için @endif veya @elseif/@else kapanışı. Bu, ana @if'i kapatır. --}}
    @endforeach {{-- @foreach($stage_questions as $stage) kapanışı --}}
    <div class="row">
...
$(document).ready(function() {
    ...
    $('#urunAraInput').on('keyup', function() {
        ...
        $('.myParcaList .checkbox').each(function() {
            ...
            if (productCode !== undefined && productCode !== null) {
                productCode = String(productCode).toLowerCase(); // Önce string'e çevir, sonra küçük harf yap
            } else {
                productCode = ''; // Eğer tanımsız veya null ise boş string yap
            }
    ...
    // Konsinye cihazlar için arama
    $('#konsinyeCihazAraInput').on('keyup', function() {
        var searchText = $(this).val().toLowerCase();
        $('.myKonsinyeCihazList .checkbox').each(function() {
            var productName = $(this).find('label').text().toLowerCase();
            var productCode = $(this).data('product-code');
            if (productCode !== undefined && productCode !== null) {
