...
<script>
  $(document).ready(function () {
    ...
    // Mevcut 2. sütun içeriğini al (1. indexteki sütun)
    const mevcutIcerik = $('td:eq(2)', row).html();

    // Müşteri adı (örneğin data[1] sütununda)
    const musteriAdi = data[2];

    // Acil durumu (örneğin data[8] gibi bir sütunda tutuluyor olabilir)
    const acilDurumu = data[2]; // doğru index’i sen belirlemelisin

    // Eğer acil ise gif ekle
    const acilGif = (acilDurumu == 1)
        ? '<img src="{{asset('frontend/img/alarm.gif')}}" style="width:18px; vertical-align:middle; margin-right:4px;"> '
        : '';

    // Yeni içerik: gif + müşteri adı + önceki içerik
    const yeniIcerik = acilGif + '<strong>' + musteriAdi + ':</strong> ' + mevcutIcerik;
    $('td:eq(2)', row).html(yeniIcerik);

    // Duruma göre arka plan rengi
    const renkHaritasi = {
        'Şikayetçi': '#e96464',
        'Yeni Servisler': '#43a1e7',
        'Tamamlandı': '#66bb6a',
        'İptal': '#e96464',
        'Güncellendi': '#ffa726'
    };
