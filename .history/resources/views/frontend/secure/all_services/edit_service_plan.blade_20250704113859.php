<div class="container">
 ...
 @elseif($soru->cevapTuru == "[Konsinye Cihaz]")
        {{-- Yeni Konsinye Cihaz Düzenleme Bölümü --}}
        <div class="row form-group">
            <div class="col-lg-4">
                <label>{{ $soru->soru }}</label>
            </div>
            <div class="col-lg-8">
                @php
                    ...
                    if ($plan->cevap) {
                        list($currentConsignmentId, $currentConsignmentQuantity) = array_pad(explode('---', $plan->cevap), 2, 0);
                        // C_ ön ekini kaldır
                        $currentConsignmentId = str_replace('C_', '', $currentConsignmentId);
                    }
                @endphp
                ...
                <select name="soru{{ $plan->id }}" class="form-control consignment-device-select" data-plan-id="{{ $plan->id }}">
                    ...
                    @foreach($consignmentDevices as $device)
                        @if($device->current_stock_quantity > 0 || $device->id == $currentConsignmentId) {{-- Mevcut cihaz seçiliyse de göster --}}
                ...
                <div class="form-group consignment-quantity-group" style="margin-top: 10px; {{ $currentConsignmentId ? 'display:block;' : 'display:none;' }}">
                    ...
                    <input type="number"
                           ...
                           min="1"
                           max="{{ $currentConsignmentId ? (App\Models\ConsignmentStock::where('stock_id', $currentConsignmentId)->sum('quantity')) : 1 }}" {{-- Max değerini dinamik olarak ayarla --}}
                           class="form-control quantity-input"
                           autocomplete="off"
                           style="width: 80px; display:inline-block;">
                    <span class="text-muted available-quantity-text">(Mevcut: {{ $currentConsignmentId ? (App\Models\ConsignmentStock::where('stock_id', $currentConsignmentId)->sum('quantity')) : 0 }})</span>
                </div>
...
$(document).ready(function(e) {
    ...
    $(document).on('change', '.consignment-device-select', function() {
        ...
        if (selectedOption.val()) {
            ...
            quantityInput.val(1); // Reset to 1 when a new device is selected
    ...
    // Initial check for consignment device on page load
