...
// 1. Parça (personel stok)
$('.stock-checkbox:checked').each(function() {
    // Checkbox'ın parent'ından (stock-item) daha üst seviyede olan .form-group'a git.
    // Sonra bu form-group içindeki 'name^="soru["' inputunu bul.
    const stageIdInput = $(this).closest('.form-group').find('input[name^="soru["]');
    
    let stageId = null;
    if (stageIdInput.length > 0) {
        const nameAttr = stageIdInput.attr('name');
    ...
    if (!stageId) {
        console.error("Hata: Parça için stageId bulunamadı. Hidden input eksik veya yanlış yolda.", this);
...
$('.consignment-checkbox:checked').each(function() {
    // Konsinye checkbox'ının adından doğrudan stageId'yi al.
    // Bu daha güvenli ve doğrudan bir yöntemdir.
    const nameAttr = $(this).attr('name');
    const match = nameAttr ? nameAttr.match(/konsinye_cihaz\[(\d+)\]/) : null;
    let stageId = null;
    if (match && match[1]) {
        stageId = match[1];
    } else {
        console.error("Hata: Konsinye için stageId bulunamadı. Checkbox name özelliği yanlış.", this);
        return true; // continue to next iteration
    }
