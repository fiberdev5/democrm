...
<div class="row form-group" style="border: 0;margin-bottom:0;">
  ...
  <div class="col-md-8 rw2">
    @php
        // Bu satırı ekleyerek değişkenin içeriğini kontrol edin
        dd($seciliKonsinyeCihazlar); 
    @endphp
      @if(!empty($seciliKonsinyeCihazlar) && count($seciliKonsinyeCihazlar) > 0)
          @foreach($seciliKonsinyeCihazlar as $konsinyeId => $adet)
              ...
              @if($urun)
                  <div style="padding: 5px 0;">
                      <strong>{{ $urun->urunAdi ?? $urun->urun_adi ?? 'Bilinmeyen Cihaz' }}</strong> - Adet: {{ $adet }}
                  </div>
              @endif
          @endforeach
      @else
          <span style="color:red;">Konsinye cihaz atanmadı.</span>
      @endif
  </div>
</div>
