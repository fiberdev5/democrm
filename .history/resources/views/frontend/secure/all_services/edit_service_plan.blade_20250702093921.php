...
@foreach($planCevaplar as $plan)
    ...
    @if($soru->cevapTuru == "[Parca]")
        {{-- Parça Seçimi için Yenilenmiş Alan --}}
        <div class="row form-group">
            <div class="col-lg-12">
                <label>{{ $soru->soru }}</label>
                {{-- Arama inputu, myParcaList div'i için unique bir ID ekliyoruz --}}
        ...
        $stokId = $stok->id; // Stock modelinin kendi ID'si
        $stokAdet = $personelStokKaydi ? ($personelStokKaydi->adet ?? 0) : 0; // PersonelStock'tan gelen adet

        $stokSec = $stok; // Zaten elimizdeki $stok objesi aslında $stokSec'in yerine geçebilir
                          // Yani Stock::where... tekrar çağırmamıza gerek yok.

                            // Mevcut plan cevabında bu stok seçili mi kontrol et
                            $isChecked = false;
                            $selectedAdet = 1;
                            if ($plan->cevap) {
                                $cevaplarArray = explode(', ', $plan->cevap);
                                foreach ($cevaplarArray as $cevapItem) {
                                    list($itemStokId, $itemAdet) = explode('---', $cevapItem);
    ...
    @php
        ...
        dd([
            'Current Stock Object (from Controller)' => $stok->toArray(),
            'Related PersonelStock Record' => $personelStokKaydi ? $personelStokKaydi->toArray() : 'NOT FOUND',
            '$stokId (Stock ID)' => $stokId,
            '$stokAdet (Quantity from PersonelStock)' => $stokAdet,
            '$stokSec (Main Stock Object)' => $stokSec ? $stokSec->toArray() : 'NOT FOUND',
            '$plan->cevap (Current Answer String)' => $plan->cevap,
            '$isChecked' => $isChecked,
    ...
    @else
        {{-- Diğer Soru Tipleri (Değişiklik yok) --}}
        <div class="row form-group">
            <div class="col-lg-4">
                <label>{{ $soru->soru }}</label>
            </div>
...
$(document).ready(function(e) {
            ...
            success: function(data) {
                if(data.error) {
                    ...
                    if(typeof loadServiceHistory === 'function' && data.servis_id) {
                        ...
                        // Eğer loadServiceHistory fonksiyonu tanımlı değilse veya servis_id yoksa,
                        // hata mesajı yerine daha genel bir başarı mesajı gösterebiliriz.
    ...
    $('.urunAraInput').keyup(function() {
        ...
        myParcaList.find(".checkbox").filter(function() {
