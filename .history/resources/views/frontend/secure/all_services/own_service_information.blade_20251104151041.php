   ...
   data.eskiIslemler.forEach(function(islem) {
    if (islem.type === 'para') {
        ...
        if (islem.pid == currentUserId) {
            ...
            buttons += `<a href="#" id="servisPlanSil" class="btn btn-outline-danger btn-sm btn-sm-custom servisPlanSil" data-id="${islem.id}" title="Sil"><i style="line-height: 1.2;" class="fas fa-trash-alt"></i></a>`;
            buttons += '</td><td class="btnCS" style="vertical-align: middle;width: 70px;padding: 0 5px;">';
            buttons += `<a style="color:#e39d23 " href="#" data-bs-id="${islem.id}" class="btn btn-outline-warning btn-sm btn-sm-custom servisPlanDuzenleBtn" title="Düzenle"><i class="fas fa-edit"></i></a>`;
            buttons += '</td>';
        } else {
            buttons += `
