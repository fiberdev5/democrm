<div class="container">
  <div class="row">
    ...
    {{-- Konsinye Cihaz Düzenleme Bölümü --}}
    <div class="row form-group">
        <div class="col-lg-4">
            <label>{{ $soru->soru }}</label>
        </div>
        <div class="col-lg-8">
            @php
                ...
                if ($plan->cevap) {
                    ...
                    if ($selectedDevice) {
                        ...
                        // Eğer cihaz detailedConsignmentStocks içinde yoksa (stok 0 olabilir)
                        // Orijinal Stock modelinden bulmaya çalış
                ...
                // Seçili konsinye cihazın mevcut stok miktarını bul
                // Bu, sadece başlangıç değeri için kullanılır. JS ile güncellenecek.
                ...
                if ($currentConsignmentId) {
                    ...
                    if ($selectedDeviceInView) {
                        ...
                        // Eğer detailedConsignmentStocks içinde yoksa, yine de stok action'lardan çek
                        // Bu, özellikle stokta kalmamış ancak önceden seçili olan cihazlar için önemlidir.
            ...
            {{-- Halihazırda Atanan Cihaz Bilgisi --}}
            @if($currentConsignmentId)
                <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9;">
            ...
            <select name="soru{{ $plan->id }}" class="form-control consignment-device-select" data-plan-id="{{ $plan->id }}">
                <option value="">-- Konsinye Cihaz Seçiniz --</option>
                {{-- detailedConsignmentStocks değişkenini kullanıyoruz --}}
                ...
                @endforeach
                 {{-- Eğer seçili cihaz detailedConsignmentStocks içinde yoksa, yine de onu ekle --}}
                @if($currentConsignmentId && !$detailedConsignmentStocks->contains('id', $currentConsignmentId))
                    ...
                    @if($originalSelectedDevice)
                        <option value="C_{{ $originalSelectedDevice->id }}"
                                data-available="{{ $selectedConsignmentDeviceCurrentStock }}" {{-- Buradaki mevcut stok miktarını zaten hesaplamıştık --}}
                                ...
                                selected disabled style="color: grey;"> {{-- Seçili olarak ve devre dışı bırakılmış olarak göster --}}
            ...
            <div class="form-group consignment-quantity-group" style="margin-top: 10px; {{ $currentConsignmentId ? 'display:block;' : 'display:none;' }}">
                <label for="consignment-quantity-edit-{{ $plan->id }}">Adet:</label>
                <input type="number"
                       name="consignmentAdet{{ $plan->id }}"
                       id="consignment-quantity-edit-{{ $plan->id }}"
                       value="{{ $currentConsignmentQuantity }}"
                       min="1"
                       max="{{ $selectedConsignmentDeviceCurrentStock }}" {{-- Max değerini PHP'den gelen mevcut stok miktarına göre ayarla --}}
                       class="form-control quantity-input"
                       autocomplete="off"
                       style="width: 80px; display:inline-block;">
                <span class="text-muted available-quantity-text">(Mevcut: {{ $selectedConsignmentDeviceCurrentStock }})</span>
            </div>
