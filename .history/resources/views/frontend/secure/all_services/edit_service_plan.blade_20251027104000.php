...
$("#servisPlanGuncelle").on('submit', function(e) {
    ...
    // ÖNEMLİ: Otomatik eklenen checkbox verilerini temizle
    ...
    // Tüm quantity input verilerini de temizle
    ...
    // Seçili PERSONEL STOĞU checkboxlarını ve adetlerini ekle
    $('.stock-checkbox:checked').each(function() {
        const stockId = $(this).val();
        const checkboxName = $(this).attr('name'); // stokCheck123
        const quantityInput = $(this).closest('.stock-item').find('.quantity-input');
        const quantity = quantityInput.val() || 1;
        const quantityName = quantityInput.attr('name'); // stokAdet123

        console.log('Personel Stoğu Ekleniyor:', checkboxName, '=', stockId, '|', quantityName, '=', quantity);
        
        formData.append(checkboxName, stockId);
        formData.append(quantityName, quantity);
    });

    // Seçili KONSİNYE CİHAZ checkboxlarını ve adetlerini ekle
    $('.consignment-checkbox:checked').each(function() {
        const stockId = $(this).val();
        const checkboxName = $(this).attr('name'); // konsinyeCheck123
        const quantityInput = $(this).closest('.stock-item').find('.consignment-quantity-input');
        const quantity = quantityInput.val() || 1;
        const quantityName = quantityInput.attr('name'); // konsinyeAdet123

        console.log('Konsinye Cihaz Ekleniyor:', checkboxName, '=', stockId, '|', quantityName, '=', quantity);
