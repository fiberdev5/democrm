...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        // Birden fazla grup olabilir, hepsini ayır
        preg_match_all('/\[Grup-(\d+)\]/', $stage->cevapTuru, $matches);
        $grupKodlari = $matches[1] ?? [];
    @endphp
    
    {{-- Sadece Grup 4 veya Grup 4,5 kombinasyonu için özel işlem --}}
    @if(in_array('4', $grupKodlari))
        @php
            // Teknisyen seçimi için personeller
            ...
            // Eğer Grup 5 de varsa, Teknisyen Yardımcısı için personeller
            $teknisyenYardimcilari = null;
        ...
        {{-- Teknisyen Seçimi (Zorunlu) --}}
        <div class="form-group mb-3">
            <label class="form-label">Teknisyen <span class="text-danger">*</span></label>
            <select class="form-control" 
                    name="soru[{{ $stage->id }}][grup_4]" 
                    required>
        ...
        {{-- Teknisyen Yardımcısı Seçimi (Opsiyonel) - Sadece Grup 5 varsa göster --}}
        @if($teknisyenYardimcilari)
            <div class="form-group mb-3">
                <label class="form-label">Teknisyen Yardımcısı</label>
                <select class="form-control" 
                        name="soru[{{ $stage->id }}][grup_5]">
                    <option value="">-Seçiniz-</option>
    ...
    {{-- Diğer gruplar için (261, 262 gibi) eski mantık --}}
    @elseif(in_array('261', $grupKodlari) || in_array('262', $grupKodlari))
        @php
            $personeller = App\Models\User::where('tenant_id', $firma->id)
                ->where('status', '1')
                ->whereHas('roles', function($query) {
                    $query->where('name', 'Atölye Ustası');
                })
                ->orderBy('name', 'asc')
                ->get();
        @endphp
        ...
        <div class="form-group mb-3">
            <label class="form-label">Atölye Ustası</label>
            <select class="form-control" 
                    name="soru[{{ $stage->id }}][grup_{{ in_array('261', $grupKodlari) ? '261' : '262' }}]" 
                    required>
                <option value="">-Seçiniz-</option>
