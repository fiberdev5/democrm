...
@elseif(str_contains($stage->cevapTuru, 'Grup'))
    @php
        ...
        // Yardımcı teknisyen grup kodunu buraya yazın (Örnek: 4)
        $yardimciTeknisyenGrupKodu = 4; 
        
        // Alanın zorunlu olup olmadığını belirle
        $zorunluMu = ($grupKodu != $yardimciTeknisyenGrupKodu);
        ...
        if (in_array($grupKodu, [261, 262])) {
            ...
            // Yardımcı teknisyenler için hem Teknisyen hem de Yardımcı Teknisyen rolleri
            // aranabilir. Şimdilik mevcut mantığınızı koruyoruz.
            $roller = ['Teknisyen'];
        }
    ...
    @if($personeller->count())
        {{-- Sadece zorunluysa 'required' özelliğini ekle --}}
        <select class="form-control" name="soru[{{ $stage->id }}]" {{ $zorunluMu ? 'required' : '' }}>
            {{-- Zorunlu değilse metni değiştir --}}
            <option value="">{{ $zorunluMu ? '-Seçiniz-' : '-Yardımcı Teknisyen Seç (İsteğe Bağlı)-' }}</option>
