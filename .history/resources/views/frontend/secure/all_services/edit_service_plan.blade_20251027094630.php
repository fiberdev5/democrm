...
$("#servisPlanGuncelle").on('submit', function(e) {
    ...
    // Zorunlu alan kontrolü (bu kısım aynı kalabilir)
    ...
    // 1. Personel Stoğu (Parça) verisini hazırla
    $('.myParcaList').each(function() {
        const parcaCevaplari = [];
        const soruId = $(this).data('soru-id');

        // Eğer soruId bulunamazsa bir sonraki döngüye geç (güvenlik önlemi)
        if (!soruId) return;
        ...
        $(this).find('.stock-checkbox:checked').each(function() {
            ...
            parcaCevaplari.push(`${stockId}---${quantity}`);
        });

        // Backend'in beklediği "123---1, 456---2" formatında birleştir ve formData'daki ilgili soruyu güncelle.
        // Formdaki <input type="hidden" name="soru{ID}" value="Parca"> alanının değerini bu yeni dize ile değiştirir.
        formData.set(`soru${soruId}`, parcaCevaplari.join(', '));
    });

    // 2. Konsinye Cihaz verisini hazırla (artık aynı mantıkla çalışıyor)
    $('.myKonsinyeList').each(function() {
        const konsinyeCevaplari = [];
        const soruId = $(this).data('soru-id'); // Adım 1'de eklediğimiz data-soru-id'den okunur

        if (!soruId) return;

        $(this).find('.consignment-checkbox:checked').each(function() {
            const stockId = $(this).val();
            const quantity = $(this).closest('.stock-item').find('.consignment-quantity-input').val();
            konsinyeCevaplari.push(`${stockId}---${quantity}`);
        });
        
        // formData'daki ilgili konsinye sorusunu güncelle
        formData.set(`soru${soruId}`, konsinyeCevaplari.join(', '));
    });
