            ...
            <div class="col-lg-8">
                @php
                    ...
                    if ($plan->cevap) {
                        ...
                        // Seçili konsinye cihazın adını bul
                        $selectedDevice = $calculatedConsignmentDevices->firstWhere('id', $currentConsignmentId);
                        if ($selectedDevice) {
                ...
                @if($currentConsignmentId)
                    <div style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; background-color: #f9f9f9;">
                        **Halihazırda Atanan Cihaz:** {{ $currentConsignmentDeviceName }} (Adet: {{ $currentConsignmentQuantity }})
                    </div>
                @endif
                ...
                <select name="soru{{ $plan->id }}" class="form-control consignment-device-select" data-plan-id="{{ $plan->id }}">
                    ...
                    {{-- calculatedConsignmentDevices değişkenini kullanıyoruz --}}
                    @foreach($calculatedConsignmentDevices as $device)
                        {{-- Sadece mevcut stok miktarı pozitif olanları veya mevcut planda seçili olanı göster --}}
