...
@elseif($stage->cevapTuru == "[Konsinye Cihaz]")
    ...
    @php
        // Tüm grup kodlarını çıkar
        preg_match_all('/\[Grup-(\d+)\]/', $stage->cevapTuru, $matches);
        $grupKodlari = $matches[1] ?? [];
        
        // Stage'e göre roller belirleniyor
        $roller = [];
        
        // Stage 24 için - Teknisyen (Grup-4 içeriyorsa)
        if ($stage->id == 24 && in_array('4', $grupKodlari)) {
            $roller = ['Teknisyen'];
        }
        
        // Stage 25 için - Yardımcı Teknisyen (Grup-4 veya Grup-5 içeriyorsa)
        if ($stage->id == 25) {
        ...
        // Roller belirlendiyse personelleri getir
        $personeller = collect();
        if (!empty($roller)) {
