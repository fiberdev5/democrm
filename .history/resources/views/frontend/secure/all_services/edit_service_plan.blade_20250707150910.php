...
<script>
    $(document).ready(function(e) {
        ...
        $("#servisPlanGuncelle").on('submit', function(e) {
            ...
            // formData nesnesini burada oluşturuyoruz
            var formData = new FormData(this); // 'this' form elementini temsil eder

            // Konsinye cihaz seçimi için özel işlem
            $('.consignment-device-select').each(function() {
                const selectElement = $(this);
                const planId = selectElement.data('stage-id');
                const selectedOption = selectElement.find('option:selected');

                if (selectedOption.val()) {
                    const deviceId = selectedOption.val().replace('C_', '');
                    const quantityInput = selectElement.closest('.col-lg-8, .col-lg-12').find('input[name^="consignment_quantity_"]');
                    const deviceNameInput = selectElement.closest('.col-lg-8, .col-lg-12').find('.consignment-device-name-input');
                    
                    const quantity = quantityInput.val();
                    const deviceName = deviceNameInput.val(); // Hidden input'tan cihaz adını al

                    // Yeni format: "Cihaz Adı: C_ID---Adet"
                    const combinedValue = `${deviceName}: C_${deviceId}---${quantity}`;
                    formData.set(`soru${planId}`, combinedValue); // Mevcut değeri FormData'da güncelle
                } else {
                    formData.set(`soru${planId}`, ''); // Seçim kaldırıldıysa boş gönder
                }
            });

            // Parça seçimi için özel işlem
            $('.myParcaList').each(function() {
                const parcaList = $(this);
                const soruId = parcaList.data('soru-id');
                let selectedParts = [];

                parcaList.find('.checkbox input[type="checkbox"]:checked').each(function() {
                    const stokId = $(this).attr('name').replace('stokCheck', '');
                    const adet = parcaList.find(`input[name="stokAdet${stokId}"]`).val();
                    selectedParts.push(`${stokId}---${adet}`);
                });
                // Soru alanı zaten soru{id} olarak adlandırılmış, bu yüzden doğrudan formData.set ile güncelleyebiliriz.
                formData.set(`soru${soruId}`, selectedParts.join(', '));
            });
            
            // Diğer normal form alanları FormData tarafından otomatik olarak alınacaktır.
            // Ancak, "[Parca]" ve "[Konsinye Cihaz]" türündeki alanların dinamik olarak güncellenen
            // gizli input'larının doğru şekilde yakalandığından emin olmalıyız.
            // Bu zaten yukarıdaki 'each' döngüleriyle sağlanıyor.


            $.ajax({
                url: $(this).attr('action'),
                type: "POST",
                data: formData, // Tanımlanan formData objesi
                contentType: false,
                cache: false,
                processData: false,
        ...
        $(document).on('change', '.consignment-device-select', function() {
            ...
            if (selectedOption.val()) { // Bir cihaz seçildiyse
                ...
                // Gizli input'u güncelle (Cihaz Adı: C_ID---Adet formatında)
                const deviceId = selectedOption.val().replace('C_', '');
                combinedValueInput.val(`${deviceName}: C_${deviceId}---${quantityInput.val()}`);
            } else {
        ...
        // Konsinye cihaz adet input'u değiştiğinde
        $(document).on('input', '.consignment-quantity-group .quantity-input', function() {
            const quantityInput = $(this);
            const selectElement = quantityInput.closest('.col-lg-8, .col-lg-12').find('.consignment-device-select');
            const selectedOption = selectElement.find('option:selected');
            const combinedValueInput = quantityInput.closest('.col-lg-8, .col-lg-12').find('[id^="consignment-combined-value-"]');
            const deviceNameInput = quantityInput.closest('.col-lg-8, .col-lg-12').find('.consignment-device-name-input');
            ...
            if (selectedOption.val()) {
                ...
                // Gizli input'u güncelle (Cihaz Adı: C_ID---Adet formatında)
                combinedValueInput.val(`${deviceName}: C_${deviceId}---${quantity}`);
            }
        });
        ...
        $('.consignment-device-select').each(function() {
            ...
            if (selectedOption.val()) { // Eğer bir cihaz seçiliyse (edit modunda genelde seçili gelir)
                ...
                // Gizli input'u güncelle (Cihaz Adı: C_ID---Adet formatında)
                const deviceId = selectedOption.val().replace('C_', '');
                combinedValueInput.val(`${deviceName}: C_${deviceId}---${currentQuantity}`);
            } else {
