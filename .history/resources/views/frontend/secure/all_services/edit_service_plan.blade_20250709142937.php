      ...
      <form method="POST" id="servisPlanGuncelle" action="{{route('update.service.plan', $tenant_id)}}">
        ...
        @foreach($planCevaplar as $plan)
          ...
          @endphp
                ...
                @elseif($cevap->cevap == "Konsinye Cihaz")
                    <input type="text" class="form-control urunAraInput" placeholder="Konsinye cihaz adı veya kodu" autocomplete="off">
                    <div class="konsinye-dropdown myKonsinyeList" style="width:100%">
                        ...
                        @forelse($konsinyeCihazlar as $konsinyeCihaz)
                            @php
                                ...
                                // Mevcut seçili konsinye cihazları kontrol et
                                $mevcutKonsinyeler = explode(', ', $cevap->cevap);
                                $seciliMi = false;
                                $seciliAdet = 1;
                                
                                foreach($mevcutKonsinyeler as $konsinyeInfo) {
                                    if(strpos($konsinyeInfo, $konsinyeId . '---') !== false) {
                                        $seciliMi = true;
                                        $seciliAdet = explode('---', $konsinyeInfo)[1] ?? 1;
                                        break;
                                    }
                                }
                            @endphp
                    ...
                    <input type="hidden" name="soru{{ $cevap->id }}" value="Konsinye Cihaz"/>
...
$(document).ready(function(e) {
 ...
 $('.urunAraInput').on('keyup', function() {
        ...
        container.find('.stock-item').each(function() {
