...
$(document).on('keyup', '.urunAraInput', function() {
    ...
    // Arama inputunun ardından gelen stok container'ını bul
    const container = $(this).nextAll('.parcalar-dropdown, .konsinye-dropdown').first();

    container.find('.stock-item').each(function() {
        const productName = ($(this).data('product-name') || '').toLowerCase();
        const productCode = ($(this).data('product-code') || '').toLowerCase();
