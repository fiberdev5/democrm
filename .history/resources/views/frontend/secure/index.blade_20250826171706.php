...
<style>
    ...
    .service-item div:not(.service-count) { font-size: 0.8rem; line-height: 1.2; }
...
<div class="dashboard-container">
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="dashboard-card">
                <div class="dashboard-card-header">
                    <h5><i class="fas fa-exclamation-triangle"></i> Kritik ve Düşük Stok Uyarıları</h5> <!-- Başlığı güncelleyebiliriz -->
                ...
                <div class="dashboard-card-body">
                    @php 
                        // Kritik ve düşük stokları tek bir listede birleştirelim
                        $all_alerts = array_merge($stock_alerts['critical'], $stock_alerts['low']);
                        $criticalCount = count($stock_alerts['critical']); 
                    @endphp
                    
                    @forelse ($all_alerts as $item)
                    <div class="stock-item">
                        <div class="stock-icon-wrapper {{ $item->alert_type }}"><i class="fas fa-box-open"></i></div>
                        <div class="stock-details">
                            <h6>{{ $item->urunAdi }}</h6>
                            <p>Kategori ID: {{ $item->urunKategori }}</p>
                        </div>
                        <div class="stock-level {{ $item->alert_type }}">
                    ...
                    @empty
                    <p class="text-center text-muted">Kritik veya düşük seviyede ürün bulunmamaktadır.</p>
                    @endforelse
                    ...
                    {{-- Eğer hiç uyarı yoksa (bu artık yukarıdaki @empty tarafından halledilir) --}}
