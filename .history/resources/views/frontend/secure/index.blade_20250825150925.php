    ...
    .status-pending { background-color: #6c757d; } /* Beklemede */
        ...
        <div class="dashboard-container">
            ...
            <div class="row">
                ...
                <div class="col-lg-3 col-md-6 mb-4">
                       <a href="#" onclick="window.location.href = buildCashUrl('total')" class="stat-card teal">
            ...
            <div class="col-lg-7">
                <div class="dashboard-card">
                    <div class="dashboard-card-header">
                        ...
                        <a href="{{-- route('servisler.index') --}}" class="view-all-btn">Tümünü Gör <i class="fas fa-arrow-right"></i></a>
                    </div>
                    <div class="dashboard-card-body">
                        @forelse ($last_services as $service)
                        <div class="service-request-item">
                            <div class="service-customer-info">
                                <h6>{{ $service->customer_name }} <small class="text-muted">#{{ $service->servis_no }}</small></h6>
                                <p>{{ $service->cihaz_modeli }}</p>
                                <p style="font-size: 0.8rem; color: #8892b0;">
                                    <i class="fas fa-user-cog"></i> {{ $service->technician_name ?? 'Atanmadı' }} | 
                                    <i class="fas fa-calendar-alt"></i> Bitiş: {{ $service->bitis_tarihi_tahmini ? \Carbon\Carbon::parse($service->bitis_tarihi_tahmini)->format('d.m.Y') : 'Belirsiz' }}
                                </p>
                            </div>
                            <span class="service-status-badge {{ $service->color_class }}">{{ $service->status_name }}</span>
                        </div>
                        @empty
            ...
            <div class="col-lg-5">
                <div class="dashboard-card">
                    <div class="dashboard-card-header">
                        ...
                        <a href="{{-- route('stoklar.index') --}}" class="view-all-btn">Tümünü Gör <i class="fas fa-arrow-right"></i></a>
                    </div>
                    <div class="dashboard-card-body">
                        @php $criticalCount = count($stock_alerts['critical']); @endphp
                        
                        @forelse ($stock_alerts['critical'] as $item)
                        <div class="stock-item">
                            ...
                            <div class="stock-level critical">
                                <div class="level-text">{{ $item->stock }} / {{ $item->critical_stock_level }}</div>
                                <div class="level-label">Kritik Seviye</div>
                            </div>
                        </div>
                        @empty
                        {{-- Boşsa bir şey gösterme --}}
                        @endforelse

                        @forelse ($stock_alerts['low'] as $item)
                        <div class="stock-item">
