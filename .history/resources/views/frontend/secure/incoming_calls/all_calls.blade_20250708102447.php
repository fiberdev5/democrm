      ...
      <div class="card" style="margin-bottom: 185px;">
        ...
        <div class="card-body" id="gelenCagrilar">
          <table id="datatableIncomingCalls" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
            <a  class="btn btn-success btn-sm mb-1 gelenCagriBtn" data-bs-toggle="modal" data-bs-target="#gelenCagriModal"><i class="fas fa-plus"></i><span>Yeni Çağrı Ekle</span></a>
            <thead class="title">
              <tr>
            ...
            <tbody>
              @foreach($incoming_calls as $item)
                <tr data-id="{{$item->id}}">
                  ...
                  <td><a class="t-link editIncomingCall" href="javascript:void(0);" data-bs-id="{{$item->id}}" data-bs-toggle="modal" data-bs-target="#editIncomingCallModal"><div class="mobileTitle">Tarih:</div>{{ Carbon::parse($item->created_at)->format('d/m/Y')}}</a></td>
                  <td><a class="t-link editIncomingCall" href="javascript:void(0);" data-bs-id="{{$item->id}}" data-bs-toggle="modal" data-bs-target="#editIncomingCallModal"><div class="mobileTitle">Telefon:</div>{{$item->marka->aciklama}}</a></td>
                  <td><a class="t-link editIncomingCall" href="javascript:void(0);" data-bs-id="{{$item->id}}" data-bs-toggle="modal" data-bs-target="#editIncomingCallModal"><div class="mobileTitle">Marka:</div>{{$item->marka->marka}}</a></td>
