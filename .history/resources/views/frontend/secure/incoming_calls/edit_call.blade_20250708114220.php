...
<form method="post" id="editCall" action="{{ route('update.call', $firma->id)}}" enctype="multipart/form-data" class="needs-validation" novalidate>
    ...
    <div class="row">
        ...
        <div class="col-sm-8">
        <select name="personel" class="form-select" required>
            <option selected disabled value="">-Seçiniz-</option>
            @foreach($operators as $item)
            <option value="{{$item->user_id}}" {{$item->user_id == $call_id->kayitAlan ? 'selected' : ''}}>{{$item->name}}</option>
            @endforeach
        </select>
        </div>
    </div>
        ...
        if (this.checkValidity() === false) {
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              var rowToUpdate = $('#datatableIncomingCall tbody tr[data-id="' + response.id + '"]');
              rowToUpdate.find('td:eq(1)').html(`<a class="t-link editIncomingCall" href="javascript:void(0);" data-bs-id="${response.id}" data-bs-toggle="modal" data-bs-target="#editIncomingCallModal"><div class="mobileTitle">Tarih:</div>${response.created_at}</a>`);    
              ...
              $('#editIncomingCallsModal').modal('hide');
            },
