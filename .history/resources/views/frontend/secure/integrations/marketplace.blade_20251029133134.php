        ...
        <div class="container">
            <div class="text-center">
                ...
                <div class="search-box">
                    <form action="{{ route('tenant.integrations.marketplace') }}" method="GET">
            ...
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a class="nav-link {{ !request('category') || request('category') == 'all' ? 'active' : '' }}" 
                       href="{{ route('tenant.integrations.marketplace') }}">
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('tenant.integrations.my') }}" class="btn btn-outline-primary">
                <i class="fas fa-check-circle"></i> Aktif Entegrasyonlarım ({{ count($activeIntegrations) }})
            </a>
        </div>
        ...
        <div class="integration-grid">
            ...
            <div class="integration-card">
                @if(in_array($integration->id, $activeIntegrations))
                <span class="active-badge"><i class="fas fa-check"></i> Aktif</span>
                @endif
                ...
                <div class="action-buttons">
                    @if(in_array($integration->id, $activeIntegrations))
                    <a href="{{ route('tenant.integrations.deactivate', $integration->id) }}" 
                       class="btn btn-deactivate"
                       onclick="return confirm('Bu entegrasyonu devre dışı bırakmak istediğinize emin misiniz?');">
                        <i class="fas fa-times-circle"></i> Devre Dışı Bırak
                    </a>
                    <a href="{{ route('tenant.integrations.settings', $integration->id) }}" class="btn btn-detail">
                        <i class="fas fa-cog"></i> Ayarlar
                    </a>
                    @else
                    <a href="{{ route('tenant.integrations.activate', $integration->id) }}" class="btn btn-activate">
                        <i class="fas fa-check-circle"></i> Aktifleştir
                    </a>
                    <a href="{{ route('tenant.integrations.show', $integration->slug) }}" class="btn btn-detail">
        ...
        <div class="no-results">
            ...
            <a href="{{ route('tenant.integrations.marketplace') }}" class="btn btn-primary mt-3">
