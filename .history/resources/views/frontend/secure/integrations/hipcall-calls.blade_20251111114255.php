...
function testApiConnection() {
    if (typeof toastr !== 'undefined') {
        toastr.info('API bağlantısı test ediliyor...');
    }
    
    $.ajax({
        url: '{{ route("tenant.integrations.hipcall.test-api", $tenant->id) }}',
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}'
        },
        success: function(response) {
            if (response.success) {
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.error(response.message || 'API bağlantısı başarısız');
                } else {
                    alert('✗ ' + (response.message || 'API bağlantısı başarısız'));
                }
            }
        },
        error: function(xhr) {
