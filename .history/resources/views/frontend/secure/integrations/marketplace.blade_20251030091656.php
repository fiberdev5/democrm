                        ...
                        <div class="integration-grid">
                            ...
                            <div class="integration-card" data-url="{{ route('tenant.integrations.show', [$tenant->id, $integration->slug]) }}">
                                ...
                                <div class="action-buttons">
                                    <a href="{{ route('tenant.integrations.activate', [$tenant->id, $integration->id]) }}" 
                                       class="btn-activate"
                                       onclick="event.stopPropagation();">
                                        <i class="fas fa-check-circle"></i> Aktifleştir
                                    </a>
                                    <a href="{{ route('tenant.integrations.show', [$tenant->id, $integration->slug]) }}" 
                                       class="btn-detail"
                                       onclick="event.stopPropagation();">
...
$(document).ready(function() {
    // Sayfa yüklendiğinde arama değeri varsa X butonunu göster
    ...
    $('#clearButton').on('click', function() {
        $('#searchInput').val('');
        toggleSearchButtons();
        // Formu otomatik submit et (aramayı temizle)
        $('#searchForm').submit();
    });

    // Entegrasyon kartlarına tıklandığında detay sayfasına git
    $('.integration-card').on('click', function(e) {
        // Eğer action-buttons içindeki bir linke tıklanmışsa, kartın tıklama olayını tetikleme
    ...
    // Action butonlarına tıklandığında kartın tıklama olayını engelle
