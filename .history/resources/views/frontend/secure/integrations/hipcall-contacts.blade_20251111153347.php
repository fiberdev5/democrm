...
<div class="page-content">
            ...
            <div class="col-12">
                ...
                <div id="loadingProgress" style="display: none;">
                    <div class="alert alert-info">
                        <h5><i class="fas fa-spinner fa-spin"></i> Hipcall Rehberi Yükleniyor...</h5>
                        ...
                        <p class="mt-2 mb-0 text-muted">
                            <small id="progressDetail">Kişiler çekiliyor...</small>
                        </p>
                    </div>
                </div>
    ...
    <tr class="{{ isset($contact['exists_in_serbis']) && $contact['exists_in_serbis'] ? 'table-light' : '' }}">
        <td>
            <input type="checkbox" 
                   class="contact-checkbox {{ isset($contact['exists_in_serbis']) && $contact['exists_in_serbis'] ? 'existing-contact' : 'new-contact' }}" 
                   value="{{ $index }}"
                   data-contact="{{ json_encode($contact) }}"
                   onchange="updateSelectedCount()"
                   {{ isset($contact['exists_in_serbis']) && $contact['exists_in_serbis'] ? 'disabled' : '' }}>
        </td>
        <td>
            <strong>{{ $contact['first_name'] ?? '' }} {{ $contact['last_name'] ?? '' }}</strong>
            @if(empty($contact['first_name']) && empty($contact['last_name']))
                <span class="text-muted">Ad bilgisi yok</span>
            @endif
        </td>
        <td>
            @if(isset($contact['display_phone']) && !empty($contact['display_phone']))
                <span class="badge bg-info">
                    {{ $contact['display_phone'] }}
                </span>
            @else
                <span class="badge bg-secondary">Telefon yok</span>
                <button type="button" 
                        class="btn btn-xs btn-outline-info ms-1" 
                        onclick="loadContactDetail({{ $contact['id'] ?? 'null' }}, {{ $index }})">
                    <i class="fas fa-sync"></i> Detay Çek
                </button>
            @endif
        </td>
        <td>{{ $contact['email'] ?? '-' }}</td>
        ...
        <td>
            @if(isset($contact['exists_in_serbis']) && $contact['exists_in_serbis'])
                <span class="badge bg-success">
                    <i class="fas fa-check"></i> Mevcut
                </span>
                @if(isset($contact['customer_id']))
                <a href="{{ route('customer.edit', [$tenant->id, $contact['customer_id']]) }}" 
...
<!-- Debug Panel -->
@if(config('app.debug') && isset($contacts) && count($contacts) > 0)
<div class="row mt-3">
    <div class="col-12">
        <div class="card border-warning">
            <div class="card-header bg-warning text-dark">
                <h5 class="mb-0"><i class="fas fa-bug"></i> Debug - İlk Kişinin Raw Data'sı</h5>
            </div>
            <div class="card-body">
                <pre class="bg-light p-3" style="max-height: 400px; overflow-y: auto;">{{ json_encode($contacts[0] ?? [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                
                <h6 class="mt-3">Tüm Fieldlar:</h6>
                <div class="bg-light p-2">
                    @if(isset($contacts[0]))
                        @foreach(array_keys($contacts[0]) as $key)
...
// Sayfa yüklendiğinde loading göster
...
// Tek bir contact'ın detayını AJAX ile çek
function loadContactDetail(contactId, rowIndex) {
    if (!contactId) {
        alert('Contact ID bulunamadı');
        return;
    }
    
    if (typeof toastr !== 'undefined') {
        toastr.info('Detay çekiliyor...');
    }
    
    $.ajax({
        url: `/{{ $tenant->id }}/integrations/hipcall/contact/${contactId}`,
        method: 'GET',
        success: function(response) {
            if (response.success && response.contact) {
                const phone = response.contact.phone 
                           || response.contact.phone_number 
                           || response.contact.mobile 
                           || 'Bulunamadı';
                
                // Satırı güncelle
                const row = $('tr').eq(rowIndex + 1); // +1 çünkü thead var
                row.find('td:eq(2)').html(`<span class="badge bg-info">${phone}</span>`);
                
                if (typeof toastr !== 'undefined') {
                    toastr.success('Telefon: ' + phone);
                }
            } else {
                if (typeof toastr !== 'undefined') {
                    toastr.warning('Telefon numarası bulunamadı');
                }
            }
        },
        error: function() {
            if (typeof toastr !== 'undefined') {
                toastr.error('Detay çekilemedi');
            }
        }
    });
}
