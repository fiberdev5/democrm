...
<div class="modal fade" id="tevkifatHesaplamaModal" tabindex='-1' role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="row mb-2">
    ...
    <div class="col-sm-8">
      <select class="form-control kdvOrani" onchange="hesapla()">
  ...
  <!-- Tevkifat Oranı -->
  <div class="row mb-2">
    <label class="col-sm-4 col-form-label">Tevkifat Oranı:</label>
    <div class="col-sm-8">
      <select class="form-control tevkifatOrani" onchange="hesapla()">
  ...
  <div class="row form-group">
    ...
    <div class="col-md-8">
      <input class="form-control tutar" type="text" placeholder="0.00" onkeyup="hesapla()" style="width: calc(100% - 25px);display: inline-block;margin-right: -10px;font-weight: 500;">
  ...
  <!-- Hesaplanan Birim Tutar -->
  <div class="row form-group">
    <div class="col-md-4"><label style="color:red">Birim Tutar</label></div>
    <div class="col-md-8">
      <input type="text" class="form-control sonuc" placeholder="0.00" style="width: calc(100% - 25px);display: inline-block;margin-right: -10px;font-weight: 500;background: #e5e5e5;" disabled>
      <input type="text" class="form-control" value="TL" disabled style="width: 30px;display: inline-block;background: #fff;border-left: 0;border-top-left-radius: 0; border-bottom-left-radius: 0;text-align: center;">
    </div>
  </div>
</div>
    </div>
...
<script>
  function hesapla() {
  let kdvDahilTutar = parseFloat(document.querySelector('.tutar').value.replace(",", ".")) || 0;
  let kdvOrani = parseFloat(document.querySelector('.kdvOrani').value) || 1.20;
  let tevkifatOrani = parseFloat(document.querySelector('.tevkifatOrani').value) || 1;

  // KDV Hariç Tutar (KDV dahil / oran)
  let kdvHaricTutar = kdvDahilTutar / kdvOrani;

  // Tevkifat varsa: sadece KDV kısmına uygulanır
  let kdvTutari = kdvDahilTutar - kdvHaricTutar;
  let tevkifatTutari = (tevkifatOrani < 10) ? (kdvTutari * (tevkifatOrani / 10)) : kdvTutari;

  // Sonuç
  let toplamTutar = kdvHaricTutar + (kdvTutari - tevkifatTutari);

  document.querySelector('.sonuc').value = toplamTutar.toFixed(2).replace(".", ",");
}
</script>
...
<script>
  ...
  function setDateRangeToFull() {
    var baslangicYil = moment('01-01-2020', 'DD-MM-YYYY');
    var today = moment();
    $('#daterange').data('daterangepicker').setStartDate(baslangicYil);
    $('#daterange').data('daterangepicker').setEndDate(today);
  }
