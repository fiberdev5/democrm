...
<div class="modal fade" id="tevkifatHesaplamaModal" tabindex='-1' role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <input type="number" class="tutar" step="0.01"><br><br>

  <label>KDV Oranı:</label>
  <select class="kdv_orani">
  ...
  <select class="tevkifat_orani">
    <option value="0">Yok</option>
    <option value="0.5">5/10 (Yarısı kesilir)</option>
    <option value="0.9">9/10</option>
    <option value="1">10/10 (Tamamı kesilir)</option>
  </select><br><br>

  <label>KDV Hariç Tutar:</label>
  <input type="text" class="sonuc" readonly><br><br>

  <label>KDV Tutarı:</label>
  <input type="text" class="kdv_tutari" readonly><br><br>

  <label>Alıcı Tarafından Ödenecek Tevkifat:</label>
  <input type="text" class="tevkifat_tutari" readonly><br><br>
      </div>
    </div>
  </div>
</div>
...
<script>
  $(document).ready(function () {
    $('#tevkifatHesaplamaModal .tutar, #tevkifatHesaplamaModal .kdv_orani, #tevkifatHesaplamaModal .tevkifat_orani').on('input change', function () {
      var tutar = parseFloat($("#tevkifatHesaplamaModal .tutar").val());
      var kdvOrani = parseFloat($("#tevkifatHesaplamaModal .kdv_orani").val());
      var tevkifatOrani = parseFloat($("#tevkifatHesaplamaModal .tevkifat_orani").val());

      if (!isNaN(tutar) && !isNaN(kdvOrani)) {
        var kdvKatsayi = 1 + kdvOrani;
        var kdvHaric = tutar / kdvKatsayi;
        var kdvTutari = tutar - kdvHaric;

        var tevkifatTutari = kdvTutari * tevkifatOrani;

        // Yuvarlama
        kdvHaric = kdvHaric.toFixed(2);
        kdvTutari = kdvTutari.toFixed(2);
        tevkifatTutari = tevkifatTutari.toFixed(2);

        $("#tevkifatHesaplamaModal .sonuc").val(kdvHaric);
        $("#tevkifatHesaplamaModal .kdv_tutari").val(kdvTutari);
        $("#tevkifatHesaplamaModal .tevkifat_tutari").val(tevkifatTutari);
      }
    });
  });
</script>
