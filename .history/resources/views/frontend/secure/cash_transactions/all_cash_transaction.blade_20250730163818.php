  ...
  $(document).ready(function () {
    ...
    var table = $('#datatableKasa').DataTable({
      ...
      ajax: {
        ...
        // Sadece boş olmayan değerleri gönder
        var odemeTuru = $('#odemeTuru').val();
        if(odemeTuru && odemeTuru !== '') {
            data.odemeTuru = odemeTuru;
        }
        
        var odemeYonu = $('#odemeYonu').val();
        if(odemeYonu && odemeYonu !== '') {
            data.odemeYonu = odemeYonu;
        }
        
        var odemeSekil = $('#odemeSekil').val();
        if(odemeSekil && odemeSekil !== '') {
            data.odemeSekil = odemeSekil;
        }
        ...
        // Ödeme durumu özel kontrol (0 = Hepsi)
        var odemeDurumu = $('#odemeDurumu').val();
        if(odemeDurumu && odemeDurumu !== '0') {
            data.odemeDurum = odemeDurumu;
        }
