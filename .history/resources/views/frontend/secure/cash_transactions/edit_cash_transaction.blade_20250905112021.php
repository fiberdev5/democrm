<form method="post" id="editDom" action="{{ route('update.cash.transaction', $firma->id) }}" enctype="multipart/form-data" class="needs-validation" novalidate>
  ...
  @if (strpos($cash_payment_id["cevaplar"], '3') !== false)
    <div class="row ">
      ...
      <div class="col-sm-8" style="align-items: center;justify-content: space-between;display: flex;">
        ...
        <a class="btn btn-outline-danger" href="{{ route('all.services', [$firma->id, 'did' => $cash_transaction_id->servis]) }}" target="_blank" style="font-weight: 500;font-size: 13px;color: red">Servisi Göster</a>
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-4">
      <input type="number" step="0.01" name="fiyat" class="form-control fiyat" value="{{$cash_transaction_id->fiyat}}" placeholder="0.00">
    </div>
    <div class="col-sm-4">
