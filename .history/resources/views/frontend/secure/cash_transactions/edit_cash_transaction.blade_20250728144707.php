<form method="post" id="editDom" action="{{ route('update.cash.transaction', $firma->id) }}" enctype="multipart/form-data" class="needs-validation" novalidate>
      ...
      <div class="col-md-8">
        <select class="form-select cihazlar" name="cihazlar" style="font-weight: 500">
          ...
          @foreach ($cihazlar as $item)
            <option value="{{$item->id}}" {{$item->id == $cash_transaction_id->cihaz}}>{{$item->cihaz}}</option>
          @endforeach
        </select>
      </div>
    </div>
      ...
      <div class="col-md-8">
        <select class="form-select markalar" name="markalar" style="font-weight: 500">
          ...
          @foreach ($markalar as $item)
            <option value="{{$item->id}}" {{$item->id == $cash_transaction_id->marka}}>{{$item->marka}}</option>
          @endforeach
        </select>
      </div>
    </div>
      ...
      <div class="col-md-8">
        <select class="form-select tedarikciler" name="tedarikciler" style="font-weight: 500">
          ...
          @foreach ($tedarikciler as $item)
            <option value="{{$item->id}}" {{$item->id == $cash_transaction_id->tedarikci}}>{{$item->tedarikci}}</option>
          @endforeach
        </select>
      </div>
    </div>
  ...
  @if (strpos($cash_payment_id["cevaplar"], '3') !== false)
    <div class="row ">
      ...
      <div class="col-sm-8">
        <input type="text" name="servis" class="form-control servis" data-id="" autocomplete="off" disabled>
  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-4">
      <select class="form-select paraBirimi" name="paraBirimi">
        @foreach($currencies as $curry)
          <option value="{{$curry->id}}" {{$curry->id == $cash_transaction_id->paraBirimi ? 'selected' : ''}}>{{$curry->baslik}} ({{$curry->aciklama}})</option>
        @endforeach
      </select>
    </div>
  </div>
