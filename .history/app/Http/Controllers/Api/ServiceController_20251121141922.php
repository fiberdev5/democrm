...
public function getStageQuestions(Request $request, $asama_id)
{
    ...
    foreach ($sorular as $soru) {
        ...
        if ($soru->cevap == '[Aciklama]') {
            $inArray['type'] = 'input';
        }
        // [Grup] - Personel seçimi
        else if (str_contains($soru->cevap, 'Grup')) {
            $inArray['type'] = 'select';
            $inArray['aciklama'] = $this->getPersonelListByGroup($soru->cevap, $tenant->id);
        }
        // [Tarih] - Tarih seçici
        else if ($soru->cevap == '[Tarih]') {
            $inArray['type'] = 'datepicker';
            $inArray['aciklama'] = $this->getDefaultDate();
        }
        // [Saat] - Saat aralığı seçimi
        else if ($soru->cevap == '[Saat]') {
        ...
        // [Fiyat] - Para input
        else if ($soru->cevap == '[Fiyat]') {
            $inArray['type'] = 'money';
        }
        // [Teklif] - Teklif input
        else if ($soru->cevap == '[Teklif]') {
            $inArray['type'] = 'money';
        }
        // [Parca] - Stok listesi
        else if ($soru->cevap == '[Parca]') {
        ...
        else if ($soru->cevap == '[Konsinye Cihaz]') {
            $inArray['type'] = 'checkbox';
    ...
    return response()->json([
        'success' => true,
        'data' => $altAsamalarArray,
        'total' => count($altAsamalarArray)
    ], 200);
}
...
{
    // Örnek: "[Grup-4], [Grup-5]" -> [4, 5]
    preg_match_all('/\[Grup-(\d+)\]/', $cevap, $matches);
    $gruplar = $matches[1] ?? [];

    // Grup numarasına göre role mapping (Web kodunuzdaki gibi)
    $roller = [];
    foreach ($gruplar as $grup) {
        if (in_array($grup, ['261', '262'])) {
            $roller[] = 'Atölye Ustası';
        } else if (in_array($grup, ['4', '5'])) {
            $roller[] = 'Teknisyen';
        }
        // Diğer grup mappingleri buraya...
    }
    ...
    // Personelleri getir (Spatie role ile)
    $personeller = \App\Models\User::where('tenant_id', $tenantId)
        ->where('status', '1')
...
{
    $araclar = \App\Models\ServiceVehicle::where('firma_id', $tenantId)
        ->where('durum', '1')
        ->orderBy('id', 'asc')
        ->get()
        ->map(function($arac) {
...
private function getPersonelStokList($userId, $tenantId)
{
    $stoklar = \DB::table('personel_stocks as ps')
        ->join('stoklar as s', 's.id', '=', 'ps.stok_id')
        ->where('ps.pid', $userId)
        ->where('ps.firma_id', $tenantId)
        ->where('ps.adet', '>', 0)
        ->select([
            'ps.id',
            'ps.stok_id as stokid',
            'ps.adet',
            's.urunAdi',
            's.urunKodu'
        ])
...
{
    // Eğer konsinye cihaz tablosu varsa benzer mantık
    $konsinyeler = DB::table('konsinye_cihazlar as kc')
        ->where('kc.firma_id', $tenantId)
        ->where('kc.adet', '>', 0)
        ->select([
            'kc.id',
            'kc.urunAdi',
        ...
        ->get()
        ->map(function($konsinye) {
            return [
                'id' => (string) $konsinye->id,
                'urunAdi' => $konsinye->urunAdi,
                'urunKodu' => $konsinye->urunKodu,
                'adet' => (string) $konsinye->adet,
            ];
        })
        ->toArray();
