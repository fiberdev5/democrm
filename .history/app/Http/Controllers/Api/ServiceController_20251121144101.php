...
{
    // Web kodunuzdaki yapıya göre uyarlayın
    // Eğer konsinye_cihaz tablosu varsa:
    $cihazlar = DB::table('konsinye_cihaz as kc')
        ->where('kc.pid', $userId)
        ->where('kc.firma_id', $tenantId)
        ->where('kc.current_stock_quantity', '>', 0) // veya ilgili stok alanı
        ->select([
            'kc.id',
            'kc.urunAdi',
            'kc.urunKodu',
            'kc.current_stock_quantity as adet'
        ])
