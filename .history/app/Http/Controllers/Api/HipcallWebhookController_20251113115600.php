    ...
    private function handleCallInit($tenant, array $data): void
{
    ...
    if ($direction == 'inbound' && $callerNumber) {
        ...
        // 🔥 CALL_INIT'TE KART GÖNDER (Geçici çözüm)
...
private function findOrCreateCustomer($tenant, ?string $phone, array $callData)
{
    ...
    // 🔥 Telefonu temizle: +905378896552 → 5378896552
    $cleanPhone = preg_replace('/[^0-9]/', '', $phone);
    
    // Türkiye kodu varsa kaldır: 905378896552 → 5378896552
    if (str_starts_with($cleanPhone, '90')) {
        $cleanPhone = substr($cleanPhone, 2);
    }
    
    // Başındaki 0'ı kaldır: 05378896552 → 5378896552
    ...
    // Müşteriyi ara - veritabanındaki boşlukları göz ardı ederek
    ...
    try {
        // 🔥 Yeni müşteri için telefonu formatla: 5378896552 → 537 889 6552
...
public function prepareCustomerCard($customer, $tenantId, $baseUrl)
{
    ...
    // Firma adı ekle (DÜZELTME: name yerine firma_adi)
