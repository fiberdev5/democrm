...
private function olusturFisIcerigi($servis, $tenant, $user, $fisTemplate)
{
    ...
    $musteriBilgileri = $servis->musteri->adSoyad . "\n" .
                       $servis->musteri->tel1 . ($servis->musteri->tel2 ? " " . $servis->musteri->tel2 : '') . "\n" .
                       ($servis->musteri->adres ?? '') . "\n" .
                       ($servis->musteri->ilce ?? '') . " / " . ($servis->musteri->il ?? '');
    ...
    $replace = [
        ...
        $tenant->tel1 ?? '',
        $tenant->tel2 ?? '',
        ($tenant->adres ?? '') . " " . ($tenant->ilce ?? '') . "/" . ($tenant->il ?? ''),
        $tenant->vergi_no ?? '',
        $tenant->vergi_dairesi ?? '',
        $tenant->mersis_no ?? '',
        $servis->id,
        $servis->musteri_id,
        $musteriBilgileri,
    ...
    // Türkçe karakterleri İngilizce karşılıklarına çevir (yazıcı uyumluluğu için)
    $turkish = ['İ', 'Ğ', 'Ü', 'Ş', 'Ö', 'Ç'];
    $english = ['I', 'G', 'U', 'S', 'O', 'C'];
