...
class ServiceController extends Controller
{
    ...
    private function getYetkiliServisIDleri($userId, $tenantId)
    {
        ...
        // Kullanıcıya atanan servisleri getir
        $servislerTable = ServiceStageAnswer::select([
                'id',
                'servisid',
                DB::raw('MAX(planid) as planid'),
                'soruid',
                'cevap',
                'cevapText'
            ])
        ...
        foreach ($servislerTable as $servisRow) {
            ...
            $perIDKontrol = StageQuestion::find($servisRow->soruid);
            
            if (!$perIDKontrol || !str_contains($perIDKontrol->cevap, 'Grup')) {
                continue;
            }
            ...
            if ($tarihDurum == 1) {
                ...
                if ($tarihCevap) {
                    ...
                    if ($bugunTimestamp == $tarihTimestamp) {
                        ...
                        // Saat kontrolü - varsayılan olarak 08:00'dan sonra göster
                        $baslangicSaati = strtotime('08:00');
                        
                        if ($simdikiSaat >= $baslangicSaati) {
                            $servisIzinler[] = $tarihCevap->servisid;
                        }
                    }
                }
            }
            // DURUM 2: Tarih YOK (tarihDurum = 0)
                ...
                if ($servis && $servis->planDurum == $servisRow->planid) {
                    $servisIzinler[] = $servisRow->servisid;
                }
                ...
                if ($servis) {
                    ...
                    if ($planSec && $planSec->kid == $userId) {
                        ...
                        $planTarih = explode(' ', $planSec->created_at)[0];
