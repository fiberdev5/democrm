    ...
    private function handleCallBridged($tenant, $data)
{
    $callerNumber = $data['caller_number'] ?? null;
    $direction = $data['direction'] ?? 'inbound';
    $callId = $data['id'] ?? $data['uuid']; // 🔥 call_id kullan!
...
{
    try {
        Log::info("🔵 sendCustomerCardToHipcall BAŞLADI", [
            'tenant_id' => $tenant->id,
            'customer_id' => $customer->id,
            'call_id' => $callId
        ]);
        ...
        Log::info("🔵 HipcallService oluşturuldu");
        
        // Site URL'ini al
        $baseUrl = rtrim(config('app.url'), '/');
        
        Log::info("🔵 Base URL alındı", ['base_url' => $baseUrl]);
        
        // Kart verisini hazırla
        $cardData = $hipcallService->prepareCustomerCard($customer, $tenant->id, $baseUrl);
        
        Log::info("🔵 Kart verisi hazırlandı");
        ...
        Log::info("🔵 sendCard tamamlandı", ['result' => $result]);
