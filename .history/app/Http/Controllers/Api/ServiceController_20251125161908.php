   ...
   public function myAssignedServiceDetail(Request $request, $id)
    ...
    public function addServicePhoto(Request $request)
    {
        ...
        try {
            ...
            // Servis başına fotoğraf sayısı kontrolü (maksimum 4)
    ...
    public function deleteServicePhoto(Request $request, $photo_id)
    {
        ...
        try {
            ...
            // GÜVENLİK KONTROLÜ: Kullanıcı bu fotoğrafı silebilir mi?
            // Sadece kendi yüklediği fotoğrafları silebilir (veya admin yetkisi varsa)
            if ($photo->kid != $user->user_id) {
                // Eğer admin kontrolü eklemek isterseniz:
                // if ($photo->kid != $user->user_id && !$user->hasRole(['Admin', 'Supervisor'])) {
                return response()->json([
                    'success' => false,
    ...
    private function mobilTeknisyenPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        foreach ($gunler as $gun) {
            ...
            // Personelin o gün yaptığı servisleri getir
            ...
            $musteriIptalIds = DB::table('service_plannings')
                ...
                ->where('gidenIslem', 244) // Müşteri iptal
                ->where('firma_id', $tenantId)
                ->pluck('servisid')
                ->toArray();
    ...
    /**
     * Atölye ustası prim hesaplama - Mobil
     */
    private function mobilAtolyeUstasiPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        foreach ($gunler as $gun) {
            ...
            $gunlukTamamlananlar = DB::table('service_plannings')
                ...
                ->whereIn('servisid', $validServisIds)
                ->where('pid', $personelId)
                ->where('gidenIslem', 252) // Teslimata hazır
                ->where('firma_id', $tenantId)
                ->groupBy('servisid')
                ->pluck('servisid');
...
/**
 * Mobil uygulama için servis fişi yazdır
 * GET /api/print-service-receipt/{servis_id}
 */
public function printServiceReceipt(Request $request, $servis_id)
{
    ...
    try {
        // Servisi ve ilişkili verileri getir
        $servis = Service::with([
            'musteri',
            'markaCihaz',
            'turCihaz',
...
private function olusturFisIcerigi($servis, $tenant, $user, $fisTemplate)
{
    ...
    // Fiş notlarını oluştur (Varsa)
    $fisNotlari = $this->getFisNotlari($servis->id, $tenant->id);

    // Müşteri bilgileri - Sade format (Etiketsiz)
    // Örnek:
    // KARDELEN KARA KOC
    // (531)-481-7775
    // ADRES...
    ...
    // Cihaz bilgileri - Sade format
    // Örnek:
    // DIGER CAMASIR MAKINASI
    // MANDALI KIRILDI
    $cihazBilgileri = ($servis->markaCihaz->marka ?? '') . " " . ($servis->turCihaz->cihaz ?? '') . "\r\n" .
                     ($servis->cihazAriza ?? '');

    // Uzun Yasal Uyarı Metni
    $yasalUyari = "ONEMLI UYARILAR\r\n\r\n" .
    "*(1) TUKETICININ BULUNDUGU YERDE TEKNIK SERVIS ISTASYONUNUN OLMAMASI HALINDE SATIS SONRASI HIZMETLERIN VERILMESINDEN , TUKETICIYE EN YAKIN YERDEKI TEKNIK SERVIS ISTASYONU SORUMLUDUR.\r\n" .
    "*(2) TUKETICIYE EN YAKIN YERDEKI TEKNIK SERVIS ISTASYONUN DA SATIS SONRASI HIZMET VERILMESININ MUMKUN OLMAMASI DURUMUN DA MALIN FIRMA MERKEZINE YA DA DIGER BIR TEKNIK SERVIS ISTASYUNA ULASTIRILMASI VE GERI GONDERILMESI ILE ILGILI OLARAK TUKETICIDEN NAKLIYE,POSTA,KARGO VEYA BENZERI HERHANGI BIR ULASIM GIDERI TALEP EDILEMEZ.\r\n" .
    "*(3) SERVIS ISTASYONLARININ BU YONETMELIGIN 11. MADDESINDE BELIRTILEN HUSUSLARI ICEREN BELGELERI DUZENLEMESI VE BUNLARIN BIRER NUSHASINI TUKETICILERE VERMESI ZORUNLUDUR.\r\n" .
    "*(4)KULLANIM OMRU SURESINCE MALIN TEKNIK SERVIS ISTASYONLARINDA KI BAKIM VE ONARIM SURESI AZAMI TAMIR SURESINI GECEMEZ. BU SURE GARANTI SURESI ICERISIN DE MALA ILISKIN ARIZANIN TEKNIK SERVIS ISTASYONUNA VEYA SATICIYA BILDIRIMI TARIHIN DE GARANTI SURESI DISINDA ISE MALIN TEKNIK SERVIS ISTASYONUNA TESLIM TARIHINDEN ITIBAREN BASLAR.TUKETICI ARIZA BILDIRIMI TELEFON, FAKS, E-POSTA IADELI TAAHHUTLU MEKTUP VE BENZERI BIR YOLLA YAPABILIR. UYUSMAZLIK HALINDE BILDIRIME ILISKIN ISPAT YUKUMLULUGU TUKETICIYE AITTIR.\r\n" .
    "*(5)MALIN TAMIRININ TAMAMLANDIGI TARIH TUKETICIYE TELEFON KISA MESAJ FAKS E-POSTA IADELI TAAHHUTLU MEKTUP VE BENZERI BIR YOLLA BILDIRILIR.UYUSMAZLIK HALINDE BILDIRIME ILISKIN ISPAT YUKUMLULUGU TUKETICIYE AITTIR.\r\n" .
    "*(6) MALIN GARANTI SURESI DISINDA SERVIS ISTASYONLARI TARAFINDAN VERILEN MONTAJ BAKIM VE ONARIM HIZMETIYLE ILGILI OLARAK BIR YIL ICERISINDE AYNI ARIZANIN TEKRARI HALIN DE TUKETICIDEN HERHANGI BIR ISIM ALTINDA UCRET ISTENEMEZ. TUKETICININ MALI KULLANIM KILAVUZUNDA YER ALAN HUSUSLARA AYKIRI KULLANMASINDAN KAYNAKLANAN ARIZALAR BU FIKRA KAPSAMI DISINDADIR.\r\n" .
    "*(7) MALIN GARANTI SURESI DISINDA GARANTI BELGESI ILE SATILMASI ZORUNLU OLMAYAN BIR PARCASININ SERVIS ISTASYONU TARAFINDAN DEGISTIRILMESI VEYA SATILMASI DURUMUNDA DEGISEN VEYA SATILAN PARCA ICIN ALTI AYDAN ASAGI OLMAMAK UZERE BIR GARANTI SURESI VERILIR.\r\n" .
    "*(8)DEGISEN PARCA TARAFIMIZDAN 1 YIL GARANTILIDIR. SERVIS UCRETIMIZ (130)TL'DIR.\r\n" .
    "ALINAN KAPORA ODEMELERININ ISLEM IPTALINDE IADESI KESINLIKLE YAPILMAMAKTADIR.\r\n\r\n" .
    "FIRMA UNVANI: " . ($tenant->firma_adi ?? 'TEKNIK PLUS') . "\r\n\r\n" .
    "BU FIS FATURA YERINE GECMEZ";

    // Placeholder'ları hazırla
    $search = [
        '[FIRMAADI]', // Bu artık şablonda kullanılmayabilir ama yedek dursun
        '[TEL]',      // Sadece Telefon
        '[ADRES]',    // Sadece Adres
        '[SNO]',      // Servis No
        '[MID]',      // Müşteri ID
        '[MUSTERIBILGILERI]', // Hazırladığımız blok
        '[CIHAZBILGILERI]',   // Hazırladığımız blok
        '[YAPILANISLEMLER]',
        '[KASAHAREKETLERI]',
        '[TEKNISYENADI]',
        '[TARIHSAAT]',
        '[MUSTERIADI]',
        '[FISNOTLARI]',
        '[YASALUYARI]',       // YENİ EKLENDİ
        '[MUSTERI-IMZASI]'    // YENİ EKLENDİ (Mobil app için placeholder)
    ];

    $replace = [
        $tenant->firma_adi ?? '',
        $tenant->tel1 ?? '', // Sadece numara
        ($tenant->adres ?? '') . " " . ($tenant->ilces->ilceName ?? '') . "/" . ($tenant->ils->name ?? ''),
        $servis->id,
        $servis->musteri_id,
        $musteriBilgileri,
        $cihazBilgileri,
        ...
        $fisNotlari,
        $yasalUyari,     // Yasal uyarı metni
        '[MUSTERI-IMZASI]' // Mobil uygulamanın imza resmini koyacağı tag'i olduğu gibi bırakıyoruz
    ];

    // Şablonu al (Veritabanındaki güncel şablon)
    $mesaj = $fisTemplate->fisTasarimi;

    // Placeholder'ları değiştir
    $mesaj = str_replace($search, $replace, $mesaj);
    ...
    // YENİ EKLENECEK KISIM: Satır sonlarını düzeltme
    // Tüm satır sonlarını önce temizleyip tek tip (\r\n) haline getiriyoruz.
    // Termal yazıcılar genellikle \r\n (Carriage Return + New Line) sever.
...
private function getYapilanIslemler($servisId, $servisDurum, $tenantId)
{
    ...
    // Servis durumuna göre kaç plan göstereceğimize karar ver
    if ($servisDurum == 239) { // Müşteriye teslim edildi
    ...
    foreach ($planlar as $plan) {
        ...
        foreach ($cevaplar as $cevap) {
            ...
            // Araç
                ...
                $arac = \App\Models\Car::find($cevap->cevap);
