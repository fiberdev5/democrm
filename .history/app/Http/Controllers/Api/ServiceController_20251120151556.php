...
class ServiceController extends Controller
{
    ...
    private function getBugunAtananServisIDleri($userId, $tenantId)
    {
        $bugun = Carbon::today();

        // 1. BUGÜN grup cevaplarından kendisine atanan servisleri bul
        $bugunAtananServisler = ServiceStageAnswer::where('firma_id', $tenantId)
            ->whereDate('created_at', $bugun) // BUGÜN ATANANLAR
        ...
        if (empty($bugunAtananServisler)) {
            return [];
        }

        // 2. Bu servislerden bugün içinde bu kullanıcının işlem yaptığı servisleri çıkar
        $bugunIslemYapilanServisler = ServicePlanning::whereIn('servisid', $bugunAtananServisler)
            ->where('kid', $userId)
        ...
        // Bugün atanan servislerden bugün işlem yapılanları çıkar
        $henuzIslemYapilmamisServisler = array_diff($bugunAtananServisler, $bugunIslemYapilanServisler);
