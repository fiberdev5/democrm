...
class HipcallWebhookController extends Controller
{
    public function handle(Request $request, $token)
    {
        try {
            ...
            Log::info('Hipcall Webhook - ' . $tenant->name, [
                'tenant_id' => $tenant->id,
    ...
    public function handleCallHangup($tenant, $data)
    {
        $uuid = $data['uuid'];
        ...
        Log::info("Arama bitti - {$tenant->name}", [
            'tenant_id' => $tenant->id,
            'uuid' => $uuid,
            'süre' => $duration
        ]);
    ...
    private function handleCallBridged($tenant, $data)
{
    ...
    Log::info("Çağrı bağlandı - {$tenant->name}", [
        'tenant_id' => $tenant->id,
        'call_id' => $callId,
        'caller' => $callerNumber,
        'full_data' => $data
