...
class ServiceController extends Controller
{
    ...
    {
        $servisIzinler = [];
        $bugunTarih = date('Y-m-d'); // Legacy format: 20.11.2025
        $bugunYmd = date('Y-m-d');

        // Mesai başlangıç saati kontrolü (Legacy: servis_zamanlama tablosu)
        // Varsayılan 08:00 alıyoruz eğer tablo yoksa.
        ...
        // Kullanıcıya atanan servisleri bul (cevap = user_id)
        // Not: Legacy kodda `GROUP BY servisid` ve `MAX(planid)` kullanılıyor.
        // Laravel'de bunu raw query yerine collection ile yapmak daha temiz olabilir ama performans için query:
        ...
        foreach ($servisCevaplari as $row) {
            ...
            $tarihDurum = $plan->tarihDurum; // 1 veya 0

            // --- DURUM 1: TARİH VAR ---
            if ($tarihDurum == 1) {
                // Tarih cevabını bul: [Tarih]
                $tarihCevap = ServiceStageAnswer::where('planid', $planId)
                    ->where('cevapText', '[Tarih]')
                    ->first();

                if ($tarihCevap) {
                    // Tarih formatı legacy kodda "/" olabiliyor, "." ya çeviriyoruz
                    $servisTarihStr = str_replace('/', '.', $tarihCevap->cevap);
                    
                    // Tarih Eşleşmesi (Bugün == Servis Tarihi)
                    if ($bugunTarih == $servisTarihStr) {
                        // Saat Kontrolü
                    ...
                    // Özel Durum: Depocu (Group 249)
                    // Tarih tutmasa bile durumu Parça Bekliyor (257) veya Tamamlandı (263) ise görsün.
                    if ($user->group_id == self::GROUP_DEPOCU) {
            ...
            // --- DURUM 2: TARİH YOK ---
                ...
                // Mantık A: Servisin şu anki aktif planı, kullanıcının dahil olduğu plan mı?
                if ($servis->planDurum == $planId) {
                    $servisIzinler[] = $servisId;
                }

                // Mantık B: Depocu değilse ve Planı oluşturan kişi bu kullanıcı ise ve BUGÜN oluşturduysa
                if ($user->group_id != self::GROUP_DEPOCU) {
                    // Planı yapan kişi (pid) kullanıcı mı?
                    if ($plan->pid == $user->user_id) {
                        // Planın oluşturulma tarihi bugün mü?
                        $planTarih = Carbon::parse($plan->created_at)->format('Y-m-d'); // veya column 'tarih'
