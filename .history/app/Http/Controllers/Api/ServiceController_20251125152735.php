    ...
    private function getAsamaDetaylari($planId, $tenantId)
{
   ...
   // ServiceController.php - printServiceReceipt metodunu güncelleyin
...
public function printServiceReceipt(Request $request, $servis_id)
{
    ...
    try {
        ...
        // Fiş şablonunu getir
        $fisTemplate = ReceiptTemplate::where('firma_id', $tenant->id)
            ->where('durum', 1)
            ->first();
        ...
        return response($fisIcerigi, 200)
...
private function olusturFisIcerigi($servis, $tenant, $user, $fisTemplate)
{
    ...
    // Fiş notlarını oluştur
    $fisNotlari = $this->getFisNotlari($servis->id, $tenant->id);

    // Müşteri telefon formatla
    $musteriTel1 = $servis->musteri->tel1 ?? '';
    if (!empty($musteriTel1) && strlen($musteriTel1) == 10) {
        // 5551234567 -> (555)-123-4567 formatına çevir
        $musteriTel1 = '(' . substr($musteriTel1, 0, 3) . ')-' . 
                       substr($musteriTel1, 3, 3) . '-' . 
                       substr($musteriTel1, 6, 4);
    }

    // Müşteri bilgileri
    $musteriBilgileri = $servis->musteri->adSoyad . "\n";
    if (!empty($musteriTel1)) {
        $musteriBilgileri .= $musteriTel1;
        if (!empty($servis->musteri->tel2)) {
            $musteriBilgileri .= " " . $servis->musteri->tel2;
        }
        $musteriBilgileri .= "\n";
    }
    $musteriBilgileri .= ($servis->musteri->adres ?? '') . "\n";
    $musteriBilgileri .= ($servis->musteri->state->ilceName ?? '') . " / " . ($servis->musteri->country->name ?? '');

    // Cihaz bilgileri
    $marka = $servis->markaCihaz ? $servis->markaCihaz->marka : '';
    $cihaz = $servis->turCihaz ? $servis->turCihaz->cihaz : '';
    $cihazBilgileri = trim($marka . " " . $cihaz);
    if (!empty($servis->cihazAriza)) {
        $cihazBilgileri .= "\n" . $servis->cihazAriza;
    }
    ...
    // Şablonu birleştir
    $mesaj = $fisTemplate->mesaj;
    if ($fisTemplate->alt_mesaj) {
        $mesaj .= "\n" . $fisTemplate->alt_mesaj;
    }
...
private function getYapilanIslemler($servisId, $servisDurum, $tenantId)
{
    ...
    foreach ($planlar as $plan) {
        ...
        $islemler .= '- ' . ($asama->asama ?? 'BILINMEYEN ASAMA') . ' -' . "\n";
        ...
        foreach ($cevaplar as $cevap) {
            ...
            // Parça
                ...
                foreach ($parcalar as $parca) {
                    if (strpos($parca, '---') !== false) {
                        ...
                        if ($stok) {
                            $parcaListesi[] = $stok->urunAdi . " (" . $adet . ")";
                        }
                    }
                }
                
                $islemler .= implode(", ", $parcaListesi) . "\n";
            }
            // Tarih
            else if ($soru->cevapTuru == '[Tarih]') {
                $tarih = $cevap->cevap;
                if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $tarih)) {
                    $tarih = \Carbon\Carbon::parse($tarih)->format('d/m/Y');
                }
            ...
            // Fiyat/Teklif
            else if ($soru->cevapTuru == '[Fiyat]' || $soru->cevapTuru == '[Teklif]') {
                $islemler .= $soru->soru . ': ' . number_format($cevap->cevap, 2, '.', '.') . " TL\n";
            }
            // Açıklama ve diğerleri
            else {
                $islemler .= $soru->soru . ': ' . $cevap->cevap . "\n";
            }
        }

        // Son plan değilse bir satır boşluk ekle
        if ($i < $planSayisi) {
            $islemler .= "\n";
        }
    }

    // Fiş notlarını da buraya ekle (eğer varsa)
    $fisNotlari = ServiceOptNote::where('servisid', $servisId)
        ->where('firma_id', $tenantId)
        ->orderBy('id', 'desc')
        ->get();
