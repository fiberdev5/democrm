    ...
    private function getAsamaDetaylari($planId, $tenantId)
{
   ...
   public function myAssignedServiceDetail(Request $request, $id)
    ...
    public function getServiceNotes(Request $request, $servis_id)
    {
        ...
        $notlar = DB::table('servis_fis_notlari') // Tablo adını kontrol edin
            ->where('servisid', $servis_id)
            ->where('firma_id', $tenant->id)
            ->orderBy('created_at', 'desc')
            ->get()
            ->map(function($not) {
                return [
                    'id' => (string) $not->id,
                    'kid' => (string) $not->kid,
                    'pid' => (string) $not->kid, // Eğer farklı bir değerse düzeltin
                    'servisid' => (string) $not->servisid,
