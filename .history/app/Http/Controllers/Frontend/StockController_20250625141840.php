...
public function AllStocks($tenant_id, Request $request)
{
    ...
    $query = Stock::select('stocks.*')
        ...
        foreach ($stocksForTotal as $stock) {
            ...
            $toplamFiyat += max($kalanAdet, 0) * ($stock->fiyat ?? 0);
        }
        ...
        return DataTables::of($query)
            ...
            ->addColumn('adet', function($row) {
                ...
                $toplamCikis = \App\Models\StockAction::where('stokId', $row->id)->whereIn('islem', [2,3])->sum('adet');
                $kalanAdet = $toplamGiris - $toplamCikis;
            ...
            ->addColumn('toplamTutar', function($row) {
                $toplamGiris = \App\Models\StockAction::where('stokId', $row->id)->where('islem', 1)->sum('adet');
                $toplamCikis = \App\Models\StockAction::where('stokId', $row->id)->whereIn('islem', [2,3])->sum('adet');
                $kalanAdet = max($toplamGiris - $toplamCikis, 0);
                $tutar = $kalanAdet * ($row->fiyat ?? 0);
                return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">' . number_format($tutar, 2, ',', '.') . ' ₺</a>';
            })
    ...
    // Ajax değilse view ve filtre verileri döner
...
public function UpdateStock(Request $request, $tenant_id, $id){
            ...
            $request->validate([
                'urunKodu' => [
                    'required',
                    'digits:13',
                    Rule::unique('stocks', 'urunKodu')->ignore($stock->id)->where(function ($query) use ($tenant_id) {
                        return $query->where('firma_id', $tenant_id);
                    }),
                ],
                'urunKategori' => 'required',
                'raf_id' => 'required',
            ],[
                'urunKodu.required' => 'Ürün kodu zorunludur.',
                'urunKodu.digits' => 'Ürün kodu tam 13 haneli olmalıdır.',
...
public function checkProductName(Request $request, $tenant_id)
{
    ...
    if ($stock) {
        return response()->json([
            'exists' => true,
            'edit_url' => route('edit.stock', ['tenant_id' => $tenant_id, 'id' => $stock->id])
        ]);
    }
...
public function consignmentDeviceData(Request $request, $tenant_id)
{
    ...
    foreach ($stocksForTotal as $stock) {
        ...
        foreach ($girisler as $giris) {
            ...
            $toplamGirisFiyat += $giris->adet * $giris->fiyat;
        }
        $cikisler = \App\Models\StockAction::where('stokId', $stock->id)
        ->where('islem', 2)
        ->get();
        $toplamCikisAdet = 0;
        $toplamCikisFiyat = 0;

        foreach ($cikisler as $cikis) {
            $toplamCikisAdet += $cikis->adet;
            $toplamCikisFiyat += $cikis->adet * $cikis->fiyat;
        }

        $kalanAdet = $toplamGirisAdet - $toplamCikisAdet;
        $kalanFiyat = $toplamGirisFiyat - $toplamCikisFiyat;
    ...
    return DataTables::of($query)
        ...
        ->addColumn('adet', function($row) {
            $toplamGiris = \App\Models\StockAction::where('stokId', $row->id)->whereIn('islem', [1, 4]) ->sum('adet'); //alış ve müşteriden iade
            $toplamCikis = \App\Models\StockAction::where('stokId', $row->id)->where('islem', 2)->sum('adet'); //serviste kullanım 
            $kalanAdet = $toplamGiris - $toplamCikis;
            return '<a href="javascript:void(0);" class="t-link editConsignment" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editConsignmentModal">' . $kalanAdet . '</a>';
        })
       ->addColumn('toplamTutar', function($row) {
    ...
    $cikisler = \App\Models\StockAction::where('stokId', $row->id)
        ->where('islem', 2)
        ->get();

    $girisTutar = $girisler->sum(function($item) {
        return $item->adet * $item->fiyat;
    });

    $cikisTutar = $cikisler->sum(function($item) {
        return $item->adet * $item->fiyat;
    });

    $tutar = max($girisTutar - $cikisTutar, 0);
...
// Konsinye cihaz ekleme formu gösterimi
...
public function EditConsignmentDevice($tenant_id, $id)
{
    ...
    // Kategori kontrolü (isteğe bağlı)
...
public function UpdateConsignmentDevice(Request $request, $tenant_id, $id)
{
    ...
    $request->validate([
        ...
        'raf_id' => 'required',
        // konsinye kategori otomatik 3 olduğu için kategori seçimini zorunlu yapma istersen
    ],[
        'urunKodu.required' => 'Ürün kodu zorunludur.',
        'urunKodu.digits' => 'Ürün kodu tam 13 haneli olmalıdır.',
        'urunKodu.unique' => 'Bu ürün kodu zaten mevcut. Lütfen farklı bir ürün kodu girin.',
    ...
    $stock->urunAdi = $request->urunAdi;
    $stock->urunKodu = $request->urunKodu;
    $stock->urunKategori = 3;  // Zorunlu konsinye kategori
    $stock->urunDepo = $request->raf_id;
    $stock->aciklama = $request->aciklama;
    $stock->fiyat = $request->fiyat;
    $stock->fiyatBirim = $request->fiyatBirim;
