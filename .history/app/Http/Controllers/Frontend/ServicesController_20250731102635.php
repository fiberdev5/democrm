    ...
    public function AllServices($tenant_id, Request $request) {
        ...
        if ($request->ajax()) {           
            ...
            if ($request->filled('ilceArama')) {
                // Service modelinizdeki 'musteri' ilişkisi üzerinden 'ilce' sütununu filtreleyin.
                $data->whereHas('musteri', function ($query) use ($request) {
                    $query->where('ilceName', $request->ilceArama);
                });
            }
