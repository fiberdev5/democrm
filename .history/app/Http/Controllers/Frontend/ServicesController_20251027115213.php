...
//Servis plan aşama düzenleme güncelleme fonksiyonu
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            // ÖNCEKİ KODDA EKSİK OLAN KISIM BURASIYDI
            // Servis planını bulup $servisPlan değişkenini tanımlıyoruz
            $servisPlan = ServicePlanning::where('id', $planid)
                ->where('firma_id', $tenant_id)
                ->first();
            ...
            // İşlemi yapan personeli güncelle (Bu kod da orijinalde vardı ve kalmalı)
            ...
            // İstekte herhangi bir stok veya konsinye checkbox'ı olup olmadığını kontrol et
            $hasStockData = false;
            $hasConsignmentData = false;
            foreach ($request->all() as $key => $value) {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($request->has($soruKey)) {
                    ...
                    if ($yeniCevap == 'Parca') {
                        // Eğer formdan parça ile ilgili (stokCheck ile başlayan) bir veri geldiyse işlemi yap.
                        // Gelmediyse, bu alana hiç dokunulmadı demektir, mevcut cevabı koru.
                        if ($hasStockData || !$cevap->cevap) { // Eğer zaten cevap boşsa veya yeni veri geldiyse güncelle
                             $parcaCevap = $this->processParcaSelection($request, $tenant_id);
                             $cevap->cevap = $parcaCevap;
                             $cevap->save();
                        }
                    } elseif ($yeniCevap == 'Konsinye Cihaz') {
                        // Eğer formdan konsinye ile ilgili bir veri geldiyse işlemi yap.
                        // Gelmediyse, mevcut cevabı koru.
                        if ($hasConsignmentData || !$cevap->cevap) { // Eğer zaten cevap boşsa veya yeni veri geldiyse güncelle
                            $konsinyeCevap = $this->processKonsinyeSelection($request, $tenant_id);
                            $cevap->cevap = $konsinyeCevap;
                            $cevap->save();
                        }
                    } else {
                        // Diğer soru tipleri için direkt güncelle
            ...
            // Artık $servisPlan tanımlı olduğu için bu satır hata vermeyecektir
            return response()->json([
                'success' => true,
