    ...
    public function SurveyStore(Request $request, $tenant_id, $servisId)
{
    ...
    try {
        $firma = Auth::user()->firma_id;
        $servis = Service::findOrFail($servisId)->where('firma_id', $firma)->firstOrFail();
        $anket = Survey::where('servisid', $servisId)->where('firma_id', $firma)->first();;
        $isNew = false;

        if (!$anket) {
            $anket = new Survey();
            $anket->servisid = $servisId;
            $anket->firma_id = $firma;
