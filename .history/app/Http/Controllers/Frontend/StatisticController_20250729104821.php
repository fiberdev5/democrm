...
///////////////////////////////////////////////////////Operatör Statistics//////////////////////////////////////////////////////////////////
    ...
    {
        if ($request->ajax()) {
            $tarih1 = $request->input('from_date')
                ? Carbon::createFromFormat('Y-m-d', $request->from_date)->startOfDay()
                : Carbon::now()->startOfMonth(); // Varsayılan başlangıç tarihi

            $tarih2 = $request->input('to_date')
                ? Carbon::createFromFormat('Y-m-d', $request->to_date)->endOfDay()
                : Carbon::now()->endOfDay(); // Varsayılan bitiş tarihi

            $query = DB::table('services as s')
                ->join('tb_user as u', 's.kayitAlan', '=', 'u.user_id')
                ->select('u.name', DB::raw('COUNT(s.id) as toplam'))
                ->whereBetween('s.kayitTarihi', [$tarih1->toDateString(), $tarih2->toDateString()])
                ->groupBy('u.user_id', 'u.name');

            return DataTables::of($query)
                ->addIndexColumn() // Sıra numarası ekler (isteğe bağlı)
                ->make(true);
        }

        // İlk yüklemede varsayılan tarihleri belirle
        $tarih1 = Carbon::now()->startOfMonth();
