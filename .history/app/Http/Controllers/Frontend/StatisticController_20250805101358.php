...
private function groupTechnicianServices($servicePlans, $technicianIds)
{
    ...
    $results = ServiceStageAnswer::select([
            ...
            'service_stage_answers.answer as technician_id'
        ])
        ->leftJoin('stage_questions', 'stage_questions.id', '=', 'service_stage_answers.question_id')
        ->whereIn('service_stage_answers.answer', $technicianIds)
        ->where('stage_questions.answer', 'LIKE', '%Grup%')
        ->whereIn('service_stage_answers.plan_id', $servicePlans)
        ->get();
    ...
    foreach ($results as $result) {
        ...
        $grouped[$result->technician_id][] = $result->service_id;
    }
