...
public function RegisterAction(Request $request) 
{
    ...
    // SMS gönderimi burada yapılabilir
    // SMS::send($validatedData['tel'], "Doğrulama kodunuz: " . $verificationCode);
    ...
    public function verifySmsCode(Request $request) {
        ...
        try {
            $this->createTenantAndUser($registrationData, $plan); // $planId'yi burdan gönderin
...
public function sendResetLinkEmail(Request $request)
{
    $request->validate([
        'email' => [
            ...
            function ($attribute, $value, $fail) {
                $tenant = Tenant::where('eposta', $value)->first();
                $user = User::where('eposta', $value)->first();
                
                if (!$tenant && !$user) {
                    $fail('Bu e-posta adresi sistemde kayıtlı değil.');
                }
            },
        ],
    ], [
    ...
    // Reset URL oluştur
    $resetUrl = route('password.reset', ['token' => $token, 'email' => $emailToSend]);

    // E-posta gönder
    try {
        Mail::to($emailToSend)->send(new PasswordResetMail(
            $resetUrl,
            $userName,
        ...
        // Activity log
        if ($tenant) {
            ActivityLogger::log('password_reset_requested', 'Şifre sıfırlama talebi (Tenant): ' . $emailToSend, [
                'module' => 'auth',
                'tenant_id' => $tenant->id,
                'user_name' => $emailToSend
            ]);
        } else {
            ActivityLogger::log('password_reset_requested', 'Şifre sıfırlama talebi (User): ' . $emailToSend, [
                'module' => 'auth',
...
public function resetPassword(Request $request)
{
    ...
    // ✅ 1. Önce Tenant tablosunda ara
    $tenant = Tenant::where('eposta', $request->email)->first();
    
    if ($tenant) {
        // Tenant'ın şifresini değiştir = Patron kullanıcısının şifresini değiştir
        ...
        // ✅ 2. Tenant değilse, User tablosunda ara
        $user = User::where('eposta', $request->email)->first();
        ...
        // Activity log
        ActivityLogger::log('password_reset_completed', 'Şifre başarıyla sıfırlandı (User)', [
            'module' => 'auth',
            'tenant_id' => $user->tenant_id,
            'user_id' => $user->user_id,
            'user_name' => $user->name,
