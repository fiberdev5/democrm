...
public function EditServicePlan($tenant_id, $planid) {
    ...
    // Personel Stoklarını al (eğer işlem parça teslim değilse)
    ...
    // --- Konsinye Cihazların hesaplanması ---
    // urunKategori'si 3 olan ve firma ile eşleşen tüm konsinye cihaz stoklarını çek
    $consignmentDevices = Stock::where('firma_id', $firma->id)
                               ->where('urunKategori', 3)
                               ->get();

    $toplamConsignmentAdedi = 0; // Toplam konsinye adedini sıfırla
    $calculatedConsignmentDevices = collect();

    // Mevcut plandaki konsinye cihaz cevabını ayrıştırma
    $currentPlanConsignmentId = null;
    $currentPlanConsignmentQuantity = 0;
    foreach ($planCevaplar as $planCevap) {
        $soru = App\Models\StageQuestion::find($planCevap->soruid);
        if ($soru && $soru->cevapTuru == "[Konsinye Cihaz]" && $planCevap->cevap) {
            list($idWithPrefix, $qty) = array_pad(explode('---', $planCevap->cevap), 2, 0);
            $currentPlanConsignmentId = str_replace('C_', '', $idWithPrefix);
            $currentPlanConsignmentQuantity = $qty;
            break; // Sadece ilk konsinye cihaz cevabını alıyoruz varsayımıyla
        }
    }
    ...
    foreach ($consignmentDevices as $device) {
        ...
        // Eğer mevcut stok pozitifse VEYA bu cihaz mevcut planda seçiliyse listeye ekle
        if ($device->current_stock_quantity > 0 || $device->id == $currentPlanConsignmentId) {
            $calculatedConsignmentDevices->push($device);
            // Toplam konsinye adedine sadece pozitif stokları ekle
            if ($device->current_stock_quantity > 0) {
                $toplamConsignmentAdedi += $device->current_stock_quantity;
            }
        }
    }
    // --- Konsinye stok hesaplaması sonu ---
    ...
    return view('frontend.secure.all_services.edit_service_plan', compact(
        ...
        'servis',
        'personellerAll',
        'stoklar', // Personel stokları
        'kullanici',
        'tenant_id',
        'toplamPersonelStokAdedi',
        'calculatedConsignmentDevices', // Güncel konsinye cihaz listesi
        'toplamConsignmentAdedi'    // Toplam konsinye adedi
    ));
}
public function UpdateServicePlan(Request $request, $tenant_id)
{
    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            $soru = App\Models\StageQuestion::find($cevap->soruid); // Get the question to check cevapTuru
