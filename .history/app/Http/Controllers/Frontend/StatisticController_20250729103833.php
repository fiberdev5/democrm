  ...
  public function OperatorStatistics(Request $request,$tenant_id)
    {
        ...
        ? Carbon::createFromFormat('d/m/Y', $request->tarih1)->startOfDay() 
        : now()->startOfMonth();

    $tarih2 = $request->input('tarih2') 
        ? Carbon::createFromFormat('d/m/Y', $request->tarih2)->endOfDay() 
        : now()->endOfDay();

    $istatistikler = DB::table('services as s')
        ->join('users as u', 's.kayitAlan', '=', 'u.user_id')
        ...
        return view('frontend.secure.statistics.operator_statistics', compact('tarih1', 'tarih2','tenant_id,'istatistikler'));
    }
