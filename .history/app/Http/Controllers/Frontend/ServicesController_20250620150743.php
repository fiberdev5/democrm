            ...
            foreach ($aciklamalar as $aciklama) {
                if (!empty($aciklama->cevap)) {
                    ...
                    if (strpos($soru->cevap, "[Grup") !== false) {
                        ...
                        // Teknisyen, Yardımcı Teknisyen gibi ID'ler için isim kontrolü
                        if (in_array($soru->soru, ['Teknisyen', 'Yardımcı Teknisyen'])) {
                            $teknisyen = User::where('tenant_id', $tenant_id)->where('user_id', $aciklama->cevap)->first();
                            $aciklamaMetni .= '<strong>' . $soru->soru . '</strong>: ' . ($teknisyen->name ?? $aciklama->cevap) . "<br>";
                        } else {
