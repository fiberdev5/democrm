...
public function StoreDealer(Request $request, $tenant_id)
{
    ...
    $firmaSlug = Str::slug($firma->username, '_'); 
    ...
    if ($request->hasFile('belgePdf')) {
        ...
        foreach ($files as $file) {
            $fileName = time() . '_' . $username . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
             $path = $file->store('dealers-documents/' . $firmaSlug, 'public'); // storage/app/public/dealers-documents
            $belgePdfPaths[] = $path;
        }
    }
...
public function UpdateDealer(Request $request, $tenant_id, $id)
{
    ...
    // Firma ismini slug haline getir
    $firmaSlug = Str::slug($firma->name, '_');
    ...
    if ($request->hasFile('belgePdf')) {
        ...
        foreach ($files as $file) {
            ...
            $path = $file->store('dealers-documents/' . $firmaSlug, 'public');
            $yeniBelgeler[] = $path;
        }
        ...
        if ($toplamBelgeSayisi > 2) {
            ...
            for ($i = 0; $i < $silinecekSayi; $i++) {
                if (isset($mevcutBelgeler[$i]) && Storage::disk('local')->exists($mevcutBelgeler[$i])) {
                    Storage::disk('local')->delete($mevcutBelgeler[$i]);
                }
