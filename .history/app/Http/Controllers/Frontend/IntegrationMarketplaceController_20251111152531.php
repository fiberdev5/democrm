...
public function showHipcallContacts($tenant_id)
{
    try {
        ...
        // Sadece listeyi çek (detaysız, hızlı)
        $hipcallService = new \App\Services\HipcallService($tenant_id);
        
        // İlk sayfayı hızlıca göster
        $result = $hipcallService->getContactsQuick(1, 100);
        
        if ($result['success']) {
            $contacts = $result['contacts'];
            
            return view('frontend.secure.integrations.hipcall-contacts-lazy', [
                'tenant' => $tenant,
                'contacts' => $contacts,
