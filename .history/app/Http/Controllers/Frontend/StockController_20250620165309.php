...
public function StokActions($tenant_id, $stock_id, Request $request)
{
    ...
    // Eğer islem parametresi varsa filtrele
    if ($request->has('islem') && $request->islem != 0) {
        $query->where('stock_actions.islem', $request->islem);
    }

    $stokHareketleri = $query->orderBy('stock_actions.id', 'desc')->get();
