...
class ImpersonationController extends Controller
{
    ...
    {
        // Impersonation oturumu mevcut mu kontrol et
        ...
        // Impersonation kaydını sonlandır (veritabanında)
        if ($impersonationId) {
        ...
        if ($originalUser) {
            // Orijinal kullanıcı olarak giriş yap
            ...
            // Eğer orijinal kullanıcı bir Super Admin ise, Super Admin dashboard'una yönlendir
            if ($originalUser->isSuperAdmin()) {
                ...
                // Değilse, kendi tenant'ının ana sayfasına yönlendir
                $notificationMessage = 'Kendi hesabınıza geri döndünüz.';
                $redirectRoute = route('secure.home', ['tenant_id' => $originalUser->tenant_id]);
            }
        } else {
            // Orijinal kullanıcı bulunamazsa, bir hata veya genel bir ana sayfaya yönlendirme yapabiliriz.
            // Bu durum genellikle olmamalı, çünkü impersonator_id session'da vardır.
            $notificationMessage = 'Orijinal kullanıcı bulunamadı, giriş sayfasına yönlendiriliyorsunuz.';
            $redirectRoute = route('login'); // Varsayılan giriş sayfasına yönlendirebilirsiniz.
        }
