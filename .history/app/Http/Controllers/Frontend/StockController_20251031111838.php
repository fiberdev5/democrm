...
{
    $token = $request->input('brand_form_token');
    if (empty($token)) {
...
{
    $token = $request->input('device_form_token');
    if (empty($token)) {
        return response()->json(['error' => 'Geçersiz token'], 400);
    }
...
public function StoreStockSupplier(Request $request, $tenant_id)
{
    $token = $request->input('supplier_form_token');
    if (empty($token)) {
        return response()->json(['error' => 'Geçersiz token'], 400);
    }
    
    $cacheKey = 'supplier_form_token_' . $token;
    if (Cache::has($cacheKey)) {
        return response()->json(['error' => 'Bu form zaten gönderildi'], 400);
    }
    Cache::put($cacheKey, true, now()->addMinutes(10));
