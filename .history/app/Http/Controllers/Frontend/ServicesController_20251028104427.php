    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($request->has($soruKey)) {
                ...
                if ($yeniCevap == 'Parca') {
                    // ÖNEMLİ: Eski cevabı da gönder
                    $parcaCevap = $this->processParcaSelection($request, $tenant_id, $cevap->cevap);
                     // DEBUG LOG
                    \Log::info('Parça Cevap', [
                        'yeni_cevap' => $parcaCevap,
                        'eski_cevap' => $cevap->cevap
                    ]);
                    ...
                    \Log::info('Kayıt Sonrası', [
                        'kaydedilen' => $cevap->cevap
                    ]);
...
private function processParcaSelection(Request $request, $tenant_id, $eskiCevap = null)
{
    ...
    // YENİ SEÇİLEN PARÇALARI AL - HEM ESKİ HEM YENİ FORMATI DESTEKLE
    $yeniSecimler = [];
    
    // ESKİ FORMAT: stokCheck{id}
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'stokCheck') !== false && strpos($key, 'stokAdet') === false) {
            $stokId = (int) filter_var($key, FILTER_SANITIZE_NUMBER_INT);
            $adet = abs($request->input("stokAdet{$stokId}", 1));
            
            if ($stokId > 0) { // ✅ 0 ID'yi filtrele
                $yeniSecimler[$stokId] = $adet;
                
                // Sadece yeni eklenen parçalar için işlem yap
    ...
    // YENİ FORMAT: parca[stageId][stockId] (JavaScript'ten gelen)
    if ($request->has('parca')) {
        $parcaData = $request->input('parca');
        foreach ($parcaData as $stageId => $stocks) {
            foreach ($stocks as $stokId => $value) {
                $stokId = (int) $stokId;
                if ($stokId > 0 && !isset($yeniSecimler[$stokId])) {
                    $adet = abs($request->input("adet.{$stageId}.{$stokId}", 1));
                    $yeniSecimler[$stokId] = $adet;
