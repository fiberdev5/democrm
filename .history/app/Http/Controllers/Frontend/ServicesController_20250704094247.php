...
class ServicesController extends Controller
{
    ...
    public function ServiceStageQuestionShow($tenant_id ,$asamaid, $serviceid) {
        ...
        // Normal servis işlemleri       
            ...
            // Konsinye cihazlar
            $consignmentDevices = Stock::where('firma_id', $firma->id)
                                   ->where('urunKategori', 3) 
                                   ->get();
        
            $toplamConsignmentAdedi = 0; // Toplam konsinye adedini sıfırla

            foreach ($consignmentDevices as $device) {
                $girisAdet = StockAction::where('stokId', $device->id)
                                        ->whereIn('islem', [1, 4]) // 1: Alış, 4: Müşteriden İade
                                        ->sum('adet');
                $cikisAdet = StockAction::where('stokId', $device->id)
                                        ->where('islem', 2) // 2: Serviste Kullanım
                                        ->sum('adet');
                // Her bir cihazın anlık stok miktarını hesapla ve modele ata
                ...
                if ($device->current_stock_quantity > 0) {
                    $toplamConsignmentAdedi += $device->current_stock_quantity;
                }
            }
            ...
            return view('frontend.secure.all_services.service_stage_questions_show', 
                    compact('stage_questions', 'stage_id', 'service_id', 'firma', 'islem', 'personeller', 'araclar','stoklar','toplamPersonelStokAdedi','consignmentDevices','toplamConsignmentAdedi'));
    ...
    {
        // --- PARÇA STOK KONTROLÜ (Mevcut haliyle bırakıldı) ---
            ...
            if (strpos($key, 'soru') !== false && $value == "Parca") {
                foreach ($request->all() as $stokKey => $stokValue) {
                    if (strpos($stokKey, 'stokCheck') !== false) {
                        ...
                        $stokHareketleri = StockAction::where('stokId', $stokId)->get();
                        ...
                        if ($stokHareketleri->count() > 0) {
                            ...
                            if ($toplam <= 0 || $adet > $toplam) {
                                ...
                                $urunAdi = $stok ? mb_convert_case($stok->urun_adi, MB_CASE_TITLE, "UTF-8") : "Bilinmeyen Ürün";
                                return "STOKHATA: **{$urunAdi}** stok adeti yetersizdir.";
                            }
                        } else {
                            $stok = Stock::where('id', $stokId)->first();
                            $urunAdi = $stok ? mb_convert_case($stok->urun_adi, MB_CASE_TITLE, "UTF-8") : "Bilinmeyen Ürün";
                ...
                if (isset($gelenIslem['id']) && $gelenIslem['id'] == 238) {
                    $stokSecildi = false;
                    foreach ($request->all() as $key => $value) {
        ...
        // --- KONSİNYE CİHAZ STOK KONTROLÜ (Yeni eklenen kısım) ---
        foreach ($request->input('soru') as $soruId => $cevapTuru) {
            if ($cevapTuru == "Konsinye Cihaz") {
                $secilenKonsinyeCihazIdVeOnEki = $request->input("parca.{$soruId}.consignment_select");
                $konsinyeCihazAdedi = abs($request->input("adet.{$soruId}.selected_consignment_quantity", 0));

                // Konsinye cihaz seçilmiş ve adet girilmişse kontrol edilir
                if ($secilenKonsinyeCihazIdVeOnEki && $konsinyeCihazAdedi > 0) {
                    $stokId = (int) str_replace('C_', '', $secilenKonsinyeCihazIdVeOnEki);

                    // Konsinye cihazın mevcut stok miktarını hesaplarız
                    $girisAdet = StockAction::where('stokId', $stokId)
                                            ->whereIn('islem', [1, 4]) // 1: Alış, 4: Müşteriden İade
                                            ->sum('adet');
                    $cikisAdet = StockAction::where('stokId', $stokId)
                                            ->whereIn('islem', [2, 3]) // 2: Serviste Kullanım, 3: Personel Üzerinden Çıkış
                                            ->sum('adet');
                    $mevcutStok = $girisAdet - $cikisAdet;

                    if ($mevcutStok < $konsinyeCihazAdedi) {
                        $konsinyeCihaz = Stock::find($stokId);
                        $urunAdi = $konsinyeCihaz ? mb_convert_case($konsinyeCihaz->urun_adi, MB_CASE_TITLE, "UTF-8") : "Bilinmeyen Konsinye Cihaz";
                        return "STOKHATA: **{$urunAdi}** konsinye cihaz stok adeti yetersizdir. Mevcut: {$mevcutStok}, Talep Edilen: {$konsinyeCihazAdedi}.";
                    }
                } else if ($konsinyeCihazAdedi > 0 && !$secilenKonsinyeCihazIdVeOnEki) {
                    // Adet girilmiş ancak cihaz seçilmemişse hata verilir
                    return "STOKHATA: Konsinye cihaz adedi girdiniz ancak bir cihaz seçmediniz.";
                }
            }
        }

        // Tüm kontroller başarılıysa null döner
        return null;
    }


private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem, $isUpdate = false)
    {
        if ($request->has('soru')) {
            foreach ($request->input('soru') as $soruId => $cevapTuru) {
                // Mevcut cevabı bulmaya çalış, yoksa yeni bir tane oluştur
                $serviceStageAnswer = ServiceStageAnswer::firstOrNew([
                    'firma_id' => $tenantId,
                    'servisid' => $servisId,
                    'planid' => $planId,
                    'soruid' => $soruId,
                ...
                if ($cevapTuru == "Parca") {
                    ...
                    if ($request->has("parca.{$soruId}")) {
                        foreach ($request->input("parca.{$soruId}") as $stokId => $value) {
                            ...
                            // Parça İşlemleri (Teslim Et veya Kullan)
                            if (isset($gelenIslem['id']) && $gelenIslem['id'] == 238) { // "Parça Teslim Et" aşaması için ID 238
                    ...
                    $serviceStageAnswer->cevap = implode(', ', $stokCevapArray);

                } elseif ($cevapTuru == "Konsinye Cihaz") {
                    $selectedConsignment = $request->input("parca.{$soruId}.consignment_select");
                    $consignmentName = $request->input("parca.{$soruId}.consignment_name");
                    $adet = abs($request->input("adet.{$soruId}.selected_consignment_quantity", 1));

                    if ($selectedConsignment && strpos($selectedConsignment, 'C_') === 0) {
                        $stokId = (int) str_replace('C_', '', $selectedConsignment);
                        
                        // Konsinye cihaz kullanımı ve FIRMWARE GÜNCELLEME İŞLEMİ
                        // Bu kısım useConsignmentDevice metoduna taşındı, çünkü hem stok düşme hem de firmware işlemini kapsayacak.
                        $this->useConsignmentDevice(
                            $stokId,
                            $adet,
                            $servisId,
                            $planId,
                            $tenantId,
                            $request->hasFile('consignment_firmware_file') ? $request->file('consignment_firmware_file') : null,
                            $request->input('consignment_firmware_version')
                        );

                        $serviceStageAnswer->cevap = $consignmentName . " - Adet: " . $adet; // Cevaba adeti de ekleyelim
                    } else {
                        // Eğer konsinye cihaz seçimi yapılmadıysa, cevabı boş bırak veya bir hata logla
                        $serviceStageAnswer->cevap = '';
                        \Log::warning("Konsinye cihaz seçimi yapılmadı veya geçersiz: Soru ID: {$soruId}, Plan ID: {$planId}");
                    }
                } else {
                    // Diğer cevap türleri
                    $serviceStageAnswer->cevap = $cevap;
                }
                
                // Cevap boş değilse veya update işlemiyse kaydet/güncelle
                if (!empty($serviceStageAnswer->cevap) || $isUpdate) {
                    $serviceStageAnswer->save();
                }
            }
        }
    }
private function useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId) {
    StockAction::create([
        'firma_id' => $tenantId,
        'kid' => auth()->id(),
        'stokId' => $stokId,
        'islem' => 2, // '2' tüm stoklar için "Serviste Kullanım" ise
        'servisid' => $servisId,
        'adet' => $adet,
        'planId' => $planId,
        'created_at' => now(),
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        $toplamPersonelStokAdedi = $stoklar->sum(function($stok) use ($personel_id) {
         ...
         // Konsinye Cihazları getir ve anlık stoklarını hesapla
        $consignmentDevices = Stock::where('firma_id', $firma->id)
                                    ->where('urunKategori', 3) // urunKategori 3 olanlar konsinye cihazlar
                                    ->get();
        ...
        foreach ($consignmentDevices as $device) {
            $girisAdet = StockAction::where('stokId', $device->id)
                                    ...
                                    ->sum('adet');
            $cikisAdet = StockAction::where('stokId', $device->id)
                                    ->whereIn('islem', [2, 3]) // 2: Serviste Kullanım, 3: Personel Üzerinden Çıkış (Parça Teslim Et)
                                    ->sum('adet');
...
public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            // Gelen verileri doğrula (Yeni validasyonlar eklendi)
            $request->validate([
                'planid' => 'required|exists:service_plannings,id',
                'planIslemiYapan' => 'nullable|exists:users,id',
                'soru' => 'required|array',
                // Konsinye cihaz için eklenecek özel validasyon kuralları:
                'parca.*.consignment_select' => 'nullable|string', // "C_ID" formatında
                'adet.*.selected_consignment_quantity' => 'nullable|integer|min:1',
                'parca.*.consignment_name' => 'nullable|string', // Konsinye cihaz adı
                // Eğer bir firmware dosyası da yüklenecekse buraya ekleyeceğiz:
                'consignment_firmware_file' => 'nullable|file|mimes:bin,hex,zip,ota|max:20480', // Örn: bin/hex/zip/ota uzantılı, maks 20MB
                'consignment_firmware_version' => 'nullable|string|max:50', // Manuel versiyon girişi
            ]);
            ...
            // Plan cevaplarını güncelle
            // Mevcut cevapları silip yeniden oluşturmak yerine, güncelliyoruz.
            // Bu kısım, mevcut [Parca] ve Konsinye Cihaz işlemleriyle uyumlu hale getirilecek.
            // soruCevaplariniIsle metodu hem yeni kayıt hem de güncelleme için kullanılacak şekilde düzenlendi.
            $this->soruCevaplariniIsle($request, $servisPlan->servisid, $planid, $tenant_id, ['id' => $servisPlan->gelenIslem], true); // true parametresi güncelleme olduğunu belirtir

            // Servis durumunu ve planId'yi güncelle
            // Bu kısım, servis planlamasıyla ilgili olduğu için,
            // servis durumunun yeni aşamaya geçirilmesi mantığı burada olmalı.
            $servis = Service::find($servisPlan->servisid);
            if ($servis) {
                // Burada mevcut "gidenIslem" değerini kullanarak servis durumunu güncelleyebilirsiniz
                // veya formdan gelen "gidenIslem" eğer servis planının "gidenIslem"ini değiştiriyorsa
                // $servis->servisDurum = $request->input('gidenIslem'); // Eğer formda gidenIslem alanı varsa
                // $servis->planDurum = $planid;
                // $servis->save();
                // Not: Sizin kodunuzda 'gidenIslem' request içinde görünmüyor, bu yüzden mevcut değerini kullanıyorum.
                // Eğer frontend'den 'gidenIslem' geliyorsa onu kullanabilirsiniz.
            }
            ...
            // Validasyon hatalarını yakala ve döndür
            return response()->json([
                'success' => false,
                'error' => 'Validasyon Hatası: ' . $e->getMessage(),
                'errors' => $e->errors()
            ], 422); // Unprocessable Entity
        } catch (\Exception $e) {
            \Log::error('Servis planı güncelleme sırasında hata oluştu: ' . $e->getMessage(), [
