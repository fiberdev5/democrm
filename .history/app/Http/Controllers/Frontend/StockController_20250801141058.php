...
// Personel filtresi (pid) doğru şekilde uygulanıyor
    if ($request->filled('personel')) {
        // stock_actions tablosuna join yapıyoruz
        $query->join('stock_actions as actions', 'actions.stokId', '=', 'stocks.id')
              // Belirli personele ait işlemleri (pid) ve çıkış işlemleri (islem = 3) filtreliyoruz
              ->where('actions.pid', $request->personel)
              ->where('actions.islem', 3)
              ->distinct(); // Birden fazla aynı stokun gelmemesi için distinct() kullanıyoruz
    }
...
// Sorgunun SQL halini ve bind edilen parametrelerini log dosyasına yazdır
\Log::info('SQL Query:', [
    'query' => $query->toSql(),
