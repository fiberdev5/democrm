...
{
    $consignmentCihazIds = $request->input('consignment_device_' . $soru_id, []);
    $consignmentAdetler = $request->input('consignment_quantity_' . $soru_id, []);
    $personel_id = auth()->user()->user_id;

    foreach ($consignmentCihazIds as $index => $stokId) {
        $adet = intval($consignmentAdetler[$index] ?? 0);
        if ($adet <= 0) continue;
...
{
    $cihazlar = $request->input('consignment_device_' . $soru_id, []);
    $adetler = $request->input('consignment_quantity_' . $soru_id, []);
    $cevaplar = [];

    foreach ($cihazlar as $index => $stokId) {
        $adet = intval($adetler[$index] ?? 0);
        if ($adet <= 0) continue;
        $cevaplar[] = "ID:" . $stokId . "-Adet:" . $adet;
    }

    return implode(', ', $cevaplar);
}
