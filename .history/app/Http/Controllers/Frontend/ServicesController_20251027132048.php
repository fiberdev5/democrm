        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($request->has($soruKey)) {
                    ...
                    if ($yeniCevap == 'Parca') {
                        // Blade'e eklediğimiz gizli input'un gelip gelmediğini kontrol et.
                        // Eğer bu input geldiyse, bu alanın güncellenmesi gerektiğini anlıyoruz.
                        if ($request->has("parca_form_gonderildi.{$cevap->id}")) {
                            $parcaCevap = $this->processParcaSelection($request, $tenant_id);
                            // processParcaSelection'dan boş string gelse bile (tümü kaldırıldıysa)
                            // bunu veritabanına yansıtıyoruz.
                            $cevap->cevap = $parcaCevap;
                            $cevap->save();
                        }
                        // Eğer gizli input gelmediyse, bu alana dokunmuyoruz.
                        
                    } elseif ($yeniCevap == 'Konsinye Cihaz') {
                        // Aynı kontrolü konsinye cihaz için de yapıyoruz.
                        if ($request->has("konsinye_form_gonderildi.{$cevap->id}")) {
