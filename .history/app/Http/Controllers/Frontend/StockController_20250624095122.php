...
class StockController extends Controller
{
     ...
     public function AllStocks($tenant_id, Request $request){
        ...
        if ($request->filled('durum') && $request->durum != 2) {
            if ($request->durum == 1) {
                // Stokta kalan > 0
                $query->whereHas('stockActions', function($q) {
                    $q->selectRaw('stokId, SUM(CASE WHEN islem=1 THEN adet ELSE 0 END) - SUM(CASE WHEN islem IN (2,3) THEN adet ELSE 0 END) as kalan')
                      ->havingRaw('kalan > 0');
                });
            } else if ($request->durum == 0) {
                // Stokta kalan <= 0
                $query->whereHas('stockActions', function($q) {
                    $q->selectRaw('stokId, SUM(CASE WHEN islem=1 THEN adet ELSE 0 END) - SUM(CASE WHEN islem IN (2,3) THEN adet ELSE 0 END) as kalan')
                      ->havingRaw('kalan <= 0');
        ...
        // Personel filtresi (stok hareketleri üzerinden filtreleme)
