...
private function getServicePlansForDates($tarihListesi, $tenant_id, $cihazTur = null)
{
    ...
    $tarihListesiYmd = array_filter($tarihListesiYmd);
    ...
    $query = DB::table('service_stage_answers as sc')
        ...
        ->where(function($q) use ($tarihListesi, $tarihListesiYmd) {
            if (!empty($tarihListesi)) {
                $q->whereIn('sc.cevap', $tarihListesi);
            }
            if (!empty($tarihListesiYmd)) {
                $q->orWhereIn('sc.cevap', $tarihListesiYmd);
            }
        });
