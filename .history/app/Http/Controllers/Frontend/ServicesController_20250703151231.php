   ...
   private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem)
{
    if ($request->has('soru')) {
        foreach ($request->input('soru') as $soruId => $cevap) {
            if ($cevap == "Parca") { // Bu, gizli input değeri 'Parca' olduğunda çalışır
                // $gelenIslem bir dizi olduğu için 'id' anahtarını kullanarak aşama ID'sini gönderin
                $this->parcaIslemleriniYap($request, $servisId, $planId, $tenantId, $soruId, $gelenIslem['id']);
                 } elseif ($cevap == "Konsinye Cihaz") { // Hidden input'tan gelen değer 'Konsinye Cihaz' ise
                $selectedConsignment = $request->input("parca.{$soruId}.consignment_select");
                if ($selectedConsignment && strpos($selectedConsignment, 'C_') === 0) { // 'C_' önekiyle gelen değeri kontrol et
                    ...
                    ServiceStageAnswer::create([
                        ...
                        'planid' => $planId,
                        'soruid' => $soruId,
                       'cevap' => json_encode(['type' => 'consignment_device', 'stok_id' => $stokId, 'adet' => $adet]), // JSON formatı
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
...
private function useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId) {
    // Konsinye cihazlar için stok düşme mantığı
    // Bu, doğrudan 'Stock' tablosunu güncellemeyi veya
    // konsinye kullanımı için belirli bir 'islem' tipiyle 'StockAction' kaydetmeyi içerecektir.
    StockAction::create([
        'firma_id' => $tenantId,
        'kid' => auth()->id(),
        'stokId' => $stokId,
    ...
    // Not: Konsinye cihazlar genellikle personel bazında değil, genel stokta takip edilir.
    // Bu yüzden burada PersonelStock güncellemesi olmaz.
}
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
    $stokCevapArray = []; // Cevapları toplamak için bir dizi kullanın

    // İstekten belirli soruId için 'parca' girişini kontrol edin
    if ($request->has("parca.{$soruId}")) {
        // 'parca' dizisinin içindeki her bir stokId ve değeri üzerinde dönün
        foreach ($request->input("parca.{$soruId}") as $stokId => $value) {
            // Eğer checkbox işaretlenmişse, $value genellikle $stokId'ye eşit olacaktır.
            // Veya sadece $stokId'nin dizide anahtar olarak var olup olmadığını kontrol edebilirsiniz.
            // Quantity input'un adı adet[{{ $stage->id }}][{{ $stokId }}] olduğu için doğrudan erişim.
            $adet = abs($request->input("adet.{$soruId}.{$stokId}", 1));

            $stokCevapArray[] = "{$stokId}---{$adet}";

            // Stok hareketini gerçekleştiren fonksiyonu çağırın
            // $gelenIslem'in burada sayısal ID (örn. 238) olduğundan emin olun
    ...
    if (!empty($stokCevap)) {
        ServiceStageAnswer::create([
            ...
            'planid' => $planId,
            'soruid' => $soruId,
            'cevap' => $stokCevap, // Biçimlendirilmiş string buraya kaydedilir
            'created_at' => now(),
            'updated_at' => now()
        ]);
    }
