  ...
  public function OperatorStatistics(Request $request)
    {
        ...
        $tarih1 = $request->input('tarih1', $today);
        $tarih2 = $request->input('tarih2', $today);

        return view('statistics.operator-statistics', compact('tarih1', 'tarih2'));
    }

    /**
     * Operatör istatistikleri tablosu için verileri getirir.
     * Bu metod AJAX isteği ile çağrılacak.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function getOperatorStatisticsData(Request $request)
    {
        $tarihler = explode("---", $request->input("optTabloGetir"));
        $tarih1Str = $tarihler[0];
        $tarih2Str = $tarihler[1];
        ...
        try {
            ...
            // Hatalı tarih formatı durumunda varsayılan değerler veya hata döndür
            $tarih1 = Carbon::now()->startOfDay();
            $tarih2 = Carbon::now()->endOfDay();
        }

        $query = Service::query();

        // Sadece durum = 1 olan servisleri filtrele
        $query->where('durum', 1);

        // Tarih aralığı filtresi
        $query->whereBetween('kayitTarihi', [$tarih1, $tarih2]);

        // Mevcut kodunuzda yorum satırı yapılmış filtreleri kaldırıyoruz
        // if (!empty($request->input("servisKaynak"))) {
        //     $query->where('servisKaynak', $request->input("servisKaynak"));
        // }
        // if (!empty($request->input("cihazMarka"))) {
        //     $query->where('cihazMarka', $request->input("cihazMarka"));
        // }
        // if (!empty($request->input("cihazTur"))) {
        //     $query->where('cihazTur', $request->input("cihazTur"));
        // }

        $getTable = $query->select('kayitAlan')
            ->selectRaw('COUNT(kayitAlan) as toplam')
            ->with(['kayitAlanUser' => function($q) {
                $q->select('user_id', 'name'); // Sadece gerekli sütunları çek
            }])
            ->groupBy('kayitAlan')
            ->orderBy('toplam', 'desc') // Yüksekten düşüğe sırala
            ->get();

        // Tablo HTML içeriğini döndür
        return response()->view('statistics.partials.operator-statistics-table', compact('getTable', 'tarih1Str', 'tarih2Str'));
    }
}
