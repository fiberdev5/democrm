...
private function getServicePlansForDates($tarihListesi, $tenant_id, $cihazTur = null)
{
    ...
    // Tarih listesi zaten Y-m-d formatında geliyorsa, direkt kullanın
    // Yoksa, sc.cevap'ın veritabanındaki formatına göre ayarlayın.
    // Eğer veritabanında d/m/Y varsa, $tarihListesi'ni d/m/Y'ye çevirin:
    /*
    $tarihListesiDmY = array_map(function($tarih) {
        return Carbon::createFromFormat('Y-m-d', $tarih)->format('d/m/Y');
    }, $tarihListesi);
    $query->whereIn('sc.cevap', $tarihListesiDmY);
    */
