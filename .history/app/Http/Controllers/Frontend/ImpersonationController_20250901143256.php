   ...
   public function stop(Request $request)
{
    ...
    $impersonatorId = session('impersonator_id');   // ✅ gerçek ID süper 
    $impersonationId = session('impersonated_user_id');  //meryem
    ...
    $originalUser = User::find($impersonationId);
    if ($originalUser) {
        Auth::logout();
        $exuser = User::find($impersonatorId);
        Auth::login($exuser);
