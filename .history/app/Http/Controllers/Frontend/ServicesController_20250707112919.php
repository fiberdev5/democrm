...
// Konsinye cihazlar (urunKategori = 3 olan stoklar)
...
foreach ($consignmentDevices as $device) {
    $girisAdet = StockAction::where('stokId', $device->id)
        ->whereIn('islem', [1, 4]) // Alış veya müşteri iade
        ->sum('adet');
...
private function getUpdatedParcaAnswer(Request $request)
{
    ...
    private function updateConsignmentSelection($request, $tenant_id, $servisId, $planid, $soru_id)
    {
        $personel_id = auth()->user()->user_id;

        $stokIdRaw = $request->input("parca.$soru_id.consignment_select");
        $adetRaw   = $request->input("adet.$soru_id.selected_consignment_quantity");
        $consignmentName = $request->input("parca.$soru_id.consignment_name"); // Cihaz adını alıyoruz

        $stokId = str_replace("C_", "", $stokIdRaw);
        $adet   = intval($adetRaw ?? 0);

        if ($stokId && $adet > 0) {
            // Önce mevcut ServiceStageAnswer kaydını sil
            ServiceStageAnswer::where('planid', $planid)
                ->where('soruid', $soru_id)
                ->where('firma_id', $tenant_id)
                ->delete();

            // Yeni ServiceStageAnswer kaydı oluştur
            ServiceStageAnswer::create([
                'firma_id' => $tenant_id,
                'kid' => $personel_id,
                'servisid' => $servisId,
                'planid' => $planid,
                'soruid' => $soru_id,
                'cevap' => "ID:" . $stokId . "-Adet:" . $adet . "-Ad:" . $consignmentName, // Adı da kaydediyoruz
                'created_at' => now(),
                'updated_at' => now()
            ]);

            // StockAction kaydı oluştur (mevcut mantığınıza göre)
            StockAction::create([
                'firma_id'  => $tenant_id,
                'stokId'    => $stokId,
                'planId'    => $planid,
            ...
            // Eğer konsinye cihaz seçimi kaldırıldıysa veya adet 0 ise mevcut kaydı sil
            ServiceStageAnswer::where('planid', $planid)
                ->where('soruid', $soru_id)
                ->where('firma_id', $tenant_id)
                ->delete();
            // İlgili StockAction kaydını da silmeniz veya geri almanız gerekebilir.
            // Bu kısım sizin iş mantığınıza bağlı.
            // StockAction::where('planId', $planid)->where('stokId', $stokId)->where('islem', 2)->delete();
        }
    }

  private function getUpdatedConsignmentAnswerForQuestion($request, $soru_id)
    {
        // Bu metot, updateConsignmentSelection içinde ServiceStageAnswer'ı oluşturduğu için
        // UpdateServicePlan içinde $cevap->cevap = this->getUpdatedConsignmentAnswerForQuestion();
        // satırına gerek kalmıyor. Ancak yine de bir değer döndürmesi gerektiği için
        // request'ten gelen değerleri kullanarak string oluşturabiliriz.
        $stokIdRaw = $request->input("parca.$soru_id.consignment_select");
        $adetRaw   = $request->input("adet.$soru_id.selected_consignment_quantity");
        $consignmentName = $request->input("parca.$soru_id.consignment_name");

        if (!$stokIdRaw || !$adetRaw) {
            return "";
        }
        ...
        return "ID:" . $stokId . "-Adet:" . $adet . "-Ad:" . $consignmentName;
    }
