     ...
     private function stokKontrolEt(Request $request, $gelenIslem)
    {
        foreach ($request->all() as $key => $value) {
          ...
          // Konsinye Cihaz Kontrolü
            foreach ($request->all() as $key => $value) {
                if (strpos($key, 'consignmentCheck') !== false) {
                    $stokId = (int) filter_var($key, FILTER_SANITIZE_NUMBER_INT);
                    $adet = abs($request->input("consignmentAdet{$stokId}", 0));
                    ...
                    if ($stokHareketleri->count() > 0) {
                        ...
                        if ($toplam <= 0 || $adet > $toplam) {
                            $stok = Stock::where('id', $stokId)->first();
                            return "STOKHATA: " . mb_convert_case($stok->urun_adi, MB_CASE_TITLE, "UTF-8") . " Konsinye Stok Adeti Yetersizdir.";
                        }
                    } else {
                        $stok = Stock::where('id', $stokId)->first();
                        return "STOKHATA: " . mb_convert_case($stok->urun_adi, MB_CASE_TITLE, "UTF-8") . " Konsinye Stok Adeti Yetersizdir.";
