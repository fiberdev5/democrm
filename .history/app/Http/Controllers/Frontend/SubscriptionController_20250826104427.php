...
class SubscriptionController extends Controller
{
    ...
    public function subscribe($planid)
    {
        ...
        if ($tenant->currentSubscription && $tenant->currentSubscription->plan_id == $planid) {
            return redirect()->route('subscription.plans')
                           ->with('info', 'Zaten bu paketi kullanıyorsunuz.');
        }
        $plan = SubscriptionPlan::where('id', $planid)->get();
        return view('frontend.secure.subscription.checkout', compact('plan', 'tenant'));
    }
