...
public function getSurveyStatisticsData(Request $request, $tenant_id)
{
    ...
    // Personel bazında tamamlanan servisleri getir (service_plannings tablosundan)
    ...
    // Anket yapılan servisleri getir
    $surveyedServicesQuery = Survey::select(
        'surveys.personel as personel_id', 
        'surveys.servisid',
        'surveys.created_at'
    )
    ...
    foreach ($groupedStats as $personelId => &$stat) {
        $stat['tamamlanan_servis_sayisi'] = count(array_unique($stat['tamamlanan_servisler']));
        $stat['anket_yapilan_servis_sayisi'] = count(array_unique($stat['anket_yapilan_servisler']));
    }

    // Toplam sayıları hesapla
    $totalCompletedServices = $completedServices->unique('servisid')->count();
    $totalSurveyedServices = $surveyedServices->unique('servisid')->count();
