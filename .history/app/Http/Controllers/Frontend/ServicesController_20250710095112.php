...
private function processKonsinyeSelection(Request $request, $tenant_id)
{
    ...
    // Tüm konsinye cihazları al
    $tumKonsinyeCihazlar = $request->input('konsinye_cihaz', []);
    $tumKonsinyeAdetler = $request->input('konsinye_adet', []);

    $cihazlar = $tumKonsinyeCihazlar[$planid] ?? [];
    $adetler = $tumKonsinyeAdetler[$planid] ?? [];

   foreach ($cihazlar as $stokId => $adetVal) {
    $adet = abs($adetVal ?? 1); 
...
// Eksik konsinyeTeslimEt fonksiyonu için örnek bir taslak
private function konsinyeTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
{
    // Örnek: islem = 4 konsinye teslimat olsun
    $existingStockAction = StockAction::where('stokId', $stokId)
        ->where('islem', 4) // Konsinye Teslimat işlemi
        ->where('planId', $planId)
        ->first();
    ...
    // Ayrıca, bu işlem Stok tablosundaki mevcut konsinye cihaz miktarını da etkileyebilir.
    // Eğer bu bir çıkış işlemiyse, Stock::find($stokId)->decrement('current_stock_quantity', $adet); gibi bir işlem gerekebilir.
    // Bu kısmı iş mantığınıza göre ayarlamanız önemlidir.
}
