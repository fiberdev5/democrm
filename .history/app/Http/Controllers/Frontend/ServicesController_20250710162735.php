...
{
    // Plan bilgisi üzerinden personeli bul (gönderen kişi)
    $sonPlan = ServicePlanning::where('servisid', $servisId)
        ->orderBy('id', 'desc')
        ->skip(1)
        ->first();

    // İade işlemini kaydet (StokAction giriş olarak)
    StockAction::create([
        'firma_id'   => $tenantId,
        'stokId'     => $stokId,
        'islem'      => 1, // 1 = Giriş (İade gibi)
        'adet'       => $adet,
        'servisid'   => $servisId,
        'planId'     => $planId,
        'depo'       => 1, // Konsinye cihaz deposu
        'personel'   => $sonPlan->user_id ?? auth()->id(),
        'kid'        => auth()->id(),
        'created_at' => now(),
        'updated_at' => now()
    ]);
