...
// Konsinye Cihaz Düzenleme Sayfası (Modal için HTML döndürme)
public function editConsignmentDevice($tenant_id, $id)
{
    $firma = Tenant::findOrFail($tenant_id);
    $stock = Stock::where('kid', $firma->id)->where('id', $id)->firstOrFail();
    
    // Gerekli veriler...
    $markalar = marka::where('kid', $firma->id)->get();
    $cihazlar = cihaz::where('kid', $firma->id)->get();
    $rafListesi = Raf::where('kid', $firma->id)->get();
    $photos = stock_photos::where('stokid', $stock->id)->where('kid', $firma->id)->get();
    $sonTedarikciler = StockSupplier::where('firma_id', $firma->id)
        ->orderBy('created_at', 'desc')
        ->limit(10)
        ->get();
    
    $stokHareketleri = StockAction::where('stokId', $stock->id)
        ->where('firma_id', $firma->id)
        ->orderBy('id', 'desc')
        ->get();
    ...
    if (request()->ajax()) {
        $html = view('partials.edit-consignment-modal-content', compact(
            'stock', 'firma', 'markalar', 'cihazlar', 'rafListesi', 
            'photos', 'sonTedarikciler', 'stokHareketleri'
        ))->render();
        
        return response()->json(['html' => $html]);
    }

    return view('consignment.edit', compact(
        'stock', 'firma', 'markalar', 'cihazlar', 'rafListesi', 
        'photos', 'sonTedarikciler', 'stokHareketleri'
    ));
}

// Konsinye Cihaz Güncelleme
public function updateConsignmentDevice(Request $request, $tenant_id, $id)
{
    $firma = Tenant::findOrFail($tenant_id);
    ...
    $request->validate([
        ...
        'cihaz_id' => 'required|exists:cihazs,id',
        ...
        'urunKodu' => [
            ...
            'string',
            'max:20',
            Rule::unique('stocks', 'urunKodu')
                ->where('kid', $firma->id)
                ->ignore($id)
        ],
        'fiyat' => 'required|numeric|min:0',
        'fiyatBirim' => 'required|in:1,2,3',
    ...
    try {
        ...
        $stock->update([
            'stok_marka' => $request->marka_id,
            'stok_cihaz' => $request->cihaz_id,
            'urunDepo' => $request->raf_id,
            'urunAdi' => $request->urunAdi,
            'urunKodu' => $request->urunKodu,
            'fiyat' => $request->fiyat,
            'fiyatBirim' => $request->fiyatBirim,
            'aciklama' => $request->aciklama
        ]);

        return response()->json([
            'message' => 'Konsinye cihaz bilgileri başarıyla güncellendi.'
        ]);
