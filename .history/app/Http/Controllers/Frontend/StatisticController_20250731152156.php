...
public function getSurveyStatisticsData(Request $request, $tenant_id)
{
    ...
    // Temel sorgu - anketlerin yapıldığı servisleri getir
    $surveysQuery = Survey::select(
        'surveys.ekleyen', // Bu 'ekleyen' alanı personel ID'si mi? Genellikle user_id olur.
        'tb_user.name as adsoyad',
        'surveys.servisid',
        'surveys.created_at as tarih'
    )
    ->leftJoin('tb_user', 'surveys.ekleyen', '=', 'tb_user.user_id')
    ...
    $surveys = $surveysQuery->orderBy('surveys.created_at', 'DESC')->get();

    // Personel bazında anket sayılarını grupla
    $groupedSurveys = [];
    foreach ($surveys as $survey) {
        if (!isset($groupedSurveys[$survey->ekleyen])) {
            $groupedSurveys[$survey->ekleyen] = [
                'personnel_id' => $survey->ekleyen,
                'adsoyad' => $survey->adsoyad,
                'survey_count' => 0, // Anket sayısını ekliyoruz
                'completed_services' => 0 // Tamamlanan servis sayısını ekliyoruz
            ];
        }
        $groupedSurveys[$survey->ekleyen]['survey_count']++; // Her anket için sayacı artır
    }

    // Tamamlanan servisleri getir (gidenIslem=255 olanlar)
    $completedServicesQuery = ServicePlanning::select('servisid', 'ekleyen') // ekleyen kolonunu da seçmeliyiz
    ...
    // Personel bazında tamamlanan servis sayılarını işle
    foreach ($completedServices as $service) {
        // Eğer personel zaten groupedSurveys içinde yoksa ekle
        if (!isset($groupedSurveys[$service->ekleyen])) {
            // Personel adını tb_user tablosundan almalıyız
            $user = \DB::table('tb_user')->where('user_id', $service->ekleyen)->first();
            if ($user) {
                $groupedSurveys[$service->ekleyen] = [
                    'personnel_id' => $service->ekleyen,
                    'adsoyad' => $user->name,
                    'survey_count' => 0,
                    'completed_services' => 0
                ];
            } else {
                continue; // Kullanıcı bulunamazsa atla
            }
        }
        $groupedSurveys[$service->ekleyen]['completed_services']++;
    }

    // Nihai personel istatistikleri dizisi
    $personnelStats = array_values($groupedSurveys);

    // Toplam tamamlanan servis ve toplam yapılan anket sayıları
    $totalCompletedServices = count($completedServices); // Bu, yukarıda çekilen tamamlanmış servislerin toplam sayısı
    $totalSurveyedServices = count($surveys); // Bu, yukarıda çekilen anketlerin toplam sayısı

    // Cihaz türlerini çek (hem personel istatistikleri hem de anket sonuçları için kullanılabilir)
    $deviceTypes = \DB::table('device_types')->select('id', 'cihaz')->get();
    
    // Bayi bilgilerini çek (anket sonuçları için kullanılabilir)
    $bayiler = \DB::table('tb_user')->select('user_id', 'name')->where('firma_id', $tenant_id)->where('user_type', 'bayi')->get(); // Varsayımsal olarak bayi user_type
    ...
    return response()->json([
        ...
        'bayiler' => $bayiler // Bu alanın survey.results.data'dan gelmesi daha uygun olabilir.
    ]);
}
public function getSurveyResults(Request $request, $tenant_id) 
{
    ...
    foreach ($questionStats as $questionKey => $stats) {
        ...
        $questionPercentages[$questionKey] = [
            'evet' => $total > 0 ? round(($stats['evet'] / $total) * 100, 1) : 0,
            'hayir' => $total > 0 ? round(($stats['hayir'] / $total) * 100, 1) : 0,
            'belli_degil' => $total > 0 ? round(($stats['belli_degil'] / $total) * 100, 1) : 0,
        ];
    }
