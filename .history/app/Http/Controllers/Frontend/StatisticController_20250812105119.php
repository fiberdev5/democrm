...
public function CashStatistics($tenant_id) {
    ...
    // PaymentMethod tablosundan ödeme şekillerini al
    $paymentMethods = PaymentMethod::where(function($query) use ($firma_id) {
        $query->where('firma_id', $firma_id)
              ->orWhereNull('firma_id');
    })->get();

    // Ödeme şekillerinin ID'lerini bul
    $nakitId = $paymentMethods->where('odemeSekli', 'Nakit')->first()->id ?? null;
    $eftId = $paymentMethods->where('odemeSekli', 'EFT / Havale')->first()->id ?? null;
    $kartId = $paymentMethods->where('odemeSekli', 'Kredi Kartı')->first()->id ?? null;

    foreach ($kasaHareketleri as $row){
        if($row->odemeSekli == $nakitId){
            $nakit += $row->fiyat;
        }elseif($row->odemeSekli == $eftId){
            $eft += $row->fiyat;
        }elseif($row->odemeSekli == $kartId){
            $kart += $row->fiyat;
        }
    }
