...
class ServicesController extends Controller
{
   ...
   foreach ($request->all() as $key => $value) {
    if (strpos($key, 'consignmentCheck') !== false) {
        ...
        // Stok kaydını çek ve kategori kontrolü yap
        $stok = Stock::where('id', $stokId)->first();
        if (!$stok || $stok->urunKategori != 3) {
            return "STOKHATA: Seçilen stok geçersiz veya Konsinye Cihaz değildir.";
        }
...
foreach ($stokHareketleri as $hareket) {
    switch ($hareket->islem) {
        case "1": // Giriş
            $toplam += $hareket->adet;
            break;

        case "2": // Çıkış
        case "3": // İade veya başka çıkışlar
            $toplam -= $hareket->adet;
            break;
    }
}
