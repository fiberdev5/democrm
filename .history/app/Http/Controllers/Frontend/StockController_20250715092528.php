...
public function DeleteStockAction($tenant_id, $id)
{
    ...
    if (in_array($stockAction->islem, [2, 4])) {
        $mesaj = 'Serviste kullanılmış bir parçayı silemezsiniz. Silmek için servis içerisinden işlem yapmanız gerekmektedir.';
        if ($request->ajax()) {
            return response()->json(['status' => 'warning', 'message' => $mesaj]);
        }
        return redirect()->back()->with(['message' => $mesaj, 'alert-type' => 'warning']);
    }
