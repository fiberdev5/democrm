    ...
    public function getStats()
    {
        try {
            $stats = [
                ...
                'total_services' => DB::table('services')->where('durum', '!=', 0)->count(), // Silinmeyenler
                
                // Müşteri sayısı
                'total_customers' => DB::table('customers')->count(),
                
                // Personel sayısı - role tablosundan (Admin, Teknisyen, vs. hariç Bayi)
                'total_personnel' => DB::table('tb_user')
                    ->join('model_has_roles', 'tb_user.user_id', '=', 'model_has_roles.model_id')
                    ->join('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->whereIn('roles.name', ['Teknisyen', 'Teknisyen Yardımcısı', 'Operatör', 'Atölye Çırağı', 'Atölye Ustası', 'Depocu'])
                    ->where('tb_user.status', 1) // Aktif olanlar
                    ->count(),
                
                // Kasa toplamı - cash_transactions tablosundan
                'total_cash' => DB::table('cash_transactions')
                    ->where('odemeDurum', 1) // Onaylanmış ödemeler
                    ->sum('fiyat') ?? 0,
                
                // Günlük servis sayıları - kayitTarihi sütununu kullanarak
                'today_services' => DB::table('services')
                    ->where('kayitTarihi', Carbon::today()->format('Y-m-d'))
                    ->where('durum', '!=', 0)
                    ->count(),
                ...
                'previous_services' => DB::table('services')
                    ->where('kayitTarihi', Carbon::today()->subDays(2)->format('Y-m-d'))
                    ->where('durum', '!=', 0)
                    ->count(),
            ];
