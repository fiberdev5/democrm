...
public function StoreConsignmentDevice(Request $request, $tenant_id)
{
    ...
    // Validation (tek seferde yapılmalı!)
    $request->validate([
        'urunKodu' => [
            'required',
            'digits:13',
        ...
        'urunAdi' => [
            'required',
            'max:255',
            Rule::unique('stocks', 'urunAdi')->where(function ($query) use ($tenant_id) {
                return $query->where('firma_id', $tenant_id);
            }),
        ],
        'raf_id' => 'required',
        ...
        'raf_id.required' => 'Raf alanı zorunludur.',
    ...
    return redirect()->route('consignmentdevice', $tenant_id)->with([
        'message' => 'Konsinye cihaz başarıyla kaydedildi.',
        'alert-type' => 'success',
    ]);
}
