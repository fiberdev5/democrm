...
public function StoreConsignmentStockAction(Request $request, $tenant_id)
{
  ...
  if ($stock) { // Stock nesnesi zaten yukarıda bulundu
            if ($request->islem == 1 || $request->islem == 4) { // Alış veya Müşteriden İade
                $stock->adet += $request->adet;
                // Fiyatı sadece alışta toplayabilirsiniz, bu kısmı ihtiyacınıza göre düzenleyin
                if ($request->islem == 1) {
                    $stock->fiyat += $request->fiyat;
                }
            } elseif ($request->islem == 2) { // Serviste Kullanım
            ...
            $stock->save(); // Stock adetini ve/veya fiyatını kaydet
        }
