        ...
        try {
            ...
            if ($user->can('Tüm Servisleri Görebilir')) {
                $data = array_merge($data, [
                    'kid' => $user->user_id,
                    'servisKaynak' => $request->kaynak ?? '',
                    'musaitTarih' => $request->musaitTarih,
                    'musaitSaat1' => $request->musaitSaat1 ?? '',
                    'musaitSaat2' => $request->musaitSaat2 ?? '',
                    'cihazMarka' => $request->cihazMarka,
                    'cihazTur' => $request->cihazTur,
                    'cihazModel' => $request->cihazModel,
                    'cihazSeriNo' => $request->cihazSeriNo ?? '',
                    'cihazAriza' => $request->cihazAriza ?? '',
                    'operatorNotu' => $request->opNot ?? '',
                    'garantiSuresi' => $request->cihazGaranti ?? '',
                    'faturaNumarasi' => $request->faturaNumarasi ?? '',
                    'konsinye' => $request->konsinye ?? '',
                ]);
            }
