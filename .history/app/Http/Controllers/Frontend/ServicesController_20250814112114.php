           ...
           if ($request->get('device_types') || $request->filled('deviceType')) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('sonlandir_action', function($row) {
                    // Servisi sonlandırılabilir aşamalarda olan servisleri kontrol et
                    $sonlandirabilir_asamalar = ['Servisi Sonlandır', ' Haber Verecek']; // Kendi aşama isimlerinize göre düzenleyin
                    
                    $disabled = in_array($row->asamalar->asama, $sonlandirabilir_asamalar) ? '' : 'disabled';
                    $checked = $row->sonlandirildi ? 'checked' : ''; // Eğer veritabanında sonlandirildi field'ınız varsa
                    
                    return '<input type="radio" name="servis_sonlandir_' . $row->id . '" 
                                class="servisi-sonlandir form-check-input" 
...
public function sonlandirServis(Request $request, $firma_id, $servis_id)
{
    try {
        ...
        $sonlandirabilir_asamalar = ['Tamamlandı', 'Test Aşaması', 'Müşteri Onayı Bekliyor']; // Kendi aşama isimlerinize göre düzenleyin
