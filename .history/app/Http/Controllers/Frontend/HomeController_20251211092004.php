...
public function ContactSubmit(Request $request)
{
    ...
    // TODO: Mail gönderimi veya DB kayıt
    // Örnek:
    // Mail::to('info@fibermedya.com.tr')->send(new ContactFormMail($request->all()));
...
public function getSubscriptionPlans()
{
    ...
    // Session'dan seçili planı al
    $selectedPlanIndex = session('selected_plan.index', null);
    
    // Plan indexine göre plan ID'sini bul
    $selectedPlanId = null;
    if ($selectedPlanIndex !== null) {
        // Fiyatlandırma sayfasındaki sıralamaya göre plan ID'sini al
        $pricingData = HomepageContent::where('section', 'pricing_content')->first();
        if ($pricingData && isset($pricingData->content['pricing_plans'][$selectedPlanIndex])) {
            $selectedPlanName = $pricingData->content['pricing_plans'][$selectedPlanIndex]['name'];
            
            // Bu isme sahip planı bul
            $selectedPlan = SubscriptionPlan::where('name', $selectedPlanName)->first();
            if ($selectedPlan) {
    ...
    return response()->json([
        ...
        'selected_plan_info' => session('selected_plan', null)
    ]);
}
