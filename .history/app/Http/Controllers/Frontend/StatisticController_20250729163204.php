...
public function StateStatistics(Request $request, $tenant_id)
{
    if ($request->ajax()) {
        ...
        $data = $query->get()->map(function($item) {
            // Duruma göre durum_kod belirle (durum metnine göre)
            $durum_kod = 'pending'; // default

            $durum_lower = mb_strtolower($item->durum);
            if (str_contains($durum_lower, 'tamamlandı') || str_contains($durum_lower, 'teslim')) {
                $durum_kod = 'completed';
            } elseif (str_contains($durum_lower, 'iptal') || str_contains($durum_lower, 'canceled')) {
                $durum_kod = 'canceled';
            } elseif (str_contains($durum_lower, 'devam') || str_contains($durum_lower, 'yönlendir') || str_contains($durum_lower, 'atölye')) {
                $durum_kod = 'inprogress';
            }

            $item->durum_kod = $durum_kod;
            return $item;
        });
