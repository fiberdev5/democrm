...
class IncomingCallsController extends Controller
{
    public function IncomingCalls($tenant_id) {
        ...
        $incoming_calls = IncomingCall::where('firma_id', $tenant_id)->with('servisKaynak','brand','kayit_alan')->get();
    ...
    public function StoreCall($tenant_id, Request $request) {
        ...
        $createdCalls = IncomingCall::find($response->id);
        return response()->json($createdCalls);
    }

    public function EditCall($tenant_id, $call_id) {
        $firma = Tenant::where('id', $tenant_id)->first();
        $call_id = IncomingCall::where('id', $call_id)->first();
        $operators = User::role(['Operatör'])->where('tenant_id', $tenant_id)->get();
        return view('frontend.secure.incoming_calls.edit_call', compact('firma','call_id','operators'));
    }
}
