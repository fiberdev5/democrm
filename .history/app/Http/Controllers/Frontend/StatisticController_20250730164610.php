...
///////////////////////////////////////////////////////İlçe Statistics///////////////////////////////////////////////////////////////////
public function IlceStatistics($tenant_id)
{
    return view('frontend.secure.statistics.ilce_statistics', compact('tenant_id'));
}
 public function getIlceStatisticsData(Request $request, $tenant_id) // tenant_id'yi parametre olarak ekle
    {
        if ($request->ajax()) {
            $from_date = Carbon::parse($request->input('from_date'))->startOfDay();
            $to_date = Carbon::parse($request->input('to_date'))->endOfDay();

            $query = DB::table('services as s') // Alias ekledik
                ->join('customer as c', 's.musteri_id', '=', 'c.id') // Alias ekledik
                ->join('ilceler as i', 'c.ilce', '=', 'i.id') // Alias ekledik
                ->select(
                    'i.ilceName as ilce',
                    DB::raw('COUNT(s.id) as toplam'),
                    'i.id as ilce_id'
                )
                ->where('s.firma_id', $tenant_id); // FIRMA ID FILTRESI EKLENDI!


            // Arama filtresi (ilçe adı)
            if ($request->has('search') && !empty($request->search['value'])) {
                $searchValue = $request->search['value'];
                $query->where('i.ilceName', 'LIKE', "%$searchValue%");
            }

            $query->groupBy('c.ilce', 'i.ilceName', 'i.id');

            // Sıralama
            if ($request->has('order')) {
                $orderColumn = $request->order[0]['column'];
                $orderDirection = $request->order[0]['dir'];
                $columns = ['ilce', 'toplam']; // Blade'deki sütun sırasına göre
            ...
            // RecordsFiltered değerini almak için paginate öncesi count al
            $filteredRecords = $query->get()->count();

            // Sayfalama (DataTables'ın istediği gibi)
            if ($request->has('start') && $request->has('length')) {
                $query->offset($request->start)->limit($request->length);
            }
            ...
            // recordsTotal için distinct ilçe sayısını al
            $totalRecords = DB::table('services as s')
                ->join('customer as c', 's.musteri_id', '=', 'c.id')
                ->join('ilceler as i', 'c.ilce', '=', 'i.id')
                ->where('s.firma_id', $tenant_id)
                ->distinct('i.id')
                ->count('i.id');
