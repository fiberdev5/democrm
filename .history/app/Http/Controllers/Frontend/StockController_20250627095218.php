...
public function StoreConsignmentDevice(Request $request, $tenant_id)
{
    ...
    // Ürün adı zaten varsa, yönlendir (girişte aynı isimli ürün varsa)
    $existingName = Stock::where('firma_id', $tenant_id)
                        ->where('urunAdi', $request->urunAdi)
                        ->first();

    if ($existingName) {
        // Aynı isimde ürün varsa, yönlendir
        $editUrl = ($existingName->urunKategori == 3)
            ? route('edit.consignment.device', ['tenant_id' => $tenant_id, 'id' => $existingName->id])
            : route('edit.stock', ['tenant_id' => $tenant_id, 'id' => $existingName->id]);

        return redirect($editUrl)->with([
            'message' => 'Bu ürün adı zaten mevcut, düzenleme sayfasına yönlendirildiniz.',
            'alert-type' => 'info',
        ]);
    }

    // VALIDATION sadece bir kez yapılır
    $request->validate([
        'urunKodu' => [
            'required',
            'digits:13',
            Rule::unique('stocks', 'urunKodu')->where(function ($query) use ($tenant_id) {
