...
// Personel stoğundan düş
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        if ($servisPlan->gidenIslem != "259") {
            $stoklar = Stock::whereHas('PersonelStock', function($query) use ($tenant_id, $personel_id) {
    ...
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($yeniCevap == 'Parca') {
                    // processParcaSelection metodunu kendi stok yönetimi mantığınıza göre uyarlayın
                    // Bu metod, SaveServicePlan'deki parcaIslemleriniYap benzeri bir işlev görmelidir.
                    // Mevcut cevaplar silinip yeniden oluşturulabilir veya güncellenebilir.
    ...
    private function updateParcaSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
{
    ...
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'stokCheck') !== false) {
            ...
            // Burada Edit/Update olduğu için, genellikle parça kullanımı söz konusu olacaktır.
            // Ancak uygulamanızın mantığına göre bu kısmı SaveServicePlan'deki gibi düzenleyebilirsiniz.
            // Mevcut durumda 'gidenIslem' kullanılmıyor, bu yüzden varsayılan olarak 'parcaKullan' çağrılacak.
            $servisPlan = ServicePlanning::find($planId);
            if ($servisPlan && $servisPlan->gelenIslem == "238") { // Varsayımsal olarak "Parça Teslim" işlem ID'si 238 ise
                 $this->parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId);
            } else {
                 $this->parcaKullan($stokId, $adet, $servisId, $planId, $tenantId);
            }
