    ...
    if ($request->ajax()) {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('urunKodu', function($row) {
                return '<a  class="t-link editStock" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$row->urunKodu.'</a>';
            })
            ->addColumn('urunAdi', function($row) {
                return '<a  class="t-link editStock" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$row->urunAdi.'</a>';
            })
            ...
            ->filter(function ($instance) use ($request) {
                if (!empty($request->get('search')['value'])) {
                    ...
                    $instance->where(function($q) use ($search) {
                        $q->where('urunAdi', 'like', "%$search%")
                          ->orWhere('urunKodu', 'like', "%$search%");
                        // İstersen burada marka, cihaz, raf da ekle
                    });
                }
            })
