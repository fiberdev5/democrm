    ...
    public function UpdateCustomer($tenant_id, $id, Request $request)
{
    ...
    if (InvoiceIntegrationFactory::hasIntegration($tenant_id)) {
        try {
            ...
            // ✅ Eğer contact ID kaydedilmişse direkt güncelle
            if (!empty($customer->parasut_contact_id)) {
                Log::info('Müşteri Paraşüt\'te güncelleniyor (ID var)', [
                    'customer_id' => $customer->id,
                    'parasut_contact_id' => $customer->parasut_contact_id
                ]);
                
                $updateResult = $parasutService->updateContact($customer->parasut_contact_id, $customerData);
                ...
                // ✅ Contact ID yoksa ara/ekle ve kaydet
                Log::info('Müşteri Paraşüt contact ID bulunamadı, senkronize ediliyor', [
                    'customer_id' => $customer->id
                ]);
                ...
                if ($syncResult['success']) {
                    // ✅✅ Dönen contact ID'yi kaydet
                    $customer->update(['parasut_contact_id' => $syncResult['customer_id']]);
                    
                    Log::info('Müşteri Paraşüt contact ID kaydedildi', [
                        'customer_id' => $customer->id,
