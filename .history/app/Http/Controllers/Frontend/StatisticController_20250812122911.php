...
///////////////////////////////////////////////////////Cash Statistics///////////////////////////////////////////////////////////////////
private function getGiderVerisi($firma_id, $startDate, $endDate)
{
    // Sabit renk paletimiz
    $renkler = [
        '#FF6384', '#36A2EB', '#FBC02D', '#4BC0C0', '#9966FF', '#FF9F40',
        '#8E44AD', '#2ECC71', '#E67E22', '#3498DB', '#1ABC9C', '#C0392B', '#27AE60'
    ];

    // Tek ve verimli bir sorgu ile tüm giderleri ve isimlerini alalım.
    // Bu, N+1 sorgu sorununu da çözer.
    $giderlerSql = \App\Models\CashTransaction::where('firma_id', $firma_id)
        ->where('odemeYonu', '2') // Sadece Giderler
        ->where('fiyat', '>', 0)  // Değeri 0 olan giderleri grafikte ve listede göstermeyelim
        ->whereBetween('created_at', [$startDate, $endDate])
        ->join('payment_types', 'cash_transactions.odemeTuru', '=', 'payment_types.id')
        ->groupBy('payment_types.odemeTuru', 'payment_types.id')
        ->selectRaw('payment_types.odemeTuru as adi, sum(cash_transactions.fiyat) as deger')
        ->orderBy('payment_types.odemeTuru', 'asc')
        ->get();

    $giderVerisi = [];
    $giderlerToplam = 0;

    foreach ($giderlerSql as $index => $gider) {
        $giderVerisi[] = [
            'adi'   => $gider->adi,
            'deger' => round($gider->deger),
            // Renk burada, verinin sırasına göre tutarlı bir şekilde atanıyor.
            'renk'  => $renkler[$index % count($renkler)]
        ];
        $giderlerToplam += $gider->deger;
    }

    return [
        'giderListesi'   => $giderVerisi,
        'giderlerToplam' => round($giderlerToplam)
    ];
}

public function CashStatistics($tenant_id)
{
    // ... firma_id alma ve kontrol kodlarınız aynı kalabilir ...
    if (!Auth::check()) { return redirect()->route('login'); }
    $user = Auth::user();
    $firma_id = $user->firma_id ?? $tenant_id;
    // ...

    // --- GELİR HESAPLAMALARI (DEĞİŞİKLİK YOK) ---
    $gelirBaslangic = Carbon::now()->startOfMonth();
    $gelirBitis = Carbon::now()->endOfMonth();
    $kasaHareketleri = \App\Models\CashTransaction::where('firma_id', $firma_id)
        ->whereBetween('created_at', [$gelirBaslangic, $gelirBitis])
        ->where('odemeYonu', '1')
        ->get();

    $nakit = $kasaHareketleri->where('odemeSekli', 1)->sum('fiyat');
    $eft = $kasaHareketleri->where('odemeSekli', 2)->sum('fiyat');
    $kart = $kasaHareketleri->where('odemeSekli', 3)->sum('fiyat');
    $gelirler = $nakit + $eft + $kart;
    $odemeSekliAll = '"Nakit","EFT/Havale","Kredi Kartı"'; // Bu şekilde daha basit

    // --- GİDER HESAPLAMALARI (YENİ YÖNTEM) ---
    // 1. Yardımcı fonksiyonumuzu çağırarak gider verilerini alıyoruz.
    $giderData = $this->getGiderVerisi($firma_id, $gelirBaslangic, $gelirBitis);

    // 2. Gelen veriyi değişkenlere atıyoruz.
    $giderListesi = $giderData['giderListesi'];
    $giderlerToplam = $giderData['giderlerToplam'];

    // 3. Grafik için gereken verileri bu tek ve tutarlı yapıdan türetiyoruz.
    $grafikEtiketleri = json_encode(array_column($giderListesi, 'adi'));
    $grafikDegerleri = json_encode(array_column($giderListesi, 'deger'));
    $grafikRenkleri = json_encode(array_column($giderListesi, 'renk'));

    return view('frontend.secure.statistics.cash_statistics', compact(
        'tenant_id', 'nakit', 'eft', 'kart', 'gelirler', 'odemeSekliAll',
        'giderListesi', // Eski 'odemeTuruAll' yerine bu gönderiliyor
        'giderlerToplam',
        'grafikEtiketleri', // Eski 'odemeTuruSonuc' yerine
        'grafikDegerleri',  // Eski 'giderler' (string) yerine
        'grafikRenkleri'    // Eski 'odemeTuruRenkler' yerine
    ));
}
...
public function giderTabloGetir(Request $request, $tenant_id)
{
    ...
    $giderData = $this->getGiderVerisi($firma_id, $startDate, $endDate);

    // 2. HTML'i bir Blade partial'ı ile oluşturmak en temiz yoldur.
    // Bu, controller içinde HTML yazmaktan daha iyidir.
    $html = view('frontend.secure.statistics.partials.gider_liste_html', [
        'giderListesi' => $giderData['giderListesi'],
        'giderlerToplam' => $giderData['giderlerToplam']
    ])->render();
...
public function giderGrafikGetir(Request $request, $tenant_id)
{
    ...
    // 2. Grafiğin ihtiyacı olan verileri JSON olarak döndür.
    return response()->json([
        'labels' => array_column($giderListesi, 'adi'),
        'values' => array_column($giderListesi, 'deger'),
        'colors' => array_column($giderListesi, 'renk'),
    ]);
}
}
