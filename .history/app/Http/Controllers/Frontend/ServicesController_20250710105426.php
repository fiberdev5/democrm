 ...
 //Servis plan aşama düzenleme güncelleme fonksiyonu
    ...
    if ($request->has($soruKey)) {
        ...
        if ($yeniCevap == 'Parca' || $yeniCevap == 'Konsinye Cihaz') {
            $stokCevap = $this->processParcaVeKonsinyeSelection($request, $tenant_id);
            $cevap->cevap = $stokCevap;
        } else {
...
private function processParcaVeKonsinyeSelection(Request $request, $tenant_id)
{
    $stokCevap = [];
    $planid = $request->input('planid');
    $servisPlan = ServicePlanning::where('id', $planid)
    ...
    foreach ($request->all() as $key => $value) {
        // 'stokCheck' ile başlayan checkbox'lar parça veya konsinye olabilir
        if (strpos($key, 'stokCheck') === 0) {
