...
public function getIlceStatisticsData(Request $request, $tenant_id) {
    $query = DB::table('services as s')
        ...
        ->select('c.il', 'c.ilce', DB::raw('COUNT(s.id) as toplam'))
        ->where('s.durum', 1);
    ...
    if ($request->has('search') && !empty($request->search['value'])) {
        ...
        $query->where(function($q) use ($searchValue) {
            $q->where('c.il', 'LIKE', '%' . $searchValue . '%')
              ->orWhere('c.ilce', 'LIKE', '%' . $searchValue . '%');
        });
    }
    
    $query->groupBy('c.il', 'c.ilce');
    ...
    if ($request->has('order')) {
        ...
        if (isset($columns[$orderColumn])) {
            if ($columns[$orderColumn] == 'ilce') {
                $query->orderBy('c.ilce', $orderDirection);
            } else {
                $query->orderBy('toplam', $orderDirection);
            }
        }
    ...
    $totalRecords = $totalQuery->select(DB::raw('COUNT(DISTINCT CONCAT(c.il, "-", c.ilce)) as total'))
        ->first()->total;
