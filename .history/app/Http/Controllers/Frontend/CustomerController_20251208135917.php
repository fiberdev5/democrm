    ...
    public function UpdateCustomer($tenant_id, $id, Request $request)
{
    ...
    // Güncellenmiş müşteriyi yeniden yükle
    $customer->refresh();
    ...
    if (InvoiceIntegrationFactory::hasIntegration($tenant_id)) {
        try {
            ...
            // Paraşüt contact ID'sini al
            $contactId = $parasutService->getContactIdByCustomer($customer);
            ...
            if ($contactId) {
                ...
                // Paraşüt'te güncelle
                $updateResult = $parasutService->updateContact($contactId, $customerData);
                ...
                if ($updateResult['success']) {
                    ...
                    Log::warning('Müşteri Paraşüt\'te güncellenemedi', [
                        'customer_id' => $customer->id,
                        'contact_id' => $contactId,
                ...
                // Contact ID bulunamadı, yeni müşteri olarak senkronize et
                Log::info('Müşteri Paraşüt\'te bulunamadı, yeni müşteri olarak senkronize ediliyor', [
                    'customer_id' => $customer->id
                ]);

                $syncResult = $parasutService->syncCustomer([
                    'adSoyad' => $request->name,
                    'musteriTipi' => $request->mTipi,
                    'email' => $customer->email ?? null,
                    'tel1' => $request->tel1 ?? null,
                ...
                if ($syncResult['success']) {
                    // Yeni oluşturulan contact ID'yi kaydet
                    $customer->update(['parasut_contact_id' => $syncResult['customer_id']]);
                    
                    Log::info('Müşteri Paraşüt\'e yeni eklendi', [
                        'customer_id' => $customer->id,
                        'contact_id' => $syncResult['customer_id']
                    ]);
                }
            }

        } catch (\Exception $e) {
            // Paraşüt hatası kullanıcıya gösterilmez, sadece loglanır
