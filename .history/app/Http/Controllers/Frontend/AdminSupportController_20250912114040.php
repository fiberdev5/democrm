...
class AdminSupportController extends Controller
{
    ...
    public function reply(Request $request, SupportTicket $ticket)
    {
        ...
        $attachments = [];
          ...
          // Dosya yükleme - firma slug'ı ile
    if ($request->hasFile('attachments')) {
        // Firma bilgisini al
        $firma = $ticket->tenant;
        $firmaSlug = $firma->firma_slug ?? 'genel';
        
        foreach ($request->file('attachments') as $file) {
            $fileName = time() . '_' . Str::random(10) . '.' . $file->getClientOriginalExtension();
            
            // ServicePhoto'daki gibi klasör yapısı
            $path = "support-attachments/firma_{$firmaSlug}/ticket_{$ticket->ticket_number}/" . now()->toDateString();
    ...
    // Dosya görüntüleme
public function viewAttachment($ticketId, $fileName)
{
    $ticket = SupportTicket::findOrFail($ticketId);
...
// Dosya indirme method'unu da güncelleyin:
public function downloadAttachment($ticketId, $fileName)
{
    $ticket = SupportTicket::findOrFail($ticketId);
