...
public function CashStatistics($tenant_id) {
    ...
    // Debug: Log the firma_id
    \Log::info('Firma ID:', ['firma_id' => $firma_id]);
    ...
    // First, let's check if there are any transactions at all for this firma
    $totalTransactions = CashTransaction::where('firma_id', $firma_id)->count();
    \Log::info('Total transactions for firma:', ['count' => $totalTransactions]);
    
    // Check transactions without date filter
    $allTransactions = CashTransaction::where('firma_id', $firma_id)
        ->where('odemeYonu', '1')
        ->get();
    \Log::info('All income transactions:', $allTransactions->toArray());
    
    // Now with improved date filtering
    $kasaHareketleri = CashTransaction::where('firma_id', $firma_id)
        ->whereDate('created_at', '>=', date("Y-m-01")) // Use whereDate for better date comparison
        ->whereDate('created_at', '<=', date("Y-m-d"))
        ->where('odemeYonu', '1')
        ->get();

    // Alternative query with whereBetween (more reliable)
    $kasaHareketleriAlt = CashTransaction::where('firma_id', $firma_id)
    ...
    // Debug için tüm kasa hareketlerini kontrol edelim
    \Log::info('Kasa Hareketleri (Original):', $kasaHareketleri->toArray());
    \Log::info('Kasa Hareketleri (Alternative):', $kasaHareketleriAlt->toArray());

    // Use the alternative query result if original is empty
    $transactions = $kasaHareketleri->count() > 0 ? $kasaHareketleri : $kasaHareketleriAlt;

    foreach ($transactions as $row){
        \Log::info('İşlenen kayıt:', [
            'id' => $row->id,
            'odemeSekli' => $row->odemeSekli,
            'fiyat' => $row->fiyat,
            'created_at' => $row->created_at,
            'odemeYonu' => $row->odemeYonu
        ]);

        // payment_methods tablosundaki id değerleriyle karşılaştır
        ...
        // Debug: Log unmatched payment types
        if(!in_array($row->odemeSekli, [1, 2, 3])) {
            \Log::warning('Unmatched payment type:', [
                'odemeSekli' => $row->odemeSekli,
