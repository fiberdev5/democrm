...
private function processKonsinyeSelection(Request $request, $tenant_id, $cevapId)
{
    $konsinyeCevap = [];
    $planid = $request->input('planid');
    $servisPlan = ServicePlanning::where('id', $planid)
    ...
    foreach ($request->all() as $key => $value) {
        if (strpos($key, "konsinyeCheck{$cevapId}_") !== false) {
            $stokId = (int) filter_var($key, FILTER_SANITIZE_NUMBER_INT);
            $adetKey = "konsinyeAdet{$cevapId}_{$stokId}";
            $adet = abs($request->input($adetKey, 1));
