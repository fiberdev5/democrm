...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 //Servis plan aşama düzenleme güncelleme fonksiyonu
    ...
    // Dizi formatındaki input'tan cevapları al
    if (is_array($request->input('soru')) && isset($request->input('soru')[$cevapId])) {
        $yeniCevap = $request->input('soru')[$cevapId];

        // Parça ya da Konsinye kontrolü
        if ($yeniCevap == 'Parca') {
