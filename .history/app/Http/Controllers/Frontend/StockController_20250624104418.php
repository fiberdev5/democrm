...
public function AllStocks($tenant_id, Request $request) {
    ...
    if ($request->ajax()) {
        ...
        if ($request->filled('raf')) {
            $query->where('raf_id', $request->raf);
        }
        if ($request->filled('marka')) {
            $query->where('marka_id', $request->marka);
        }
        if ($request->filled('cihaz')) {
            $query->where('cihaz_id', $request->cihaz);
        }
        if ($request->filled('personel')) {
            $query->whereHas('stockActions', function($q) use ($request) {
                $q->where('personel', $request->personel);
            });
        }

        // Toplam hesaplamalar için filtrelenmiş veriyi kullan
        ...
        foreach ($filteredStocks as $stock) {
            ...
            if ($kalanAdet > 0) {
                $fiyat = $stock->fiyat ?? 0;
                $toplamAdet += $kalanAdet;
                $toplamFiyat += $kalanAdet * $fiyat;
            }
        }
        ...
        return DataTables::of($query)
            ...
            ->addColumn('urunKodu', function($row) {
                return '<a href="javascript:void(0);" class="editStock" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$row->urunKodu.'</a>';
            })
            ->addColumn('urunAdi', function($row) {
                return '<a href="javascript:void(0);" class="editStock" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$row->urunAdi.'</a>';
            })
            ->addColumn('adet', function ($stock) {
                $toplamGiris = \App\Models\StockAction::where('stokId', $stock->id)->where('islem', 1)->sum('adet');
                $toplamCikis = \App\Models\StockAction::where('stokId', $stock->id)->whereIn('islem', [2, 3])->sum('adet');
                $kalanAdet = $toplamGiris - $toplamCikis;
                return $kalanAdet > 0 ? $kalanAdet : 0;
            })
            ->addColumn('toplamTutar', function ($stock) {
                $toplamGiris = \App\Models\StockAction::where('stokId', $stock->id)->where('islem', 1)->sum('adet');
                $toplamCikis = \App\Models\StockAction::where('stokId', $stock->id)->whereIn('islem', [2, 3])->sum('adet');
                $kalanAdet = $toplamGiris - $toplamCikis;
                $tutar = $kalanAdet * ($stock->fiyat ?? 0);
            ...
            // Global search filtresi
            ->filterColumn('urunAdi', function($query, $keyword) {
                $query->where('urunAdi', 'like', "%{$keyword}%");
            })
            ->filterColumn('urunKodu', function($query, $keyword) {
                $query->where('urunKodu', 'like', "%{$keyword}%");
            })
            ->filterColumn('raf_adi', function($query, $keyword) {
                $query->whereHas('raf', function($q) use ($keyword) {
                    $q->where('raf_adi', 'like', "%{$keyword}%");
                });
            })
            ->filterColumn('marka_cihaz', function($query, $keyword) {
                $query->where(function($q) use ($keyword) {
                    $q->whereHas('marka', function($q2) use ($keyword) {
                        $q2->where('marka', 'like', "%{$keyword}%");
                    })->orWhereHas('cihaz', function($q3) use ($keyword) {
                        $q3->where('cihaz', 'like', "%{$keyword}%");
                    });
                });
            })
            ->filter(function ($instance) use ($request) {
                if ($request->has('search') && !empty($request->get('search')['value'])) {
                    ...
                    $instance->where(function($w) use ($search) {
                        $w->where('urunAdi', 'like', "%$search%")
                          ->orWhere('urunKodu', 'like', "%$search%")
                          ->orWhereHas('marka', function ($q) use ($search) {
                              $q->where('marka', 'like', "%$search%");
                          })
                          ->orWhereHas('cihaz', function ($q) use ($search) {
                              $q->where('cihaz', 'like', "%$search%");
                          })
                          ->orWhereHas('raf', function ($q) use ($search) {
                              $q->where('raf_adi', 'like', "%$search%");
                          });
                    });
                }
            })
            ->rawColumns(['urunKodu','urunAdi','action'])
            ->with([
                'toplamAdet' => number_format($toplamAdet, 0, ',', '.'),
