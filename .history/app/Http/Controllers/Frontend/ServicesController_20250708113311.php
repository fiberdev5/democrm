 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
         ...
         $soruKey = "soru." . $cevap->id; // Blade'den gelen isim formatına uygun
        $soru = \App\Models\StageQuestion::find($cevap->soruid); // StageQuestion modelini kullanıyoruz

        if ($soru && $soru->cevapTuru == '[Parca]') {
            // Parça tipi sorular için updateParcaSelection'ı çağır.
            // Bu fonksiyon zaten ServiceStageAnswer'ı yönettiği için
            // burada $cevap objesini tekrar kaydetmeye gerek yok.
            // Sadece blade'den 'soru.<plan_id>' hidden input'u geliyorsa,
            // bunu "Parca" olarak kontrol etmek için kullanıyoruz.
            if ($request->has($soruKey)) { // Hidden input'un gönderildiğini kontrol et
                $this->updateParcaSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
                // $cevap objesi delete edildiği ve yeniden oluşturulduğu için
                // bu döngüdeki $cevap objesi artık güncel olmayabilir.
                // Loop'un sonunda $planCevaplar'ı yeniden çekmek veya
                // updateParcaSelection'ın davranışını değiştirmek gerekebilir.
                // Şimdilik sorun değil, çünkü Blade'de $soru->cevapTuru kullanılıyor.
            }
        } else {
            // Diğer cevap türleri için
