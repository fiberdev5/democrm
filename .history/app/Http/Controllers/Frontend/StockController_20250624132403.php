...
->addColumn('urunKodu', function($row) {
    $urunKodu = e($row->urunKodu);
    // 11 veya 13 haneli rakam mı kontrol et
    if (preg_match('/^\d{11}$|^\d{13}$/', $row->urunKodu)) {
        return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="' . $row->id . '" data-bs-toggle="modal" data-bs-target="#editStockModal">' . $urunKodu . '</a>';
    } else {
        return '<span style="color:red; font-weight:bold;" title="Ürün kodu sadece 11 veya 13 haneli rakamlardan oluşmalıdır.">' . $urunKodu . '</span>';
    }
})
...
$request->validate([
    'urunKodu' => ['required', 'regex:/^\d{11}$|^\d{13}$/'],
]);
 ...
 public function EditStock($tenant_id, $id) {
        ...
        public function UpdateStock(Request $request, $tenant_id, $id){
            ...
            $stock = Stock::findOrFail($id);
                $request->validate([
        'urunKodu' => 'required|string|size:12',
        // diğer kurallar
    ]);
