...
public function stockDetail($tenant_id, $stock_id)
{
    $stock = Stock::with(['marka', 'cihaz', 'raf'])->where('firma_id', $tenant_id)->findOrFail($stock_id);

    // Stok hareketlerini join ile getir
    $stokHareketleri = StockAction::select('stok_hareketleri.*', 'stok_tedarikcileri.tedarikci', 'personeller.adsoyad')
        ->leftJoin('stok_tedarikcileri', 'stok_tedarikcileri.id', '=', 'stok_hareketleri.tedarikci')
        ->leftJoin('personeller', 'personeller.id', '=', 'stok_hareketleri.pid')
        ->where('stokid', $stock_id)
        ->orderBy('stok_hareketleri.id', 'desc')
        ->get();

    // kalanGun gibi değişkenler varsa burada tanımla veya controllerda hesapla
    $kalanGun = 10; // Örnek

    return view('frontend.secure.stocks.edit_stock', compact('stock', 'stokHareketleri', 'kalanGun'));
}
