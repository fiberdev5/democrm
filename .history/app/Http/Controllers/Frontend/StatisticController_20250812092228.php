...
public function CashStatistics($tenant_id) {
    ...
    // Mevcut ay için kasa hareketlerini getir (created_at kullanıyoruz)
    ...
    foreach ($kasaHareketleri as $row){
        if($row->odemeSekli == "7"){
            $nakit += $row->fiyat;
        }elseif($row->odemeSekli == "8"){
            $eft += $row->fiyat;
        }elseif($row->odemeSekli == "9"){
            $kart += $row->fiyat;
        }
    }
    ...
    $odemeSekliAll = "";
    $odemeSekilleri = PaymentMethod::where('firma_id', $firma_id)->orderBy('id','asc')->get();
    ...
    // Gider tablosu için - odemeTuru kolonunu isim olarak kullanacağız
    $odemeTurleri = PaymentType::where('firma_id', $firma_id)->orderBy('odemeTuru','asc')->get();
    $odemeTuruAll = [];
    ...
    // Grafik için veri hazırlama - renk kolonu olmadığı için varsayılan renkler kullanacağız
...
public function kasaFilteredData(Request $request, $tenant_id){
    ...
    // Verileri işle ve JSON formatında geri gönder
    $nakit = 0;
    $eft = 0;
    $kart = 0;
    foreach ($kasaHareketleri as $row) {
...
// Test ve debug için alternatif fonksiyon

public function CashStatisticsDebug($tenant_id) {
    $firma_id = Auth::user()->firma_id;
    
    // 1. ADIM: Veri tabanı bağlantısını kontrol et
    $allTransactions = CashTransaction::where('firma_id', $firma_id)->get();
    dd([
        'firma_id' => $firma_id,
        'toplam_transaction' => $allTransactions->count(),
        'ilk_5_transaction' => $allTransactions->take(5)->toArray(),
        'odeme_yonleri' => $allTransactions->pluck('odemeYonu')->unique()->toArray(),
        'odeme_sekilleri' => $allTransactions->pluck('odemeSekli')->unique()->toArray(),
        'payment_methods' => PaymentMethod::where('firma_id', $firma_id)->get()->toArray(),
        'payment_types' => PaymentType::where('firma_id', $firma_id)->get()->toArray()
    ]);
}

// Eğer hiç veri yoksa test verisi oluştur
public function createTestData($tenant_id) {
    $firma_id = Auth::user()->firma_id;
    
    // Test payment methods oluştur (yoksa)
    if(PaymentMethod::where('firma_id', $firma_id)->count() == 0) {
    ...
    // Test payment types oluştur (yoksa)
    if(PaymentType::where('firma_id', $firma_id)->count() == 0) {
        PaymentType::create([
            'firma_id' => $firma_id,
            'odemeTuru' => 'Kira Gideri'
        ]);
        PaymentType::create([
            'firma_id' => $firma_id,
            'odemeTuru' => 'Personel Maaşı'
        ]);
        PaymentType::create([
            'firma_id' => $firma_id,
            'odemeTuru' => 'Malzeme Alımı'
        ]);
    }
    ...
    CashTransaction::create([
        'firma_id' => $firma_id,
        'odemeYonu' => '2', // Gider
        'odemeTuru' => PaymentType::where('firma_id', $firma_id)->first()->id,
        'fiyat' => 500,
        'created_at' => now()
    ]);
...
// Basitleştirilmiş versiyonu - tüm zamanları kapsayacak şekilde
public function CashStatisticsSimple($tenant_id) {
    $firma_id = Auth::user()->firma_id;
    
    // Tüm zamanları kapsayacak şekilde gelir verileri
    $kasaHareketleri = CashTransaction::where('firma_id', $firma_id)
        ->where('odemeYonu', '1')
        ->get();
    ...
    foreach ($kasaHareketleri as $row){
        if($row->odemeSekli == 7){ // String yerine integer dene
            $nakit += $row->fiyat;
        }elseif($row->odemeSekli == 8){
            $eft += $row->fiyat;
        }elseif($row->odemeSekli == 9){
    ...
    // Ödeme şekillerini getir
    $odemeSekliAll = '"Nakit","EFT/Havale","Kredi Kartı"'; // Sabit değer

    // Gider tablosu için
    $odemeTurleri = PaymentType::where('firma_id', $firma_id)->get();
    $odemeTuruAll = [];
    ...
    foreach($odemeTurleri as $odemeTuru){
        $gelir = CashTransaction::where('firma_id', $firma_id)
            ...
            ->where('odemeTuru', $odemeTuru->id)
            ->sum('fiyat');
        $odemeTuruAll[$odemeTuru->odemeTuru] = round($gelir ?: 0);
    }
    ...
    // Varsayılan renk paleti
    $renkler = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40'];
    $odemeTuruRenkler = json_encode(array_slice($renkler, 0, count($odemeTuruAll)));
    $giderler = rtrim(implode(",", $odemeTuruAll), ",");
