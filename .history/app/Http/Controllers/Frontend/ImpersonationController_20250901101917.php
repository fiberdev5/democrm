...
class ImpersonationController extends Controller
{
    public function start(Request $request, $userId)
    {
        ...
        // Session bilgilerini ayarla - ÖNCE SESSION AYARLA
        session([
            'impersonator_id' => $currentUser->user_id,
        ...
        // Auth kullanıcısını değiştir - REMEMBER TOKEN OLMADAN
        Auth::setUser($targetUser);
        
        // Session'da login durumunu manuel olarak ayarla
        session([
            'login_web_' . sha1(Auth::getDefaultDriver()) => $targetUser->user_id
        ]);
    ...
    public function stop(Request $request)
    {
        ...
        $originalTenantId = session('original_user_tenant_id'); // Orijinal tenant ID'yi al
        ...
        // Session'ları temizle - AUTH İŞLEMLERİNDEN ÖNCE
        session()->forget([
            'impersonator_id',
        ...
        if ($originalUser) {
            ...
            // Session'da login durumunu manuel olarak ayarla
            session([
                'login_web_' . sha1(Auth::getDefaultDriver()) => $originalUser->user_id
            ]);
            ...
            $originalUser->fresh()->load('roles', 'permissions');
        }
        ...
        return redirect()->route('secure.home', ['tenant_id' => $originalTenantId ?? $originalUser->tenant_id])->with($notification);
    }
    ...
    public function getUsersForImpersonation(Request $request, $tenantId)
    {
        ...
        // Tenant bilgilerini de döndürelim
