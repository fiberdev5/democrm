...
class SuperAdminInvoicesController extends Controller
{
    public function AllInvoice(Request $request) {
        ...
        if ($request->ajax()) {           
           ...
           ->addColumn('firma_id', function($row){
                $ilAdi = $row->tenant?->ils?->name ?? 'Bilinmiyor';
                $ilceAdi = $row->tenant?->ilces?->ilceName ?? 'Bilinmiyor';
                ...
                return '<a class="t-link editInvoice address" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editInvoiceModal">
                    <span class="mobileTitle">Firma:</span><strong>'.$row->tenant?->firma_adi.'</strong><br>
                    <div style="font-size:12px;">'.$row->tenant?->tel1.'</div>
                    <div style="font-size:11px; color: #666;">'.$ilAdi.'/'.$ilceAdi.'</div>
                </a>';
            })
    ...
    $firmalar = Tenant::where('status', 1)
