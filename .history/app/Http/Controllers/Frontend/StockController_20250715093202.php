...
public function DeleteStockAction($tenant_id, $id) {
    ...
    try {
        // 1. Adım: İşlem türü kontrolü (İşlem 2 ve 4 kontrol)
        ...
        // 2. Adım: Stok hareket sayısı kontrolü
        $stokHareketSayisi = StockAction::where('stokId', $stockAction->stokId)
            ->where('id', '!=', $id) // Kendisi hariç
            ->count();
        
        if ($stokHareketSayisi > 0) {
            // 3. Adım: Alış işleminden sonra çıkış yapılmış mı kontrolü
            ...
            // Diğer durumlar için genel hareket kontrolü
            if ($stockAction->islem != 1) {
                return response()->json([
                    'success' => false,
                    'message' => 'Ürün içerisinde stok hareket kaydı bulunduğundan dolayı silme işlemi gerçekleştirilemez.'
