...
class SurveyStatisticsController extends Controller
{
    public function index($tenant_id)
    {
        ...
        $deviceTypes = DeviceType::where('firma_id', $tenant_id)->get();
        
        // Varsayılan tarih (dün)
        $defaultDate = Carbon::yesterday()->format('d/m/Y');
        
        return view('frontend.secure.surveys.statistics', compact('firma', 'deviceTypes', 'defaultDate'));
    }
    ...
    public function getData(Request $request, $tenant_id)
    {
        ...
        $query = DB::table('surveys as s')
            ...
            ->leftJoin('device_types as dt', 'sv.cihaz_id', '=', 'dt.id')  // cihaz_id olarak düzeltildi
        ...
        $completedServicesQuery = DB::table('services')
            ->where('firma_id', $tenant_id)  // tenant_id yerine firma_id
            ->where('durum', 'tamamlandi') // status yerine durum, completed yerine tamamlandi
            ->whereBetween('updated_at', [$startDate, $endDate]); // completed_at yerine updated_at

        if ($deviceType) {
            $completedServicesQuery->where('cihaz_id', $deviceType); // device_type_id yerine cihaz_id
        }
    ...
    public function getPersonelDetails(Request $request, $tenant_id)
    {
        ...
        $query = DB::table('surveys as s')
            ...
            ->leftJoin('customers as c', 'sv.musteri_id', '=', 'c.id')  // customer_id yerine musteri_id
            ->leftJoin('device_types as dt', 'sv.cihaz_id', '=', 'dt.id')  // device_type_id yerine cihaz_id
            ->where('s.firma_id', $tenant_id)
            ...
            ->select([
                's.id as survey_id',
                's.servisid',
                'sv.servis_no as service_number',  // service_number yerine servis_no
                'c.name as customer_name',
                'c.phone as customer_phone',
                'dt.cihaz as device_type',  // name yerine cihaz
                's.soru1', 's.soru2', 's.soru3', 's.soru5',
                's.created_at as survey_date',
                'sv.updated_at as service_date'  // completed_at yerine updated_at
            ]);
    ...
    public function exportExcel(Request $request, $tenant_id)
    {
        // Excel export işlemi için kod
        // Maatwebsite/Laravel-Excel paketi kullanılabilir
    }
        ...
        $dailyStats = DB::table('surveys')
            ...
            ->when($deviceType, function ($query) use ($deviceType) {
                return $query->join('services', 'surveys.servisid', '=', 'services.id')
                            ->where('services.cihaz_id', $deviceType);  // device_type_id yerine cihaz_id
            })
            ->select(
