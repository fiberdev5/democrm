...
///////////////////////////////////////////////////////Technician Statistics///////////////////////////////////////////////////////////////////
    ...
    public function getTechnicianStatisticsData(Request $request, $tenant_id)
    {
       ...
       $technicians = User::role('Teknisyen')
            ->where('tenant_id', $tenant_id) // firma_id yerine tenant_id
            ->where('status', 1)
            ->orderBy('name', 'asc')
            ->get();
   ...
   private function getPlanIdsByDateRange($startDate, $endDate, $tenant_id = null, $cihazTur = null)
{
    try {
        $query = DB::table('service_stage_answers as ssa')
            ->join('tb_service_plan as sp', 'ssa.plan_id', '=', 'sp.plan_id')
            ->where('ssa.created_at', '>=', $startDate)
            ->where('ssa.created_at', '<=', $endDate);
            
        // Tenant filtresi ekle
        if ($tenant_id) {
            $query->where('sp.tenant_id', $tenant_id); // veya firma_id
        }
        
        // Cihaz türü filtresi ekle
        if ($cihazTur) {
            $query->where('sp.cihaz_tur_id', $cihazTur);
        }
        
        return $query->pluck('ssa.plan_id')->unique()->toArray();
        
    } catch (\Exception $e) {
        \Log::error('getPlanIdsByDateRange error: ' . $e->getMessage());
        return [];
    }
}
