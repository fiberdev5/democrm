 ...
 public function StoreStockAction(Request $request, $tenant_id)
    {
        ...
        // Custom hata mesajları (isteğe bağlı, ama kullanıcı deneyimini iyileştirir)
        $messages = [
        ...
        // Doğrulama başarılı oldu, artık $validated verilerine ihtiyacınız yok,
        // $request ile doğrudan erişebilirsiniz veya isterseniz $validator->validated() kullanabilirsiniz.
        // $validated = $validator->validated(); // Eğer validated verileri bir değişkene atmak isterseniz
        ...
        if ($request->islem == 1) { // Alış işlemi ise
            ...
            $stockAction->personel = $request->personel; // Stokun gönderildiği personel
        }
        $stockAction->save();
        ...
        if ($request->islem == 3) {
            $personelStok = PersonelStock::create([
                ...
                'kid'    => $firma->id,
                'firma_id' => $firma->id,
                'pid'    => $request->personel, //gönderilen personel
                'adet'   => $request->adet,
            ]);
...
public function DeleteConsignmentStockAction($tenant_id, $id)
{
    ...
    if ($stockAction->islem == 1) {
        ...
        $cikisVarMi = StockAction::where('stokId', $stockAction->stokId)
            ->where('firma_id', $firma->id)
            ->where('islem', 2) // sadece serviste kullanım çıkışı
            ->where('created_at', '>', $girisTarihi) // bu alıştan sonra yapılmış mı?
            ->exists();
