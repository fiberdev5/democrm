...
public function CashStatistics($tenant_id) {
    ...
    // Ödeme şekillerini al (firma_id NULL veya kendi firma_id)
    $paymentMethods = PaymentMethod::where(function($query) use ($firma_id) {
        $query->where('firma_id', $firma_id)
              ->orWhereNull('firma_id');
    })->orderBy('id')->get();

    // kid => odemeSekli string map'i
    $odemeSekliMap = $paymentMethods->pluck('odemeSekli', 'kid')->toArray();

    // Ay başı ve sonu tarih aralığı (Şimdilik bu ay)
    $startDate = date("Y-m") . '-01 00:00:00';
    $endDate = date("Y-m-d") . ' 23:59:59';

    // Gelir olan kasa hareketleri
    $kasaHareketleri = CashTransaction::where('firma_id', $firma_id)
        ->whereBetween('created_at', [$startDate, $endDate])
        ->where('odemeYonu', 1)
        ->get();

    // Ödeme şekillerine göre tutarları sıfırla
    $tutarlar = [];
    foreach ($odemeSekliMap as $kid => $isim) {
        $tutarlar[$isim] = 0;
    }

    // Gelir tutarlarını ödeme şekline göre topla
    foreach ($kasaHareketleri as $row) {
        $odemeIsmi = $odemeSekliMap[$row->odemeSekli] ?? null;
        if ($odemeIsmi) {
            $tutarlar[$odemeIsmi] += $row->fiyat;
        }
    }

    // Yuvarla
    foreach ($tutarlar as $key => $val) {
        $tutarlar[$key] = round($val);
    }

    $gelirler = array_sum($tutarlar);

    // Örnek olarak özel 3 ödeme şekli için değişken atama (varsayalım bu 3 var)
    $nakit = $tutarlar['Nakit'] ?? 0;
    $eft = $tutarlar['EFT / Havale'] ?? 0;
    $kart = $tutarlar['Kredi Kartı'] ?? 0;

    // Giderleri çek (odemeYonu = 2)
    $odemeTurleri = PaymentType::orderBy('odemeTuru', 'asc')->get();
    $odemeTuruAll = [];

    foreach ($odemeTurleri as $odemeTuru) {
        $toplamGider = CashTransaction::where('firma_id', $firma_id)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('odemeYonu', 2)
            ->where('odemeTuru', $odemeTuru->id)
            ->sum('fiyat');

        $odemeTuruAll[$odemeTuru->odemeTuru] = round($toplamGider);
    }
    ...
    // Ödeme şekillerini JSON string (grafik için)
    $odemeSekliAll = '"' . implode('","', $paymentMethods->pluck('odemeSekli')->toArray()) . '"';

    return view('frontend.secure.statistics.cash_statistics', compact(
        'tenant_id', 'nakit', 'eft', 'kart', 'gelirler', 'odemeSekliAll',
        'odemeTuruAll', 'giderlerToplam', 'odemeTuruSonuc',
        'odemeTuruRenkler', 'giderler'
    ));
}
...
public function kasaFilteredData(Request $request, $tenant_id){
    ...
    // Belirtilen tarih aralığında gelirler
    $kasaHareketleri = CashTransaction::where('firma_id', $firma_id)
    ...
    $nakit = round($tutarlar['Nakit'] ?? 0);
    $eft = round($tutarlar['EFT / Havale'] ?? 0);
    $kart = round($tutarlar['Kredi Kartı'] ?? 0);
...
public function giderTabloGetir(Request $request, $tenant_id){
    ...
    foreach($odemeTurleri as $odemeTuru){
        $gider = CashTransaction::where('firma_id', $firma_id)
            ->whereBetween('created_at', [$startDate.' 00:00:00', $endDate.' 23:59:59'])
            ->where('odemeYonu', 2)
            ->where('odemeTuru', $odemeTuru->id)
            ->sum('fiyat');
        $odemeTuruAll[$odemeTuru->odemeTuru] = round($gider);
    }
