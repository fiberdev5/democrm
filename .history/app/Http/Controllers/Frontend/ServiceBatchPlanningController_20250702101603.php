...
class ServiceBatchPlanningController extends Controller
{
    ...
    public function getServiceList(Request $request,$tenant_id)
    {
        ...
        $personeller = User::query()
            ->role($defaultRoles)            // 👈 sadece bu rollere sahip kullanıcılar
        ...
        $firma = Tenant::where('id', $tenant_id)->first();
        if ($persID) {
        // Service modelinde 'assignments' diye ilişki varsa:
        // (ServiceStageAnswer tablosunda cevap = personel_id)
        $query->whereHas('cevaplar', function ($q) use ($persID) {
            $q->where('cevap', $persID);
        });

        /*
        // Eğer servis tablosunda doğrudan 'user_id' (atanan teknisyen) kolonu varsa:
        $query->where('user_id', $persID);
        */
    }
