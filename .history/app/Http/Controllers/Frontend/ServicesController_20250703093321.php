        ...
        if ($request->has('soru')) {
            foreach ($request->input('soru') as $soruId => $cevap) {
                ...
                elseif ($stageQuestion->cevapTuru == "[KonsinyeCihaz]") {
                        // Konsinye cihazlar için gelen veriyi işleyin
                        if ($request->has('selectedConsignmentDevices') && is_array($request->input('selectedConsignmentDevices'))) {
                            foreach ($request->input('selectedConsignmentDevices') as $cihazId => $value) {
                                $adet = $request->input("consignmentDeviceQuantities.{$cihazId}");

                                if ($adet > 0) { // Adet kontrolü stokKontrolEt'te yapıldı, burada sadece pozitiflik kontrolü
                                    // StockAction kaydı oluştur (islem: 2 servis kullanımı)
                                    $stockAction = new StockAction();
                                    $stockAction->firma_id   = $tenant_id;
                                    ...
                                    $stockAction->fiyat      = 0; // Konsinye olduğu için başlangıçta fiyat 0 olabilir veya Stock'tan çekilebilir
                                    $stockAction->fiyatBirim = 'TL'; // Veya ilgili birim
