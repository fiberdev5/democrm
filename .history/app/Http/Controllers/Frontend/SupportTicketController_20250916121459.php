  ...
  public function store(Request $request, $tenant_id)
{
    ...
    // Eğer kullanıcı yanıt veriyorsa ve durum 'cevaplandi' ise 'acik' yap
    if (!$user->isSuperAdmin() && $oldStatus === 'cevaplandi') {
        $ticket->update([
            'status' => 'acik',
            'last_reply_at' => now()
        ]);
        ...
        // Sadece last_reply_at'i güncelle
        $ticket->update(['last_reply_at' => now()]);
    }

    // Yanıt logunu ekle
    ActivityLogger::logSupportTicketReply($ticket->id, $ticket->ticket_number, $user->isSuperAdmin());
