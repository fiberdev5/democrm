...
class StockController extends Controller
{
     ...
     public function AllStocks($tenant_id, Request $request){
                    ...
                    return DataTables::of($query)  //adet bilgisi 
                        ...
                        // Birim fiyat bilgisi
                        ->addColumn('birimFiyat', function ($stock) {
                            return $stock->birimFiyat ? number_format($stock->birimFiyat, 2) . ' ₺' : '-';
                        })

                        // Toplam tutar (adet * birim fiyat)
                        ->addColumn('toplamTutar', function ($stock) {
                            if ($stock->sonHareket && $stock->birimFiyat) {
                                $tutar = $stock->sonHareket->adet * $stock->birimFiyat;
