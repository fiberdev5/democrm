...
class CashTransactionsController extends Controller
{
    ...
    public function EditCashTransaction($tenant_id, $id) {
        ...
        $payment_types = PaymentType::where()->orderBy('odemeTuru','asc')->get();
        $type_id = $cash_transaction_id->odemeTuru;
        $cash_payment_id = PaymentType::where('id',$type_id)->first();
        $personeller = User::where('status',1)->orderBy('name','asc')->get();
        return view('backend.cash_transactions.edit_cash_transaction',compact('cash_transaction_id','payment_methods','payment_types','cash_payment_id','currencies','personeller','categories','banks','musteri', 'musteriler'));

    }

    public function UpdateCashTransaction(Request $request) {
        $cash_transaction_id = $request->id;
        $fiyat = str_replace(',', '.', str_replace('.', '', $request->fiyat));

        CashTransaction::findOrFail($cash_transaction_id)->update([
            'odemeYonu' => $request->odeme_yonu,
            'odemeSekli' => $request->odeme_sekli,
            'odemeTuru' => $request->odeme_turu,
            'odemeDurumu' => $request->odeme_durum,
            'odenenBanka' => $request->banka,
            'fiyat' => $request->fiyat,
            'paraBirimi' => $request->paraBirimi,
            'aciklama' => $request->aciklama,
            'personel' => $request->personeller,
            'firma' => $request->firmalar,
