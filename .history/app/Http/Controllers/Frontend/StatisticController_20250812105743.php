...
public function CashStatistics($tenant_id) {
    ...
    // 1. Önce temel bilgileri kontrol edelim
    \Log::info('=== CASH STATISTICS DEBUG START ===');
    \Log::info('User ID: ' . Auth::user()->id);
    \Log::info('Firma ID: ' . $firma_id);
    \Log::info('Tenant ID: ' . $tenant_id);
    
    // 2. CashTransaction tablosunda hiç veri var mı?
    $totalCount = CashTransaction::count();
    \Log::info('Total records in CashTransaction table: ' . $totalCount);
    
    // 3. Bu firma_id ile hiç kayıt var mı?
    $firmaCount = CashTransaction::where('firma_id', $firma_id)->count();
    \Log::info('Records for this firma_id: ' . $firmaCount);
    
    // 4. Eğer veri varsa, örnek kayıtları görelim
    if ($firmaCount > 0) {
        $samples = CashTransaction::where('firma_id', $firma_id)
            ->orderBy('created_at', 'desc')
            ->limit(5)
            ->get(['id', 'firma_id', 'odemeSekli', 'odemeYonu', 'fiyat', 'created_at']);
        \Log::info('Sample records:', $samples->toArray());
        
        // 5. odemeYonu değerlerini kontrol edelim
        $odemeYonuValues = CashTransaction::where('firma_id', $firma_id)
            ->distinct()
            ->pluck('odemeYonu')
            ->toArray();
        \Log::info('Unique odemeYonu values: ', $odemeYonuValues);
        
        // 6. odemeSekli değerlerini kontrol edelim
        $odemeSekliValues = CashTransaction::where('firma_id', $firma_id)
            ->distinct()
            ->pluck('odemeSekli')
            ->toArray();
        \Log::info('Unique odemeSekli values: ', $odemeSekliValues);
    }
    
    // 7. Tarih aralığını kontrol edelim
    $currentMonth = date("Y-m");
    $startDate = $currentMonth . '-01 00:00:00';
    $endDate = date("Y-m-d") . ' 23:59:59';
    
    \Log::info('Date range check:');
    \Log::info('Start: ' . $startDate);
    \Log::info('End: ' . $endDate);
    \Log::info('Current date: ' . date("Y-m-d H:i:s"));
    
    // 8. Tarih filtresiz gelir kayıtları
    $allIncome = CashTransaction::where('firma_id', $firma_id)
        ->where('odemeYonu', '1')
        ->get(['id', 'odemeSekli', 'fiyat', 'created_at']);
    \Log::info('All income records (no date filter): ' . $allIncome->count());
    if ($allIncome->count() > 0) {
        \Log::info('First 3 income records:', $allIncome->take(3)->toArray());
    }
    
    // 9. Farklı tarih filtreleme yöntemlerini deneyelim
    
    // Yöntem 1: Orijinal
    $method1 = CashTransaction::where('firma_id', $firma_id)
        ->where('created_at', '>=', $startDate)
        ->where('created_at', '<=', $endDate)
        ->where('odemeYonu', '1')
        ->get();
    \Log::info('Method 1 (original) count: ' . $method1->count());
    
    // Yöntem 2: whereBetween
    $method2 = CashTransaction::where('firma_id', $firma_id)
        ->whereBetween('created_at', [$startDate, $endDate])
        ->where('odemeYonu', '1')
        ->get();
    \Log::info('Method 2 (whereBetween) count: ' . $method2->count());
    
    // Yöntem 3: whereDate
    $method3 = CashTransaction::where('firma_id', $firma_id)
        ->whereDate('created_at', '>=', date('Y-m-01'))
        ->whereDate('created_at', '<=', date('Y-m-d'))
        ->where('odemeYonu', '1')
        ->get();
    \Log::info('Method 3 (whereDate) count: ' . $method3->count());
    
    // Yöntem 4: Bu ay için (sadece ay kontrolü)
    $method4 = CashTransaction::where('firma_id', $firma_id)
        ->whereYear('created_at', date('Y'))
        ->whereMonth('created_at', date('m'))
        ->where('odemeYonu', '1')
        ->get();
    \Log::info('Method 4 (whereYear+whereMonth) count: ' . $method4->count());
    
    // 10. En fazla sonuç veren yöntemi kullanalım
    $kasaHareketleri = $method4; // Bu genellikle en güvenilir olanı
    
    if ($kasaHareketleri->count() > 0) {
        \Log::info('Using method 4 results. First record:', $kasaHareketleri->first()->toArray());
    }
    ...
    foreach ($kasaHareketleri as $index => $row){
        \Log::info("Transaction $index:", [
            'id' => $row->id,
            'odemeSekli' => $row->odemeSekli . ' (type: ' . gettype($row->odemeSekli) . ')',
            'fiyat' => $row->fiyat . ' (type: ' . gettype($row->fiyat) . ')',
            'created_at' => $row->created_at,
            'odemeYonu' => $row->odemeYonu . ' (type: ' . gettype($row->odemeYonu) . ')'
        ]);

        // Tip dönüşümü yaparak karşılaştıralım
        $odemeSekli = (int)$row->odemeSekli;
        $fiyat = (float)$row->fiyat;
        
        if($odemeSekli === 1){ // Nakit
            $nakit += $fiyat;
            \Log::info("Added to nakit: $fiyat. New total: $nakit");
        }elseif($odemeSekli === 2){ // EFT/Havale  
            $eft += $fiyat;
            \Log::info("Added to eft: $fiyat. New total: $eft");
        }elseif($odemeSekli === 3){ // Kredi Kartı
            $kart += $fiyat;
            \Log::info("Added to kart: $fiyat. New total: $kart");
        } else {
            \Log::warning("Unmatched odemeSekli: $odemeSekli for transaction ID: " . $row->id);
        }
    }
    ...
    \Log::info('FINAL CALCULATED VALUES:', [
        ...
        'kart' => $kart,
        'toplam' => $gelirler,
        'processed_records' => $kasaHareketleri->count()
    ]);
    \Log::info('=== CASH STATISTICS DEBUG END ===');
    ...
    $renkler = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#FF9F40', '#FF6384', '#C7C7C7'];
...
public function giderTabloGetir(Request $request, $tenant_id){
    ...
    foreach ($odemeTuruAll as $key => $value) {
        ...
        $html .= '<li class="gider"><div class="renk" style="background:'.$renk.'"></div><div class="adi">'.$key.'</div><div class="para">'.number_format($value, 0, ',', '.').'TL</div></li>';
        $colorIndex++;
    }
    $html .= '<li class="gider"><div class="renk" style="background:#000"></div><div class="adi">Toplam</div><div class="para">'.number_format($giderlerToplam, 0, ',', '.').'TL</div></li>';
    $html .= "</ul>";
