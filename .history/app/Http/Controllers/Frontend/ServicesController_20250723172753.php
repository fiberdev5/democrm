            ...
            if ($request->filterType && $request->filters) {
                ...
                switch ($request->filterType) {
                    ...
                    case 'yapilmayanAnketler':
                        ...
                        // İlk olarak, tarih aralığına göre servisleri filtrele
                        $data->whereBetween('created_at', [$tarih1, $tarih2]);

                        // Personel filtresi: Belirli bir teknisyen tarafından yapılmış bir ServiceStageAnswer kaydı olmayan servisleri bul.
                        if (!empty($filters['yapilmayan_personel']) && $filters['yapilmayan_personel'] != '0') {
                            $personelIdToFilter = $filters['yapilmayan_personel'];
                            $data->whereDoesntHave('cevaplar', function ($q) use ($personelIdToFilter) {
                                $q->where('soruid', 45) // Teknisyen bilgisinin tutulduğu soruid
                                    ->where('cevap', $personelIdToFilter);
                            });
                        }

                        // Bayi filtresi: Belirli bir bayi tarafından yapılmış bir ServiceStageAnswer kaydı olmayan servisleri bul.
                        if (!empty($filters['bayiler']) && $filters['bayiler'] = '0') {
                            $bayiIdToFilter = $filters['bayiler'];
                            $data->whereDoesntHave('cevaplar', function ($q) use ($bayiIdToFilter) {
                                $q->where('soruid', 3) // Bayi bilgisinin tutulduğu soruid
                                    ->where('cevap', $bayiIdToFilter);
                            });
                        }

                        // Anketleri olmayan servisleri getir (yani Survey tablosunda kaydı olmayan)
