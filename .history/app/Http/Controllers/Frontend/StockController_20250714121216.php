...
public function barkodPdf($tenant_id, $id) {
    ...
    $pdf = Pdf::loadView('frontend.secure.stocks.stocks_barkod', compact('stock'))
        // 50mm x 25mm boyutları piksel olarak: 50mm * (300dpi / 25.4mm/inç) = ~590.55px
        // 25mm * (300dpi / 25.4mm/inç) = ~295.27px
        // Veya doğrudan milimetre olarak belirterek deneyin, daha kesin olabilir.
        ...
        ->setOptions([
            ...
            'margin-bottom' => 0,
            'margin-left' => 0,
            // 'page-width' => '50mm', // Bu ayarlar zaten setPaper ile belirlendiği için çakışabilir
            // 'page-height' => '25mm' // Bu ayarları kaldırın veya setPaper ile aynı tutarlı birimlerde kullanın
        ]);
