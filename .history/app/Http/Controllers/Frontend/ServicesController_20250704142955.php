 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        $toplamPersonelStokAdedi = $stoklar->sum(function($stok) use ($personel_id) {
         ...
         $consignmentStocks = Stock::where('firma_id', $firma->id) // Assuming 'firma_id' is the tenant_id in Stock model
            ->where('urunKategori', 3) // Filter by urunKategori = 3 for consignment devices
            ->get();

        $detailedConsignmentStocks = $consignmentStocks->map(function ($stock) {
            // Calculate total incoming stock for the device (operations 1: Purchase, 4: Return from Customer)
            $girisAdet = StockAction::where('stokId', $stock->id)
                ->whereIn('islem', [1, 4])
                ->sum('adet');

            // Calculate total outgoing stock for the device (operation 2: Used in Service)
            ...
            // Include stocks with current quantity greater than or equal to 0
