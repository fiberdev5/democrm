...
class HomeController extends Controller
{
    ...
    // Step 1: Firma bilgileri formu
    public function step1Form()
    {
        return view('frontend.auth.register_step1');
    }
    ...
    public function step1Post(Request $request)
    {
        ...
        // Session’a kaydet
        session([
            'firma_temp' => $request->only('firma_adi','yetkili','telefon','email')
        ]);
        ...
        session([
            'sms_code' => $code,
            'sms_code_expires_at' => Carbon::now()->addMinutes(5)
        ]);

        // SmsService::send($request->telefon, "Doğrulama kodunuz: {$code}");
        // Test amaçlı ekrana göstermek için:
        return redirect()->route('firma.register.sms')->with('status', "Test kod: {$code}");
    }

    // Step 2: SMS doğrulama formu
    public function smsForm()
    {
        if (!session('firma_temp')) {
            return redirect()->route('firma.register.step1');
        }
    ...
    public function smsPost(Request $request)
    {
        ...
        if (!session('sms_code') || session('sms_code_expires_at') < now()) {
            return back()->withErrors(['sms_code' => 'Kodun süresi doldu!']);
        }

        if ($request->sms_code != session('sms_code')) {
            return back()->withErrors(['sms_code' => 'Kod hatalı!']);
        }

        // Firma bilgilerini kaydet
        $firma = Tenant::create(session('firma_temp'));

        // Session temizle
        session()->forget(['firma_temp','sms_code','sms_code_expires_at']);

        return redirect()->route('frontend.auth.complete');
    }

    // Step 3: Tamamlama sayfası
    public function complete()
    {
        return view('frontend.auth.register_complete');
    }
