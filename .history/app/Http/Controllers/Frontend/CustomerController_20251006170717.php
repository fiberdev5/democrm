...
{
    // İl ve ilçe bilgilerini lazy load yerine join kullanarak al
    // Veya cache kullan
    $il = $row->il ? \DB::table('ils')->where('id', $row->il)->value('name') : '';
    $ilce = $row->ilce ? \DB::table('ilces')->where('id', $row->ilce)->value('ilceName') : '';
...
private function colActions($row, $canView, $canEdit, $canDelete): string
{
    ...
    if ($canView) {
        $viewButton = '<a href="javascript:void(0);" data-bs-id="'.$row->id.'" 
                          class="btn btn-outline-primary btn-sm editCustomer mobilBtn mbuton1" 
    ...
    if ($canDelete) {
        $deleteButton = '<a href="'.$deleteUrl.'" 
                            class="btn btn-outline-danger btn-sm mobilBtn" 
                            id="delete" title="Sil">
