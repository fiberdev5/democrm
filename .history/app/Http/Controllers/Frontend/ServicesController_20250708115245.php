 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    if ($request->has($soruKey)) {
        ...
        if ($yeniCevap == 'Parca') {
            ...
            $parcaCevap = $this->processParcaSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
            $cevap->cevap = $parcaCevap; // Bu satır artık gerekli olmayabilir, çünkü processParcaSelection kendi kaydını yapıyor
        } else {
    ...
    private function processParcaSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
    {
