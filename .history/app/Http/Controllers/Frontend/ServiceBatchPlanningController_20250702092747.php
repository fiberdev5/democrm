...
class ServiceBatchPlanningController extends Controller
{
    ...
    public function getServiceList(Request $request,$tenant_id)
    {
        ...
        /* 2.1  Planlama ayarları (grup filtresi) */
        $defaultRoles = [
            '4',
            '5',
            '261',
        ...
        $setting = PlanningSetting::firstWhere('firma_id', $tenant_id);

        $personelQuery = Personel::where('firma_id', $tenant_id)
            ->where('yetki', 0)   // teknisyen
            ->where('durum', 1);  // aktif

        if ($setting && $setting->gruplar !== '0') {
            $personelQuery->whereIn('grup', explode(',', $setting->gruplar));
        }

        $personeller = $personelQuery
            ->orderBy('adsoyad')
            ->get(['id', 'adsoyad']);

        /* 2.2  Bugün atama yapılan planId’leri  (soruid = 296, cevap = bugünün tarihi) */
        $today      = Carbon::now()->format('d/m/Y');
        $todayPlans = ServiceStageAnswer::where('soruid', 296)
            ->where('cevap', $today)
            ->pluck('planid');

        /* 2.3  Kişi başına “bugün atanan servis” sayısı */
        $rawCounts = ServiceStageAnswer::select('cevap as personel_id', DB::raw('COUNT(*) as toplam'))
            ->whereIn('cevap', $personeller->pluck('id'))
        ...
        foreach ($personeller as $p) {
            $personelAtamaSayilari[$p->id] = $rawCounts[$p->id] ?? 0;
        }
