...
class TenantsController extends Controller
{
        ...
        if ($request->ajax()) {           
            ...
            return DataTables::of($filteredData)
                ...
                ->addColumn('durum', function($row) {
                    // Önce durum etiketini (badge) oluşturalım
                    ...
                    // Şimdi bu etiketi linkin içine yerleştirelim
    ...
    // Kullanıcıları görüntüleme butonu
    $usersButton = '<button class="btn btn-primary btn-sm mobilBtn view-tenant-users" data-tenant-id="'.$row->id.'" title="Kullanıcıları Görüntüle"><i class="fas fa-users"></i></button>';
    
    // Impersonation butonu - sadece aktif firmalar için
    if ($row->status == 1) {
        // Firma patronunu bul
        $tenantOwner = \App\Models\User::where('tenant_id', $row->id)
        ...
        if ($tenantOwner) {
            $impersonateButton = '<button class="btn btn-success btn-sm mobilBtn impersonate-tenant-owner" 
    ...
    return $editButton . ' ' . $usersButton . ' ' . $impersonateButton . ' ' . $deleteButton;
})
    ...
    public function EditTenant($tenant_id, $id) {
        ...
        $topServisSayisi = $servisler = Service::where('firma_id', $id) 
            ->where('durum', 1)
            ->count();
        return view('frontend.secure.tenants.edit_tenant', compact('tenant','countries','firma', 'periodStats','topServisSayisi'));
    ...
    public function DeleteTenant($tenant_id, $id) {
        $tenant = Tenant::findOrFail($id);
