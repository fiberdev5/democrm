...
public function giderTabloGetir(Request $request, $tenant_id){
    ...
    // 1. Firma ID'sini kontrol edin
    // dd($firma_id);

    $startDate = Carbon::parse($request->input('startDate'))->startOfDay();
    $endDate = Carbon::parse($request->input('endDate'))->endOfDay();
    
    // 2. Tarihleri kontrol edin
    // dd("Başlangıç: ".$startDate, "Bitiş: ".$endDate);

    $odemeTurleri = PaymentType::orderBy('odemeTuru', 'asc')->get();
    
    // 3. PaymentType tablosundan gelen verileri kontrol edin
    // dd($odemeTurleri);
    ...
    foreach($odemeTurleri as $odemeTuru){
        ...
        $sorgu = CashTransaction::where('firma_id', $firma_id)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('odemeYonu', '2')
            ->where('odemeTuru', $odemeTuru->id);

        $gelir = $sorgu->sum('fiyat');
        
        // 4. Her bir döngüde sorgu sonucunu ve SQL'i kontrol edin
        // if ($gelir == 0) {
        //     dd("Sorgu 0 döndü. SQL: " . $sorgu->toSql());
        // }
        
        $odemeTuruAll[$odemeTuru->odemeTuru] = round($gelir);
    }

    // 5. En son oluşan diziyi kontrol edin
    // dd($odemeTuruAll);
