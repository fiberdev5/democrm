...
class ServicesController extends Controller
{
    ...
    private function getYetkiliServisIDleri($user, $tenant_id)
    {
        ...
        $servisler = ServiceStageAnswer::selectRaw('servisid, MAX(planid) as planid, soruid, cevap, cevapText')
        ...
        foreach ($servisler as $servisRow) {
                ...
                if ($tarihDurum == "1") {
                    ...
                    if ($tarihCevap) {
                        ...
                        if ($bugun == $tarih) {
                            ...
                            if ($zaman) {
                                $ilksaat = strtotime(date("H.i")); // Şu anki saat
                                $sonsaat = strtotime($zaman->zaman); // Servis zamanı
            ...
            // Bayi kontrolü
            if (str_contains($soru->cevap, 'Bayi')) {
                $plan = ServicePlanning::find($servisRow->planid);
                if (!$plan) continue;
