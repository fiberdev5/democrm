...
public function updateHomepageContent(Request $request)
{
    ...
    if($section == 'hero') {
        ...
        $content = [
            ...
            'image' => $request->input('existing_image') // Mevcut resmi koru
        ];
        
        // Yeni resim yüklendiyse
        if ($request->hasFile('image')) {
            // Eski resmi sil
            $oldContent = HomepageContent::where('section', 'hero')->first();
            if($oldContent && isset($oldContent->content['image'])) {
                $oldImagePath = public_path($oldContent->content['image']);
