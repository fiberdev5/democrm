...
class InvoicesController extends Controller
{
    ...
    public function EditInvoice($tenant_id,$id) {
        ...
        return view('frontend.secure.invoices.edit_invoices',compact('invoice_id','musteri', 'musteriler','payment_methods','currencies','invoice_products'));
    ...
    public function UpdateInvoice(Request $request) {
        ...
        $invoice->pid = $pid;
        $invoice->mid = $request->musteri;
        $invoice->paraBirimi = $request->paraBirimi;
        $invoice->faturaNumarasi = $request->faturaNumarasi;
        $invoice->faturaTarihi = $request->faturaTarihi;
        ...
        $invoice->odemeDurum = $request->odemeDurum;
        $invoice->save();

        $oldProducts = InvoiceProduct::where('faturaid', $invoice_id)->get();
        foreach($oldProducts as $product){
            InvoiceProduct::findOrFail($product->id)->delete();
        }
        ...
        Musteriler::where('id', $request->musteri)->update([
            'aksiyon' => 1,
        ]);
        $notification = array(
