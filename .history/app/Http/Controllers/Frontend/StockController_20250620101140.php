...
{
    $photos = StockPhoto::where('kid', $tenant_id)
                        ->where('stock_id', $stock_id)
                        ->latest()
                        ->get();

    return view('frontend.secure.stock.stock_photos', compact('photos', 'stock_id', 'tenant_id'));
}

public function uploadPhoto(Request $request, $tenant_id)
{
    $request->validate([
        'resim' => 'required|image|mimes:jpg,jpeg,png|max:5120',
        'stock_id' => 'required|integer'
    ]);

    $file = $request->file('resim');
    $filename = uniqid() . '.' . $file->getClientOriginalExtension();
    $path = 'uploads/stock_photos/'; // public altında

    $file->move(public_path($path), $filename);

    $resimyol = '/' . $path . $filename; // /uploads/stock_photos/abc.jpg
    ...
    // JSON yerine "bildirim verisi" döndür
    return response([
...
{
    $photo = StockPhoto::where('id', $request->id)
                       ->where('kid', $tenant_id)
                       ->firstOrFail();

    // Depodan sil
    Storage::delete(str_replace('/storage/', 'public/', $photo->resimyol));
