...
class ServicesController extends Controller
{
    public function AllServices($tenant_id, Request $request) {
        ...
        if ($request->ajax()) {           
            ...
            if ($user->can('Kendi Servislerini Görebilir')) {
                ...
                $servisIDleri = $this->getYetkiliServisIDleri($user->user_id);
    ...
    private function getYetkiliServisIDleri($user)
    {
        $yetkiliServisIDler = [];

        $cevaplar = ServiceStageAnswer::where('cevap', $user)
            ->get()
            ->groupBy('servisid');

        foreach ($cevaplar as $servisID => $cevapListesi) {
            $cevapRow = $cevapListesi->last();
            $plan = ServicePlanning::find($cevapRow->planid);
            $soru = StageQuestion::find($cevapRow->soruid);

            if (!$plan || !$soru) continue;

            if (str_contains($soru->cevapTuru, '[Grup-3], [Grup-4]')) {
                if ($plan->tarihDurum) {
                    ...
                    if ($tarihCevap) {
                        $bugun = strtotime(now()->format('Y-m-d'));
                    ...
                    if ($servis && $servis->planDurum == $plan->id) {
                        ...
                        if ($plan->pid == $user && !$user->hasRole('Depocu')) {
