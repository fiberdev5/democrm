...
public function StoreDealer(Request $request, $tenant_id)
{
    ...
    //Belge Yükle
     $belgePdfPaths = [];
    // 1. Gelen dosyanın bir dizi olup olmadığını kontrol et
    if ($request->hasFile('belgePdf')) {
        // 2. Gelen dosyaları al ve array_slice ile ilk 2 tanesiyle sınırla
        $files = array_slice($request->file('belgePdf'), 0, 2); 
        
        // 3. Her bir dosyayı döngüyle tek tek işle (BURASI EN ÖNEMLİ KISIM)
        foreach ($files as $file) {
            // Artık '$file' değişkeni bir dizi değil, tek bir dosya nesnesidir.
            $fileName = time() . '_' . $username . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            // store() fonksiyonu tek bir dosya nesnesi üzerinde çalışır.
            $path = $file->storeAs('dealers-documents', $fileName, 'local');
            $belgePdfPaths[] = $path;
        }
    }
...
public function UpdateDealer(Request $request, $tenant_id, $id)
{
    ...
    if ($request->hasFile('belgePdf')) {
        // Eski belgeleri sil
        if ($bayi->belgePdf) {
            // JSON formatındaki eski belgeleri diziye çeviriyoruz
            $eskiBelgeler = json_decode($bayi->belgePdf, true);
            if (is_array($eskiBelgeler)) {
                foreach ($eskiBelgeler as $eskiBelge) {
                    if (Storage::disk('local')->exists($eskiBelge)) {
                        Storage::disk('local')->delete($eskiBelge);
                    }
                }
            }
        }
        ...
        $belgePdfPaths = [];
        // Gelen dosyaları alıp ilk 2 tanesini seçiyoruz
        $files = array_slice($request->file('belgePdf'), 0, 2);
        
        foreach ($files as $file) {
            // Dosya adını oluşturup kaydediyoruz
            $username = Str::slug($request->username, '-');
            $fileName = time() . '_' . $username . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            $path = $file->storeAs('dealers-documents', $fileName, 'local'); // storeAs kullanımı daha güvenli
            $belgePdfPaths[] = $path;
        }
        
        // Yeni dosya yollarını JSON olarak kaydediyoruz
        $bayi->belgePdf = json_encode($belgePdfPaths);
    }
