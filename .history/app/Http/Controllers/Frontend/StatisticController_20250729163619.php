        ...
        if ($request->ajax()) {
            $query = DB::table('services as s')
                ...
                ->select(
                    ...
                    DB::raw('LOWER(REPLACE(ss.asama, " ", "-")) as durum_slug'), // Durum adından slug oluştur
                    'ss.id as durum_id' // Durum ID'sini de çek
                )
            ...
            $query->groupBy('ss.asama', 'durum_slug', 'durum_id'); // groupBy'a durum_slug ve durum_id ekleyin
            ...
            if ($request->has('order')) {
                ...
                if (isset($columns[$orderColumn])) {
                    // Veritabanı sütun isimlerine göre sıralama yapın
            ...
            $totalRecords = DB::table('services as s')
                ...
                ->where('s.firma_id', $tenant_id)
                ->where('s.durum', 1)
                ->select('ss.asama') // Distinct için sadece aşama sütununu seç
                ->distinct()
                ->get()
                ->count();
            
            // Filtrelenmiş kayıt sayısı (sayfalama öncesi)
            // Bu kısım, DataTables'ın "recordsFiltered" değeri için doğru sayıyı verecektir.
            // Sayfalama yapılmadan önceki sorgunun sonucunu saymak için 'count' kullanıyoruz.
            $filteredRecords = $query->get()->count();


            // Sayfalama uygulandıktan sonraki veriler
            if ($request->has('start') && $request->has('length')) {
                if ($request->length != -1) { // Eğer "Tümü" seçeneği seçilmediyse
                    $query->offset($request->start)->limit($request->length);
                }
            }
            ...
            return response()->json([
                'draw' => intval($request->draw),
                'recordsTotal' => $totalRecords,
                'recordsFiltered' => $filteredRecords, // Filtrelenmiş kayıtların sayısı
                'data' => $data
            ]);
        }
