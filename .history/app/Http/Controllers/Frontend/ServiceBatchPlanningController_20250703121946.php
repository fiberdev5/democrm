...
class ServiceBatchPlanningController extends Controller
{
    ...
    public function assignService(Request $request, $tenant_id)
    {
        ...
        /* ----------------------------------------------------
         | 1. Servislerin gerçekten bu kiracıya ait olduğunu kontrol et
         |----------------------------------------------------*/
        ...
        if ($services->count() !== count($ids)) {
            return back()->withErrors('Hatalı servis ID');
        }

        /* ----------------------------------------------------
         | 2. Özel durum: hem gelen hem giden 264 ise -> eski bayi planlarını sil
         |----------------------------------------------------*/
        ...
        /* ----------------------------------------------------
         | 3. Toplu işlem - transaction
         |----------------------------------------------------*/
        DB::transaction(function () use ($ids, $gelenIslem, $gidenIslem, $request, $tenant_id, $today) {

            foreach ($ids as $sid) {

                /* 3.1  servis_planlama kaydı */
        ...
        return back()->with('success', 'Servis durumları güncellendi.');
    }
}
