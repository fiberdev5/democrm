...
public function StoreConsignmentStockAction(Request $request, $tenant_id)
{
    ...
    $stock = Stock::where('id', $stokId)->where('urunKategori', 3)->first();
    ...
    if ($request->islem == 1) { // Alış işlemi ise
            $stockAction->personel = $personel_id; // İşlemi yapan personel (alışı kim yaptı?)
        } elseif ($request->islem == 2) { // Serviste Kullanım
            $stockAction->servisid = $request->servisid;
            $stockAction->personel = $personel_id; // Serviste kullanan personel (mevcut kullanıcı)
        } elseif ($request->islem == 4) { // Müşteriden Geri Alma
            $stockAction->servisid = $request->servisid; 
        }
    $stockAction->save();

        if ($stock) {
            if ($request->islem == 1 || $request->islem == 4) { // Alış veya Müşteriden Geri Alma ise stok artar
                $stock->adet += $request->adet;
                $actionMessage = 'Konsinye stok başarıyla güncellendi (eklendi).';
            } elseif ($request->islem == 2) { // Serviste Kullanım ise stok azalır
                $stock->adet -= $request->adet;
                $actionMessage = 'Konsinye stok başarıyla güncellendi (azaltıldı).';
            }
            $stock->save(); // Stock modelindeki adet bilgisini kaydet
        } else {
            // Eğer stok bulunamazsa veya kategori 3 değilse bir hata mesajı göster
            return redirect()->back()->with([
                'message'    => 'Hata: Belirtilen stok ID\'sine sahip konsinye stok (kategori 3) bulunamadı.',
                'alert-type' => 'error',
            ]);
        }
