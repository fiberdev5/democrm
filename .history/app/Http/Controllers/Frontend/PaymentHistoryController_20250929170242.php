...
class PaymentHistoryController extends Controller
{
    ...
    private function getDataTablesData(Request $request, $tenant)
    {
        ...
        $data = $paymentsForPage->map(function($payment) {
            // Açıklamayı temizle
            ...
            if ($payment['invoice_path']) {
                $invoiceStatus = '<span class="badge badge-success"><i class="fas fa-check mr-1"></i>Mevcut</span>';
            } else {
                $invoiceStatus = '<span class="badge badge-warning"><i class="fas fa-clock mr-1"></i>Bekleniyor</span>';
            }

            return [
                'id' => $payment['id'],
                'type_label' => '<strong>' . $payment['type_label'] . '</strong>',
                'description' => '<strong>' . $description . '</strong>',
                'amount' => '<strong>' . number_format($payment['amount'], 2) . ' ' . strtoupper($payment['currency']) . '</strong>',
                'status_label' => '<span class="badge badge-' . $statusClass . '">' . $payment['status_label'] . '</span>',
                'created_at' => '<strong>' . $payment['created_at']->format('d.m.Y') . '</strong>',
