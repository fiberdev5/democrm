...
class ServicesController extends Controller
{
    ...
    // Anketleri olan servisleri getir ve anketin yapıldığı tarihe göre filtrele
    $data->whereHas('surveys', function($query) use ($tarih1, $tarih2, $request) {
        // surveys tablosundaki created_at alanını kullanarak filtreleme
        $query->whereBetween('created_at', [$tarih1, $tarih2]);

        // Anketi yapılan personel filtresi
        if ($request->filled('yapilan_anketi_yapilan_personel') && $request->yapilan_anketi_yapilan_personel != 0) {
            $query->where('personel', $request->yapilan_anketi_yapilan_personel);
        }

        // Anketi ekleyen personel filtresi
        if ($request->filled('yapilan_anketi_yapan_personel') && $request->yapilan_anketi_yapan_personel != 0) {
            $query->where('ekleyen', $request->yapilan_anketi_yapan_personel);
        }

        // Bayi filtresi
        if ($request->filled('bayiler') && $request->bayiler != 0) {
            $query->where('bayi', $request->bayiler);
        }
    });
    break;
                        ...
                        $tarih2 = Carbon::parse($filters['yapilmayananket_tarih2'])->endOfDay();
                         // Personel filtresi (kayitAlan, yani servisi kaydeden personel)
                        if (!empty($filters['yapilmayan_personel']) && $filters['yapilmayan_personel'] != '0') { // Frontend'de bu alanı kullandığınızdan emin olun
                            $data->where('kayitAlan', $filters['yapilmayan_personel']);
                        }
                        // Tarih filtresi
                        if (!empty($tarih1) && !empty($tarih2)) {
                            $data->whereBetween('created_at', [$tarih1, $tarih2]);
                        ...
                        $data->doesntHave('surveys'); // Servisin hiç anket kaydı yoksa
                        break;
