...
class ServicesController extends Controller
{
    ...
    public function ServicetoPdf($tenant_id, $id) {
        ...
        // Mesaj ayarlarını al
        $mesaj = ServiceFormSetting::where('firma_id', $tenant_id)->where('kid', $kid)->first();
        ...
        $servisPlanlama = ServiceStageAnswer::where('firma_id', $tenant_id)->where('servisid', $servis->id)
                        ->orderBy('id', 'DESC')
                        ->get();
        ...
        foreach ($servisPlanlama as $asama) {
            ...
            if ($asama && $asama->cevapText == '[Bayi]') {
                ...
                $getUye = User::where('tenant_id', $tenant_id)->where('status', '1')
                ...
                if ($getUye) {
                    ...
                    if ($mesaj) {
                        $mesaj->mesaj = str_replace("[TEL]", $getUye->tel, $mesaj->mesaj);
                    }
                }
            } else {
                $getUye = Tenant::where('id', $tenant_id)->where('status', '1')->first();
                ...
                if ($getUye) {
                    ...
                    if ($mesaj) {
                        $mesaj->mesaj = str_replace("[TEL]", $getUye->tel1, $mesaj->mesaj);
                    }
                }
            }
        }
        ...
        $data = [
            ...
            'eskiIslemler' => $eskiIslemler,
            'paraIslemler' => $paraIslemler,
            'mesaj' => $mesaj,
            'garantiBitis' => $garantiBitis,
            'servisPlanlama' => $servisPlanlama,
            'getUye' => $getUye,
            'logoPath' => $logoPath,
            'webSitesi' => $webSitesi
        ];
