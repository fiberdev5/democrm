...
{
    $complaintServices = ServicePlanning::whereIn('service_id', explode(',', $serviceIds))
        ->where('process_type', 'complaint') // Şikayet işlem türü
        ->groupBy('service_id')
        ->get();

    $count = 0;
    foreach ($complaintServices as $service) {
        $firstService = ServicePlanning::where('service_id', $service->service_id)
            ->where('technician_id', $technicianId)
            ->orderBy('date', 'ASC')
            ->first();

        $lastComplaint = ServicePlanning::where('service_id', $service->service_id)
            ->where('process_type', 'complaint')
            ->orderBy('date', 'DESC')
            ->first();

        if ($firstService && $lastComplaint) {
            $firstDate = Carbon::parse($firstService->date)->format('Y-m-d');
            $lastDate = Carbon::parse($lastComplaint->date)->format('Y-m-d');
