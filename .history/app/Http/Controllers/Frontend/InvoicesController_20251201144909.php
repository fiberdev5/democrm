...
public function addPaymentToInvoice(Request $request, $firma_id)
{
    try {
        ...
        // ⭐ Eğer zaten tahsilat varsa izin verme
        if ($invoice->has_payment) {
            return response()->json([
                'success' => false,
                'message' => 'Bu faturaya zaten tahsilat eklenmiş. Yeni tahsilat eklemek için Paraşüt\'ü kullanın.'
            ], 400);
        }
        ...
        // Paraşüt Invoice ID'sini al
        $parasutInvoiceId = $parasutService->getInvoiceId($invoice);
        
        if (!$parasutInvoiceId) {
            return response()->json([
                'success' => false,
                'message' => 'Bu fatura Paraşüt\'te bulunamadı. Fatura numarası: ' . $invoice->faturaNumarasi
            ], 404);
        }

        // Ödeme ekle
        $result = $parasutService->addPayment($parasutInvoiceId, [
            'account_id' => $validated['account_id'],
            'amount' => $validated['amount'],
            'date' => $validated['date'],
            'description' => $validated['description'] ?? 'Tahsilat',
            'exchange_rate' => null
        ]);

        if ($result['success']) {
            // ⭐ VERİTABANINA TAHSİLAT BİLGİSİNİ KAYDET
            ...
            Log::info('Tahsilat veritabanına kaydedildi', [
                'invoice_id' => $invoice->id,
        ...
        return response()->json([
            'success' => false,
            'message' => $result['error'] ?? 'Ödeme eklenemedi'
        ], 400);
