...
class DeletedServicesController extends Controller
{
    ...
    private function deleteExpiredServices($tenant_id)
    {
        ...
        foreach ($expired_services as $service) {
            ...
            Log::info( $tenant_id . ' firmasının ' . $service->id. ' IDli servislerinin süresi geçtiği için sistemden silindi.');
        }
    }
    ...
    public function RestoreService($tenant_id, $service_id) {
        ...
        Log::info( $firma->id . ' firmasının ' . Auth::user()->name . '  personeli ' . $service_id. ' IDli servisi silinenlerden geri aldı.', [
