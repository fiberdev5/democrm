...
class SuperAdminInvoicesController extends Controller
{
        ...
        $storagePayments = StoragePurchase::where('tenant_id', $tenantId)
            ...
            ->map(function($payment) {
                return [
                    ...
                    'amount' => $payment->amount,
                    'currency' => 'TL', // Varsayılan
                    'payment_method' => 'Kredi Kartı', // Varsayılan
    ...
    public function StoreInvoice(Request $request){
        $validateData = $request->validate([
            'document'=> 'max:2000',
            'payment_type' => 'required|in:subscription,storage',
        ...
        try {
            ...
            if($invoice){
                ...
                // İlgili ödeme tablosunda invoice_path'i güncelle
                $this->updatePaymentInvoicePath($request->payment_type, $request->payment_id, $save_url);
                
                DB::commit();
                
                $notification = array(
                    'message' => 'Fatura Başarıyla Eklendi ve Ödeme Kaydı Güncellendi',
                    'alert-type' => 'success'
                );
    ...
    public function UpdateInvoice(Request $request) {
        ...
        try {
            ...
            // Ödeme bilgisi değiştiyse eski kaydı temizle ve yenisini güncelle
            if (isset($request->payment_type) && isset($request->payment_id)) {
                if ($oldPaymentType && $oldPaymentId) {
                    $this->clearPaymentInvoicePath($oldPaymentType, $oldPaymentId);
                }
    ...
    public function DeleteInvoice($id) {
        ...
        try {
            ...
            $notification = [
                'message' => 'Fatura ve İlgili Ödeme Kaydı Başarıyla Silindi',
                'alert-type' => 'success'
            ];
