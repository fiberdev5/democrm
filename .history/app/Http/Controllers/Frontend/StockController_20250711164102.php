...
public function GetPersonelStocks($tenant_id, $stok_id)
{
    ...
    $hareketler = StockAction::with('aliciPersonel')
        ...
        ->get()  //Güncel peronele ait stok miktarını çekme 
        ->map(function ($hareket) use ($stok_id) {
...
{
    php// DeleteConsignmentStockAction fonksiyonunun başına ekleyin
Log::info('Stock Action Debug', [
    'stock_action_id' => $stockAction->id,
    'islem_value' => $stockAction->islem,
    'islem_type' => gettype($stockAction->islem),
    'raw_attributes' => $stockAction->getAttributes()
]);

// Veya tarayıcıda görmek için:
dd([
    'stock_action_id' => $stockAction->id,
    'islem_value' => $stockAction->islem,
    'islem_type' => gettype($stockAction->islem),
    ...
    // Stok hareketine ait işlem türü kontrolü- Serviste Kullanım 
    if ($stockAction->islem == 2) {
        $notification = [
            'message' => 'Serviste kullanılmış bir parçayı silemezsiniz. Silmek için servis içerisinden işlem yapmanız gerekmektedir.',
            'alert-type' => 'warning'
    ...
    // Stok hareketine ait işlem türü kontrolü- Müşteriden Geri Alındı
