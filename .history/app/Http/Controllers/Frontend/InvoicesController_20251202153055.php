...
class InvoicesController extends Controller
{
    ...
    // ⭐ Resmileştir butonu (Paraşüt entegrasyonu varsa)
    if($hasParasutIntegration) {
        if($row->formalized) {
            // Zaten resmileştirilmiş
            $statusColors = [
                'pending' => 'warning',
                'sent' => 'success',
                'error' => 'danger'
            ...
            $text = $statusTexts[$row->formalization_status] ?? 'Bilinmiyor';
            $icon = $statusIcon[$row->formalization_status] ?? 'fa-question-circle';
            $typeText = $row->formalization_type === 'e-invoice' ? 'e-Fatura' : 'e-Arşiv';
            
            $buttons .= '<button type="button" class="btn btn-'.$color.' btn-sm mobilBtn mbuton1 checkFormalizationStatus" 
                data-invoice-id="'.$row->id.'" 
                title="'.$typeText.' - '.$text.'">
                <i class="fas '.$icon.'"></i>
            </button> ';
        } else {
            // Henüz resmileştirilmemiş
...
public function getInvoicePayments($firma_id, $invoice_id)
{
    try {
        ...
        $result = $parasutService->getInvoicePayments($parasutInvoiceId);

        // ⭐ Eğer Paraşüt'te tahsilat varsa ama veritabanında yoksa, senkronize et
...
public function formalizeInvoice(Request $request, $firma_id)
{
    ...
    try {
        ...
        // Paraşüt invoice ID kontrolü
        $parasutInvoiceId = $parasutService->getInvoiceId($invoice);
        
        if (!$parasutInvoiceId) {
            return response()->json([
                'success' => false,
                'message' => 'Bu fatura Paraşüt\'te bulunamadı. Önce faturayı Paraşüt\'e gönderin.'
            ], 404);
        }

        // Zaten resmileştirilmiş mi kontrol et
        if ($invoice->formalized && $invoice->formalization_status === 'sent') {
            return response()->json([
                'success' => false,
                'message' => 'Bu fatura zaten resmileştirilmiş',
        ...
        // ✅ Müşterinin VKN veya TC No'sunu al
        $vkn = null;
        if ($customer->musteriTipi == '2') {
            // Kurumsal müşteri → Vergi No
            $vkn = $customer->vergiNo;
        } elseif ($customer->musteriTipi == '1') {
            // Bireysel müşteri → TC No
            $vkn = $customer->tcNo ?? '11111111111'; // Varsayılan TC No
        }

        if (empty($vkn)) {
            Log::warning('Müşteri VKN/TC No bulunamadı, e-arşiv olarak gönderilecek', [
                'customer_id' => $customer->id,
                'customer_name' => $customer->adSoyad
            ]);
        }

        Log::info('Müşteri bilgileri', [
            'customer_id' => $customer->id,
            'customer_name' => $customer->adSoyad,
            'customer_type' => $customer->musteriTipi == '2' ? 'Kurumsal' : 'Bireysel',
            'has_vkn' => !empty($vkn)
        ]);

        // ✅ VKN ile e-Fatura mükellefiyetini kontrol et
        ...
        Log::info('Fatura resmileştirme başlatılıyor', [
            'invoice_id' => $invoice->id,
            'parasut_invoice_id' => $parasutInvoiceId,
            'type' => $formalizationType,
            'customer_name' => $customer->adSoyad,
            'vkn' => $vkn ? '***' . substr($vkn, -4) : 'yok',
            'is_e_invoice_user' => $eInvoiceStatus['is_e_invoice_user']
        ]);

        // Resmileştirme işlemini yap
        if ($formalizationType === 'e-invoice') {
            $result = $parasutService->formalizeAsEInvoice($parasutInvoiceId);
        } else {
            $result = $parasutService->formalizeAsEArchive($parasutInvoiceId);
        }
        ...
        if ($result['success']) {
            ...
            $invoice->update([
                ...
                'formalization_job_id' => $result['job_id'] ?? null
            ]);

            // Durum kontrolü yap (birkaç saniye sonra)
            sleep(5);
            $statusCheck = $parasutService->checkFormalizationStatus($parasutInvoiceId);
            
            if ($statusCheck['success'] && $statusCheck['formalized']) {
                $invoice->update([
                    'formalization_status' => $statusCheck['status']
                ]);
            }

            $typeText = $formalizationType === 'e-invoice' ? 'e-Fatura' : 'e-Arşiv';
            $customerTypeText = $customer->musteriTipi == '2' ? 'Kurumsal' : 'Bireysel';

            return response()->json([
                'success' => true,
                'message' => "Fatura {$typeText} olarak resmileştirildi ve GİB'e gönderildi",
                'type' => $formalizationType,
                'status' => $invoice->formalization_status,
            ...
            $invoice->update([
                ...
                'formalization_error' => $result['error'] ?? 'Bilinmeyen hata'
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Resmileştirme başarısız: ' . ($result['error'] ?? 'Bilinmeyen hata')
            ], 400);
        }

    } catch (\Exception $e) {
        Log::error('Fatura resmileştirme hatası: ' . $e->getMessage(), [
            'trace' => $e->getTraceAsString()
        ]);
...
public function checkFormalizationStatusController(Request $request, $firma_id, $invoice_id)
{
    try {
        ...
        $parasutService = InvoiceIntegrationFactory::make($firma_id);
        $parasutInvoiceId = $invoice->integration_invoice_id;
        ...
        if ($statusCheck['success']) {
            ...
            return response()->json([
                ...
                'status_text' => $statusText[$statusCheck['status']] ?? 'Bilinmiyor',
                'type' => $invoice->formalization_type,
                'pdf_url' => $statusCheck['pdf_url'] ?? null
            ]);
        } else {
            return response()->json([
                'success' => false,
