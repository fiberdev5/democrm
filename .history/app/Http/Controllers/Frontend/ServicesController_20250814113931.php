...
class ServicesController extends Controller
{
        ...
        if ($request->ajax()) {           
           ...
           if ($request->get('device_types') || $request->filled('deviceType')) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('sonlandir_action', function($row){
    // Servisi "Sonlandırıldı" durumuna geçirmek için kullanılacak buton.
    $sonlandirabilir_asamalar = ['Servisi Sonlandır', 'Haber Verecek'];
    $zaten_sonlanmis_asama = 'Sonlandırıldı';

    if ($row->asamalar) {
        if (in_array($row->asamalar->asama, $sonlandirabilir_asamalar) && $row->asamalar->asama != $zaten_sonlanmis_asama) {
            
            // BURAYI KENDİ VERİTABANINIZA GÖRE GÜNCELLEYİN
            $sonlandirildi_asama_id = 255; // <<< DEĞİŞTİRİN: 'Sonlandırıldı' aşamanızın ID'si.

            return '<button type="button" 
                            class="btn btn-sm btn-success servisi-sonlandir-btn" 
                            data-servis-id="' . $row->id . '" 
                            data-gelen-islem-id="' . $row->servisDurum . '"
                            data-giden-islem-id="' . $sonlandirildi_asama_id . '"
                            title="Bu servisi tamamlandı olarak işaretle">
                            <i class="fas fa-check-circle"></i> Sonlandır
                    </button>';
        }
        
        if ($row->asamalar->asama == $zaten_sonlanmis_asama) {
            return '<span class="badge bg-info">Tamamlandı</span>';
        }
    }
    return '';
})
    ...
    // Servisi sonlandırma endpoint'i
public function sonlandirServis(Request $request, $firma_id, $servis_id)
{
    try {
        $servis = Service::where('firma_id', $firma_id)->find($servis_id);
        
        if (!$servis) {
            return response()->json(['success' => false, 'message' => 'Servis bulunamadı.']);
        }
        
        // Servisi sonlandırılabilir aşamalarda olup olmadığını kontrol et
        $sonlandirabilir_asamalar = ['Servisi Sonlandır', ' Haber Verecek']; // Kendi aşama isimlerinize göre düzenleyin
        
        if (!in_array($servis->asamalar->asama, $sonlandirabilir_asamalar)) {
            return response()->json(['success' => false, 'message' => 'Bu aşamada servis sonlandırılamaz.']);
        }
        
        // Servisi sonlandır
        $servis->sonlandirildi = true; // Eğer böyle bir field'ınız yoksa ekleyin
        $servis->sonlandirilma_tarihi = now();
        $servis->sonlandiran_kullanici_id = auth()->user()->id;
        $servis->save();
        
        return response()->json(['success' => true, 'message' => 'Servis başarıyla sonlandırıldı.']);
