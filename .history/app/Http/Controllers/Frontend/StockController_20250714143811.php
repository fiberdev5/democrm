...
public function GetPersonelStocks($tenant_id, $stok_id)
{
   ...
   $hareketler = StockAction::with('aliciPersonel')
    ...
    ->where('islem', 3)
    ->get()
    ->groupBy('pid') // personel ID'ye göre grupla
    ->map(function ($grup) {
        $ilk = $grup->first();

        return (object)[
            'personel_adi' => $ilk->aliciPersonel->name ?? 'Bilinmiyor',
            'adet'         => $grup->sum('adet'),
            'tarih'        => $grup->sortByDesc('created_at')->first()->created_at,
            'id'           => $ilk->id, // detay için ilk ID yeterli olur
        ];
    });
