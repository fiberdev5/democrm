...
{
    try {
        // Debug kodlarını kaldır - sadece log tutalım
        ...
        $paymentDetails = [];
        if ($request->payment_type && is_array($request->payment_type)) {
        ...
        try {
            ...
            $invoice = Invoice::create([
                ...
                'faturaPdf' => $save_url,
                'payment_details' => $paymentDetails, // Model'de cast olarak array
                'faturaTipi' => 'manual',
                'faturaDurumu' => 'draft',
                'durum' => '1',
            ]);

            // Ürünleri kaydet - Model kullanarak
            ...
            $notification = array(
                'message' => "Fatura Başarıyla Eklendi (ID: {$invoice->id}, {$productCount} ürün, " . count($paymentDetails) . " ödeme)",
                'alert-type' => 'success'
            );
