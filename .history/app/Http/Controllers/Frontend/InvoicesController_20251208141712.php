            ...
            if (InvoiceIntegrationFactory::hasIntegration($tenant_id)) {
                ...
                try {
                    ...
                    if ($integration) {
                        ...
                        // ✅ Müşteriyi senkronize et VE dönen ID'yi kaydet
        $syncResult = $parasutService->syncCustomer($customerData);
        
        if (!$syncResult['success']) {
            throw new Exception('Müşteri senkronize edilemedi: ' . ($syncResult['error'] ?? 'Bilinmeyen hata'));
        }

        $parasutCustomerId = $syncResult['customer_id'];

        // ✅✅ CONTACT ID'Yİ VERİTABANINA KAYDET
        if (!$invoice->customer->parasut_contact_id) {
            $invoice->customer->update([
                'parasut_contact_id' => $parasutCustomerId
            ]);
