    ...
    public function StoreInvoice(Request $request, $tenant_id)
{
    ...
    try {
        ...
        // ✅ PARAŞÜT ENTEGRASYONU - ASENKRON (QUEUE)
        $integrationMessage = '';
        
        if (InvoiceIntegrationFactory::hasIntegration($tenant_id)) {
            Log::info('Fatura queue\'ya ekleniyor', [
            ...
            try {
                ...
                // ✅ Job'ı queue'ya ekle
                \App\Jobs\SendInvoiceToParasut::dispatch($invoice, $invoiceData, $invoiceProducts);
                
                $integrationMessage = ' Fatura Paraşüt\'e gönderiliyor...';
                
                Log::info('Job queue\'ya eklendi', [
                    'invoice_id' => $invoice->id
                ]);
                
            } catch (\Exception $e) {
                Log::error('Job ekleme hatası', [
                    'invoice_id' => $invoice->id,
                    'error' => $e->getMessage()
                ]);
                
                $integrationMessage = ' UYARI: Arka plan işlemi başlatılamadı.';
            }
        }
        ...
        return response()->json([
            ...
            'invoice_id' => $invoice->id,
            'processing_async' => InvoiceIntegrationFactory::hasIntegration($tenant_id)
        ]);
