...
class StockController extends Controller
{
     ...
     public function AllStocks($tenant_id, Request $request){
        ...
        // Kopya query ile toplam hesaplanmalı (orijinal query paginate ediliyor)
        $toplamQuery = clone $query;
        // Toplam adet (sonHareket->adet olabilir null, o yüzden 0 kabul et)
        $toplamAdet = $toplamQuery->with('sonHareket')->get()->sum(function ($stock) {
            return $stock->sonHareket ? (int)$stock->sonHareket->adet : 0;
        });

        // Toplam fiyat (fiyat ve adet çarpımı)
        $toplamFiyat = $toplamQuery->with('sonHareket')->get()->sum(function ($stock) {
            return $stock->sonHareket ? ((float)$stock->fiyat * (int)$stock->sonHareket->adet) : 0;
        });
                    ...
                    return DataTables::of($query)  //adet bilgisi 
                        ...
                        // Toplamları ekle
                        $data['toplam_adet'] = $toplamAdet;
                        $data['toplam_fiyat'] = $toplamFiyat;
