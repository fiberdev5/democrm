    ...
    public function SurveyStore(Request $request, $tenant_id, $servisId)
    {
        ...
        try {
            ...
            $personelYapilanServis = ServiceStageAnswer::where('servisid', $servisId)
                   ...
                   // İlk olarak ServiceStageAnswer tablosundan gelen personel ID'si atanır
            if ($personelYapilanServis) {
                $personelIdToAssign = (int)$personelYapilanServis->cevap; 
            }
            // Eğer request'ten (formdan) bir personel ID'si geliyorsa, o öncelikli olur
            if ($request->filled('personel')) {
                $personelIdToAssign = $request->input('personel');
            }

            // Anketin yapıldığı personel ID'sini Survey modeline ata
            $anket->personel = $personelIdToAssign ?? 0; 

            // **BURASI İSTEDİĞİNİZ KONTROLÜ YAPIYOR:**
            // Eğer anket yapılan personel ID'si varsa, bu kullanıcının rolünü kontrol et
            if ($personelIdToAssign) {
                $anketYapilanUser = User::find($personelIdToAssign);
                if ($anketYapilanUser) {
                    // Bayi rolünü adıyla kontrol ediyoruz (örneğin 'Bayi' adında bir rolünüz varsa)
                    // Ya da sabit ID ile: $bayiRole = Role::find(259);
                    $bayiRole = Role::where('name', 'Bayi')->first(); 
                    
                    // Eğer bayi rolü tanımlıysa VE anket yapılan kullanıcı bu role sahipse
                    if ($bayiRole && $anketYapilanUser->hasRole($bayiRole->name)) { 
                        $anket->bayi = $personelIdToAssign; // Bayi sütununa anket yapılan kişinin ID'sini ata
                    } else {
                        $anket->bayi = null; // Eğer bayi rolünde değilse null yap
                    }
                } else {
                    $anket->bayi = null; // Kullanıcı bulunamazsa null yap
                }
            } else {
                $anket->bayi = null; // Anket yapılan personel ID'si yoksa null yap
            }
            // Verileri ata
