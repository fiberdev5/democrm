...
public function formalizeInvoice(Request $request, $firma_id)
{
    ...
    try {
        ...
        // Müşterinin Paraşüt contact ID'sini bul
        $customerData = [
            'adSoyad' => $customer->adSoyad,
        ...
        $existingCustomer = $parasutService->syncCustomer($customerData);
        
        if (!$existingCustomer['success']) {
            return response()->json([
                'success' => false,
                'message' => 'Müşteri Paraşüt\'te bulunamadı'
            ], 404);
        }

        $contactId = $existingCustomer['customer_id'];

        // Müşterinin e-Fatura mükellefiyetini kontrol et
        $eInvoiceStatus = $parasutService->checkCustomerEInvoiceStatus($contactId);
...
if ($result['success']) {
    ...
    $invoice->update([
        'formalized' => true,
        'formalization_status' => 'pending', // Onay bekliyor
        'formalization_type' => $formalizationType,
        'formalization_error' => null,
        'formalization_job_id' => $result['job_id'] ?? null // ✅ Job ID'yi kaydet
    ]);

    // ⚠️ Durum kontrolü için daha uzun bekle (Paraşüt job processing için)
    sleep(5);
    ...
    if ($statusCheck['success'] && $statusCheck['formalized']) {
        ...
        // ✅ Eğer PDF varsa indir
        if ($statusCheck['pdf_url']) {
            Log::info('e-Arşiv PDF URL bulundu', ['url' => $statusCheck['pdf_url']]);
        }
    }
