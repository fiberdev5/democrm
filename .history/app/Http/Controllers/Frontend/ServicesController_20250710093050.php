...
private function processKonsinyeSelection(Request $request, $tenant_id)
{
    ...
    // Konsinye cihaz verilerini al
    $tumKonsinyeCihazlar = $request->input('konsinye_cihaz', []);
    $tumKonsinyeAdetler = $request->input('konsinye_adet', []);

    // Debug için log ekleyin
    \Log::info('Konsinye Request Data:', [
        'konsinye_cihaz' => $tumKonsinyeCihazlar,
        'konsinye_adet' => $tumKonsinyeAdetler,
        'planid' => $planid
    ]);

    // Bu plana ait konsinye cihazları al
    $planKonsinyeCihazlar = $tumKonsinyeCihazlar[$planid] ?? [];
    $planKonsinyeAdetler = $tumKonsinyeAdetler[$planid] ?? [];

    // Eğer veri yoksa, boş string döndür
    if (empty($planKonsinyeCihazlar)) {
        return '';
    }

    foreach ($planKonsinyeCihazlar as $stokId => $stokIdVal) {
        $adet = abs($planKonsinyeAdetler[$stokId] ?? 1);
        $konsinyeCevap[] = "{$stokId}---{$adet}";

        // Debug için log
        \Log::info('Processing konsinye:', [
            'stokId' => $stokId,
            'adet' => $adet,
            'gelenIslem' => $servisPlan->gelenIslem
        ]);
