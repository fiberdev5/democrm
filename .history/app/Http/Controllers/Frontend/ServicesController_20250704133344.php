...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        // Konsinye stokların hesaplanması
    $consignmentDevices = Stock::where('firma_id', $firma->id)
        ->where('urunKategori', 3) 
        ->get();
    
    $toplamConsignmentAdedi = 0; 
    $detailedConsignmentStocks = collect(); 
    ...
    foreach ($consignmentDevices as $device) {
        ...
        $device->current_stock_quantity = max(0, $girisAdet - $cikisAdet); 
        
        // Stoğu pozitif olan cihazları ekle
        if ($device->current_stock_quantity > 0) {
            $detailedConsignmentStocks->push($device); 
        }
    }

    // Ek olarak, mevcut servis planında atanan konsinye cihazı kontrol et ve eğer zaten listede yoksa ekle
    foreach ($planCevaplar as $plan) {
        $soru = StageQuestion::find($plan->soruid);
        if ($soru && $soru->cevapTuru == "[Konsinye Cihaz]" && $plan->cevap) {
            list($currentConsignmentIdWithPrefix, $currentConsignmentQuantity) = array_pad(explode('---', $plan->cevap), 2, 0);
            $currentConsignmentId = str_replace('C_', '', $currentConsignmentIdWithPrefix);

            // Eğer atanan cihaz zaten detailedConsignmentStocks içinde değilse (yani stoğu 0 veya daha az ise)
            if ($currentConsignmentId && !$detailedConsignmentStocks->contains('id', $currentConsignmentId)) {
                $originalSelectedDevice = Stock::find($currentConsignmentId);
                if ($originalSelectedDevice) {
                    // Atanan cihazın güncel stoğunu tekrar hesapla
                    $girisAdetOriginal = StockAction::where('stokId', $originalSelectedDevice->id)
                        ->whereIn('islem', [1, 4])->sum('adet');
                    $cikisAdetOriginal = StockAction::where('stokId', $originalSelectedDevice->id)
                        ->where('islem', 2)->sum('adet');
                    $originalSelectedDevice->current_stock_quantity = max(0, $girisAdetOriginal - $cikisAdetOriginal);
                    
                    // Koleksiyona ekle, böylece Blade'de her zaman listelenir
                    $detailedConsignmentStocks->push($originalSelectedDevice);
                }
            }
        }
    }

    // Toplam konsinye adedini, tüm detaylı stokların miktarını toplayarak güncelle
    $toplamConsignmentAdedi = $detailedConsignmentStocks->sum('current_stock_quantity');
        ...
        return view('frontend.secure.all_services.edit_service_plan', compact(
            ...
            'detailedConsignmentStocks', // Konsinye cihazların detaylı listesi
...
public function UpdateServicePlan(Request $request, $tenant_id)
{
    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            $soru = StageQuestion::find($cevap->soruid); // Get the question to check cevapTuru
            ...
            if ($soru->cevapTuru == '[Parca]') {
                // Parça seçimi varsa, checkbox'ları işle
                ...
                // Konsinye Cihaz seçimi varsa, işle
                $selectedConsignment = $request->input('soru' . $cevap->id); // This will be like "C_123"
                $consignmentQuantity = $request->input('consignmentAdet' . $cevap->id);

                if ($selectedConsignment && $consignmentQuantity > 0) {
                    $cevap->cevap = $selectedConsignment . '---' . $consignmentQuantity;
                } else {
                    $cevap->cevap = null; // Or handle as per your logic if nothing is selected
                }
            } else {
                // Diğer cevap türleri için genel güncelleme
        ...
        return response()->json([
            ...
            'servis_id' => $servisPlan->servisid // Ensure this is returned for history update
        ]);

    } catch (\Exception $e) {
        // Log the error for debugging
        \Log::error("Servis Planı Güncelleme Hatası: " . $e->getMessage() . " on line " . $e->getLine());
