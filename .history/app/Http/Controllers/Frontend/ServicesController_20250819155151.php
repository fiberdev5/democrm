...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
    ...
    if (!empty($stokCevap)) {
        \App\Models\ServiceStageAnswer::create([
            'firma_id' => $tenantId,
            'kid'      => auth()->user()->user_id, // işlemi yapanı eklemek iyi bir pratik
            'servisid' => $servisId,
            'planid'   => $planId,
            'soruid'   => $soruId,
            'cevap'    => $stokCevap, 
   ...
   private function parcaKullan($stokId, $adet, $servisId, $planId, $tenantId)
{
    ...
    // 1. Stok Hareketi Kaydet (Depo mantığıyla uyumlu)
    ...
    // 2. Personel Stoğundan Düşür (Depo mantığıyla uyumlu)
    ...
    // 3. Finansal Kayıtları Oluştur (Bu kısımlarınız doğruysa dokunulmadı)
