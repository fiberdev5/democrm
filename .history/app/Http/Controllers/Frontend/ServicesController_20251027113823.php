...
public function UpdateServicePlan(Request $request, $tenant_id)
{
    ...
    try {
        ...
        // İşlemi yapan personel güncellemesi
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($request->has($soruKey)) {
                ...
                if ($yeniCevap == 'Parca') {
                    // Yeni parça seçimi var mı?
                    $hasNewParca = collect($request->all())
                        ->keys()
                        ->contains(fn($key) => str_contains($key, 'stokCheck'));

                    if ($hasNewParca) {
                        $parcaCevap = $this->processParcaSelection($request, $tenant_id);
                        $cevap->cevap = $parcaCevap;
                    }
                    // Eğer yeni seçim yoksa eski değer korunur
                }
                elseif ($yeniCevap == 'Konsinye Cihaz') {
                    // Yeni konsinye seçimi var mı?
                    $hasNewKonsinye = collect($request->all())
                        ->keys()
                        ->contains(fn($key) => str_contains($key, 'konsinyeCheck'));
                    ...
                    // Normal cevap türü (metin, sayı vb.)
