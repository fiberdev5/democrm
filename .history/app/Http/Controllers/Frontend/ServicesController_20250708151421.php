         ...
         if ($request->has('soru')) {
            foreach ($request->input('soru') as $soruId => $cevap) {
                if ($cevap == "Parca") {
                    ...
                    // Konsinye Cihaz işlemlerini doğrudan burada yapıyoruz
                    // Yeni JavaScript'ten gelen input isimlerine göre güncellendi
                    if ($request->has("konsinye_cihaz.{$soruId}")) {
                        foreach ($request->input("konsinye_cihaz.{$soruId}") as $konsinyeId => $value) {
                            ...
                            if ($adet > 0) { // Sadece adet girildiyse işlem yap
                                ...
                                $consignmentName = $consignmentStock ? ($consignmentStock->urunAdi ?? $consignmentStock->urun_adi ?? 'N/A') : 'N/A';
                                ...
                                ServiceStageAnswer::create([
                                    ...
                                    'planid' => $planId,
                                    'soruid' => $soruId,
                                    'cevap' => "Konsinye Cihaz: " . $consignmentName . " (Adet: " . $adet . ")", // Cevabı daha açıklayıcı yapalım
                                    'kid' => $kid,
                                    'created_at' => now(),
                                    'updated_at' => now()
                                ]);
