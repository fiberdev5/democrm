 ...
 //Servis plan aşama düzenleme güncelleme fonksiyonu
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            // BURAYI DEĞİŞTİRİN: $cevap->id'yi parametre olarak gönderin
            $konsinyeCevap = $this->processKonsinyeSelection($request, $tenant_id, $cevap->id);
...
private function processKonsinyeSelection(Request $request, $tenant_id, $currentStageAnswerId)
{
    $konsinyeCevap = [];
    $planid = $request->input('planid'); // Bu hala geçerli
    ...
    // Sadece ilgili `currentStageAnswerId`'ye ait verileri işleyin
    if (isset($request->konsinye_cihaz[$currentStageAnswerId])) {
        foreach ($request->konsinye_cihaz[$currentStageAnswerId] as $konsinyeId => $value) {
            // Adet değerini kontrol ederken, quantity inputunun name'i konsinye_adet[stageId][consignmentId] olduğu için
            // buna uygun şekilde erişmelisiniz.
            $adet = abs($request->input("konsinye_adet.{$currentStageAnswerId}.{$konsinyeId}", 1));

            $konsinyeCevap[] = "{$konsinyeId}---{$adet}";

            // Kullanım mı teslim mi kontrol et
            if ($servisPlan->gelenIslem == "238") {
                $this->konsinyeTeslimEt($konsinyeId, $adet, $servisid, $planid, $tenant_id);
            } else {
                $this->konsinyeKullan($konsinyeId, $adet, $servisid, $planid, $tenant_id);
            }
        }
    }
