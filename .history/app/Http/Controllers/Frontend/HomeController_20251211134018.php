    ...
    if ($pricingData && $pricingData->is_active && isset($pricingData->content['pricing_plans'][$planIndex])) {
        ...
        if ($dbPlan) {
            ...
            session([
                ...
                'show_register' => true,
                'from_pricing' => true // Fiyatlandırma sayfasından geldiğini işaretle
            ]);
            
            // ?register=1 parametresi ile yönlendir
            return redirect()->route('giris', ['register' => 1])->with([
                'plan_selected' => true,
            ]);
        }
    }
    ...
    public function getSubscriptionPlans()
{
    ...
    // 1. Öncelik: Fiyatlandırma sayfasından seçilen plan
    ...
    // 2. Session'da plan ID varsa
    elseif (session()->has('selected_plan')) {
        $selectedPlanId = session('selected_plan');
    }
    // 3. Hiçbir plan seçili değilse, en ucuz planı default yap
    else {
        $cheapestPlan = $plans->first(); // Zaten price'a göre sıralı
        if ($cheapestPlan) {
            $selectedPlanId = $cheapestPlan->id;
            session(['selected_plan' => $cheapestPlan->id]);
        }
    }
    ...
    return response()->json([
        ...
        'plans' => $plans,
        'selected_plan_id' => $selectedPlanId,
        'selected_plan_info' => session('selected_plan_info')
    ]);
}
...
{
    // Sadece "Giriş Yap" butonundan geliyorsa session temizle
    // Eğer ?register=1 varsa, kayıt modu - session kalsın
    
    if (!request()->has('register')) {
        // Direkt giriş yapılıyor, tüm plan bilgilerini temizle
        session()->forget([
            'selected_plan', 
            'selected_plan_index', 
            'selected_plan_info', 
        ...
        // Kayıt yapılacak
        // Eğer fiyatlandırma sayfasından gelmiyorsa (selected_plan yoksa)
        // Default en ucuz planı seç
        if (!session()->has('selected_plan')) {
            $cheapestPlan = SubscriptionPlan::where('is_active', 1)
                ->orderBy('price', 'asc')
                ->first();
            
            if ($cheapestPlan) {
                session([
                    'selected_plan' => $cheapestPlan->id,
                    'show_register' => true
                ]);
            }
        }
    ...
    return view('frontend.auth.giris');
}
