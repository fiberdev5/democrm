    ...
    private function handleIntegrationPayment($postData)
{
    try {
        ...
        $purchase = \App\Models\IntegrationPurchase::where('payment_token', $postData['merchant_oid'])->first();

        if (!$purchase) {
            Log::error('Integration purchase not found: ' . $postData['merchant_oid'], [
                'all_purchases' => \App\Models\IntegrationPurchase::latest()->take(5)->pluck('payment_token')->toArray()
            ]);
            return response('FAIL', 200);
        }
        ...
        // Ödeme zaten işlenmişse
        if ($purchase->status !== 'pending') {
            Log::info('Integration payment already processed: ' . $postData['merchant_oid'], [
        ...
        if ($postData['status'] == 'success') {
            $purchase->update([
                ...
                'is_active' => 1,
                'activated_at' => now(),
                'transaction_id' => $postData['payment_id'] ?? $postData['merchant_oid'],
                'gateway' => 'paytr',
            ...
            // Başarı mesajını session'a kaydet
            session()->flash('integration_payment_success', [
            ...
            Log::info('Integration payment completed successfully: ' . $postData['merchant_oid'], [
                'tenant_id' => $purchase->tenant_id,
                'integration_id' => $purchase->integration_id,
                'amount' => $purchase->amount,
            ]);
            ...
            // Hata mesajını session'a kaydet
            session()->flash('integration_payment_error', [
                'message' => 'Entegrasyon ödeme işlemi başarısız.',
                'tenant_id' => $purchase->tenant_id,
                'reason' => $postData['failed_reason_msg'] ?? 'Bilinmeyen hata'
            ]);

            Log::warning('Integration payment failed: ' . $postData['merchant_oid'], [
                'integration_id' => $purchase->integration_id,
                'reason' => $postData['failed_reason_msg'] ?? 'No reason provided',
                'failed_reason_code' => $postData['failed_reason_code'] ?? 'N/A'
            ]);
