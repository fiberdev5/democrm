 ...
 public function StoreStockAction(Request $request, $tenant_id)
    {
        ...
        // islem değeri 3 ise 'personel' alanını zorunlu yap
        $validator->sometimes('personel', 'required|integer', function ($input) {
            return $input->islem == 3;
        });
        ...
        // Personel'e gönder için yeterli stok var mı?
        if ($request->islem == 3) {
            $mevcutStok =StockAction::where('stokId', $stokId)
                ->where('firma_id', $firma->id)
                ->selectRaw("
        ...
        if ($request->islem == 1) { // Alış işlemi ise
            $stockAction->kid = $personel_id; // İşlemi yapan personel (alışı kim yaptı?)
        } elseif ($request->islem == 2) { // Serviste Kullanım
            $stockAction->servisid = $request->servisid;
            $stockAction->kid = $personel_id; // Serviste kullanan personel (mevcut kullanıcı)
        } elseif ($request->islem == 3) { // Personel'e Gönder
             $stockAction->kid = $personel_id; 
            $stockAction->pid = $request->personel; // Stokun gönderildiği personel
        }
        $stockAction->save();
        ...
        // Personel'e gönderme işlemi ise
         if ($request->islem == 3) {
        // Hedef personelde bu stoktan var mı kontrol et
        ...
        if ($personelStok) {
            ...
            $personelStok = PersonelStock::create([
                ...
                'kid'      => $firma->id,
                'firma_id' => $firma->id,
                'pid'      => $request->personel, // Stokun gönderildiği personel
                'adet'     => $request->adet,
            ]);
        ...
        $stockAction->save(); // perStokId'yi kaydetmek için tekrar save

        // Personel stok hareketinden sonra başarılı mesaj
        return redirect()->back()->with([
...
public function DeleteStockAction($tenant_id, $id)
{
    ...
    if ($stockAction->islem == 1) {
        ...
        if ($cikisVarMi) {
            return redirect()->back()->with([
                'message'    => 'Bu alış hareketinden sonra çıkış yapıldığı için silinemez.',
                'alert-type' => 'warning',
            ]);
        }
    }
...
public function DeleteConsignmentStockAction($tenant_id, $id)
{
    ...
    // 1. Adım: islem 2 (serviste kullanım) veya islem 4 (müşteriden iade) ise doğrudan silmeyi engelle.
    // Bu, sizin özel isteğiniz ve öncelikli kontrolünüzdür.
    if (in_array($stockAction->islem, [2, 4])) {
        $notification = [
            'message' => 'Bu tür bir stok hareketi (serviste kullanım veya müşteriden iade) doğrudan silinemez. İşlem yapmak için ilgili kayıt üzerinden düzenleme yapmanız gerekmektedir.',
            'alert-type' => 'warning'
        ];
        return redirect()->back()->with($notification);
    }

    // 2. Adım: Genel Kural - Bu stok hareketinin ait olduğu ürünün (stokId) başka hareketleri var mı kontrol et.
    // Silmeye çalıştığımız hareket hariç, aynı stokId'ye sahip başka hareket var mı?
    $otherStockMovementsCount = StockAction::where('stokId', $stockAction->stokId)
                                           ->where('id', '!=', $stockAction->id) // Silmeye çalıştığımız hareketi hariç tut
                                           ->count();

    if ($otherStockMovementsCount > 0) {
        $notification = [
            'message' => 'Bu ürüne ait başka stok hareket kayıtları bulunduğundan, bu işlem silinemez. Önce diğer hareketleri incelemelisiniz.',
            'alert-type' => 'warning'
        ];
        return redirect()->back()->with($notification);
    }
    
    // 3. Adım: islem 1 (alış) ise ve bu alıştan sonra çıkış yapılmışsa silinemez kontrolü.
    // Bu kontrol, genel "başka hareket var mı" kontrolünden sonra gelir, çünkü daha spesifiktir.
    if ($stockAction->islem == 1) {
        $girisTarihi = $stockAction->created_at;

        $cikisVarMi = StockAction::where('stokId', $stockAction->stokId)
            ->where('firma_id', $firma->id)
            ->whereIn('islem', [2, 3]) // Çıkış işlemleri: Serviste kullanım (2), Tedarikçiye iade (3)
            ->where('created_at', '>', $girisTarihi) // Bu alıştan sonra yapılmış mı?
            ->exists();
    ...
    // Eğer yukarıdaki hiçbir koşula takılmazsa, silme işlemine geç.
    try {
        // Silme işleminden önce stok adedini güncelleme (geri alma)
        $stock = \App\Models\Stock::find($stockAction->stokId);
        if ($stock) {
            if ($stockAction->islem == 1) { // Alış işlemi siliniyorsa, stok adeti azaltılmalı
                $stock->adet -= $stockAction->adet;
            } 
            // islem 2 ve 4 zaten yukarıda engellendiği için buraya gelmezler.
            // Eğer islem 3 (tedarikçiye iade) siliniyorsa, stok adeti artırılmalı (çünkü iade stok azaltmıştı)
            elseif ($stockAction->islem == 3) { 
                $stock->adet += $stockAction->adet;
            }
            $stock->save();
        }
        ...
        // Hata durumunda, stok adetini geri al (eğer yukarıda değişiklik yapıldıysa ve hata oluştuysa)
        if (isset($stock) && $stock->isDirty()) {
             $stock->refresh(); 
        }
        return redirect()->back()->with([
