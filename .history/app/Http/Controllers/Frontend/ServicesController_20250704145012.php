...
// Servis planını güncelleyen fonksiyon
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        $planid = $request->input('planid');

        try {
            // Log 1: Request'ten gelen tüm veriyi log'a yazalım
            Log::info('UpdateServicePlan Request Data:', $request->all());
            ...
            if (!$servisPlan) {
                Log::error('Plan bulunamadı', ['planid' => $planid, 'tenant_id' => $tenant_id]);
                return response()->json(['error' => 'Plan bulunamadı'], 404);
            }
            ...
            if ($request->has('planIslemiYapan')) {
                ...
                $servisPlan->save();
                Log::info('Servis Planı İşlemi Yapan Güncellendi:', ['pid' => $servisPlan->pid]);
            }
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if (!$soru) {
                    Log::warning('Soru bulunamadı, atlanıyor:', ['soruid' => $cevap->soruid]);
                    continue;
                }
                ...
                if ($soru->cevapTuru == '[Parca]') {
                    ...
                    Log::info('Parça Cevabı Güncellendi:', ['cevap_id' => $cevap->id, 'yeni_cevap' => $cevap->cevap]);
                } elseif ($soru->cevapTuru == '[Konsinye Cihaz]') {
                    // Blade'deki hidden input'un adı 'soru' + $cevap->id
                    $selectedConsignmentValue = $request->input('soru' . $cevap->id); 
                    
                    // Log 2: Konsinye cihaz için request'ten gelen değeri log'a yazalım
                    Log::info('Konsinye Cihaz Cevabı Requestten:', ['cevap_id' => $cevap->id, 'gelen_deger' => $selectedConsignmentValue]); 

                    if ($selectedConsignmentValue) {
                        $cevap->cevap = $selectedConsignmentValue;
                    } else {
                        $cevap->cevap = null;
                    }
                    Log::info('Konsinye Cihaz Cevabı Kaydediliyor:', ['cevap_id' => $cevap->id, 'kaydedilen_deger' => $cevap->cevap]);
                    
                } else {
                    $soruKey = 'soru' . $cevap->id;
                    if ($request->has($soruKey)) {
                        $cevap->cevap = $request->input($soruKey);
                        Log::info('Diğer Cevap Türü Güncellendi:', ['cevap_id' => $cevap->id, 'yeni_cevap' => $cevap->cevap]);
                    }
                }
            ...
            Log::info('Plan başarıyla güncellendi', ['planid' => $planid]);
            return response()->json([
                'success' => true,
            ...
            Log::error("Servis Planı Güncelleme Hatası: " . $e->getMessage() . " on line " . $e->getLine(), [
                'exception' => $e,
                'request_data' => $request->all() // Hata durumunda tüm request verisini log'a ekleyelim
            ]);
