...
private function getLastServices($tenant_id)
{
    ...
    $statusMap = [
        ...
        270 => ['name' => 'Deneme Aşaması', 'class' => 'status-pending'],
...
//Kritik stoklar
//Kritik stoklar - Debug ve düzeltilmiş versiyon
private function getStockAlerts($tenant_id)
{
    $critical_level = 3; // Bu seviye ve altı KRİTİK
    $low_level = 4;      // Bu seviye ve altı DÜŞÜK (ama kritik değil)

    try {
        // ÖNCE - Debug için tüm stok hareketlerini kontrol et
        $debugStock = DB::table('stock_actions')
            ->where('firma_id', $tenant_id)
            ->select('stokId', 'adet', 'created_at')
            ->orderBy('stokId')
            ->orderBy('created_at', 'desc')
            ->take(20)
            ->get();

        // Debug için log yazalım
        \Log::info('Debug Stock Actions:', $debugStock->toArray());

        // Her bir ürünün güncel stoğunu hesapla
        // islem = 1 (giriş) -> pozitif, islem = 2 (çıkış) -> negatif
        $currentStock = DB::table('stock_actions')
            ->select(
                'stokId',
                DB::raw('SUM(CASE WHEN islem = 1 THEN adet WHEN islem = 2 THEN -adet ELSE adet END) as current_stock'),
                DB::raw('COUNT(*) as total_actions'),
                DB::raw('MAX(created_at) as last_action_date')
            )
            ->where('firma_id', $tenant_id)
            ->groupBy('stokId');

        // Debug için stok hesaplamalarını kontrol et
        $stockDebug = $currentStock->get();
        \Log::info('Debug Current Stock Calculations:', $stockDebug->toArray());

        // Stok adedi düşük olan ürünleri getir
        $products = DB::table('stocks as s')
            ->joinSub($currentStock, 'sa', function ($join) {
                $join->on('s.id', '=', 'sa.stokId');
            })
            ->where('s.firma_id', $tenant_id)
            ->where('s.urunKategori', '!=', 3) // Konsinye kategorisini hariç tut
            ->where('sa.current_stock', '<=', $low_level) // Bu şarta taşıdık
            ->where('sa.current_stock', '>', 0) // Sıfır stoklu olanları hariç tut
            ->select(
                's.id',
                's.urunAdi',
                's.urunKodu',
            ...
            ->orderBy('sa.current_stock', 'asc')
            ->get();

        // Debug için sonuçları log'a yazalım
        \Log::info('Debug Products Found:', $products->toArray());
        ...
        // Ürünleri kritik ve düşük olarak ayır
        foreach ($products as $product) {
            ...
            } elseif ($product->current_stock <= $low_level) {
        ...
        // Final sonuçları da log'a yazalım
        \Log::info('Final Alerts Result:', [
            'critical_count' => count($alerts['critical']),
            'low_count' => count($alerts['low']),
            'critical_items' => $alerts['critical'],
            'low_items' => $alerts['low']
        ]);

        return $alerts;

    } catch (\Illuminate\Database\QueryException $e) {
        \Log::error('Stock alerts query error: ' . $e->getMessage());
        return ['critical' => [], 'low' => []];
    }
}
