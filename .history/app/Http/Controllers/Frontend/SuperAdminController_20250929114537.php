...
public function allPaymentHistory(Request $request)
{
    ...
    $isDefaultRequest = !$request->hasAny(['date_from', 'date_to', 'payment_method', 'status', 'type', 'tenant_id']) || $request->has('clear');
    ...
    // Tüm firmaları getir (Super Admin hariç)
    ...
    foreach ($tenantsToProcess as $tenant) {
        ...
        if (in_array($type, ['all', 'subscription'])) {
            ...
            if (method_exists($tenant, 'subscriptionPayments')) {
                $subscriptionQuery = $tenant->subscriptionPayments()
                ...
                $subscriptionPayments = $subscriptionQuery->get()
        ...
        if (in_array($type, ['all', 'storage'])) {
            ...
            if (method_exists($tenant, 'storagePurchases')) {
                $storageQuery = $tenant->storagePurchases()
                    ->when($dateFrom, function($query) use ($dateFrom) {
                        return $query->where('created_at', '>=', Carbon::parse($dateFrom)->startOfDay());
                    })
                    ->when($dateTo, function($query) use ($dateTo) {
                ...
                $storagePurchases = $storageQuery->get()
                    ->map(function($purchase) use ($tenant) {
                        ...
                        return [
                            ...
                            'payment_method' => $this->extractPaymentMethodUnifiedForAdmin($purchase),
                ...
                // Payment method filtresi için ek kontrol (depolama için)
                if ($paymentMethod) {
                    $storagePurchases = $storagePurchases->filter(function($purchase) use ($paymentMethod) {
                        return $purchase['payment_method'] === $this->formatPaymentType($paymentMethod);
                    });
                }
    ...
    $payments = $allPayments->sortByDesc('created_at')->values();
        ...
        // ÖZET İSTATİSTİKLERİ HESAPLA (tüm ödemeler üzerinden)
    $summaryStats = [
        ...
        'subscription_completed' => $payments->where('type', 'subscription')->where('status', 'completed')->sum('amount'),
        'subscription_pending' => $payments->where('type', 'subscription')->where('status', 'pending')->sum('amount'),
        'subscription_failed' => $payments->where('type', 'subscription')->where('status', 'failed')->sum('amount'),
        'subscription_total' => $payments->where('type', 'subscription')->sum('amount'),
        
        'storage_completed' => $payments->where('type', 'storage')->where('status', 'completed')->sum('amount'),
        'storage_pending' => $payments->where('type', 'storage')->where('status', 'pending')->sum('amount'),
        'storage_failed' => $payments->where('type', 'storage')->where('status', 'failed')->sum('amount'),
        'storage_total' => $payments->where('type', 'storage')->sum('amount'),
    ];
    ...
    // Filtreler için veri
    $paymentMethods = $this->getAllPaymentMethods($tenants);
    $statuses = [
        'active' => 'Aktif',
