...
public function SurveyStore(Request $request, $tenant_id, $servisId)
{
    ...
    try {
        ...
        if (!$anket) {
            ...
            $anket->ekleyen = $user->id;
            $isNew = true;
        }

        // Öncelikle ServiceStageAnswer'dan gelen personel
        $personelIdToAssign = ServiceStageAnswer::where('servisid', $servisId)
                                ->where('soruid', 45)
                                ->value('cevap');

        // Bayi ise: formdan gelen geçerli bir personel ID varsa onu kullan
        if ($isBayi) {
            if ($request->filled('personel')) {
                $personelIdToAssign = (int) $request->input('personel');
            }
        } else {
            // Merkez personel: anketi yapan kişi kendi ID'sini yazdırır
