...
public function SurveyCreate($tenant_id, $servisId)
{
    ...
    $mevcutAnket = Survey::with(['bayiUser', 'personelUser'])->where('servisid', $servisId)->first();
   ...
   public function SurveyStore(Request $request, $tenant_id, $servisId)
{
    ...
    try {
        ...
        if (!$anket) {
            ...
            $anket->ekleyen = Auth::user()->user_id; 
            dd(Auth::user());
 ...
 if ($isBayi) {
            $anket->bayi = $user->user_id;
            $anket->personel = null;  // bayi ise personel boş
        } else {
            // bayi değilse, personel id atanır
            $personelYapilanServis = ServiceStageAnswer::where('servisid', $servisId)
                ->where('soruid', 45)
                ->first();

            if ($personelYapilanServis && is_numeric($personelYapilanServis->cevap)) {
                $anket->personel = (int) $personelYapilanServis->cevap;
            } elseif ($request->filled('personel')) {
                $anket->personel = (int) $request->input('personel');
            } else {
                $anket->personel = null;
            }

            $anket->bayi = null; // bayi değilse bayi alanı boş
        }
