...
{
    // Firma ve servis kayıtlarını bulun. Bulunamazsa 404 hatası döndürür.
    $firma = Tenant::findOrFail($tenant_id);
    $service_id = Service::where('firma_id', $firma->id)->findOrFail($id);

    // Gereken tüm verileri tek tek sorgulayın.
    $service_resources = ServiceResource::where('firma_id', $firma->id)->orderBy('kaynak', 'asc')->get();
    $iller = DB::table('ils')->orderBy('name', 'ASC')->get();
    $device_brands = DeviceBrand::where('firma_id', $firma->id)->orderBy('marka', 'asc')->get();
    $device_types = DeviceType::where('firma_id', $firma->id)->orderBy('cihaz', 'asc')->get();
    ...
    // Konsinye cihazlar için kategori ID'si
    $konsinyeKategoriId = 3;

    // Konsinye stokları çekin.
    $konsinyeCihazlar = Stock::where('firma_id', $firma->id)
        ->where('urunKategori', $konsinyeKategoriId)
        ->get();

    // Seçili konsinye cihazları güvenli bir şekilde JSON'dan çözün.
    // Eğer `konsinye_cihazlar_json` null veya boşsa, boş bir dizi döner.
    $seciliKonsinyeCihazlar = json_decode($service_id->konsinye_cihazlar_json ?? '[]', true);

    // Alt aşamaları çekin.
    $altAsamalar = collect(); // Başlangıçta boş bir koleksiyon tanımlayın.
    ...
    // Gerekli tüm verileri view'e gönderin.
    return view('frontend.secure.all_services.service_information', compact(
        'firma',
        'service_id',
        'service_resources',
        'iller',
