...
class ServicesController extends Controller
{
    ...
    public function ServicetoPdf($tenant_id, $id) {
        ...
        // PDF oluştur
        $pdf = Pdf::loadView('pdf.servis_formu', $data);
        ...
        $pdf = Pdf::loadView('frontend.secure.all_services.service_to_pdf',$data)->setOption('isHtml5ParserEnabled', true)
        ->setOption('isPhpEnabled', true)
        ->setOption('isHtml5ParserEnabled', true);
        return $pdf->stream();
    }

    private function getServisDetay($servisId, $kid)
    {
        // Servis bilgisini al
        $servis = DB::table('servisler')->where('id', $servisId)->first();
        ...
        // Tarih ve saat bilgilerini ayır
        $tarihSaat = explode(' ', $servis->kayitTarihi);
        $tarih = explode('-', $tarihSaat[0]);
        $saat = explode(':', $tarihSaat[1]);
        
        // İlgili tabloların bilgilerini al
        $musteri = DB::table('musteriler')->where('id', $servis->musteriid)->first();
        $cihazMarka = DB::table('cihaz_markalari')->where('id', $servis->cihazMarka)->first();
        $cihazTur = DB::table('cihaz_turleri')->where('id', $servis->cihazTur)->first();
        $servisDurum = DB::table('servis_asamalari')->where('id', $servis->servisDurum)->first();
        
        // Son 5 işlemi al
        $eskiIslemler = DB::table('servis_planlama')
        ...
        $paraIslemler = DB::table('servis_para_hareketleri')
                         ->where('servisid', $servis->id)
                         ->orderBy('id', 'DESC')
                         ->get();
        
        // Mesaj ayarlarını al
        $mesajObj = DB::table('servis_form_ayarlari')->where('kid', $kid)->first();
        $mesaj = $mesajObj ? $mesajObj->mesaj : '';
        
        // Garanti kontrolü
        $garantiBitis = null;
        if ($servis->garantiSuresi != "0") {
            $garanti = DB::table('garanti_sureleri')->where('id', $servis->garantiSuresi)->first();
        ...
        $servisPlanlama = DB::table('servis_asama_cevaplari')
                           ->where('servisid', $servis->id)
                           ->orderBy('id', 'DESC')
                           ->get();
        ...
        foreach ($servisPlanlama as $asama) {
            if ($asama && $asama->cevapText == '[Bayi]') {
                ...
                $getUye = DB::table('personeller')->where('id', $bayiPersonelId)->first();
                
                if ($getUye) {
                    $logoPath = $getUye->resimyol;
                    $webSitesi = " ";
        ...
        if (!$getUye) {
            $getUye = DB::table('uyeler')->where('id', $kid)->first();
            if ($getUye) {
                $logoPath = $getUye->logo;
                $webSitesi = $getUye->webSitesi ?? " ";
                $mesaj = str_replace("[TEL]", $getUye->tel2, $mesaj);
            }
        }
        ...
        $islemDetaylari = [];
        foreach ($eskiIslemler as $eskiIslem) {
            $tarihSaat = explode(" ", $eskiIslem->tarih);
            $tarihArray = explode("-", $tarihSaat[0]);
            $saatArray = explode(":", $tarihSaat[1]);
            
            $asama = DB::table('servis_asamalari')->where('id', $eskiIslem->gidenIslem)->first();
            $aciklamalar = DB::table('servis_asama_cevaplari')
            ...
            foreach ($aciklamalar as $aciklama) {
                if (!empty($aciklama->cevap)) {
                    $soru = DB::table('servis_asama_sorulari')->where('id', $aciklama->soruid)->first();
                    
                    if (strpos($soru->cevap, "Grup") !== false) {
                        $personel = DB::table('personeller')->where('id', $aciklama->cevap)->first();
                        $aciklamaMetni .= '<strong>' . $soru->soru . '</strong>: ' . ($personel->adsoyad ?? '') . "<br>";
                    } else if ($soru->cevap == "[Arac]") {
                        $arac = DB::table('servis_araclari')->where('id', $aciklama->cevap)->first();
                        ...
                        foreach ($parcalar as $parca) {
                            ...
                            $adet = $parcaArray[1] ?? 1;
                            $stok = DB::table('stoklar')->where('id', $parcaId)->first();
                            $aciklamaMetni .= ($stok->urunAdi ?? '') . " (" . $adet . "), ";
                        }
                        $aciklamaMetni .= "<br>";
                    } else if ($soru->cevap == "[Bayi]") {
                        $bayi = DB::table('personeller')->where('id', $aciklama->cevap)->first();
                        $aciklamaMetni .= '<strong>' . $soru->soru . '</strong>: ' . ($bayi->adsoyad ?? '') . "<br>";
                    } else {
        ...
        foreach ($paraIslemler as $paraIslem) {
            $tarihSaat = explode(" ", $paraIslem->tarih);
            $tarihArray = explode("-", $tarihSaat[0]);
            
            $personel = DB::table('personeller')->where('id', $paraIslem->pid)->first();
            $odemeSekli = DB::table('odeme_sekilleri')->where('id', $paraIslem->odemeSekli)->first();
