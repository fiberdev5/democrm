...
public function stop(Request $request)
{
    ...
    if ($request->expectsJson()) {
        // Super admin ise kendi dashboardına, değilse normal home'a yönlendir
        $redirectUrl = $originalUser->isSuperAdmin() 
            ? route('super.admin.dashboard', ['tenant_id' => $originalUser->tenant_id])
            : route('secure.home', ['tenant_id' => $originalUser->tenant_id]);
    ...
    return redirect()->route($redirectRoute, ['tenant_id' => $originalUser->tenant_id])->with($notification);
}
