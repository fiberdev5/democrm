   ...
   private function getServicePlansForDates($tarihListesi, $tenant_id, $cihazTur = null)
{
    $query = ServiceStageAnswer::query()
        ...
        ->whereIn('service_stage_answers.soruid', [48, 35]); // Örnek ID'ler

    // getDatesFromRange fonksiyonundan dönen d/m/Y formatını Y-m-d formatına çevirelim
    $tarihListesiYmd = array_map(function($tarih) {
        return Carbon::createFromFormat('d/m/Y', $tarih)->format('Y-m-d');
    }, $tarihListesi);

    $query->whereIn('service_stage_answers.cevap', $tarihListesiYmd);
