...
class ServicesController extends Controller
{
    ...
    public function StoreServicePhoto($tenant_id, Request $request) {
        $document = $request->file('belge');
        $extension = $request->file('belge')->extension();
        if($extension != "jpg" && $extension != "png" && $extension != "jpeg"){
            $notification = array(
                'message' => ' Dosya uzantısı sadece jpg,png,jpeg olmalıdır.',
                'alert-type' => 'warning'
            );
            return redirect()->back()->with($notification);
        }
        $fileName = time().'.'.$document->getClientOriginalExtension();  
        $save_url = $document->move('upload/servis_fotolari', $fileName);
        ...
        return response()->json([
            ...
            'html'    => ['photo' => $photo, 'canDelete' => true],
        ]);
    }
}
