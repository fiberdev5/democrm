...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        ->whereHas('consignmentStocks', function($query) {
            $query->where('quantity', '>', 0);
        })
        ->with(['consignmentStocks' => function($query) {
            $query->select('stock_id', \DB::raw('SUM(quantity) as current_stock_quantity'))
                ->groupBy('stock_id');
        }])
        ->get()
        ->map(function($stock) {
            $stock->current_stock_quantity = $stock->consignmentStocks->sum('quantity');
            return $stock;
        });

    // Toplam Konsinye Cihaz Stoğu
