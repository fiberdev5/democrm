...
public function uploadPhoto(Request $request, $tenant_id)
{
   $request->validate([
    ...
    // Bu stok için mevcut fotoğrafları kontrol et
    $existingPhotos = stock_photos::where('stock_id', $request->stock_id)
                                  ->where('kid', $tenant_id)
                                  ->get();

    // Eğer zaten bir fotoğraf varsa, eskisini sil ve yenisini ekle
