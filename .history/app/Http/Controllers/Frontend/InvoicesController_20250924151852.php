...
class InvoicesController extends Controller
{
    public function AllInvoice(Request $request, $tenant_id) {
        ...
        if ($request->ajax()) {           
            ...
            ->rawColumns(['id','faturaTarihi','faturaNumarasi','mid','genelToplam','actions'])
            ->make(true);
    ...
    public function AddInvoice($tenant_id) {
        ...
        $musteriler = Customer::where('firma_id', $tenant_id)->orderBy('adSoyad', 'ASC')->get();
        $tenants = Tenant::where('firma_adi', '!=', 'Super Admin Panel')->orderBy('firma_adi', 'ASC')->get(); 
   ...
   public function StoreInvoice(Request $request, $tenant_id)
{
    try {
        ...
        \Log::info('Fatura ekleme isteği:', [
            'tenant_id' => $tenant_id,
            'firma_id' => $request->firma_id,
            'payment_count' => count($request->payment_type ?? [])
        ]);

        // Validation
        $validateData = $request->validate([
            'document' => 'required|max:2000|mimes:jpg,png,jpeg,pdf',
            'firma_id' => 'required|exists:tenants,id',
            'faturaNumarasi' => 'required|string|max:255',
            'faturaTarihi' => 'required|date',
            'odemeSekli' => 'required|integer',
            'toplam' => 'required|numeric|min:0',
            'genelToplam' => 'required|numeric|min:0',
            ...
            'document.required' => 'Fatura dosyası gereklidir',
            'document.mimes' => 'Dosya uzantısı sadece jpg, png, jpeg veya pdf olmalı',
            'firma_id.required' => 'Firma seçimi gereklidir',
            'faturaNumarasi.required' => 'Fatura numarası gereklidir',
            'aciklama.*.required' => 'Ürün açıklaması gereklidir',
        ]);

        $toplam = $this->convertToDecimal($request->toplam);
        $indirim = $this->convertToDecimal($request->indirim ?? 0);
        $kdv = $this->convertToDecimal($request->kdv ?? 0);
        ...
        // Çoklu ödeme bilgilerini hazırla
        $paymentDetails = [];
        if ($request->payment_type && $request->payment_id) {
            foreach ($request->payment_type as $index => $type) {
                $paymentDetails[] = [
                    'payment_type' => $type,
                    'payment_id' => $request->payment_id[$index] ?? null,
                    'description' => $request->aciklama[$index] ?? null,
                    'amount' => $this->convertToDecimal($request->tutar[$index] ?? 0),
                ];
            }
        }

        // Database işlemi - Transaction kullan
        DB::beginTransaction();
        
        try {
            // Hangi tabloyu kullanacağınızı belirleyin
            // Seçenek 1: invoices tablosunu kullan (Model ile)
            $invoiceId = DB::table('super_admin_invoices')->insertGetId([
                'firma_id' => $request->firma_id,
                'payment_type' => $request->payment_type[0] ?? null,
                'odeme_id' => $request->payment_id[0] ?? null,
                ...
                'indirim' => $indirim,
                'kdv' => $kdv,
                'kdvTutar' => $request->kdvTutar ?? 0,
                'genelToplam' => $genelToplam,
                'toplamYazi' => $request->toplamYazi ?? '',
                'kayitAlan' => auth()->user()->user_id ?? auth()->id(),
                'faturaPdf' => $save_url,
            ...
            // VEYA Seçenek 2: super_admin_invoices kullanmaya devam edin
            /*
            ...
            */
            ...
            foreach($aciklama as $key => $val){
                if(!empty($val)){
                    // invoices tablosu için InvoiceProduct model'i kullan
                    ...
                    // VEYA super_admin_invoices için:
                    /*
                    DB::table('super_admin_invoice_products')->insert([
                        'faturaid' => $invoiceId,
                        'aciklama' => $val,
                        'miktar' => $miktar[$key] ?? 1,
                        'fiyat' => $this->convertToDecimal($fiyat[$key] ?? 0),
            ...
            $customer = Tenant::find($request->firma_id);
            $customerName = $customer ? $customer->firma_adi : null;
            
            // Log kaydı
            ActivityLogger::logInvoiceCreated($invoiceId, $request->faturaNumarasi, $customerName);
            
            $notification = array(
                'message' => "Fatura Başarıyla Eklendi (ID: {$invoiceId}, {$productCount} ürün, " . count($paymentDetails) . " ödeme)",
                'alert-type' => 'success'
            );
        ...
        \Log::error('StoreInvoice genel hata:', [
            'error' => $e->getMessage(),
