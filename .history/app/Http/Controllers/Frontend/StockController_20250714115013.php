...
public function barkodPdf($tenant_id, $id) {
    ...
    // Debug: Barkod HTML'ini kontrol edin
    $barcodeHTML = DNS1D::getBarcodeHTML($stock->urunKodu, 'C128', 2, 60);
    
    // Eğer boşsa alternatif kullanın
    if (empty($barcodeHTML) || strlen($barcodeHTML) < 50) {
        // Alternatif barkod kütüphanesi
        $generator = new \Picqer\Barcode\BarcodeGeneratorHTML();
        $barcodeHTML = $generator->getBarcode($stock->urunKodu, $generator::TYPE_CODE_128);
    }
    
    $pdf = Pdf::loadView('frontend.secure.stocks.stocks_barkod', compact('stock', 'barcodeHTML'))
