...
class ServicesController extends Controller
{
    ...
    public function ServiceStageQuestionShow($tenant_id ,$asamaid, $serviceid) {
        ...
        // Normal servis işlemleri
            ...
            // urunKategori = 3, konsinye cihazlar için varsayılan kategoridir.
            $konsinyeCihazlar = Stock::where('firma_id', $firma->id)
                                    ->where('urunKategori', 3) // Konsinye cihaz kategorisi ID'si
                                    ->orderBy('id', 'asc')
                                    ->get();

            // Konsinye cihazlar için toplam kalan stok adedi hesaplama
            $toplamKonsinyeCihazAdedi = 0;
            foreach ($konsinyeCihazlar as $cihaz) {
                $girisAdet = StockAction::where('stokId', $cihaz->id)
                                        ->whereIn('islem', [1, 4]) // Alış veya müşteriden iade
                                        ->sum('adet');
                $cikisAdet = StockAction::where('stokId', $cihaz->id)
                                        ->where('islem', 2) // Serviste kullanım (çıkış)
                                        ->sum('adet');
                $kalanStok = $girisAdet - $cikisAdet;
                $toplamKonsinyeCihazAdedi += $kalanStok;
            }
            ...
            return view('frontend.secure.all_services.service_stage_questions_show', 
                    compact('stage_questions', 'stage_id', 'service_id', 'firma', 'islem', 'personeller', 'araclar','stoklar','toplamPersonelStokAdedi','konsinyeCihazlar','toplamKonsinyeCihazAdedi'));
    ...
    public function SaveServicePlan(Request $request, $tenant_id) {
        ...
        try {
            ...
            // 'gelenIslem' verisini güvenli bir şekilde al ve JSON'dan diziye dönüştür.
            $gelenIslemInput = $request->input('gelenIslem');
            $gelenIslem = []; // Varsayılan olarak boş bir dizi tanımla
            $gelenIslemId = null; // **BURADA EKLENDİ: $gelenIslemId'yi başlangıçta null olarak tanımla**

            if ($gelenIslemInput) {
                $decoded = json_decode($gelenIslemInput, true);
                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                    $gelenIslem = $decoded; // Eğer başarılı ve dizi ise $gelenIslem'e ata
                } else {
                    // JSON çözmede hata varsa veya dizi değilse logla
                    Log::error("SaveServicePlan: gelenIslem input'u geçersiz veya dizi değil. Input: " . $gelenIslemInput);
                    return response()->json(['status' => 'error', 'message' => 'Hata: Gelen işlem verisi hatalı.']);
                }
            } else {
                Log::warning("SaveServicePlan: gelenIslem input'u boş geldi. Servis ID: " . $servisId);
                // Gelen işlem bilgisi olmadan devam edilemiyorsa hata döndür.
                return response()->json(['status' => 'error', 'message' => 'Hata: Gelen işlem bilgisi eksik.']);
            }

            // 'gelenIslem' ID'sinin varlığını kontrol et
            if (!isset($gelenIslem['id'])) {
                Log::error("SaveServicePlan: gelenIslem dizisi içinde 'id' anahtarı bulunamadı. Array: " . json_encode($gelenIslem));
                return response()->json(['status' => 'error', 'message' => 'Hata: Gelen işlem ID\'si bulunamadı.']);
            }

            // **BURADA DÜZELTME: Sadece 'id' varsa $gelenIslemId'yi ata**
            $gelenIslemId = $gelenIslem['id'];
            ...
            // Stok kontrolü: Hem personel hem de konsinye cihaz stokları kontrol edilecek.
            // Güvenli bir şekilde aldığımız ID'yi gönderiyoruz.
            // **BURADA KULLANILIYOR: $gelenIslemId tanımlı olduğundan emin olmalıyız**
            $stokHatasiVar = $this->stokKontrolEt($request, $gelenIslemId);
            if ($stokHatasiVar) {
            ...
            $planData = [
                ...
                'pid' => $kid,
                'servisid' => $servisId,
                'gelenIslem' => $gelenIslemId, // Güvenli ID'yi kullanıyoruz
            ...
            if ($planId) {
                ...
                // **BURADA KULLANILIYOR: $gelenIslemId tanımlı olduğundan emin olmalıyız**
                $this->soruCevaplariniIsle($request, $servisId, $planId, $tenant_id, $gelenIslemId);
                ...
                // 'asamalar' ilişkisinin varlığını kontrol edelim
                $guncellenmisAsamaBilgisi = 'Bilinmiyor';
                $altAsamalar = collect();
    ...
    return response()->json([
        'status' => 'error',
        'message' => 'HATA! Servis aşama eklenemedi.'
    ]);
}

        } catch (\Exception $e) {
            // Hata oluşursa logluyoruz ve kullanıcıya bilgi veriyoruz.
            Log::error("SaveServicePlan Hatası: " . $e->getMessage() . " Satır: " . $e->getLine() . " Dosya: " . $e->getFile());
    ...
    {
    $tenant_id = $request->route('tenant_id'); // URL'den tenant_id'yi alıyoruz.

        // 1. Personel Stokları İçin Kontrol
        // Formdan gelen 'personelStokCheck' ve 'personelStokAdet' inputlarını işliyoruz.
        if ($request->has('personelStokCheck') && is_array($request->input('personelStokCheck'))) {
            $personel_id = Auth::user()->user_id;

            foreach ($request->input('personelStokCheck') as $stokId => $checked) {
                if ($checked == 'on') { // Sadece seçili olanları kontrol et.
                    $adet = (int) $request->input("personelStokAdet.{$stokId}");

                    $personelStok = PersonelStock::where('pid', $personel_id)
                                                 ->where('stokid', $stokId)
                                                 ->first();

                    // Yetersiz personel stoğu kontrolü.
                    if (!$personelStok || $adet <= 0 || $adet > $personelStok->adet) {
                        $urunAdi = 'Bilinmeyen Ürün';
                        // PersonelStock modelinde 'stock' ilişkisi olduğunu varsayarak ürün adını alıyoruz.
                        if ($personelStok && $personelStok->stock) {
                            $urunAdi = $personelStok->stock->urunAdi;
                        }
                        $mevcutAdet = $personelStok ? $personelStok->adet : 0;
                        return "STOKHATA: " . mb_convert_case($urunAdi, MB_CASE_TITLE, "UTF-8") . " (Personel Stoğu) Adeti Yetersizdir. Mevcut: {$mevcutAdet}, İstenen: {$adet}.";
                    }
                }
            }
        }

        // 2. Konsinye Cihaz Stokları İçin Kontrol
        // Formdan gelen 'selectedConsignmentDevices' ve 'consignmentDeviceQuantities' inputlarını işliyoruz.
        if ($request->has('selectedConsignmentDevices') && is_array($request->input('selectedConsignmentDevices'))) {
            foreach ($request->input('selectedConsignmentDevices') as $cihazId => $value) {
                $adet = (int) $request->input("consignmentDeviceQuantities.{$cihazId}");

                // Konsinye cihazın mevcut genel stoğunu hesaplıyoruz.
                // StockAction tablosundaki giriş ve çıkış hareketlerini topluyoruz.
                $girisAdet = StockAction::where('stokId', $cihazId)
                                        ->whereIn('islem', [1, 4]) // 1: Giriş, 4: Müşteriden İade
                                        ->sum('adet');
                $cikisAdet = StockAction::where('stokId', $cihazId)
                                        ->where('islem', 2) // 2: Serviste Kullanım (Çıkış)
                                        ->sum('adet');
                $kalanStok = $girisAdet - $cikisAdet;

                // Yetersiz konsinye cihaz stoğu kontrolü.
                if ($adet <= 0 || $adet > $kalanStok) {
                    $cihazAdi = Stock::find($cihazId) ? Stock::find($cihazId)->urunAdi : 'Bilinmeyen Cihaz';
                    return "STOKHATA: " . mb_convert_case($cihazAdi, MB_CASE_TITLE, "UTF-8") . " (Konsinye Cihaz Stoğu) Adeti Yetersizdir. Mevcut: {$kalanStok}, İstenen: {$adet}.";
                }
            }
        }

        // 3. Özel Durum: Aşama 238 (Parça Teslim Et) ise stok seçimi zorunlu.
        if ($gelenIslemId == "238") {
            $stokSecildi = false;
            // Hem personel stoğu hem de konsinye cihaz stoğundan herhangi birinin seçilip seçilmediğini kontrol ediyoruz.
            if (($request->has('personelStokCheck') && count($request->input('personelStokCheck')) > 0) ||
                ($request->has('selectedConsignmentDevices') && count($request->input('selectedConsignmentDevices')) > 0)) {
                $stokSecildi = true;
            }

            if (!$stokSecildi) {
                return "STOKHATA: Parça Teslim Ederken Stok Seçimi Zorunludur.";
            }
        }

        return null; // Herhangi bir stok hatası yoksa null döner.
    }
    ...
    private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem)
    {
       $personel_id = Auth()->user()->user_id;
        ...
        // 'soru' inputunun varlığını ve dizi olup olmadığını kontrol ediyoruz.
        if ($request->has('soru') && is_array($request->input('soru'))) {
            foreach ($request->input('soru') as $questionId => $answerValue) {
                $stageQuestion = StageQuestion::find($questionId);

                if ($stageQuestion) {
                    if ($stageQuestion->cevapTuru == "[Parca]") {
                        // Cevap türü '[Parca]' ise, personel stoklarını işliyoruz.
                        if ($request->has('personelStokCheck') && is_array($request->input('personelStokCheck'))) {
                            foreach ($request->input('personelStokCheck') as $stokId => $checked) {
                                if ($checked == 'on') { // Sadece seçili olanları işliyoruz.
                                    $adet = (int) $request->input("personelStokAdet.{$stokId}");

                                    $personelStok = PersonelStock::where('pid', $personel_id)
                                                                 ->where('stokid', $stokId)
                                                                 ->first();
                                    // Adet 0'dan büyükse ve personel stoğu varsa işlemi yapıyoruz.
                                    if ($personelStok && $adet > 0) {
                                        // Personel stoğundan düşüş yapıyoruz.
                                        $personelStok->decrement('adet', $adet);

                                        // StockAction kaydı oluşturuyoruz (islem: 2 servis kullanımı).
                                        $stockAction = new StockAction();
                                        $stockAction->firma_id   = $tenantId;
                                        $stockAction->pid        = $personel_id;
                                        $stockAction->stokId     = $stokId;
                                        $stockAction->servisid   = $servisId;
                                        $stockAction->islem      = 2; // Serviste kullanım (Personel Stoğundan).
                                        $stockAction->adet       = $adet;
                                        $stockAction->fiyat      = 0; // İsterseniz stoktan fiyat çekebilirsiniz.
                                        $stockAction->fiyatBirim = 'TL';
                                        $stockAction->save();

                                        Log::info("Personel Stoğu Düşüldü: Stok ID {$stokId}, Adet {$adet}, Servis ID {$servisId}");
                                    }
                                }
                            }
                        }
                    } elseif ($stageQuestion->cevapTuru == "[KonsinyeCihaz]") {
                        // Cevap türü '[KonsinyeCihaz]' ise, genel konsinye cihaz stoklarını işliyoruz.
                        if ($request->has('selectedConsignmentDevices') && is_array($request->input('selectedConsignmentDevices'))) {
                            foreach ($request->input('selectedConsignmentDevices') as $cihazId => $value) {
                                $adet = (int) $request->input("consignmentDeviceQuantities.{$cihazId}");

                                // Adet 0'dan büyükse işlemi yapıyoruz.
                                if ($adet > 0) {
                                    // StockAction kaydı oluşturuyoruz (islem: 2 servis kullanımı).
                                    $stockAction = new StockAction();
                                    $stockAction->firma_id   = $tenantId;
                                    $stockAction->pid        = $personel_id; // İşlemi yapan personel.
                                    $stockAction->stokId     = $cihazId;
                                    $stockAction->servisid   = $servisId; // İlgili servis ID'si.
                                    $stockAction->islem      = 2; // Serviste kullanım (Konsinye Cihaz Stoğundan).
                                    $stockAction->adet       = $adet;
                                    $stockAction->fiyat      = 0; // Konsinye olduğu için başlangıçta fiyat 0 olabilir.
                                    $stockAction->fiyatBirim = 'TL';
                                    $stockAction->tedarikci  = null; // Serviste kullanım olduğu için tedarikçi yok.
                                    $stockAction->save();

                                    Log::info("Konsinye Cihaz Stoğu Düşüldü: Cihaz ID {$cihazId}, Adet {$adet}, Servis ID {$servisId}");
                                }
                            }
                        }
                    } else {
...
// Personel stoğundan düş
    $perStok = PersonelStock::where('pid', auth()->user()->user_id) // <-- DOĞRU
        ->where('stokid', $stokId) // <-- DOĞRU
        ->first();
        if ($perStok) {
            PersonelStock::where('id', $perStok->id)
                ->update([
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        // Personelin üzerindeki toplam stok adedini hesapla
        $toplamPersonelStokAdedi = $stoklar->sum(function($stok) use ($personel_id) {
            $personelStokKaydi = $stok->personelStoklar->where('pid', $personel_id)->first();
            return $personelStokKaydi ? ($personelStokKaydi->adet ?? 0) : 0;
        });
    ...
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($yeniCevap == 'Parca') {
                    $this->updateParcaSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
                    $cevap->cevap = $this->getUpdatedParcaAnswer($request); // Yeni stok cevaplarını al
                } else {
    ...
    private function updateParcaSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
{
    // Mevcut parça cevaplarını sil
    ServiceStageAnswer::where('planid', $planId)
        ->where('soruid', $soruId)
        ->where('firma_id', $tenantId)
        ->delete();

    $stokCevap = []; // Stok cevaplarını tutmak için dizi
    ...
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'stokCheck') !== false) {
            ...
            // Parça teslim mi yoksa kullanma mı olduğunu belirlemek için gelenIslem değerini kullanın.
            $servisPlan = ServicePlanning::find($planId);
            if ($servisPlan && $servisPlan->gelenIslem == "238") { //parcateslim
                 $this->parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId);
            } else {
                 $this->parcaKullan($stokId, $adet, $servisId, $planId, $tenantId);
            }
    ...
    // Yeni parça cevaplarını kaydet
    if (!empty($stokCevap)) {
        ServiceStageAnswer::create([
            'firma_id' => $tenantId,
            'kid' => auth()->user()->user_id, // Güncelleme yapan kullanıcı ID'si
            'servisid' => $servisId,
            'planid' => $planId,
            'soruid' => $soruId,
            'cevap' => implode(', ', $stokCevap),
            'created_at' => now(),
            'updated_at' => now()
        ]);
    }
...
private function getUpdatedParcaAnswer(Request $request)
{
    $stokCevap = [];
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'stokCheck') !== false) {
