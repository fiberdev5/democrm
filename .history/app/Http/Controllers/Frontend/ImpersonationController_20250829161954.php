...
class ImpersonationController extends Controller
{
    ...
    {
        // Impersonation session bilgileri yoksa hata döndür
        if (!Session::has('impersonator_id') || !Session::has('impersonated_user_id')) {
            // Eğer session bilgileri zaten yoksa, kullanıcı zaten impersonate etmiyor demektir.
            // Bu durumda bir hata mesajı ile ana sayfaya yönlendirebiliriz.
            $notification = [
                'message' => 'Aktif impersonation oturumu bulunamadı.',
                'alert-type' => 'info'
            ];
            // Yönlendirme hedefi olarak güvenli bir yer seçin, örneğin login sayfası veya dashboard
            return redirect()->route('giris')->with($notification); 
        }

        $impersonatorId = Session::get('impersonator_id');
        $impersonationId = Session::get('impersonation_id');
        $impersonatedUserId = Session::get('impersonated_user_id'); // Impersonate edilen kullanıcının ID'sini de alalım

        // Impersonation kaydını sonlandır
        if ($impersonationId) {
            $impersonation = UserImpersonation::find($impersonationId);
            if ($impersonation) {
        ...
        // Session'ları temizlemeden önce orijinal kullanıcıyı auth etmeliyiz.
        // Bu, redirect loop'u engellemek için önemli.
        Session::forget([
            'impersonator_id',
        ...
        if ($originalUser) {
            // Orijinal kullanıcıyı direkt olarak login yapıyoruz.
            // Bu, Laravel'in oturum mekanizmasıyla doğru çalışmasını sağlar.
            ...
            // Eğer JSON bekleniyorsa ona göre yanıt ver
            if ($request->expectsJson()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Kendi hesabınıza geri döndünüz.',
                    'redirect_url' => route('secure.home', ['tenant_id' => $originalUser->tenant_id])
                ]);
            }
            ...
            // Orijinal kullanıcının tenant_id'sine yönlendir
            return redirect()->route('secure.home', ['tenant_id' => $originalUser->tenant_id])->with($notification);

        } else {
            // Eğer orijinal kullanıcı bulunamazsa, bir hata mesajı ile giriş sayfasına yönlendir
            $notification = [
                'message' => 'Orijinal kullanıcı bulunamadı. Lütfen tekrar giriş yapın.',
                'alert-type' => 'danger'
            ];
            // Tüm oturumu temizleyip giriş sayfasına yönlendirmek en güvenli yoldur.
            Auth::logout();
