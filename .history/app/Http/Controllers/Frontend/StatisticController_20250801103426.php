    ...
    foreach ($surveys as $survey) {
        if (!isset($groupedSurveys[$survey->ekleyen])) {
            $groupedSurveys[$survey->ekleyen] = [
                'personel_id' => $survey->ekleyen,
                'adsoyad' => $survey->adsoyad,
                'servisler' => [],
                'completed_count' => 0
            ];
        }
        $groupedSurveys[$survey->ekleyen]['servisler'][] = $survey->servisid;
    }

    // Tamamlanan servisleri getir (gidenIslem=255 olanlar)
    // Tamamlanan servisleri personellere göre grupla
$completedSurveys = Survey::select('ekleyen', 'servisid')
    ->leftJoin('service_plannings', 'surveys.servisid', '=', 'service_plannings.servisid')
    ->where('surveys.firma_id', $tenant_id)
    ->where('service_plannings.gidenIslem', 255)
    ->whereBetween('service_plannings.created_at', [$startDateTime, $endDateTime]);

if ($deviceTypeId) {
    $completedSurveys->leftJoin('services', 'surveys.servisid', '=', 'services.id')
                     ->where('services.cihazTur', $deviceTypeId);
}

$completedData = $completedSurveys->get();

foreach ($completedData as $item) {
    if (!isset($groupedSurveys[$item->ekleyen])) {
        $groupedSurveys[$item->ekleyen] = [
            'personel_id' => $item->ekleyen,
            'adsoyad' => '-', // ad boş çünkü bu personel sadece tamamladı ama anket yok
            'servisler' => [],
            'completed_count' => 1
        ];
    } else {
        $groupedSurveys[$item->ekleyen]['completed_count']++;
    }
}



    $completedServices = $completedServicesQuery->pluck('servisid')->toArray();

    // Yapılan anketleri getir
   $surveyedServicesQuery = Survey::select('servisid')
    ->where('surveys.firma_id', $tenant_id); 


    if ($deviceTypeId) {
        $surveyedServicesQuery->leftJoin('services', 'surveys.servisid', '=', 'services.id')
                             ->where('services.cihazTur', $deviceTypeId);
    }

    $surveyedServices = $surveyedServicesQuery->pluck('servisid')->toArray();

    // Anket yapılmamış servisleri bul
    $pendingSurveys = array_diff($completedServices, $surveyedServices);
    $pendingSurveys = array_unique($pendingSurveys);

    // Toplam sayıları hesapla
    $totalCompletedServices = count($completedServices);
    $totalSurveyedServices = array_sum(array_map(function($item) {
        return count($item['servisler']);
    }, $groupedSurveys));
