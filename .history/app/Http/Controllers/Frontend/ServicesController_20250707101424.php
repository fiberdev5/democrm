...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    foreach ($consignmentDevices as $device) {
        ...
        // Servis planına ait bu konsinye cihazın onaylı adedini al (varsa)
        // Burada $planCevaplar içindeki StageQuestion ilişkisi yüklü olmalı
        $approvedConsignmentAnswer = $planCevaplar->first(function($cevap) use ($device) {
            // $cevap->soru null olabilir, bu yüzden güvenli kontrol yapıyoruz
            if ($cevap->soru?->cevapTuru == '[Konsinye Cihaz]') {
                // Cevabın JSON formatında detay içerdiğini varsayalım
                $cevapDetay = json_decode($cevap->detay ?? '{}', true);
                return isset($cevapDetay['consignment_id']) && $cevapDetay['consignment_id'] == $device->id;
            }
            return false;
        });
// Yeni hali:
$approvedConsignmentAnswer = $planCevaplar->first(function($cevap) use ($device) {
    // Önce $cevap->soru'nun null olup olmadığını kontrol edin
    // Ardından cevapTuru'na erişin
    // plan->detay alanının nasıl kaydedildiğine bağlı olarak buradaki kontrol değişebilir
    if ($cevap->soru?->cevapTuru == '[Konsinye Cihaz]') {
        // Cevabın bir JSON stringi olduğunu varsayarsak
        $cevapDetay = json_decode($cevap->detay ?? '{}', true); // detay alanı null ise boş obje olarak alın
        return isset($cevapDetay['consignment_id']) && $cevapDetay['consignment_id'] == str_replace('C_', '', $device->id);
    }
    return false;
});
            $device->approved_quantity = 0;
            if ($approvedConsignmentAnswer) {
                // Cevap formatına göre onaylı adeti çıkar
                // Örnek: Cevap "{"stok_id":1, "adet":5}" şeklinde JSON ise
                $cevapData = json_decode($approvedConsignmentAnswer->cevap);
                if (isset($cevapData->adet)) {
                    $device->approved_quantity = $cevapData->adet;
                }
            }
        }
    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($soru->cevapTuru == '[Parca]') {
                ...
                $this->updateConsignmentSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
                $cevap->cevap = $this->getUpdatedConsignmentAnswerForQuestion($request, $cevap->soruid);
            }
            else {
                // Diğer cevap türleri
...
// Konsinye cihaz seçimi ve adetlerini güncelleyen fonksiyon
private function updateConsignmentSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
{
    ...
    $consignmentCevaplari = []; // Konsinye cihaz cevaplarını tutmak için dizi

    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'consignmentCheck_') === 0) { // 'consignmentCheck_' ile başlayanları yakala
            $stokId = (int) str_replace('consignmentCheck_', '', $key); // ID'yi temizle
            $adet = abs($request->input("consignmentAdet_{$stokId}", 1)); // İlgili adeti al, varsayılan 1

            if ($adet > 0) { // Sadece pozitif adetleri işle
                $consignmentCevaplari[] = [
                    'stok_id' => $stokId,
                    'adet' => $adet
                ];
                ...
                $servisPlan = ServicePlanning::find($planId);
                if ($servisPlan) {
                    $islemTuru = ($servisPlan->gidenIslem == "259") ? 3 : 2;
                    $this->handleStockAction($stokId, $adet, $servisId, $planId, $tenantId, $islemTuru, 3); // 3: UrunKategori Cihaz
                }
            }
        }
    }
    ...
    if (!empty($consignmentCevaplari)) {
        ServiceStageAnswer::create([
            ...
            'planid' => $planId,
            'soruid' => $soruId,
            'cevap' => json_encode($consignmentCevaplari), // JSON olarak kaydet
            'created_at' => now(),
            'updated_at' => now()
        ]);
    }
...
// Güncellenmiş konsinye cihaz cevabını JSON formatında döndüren fonksiyon
private function getUpdatedConsignmentAnswerForQuestion(Request $request, $soruId)
{
    $consignmentCevaplari = [];
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'consignmentCheck_') === 0) {
            $stokId = (int) str_replace('consignmentCheck_', '', $key);
            $adet = abs($request->input("consignmentAdet_{$stokId}", 1));
    ...
    return json_encode($consignmentCevaplari);
}
