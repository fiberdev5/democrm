...
private function calculateCancelledServices($teknisyenId, $servisler)
{
    ...
    foreach ($iptaller as $servis) {
        ...
        if ($ilkServis && $sonIptal) {
            $ilkTarih = strtotime(explode(" ", $ilkServis->created_at)[0]);
            $iptalTarih = strtotime(explode(" ", $sonIptal->created_at)[0]);
...
private function calculateOfferedAmount($teknisyenId, $servisler, $tarih1, $tarih2)
{
    ...
    try {
        if (Schema::hasTable('service_plannings')) {
...
// calculateDailyGraphData fonksiyonunu tamamen değiştirin:
private function calculateDailyGraphData($servisler, $tarihListesi, $teknisyenId)
{
    ...
    foreach ($tarihListesi as $tarih) {
        ...
        // **TAMAMLANAN SERVİSLER - DÜZELTİLDİ**
        ...
        // **GELİR HESAPLAMA - DÜZELTİLDİ**
        $gelir = 0;
        if (!empty($gunlukServisler)) {
            // cash_transactions tablosunu kullan (kasa_hareketleri değil)
            try {
...
// Ayrıca calculateCollectedAmount fonksiyonunu da düzeltin:
private function calculateCollectedAmount($teknisyenId, $servisler, $tarih1, $tarih2)
{
    if (empty($servisler)) {
        return 0;
    ...
    try {
        ...
        if (Schema::hasTable('cash_transactions')) {
            ...
            $paraToplam = floatval($paraToplam ?: 0);
...
// calculateOfferedAmount fonksiyonunu da kontrol edin:
private function calculateOfferedAmount($teknisyenId, $servisler, $tarih1, $tarih2)
{
    if (empty($servisler)) {
        return 0;
