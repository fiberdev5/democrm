...
public function changeTenantStatus($id)
{
    ...
    // Firma aktif edildiğinde trial başlat ve mail gönder
    if ($tenant->status == 1) {
        try {
            // Eğer trial henüz kullanılmamışsa başlat
            if (!$tenant->trial_used) {
                $tenant->startTrial();
                
                Log::info('Trial Süreci Başlatıldı', [
                    'tenant_id' => $tenant->id,
            ...
            // Trial days hesapla (tenant yeniden yüklendiği için güncel değerler gelecek)
            $tenant->refresh(); // Tenant'ı yeniden yükle
            ...
            // Mail data hazırla
            $mailData = [
                'username' => $patronUser ? $patronUser->username : 'Belirsiz',
                'firma_kodu' => $tenant->firma_kodu,
                'tenant' => $tenant,
            ...
            Log::error('Firma Aktifleştirme/Trial Başlatma Hatası', [
                'tenant_id' => $tenant->id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            // Hata olsa bile status değişikliği devam etsin
        }
    }
