...
private function getServicePlansForDates($tarihListesi, $tenant_id, $cihazTur = null)
{
    ...
    // Tarih listesini hem d/m/Y hem de Y-m-d formatlarında oluştur
    $tarihListesiForQuery = [];
    foreach ($tarihListesi as $tarih) {
        $tarihListesiForQuery[] = $tarih; // d/m/Y formatı
        try {
            $tarihListesiForQuery[] = Carbon::createFromFormat('d/m/Y', $tarih)->format('Y-m-d'); // Y-m-d formatı
        } catch (\Exception $e) {
            // Hata durumunda bu formatı ekleme, logla
            \Log::warning('Geçersiz tarih formatı (d/m/Y to Y-m-d dönüşümünde): ' . $tarih);
        }
    }
    $tarihListesiForQuery = array_unique($tarihListesiForQuery); // Tekrarları kaldır

    $query->whereIn('sc.cevap', $tarihListesiForQuery);
    ...
    \Log::info('Servis plan sorgusu sonucu:', [
        'tarih_listesi_query' => $tarihListesiForQuery,
        ...
        try {
            if (Schema::hasTable($tablo)) {
                $teklifToplam = DB::table('service_stage_answers as sc')
                    ...
                    ->whereBetween('sp.tarih', [$tarih1, $tarih2])
