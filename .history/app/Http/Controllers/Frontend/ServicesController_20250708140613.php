        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($request->has($soruKey)) {
                    ...
                    if ($yeniCevap == 'Konsinye') {
                        $cevap->cevap = $this->processKonsinyeSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
                    }
                    else {
...
private function processKonsinyeSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
{
    // Cevabı temizle
    ...
    $inputKey = "konsinye[$soruId]";
    $adetKey = "konsinyeAdet[$soruId]";

    $stokId = $request->input("konsinye.$soruId");
    $adet = abs($request->input("konsinyeAdet.$soruId", 1));

    if ($stokId && $adet) {
        $stokCevap[] = "{$stokId}---{$adet}";

        // Eğer gerekiyorsa stoğu düş, logla, vb.
        // $this->konsinyeCihazCikar($stokId, $adet, $servisId, $planId, $tenantId);
    }
