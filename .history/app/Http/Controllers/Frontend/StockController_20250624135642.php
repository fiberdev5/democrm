...
public function AllStocks($tenant_id, Request $request)
{
    ...
    if ($request->ajax()) {
        $query = Stock::where('firma_id', $tenant_id);
...
//////////////////////////////////////////////Konsinye Cihazlar///////////////////////////////////////////////////////////////////////////
public function konsinyeCihazlar($tenant_id)
{
    $firma = Tenant::findOrFail($tenant_id);
    return view('stocks.konsinye', compact('firma'));
}

public function konsinyeCihazlarData(Request $request, $tenant_id)
{
    $konsinyeKategoriId = 5; // senin konsinye kategori ID’n

    $query = Stock::where('firma_id', $tenant_id)
                  ->where('kategori_id', $konsinyeKategoriId);

    // Buraya filtre eklemek istersen buradan ekleyebilirsin

    return DataTables::of($query)
        ->addColumn('action', function($row){
            return '<button class="btn btn-sm btn-primary editKonsinye" data-id="'.$row->id.'">Düzenle</button>
                    <button class="btn btn-sm btn-danger deleteKonsinye" data-id="'.$row->id.'">Sil</button>';
        })
        ->make(true);
}
