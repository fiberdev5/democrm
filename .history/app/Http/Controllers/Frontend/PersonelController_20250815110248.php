...
if ($request->hasFile('belgePdf')) {
    ...
    foreach ($files as $file) {
        ...
        // Yükleme yolu: storage/app/public/dealers-documents/{username}/
        $path = $file->storeAs(
            'public/dealers-documents/' . $username, // klasör
            $fileName, // dosya adı
        );
        
        // Public URL için storage linki üzerinden erişim
        $belgePdfPaths[] = str_replace('public/', 'storage/', $path);
    }
}
...
public function UpdateDealer(Request $request, $tenant_id, $id)
{
    ...
    if ($request->hasFile('belgePdf')) {
        // Eski belgeleri sil
        if ($bayi->belgePdf) {
            $eskiBelgeler = json_decode($bayi->belgePdf, true) ?: [$bayi->belgePdf];
