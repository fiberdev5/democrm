...
foreach ($stocks as $stock) {
    if ($stock->sonHareket) {
        ...
        $fiyatBirim = ($stock->fiyatBirim ?? 0) / 100;
    ...
    // Birim fiyat - kuruşu TL'ye çevir
    ->addColumn('fiyatBirim', function ($stock) {
        if ($stock->fiyatBirim) {
            $fiyatTL = $stock->fiyatBirim / 100; // Kuruşu TL'ye çevir
            return number_format($fiyatTL, 2) . ' ₺';
        }
        return '-';
    })

    // Toplam tutar - kuruşu TL'ye çevir
    ->addColumn('toplamTutar', function ($stock) {
        if ($stock->sonHareket && $stock->fiyatBirim) {
            $adet = $stock->sonHareket->adet;
            $fiyatTL = $stock->fiyatBirim / 100; // Kuruşu TL'ye çevir
            $tutar = $adet * $fiyatTL;
