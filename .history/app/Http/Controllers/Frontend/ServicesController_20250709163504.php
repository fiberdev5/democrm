...
private function processKonsinyeSelection(Request $request, $tenant_id)
{
    ...
    // Konsinye cihaz stokId'lerini ve adetlerini request'ten alıyoruz.
    // Örnek input isimleri: konsinye_cihaz[stokId] ve konsinye_adet[stokId]
    ...
    foreach ($konsinyeCihazData as $stokId => $value) {
        $adet = 1; // Default adet 1
        if (isset($konsinyeAdetData[$stokId])) {
            $adet = abs($konsinyeAdetData[$stokId]);
        }

        // konsinyeCevap dizisine stokId---adet formatında ekle
