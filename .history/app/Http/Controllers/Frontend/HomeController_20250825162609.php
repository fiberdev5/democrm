...
{
    // 1. Adım: Veritabanından son 5 servisi çekiyoruz.
    $services_query = DB::table('services as s')
        ...
        ->select(
            ...
            's.musaitTarih as estimated_date',
    ...
    // 2. Adım: Servis durum ID'lerini metne ve CSS sınıfına çeviren harita.
    // !!! BU DİZİYİ KENDİ DURUMLARINIZA GÖRE DOĞRU ŞEKİLDE DÜZENLEYİNİZ !!!
    $statusMap = [
        // Tamamlandı / Teslimat (Yeşil)
        255 => ['name' => 'Tamamlandı', 'class' => 'status-completed'],
        253 => ['name' => 'Cihaz Teslim Edildi', 'class' => 'status-completed'],
        260 => ['name' => 'Cihaz Parça İle Teslim', 'class' => 'status-completed'],
        252 => ['name' => 'Teslimata Hazır', 'class' => 'status-completed'],
        256 => ['name' => 'Cihaz Satışı Yapıldı', 'class' => 'status-completed'],
        272 => ['name' => 'Konsinye Cihaz Geri Alındı', 'class' => 'status-medium'],

        // Sorun / Problem / İptal (Kırmızı)
        244 => ['name' => 'İptal Edildi', 'class' => 'status-high'],
        246 => ['name' => 'Tamir Edilemiyor', 'class' => 'status-high'],
        241 => ['name' => 'Fiyat Anlaşılamadı', 'class' => 'status-high'],
        243 => ['name' => 'Müşteriye Ulaşılamadı', 'class' => 'status-high'],
        242 => ['name' => 'Garantili Çıktı', 'class' => 'status-high'],
        // 'sikayetci_servis' için ID belirtilmemiş, gerekirse eklenir.

        // İşlemde / Atölyede (Turuncu)
        250 => ['name' => 'Atölyede Tamir', 'class' => 'status-medium'],
        240 => ['name' => 'Atölyeye Aldır', 'class' => 'status-medium'],
        237 => ['name' => 'Cihaz Atölyede', 'class' => 'status-medium'],
        262 => ['name' => 'Nakliyeye Gönder', 'class' => 'status-medium'],
        261 => ['name' => 'Parça Hazır', 'class' => 'status-medium'],
        238 => ['name' => 'Parça Talep Edildi', 'class' => 'status-medium'],
        259 => ['name' => 'Parça Teslim Et', 'class' => 'status-medium'],
        245 => ['name' => 'Parça Atölyeye Alındı', 'class' => 'status-medium'],
        258 => ['name' => 'Tahsilata Gönder', 'class' => 'status-medium'],
        239 => ['name' => 'Yerinde Bakım', 'class' => 'status-medium'],
        271 => ['name' => 'Konsinye Cihaz Ata', 'class' => 'status-medium'],

        // Beklemede / Bilgi (Gri)
        247 => ['name' => 'Haber Verilecek', 'class' => 'status-pending'],
        249 => ['name' => 'Müşteri Atölyeye Getirdi', 'class' => 'status-pending'],
        251 => ['name' => 'Nakliye Teslim Aldı', 'class' => 'status-pending'],
        263 => ['name' => 'Parça Siparişte', 'class' => 'status-pending'],
        257 => ['name' => 'Parça Tek Yön', 'class' => 'status-pending'],
        248 => ['name' => 'Yeniden Tek Yön', 'class' => 'status-pending'],

        // Eşleşmeyen bir ID için varsayılan durum
        'default' => ['name' => 'Yeni Servis', 'class' => 'status-pending']
    ];

    // 3. Adım: Çektiğimiz verilere, yukarıdaki haritaya göre durum metnini ve CSS sınıfını ekleyelim.
...
private function getStockAlerts($tenant_id)
{
    ...
    $critical_level = 5; // Bu seviye ve altı KRİTİK
    $low_level = 15;      // Bu seviye ve altı DÜŞÜK (ama kritik değil)
    ...
    try {
        ...
        $currentStock = DB::table('stock_actions')
            ->select('stokId', DB::raw('SUM(adet) as current_stock'))
            ->where('firma_id', $tenant_id)
            ->groupBy('stokId');
