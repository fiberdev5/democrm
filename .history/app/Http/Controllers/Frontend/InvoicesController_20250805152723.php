...
class InvoicesController extends Controller
{
    public function AllInvoice(Request $request, $tenant_id) {
        ...
        if ($request->ajax()) {           
            ...
            ->addColumn('actions', function($row){
                ...
                $earsivButton = '<a href="'.$row->faturaPdf.'" target="_blank" class="btn btn-primary2 btn-sm mobilBtn mbuton1" title="Faturayı görüntüle"><i class="far fa-eye"></i></a>';
                $editButton = '';
                $deleteButton = '';
    ...
    public function DeleteInvoice($tenant_id,$id) {
        Invoice::where('firma_id', $tenant_id)->findOrFail($id)->delete();

        $eskiUrunler = InvoiceProduct::where('firma_id', $tenant_id)->where('faturaid', $id)->get();
        foreach($eskiUrunler as $urun) {
            $urun->delete();
        }
        $fatura = Invoice::where('firma_id', $tenant_id)->findOrFail($id)->first();
    ...
    public function DeleteEinvoice($id,$tenant_id){
        $invoice = Invoice::find($id);
    if (!$invoice) {
        return response()->json(false);
    }

    // PDF dosyasını da silebilirsin
    if ($invoice->faturaPdf && file_exists(public_path($invoice->faturaPdf))) {
        unlink(public_path($invoice->faturaPdf));
    }

    $invoice->faturaPdf = null;
    $invoice->save();
