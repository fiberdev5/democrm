...
public function uploadPhoto(Request $request, $tenant_id)
{
        // Validasyon
        $request->validate([
            'belge' => 'required|file|mimes:jpg,jpeg,png|max:5120',
       ...
       // Kayıt yolu
    $path        = "stock_photos/firma_{$tenant_id}/stock_{$request->stock_id}/" . now()->toDateString();
    $storedPath  = $file->storeAs($path, $uuid, 'public'); // 'public' disk kullanılmalı

    // Tam dosya yolu
    $full_save_path = storage_path("app/public/{$storedPath}");



    // Resmi yeniden boyutlandır (665px genişlik)
    ...
    // Veritabanına kayıt
    $photo = \App\Models\stock_photos::create([
        'kid'      => auth()->id(), // yükleyen kişi
