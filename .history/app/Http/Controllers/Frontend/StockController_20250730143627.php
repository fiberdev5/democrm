...
public function AllStocks($tenant_id, Request $request)
{
    ...
    //Stok istatistikleri 
    $depo_personel_id = $request->depo_personel_id;
    ...
    $query = Stock::select('stocks.*')
        ...
        if ($request->filled('personel')) {
                    $personelId = $request->personel;
                    $query->whereExists(function ($q) use ($personelId) {
                        $q->select(\DB::raw(1))
                            ->from('personel_stoklar')
                            ->whereRaw('personel_stoklar.stok_id = stocks.id')
                            ->where('personel_stoklar.pid', $personelId);
                    });
                }
    ...
    // Personel sadece teknisyen-teknisyen yardımcısı
