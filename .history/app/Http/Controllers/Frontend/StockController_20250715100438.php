...
public function DeleteStockAction(Request $request, $tenant_id, $id)
{
    ...
    // Bu ID'ye sahip başka hareketler varsa, silme!
    $stokHareketSayisi = StockAction::where('stokId', $id)->count();

    if ($stokHareketSayisi > 1) { // en az bir kayıt daha varsa
        return response()->json([
            'status' => 'warning',
            'message' => 'Bu harekete bağlı başka kayıtlar var, silemezsiniz.'
        ]);
    }

    // Kısıtlama: servis işlemi ise engelle
    if (in_array($stockAction->islem, [2, 4])) {
        return response()->json([
            'status' => 'warning',
            'message' => 'Serviste kullanılmış bir parçayı silemezsiniz.'
        ]);
    }

    // Alış ise, sonrası kontrolü
    if ($stockAction->islem == 1) {
        ...
        if ($cikisVarMi) {
            return response()->json([
                'status' => 'warning',
                'message' => 'Bu alıştan sonra çıkış yapıldığı için silinemez.'
            ]);
        }
    }
