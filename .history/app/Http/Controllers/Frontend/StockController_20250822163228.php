...
public function searchBrands(Request $request, $tenant_id)
    {
        $search = $request->input('q');
        $data = DeviceBrand::where('firma_id', $tenant_id) 
...
{
        $search = $request->input('q');
        $data = DeviceType::where('firma_id', $tenant_id)
                                       ->where('cihaz', 'LIKE', "%{$search}%")
                                       ->latest()
                                       ->take(5)
...
{
        $search = $request->input('q');
        $data = StockCategory::where('firma_id', $tenant_id)
...
public function uploadPhoto(Request $request, $tenant_id)
{
    ...
    // // Bu stok için mevcut fotoğraf sayısını kontrol et
    // $existingPhotoCount = stock_photos::where('stock_id', $request->stock_id)
    //                                   ->where('kid', $tenant_id)
    //                                   ->count();

    // // Eğer zaten bir fotoğraf varsa, hata döndür
    // if ($existingPhotoCount > 0) {
    //     return response()->json([
    //         'message' => 'Bu ürün için zaten bir fotoğraf mevcut. Yeni fotoğraf eklemek için eskisini silmelisiniz.'
    //     ], 409); // 409 Conflict status kodu bu durum için uygundur.
    // }
...
public function StoreConsignmentStockAction(Request $request, $tenant_id)
{
    try {
        ...
        $stokId = $request->stok_id;
        $personel_id = Auth::user()->user_id; // İşlemi yapan (oturum açmış) personelin ID'si
        ...
        // === YENİ VE GÜNCELLENMİŞ MANTIK BLOGU ===
        // Her işlem türü için ilgili alanları ayrı ayrı dolduruyoruz.
        // Bu, kodun daha okunaklı olmasını sağlar ve hataları önler.

        if ($request->islem == 1) { // Alış
            $stockAction->pid = $personel_id; // Alışı kaydeden personel
            $stockAction->tedarikci = $request->tedarikci;
            if ($request->filled('fiyat')) {
                $fiyat = floatval(str_replace(['.', ','], ['', '.'], $request->fiyat));
                $stockAction->fiyat = $fiyat;
                $stockAction->fiyatBirim = $request->fiyatBirim;
            }
        }
        elseif ($request->islem == 2) { // Serviste Kullanım
            $stockAction->pid = $personel_id; // Parçayı kullanan teknisyen (işlemi yapan kişi)
            $stockAction->servisid = $request->servisid;
        }
        elseif ($request->islem == 4) { // Müşteriden Geri Alma
            $stockAction->pid = $personel_id; // Parçayı geri alan personel
            // İsteğe bağlı olarak, geri almanın hangi servisle ilgili olduğunu da kaydedebilirsiniz.
            // $stockAction->servisid = $request->servisid;
        }
        // === GÜNCELLEME BİTTİ ===
        ...
        // Alış işlemi ise ana stok kartındaki toplam fiyatı güncelle
        if ($request->islem == 1 && isset($fiyat)) {
            $stock = Stock::find($stokId);
            if ($stock) {
                // Not: Burada fiyatı eklemek yerine, toplam değeri başka bir yerde hesaplamak daha doğru olabilir.
                // Ancak mevcut yapınıza uygun olarak devam ediyoruz.
                $stock->fiyat += $fiyat;
                $stock->save();
            }
        }
        ...
        return response()->json([
            'status' => 'error',
            'message' => 'Stok hareketi kaydedilirken bir sunucu hatası oluştu: ' . $e->getMessage()
        ], 500);
    }
}
