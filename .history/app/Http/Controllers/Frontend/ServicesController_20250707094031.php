    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        foreach ($consignmentDevices as $device) {
            ...
            $approvedConsignmentAnswer = $planCevaplar->first(function($cevap) use ($device) {
                // Cevap türü '[Konsinye Cihaz]' ise ve cevapta cihaz ID'si veya adı varsa
                // Örneğin: $cevap->soru->cevapTuru == '[Consign]' ve $cevap->cevap bir JSON nesnesi ise
                return $cevap->soru->cevapTuru == '[Konsinye Cihaz]' && str_contains($cevap->cevap, 'id":' . $device->id);
            });
