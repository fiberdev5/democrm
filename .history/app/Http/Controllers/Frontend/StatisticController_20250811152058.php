...
{
    $kasaHareketleri = CashTransaction::where('created_at', '>=', date("Y-m").'-01 00:00:00')
    ...
    $odemeSekliAll = "";
    $odemeSekilleri = PaymentMethod::orderBy('id','asc')->get();
    foreach ($odemeSekilleri as $odemeSekli){
        if(empty($odemeSekliAll)){
          $odemeSekliAll = '"'.$odemeSekli->odeme_sekli.'"';
        }else{
          $odemeSekliAll .= ',"'.$odemeSekli->odeme_sekli.'"';
        }
    }
    ...
    foreach($odemeTurleri as $odemeTuru){
        $gelir = CashTransaction::where('created_at	', '>=', date("Y-m").'-01 00:00:00')
                                ->where('created_at	', '<=', date("Y-m-d").' 23:59:59')
                                ->where('odemeYonu', '2')
                                ->where('odemeTuru', $odemeTuru->id)
                                ->sum('fiyat');
        $odemeTuruAll[$odemeTuru->tur] = round($gelir);
    }

    $giderlerToplam = array_sum($odemeTuruAll);

    $odemeTuruSonuc = json_encode(array_keys($odemeTuruAll));
    $odemeTuruRenkler = json_encode(array_values($odemeTurleri->pluck('renk')->toArray()));
 ...
 public function GetKasaStatistics(){
        $kasaHareketleri = CashTransaction::where('islemTarihi', '>=', date("Y-m").'-01 00:00:00')
            ->where('islemTarihi', '<=', date("Y-m-d").' 23:59:59')
            ->where('odemeYonu', '1')
            ->get();
        ...
        $getTable = CashTransaction::where('islemTarihi', '>=', date("Y-m").'-01 00:00:00')
                                    ->where('islemTarihi', '<=', date("Y-m-d").' 23:59:59')
                                    ->where('odemeYonu', '2')->get();
        $odemeTuruAll = array();
        $odemeTurleri = PaymentType::orderBy('tur','asc')->get();
        
        foreach($getTable as $row){
            foreach ($odemeTurleri as $odemet){
                if($row->odemeTuru == $odemet->id){
                  $odemeTuruAll[$odemet->tur] = round($row->fiyat);
                }
              }
        }
        foreach ($odemeTurleri as $odemeTuru){
            if (!array_key_exists($odemeTuru["tur"],$odemeTuruAll)){
              $odemeTuruAll[$odemeTuru["tur"]] = 0;
            }
        }
        ...
        foreach ($odemeTuruAll as $key => $value) {
          $giderler .= round($value).",";
          $giderlerToplam += round($value);
        }
 
        $odemeTurleri = PaymentType::orderBy('tur','asc')->get();
        $odemeTuruAll = [];
        foreach($odemeTurleri as $odemeTuru){
            $gelir = CashTransaction::where('islemTarihi', '>=', date("Y-m").'-01 00:00:00')
                                    ->where('islemTarihi', '<=', date("Y-m-d").' 23:59:59')
                                    ->where('odemeYonu', '2')
                                    ->where('odemeTuru', $odemeTuru->id)
                                    ->sum('fiyat');
        ...
        return view('frontend.secure.statistics.cash_statistics',compact('nakit','eft','kart','gelirler','odemeSekliAll','odemeTuruAll','giderlerToplam','odemeTuruSonuc','odemeTuruRenkler','giderler'));
    }

    public function kasaFilteredData(Request $request){
      $startDate = $request->input('startDate');
      $endDate = $request->input('endDate');

        // Veritabanından belirli bir tarih aralığına göre verileri al
        $kasaHareketleri = CashTransaction::whereBetween('islemTarihi', [$startDate, $endDate])
                                            ->where('odemeYonu', '1')
                                            ->get();
    ...
    public function giderTabloGetir(Request $request){
        $startDate = $request->input('startDate');
        $endDate = $request->input('endDate');

        $giderHareketleri = CashTransaction::whereBetween('islemTarihi', [$startDate, $endDate])
            ->where('odemeYonu', '2')
            ->get();
        ...
        foreach($giderHareketleri as $row){
            foreach ($odemeTurleri as $odemet){
                if($row->odemeTuru == $odemet->id){
                  $odemeTuruAll[$odemet->tur] = round($row->fiyat);
                }
        ...
        $odemeTuruAll = [];
        foreach($odemeTurleri as $odemeTuru){
            $gelir = CashTransaction::where('islemTarihi', '>=', $startDate)
                    ->where('islemTarihi', '<=', $endDate)
                    ->where('odemeYonu', '2')
                    ->where('odemeTuru', $odemeTuru->id)
                    ->sum('fiyat');
            $odemeTuruAll[$odemeTuru->tur] = round($gelir);
        ...
        echo "<ul>";
        foreach ($odemeTuruAll as $key => $value) {
            $odemeTurSec = PaymentType::where('tur', $key)->first();
            echo '<li class="gider"><div class="renk" style="background:'.$odemeTurSec["renk"].'"></div><div class="adi">'.$key.'</div><div class="para">'.number_format($value, 0, ',', '.').'TL</div></li>';
        }
        echo '<li class="gider"><div class="renk" style="background:#000"></div><div class="adi">Toplam</div><div class="para">'.number_format($giderlerToplam, 0, ',', '.').'TL</div></li>';
        echo "</ul>";
    }

    public function gelirGrafikGetir(Request $request){
        $startDate = $request->input('startDate');
        $endDate = $request->input('endDate');
    ...
    public function giderGrafikGetir(Request $request){
        $startDate = $request->input('startDate');
        $endDate = $request->input('endDate');

        $getTable = CashTransaction::whereBetween('islemTarihi', [$startDate, $endDate])
                                    ->where('odemeYonu', '2')->get();
        $odemeTuruAll = array();
        $odemeTurleri = PaymentType::orderBy('tur','asc')->get();
        ...
        $odemeTuruAll = [];
        foreach($odemeTurleri as $odemeTuru){
            $gelir = CashTransaction::whereBetween('islemTarihi', [$startDate, $endDate])
                                    ->where('odemeYonu', '2')
                                    ->where('odemeTuru', $odemeTuru->id)
                                    ->sum('fiyat');
            $odemeTuruAll[$odemeTuru->tur] = round($gelir);
