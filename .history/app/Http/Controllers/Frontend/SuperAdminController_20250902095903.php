...
class SuperAdminController extends Controller
{
    ...
    {
        // Super Admin rolü olanları hariç tut
        $superAdminRoleId = DB::table('roles')->where('name', 'Super Admin')->first()->id ?? null;
        
        $stats = [
            'total_tenants' => Tenant::count(),
            'active_tenants' => Tenant::where('status', 1)->count(),
            'total_users' => User::when($superAdminRoleId, function($query) use ($superAdminRoleId) {
                return $query->whereNotExists(function($subQuery) use ($superAdminRoleId) {
                    $subQuery->select(DB::raw(1))
                             ->from('model_has_roles')
                             ->whereColumn('model_has_roles.model_id', 'tb_user.user_id')
                             ->where('model_has_roles.role_id', $superAdminRoleId);
                });
            })->count(),
    ...
    public function allTenants(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            // Filtreleme işlemleri - musteriTipi yerine tenant_type veya başka bir alan kullanın
            if ($request->filled('tip')) {
                // Eğer tenants tablosunda tenant_type kolonu varsa:
                if ($request->get('tip') == 1) {
                    $data->where('tenant_type', 1); // veya uygun kolon adı
                } elseif ($request->get('tip') == 2) {
                    $data->where('tenant_type', 2); // veya uygun kolon adı
                }
            }
