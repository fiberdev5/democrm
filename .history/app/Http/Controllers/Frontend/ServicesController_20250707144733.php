        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($soru->cevapTuru == '[Parca]') {
                    // Parça seçimi
                    // updateParcaSelection ve getUpdatedParcaAnswer metodlarınızın doğru çalıştığını varsayıyorum.
                    // Eğer parçalar FormData içinde 'parca[soruId][personel_stock]' olarak geliyorsa,
                    // direkt olarak request'ten almanız gerekebilir.
                    // Örneğin: $request->input('parca')[$cevap->soruid]['personel_stock']
                    // Mevcut kodunuzda updateParcaSelection'ın ne döndürdüğünü bilmediğim için,
                    // bu kısmın düzeltilmesi gerekebilir.
                    // Şimdilik sadece bu kısmı koruyorum:
                    $this->updateParcaSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
                    $cevap->cevap = $this->getUpdatedParcaAnswer($request); // Bu metodu kontrol edin!

                } elseif ($soru->cevapTuru == '[Konsinye Cihaz]') {
                    // Konsinye cihazın ID'si ve adedi FormData'da ayrı ayrı geliyor.
                    // Bizim birleşik bir string (C_ID---Adet) kaydetmemiz gerekiyor.
                    $consignmentDeviceId = $request->input('parca')[$cevap->id]['consignment_id'] ?? null;
                    $consignmentQuantity = $request->input('adet')[$cevap->id]['consignment_quantity'] ?? null;
                    
                    if ($consignmentDeviceId && $consignmentQuantity !== null) {
                        $cevap->cevap = "C_{$consignmentDeviceId}---{$consignmentQuantity}";
                    } else {
                        $cevap->cevap = null; // Cihaz seçilmemişse veya adet girilmemişse boş bırak
                    }
                } else {
                    // Diğer cevap türleri
