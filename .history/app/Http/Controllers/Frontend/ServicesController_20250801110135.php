...
class ServicesController extends Controller
{
 ...
 // Personel filtresi - surveys tablosunda bu personelin anket yaptığı servisleri getir
if ($request->filled('personel_id')) {
    $personelId = $request->personel_id;
    
    // Bu personelin anket yaptığı servis ID'lerini al
    $anketYapilanServisler = Survey::where('personel', $personelId)
        ->where('firma_id', $firma->id)
        ->pluck('servisid')
        ->toArray();
    
    // Eğer anket yapılmış servisler varsa, sadece bunları göster
    if (!empty($anketYapilanServisler)) {
        $data->whereIn('id', $anketYapilanServisler);
    } else {
        // Eğer bu personelin hiç anketi yoksa, boş sonuç döndür
        $data->where('id', 0); // Hiçbir servis gösterme
    }
}
                
           // Personel istatistikleri tarih aralığı filtresi
if ($request->filled('personel_istatistik_tarih1') && $request->filled('personel_istatistik_tarih2')) {
    $from = Carbon::createFromFormat('Y-m-d', $request->personel_istatistik_tarih1)->startOfDay();
    $to = Carbon::createFromFormat('Y-m-d', $request->personel_istatistik_tarih2)->endOfDay();
    
    // Eğer personel filtresi varsa, survey tablosundaki tarihleri kontrol et
    if ($request->filled('personel_id')) {
        $personelId = $request->personel_id;
        
        // Bu tarih aralığında bu personelin anket yaptığı servis ID'lerini al
        $anketYapilanServisler = Survey::where('personel', $personelId)
            ->where('firma_id', $firma->id)
        ...
        // Personel filtresi yoksa, normal tarih filtresi uygula
        $data->whereBetween('created_at', [$from, $to]);
    }
}
