...
public function AllStocks($tenant_id, Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        if ($request->filled('raf')) {
            $query->where('raf', $request->raf);
        }
        if ($request->filled('cihaz')) {
            $query->where('cihaz', $request->cihaz);
        }
        if ($request->filled('personel')) {
            // Burada personel filtresi stokta doğrudan var ise
            $query->where('personel_id', $request->personel);
        }
        ...
        return DataTables::of($query)
            ...
            ->addColumn('urunKodu', function($row) {
                return '<a href="javascript:void(0);" class="editStock" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">' . e($row->urunKodu) . '</a>';
            })
            ->addColumn('urunAdi', function($row) {
                return '<a href="javascript:void(0);" class="editStock" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">' . e($row->urunAdi) . '</a>';
            })
            ...
            ->addColumn('raf_adi', function($row) {
                return $row->raf ? e($row->raf->raf_adi) : '-';
            })
