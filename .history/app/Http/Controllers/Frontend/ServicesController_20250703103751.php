  ...
  public function SaveServicePlan(Request $request, $tenant_id)
{
    ...
    try {
        ...
        // Gelen işlem JSON verisini çöz
        $gelenIslem = json_decode($gelenIslemInput, true);
        if (!$gelenIslem || !isset($gelenIslem['id'])) {
            return response()->json(['status' => 'error', 'message' => 'Hata: Geçersiz gelen işlem verisi.']);
        }
        $gelenIslemId = $gelenIslem['id'];

        // Servis kontrolü
        $servis = Service::where('firma_id', $firma->id)->where('id', $servisId)->first();
        if (!$servis) {
            return response()->json(['status' => 'error', 'message' => 'Hata: Servis bulunamadı.']);
        }
        ...
        $planId = ServicePlanning::insertGetId([
            'firma_id' => $tenant_id,
            'kid' => $kid,
            'pid' => $kid,
            'servisid' => $servisId,
        ...
        // Özel durumlar
        $this->ozelDurumlariIsle($request, $servisId, $planId, $tenant_id, $gidenIslem, $servis);

        // Tarih durumu
        $this->tarihDurumuKontrolEt($tenant_id);

        // Aşama ve alt aşamalar
        $guncellenmisAsamaBilgisi = $servis->asamalar->asama ?? 'Bilinmiyor';
        $altAsamalar = collect();
