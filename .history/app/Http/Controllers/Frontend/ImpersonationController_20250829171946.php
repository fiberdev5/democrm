...
class ImpersonationController extends Controller
{
     ...
     public function stop(Request $request)
    {
        ...
        $redirectRoute = ''; // Tanımlandı
        $notificationMessage = ''; // Tanımlandı
        ...
        if ($originalUser) {
            ...
            // Yönlendirme ve bildirim mesajını belirle
            if ($originalUser->isSuperAdmin()) {
                $notificationMessage = 'Super Admin hesabınıza geri döndünüz.';
                $redirectRoute = route('super.admin.dashboard');
            } else {
                $notificationMessage = 'Kendi hesabınıza geri döndünüz.';
                $redirectRoute = route('secure.home', ['tenant_id' => $originalUser->tenant_id]);
            }
        } else {
            // Orijinal kullanıcı bulunamazsa (nadiren olmalı)
            $notificationMessage = 'Orijinal kullanıcı bulunamadı, giriş sayfasına yönlendiriliyorsunuz.';
            $redirectRoute = route('login'); // Veya başka bir varsayılan rota
        }

        // --- Düzeltilmiş dd() bloğu ---
        // dd() çağrıldığında $redirectRoute artık tanımlı olacak
        dd([
            'message' => 'Impersonation STOP metodu sonu',
            'impersonator_id' => $impersonatorId,
            'original_user_is_super_admin' => $originalUser ? $originalUser->isSuperAdmin() : 'N/A',
            // Rota ismini almak için Request::create($redirectRoute) kullanırken dikkatli olun.
            // Eğer $redirectRoute geçerli bir URL değilse (örn. null ise) hata verebilir.
            // Güvenli olması için:
            'redirect_route_name' => $redirectRoute ? app('router')->getRoutes()->match(Request::create($redirectRoute))->getName() : 'N/A',
            'redirect_route_url' => $redirectRoute,
            'current_auth_user_id' => Auth::id(),
            'current_auth_user_is_super_admin' => Auth::user() ? Auth::user()->isSuperAdmin() : 'N/A',
            'session_has_impersonator_id_before_forget' => session()->has('impersonator_id'), // Session temizlenmeden önceki durumu gösterelim
        ]);
        // --- dd() bloğu sonu ---
