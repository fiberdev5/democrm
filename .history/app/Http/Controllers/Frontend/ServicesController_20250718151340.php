                ...
                if ($tarihDurum == "1") {
                    ...
                    if ($tarihCevap) {
                        ...
                        if ($bugun == $tarih) {
                            ...
                            if ($zaman) {
                                $ilksaat = date("H.i"); // Şu anki saat
                                $sonsaat = $zaman->zaman; // Servis zamanı
    ...
    public function UpdateService($tenant_id, Request $request) 
    {
        ...
        if ($user->can('Tüm Servisleri Görebilir')) {
            $rules = array_merge($rules, [
                ...
                'cihazMarka' => 'nullable|integer|exists:device_brands,id',
                'cihazTur' => 'nullable|integer|exists:device_types,id',
        ...
        try {
            ...
            if ($user->can('Tüm Servisleri Görebilir')) {
                $data = array_merge($data, [
                    'kid' => $user->user_id,
                    'servisKaynak' => strip_tags(trim($request->servisKaynak ?? '')),
                    'musaitTarih' => $request->musaitTarih,
                    'musaitSaat1' => strip_tags(trim($request->musaitSaat1 ?? '')),
                    'musaitSaat2' => strip_tags(trim($request->musaitSaat2 ?? '')),
                    'cihazMarka' => $request->cihazMarka,
                    'cihazTur' => $request->cihazTur,
                    'cihazSeriNo' => strip_tags(trim($request->cihazSeriNo ?? '')),
                    'cihazAriza' => strip_tags(trim($request->cihazAriza ?? '')),
                    'operatorNotu' => strip_tags(trim($request->opNot ?? '')),
                    'garantiSuresi' => strip_tags(trim($request->cihazGaranti ?? '')),
                    'faturaNumarasi' => strip_tags(trim($request->faturaNumarasi ?? '')),
                    'konsinye' => strip_tags(trim($request->konsinye ?? '')),
                ]);
            }
