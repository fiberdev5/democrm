...
class StockController extends Controller
{
     ...
     public function AllStocks($tenant_id, Request $request){
                   ...
                   return DataTables::of($query)  // $query = stokların sorgusu

    ->addIndexColumn()

->addColumn('adet', function ($stock) {
    $adet = ($stock->sonHareket && isset($stock->sonHareket->adet)) ? $stock->sonHareket->adet : '-';
    return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$stock->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$adet.'</a>';
})

    ->addColumn('fiyatBirim', function ($stock) {
        $fiyat = $stock->fiyatBirim ? number_format($stock->fiyatBirim, 2) . ' ₺' : '-';
        return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$stock->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$fiyat.'</a>';
    })

    ->addColumn('toplamTutar', function ($stock) {
        if ($stock->sonHareket && $stock->fiyatBirim) {
            $tutar = $stock->sonHareket->adet * $stock->fiyatBirim;
            $tutarFormatted = number_format($tutar, 2) . ' ₺';
        } else {
            $tutarFormatted = '-';
        }
        return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$stock->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$tutarFormatted.'</a>';
    })

    ->addColumn('raf_adi', function ($stock) {
        $rafAdi = $stock->raf ? $stock->raf->raf_adi : '-';
        return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$stock->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$rafAdi.'</a>';
    })
    ...
    ->addColumn('marka_cihaz', function ($stock) {
        ...
        return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$stock->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">'.$text.'</a>';
    })

    ->editColumn('created_at', function($stock) {
        return '<a href="javascript:void(0);" class="t-link editStock" data-bs-id="'.$stock->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal">' . $stock->created_at->format('d.m.Y H:i') . '</a>';
    })
    ...
    ->filter(function ($instance) use ($request) {
        if ($search = $request->get('search')) {
            $instance->where(function($w) use ($search) {
                $w->whereHas('raf', function($q) use ($search) {
    ...
    ->rawColumns(['adet', 'fiyatBirim', 'toplamTutar', 'raf_adi', 'marka_cihaz', 'created_at', 'action'])
