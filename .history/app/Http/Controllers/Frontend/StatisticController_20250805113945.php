...
public function TechnicianStatistics(Request $request, $tenant_id)
{
    // Tarih aralığı kontrolü
    $startDate = $request->tarih1 ? Carbon::createFromFormat('d/m/Y', $request->tarih1)->format('Y-m-d') : Carbon::now()->format('Y-m-d');
    $endDate = $request->tarih2 ? Carbon::createFromFormat('d/m/Y', $request->tarih2)->format('Y-m-d') : Carbon::now()->format('Y-m-d');
    ...
    // Detay (örnek olarak ilk teknisyenin detayını getiriyoruz; istersen AJAX’a da ayırabiliriz)
    $detailData = null;
    if (count($results)) {
        $firstTechnicianId = $results[0]['id'];
        $detailData = $this->getTechnicianDetailData($firstTechnicianId, $startDate, $endDate);
    }
