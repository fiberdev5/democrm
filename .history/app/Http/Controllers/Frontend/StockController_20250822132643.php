...
public function EditConsignmentDevice($tenant_id, $id)
{
    ...
    // 1. İlişkili tüm verileri tek seferde çek (Eager Loading)
    // Performans için ->with() ile gerekli tüm ilişkileri önceden yüklüyoruz.
    $stock = Stock::with(['raf', 'marka', 'cihaz', 'kategori'])->findOrFail($id);

    // 2. Kategori Kontrolü (Bu önemli bir güvenlik adımı)
    // Varsayılan olarak '3' ID'sinin 'Konsinye' kategorisi olduğunu kabul ediyoruz.
    if ($stock->urunKategori != 3) {
        // AJAX isteği olduğu için 404 yerine JSON hatası dönmek daha iyi olabilir.
        // return response()->json(['error' => 'Bu ürün bir konsinye cihazı değil.'], 404);
        abort(404, "Bu ürün bir konsinye cihazı olarak kayıtlı değil.");
    }

    // 3. Konsinyeye Özel Stok Hareketlerini Çek
    $stokHareketleri = StockAction::with(['servis.musteri'])
        ->select(
            'stock_actions.*',
            'ss.tedarikci as tedarikci_adi',      // Tedarikçi adını almak için join
            'up.name as performer_name'           // Teknisyen/Kullanıcı adını almak için join
        )
        ->leftJoin('stock_suppliers as ss', 'ss.id', '=', 'stock_actions.tedarikci')
        ->leftJoin('tb_user as up', 'up.user_id', '=', 'stock_actions.pid')
        ->where('stock_actions.stokId', $id)
        ->whereIn('stock_actions.islem', [1, 2, 4]) // 1: Alış, 2: Serviste Kullanım, 4: Müşteriden Geri Alma
        ->orderBy('stock_actions.id', 'desc')
        ->get();

    // 4. Ürüne Ait Fotoğrafları Çek
    $photos = stock_photos::where('kid', $tenant_id)
                          ->where('stock_id', $id)
                          ->latest()
                          ->get();
    ...
    // Not: Eski kodunuzdaki `$consignmentDevice` yerine doğrudan `$stock` kullanmak daha tutarlı.
