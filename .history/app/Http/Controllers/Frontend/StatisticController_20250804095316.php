...
private function calculateTechnicianStats($userId, $dateRange, $deviceType, $tenantId)
{
    ...
    // Teknisyenin grup ID'sini al
    $technician = User::find($userId);
    if (!$technician || !$technician->grup_id) {
        return $this->getEmptyStats();
    }

    $grupId = $technician->grup_id;
    $groupTag = '[Grup-' . $grupId . ']';
    ...
    $serviceIds = ServiceStageAnswer::join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ...
        ->where('service_stage_answers.cevap', 'like', '%' . $groupTag . '%')
