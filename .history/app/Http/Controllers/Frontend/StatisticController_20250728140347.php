...
private function getServicePlansForDates($tarihListesi, $tenant_id, $cihazTur = null)
{
    $query = DB::table('service_stage_answers as sc')
        ...
        ->where('ss.cevapTuru', 'LIKE', '%Tarih%') // Tarih tipindeki cevapları filtrele
        ->where('services.firma_id', $tenant_id);

    // service_stage_answers.cevap sütunundaki tarihlerin YYYY-MM-DD formatında olduğunu varsayıyoruz.
    // getDatesFromRange fonksiyonundan gelen $tarihListesi de YYYY-MM-DD formatında olduğu için
    // direkt kullanabiliriz.
    $query->whereIn('sc.cevap', $tarihListesi);

    if ($cihazTur) {
        $query->where('services.cihazTur', $cihazTur);
    }

    // Bu soru ID'lerinin gerçekten servis tarihi soruları olduğunu doğrulayın.
    $seciliTarihler = $query->whereIn('sc.soruid', [296, 326, 369, 306, 337, 341])
                            ->pluck('sc.planid')
                            ->toArray();

    \Log::info('Servis plan sorgusu sonucu:', [
        'kullanilan_tarih_listesi_format' => 'YYYY-MM-DD', // Doğrulama için log
        'tarih_listesi_query' => $tarihListesi,
