    ...
    {   
        ...
        if ($request->ajax()) {
            ...
            if ($request->filled('bayi') && $request->get('bayi') != '') {
                // Belirli bayi seçilmişse sadece onu filtrele
                $data->where('personel', $request->get('bayi'));
            } else {
                // Bayi seçilmemişse: sadece bayi rolündeki kullanıcıları getir
                $bayiIds = User::role('Bayi')->where('tenant_id', $tenant_id)->pluck('user_id')->toArray();
