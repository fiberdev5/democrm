...
class SuperAdminController extends Controller
{
    ...
    public function allTenants(Request $request)
    {
        ...
        if ($request->ajax()) {
            // Eager load 'ils' and 'ilces' relationships to access their names for the address column
            $data = Tenant::with(['ils', 'ilces']);

            // Search filtering (if any)
            ...
            // District (Ilce) filtering
            if ($request->get('ilce')) {
                $data->where('ilce', $request->get('ilce'));
            }

            // Order by (using DataTables default order logic, but ensure column names match)
            ...
            return DataTables::of($data)
                ...
                ->addColumn('address', function($row) {
                    // Concatenate adres, il, and ilce for a complete address string
                    $addressParts = [];
                    if (!empty($row->adres)) {
                        $addressParts[] = $row->adres;
                    }
                    if ($row->ils) { // Check if ils relation exists
                        $addressParts[] = $row->ils->name;
                    }
                    if ($row->ilces) { // Check if ilces relation exists
                        $addressParts[] = $row->ilces->ilceName;
                    }
                    $fullAddress = implode(' / ', array_filter($addressParts)); // Filter out empty parts and join

                    return '<a class="t-link editTenant address" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editTenantModal"><div class="mobileTitle">Adres:</div>' . $fullAddress . '</a>';
                })
    ...
    {
        // Eager load ils and ilces for the edit view if they are used there
        $tenant = Tenant::with(['ils', 'ilces'])->findOrFail($id);
        if(!$tenant) {
            $notification = array(
        ...
        foreach ($periods as $key => $period) {
            ...
            ->whereIn('user_id', function ($query) {
                $query->select('model_id')
                    ...
                    ->whereIn('role_id', [1, 5, 263]); // Assuming these are relevant role IDs
            })
            ->get();
    ...
    public function deleteTenant($id)
    {
        ...
        // Instead of deleting, we set status to 0 (passive)
        $tenant->status = 0;
        $tenant->save();
...
public function changeTenantStatus($id)
    {
        ...
        Mail::to($tenant->eposta)->queue(new UserRegisteredMail($customUsername));
        return redirect()->back()->with($notification);
    }
