...
private function getOtherFilesBreakdown($tenant)
{
    try {
        ...
        $ticketAttachments = $this->getTicketAttachmentsBreakdown($tenant);
        $breakdown['ticket_attachments'] = $ticketAttachments;
        
        $totalSize += $ticketAttachments['size'];
        $totalCount += $ticketAttachments['count'];
...
private function safeCalculateDirectorySize($directory)
{
    if (!is_dir($directory)) {
        return 0;
    }
...
private function getTicketAttachmentsBreakdown($tenant)
{
    try {
        $firmSlug = $tenant->firma_slug ?? $tenant->id;
        $totalSize = 0;
        ...
        // support_ticket_replies tablosundan attachments'ları al
        ...
        foreach ($ticketReplies as $reply) {
            if (!empty($reply->attachments)) {
                ...
                if (is_array($attachments)) {
                    ...
                    $replyAttachmentSize = 0;
                    
                    foreach ($attachments as $attachment) {
                        if (isset($attachment['path']) && str_contains($attachment['path'], "firma_{$firmSlug}")) {
                            ...
                            if (file_exists($filePath)) {
                                ...
                                $replyAttachmentSize += $fileSize;
                            }
                        }
                    }
                    
                    // Ticket bazlı gruplandırma için
                    if ($replyAttachmentCount > 0) {
                        // Ticket numarasını path'den çıkar
                        $ticketNumber = $this->extractTicketNumber($attachments[0]['path'] ?? '');
                        
                        if ($ticketNumber && !isset($ticketData[$ticketNumber])) {
                            $ticketData[$ticketNumber] = [
                                'ticket_number' => $ticketNumber,
                                'file_count' => $replyAttachmentCount,
                                'total_size' => $replyAttachmentSize,
                                'total_size_formatted' => $this->formatBytes($replyAttachmentSize),
                                'last_updated' => $reply->created_at,
                                'last_updated_formatted' => date('d.m.Y H:i', strtotime($reply->created_at))
                            ];
                            $ticketCount++;
                        }
                    }
        ...
        // Son 5 ticket'ı al
        $recentTickets = array_slice(array_values($ticketData), 0, 5);
        ...
        \Log::error('Ticket attachments breakdown error', ['error' => $e->getMessage()]);
        return [
            'count' => 0,
            'size' => 0,
            'size_formatted' => '0 B',
...
private function extractTicketNumber($path)
{
    // support_attachments/firma_test-teknik/ticket_TKT-2025-2056/replies/2025-09-15/...
    if (preg_match('/ticket_([^\/]+)/', $path, $matches)) {
        return $matches[1];
    }
    return null;
}
...
private function getRecentUploads($tenant, $limit = 10)
{
    try {
        ...
        $recentServicePhotos = ServicePhoto::where('firma_id', $tenant->id)
                                         ...
                                         ->map(function($photo) {
                                             return [
                                                 ...
                                                 'size_formatted' => $this->formatBytes($photo->file_size ?? 0),
                                                 'uploaded_at' => $photo->created_at,
                                                 'service_id' => $photo->servisid ?? null
                                             ];
                                         });
        
        // Ticket attachments
        $firmSlug = $tenant->firma_slug ?? $tenant->id;
        $recentTicketAttachments = DB::table('support_ticket_replies')
                                    ->whereJsonLength('attachments', '>', 0)
                                    ->orderByDesc('created_at')
                                    ->take($limit)
                                    ->get()
                                    ->flatMap(function($reply) use ($firmSlug) {
                                        $attachments = json_decode($reply->attachments, true);
                                        $replyAttachments = [];
                                            ...
                                            foreach ($attachments as $attachment) {
                                                if (isset($attachment['path']) && str_contains($attachment['path'], "firma_{$firmSlug}")) {
                                                    $replyAttachments[] = [
                                                        ...
                                                        'name' => $attachment['original_name'] ?? 'Ticket Eki',
                                                        'size' => $attachment['size'] ?? 0,
                                                        'size_formatted' => $this->formatBytes($attachment['size'] ?? 0),
                                                        'uploaded_at' => \Carbon\Carbon::parse($reply->created_at),
                                                        'ticket_number' => $this->extractTicketNumber($attachment['path'] ?? '')
                                                    ];
                                                }
                                            }
                                        }
        ...
        // Tüm dosyaları birleştir ve sırala
        $allFiles = $recentServicePhotos->concat($recentTicketAttachments);
        
        return $allFiles->sortByDesc('uploaded_at')->take($limit)->values()->toArray();
        
    } catch (\Exception $e) {
        \Log::error('Recent uploads error', ['error' => $e->getMessage()]);
        return [];
    }
}
...
private function getLargestFiles($tenant, $limit = 5)
{
    try {
        ...
        // Ticket attachments - JSON'dan en büyükleri bul
        $firmSlug = $tenant->firma_slug ?? $tenant->id;
        $ticketAttachments = [];
                        ...
                        if (isset($attachment['path']) && str_contains($attachment['path'], "firma_{$firmSlug}")) {
                            ...
                            if (file_exists($filePath)) {
                                ...
                                $ticketAttachments[] = [
                                    ...
                                    'uploaded_at' => date('d.m.Y H:i', strtotime($reply->created_at)),
                                    'ticket_number' => $this->extractTicketNumber($attachment['path'])
                                ];
                            }
                        }
        ...
        // Ticket attachments'ları boyuta göre sırala
        usort($ticketAttachments, function($a, $b) {
            return $b['size'] - $a['size'];
        });
        
        // Tüm dosyaları birleştir ve en büyük 5'ini al
        $allFiles = $largestServicePhotos->concat(array_slice($ticketAttachments, 0, $limit));
        
        return $allFiles->sortByDesc('size')->take($limit)->values()->toArray();
        
    } catch (\Exception $e) {
        \Log::error('Largest files error', ['error' => $e->getMessage()]);
        return [];
    }
}
