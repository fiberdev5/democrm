...
class CashTransactionsController extends Controller
{
    ...
    public function AddCashTransaction($tenant_id) {    
        ...
        $payment_types = PaymentType::orderBy('tur','asc')->get();

        return view('backend.cash_transactions.add_cash_transaction', compact('payment_methods','payment_types'));
    }

    public function GetCashPayment($id){
        $cash_payment_id = PaymentType::findOrFail($id);
        $ans = $cash_payment_id->cevaplar;
        
        $answers = explode(", ", $ans);
        $personeller = User::where('status',1)->get();
        $categories = Category::get();
        $musteriler = Musteriler::orderBy('m_adi', 'ASC')->where('durum',1)->get();

        return view('backend.cash_transactions.get_cash_payments',compact('cash_payment_id','answers','personeller','categories','musteriler'));
    }
}
