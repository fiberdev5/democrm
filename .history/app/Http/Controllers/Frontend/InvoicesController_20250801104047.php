...
class InvoicesController extends Controller
{
    ...
    public function AddInvoice($tenant_id) {
        ...
        return view('frontend.secure.invoices.add_invoices',compact('musteriler','payment_methods','firma'));
    }

    public function searchMusteri(Request $request)
    {
        $searchField = $request->input('musteriGetir');
        $musteriler = Musteriler::where('m_adi', 'like', '%' . $searchField . '%')->where('durum',1)->get();
