...
public function addPaymentToInvoice(Request $request, $firma_id)
{
    ...
    try {
        ...
        // ⭐ KALAN TUTAR KONTROLÜ (has_payment kontrolü kaldırıldı)
        $parasutService = new ParasutService($firma_id);
        $parasutInvoiceId = $parasutService->getInvoiceId($invoice->id);
        ...
        if ($request->amount > $remainingAmount) {
            return response()->json([
                'success' => false,
                'message' => "Ödeme tutarı kalan tutardan fazla olamaz. Kalan: {$remainingAmount} TL"
            ], 400);
        }

        // Paraşüt'e ödeme ekle
        $paymentResult = $parasutService->addPayment(
            $parasutInvoiceId,
            $request->account_id,
            $request->amount,
            $request->date,
            $request->description ?? 'Tahsilat'
        );

        if (!$paymentResult['success']) {
            return response()->json([
                'success' => false,
                'message' => $paymentResult['message'] ?? 'Ödeme eklenemedi'
            ], 400);
        }

        // Veritabanını güncelle
        $paymentIds = $invoice->parasutPaymentIds ?? [];
        $paymentIds[] = $paymentResult['payment_id'];
...
public function deletePaymentFromInvoice(Request $request, $firma_id)
{
    ...
    try {
        ...
        // Payment ID'nin varlığını kontrol et (string karşılaştırması)
        ...
        // Paraşüt'ten sil
        $parasutService = new ParasutService($firma_id);
        $deleteResult = $parasutService->deletePayment(
            $invoice->parasutInvoiceId,
            $request->payment_id
        );
        ...
        // Array'den bu payment_id'yi çıkar
        $paymentIds = array_values(array_filter($paymentIds, function($id) use ($request) {
            return $id != $request->payment_id;
        }));
