    ...
    public function paymentCallback(Request $request)
{
    ...
    private function handleStoragePayment($postData)
    {
        ...
        ActivityLogger::logStoragePurchased(
            $purchase->id,
            $purchase->storage_gb,
            $purchase->amount,
            $purchase->package->name ?? 'Storage Paketi'
        );
        ...
        ActivityLogger::logStoragePurchaseFailed(
            $purchase->id,
            $purchase->storage_gb,
            $purchase->amount,
            $postData['failed_reason_msg'] ?? 'Bilinmeyen hata'
        );
    ...
    private function handleIntegrationPayment($postData)
{
        ...
        if ($postData['status'] == 'success') {
            ...
            if ($integration && $integration->slug === 'hipcall') {
                ...
                if (!$purchase->webhook_token) {
                    ...
                    ActivityLogger::logHipcallWebhookCreated(
                        $purchase->id,
                        $webhookUrl
                    );
                }
            ...
            ActivityLogger::logIntegrationPurchased(
                $purchase->id,
                $purchase->integration->name ?? 'Entegrasyon',
                $purchase->amount
            );
            ...
            ActivityLogger::logIntegrationPurchaseFailed(
                $purchase->id,
                $purchase->integration->name ?? 'Entegrasyon',
                $purchase->amount,
                $postData['failed_reason_msg'] ?? 'Bilinmeyen hata'
    ...
    private function handleSubscriptionPayment($postData)
    {
        ...
        if ($postData['status'] == 'success') {
            ...
            ActivityLogger::logPaymentSuccess(
                $payment->id,
                $payment->amount,
    ...
    public function paymentSuccess()
{
    ...
    if (!$billingData || !$planData) {
        ...
        ActivityLogger::logSubscriptionPurchased(
            $subscription->id,
            $plan->name,
            $plan->price,
            $plan->billing_cycle
