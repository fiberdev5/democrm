    ...
    public function UpdateServicePlan(Request $request, $tenant_id)
{
    ...
    try {
        // ... (servisPlan ve planIslemiYapan güncelleme kodları aynı kalacak) ...

        // Plan cevaplarını güncelle
        $planCevaplar = ServiceStageAnswer::where('firma_id', $tenant_id)->where('planid', $planid)->get();

        // İstekte herhangi bir stok veya konsinye checkbox'ı olup olmadığını kontrol etmek için bir flag
        $hasStockData = false;
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($request->has($soruKey)) {
                ...
                if ($yeniCevap == 'Parca') {
                    // Sadece parça verisi gönderildiyse bu bloğu çalıştır
                    if ($hasStockData) {
                        $parcaCevap = $this->processParcaSelection($request, $tenant_id);
                        $cevap->cevap = $parcaCevap;
                        $cevap->save();
                    }
                    // Eğer parça verisi gelmediyse, mevcut cevaba dokunma.
                    
                } elseif ($yeniCevap == 'Konsinye Cihaz') {
                    // Sadece konsinye cihaz verisi gönderildiyse bu bloğu çalıştır
                    if ($hasConsignmentData) {
                        $konsinyeCevap = $this->processKonsinyeSelection($request, $tenant_id);
                        $cevap->cevap = $konsinyeCevap;
                        $cevap->save();
                    }
                    // Eğer konsinye verisi gelmediyse, mevcut cevaba dokunma.
