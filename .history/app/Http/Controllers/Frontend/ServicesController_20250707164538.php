...
class ServicesController extends Controller
{
     ...
     private function stokKontrolEt(Request $request, $gelenIslem)
    {
        foreach ($request->all() as $key => $value) {
            ...
            if (strpos($key, 'soru') !== false && $value == "Konsinye Cihaz") { // Örneğin, bir soruya "ConsignmentDevice" cevabı verildiğinde
            // Burada konsinye cihazlar için özel input alanlarını kontrol etmeliyiz.
            // Örneğin, 'consignment_device_id' ve 'consignment_device_adet' gibi input isimleri varsayalım.
            foreach ($request->all() as $consignmentKey => $consignmentValue) {
                if (strpos($consignmentKey, 'consignment_device_id_') !== false) { // Eğer formda konsinye cihaz ID'si gönderiliyorsa
                    $consignmentDeviceId = (int) str_replace('consignment_device_id_', '', $consignmentKey);
                    $consignmentAdet = abs($request->input("consignment_device_adet_{$consignmentDeviceId}", 0));

                    if ($consignmentAdet > 0) { // Sadece adet girildiyse kontrol yap
                        $girisAdet = StockAction::where('stokId', $consignmentDeviceId)
                                                ->whereIn('islem', [1, 4]) // 1: Alış, 4: Müşteriden İade
                                                ->sum('adet');
                        $cikisAdet = StockAction::where('stokId', $consignmentDeviceId)
                                                ->where('islem', 2) // 2: Serviste Kullanım
                                                ->sum('adet');
                        ...
                        if ($consignmentAdet > $currentConsignmentStock) {
                            $consignmentStock = Stock::where('id', $consignmentDeviceId)->first();
...
private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem)
{
    if ($request->has('soru')) {
        foreach ($request->input('soru') as $soruId => $cevap) {
            if ($cevap == "Parca") {
                $this->parcaIslemleriniYap($request, $servisId, $planId, $tenantId, $soruId, $gelenIslem['id']);
                ...
                if ($selectedConsignment && strpos($selectedConsignment, 'C_') === 0) {
                    ...
                    // Konsinye cihaz kullanımı için özel fonksiyonu çağır
                    $this->useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId);

                    // ServiceStageAnswer kaydını oluştur
                    ServiceStageAnswer::create([
                        'firma_id' => $tenantId,
                        'servisid' => $servisId,
                        'planid' => $planId,
                        'soruid' => $soruId,
                        'cevap' => $consignmentName, 
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        // Konsinye cihazların listesini al
        // Değişken adını $tumConsinyeCihazlar olarak güncelliyoruz.
        $tumConsinyeCihazlar = Stock::where('firma_id', $firma->id)
                                    ->where('urunKategori', 3) // Konsinye cihaz kategorisi (3 olduğunu varsayıyorum)
                                    ->get();

        // Konsinye cihazların toplam adedini hesapla
        $toplamConsignmentAdedi = 0; // Toplam konsinye adedini sıfırla

        foreach ($tumConsinyeCihazlar as $device) { // $tumConsinyeCihazlar üzerinden dön
            $girisAdet = StockAction::where('stokId', $device->id)
                                    ->whereIn('islem', [1, 4]) // 1: Alış, 4: Müşteriden İade
                                    ->sum('adet');
            $cikisAdet = StockAction::where('stokId', $device->id)
                                    ->where('islem', 2) // 2: Serviste Kullanım
                                    ->sum('adet');
            // Her bir cihazın anlık stok miktarını hesapla
            $current_stock_quantity = $girisAdet - $cikisAdet;

            // Sadece pozitif stokları toplama dahil et
            if ($current_stock_quantity > 0) {
                $toplamConsignmentAdedi += $current_stock_quantity;
            }
        }
        ...
        return view('frontend.secure.all_services.edit_service_plan', compact(
            ...
            'tumConsinyeCihazlar' // <<< BURAYI EKLİYORUZ
        ));
    }
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($soru->cevapTuru == '[Parca]') {
                    ...
                    $cevap->cevap = $this->getUpdatedParcaAnswer($request);
                } elseif ($soru->cevapTuru == '[Konsinye Cihaz]') {
                    // Konsinye cihaz cevabını ve stok hareketini güncelle
                    $selectedConsignmentValue = $request->input('soru' . $cevap->id);
                    $quantityInputName = 'consignment_quantity_' . $cevap->id;
                    $selectedQuantity = $request->input($quantityInputName, 1);

                    Log::info("UpdateServicePlan - Konsinye Cihaz İşleniyor. Soru ID: {$cevap->soruid}, Plan ID: {$planid}, Cevap ID: {$cevap->id}");
                    Log::info("Gelen combinedValue: {$selectedConsignmentValue}, Gelen Quantity: {$selectedQuantity}");

                    // Eğer combinedValue C_ID---adet formatında değilse, ayrıştır
                    $consignmentDeviceId = null;
                    if (str_contains($selectedConsignmentValue, 'C_')) {
                        $parts = explode('C_', $selectedConsignmentValue);
                        if (isset($parts[1])) {
                             $subParts = explode('---', $parts[1]);
                             if (isset($subParts[0])) {
                                 $consignmentDeviceId = (int)$subParts[0];
                             }
                        }
                    }

                    if ($consignmentDeviceId) {
                        $this->updateConsignmentStock($tenant_id, $servisPlan->servisid, $planid, $consignmentDeviceId, $selectedQuantity);
                    } else {
                        Log::warning("Konsinye cihaz ID bulunamadı veya geçersiz format: {$selectedConsignmentValue}");
                    }

                    // Cevabı yeni formatta kaydet
                    $device = Stock::find($consignmentDeviceId);
                    $deviceName = $device ? ($device->urunAdi ?? $device->urun_adi ?? 'Cihaz Adı Bulunamadı') : 'Bilinmeyen Cihaz';
                    $cevap->cevap = "{$deviceName}: C_{$consignmentDeviceId}---{$selectedQuantity}";
                } else {
                    // Diğer cevap türleri
                    $soruKey = 'soru' . $cevap->id;
                    if ($request->has($soruKey)) {
                        $cevap->cevap = $request->input($soruKey);
                    }
                }
                $cevap->save();
            }
            ...
            Log::error("Plan güncellenirken bir hata oluştu: " . $e->getMessage() . " Satır: " . $e->getLine());
            return response()->json(['error' => 'Plan güncellenirken bir hata oluştu: ' . $e->getMessage()], 500);
        }
    }
  private function updateConsignmentStock($tenantId, $servisId, $planId, $stokId, $adet)
    {
        Log::info("updateConsignmentStock çağrıldı. Stok ID: {$stokId}, Adet: {$adet}, Plan ID: {$planId}");

        // Öncelikle bu planID ve stokID için daha önce yapılmış bir "Serviste Kullanım" işlemi var mı kontrol et.
        // Eğer varsa, eski kaydı silip yeni adeti girebiliriz veya güncelleyebiliriz.
        // Bu, aynı konsinye cihazın adetinin güncellenmesi durumunda tekrar tekrar stok düşümünü önler.
        $existingStockAction = StockAction::where('stokId', $stokId)
            ->where('islem', 2) // 2: Serviste Kullanım
            ->where('planId', $planId) // Plan ID'sine göre de kontrol edelim
            ->first();

        if ($existingStockAction) {
            // Eğer yeni adet 0 ise veya cihaz kaldırıldıysa kaydı sil
            if ($adet <= 0) {
                $existingStockAction->delete();
                Log::info("Mevcut konsinye stok hareketi silindi: Stok ID: {$stokId}, Plan ID: {$planId}");
                return;
            }

            // Mevcut kaydı güncelle (adet farkını hesapla)
            $oldAdet = $existingStockAction->adet;
            $diffAdet = $adet - $oldAdet;

            if ($diffAdet !== 0) {
                $existingStockAction->adet = $adet;
                $existingStockAction->save();
                Log::info("Konsinye stok hareketi güncellendi: Stok ID: {$stokId}, Eski Adet: {$oldAdet}, Yeni Adet: {$adet}");
            } else {
                Log::info("Konsinye stok adedi değişmedi: Stok ID: {$stokId}, Adet: {$adet}");
            }
        } else {
            ...
            if ($adet > 0) {
                StockAction::create([
                    ...
                    'servisId' => $servisId,
                    'planId' => $planId, // İlişki kurmak için plan ID'yi kaydedin
                    'created_at' => now(),
                    'updated_at' => now()
                ]);
                Log::info("Yeni konsinye stok hareketi oluşturuldu: Stok ID: {$stokId}, Adet: {$adet}, Plan ID: {$planId}");
            }
        }
    }

// private function updateConsignmentDeviceAnswer(Request $request, $cevapId)
// {
//     // Frontend'den gönderilen hidden input'un değerini al
//     $combinedConsignmentValue = $request->input('soru' . $cevapId);
    
//     // Debug için log ekle
//     \Log::info("Konsinye cihaz güncellemesi - Cevap ID: {$cevapId}, Değer: {$combinedConsignmentValue}");
    
//     // Eğer değer boşsa, null döndür
//     if (empty($combinedConsignmentValue)) {
//         return null;
//     }
    
//     // Değer zaten doğru formatta mı kontrol et
//     if (strpos($combinedConsignmentValue, ': C_') !== false) {
//         // Yeni format: "Cihaz Adı: C_ID---Adet"
//         return $combinedConsignmentValue;
//     }
    
//     // Eski format kontrolü: "C_ID---Adet"
//     if (str_starts_with($combinedConsignmentValue, 'C_')) {
//         $parts = explode('---', substr($combinedConsignmentValue, 2));
//         if (count($parts) === 2) {
//             $deviceId = $parts[0];
//             $quantity = $parts[1];
            
//             // Cihaz adını veritabanından al
//             $device = Stock::find($deviceId);
//             if ($device) {
//                 $deviceName = $device->urunAdi ?? $device->urun_adi ?? 'Cihaz Adı Bulunamadı';
//                 return "{$deviceName}: C_{$deviceId}---{$quantity}";
//             }
//         }
//     }
    
//     // Hiçbir format uymazsa orijinal değeri döndür
//     return $combinedConsignmentValue;
// }
