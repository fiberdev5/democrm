 ...
 public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            // Request'ten hem eski hem yeni format soru verilerini al
            $soruData = $request->input('soru', []);
            
            foreach ($planCevaplar as $cevap) {
                $soruKey = 'soru' . $cevap->id;      // Eski format: soru872
                $soruArrayKey = $cevap->id;         // Yeni format: soru[873]
                
                $yeniCevap = null;
                
                // Hem eski hem yeni formatları kontrol et
                if ($request->has($soruKey)) {
                    $yeniCevap = $request->input($soruKey);
                } elseif (isset($soruData[$soruArrayKey])) {
                    $yeniCevap = $soruData[$soruArrayKey];
                }
                ...
                if ($yeniCevap !== null) {
                    ...
                    if ($yeniCevap == 'Parca') {
                        ...
                        // Yeni konsinye cihaz seçimi işlemcisi
                        $konsinyeCevap = $this->processKonsinyeSelection($request, $cevap->id, $tenant_id); // plan cevabının id'sini de gönderiyoruz
            ...
            Log::error('UpdateServicePlan Error: ' . $e->getMessage(), ['exception' => $e]); // Detaylı hata loglaması
            return response()->json([
                'success' => false,
...
private function processParcaSelection(Request $request, $tenant_id)
{
 ...
 private function processKonsinyeSelection(Request $request, $planCevapId, $tenant_id) // planCevapId parametresini ekledik
    {
        $konsinyeCevap = ''; // Tek bir konsinye cihaz olacağı için string olarak tutulur
        $planid = $request->input('planid');
        ...
        // konsinye_cihaz_select[planCevapId] ve konsinye_adet_select[planCevapId] formatında geliyorlar
        $selectedKonsinyeId = $request->input("konsinye_cihaz_select.{$planCevapId}");
        $selectedKonsinyeAdet = $request->input("konsinye_adet_select.{$planCevapId}");

        Log::info('Processing Konsinye Selection (Controller):', [
            'planCevapId' => $planCevapId,
            'selectedKonsinyeId' => $selectedKonsinyeId,
            'selectedKonsinyeAdet' => $selectedKonsinyeAdet,
            'servisid' => $servisid,
            'planid' => $planid,
            'tenant_id' => $tenant_id,
        ]);

        if ($selectedKonsinyeId) {
            $stokId = (int) $selectedKonsinyeId;
            $adet = abs((int) $selectedKonsinyeAdet); // Adeti integer olarak al ve mutlak değerini kullan

            // Cevap stringini oluştur
            ...
            // Eğer select kutusundan hiçbir şey seçilmemişse, önceki kaydı temizle
            // Bu, kullanıcının daha önce seçtiği konsinye cihazı kaldırmasına olanak tanır.
            $existingStockAction = StockAction::where('islem', 2) // Serviste Kullanım
                ->where('planId', $planid)
                ->first();
            if ($existingStockAction) {
                $existingStockAction->delete();
                Log::info("Existing StockAction for planId {$planid} deleted as no konsinye selected.");
            }
        }

        return $konsinyeCevap;
    }
