...
public function TechnicianStatistics($tenant_id)
{
   ...
   public function getTechnicianStatisticsData(Request $request)
    ...
    // app/Http/Controllers/StatisticController.php içine
public function getTechnicianDetailData(Request $request)
{
    $tenant_id = $request->route('tenant_id');
    $persid = $request->input('personelTabloDetayGetir');
    $gelenTarih1Str = $request->input('tarih1'); // 'dd/mm/yyyy'
    $gelenTarih2Str = $request->input('tarih2'); // 'dd/mm/yyyy'
    $cihazTurId = $request->input('cihazTur');

    $gelenTarih1 = Carbon::createFromFormat('d/m/Y', $gelenTarih1Str)->startOfDay();
    $gelenTarih2 = Carbon::createFromFormat('d/m/Y', $gelenTarih2Str)->endOfDay();

    // Grafik label'ları için tarih aralığı
    $labels = collect();
    $currentDate = $gelenTarih1->copy();
    while ($currentDate->lte($gelenTarih2)) {
    ...
    $secilenTarihlerdekiPlanidler = ServiceStageAnswer::query()
        ...
        ->whereIn('soruid', [296, 326, 369, 306, 337, 341]) // 'Tarih' tipindeki soruların ID'leri
        ->whereBetween(
            \DB::raw("STR_TO_DATE(cevap, '%Y-%m-%d')"),
            [$gelenTarih1->toDateString(), $gelenTarih2->toDateString()]
        )
        ->when($cihazTurId, function ($query, $cihazTurId) {
            $query->join('services', 'service_stage_answers.servisid', '=', 'services.id')
                  ->where('services.cihazTur', $cihazTurId);
    ...
    if (empty($secilenTarihlerdekiPlanidler)) {
        // Detaylarda gösterilecek veri yoksa
        return view('frontend.secure.statistics.technician_statistics_table_detail', [
            'labels' => '',
            'tamamlananServisler' => '',
            'iptalServisler' => '',
            'alinanUcretler' => '',
    ...
    $tamamlananServislerArray = [];
    $iptalServislerArray = [];
    $alinanUcretlerArray = [];
    $maxTop = 0; // Grafik için maksimum değer
    $maxTopGelir = 0; // Gelir grafiği için maksimum değer

    $currentDate = $gelenTarih1->copy();
    while ($currentDate->lte($gelenTarih2)) {
        $dateStr = $currentDate->format('Y-m-d');
        $dateForDb = $currentDate->format('d/m/Y'); // Eski koddaki cevap formatı

        $planidsForThisDay = ServiceStageAnswer::where('firma_id', $tenant_id)
            ->whereIn('soruid', [296, 326, 369, 306, 337, 341])
            ->where('cevap', $dateForDb) // Sadece bu güne ait tarih cevaplarını filtrele
            ->when($cihazTurId, function ($query, $cihazTurId) {
                $query->join('services', 'service_stage_answers.servisid', '=', 'services.id')
                      ->where('services.cihazTur', $cihazTurId);
            })
        ...
        if (!empty($planidsForThisDay)) {
            $servislerBuGuneAit = ServiceStageAnswer::where('cevap', $persid)
                ->whereHas('serviceStageQuestion', function ($query) {
                    $query->where('cevapTuru', 'like', '%Grup%');
                })
                ->whereIn('planid', $planidsForThisDay)
                ->pluck('servisid')
                ->unique()
                ->toArray();
        } else {
        ...
        $tamamlananBugun = Service::whereIn('id', $servislerBuGuneAit)
                                 ->where('servisDurum', 255)
                                 ->count();
        $tamamlananServislerArray[] = $tamamlananBugun;
        $maxTop = max($maxTop, $tamamlananBugun);

        // İptal Servisler (bu güne ait olanlar)
        $iptalBugun = ServicePlanning::whereIn('servisid', $servislerBuGuneAit)
                                     ->where('gidenIslem', 244)
                                     ->where('pid', $persid)
                                     ->whereDate('created_at', $currentDate->toDateString()) // Sadece bu güne ait kayıtlar
                                     ->groupBy('servisid')
                                     ->get()
                                     ->filter(function ($plan) use ($persid) {
                                        $ilkServis = ServicePlanning::where('servisid', $plan->servisid)
                                                                    ->where('pid', $persid)
                                                                    ->orderBy('tarih')
                                                                    ->first();
        ...
        $iptalServislerArray[] = $iptalBugun;
        $maxTop = max($maxTop, $iptalBugun);


        // Alınan Ücretler
        $paraToplamBugun = CashMovement::whereIn('servis', $servislerBuGuneAit)
                                       ->where('odemeYonu', 1)
                                       ->where('personel', $persid)
                                       ->whereDate('islemTarihi', $currentDate->toDateString())
                                       ->sum('fiyat');
        $alinanUcretlerArray[] = $paraToplamBugun;
        $maxTopGelir = max($maxTopGelir, $paraToplamBugun);

        $currentDate->addDay();
    }

    $tamamlananServisler = implode(',', $tamamlananServislerArray);
    $iptalServisler = implode(',', $iptalServislerArray);
    $alinanUcretler = implode(',', $alinanUcretlerArray);

    return view('frontend.secure.statistics.technician_statistics_table_detail', compact(
        'labels',
        'tamamlananServisler',
        'iptalServisler',
        'alinanUcretler',
