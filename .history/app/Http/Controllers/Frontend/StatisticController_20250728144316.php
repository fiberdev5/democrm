...
private function getServicePlansForDates($tarihListesi, $tenant_id, $cihazTur = null)
{
    ...
    $query->where(function($q) use ($tarihListesi) {
        // Y-m-d formatında tarih kontrolü
        $q->whereIn('sc.cevap', $tarihListesi);
        
        // d/m/Y formatına çevir ve tekrar kontrol et
        $tarihListesiDMY = array_map(function($tarih) {
            try {
                return Carbon::createFromFormat('Y-m-d', $tarih)->format('d/m/Y');
            } catch (\Exception $e) {
                return $tarih; // Eğer format hatası varsa orijinal değeri kullan
            }
        }, $tarihListesi);
        
        $q->orWhereIn('sc.cevap', $tarihListesiDMY);
    });
