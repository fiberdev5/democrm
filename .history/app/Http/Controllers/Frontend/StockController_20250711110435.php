...
public function barkodPdf($tenant_id, $id) {
    ...
    $pdf = Pdf::loadView('frontend.secure.stocks.stocks_barkod', compact('stock'))
        ->setPaper([0, 0, 141.7, 70.85], 'portrait') // 50mm x 25mm
        ->setOptions([
            ...
            'isPhpEnabled' => false, // güvenlik için
            'debugKeepTemp' => false,
            'debugCss' => false,
            'debugLayout' => false,
...
public function DeleteConsignmentStockAction($tenant_id, $id)
{
    ...
    // Stok hareketine ait işlem türü kontrolü: islem 4 ise silinemez
    if ($stockAction->islem == 4) {
        $notification = [
            'message' => 'Müşteriden iade hareketi silinemez. İşlem yapmak için ilgili iade kaydı üzerinden düzenleme yapmanız gerekmektedir.',
            'alert-type' => 'warning'
        ];
        return redirect()->back()->with($notification);
    }
