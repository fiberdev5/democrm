...
private function processParcaSelection(Request $request, $tenant_id)
{
    ...
    // ÖNEMLİ: Önce mevcut cevabı al
    $mevcutCevap = ServiceStageAnswer::where('firma_id', $tenant_id)
        ->where('planid', $planid)
        ->where('cevap', 'LIKE', '%---%') // Parça formatındaki cevapları bul
        ->first();
    
    $eskiParcalar = [];
    if ($mevcutCevap && $mevcutCevap->cevap) {
        // Eski parçaları diziye çevir
        $eskiCevaplar = explode(', ', $mevcutCevap->cevap);
        foreach ($eskiCevaplar as $eski) {
            $parts = explode('---', $eski);
            if (count($parts) == 2) {
                $eskiParcalar[$parts[0]] = $parts[1]; // stokId => adet
            }
        }
    }
    ...
    // Eski ve yeni seçimleri birleştir
    // Yeni seçimler öncelikli
    $tumParcalar = array_merge($eskiParcalar, $yeniSecimler);
    
    $stokCevap = [];
    foreach ($tumParcalar as $stokId => $adet) {
        $stokCevap[] = "{$stokId}---{$adet}";
        
        // Sadece yeni seçilen parçalar için işlem yap
        if (isset($yeniSecimler[$stokId])) {
            if ($servisPlan->gelenIslem == "238") {
                $this->parcaTeslimEt($stokId, $adet, $servisid, $planid, $tenant_id);
            } else {
                $this->parcaKullan($stokId, $adet, $servisid, $planid, $tenant_id);
