    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($request->has($soruKey)) {
                ...
                // ⭐ PARÇA SEÇİMİ
                if ($yeniCevap == 'Parca') {
                    // ÖNCEKİ PARÇA KULLANIMLARINI SİL
                    $this->deleteOldParcaUsage($cevap, $servisPlan, $tenant_id);
                    
                    // YENİ PARÇALARI KAYDET
                    $parcaCevap = $this->processParcaSelection($request, $tenant_id);
                    $cevap->cevap = $parcaCevap;
                }
                // ⭐ KONSİNYE SEÇİMİ
                elseif ($yeniCevap == 'Konsinye Cihaz') {
                    // ÖNCEKİ KONSİNYE KULLANIMLARINI SİL
                    $this->deleteOldKonsinyeUsage($cevap, $servisPlan, $tenant_id);
                    
                    // YENİ KONSİNYELERİ KAYDET
        ...
        \Log::error('UpdateServicePlan Hatası: ' . $e->getMessage());
        return response()->json([
            'success' => false,
...
private function deleteOldParcaUsage($cevap, $servisPlan, $tenant_id)
{
    // Eski cevaptaki parçaları bul
    if (!empty($cevap->cevap)) {
        $eskiParcalar = explode(', ', $cevap->cevap);
        
        foreach ($eskiParcalar as $eskiParca) {
            $parts = explode('---', $eskiParca);
            if (count($parts) == 2) {
                $eskiStokId = (int)$parts[0];
                $eskiAdet = (int)$parts[1];
                
                // Bu planID ve stokId ile yapılmış stok hareketlerini sil
                $stokHareketleri = StockAction::where('firma_id', $tenant_id)
                    ->where('planId', $cevap->planid)
                    ->where('stokId', $eskiStokId)
                    ->where('servisid', $servisPlan->servisid)
                    ->get();
                
                foreach ($stokHareketleri as $hareket) {
                    // Stok hareketini geri al (stoğu iade et)
                    $this->reverseStockMovement($hareket, $tenant_id);
...
// ============================================================================
// ⭐ YENİ FONKSİYON: ESKİ KONSİNYE KULLANIMLARINI SİL
// ============================================================================

private function deleteOldKonsinyeUsage($cevap, $servisPlan, $tenant_id)
{
    // Eski cevaptaki konsinyeları bul
    if (!empty($cevap->cevap)) {
        $eskiKonsinyeler = explode(', ', $cevap->cevap);
        
        foreach ($eskiKonsinyeler as $eskiKonsinye) {
            $parts = explode('---', $eskiKonsinye);
            if (count($parts) == 2) {
                $eskiStokId = (int)$parts[0];
                $eskiAdet = (int)$parts[1];
...
// ============================================================================
// ⭐ YENİ FONKSİYON: STOK HAREKETİNİ GERİ AL
// ============================================================================

private function reverseStockMovement($hareket, $tenant_id)
{
    // Eğer personel stoğundan kullanıldıysa, personel stoğuna geri ekle
    if ($hareket->islem == 2) { // Serviste kullanım
        $personelStok = PersonelStok::where('firma_id', $tenant_id)
            ->where('personelid', auth()->id())
            ->where('stok_id', $hareket->stokId)
            ->first();
    ...
    // Eğer konsinye kullanıldıysa, konsinye stoğuna geri ekle
    $stok = Stock::find($hareket->stokId);
    if ($stok) {
        $stok->current_stock_quantity += $hareket->adet;
        $stok->save();
    }
    
    // Kasa hareketini de geri al (varsa)
    // ... Kasa işlemleri için gerekli kod ...
}
...
private function processParcaSelection(Request $request, $tenant_id)
{
    ...
    // ⭐ YENİ SEÇİLEN PARÇALARI İŞLE
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'stokCheck') === 0 && $value === 'on') {
...
private function processKonsinyeSelection(Request $request, $tenant_id)
{
    ...
    // ⭐ YENİ SEÇİLEN KONSİNYELERİ İŞLE
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'konsinyeCheck') === 0) {
            $stokId = (int) filter_var($key, FILTER_SANITIZE_NUMBER_INT);
