...
public function uploadPhoto(Request $request, $tenant_id)
{
    ...
    // Klasör yolu (storage/app/public/upload/stock_photos)
    $folderPath = 'upload/stock_photos';

    // Geçici olarak bellekte resize yapılacak
    ...
    // Dosyayı storage/public/upload/stock_photos dizinine kaydet
    Storage::disk('public')->put($folderPath . '/' . $name_gen, $img);

    // Veritabanına kayıt
    $photo = stock_photos::create([
        'kid' => $tenant_id,
        'stock_id' => $request->stock_id,
        'resimyol' => $folderPath . '/' . $name_gen,
    ]);

    return response()->json([
        'id' => $photo->id,
        'resim_yolu' => Storage::disk('public')->url($photo->resimyol),
