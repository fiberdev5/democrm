...
// Eğer parça seçimi varsa (formda gizli input değeri 'Parca' geliyorsa)
if ($yeniCevap == 'Parca') {
    $selectedStokIds = $request->input("stokCheck_{$cevap->planid}");
    $stokAdetleri = $request->input("stokAdet_{$cevap->planid}");

    $updatedParts = [];
    if (!empty($selectedStokIds) && is_array($selectedStokIds)) {
        foreach ($selectedStokIds as $stokId) {
            $adet = isset($stokAdetleri[$stokId]) ? (int)$stokAdetleri[$stokId] : 1;

            // Adet 0 veya negatif ise dikkate alma
            if ($adet > 0) {
                // Stok adetini güncelle (stoktan düş)
                $stockItem = Stock::where('firma_id', $tenant_id)->find($stokId);
                if ($stockItem) {
                    // Eski adet ile yeni adet arasındaki farkı bul
                    // Planın eski cevabını bulup eski adeti çıkarıp yeni adeti eklememiz gerek.
                    // Bu kısım biraz daha karmaşık, şimdilik sadece yeni miktarı kaydedelim,
                    // stok düşme işlemi ayrı bir fonksiyonda daha dikkatli yapılmalı.
                    // Ancak şimdilik sadece plan cevabını düzgün kaydedelim.
    ...
    $cevap->cevap = json_encode($updatedParts); // JSON olarak kaydet
} else {
...
public function searchStock(Request $request)
{
    ...
    if (!$query || strlen($query) < 3) {
        return response()->json([]); // Sorgu çok kısaysa boş yanıt dön
    }

    $stocks = Stock::where('firma_id', $tenant_id)
        ->where(function($q) use ($query) {
            $q->where('urunAdi', 'like', '%' . $query . '%')
              ->orWhere('urunKodu', 'like', '%' . $query . '%');
        })
        ->where('adet', '>', 0) // Sadece stokta olanları göster (0'dan büyük)
        ->orderBy('urunAdi', 'asc')
        ->limit(10) // İlk 10 sonucu al
        ->get(['id', 'urunAdi', 'urunKodu', 'adet']); // Sadece gerekli alanları seç
