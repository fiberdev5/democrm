...
public function UpdateDealer(Request $request, $tenant_id, $id)
{
    ...
    if ($request->hasFile('belgePdf')) {
        ...
        if ($bayi->belgePdf && file_exists(public_path('storage/' . $bayi->belgePdf))) {
            unlink(public_path('storage/' . $bayi->belgePdf));
        }

        // Yeni belgeyi yükle
        $file = $request->file('belgePdf');
        $fileName = time() . '_' . Str::slug($request->username, '-') . '.' . $file->getClientOriginalExtension();
        $belgePdfPath = $file->storeAs('bayi-belgeleri', $fileName, 'public');
        $bayi->belgePdf = $belgePdfPath;
    }
