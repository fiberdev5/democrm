...
public function AllStocks($tenant_id, Request $request)
{
 ...
 public function EditStock($tenant_id, $id) {
    ...
    $stokHareketleri = StockAction::with(['musteri'])
        ...
        ->leftJoin('tb_user as user_performer', 'user_performer.user_id', '=', 'stock_actions.kid') // kid ile join daha doğru olabilir
        ->where('stock_actions.stokId', $id)
        ->orderBy('stock_actions.id', 'desc')
        ->get();

    // 2. PERSONEL STOKLARINI ÇEK (GetPersonelStocks fonksiyonunuzdaki mantıkla)
    ...
    // 3. FOTOĞRAFLARI ÇEK
    $photos = stock_photos::where('firma_id', $tenant_id) // kid yerine firma_id daha mantıklı olabilir, modelinize bağlı
                          ->where('stock_id', $id)
                          ->latest()
                          ->get();
            $html = view('frontend.secure.stocks.edit_stock', compact('firma', 'stock', 'rafListesi', 'markalar',
     'kategoriler', 'cihazlar','stokHareketleri',  // YENİ
        'hareketler',  
        'photos'       ))->render();
