...
public function DeleteConsignmentStockAction($tenant_id, $id)
{
    ...
    // Bu blok, islem 2 (serviste kullanım) ve islem 4 (müşteriden iade) kayıtlarını silmeyi engelleyecektir.
    // Artık yukarıdaki hatalı `$stokHareketSayisi` kontrolünü kullanmıyoruz.
    ...
    // İşlem 1 (alış) ise ve bu alıştan sonra çıkış yapılmışsa silinemez kontrolü
    if ($stockAction->islem == 1) {
        $girisTarihi = $stockAction->created_at;
    ...
    try {
        // Silme işleminden önce stok adedini güncelleme mantığını ekleyebiliriz
        ...
        if ($stock) {
            ...
            // islem 4 için buraya gelmeyecek, çünkü yukarıdaki in_array bloğunda engelleniyor.
            // Ama eğer islem 4'ün silinmesine izin verirsek (test amaçlı vs.) ve stok da güncellensin istersek:
            // elseif ($stockAction->islem == 4) { // Müşteriden iade işlemi siliniyorsa, stok adeti azaltılmalı
            //     $stock->adet -= $stockAction->adet;
            // }
            $stock->save();
        }
