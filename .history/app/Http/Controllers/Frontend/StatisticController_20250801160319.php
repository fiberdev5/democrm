...
{
    $start = Carbon::createFromFormat('d/m/Y', $tarihListesi[0])->format('Y-m-d');
    $end = Carbon::createFromFormat('d/m/Y', end($tarihListesi))->format('Y-m-d');
    ...
    // Cevap sütunundaki tarih değerlerini doğrudan bir aralıkta sorgula
    $query->whereRaw("STR_TO_DATE(service_stage_answers.cevap, '%d/%m/%Y') BETWEEN ? AND ?", [$start, $end]);

    if ($cihazTur) {
        $query->where('services.cihazTur', $cihazTur);
    }

    $seciliTarihler = $query->pluck('service_stage_answers.planid')
                           ->unique()
                           ->toArray();
