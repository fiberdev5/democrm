    ...
    public function AllServices($tenant_id, Request $request) {
        ...
        if ($request->ajax()) {           
            ...
            if ($user->can('Kendi Servislerini Görebilir')) {
                    ...
                    $servisIDleri = \App\Services\ServiceFilter::getYetkiliServisIDleri($user);

    // Eğer hiçbir yetkili servis yoksa, hiç veri göstermeyelim
    if (empty($servisIDleri)) {
        $data->whereNull('id'); // hiçbir id eşleşmez
    } else {
