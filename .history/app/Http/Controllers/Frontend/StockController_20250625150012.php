...
public function AllStocks($tenant_id, Request $request)
{
    ...
    $query = Stock::select('stocks.*')
        ...
        foreach ($stocksForTotal as $stock) {
            ...
            $toplamCikis = \App\Models\StockAction::where('stokId', $stock->id)->whereIn('islem', 2)->sum('adet');  //personele gonder
        ...
        return DataTables::of($query)
            ...
            ->addColumn('adet', function($row) {
                ...
                $toplamCikis = \App\Models\StockAction::where('stokId', $row->id)->whereIn('islem', 2)->sum('adet');
                $kalanAdet = $toplamGiris - $toplamCikis;
