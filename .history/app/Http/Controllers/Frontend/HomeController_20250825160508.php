...
{
    // ÖNEMLİ: Lütfen 'service_status' tablonuzun ve sütunlarınızın adlarını
    // sorgudaki adlarla eşleştiğinden emin olun.
    // Varsayılanlar: Tablo='service_status', Sütunlar='id', 'status_name', 'color_class'

    return DB::table('services as s')
        ->join('customers as c', 's.musteri_id', '=', 'c.id')
        ->leftJoin('tb_user as u', 's.pid', '=', 'u.user_id')
        
        // YENİ: Servis durumu tablosunu join ile bağlıyoruz.
        // Eğer durum ID'si geçersizse bile satırın gelmesi için 'leftJoin' kullanmak daha güvenlidir.
        ->leftJoin('service_status as ss', 's.servisDurum', '=', 'ss.id')
        ...
        ->select(
            ...
            // YENİ: Durum adını ve renk sınıfını doğrudan DB'den alıyoruz.
            // IFNULL ile, eşleşen bir durum bulunamazsa varsayılan değerler atıyoruz.
            DB::raw("IFNULL(ss.status_name, 'Durum Belirsiz') as status_name"),
            DB::raw("IFNULL(ss.color_class, 'status-pending') as color_class")
        )
    ...
    // NOT: $statusMap dizisi ve foreach döngüsü artık GEREKLİ DEĞİL ve SİLİNDİ.
    // Veri, doğrudan sorgu ile hazır geliyor.
}
