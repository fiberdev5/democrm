...
public function consignmentDeviceData(Request $request, $tenant_id)
{
    ...
    foreach ($stocksForTotal as $stock) {
        $toplamGiris = \App\Models\StockAction::where('stokId', $stock->id)->whereIn('islem', [1, 3])->sum('adet'); //alış ve müşteriden iade
    ...
    return DataTables::of($query)
        ...
        ->addColumn('action', function($row) use ($tenant_id) {
            ...
            $hareketEkleBtn = '<button type="button" class="btn btn-success btn-sm hareketEkleBtn" data-stokid="'.$row->id.'" title="Hareket Ekle"><i class="fas fa-plus"></i></button>';
            $delBtn = '<a href="'.$deleteUrl.'" class="btn btn-danger btn-sm" title="Sil" onclick="return confirm(\'Silmek istediğinize emin misiniz?\');"><i class="fas fa-trash-alt"></i></a>';
            return $editBtn . ' ' . $hareketEkleBtn . ' ' . $delBtn;
        })
        ->filter(function ($query) use ($request) {
            if ($search = $request->get('search')['value'] ?? null) {
                $query->where(function ($q) use ($search) {
...
public function StoreConsignmentStockAction(Request $request, $tenant_id)
{
    ...
    $toplamGiris = StockAction::where('stokId', $stokId) ->whereIn('islem', [1, 3])->sum('adet'); //alış ve müşteriden iade
