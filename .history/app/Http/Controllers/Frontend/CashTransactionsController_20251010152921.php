...
class CashTransactionsController extends Controller
{
    ...
    {   
        ...
        // Tarih filtreleme mantığı - KASA İÇİN GÜNLÜK (BUGÜN)
        $hasUserSelectedCashDate = $request->filled('from_date') && $request->filled('to_date') && !$this->isDefaultCashDateRange($request);
        ...
        // Dashboard filtreleme kontrolü
        $hasDashboardFilter = $request->has('dashboard_filter') && $request->dashboard_filter == 1;

        if ($hasDashboardFilter && $request->has('dashboard_istatistik_tarih1') && $request->has('dashboard_istatistik_tarih2')) {
            // Dashboard'dan gelen filtre varsa onu kullan
            $data->whereDate('created_at', '>=', $request->dashboard_istatistik_tarih1)
                ->whereDate('created_at', '<=', $request->dashboard_istatistik_tarih2);
        } elseif ($hasUserSelectedCashDate) {
            // Kullanıcının seçtiği tarih filtresi en yüksek önceliğe sahiptir
            $this->applyMainCashDateRange($data, $request);
        } elseif (!$hasUserSelectedCashDate && !$hasSearchOrOtherFilters && !$hasDashboardFilter) {
            // Hiçbir tarih veya arama/filtre yoksa, varsayılan BUGÜNÜ uygula
        ...
        // ... geri kalan filtreleme kodları aynen kalacak (ödeme şekli, personel vs.)
    ...
    // Helper Methods - GÜNLÜK İÇİN
private function isDefaultCashDateRange(Request $request): bool
{
...
private function applyMainCashDateRange($query, Request $request): void
{
    if ($request->filled('from_date') && $request->filled('to_date')) {
        $from = Carbon::createFromFormat('Y-m-d', $request->from_date)->startOfDay();
        $to   = Carbon::createFromFormat('Y-m-d', $request->to_date)->endOfDay();
