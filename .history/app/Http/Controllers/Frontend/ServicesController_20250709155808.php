...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    private function konsinyeKullan($konsinyeId, $adet, $servisId, $planId, $tenantId)
{
    // Konsinye cihazın bilgilerini al
    // 'Stock' modeliniz konsinye cihazları da içeriyorsa onu kullanabilirsiniz.
    // Eğer konsinye cihazlar için ayrı bir modeliniz varsa (örn: ConsignmentDevice), onu kullanmalısınız.
    $konsinyeCihaz = App\Models\Stock::where('id', $konsinyeId)->first(); 

    if (!$konsinyeCihaz) {
        // Loglama veya hata fırlatma: Konsinye cihaz bulunamadı
        \Log::error("Konsinye cihaz bulunamadı: ID {$konsinyeId}");
        return; 
    }

    $fiyat = $adet * $konsinyeCihaz->fiyat; // Konsinye cihazın fiyatı üzerinden hesaplama

    // Stok hareketi kaydet (islem: 4 - Konsinye Kullanım olarak belirledim, siz ihtiyaca göre değiştirebilirsiniz)
    $stokHareketId = App\Models\StockAction::insertGetId([
        'firma_id' => $tenantId,
        'kid' => auth()->id(), // İşlemi yapan kullanıcı
        'stokId' => $konsinyeId,
        'islem' => 2, 
        'servisid' => $servisId,
        'adet' => $adet,
        'fiyat' => $fiyat,
        'fiyatBirim' => 1, // Fiyat birimi
        'planId' => $planId,
        'created_at' => now(),
        'updated_at' => now()
    ]);

    // Konsinye stoktan düş
    // Stock modelindeki 'current_stock_quantity' veya benzeri bir alanı güncelleyin
    if ($konsinyeCihaz) {
        $konsinyeCihaz->current_stock_quantity -= $adet; // Konsinye stok miktarını azalt
        $konsinyeCihaz->save(); 
    }

    // Kasa ve Servis Para hareketlerini de isterseniz buraya uyarlayabilirsiniz.
    // Şimdilik yorum satırı olarak bırakılmıştır.
    /*
    $servisDurum = App\Models\Service::where('id', $servisId)->first();
    $stokIslem = App\Models\PaymentType::where('parca', '1')->first(); // Eğer konsinyeler için farklı bir PaymentType'ınız varsa onu kullanın
    ...
    DB::table('kasa_hareketleri')->insert([
        ...
        'islem_tarihi' => now(),
        'odeme_yonu' => 2, // Çıkış
        'odeme_sekli' => 178, // Nakit (Örnek değer)
        'odeme_turu' => $stokIslem->id ?? null,
        'odeme_durum' => 1,
        'fiyat' => $fiyat,
        'fiyat_birim' => 1,
        'aciklama' => "Konsinye Cihaz ID: {$konsinyeId} ({$konsinyeCihaz->urun_adi}) Kullanım",
        'marka' => $servisDurum->cihaz_marka ?? null,
        'cihaz' => $servisDurum->cihaz_tur ?? null,
        'servis_id' => $servisDurum->id ?? null,
        'stok_islem' => $stokHareketId,
        'created_at' => now(),
        'updated_at' => now()
    ]);
...
private function konsinyeTeslimEt($konsinyeId, $adet, $servisId, $planId, $tenantId)
{
    // // Konsinye cihazın bilgilerini al
    // $konsinyeCihaz = App\Models\Stock::where('id', $konsinyeId)->first();
    ...
    // // Stock modelindeki 'current_stock_quantity' veya benzeri bir alanı güncelleyin
    // if ($konsinyeCihaz) {
    //     $konsinyeCihaz->current_stock_quantity += $adet; // Konsinye stok miktarını artır
    //     $konsinyeCihaz->save();
    // }

  
    // App\Models\StockAction::create([
    //     'firma_id' => $tenantId,
    //     'stokId' => $konsinyeId,
    //     'islem' => 4, // 4: Konsinye Teslimat/İade (Örnek değer)
    //     'adet' => $adet,
    //     'servisid' => $servisId, // Eğer bir servis ile ilişkilendirilecekse
    //     'fiyat' => 0, // Teslimatta genellikle fiyat olmaz
    //     'fiyatBirim' => 1,
    //     'planId' => $planId,
    //     'kid' => auth()->id(), // İşlemi yapan kullanıcı
    //     'created_at' => now(),
    ...
    // Kasa ve Servis Para hareketleri burada genellikle olmaz,
    // çünkü bu bir stok girişi veya iade işlemidir, bir satış değildir.
}
...
private function processKonsinyeSelection(Request $request, $tenant_id)
{
    ...
    $konsinyeCihazData = $request->input('konsinye_cihaz', []);
    $konsinyeAdetData = $request->input('konsinye_adet', []);



  foreach ($konsinyeCihazData as $stageId => $consignmentIds) {
        foreach ($consignmentIds as $consignmentId => $value) { // value aslında consignmentId olacak, ama bu döngüde kullanmıyoruz
            $adet = 1; // Varsayılan adet 1
            if (isset($konsinyeAdetData[$stageId][$consignmentId])) {
                $adet = abs($konsinyeAdetData[$stageId][$consignmentId]);
            }

            // konsinyeCevap dizisine ekle
            $konsinyeCevap[] = "{$consignmentId}---{$adet}";

            // Kullanım mı teslim mi kontrol et
            if ($servisPlan->gelenIslem == "238") {
                $this->konsinyeTeslimEt($consignmentId, $adet, $servisid, $planid, $tenant_id);
            } else {
                $this->konsinyeKullan($consignmentId, $adet, $servisid, $planid, $tenant_id);
            }
        }
    }
