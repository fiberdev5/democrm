...
class ServicesController extends Controller
{
    ...
    foreach ($consignmentDevices as $device) {
        ...
        // Önceki adımdaki $device->name atamasını kaldırıyoruz.
        // Blade'de $device->urunAdi veya $device->urun_adi kullanılacak.
    ...
    private function stokKontrolEt(Request $request, $gelenIslem)
{
    foreach ($request->input('soru', []) as $soruId => $cevap) {
        ...
        if ($cevap == "Parca") {
            foreach ($request->all() as $stokKey => $stokValue) {
                if (strpos($stokKey, 'stokCheck') !== false) {
                    ...
                    $currentStock = $this->calculateCurrentStock($stokId);

                    if ($currentStock <= 0 || $adet > $currentStock) {
        ...
        // Yeni: Konsinye Cihaz Kontrolü
        elseif ($cevap == "Konsinye Cihaz") {
            $consignmentSelectName = "parca[{$soruId}][consignment_select]";
            $consignmentQuantityName = "adet[{$soruId}][selected_consignment_quantity]";

            $selectedConsignmentValue = $request->input($consignmentSelectName);
            $selectedConsignmentQuantity = (int) $request->input($consignmentQuantityName, 0);

            // Eğer bir konsinye cihaz seçilmişse (boş değilse ve 'C_' ile başlıyorsa)
            if (!empty($selectedConsignmentValue) && strpos($selectedConsignmentValue, 'C_') === 0) {
                $consignmentDeviceId = (int) str_replace('C_', '', $selectedConsignmentValue);

                $currentConsignmentStock = $this->calculateConsignmentStock($consignmentDeviceId);

                if ($currentConsignmentStock <= 0 || $selectedConsignmentQuantity > $currentConsignmentStock) {
                    $device = Stock::where('id', $consignmentDeviceId)->first();
                    return "STOKHATA: " . mb_convert_case($device->urunAdi ?? $device->urun_adi, MB_CASE_TITLE, "UTF-8") . " Konsinye Cihaz Stoğu Yetersizdir.";
                }
                // Seçim yapıldıysa ve adet 0 ise veya negatif ise hata döndür
                if ($selectedConsignmentQuantity <= 0) {
                     return "STOKHATA: Konsinye Cihaz Adeti sıfır veya negatif olamaz.";
                }
            }
        }
    }
...
// Yardımcı fonksiyon: Genel stok hesaplaması
private function calculateCurrentStock($stokId)
{
    $stokHareketleri = StockAction::where('stokId', $stokId)->get();
    $toplam = 0;
    foreach ($stokHareketleri as $hareket) {
        if ($hareket->islem == "1" || $hareket->islem == "4") { // 1: Alış, 4: Müşteriden İade (giriş işlemleri)
            $toplam += $hareket->adet;
        } elseif ($hareket->islem == "2" || $hareket->islem == "3") { // 2: Serviste Kullanım, 3: Personel Teslim (çıkış işlemleri)
            if ($hareket->plan_id == 0) { // Sadece henüz planlanmamış hareketleri düş
                $toplam -= $hareket->adet;
            }
        }
    }
...
// Yardımcı fonksiyon: Konsinye stok hesaplaması
private function calculateConsignmentStock($consignmentDeviceId)
{
   ...
   private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem)
{
    if ($request->has('soru')) {
        foreach ($request->input('soru') as $soruId => $cevap) {
            if ($cevap == "Parca") {
                // Mevcut parça işleme mantığı
                $this->parcaIslemleriniYap($request, $servisId, $planId, $tenantId, $soruId, $gelenIslem);
            } elseif ($cevap == "Konsinye Cihaz") { // Yeni: Konsinye Cihaz İşleme
                $consignmentSelectName = "parca[{$soruId}][consignment_select]";
                $consignmentQuantityName = "adet[{$soruId}][selected_consignment_quantity]";
                ...
                if (!empty($selectedConsignmentValue) && strpos($selectedConsignmentValue, 'C_') === 0) {
                    ...
                    // Konsinye cihazı kaydet ve stok hareketini yap
                    $this->consignmentCihazIsle($consignmentDeviceId, $selectedConsignmentQuantity, $servisId, $planId, $tenantId, $soruId);

                    // Cevabı kaydet (Örn: "C_123---2" formatında)
                    ServiceStageAnswer::create([
                        ...
                        'planid' => $planId,
                        'soruid' => $soruId,
                        'cevap' => "C_{$consignmentDeviceId}---{$selectedConsignmentQuantity}", // Konsinye cihaz cevabı
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
...
private function consignmentCihazIsle($consignmentDeviceId, $adet, $servisId, $planId, $tenantId, $soruId)
{
    $stok = Stock::where('id', $consignmentDeviceId)->first();
    if (!$stok) {
        // Cihaz bulunamazsa hata logla veya fırlat
        \Log::error("Konsinye cihaz bulunamadı: ID {$consignmentDeviceId}");
        return;
    }

    $fiyat = $adet * $stok->fiyat; // Konsinye cihazın fiyatı varsa kullan

    // Stok hareketi kaydet (islem: 2 Serviste Kullanım)
    $stokHareketId = StockAction::insertGetId([
        'firma_id' => $tenantId,
        'kid' => auth()->id(),
        'stokId' => $consignmentDeviceId,
        'islem' => 2, // 2: Serviste Kullanım (Konsinye cihazlar için de bu işlemi kullanıyoruz)
        'servisid' => $servisId,
        'depo' => 1, // Varsa varsayılan depo ID'si, yoksa null bırakılabilir
        'adet' => $adet,
        'fiyat' => $fiyat,
        'fiyatBirim' => 1, // Varsayılan fiyat birimi
        'planId' => $planId,
        'created_at' => now(),
        'updated_at' => now()
    ]);

    // Konsinye cihaz stoğundan düşme işlemi burada yapılmış oluyor (StockAction ile)
    // PersonelStock tablosu konsinye cihazlar için kullanılmıyor, çünkü onlar genel stoktan düşer.
    // Eğer konsinye cihazlar da personele atanmış gibi takip ediliyorsa, PersonelStock tablosuna da ekleme yapmanız gerekebilir.
    // Ancak genel konsinye cihaz mantığında bu gerekmez.

    // Kasa ve servis para hareketleri, eğer konsinye cihazlar için de maliyet veya gelir kaydı yapılıyorsa eklenebilir.
    // Şu anki kodunuzda yorum satırı olan kısımları buraya ekleyebilirsiniz, fakat mantığına göre.
    // Örneğin, konsinye cihazlar genellikle müşteriye fatura kesildiğinde gelir oluşturur,
    // doğrudan serviste kullanıldığında bir maliyet olarak yansımayabilir.
    /*
    $servisDurum = Service::where('id', $servisId)->first();
    $stokIslem = PaymentType::where('parca', '1')->first(); // Eğer bu "parça kullanımı" ile ilgili bir ödeme türüyse
    ...
    DB::table('kasa_hareketleri')->insert([
        ...
        'personel_id' => auth()->id(),
        'islem_tarihi' => now(),
        'odeme_yonu' => 2, // 2: Gider
        'odeme_sekli' => 178, // Ödeme şekli (örneğin nakit)
        'odeme_turu' => $stokIslem->id,
        'odeme_durum' => 1,
        'fiyat' => $fiyat,
        'fiyat_birim' => 1,
        'aciklama' => "Konsinye Cihaz Kullanımı: ID {$consignmentDeviceId} ({$stok->urun_adi})",
        'marka' => $servisDurum->cihaz_marka ?? null,
        'cihaz' => $servisDurum->cihaz_tur ?? null,
