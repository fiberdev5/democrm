...
class SuperAdminController extends Controller
{
    ...
    {
        // Temel istatistikler - sadece mevcut alanları kullan
        $stats = [
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('subscription', function($row) {
                    // Abonelik durumu - sadece trial ve active var
                    $subscriptionBadge = '';
                    if($row->subscription_status === 'trial') {
                        if($row->trial_ends_at && \Carbon\Carbon::parse($row->trial_ends_at)->isFuture()) {
                            $subscriptionBadge = '<span class="badge bg-warning"><i class="mdi mdi-timer"></i> Deneme</span>';
                        } else {
                            $subscriptionBadge = '<span class="badge bg-secondary"><i class="mdi mdi-clock-alert"></i> Deneme Dolmuş</span>';
                        }
                    } elseif($row->subscription_status === 'active') {
                        $subscriptionBadge = '<span class="badge bg-primary"><i class="mdi mdi-crown"></i> Aboneli</span>';
                    } else {
                        $subscriptionBadge = '<span class="badge bg-light text-dark">Paket Yok</span>';
                    }
                ...
                ->addColumn('action', function($row) {
                    ...
                    $impersonateButton = '';
                    // Sadece genel durum aktif olanlar için giriş butonu (abonelik durumu önemli değil)
                    if ($row->status == 1) {
    ...
    public function changeTenantStatus($id)
    {
        ...
        // Firma aktif edildiğinde mail gönder (abonelik durumuna bakılmaksızın)
        if ($tenant->status == 1) {
