    ...
    private function getHourlyData($startDate, $endDate, $tenant_id)
{
    ...
    $hourlyStats = Service::where('firma_id', $tenant_id)
                          ->where('durum', 1)
                          ->whereBetween('created_at', [ // <-- Burayı 'created_at' olarak değiştirdim
                              Carbon::parse($startDate)->startOfDay(), // Tarih aralığını Carbon ile oluştur
                              Carbon::parse($endDate)->endOfDay()     // böylece saat bilgisi de dahil olur
                          ])
                          ->select(DB::raw('HOUR(created_at) as hour, COUNT(*) as count')) // <-- Burayı 'created_at' olarak değiştirdim
                          ->groupBy(DB::raw('HOUR(created_at)')) // <-- Burayı 'created_at' olarak değiştirdim
                          ->orderBy('hour')
                          ->get();
