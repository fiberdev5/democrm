...
{
    // Not: service_status, customers ve users tablolarınızın adları ve sütunları farklıysa düzenlemeniz gerekebilir.
    return DB::table('services as s')
        ->join('customers as c', 's.musteri_id', '=', 'c.id')
        ->join('users as u', 's.teknisyen_id', '=', 'u.id', 'left') // Teknisyen atanmamış olabilir
        ->join('service_status as ss', 's.durum_id', '=', 'ss.id')
        ->select(
            's.id as service_id',
            's.servis_no',
            'c.name as customer_name',
            's.cihaz_modeli', // Veya sorunu belirten başka bir sütun
            'u.name as technician_name',
            's.bitis_tarihi_tahmini',
            'ss.status_name',
            'ss.color_class' // Durum için bir renk sınıfı (örn: 'status-high', 'status-completed')
        )
...
// YENİ: Kritik stokları getiren fonksiyon
private function getStockAlerts($tenant_id)
{
    // Not: products tablonuzun adının ve sütunlarının (stock, critical_stock_level, low_stock_level)
    // varsayımsal olduğunu unutmayın. Kendi yapınıza göre güncelleyin.
    $products = DB::table('products')
        ->where('firma_id', $tenant_id)
        ->where('stock', '<=', DB::raw('low_stock_level')) // Sadece düşük veya kritik olanları getir
        ->select('id', 'product_name', 'category', 'stock', 'critical_stock_level')
        ->orderBy('stock', 'asc') // En az stoklu olan en üstte
        ->get();
    ...
    // Her birinden en fazla 2'şer tane alalım
