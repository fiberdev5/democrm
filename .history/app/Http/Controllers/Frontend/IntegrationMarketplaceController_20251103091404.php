...
class IntegrationMarketplaceController extends Controller
{
    ...
    public function index(Request $request, $tenant_id) {
        ...
        $activeIntegrations = IntegrationPurchase::where('tenant_id', $tenant->id)
             ->where('is_active', 1)
             ->pluck('integration_id')
             ->toArray();

        return view('frontend.secure.integrations.marketplace', compact('integrations', 'categories', 'tenant', 'activeIntegrations'));
    }
