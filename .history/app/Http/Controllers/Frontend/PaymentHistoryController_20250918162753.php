...
class PaymentHistoryController extends Controller
{
            ...
            $storagePurchases = $tenant->storagePurchases()
                ...
                ->when($paymentMethod, function($query) use ($paymentMethod) {
                    return $query->where(function($q) use ($paymentMethod) {
                        $q->whereJsonContains('payment_response', ['payment_method' => $paymentMethod])
                ...
                ->map(function($purchase) {
                    return [
                        ...
                        'payment_method' => $this->extractPaymentMethod($purchase->payment_response ?? $purchase->payment_method),
    ...
    private function extractPaymentMethod($paymentResponse)
    {
        ...
        if (is_array($paymentResponse) && isset($paymentResponse['payment_method'])) {
            return $paymentResponse['payment_method'];
        }

        return $paymentResponse ?: 'Belirtilmemiş';
    }
    ...
    public function export(Request $request, $tenant_id)
    {
        ...
        if (method_exists($tenant, 'storagePurchases')) {
            $storagePurchases = $tenant->storagePurchases()
                ...
                ->when($paymentMethod, function($query) use ($paymentMethod) {
                    return $query->where(function($q) use ($paymentMethod) {
                        $q->whereJsonContains('payment_response', ['payment_type' => $paymentMethod])
                ...
                ->map(function($purchase) {
                    return [
                        ...
                        'currency' => 'TL',
                        'payment_method' => $this->extractPaymentMethod($purchase->payment_response ?? $purchase->payment_type),
                        'status_label' => $this->getStatusLabel($purchase->status),
        ...
        // CSV olarak export et (Excel yerine basit CSV)
        $filename = 'odeme-gecmisi-' . $tenant->name . '-' . now()->format('Y-m-d') . '.csv';
