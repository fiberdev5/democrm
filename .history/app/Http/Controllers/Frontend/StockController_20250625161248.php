...
public function StoreStockAction(Request $request, $tenant_id)
{
    ...
    // İşleme göre kuralları dinamik belirle
    $rules = [
    ...
    if ($request->islem == 1) {
        $rules['fiyat']     = 'required|numeric';
        $rules['tedarikci'] = 'nullable|string|max:255';
    } elseif ($request->islem == 2) {
        $rules['personel'] = 'required|integer|exists:users,user_id';
    }
    ...
    // Eğer işlem personel gönderimiyse, yeterli stok kontrolü
    if ($request->islem == 2) {
        $toplamGiris = StockAction::where('stokId', $stokId)->where('islem', 1)->sum('adet');
        $toplamCikis = StockAction::where('stokId', $stokId)->where('islem', 2)->sum('adet');
        $kalan = $toplamGiris - $toplamCikis;

        if ($request->adet > $kalan) {
            return redirect()->back()->with([
                'message' => 'Yeterli stok yok. Kalan: ' . $kalan,
                'alert-type' => 'warning',
            ]);
        }
    }
