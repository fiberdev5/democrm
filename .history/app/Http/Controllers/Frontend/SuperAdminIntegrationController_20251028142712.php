...
class SuperAdminIntegrationController extends Controller
{
    public function AllIntegrations(Request $request) {
        ...
        if ($request->ajax()) {           
            $data = Integration::where('is_active', '1');
            ...
            ->addColumn('status', function($row){
                if($row->is_active){
                    return '<a class="t-link editIntegration" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editIntegrationModal"><div class="mobileTitle">Durum:</div><span style="color: green; font-weight:700;">Aktif</span></a>';
                }else{
                    return '<a class="t-link editIntegration" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editIntegrationModal"><div class="mobileTitle">Durum:</div><span style="color: red; font-weight:700;">Pasif</span></a>';
                }
            })
            ...
            ->addColumn('actions', function($row){
                $deleteUrl = route('super.admin.integrations.delete', $row->id);
            ...
            ->rawColumns(['id','name','logo','category','price','status','created_at','actions'])
            ->make(true);
        }

        return view('frontend.secure.super_admin.integrations.all_integrations', compact('integrations'));
    }
