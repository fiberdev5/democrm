...
{
    $konsinyeCevap = [];
    $planid = $request->input('planid'); // Ana plan ID'si (ServicePlanning ID)
    ...
    // Formdan gelen tüm soruları (soru[planId]) içeren inputları alıyoruz
    // Bu inputlar konsinye cihaz seçimini tetikleyen ana 'soru' inputlarıdır.
    $soruInputlari = $request->input('soru', []);

    foreach ($soruInputlari as $cevapPlanId => $cevapValue) {
        // Eğer bu soru bir konsinye cihaz sorusuysa ve "Konsinye Cihaz" değerine sahipse
        if ($cevapValue === 'Konsinye Cihaz') {
            // Bu spesifik cevap için gönderilen konsinye cihaz verilerini al
            $konsinyeCihazData = $request->input("konsinye_cihaz.{$cevapPlanId}", []);
            $konsinyeAdetData = $request->input("konsinye_adet.{$cevapPlanId}", []);

            // Loglama için
            // Log::info("İşlenen Soru Plan ID: {$cevapPlanId}");
            // Log::info("Gelen Konsinye Cihaz Data:", $konsinyeCihazData);
            // Log::info("Gelen Konsinye Adet Data:", $konsinyeAdetData);

            foreach ($konsinyeCihazData as $stokId => $value) {
                // $value burada aslında checkbox'ın value'su yani stokId'nin kendisi
                // Zaten foreach anahtarında ($stokId) doğru değeri alıyoruz.
                $adet = 1; 
                if (isset($konsinyeAdetData[$stokId])) {
                    $adet = abs($konsinyeAdetData[$stokId]);
                }

                $konsinyeCevap[] = "{$stokId}---{$adet}";

                try {
                    // $servisPlan->gelenIslem yerine, eğer her soru için farklı bir işlem durumu varsa
                    // bunu cevaba göre dinamik hale getirmemiz gerekebilir.
                    // Ancak mevcut yapıda, ana servis planının gelenIslem değeri kullanılıyor.
                    if ($servisPlan->gelenIslem == "238") {
                    ...
                    Log::error("Konsinye cihaz işlemi sırasında hata oluştu (processKonsinyeSelection): " . $e->getMessage(), [
                        'stokId' => $stokId,
                        'adet' => $adet,
                        'servisId' => $servisid,
                        'planId' => $planid, // Ana ServicePlanning ID'si
                        'cevapPlanId' => $cevapPlanId, // Bu cevabın ait olduğu ServiceStageAnswer ID'si
                        'tenantId' => $tenant_id
                    ]);
                    throw new \Exception("Konsinye cihaz işlemi tamamlanamadı: " . $e->getMessage());
                }
            }
        }
    }
...
private function konsinyeKullan($stokId, $adet, $servisId, $planId, $tenantId)
{
    ...
    // Fiyatı stoktan al veya varsayılan yap
    $fiyat = $stok->fiyat ?? 0;
    ...
    $stokHareketId = StockAction::insertGetId([
        ...
        'stokId' => $stokId,
        'islem' => 2,  // 2: Serviste Kullanım
        'servisid' => $servisId,
        'adet' => $adet,
        'fiyat' => $fiyat,
        'fiyatBirim' => 1, // Fiyat birimi (örnek)
        'planId' => $planId,
        'created_at' => now(),
        'updated_at' => now()
    ]);

    // İstersen buraya kasa ve servis para hareketlerini ekleyebilirsin
}
