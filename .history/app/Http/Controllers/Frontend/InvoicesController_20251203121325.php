...
class InvoicesController extends Controller
{
    ...
    if($hasParasutIntegration) {
        if($row->formalized) {
            ...
            // ✅ PDF varsa görüntüle butonu ekle
            if($row->formalization_status === 'sent' && !empty($row->formalization_pdf_url)) {
                $buttons .= '<a href="'.$row->formalization_pdf_url.'" target="_blank" class="btn btn-success btn-sm mobilBtn mbuton1" title="'.$typeText.' PDF - Görüntüle">
