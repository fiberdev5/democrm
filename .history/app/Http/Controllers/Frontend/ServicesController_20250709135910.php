...
class ServicesController extends Controller
{
    ...
    public function ServiceStageQuestionShow($tenant_id ,$asamaid, $serviceid) {
        ...
        // Normal servis işlemleri       
            ...
            $konsinyeCihazlar = Stock::withSum('stockActions as adet', 'adet')
...
private function stokKontrolEt(Request $request, $gelenIslem)
{
    foreach ($request->all() as $key => $value) {
        if (strpos($key, 'soru') !== false && $value == "Parca") {
            // "Parca" türündeki sorular için, formdan gelen 'parca' ve 'adet' dizilerini kontrol ediyoruz.
            // Blade şablonunda 'parca[stage_id][stok_id]' ve 'adet[stage_id][stok_id]' şeklinde isimler kullanılıyor.
        ...
        if (strpos($key, 'soru') !== false && $value == "Konsinye Cihaz") {
            // "Konsinye Cihaz" türündeki sorular için, formdan gelen 'konsinye_cihaz' ve 'konsinye_adet' dizilerini kontrol ediyoruz.
            // Blade şablonunda 'konsinye_cihaz[stage_id][konsinye_id]' ve 'konsinye_adet[stage_id][konsinye_id]' şeklinde isimler kullanılıyor.
...
private function useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId, $soruId)
{
    ...
    // StockAction kaydını bul veya oluştur/güncelle
    $stockAction = StockAction::updateOrCreate(
        [
            'firma_id' => $tenantId,
            'stokId' => $stokId,
            'planId' => $planId,
            'islem' => 2, // Konsinye cihaz kullanımı (çıkış işlemi)
            'servisid' => $servisId, // servisid de benzersizliği sağlamak için kullanılmalı
        ],
        [
            'adet' => $adet,
            'kid' => auth()->id(), // İşlemi yapan kullanıcı ID'si
            'depo' => 1, // Varsayılan depo
            // 'created_at' ve 'updated_at' updateOrCreate tarafından otomatik yönetilir
        ]
    );

    // ServiceStageAnswer kaydını bul veya oluştur/güncelle
    // Eğer aynı sorudan aynı plan için birden fazla konsinye cihaz seçilebiliyorsa,
    // 'cevap' alanını benzersizlik kriteri olarak kullanmamak daha doğru olabilir.
    // Ancak mevcut yapıda 'cevap' içinde stokId ve adet olduğu için,
    // her bir seçilen cihaz için ayrı bir cevap kaydı tutuluyor gibi görünüyor.
    // Bu durumda, stokId'yi de benzersizlik kriterine ekleyebiliriz.
    ServiceStageAnswer::updateOrCreate(
        [
            'firma_id' => $tenantId,
            'servisid' => $servisId,
            'planid' => $planId,
            'soruid' => $soruId,
            // Cevap formatı "stokId---adet" olduğu için, sadece stokId'yi benzersizlik için kullanmak daha güvenli.
            // Ancak bu, bir soruda aynı stokId'den birden fazla kez seçilemeyeceği varsayımına dayanır.
            // Eğer aynı soruda aynı cihazdan farklı adetlerde birden fazla giriş mümkünse,
            // bu kısım daha detaylı bir mantık gerektirebilir.
            // Şimdilik, mevcut cevap formatını koruyarak, aynı soruya aynı cihazın tekrar cevap olarak verilmesini engellemek için
            // cevap alanının bir kısmını (stokId) benzersizlik kriteri olarak kullanabiliriz.
            // Basitlik adına, mevcut 'cevap' formatını koruyarak, sadece planId ve soruid'e göre güncelleme yapalım.
            // Ancak bu, aynı soruya birden fazla konsinye cihaz seçildiğinde sadece sonuncusunun güncellenmesine neden olabilir.
            // Daha doğru bir yaklaşım için, ServiceStageAnswer tablosunda stokId için ayrı bir sütun tutmak daha iyi olur.
            // Mevcut yapıyı koruyarak, cevap alanının içinde stokId'yi arayarak güncelleme yapmak daha karmaşık olur.
            // Bu nedenle, ServiceStageAnswer için sadece planId ve soruId'ye göre güncelleme yaparken dikkatli olunmalı.
            // Eğer her bir seçilen konsinye cihaz için ayrı bir ServiceStageAnswer kaydı tutuluyorsa,
            // o zaman updateOrCreate yerine her zaman create yapılmalı ve eski kayıtlar silinmeli (ama bu sefer sadece ilgili cihazın cevabı).
            // Ancak bu, ilk sorunuzdaki "silme-yaratma" problemine geri döner.

            // En doğru yaklaşım, ServiceStageAnswer tablosuna 'stok_id' gibi ayrı bir sütun eklemektir.
            // Mevcut yapıyı bozmadan, sadece 'cevap' alanını güncelleyerek ilerleyelim:
            'cevap' => "{$stokId}---{$adet}", // Cevap alanının kendisi benzersizlik kriteri olarak kullanılamaz.
        ],
        [
            'kid' => auth()->user()->user_id,
            'created_at' => now(), // Bu alanın sadece ilk oluşturmada ayarlanması ve güncellemede değişmemesi isteniyorsa
            'updated_at' => now()
        ]
    );
    // Yukarıdaki ServiceStageAnswer::updateOrCreate çağrısı, eğer aynı planId ve soruId için birden fazla konsinye cihaz seçiliyorsa,
    // sadece son seçilen cihazın cevabını saklayacaktır. Bu nedenle, eğer her bir seçilen cihaz için ayrı ayrı cevaplar tutulmak isteniyorsa,
    // ServiceStageAnswer kaydının mantığı yeniden düşünülmelidir.
    // Ancak, stok hareketleri için updateOrCreate doğru çözümdür.
}
