...
{
    // 1. Kullanıcı ARAMA yapıyor mu kontrol et
    $hasSearch = $request->filled('search') && !empty($request->get('search')['value']);

      
    // Debug log
    \Log::info('DataTable Filtreleme:', [
        'has_search' => $hasSearch,
        'search_term' => $request->get('search')['value'] ?? null,
        'from_date' => $request->from_date,
        'to_date' => $request->to_date,
    ]);
    
    // 2. Ana daterangepicker'dan gelen tarih var mı?
    $hasMainDateRange = $request->filled('from_date') && $request->filled('to_date');

    // 3. İstatistik sayfalarından gelen tarih var mı?
    $hasStatsDateRange =
    ...
    // 4. Raporlama filtrelerinden gelen tarih var mı?
    $hasReportingFilter = $request->filled('filterType') && $request->filled('filters');

    // 5. Dashboard filtrelerinden gelen tarih var mı?
    ...
    // ✅ ÇÖZÜM: Eğer ARAMA yapılıyorsa ve başka tarih filtresi yoksa, tarih sınırı koyma
    if ($hasSearch && !$hasMainDateRange && !$hasStatsDateRange && !$hasReportingFilter && !$hasDashboardFilter) {
        // Arama yapılıyor, tarih sınırlaması yapma - tüm kayıtlarda ara
        return;
    }

    // Eğer hiçbir tarih/arama yoksa, varsayılan son 3 günü göster
    if (!$hasMainDateRange && !$hasStatsDateRange && !$hasReportingFilter && !$hasDashboardFilter && !$hasSearch) { 
