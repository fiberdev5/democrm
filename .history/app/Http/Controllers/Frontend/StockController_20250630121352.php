...
public function DeleteStockAction($tenant_id, $id)
{
    ...
    // 🔐 Bu alıştan sonra çıkış işlemi varsa ve adet bazlı yeterlilik yoksa silme!
    if ($stockAction->islem == 1) {
        $girisAdet = $stockAction->adet;

        $sonrakiCikisAdet = StockAction::where('stokId', $stockAction->stokId)
            ->where('firma_id', $firma->id)
            ->whereIn('islem', [2, 3]) // çıkış işlemleri
            ->where('created_at', '>', $stockAction->created_at) // alıştan sonra
            ->sum('adet');

        if ($sonrakiCikisAdet >= $girisAdet) {
            return redirect()->back()->with([
                'message'    => 'Bu alış silinemez çünkü sonrasında yapılan çıkışlar bu adet kadar veya daha fazladır.',
                'alert-type' => 'warning',
            ]);
        }
    }
    ...
    try {
        // Eğer personel gönderimi ise, personel stok kaydını da sil
