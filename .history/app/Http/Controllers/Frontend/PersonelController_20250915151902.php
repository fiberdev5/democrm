...
class PersonelController extends Controller
{
    ...
    public function StoreStaff(Request $request, $tenant_id) {
        ...
        // Aynı isimde personel kontrolü (sadece aktif olanları kontrol et)
        ...
        if ($limit !== null && $limit !== -1 && $current >= $limit) {
            return back()->with('error', 'Maksimum personel limitine ulaştınız.');
        }
        ...
        $roleName = null; // Bu satırı ekleyin
        if($request->roles){
    ...
    $existingUser = User::where('tenant_id', $firma->id)
        ->where('name', $request->name)
        ->where('user_id', '!=', $id) // Kendisi hariç
        ->where('status', 1)
        ->whereDoesntHave('roles', function ($query) {
            $query->where('name', 'Bayi');
        })
...
public function StoreDealer(Request $request, $tenant_id)
{
        ...
        // Vergi numarası kontrolü (bayiler için)
    ...
    $username = Str::slug($request->username, '-');
    // Firma ismini slug haline getir (klasör adı için uygun hale getir)
     $firmaSlug = $firma->firma_slug;
    // 1. Mevcut personel sayısı
        $current =User::where('tenant_id', $firma->id)->where('status','1')
        ->whereHas('roles', function ($query) {
            $query->where('name', 'Bayi');
        })->count();
        ...
        // 3. Limit dolmuş mu kontrol et
        if ($limit !== null && $limit !== -1 && $current >= $limit) {
            return back()->with('error', 'Maksimum bayi limitine ulaştınız.');
        }
   // Belge yükle (maksimum 2 adet)
    $belgePdfPaths = [];
    if ($request->hasFile('belgePdf')) {
        $files = array_slice($request->file('belgePdf'), 0, 2); // Max 2 dosya
        
        foreach ($files as $file) {
            $fileName = time() . '_' . $username . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            // ServicePhoto'daki gibi firma slug ile klasör yapısı
            $path = "dealers-documents/firma_{$firmaSlug}/bayi_{$username}/" . now()->toDateString();
            
            // Manuel klasör oluşturma (ServicePhoto'daki gibi)
...
public function UpdateDealer(Request $request, $tenant_id, $id)
{
    ...
    $bayi = User::where('tenant_id', $tenant_id)
                ...
                ->firstOrFail();
    //Güncelleme sırasında aynı isimde başka bayi var mı kontrolü
...
// Firma slug'ını kullan
    $firmaSlug = $firma->firma_slug;

    // Belge güncellemesi (maksimum 2 adet)
    if ($request->hasFile('belgePdf')) {
        // Mevcut belgeleri al
        $mevcutBelgeler = [];
        if ($bayi->belgePdf) {
        ...
        foreach ($files as $file) {
            $fileName = time() . '_' . Str::slug($request->username, '-') . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            
            // ServicePhoto'daki gibi klasör yapısı
            $path = "dealers-documents/firma_{$firmaSlug}/bayi_" . Str::slug($request->username, '-') . "/" . now()->toDateString();
            
            // Manuel klasör oluşturma
            $fullPath = storage_path('app/public/' . $path);
            if (!file_exists($fullPath)) {
                mkdir($fullPath, 0775, true);
            }
        ...
        // Toplam belge sayısını kontrol et (mevcut + yeni)
        $toplamBelgeSayisi = count($mevcutBelgeler) + count($yeniBelgeler);
        
        if ($toplamBelgeSayisi > 2) {
            // Eğer toplam 2'den fazla ise, en eski belgeleri sil
            $silinecekSayi = $toplamBelgeSayisi - 2;
            
            for ($i = 0; $i < $silinecekSayi; $i++) {
                if (isset($mevcutBelgeler[$i]) && Storage::disk('local')->exists($mevcutBelgeler[$i])) {
        ...
        // Mevcut belgeler + yeni belgeler birleştir
        $tumBelgeler = array_merge($mevcutBelgeler, $yeniBelgeler);
        
        // Maksimum 2 belge tutmak için sınırla
