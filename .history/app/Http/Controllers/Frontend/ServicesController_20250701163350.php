...
// Personel stoğundan düş
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        foreach ($planCevaplar as $planCevap) {
            // Her bir cevabın ilgili sorusunu bulalım
            $soru = StageQuestion::find($planCevap->soruid);

            // Eğer soru tipi '[Parca]' ise ve cevabı varsa
            if ($soru && $soru->cevapTuru == "[Parca]" && !empty($planCevap->cevap)) {
                // Cevap JSON formatında olmalı (örneğin: {"1":2,"5":1} )
                $partsData = json_decode($planCevap->cevap, true);

                if (is_array($partsData)) {
                    // Bu $planCevapId'ye ait tüm parçaları ve miktarlarını kaydedelim
                    // '$selectedParts' dizisi, her bir 'planCevap->id' için
                    // seçilen parçaları ve miktarlarını tutacak şekilde düzenleniyor.
                    // Bu sayede, view'da doğru input'lara doğru değerleri basabiliriz.
                    $selectedParts[$planCevap->id] = $partsData;
                }
            }
        }
        ...
        // $personel_id = auth()->user()->user_id;
         $stoklar = Stock::where('firma_id', $tenant_id)
                         ->where('status', 1) // Sadece aktif stokları çekmek için ekledik (varsa)
                         ->orderBy('urunAdi', 'ASC') // urunAdi veya urun_adi gibi alanınıza göre sıralayın
                         ->get();
 ...
 public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        DB::beginTransaction(); // İşlemlerin tutarlılığı için transaction başlattık
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                $soruKey = 'soru' . $cevap->id; // Bu, blade'deki hidden input name="soru{{ $plan->id }}" ile eşleşiyor
                
                // Cevabın ait olduğu StageQuestion'ı bulalım
                $soru = StageQuestion::find($cevap->soruid);
                
                // Eğer StageQuestion bulunamadıysa veya cevapTuru '[Parca]' değilse
                if ($soru && $soru->cevapTuru == "[Parca]") {
                    // Bu bir parça cevabı ise, özel olarak işleyeceğiz
                    $selectedPartsForThisAnswer = []; 
                    
                    // Frontend'den gelen 'stokCheck[soru_id][stok_id]' ve 'stokAdet[soru_id][stok_id]' formatındaki veriyi al
                    // Blade'deki input name'leri:
                    // name="stokCheck[{{ $soru->id }}][{{ $stokId }}]"
                    // name="stokAdet[{{ $soru->id }}][{{ $stokId }}]"
                    // Bu yüzden $soru->id'yi kullanarak verilere erişiyoruz.
                    
                    $checkboxes = $request->input("stokCheck.{$soru->id}", []); // $soru->id'ye ait seçili checkbox'lar
                    $quantities = $request->input("stokAdet.{$soru->id}", []); // $soru->id'ye ait miktarlar

                    foreach ($checkboxes as $stokId => $checked) {
                        if ($checked === 'on') { // Eğer checkbox işaretliyse
                            $quantity = (int)($quantities[$stokId] ?? 1); // Miktarı al, yoksa varsayılan 1
                            $selectedPartsForThisAnswer[$stokId] = $quantity;
                        }
                    }
                    
                    // Seçilen parçaları JSON olarak kaydet
                    $cevap->cevap = json_encode($selectedPartsForThisAnswer);

                } elseif ($request->has($soruKey)) {
                    // Diğer soru tipleri için doğrudan gelen değeri kaydet
                    $cevap->cevap = $request->input($soruKey);
                }
                $cevap->save(); // Cevabı veritabanına kaydet
            }

            // Eski bayi bilgisi varsa ve yeni bayi atanmışsa güncelleyelim
            // (Bu kısım mevcut kodunuzda vardı, eğer ihtiyacınız yoksa kaldırabilirsiniz)
            // $eskiBayi = $request->input('eskiBayi');
            // if (!empty($eskiBayi) && $request->has('soruXX')) { // 'soruXX' kısmı doğru soru key'i olmalı
            //     // Eski bayi ile ilgili ek işlemler
            // }
            ...
            return response()->json([
                ...
                'servis_id' => $servisPlan->servisid // Frontend'deki loadServiceHistory için gerekli
            ]);

        } catch (\Exception $e) {
            DB::rollBack(); // Herhangi bir hata oluşursa işlemi geri al
            return response()->json([
                'success' => false,
