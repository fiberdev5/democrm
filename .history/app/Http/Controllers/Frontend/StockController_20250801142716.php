...
public function AllStocks($tenant_id, Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        if ($request->has('order')) {
            ...
            $query->orderBy('stocks.id', 'desc'); // Tablonun prefix'ini belirtin
        }

        // Grup by ekleyin (personel filtresi join yaptığında duplicate kayıtları önlemek için)
        if ($request->filled('personel')) {
            $query->groupBy('stocks.id');
        }
            ...
            ->filter(function ($query) use ($request) {
                if ($search = $request->get('search')['value'] ?? null) {
                    $query->where(function ($q) use ($search) {
                        $q->where('stocks.urunAdi', 'like', "%{$search}%")
                          ->orWhere('stocks.urunKodu', 'like', "%{$search}%");
                    });
                }
            })
