                ...
                switch ($request->filterType) {
                    case 'operator':
                        ...
                        if (!empty($filters['operator_pers']) && $filters['operator_pers'] != '0') {
                            $data->where('kid', $filters['operator_pers']);
                        }
                    ...
                    case 'teknisyen':
                        ...
                        $data->whereHas('cevaplar', function ($query) use ($filters) {

                            if (!empty($filters['teknisyen']) && $filters['teknisyen'] != '0') {
                                // 47: Teknisyen ID'sinin tutulduğu service_stage_question_id
                                $query->where('soruid', 47)
                                    ->where('cevap', $filters['teknisyen']);
                            }

                            if (!empty($filters['yarPersoneller']) && $filters['yarPersoneller'] != '0') {
                                // 46: Yardımcı teknisyen ID
                                $query->where('soruid', 46)
                                    ->where('cevap', $filters['yarPersoneller']);
                            }

                            if (!empty($filters['tekArac']) && $filters['tekArac'] != '0') {
                                // 48: Araç ID'si
                                $query->where('soruid', 48)
                                    ->where('cevap', $filters['tekArac']);
                            }

                            if (!empty($filters['tekTarih'])) {
                                // 49: Tarih sorusu
                                $query->where('soruid', 49)
                                    ->where('cevap', $filters['tekTarih']);
                            }

                            // Not: Eğer saat gibi alan da varsa benzer şekilde eklenebilir.
                        });
                    break;
                }
            }
