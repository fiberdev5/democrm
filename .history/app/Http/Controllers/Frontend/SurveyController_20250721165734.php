        ...
        try {
            ...
            $anket->personel = $personelIdToAssign ?? 0; 
             if ($personelIdToAssign) {
                ...
                if ($anketYapilanUser) {
                    // Bayi rolünü ID'si ile değil, doğrudan adı ile kontrol etmek daha güvenli olabilir
                    // Örneğin: 'Bayi' rolünün adı 'Bayi' ise
                    $bayiRole = Role::where('name', 'Bayi')->first(); 
                    // Ya da Role ID'niz sabit ise: $bayiRole = Role::find(259);
                    
                    if ($bayiRole && $anketYapilanUser->hasRole($bayiRole->name)) { // hasRole'a rol objesi yerine rol ismi de verebilirsin
                        $anket->bayi = $personelIdToAssign;
                    } else {
