...
public function AllStocks($tenant_id, Request $request) {
    // Tenant ve kullanıcı kontrolü isteğe bağlı eklenebilir
    
    $firma = Tenant::findOrFail($tenant_id);

    if ($request->ajax()) {
        $data = Stock::with(['raf', 'marka', 'cihaz', 'sonHareket'])
            ->where('firma_id', $tenant_id);

        // Durum, grup veya diğer filtrelerin yerine stok filtreleri:
        ...
        if ($request->filled('personel')) {
            $data->whereHas('stockActions', function($q) use ($request) {
                $q->where('personel', $request->personel);
            });
        }

        // Global arama (search) kontrolü
        if (!empty($request->get('search')['value'])) {
            $search = $request->get('search')['value'];
            $data->where(function($q) use ($search) {
                $q->where('urunAdi', 'like', "%$search%")
                  ->orWhere('urunKodu', 'like', "%$search%")
                  ->orWhereHas('marka', function ($q2) use ($search) {
                      $q2->where('marka', 'like', "%$search%");
                  })
                  ->orWhereHas('cihaz', function ($q3) use ($search) {
                      $q3->where('cihaz', 'like', "%$search%");
                  })
                  ->orWhereHas('raf', function ($q4) use ($search) {
                      $q4->where('raf_adi', 'like', "%$search%");
                  });
            });
        }
        ...
        if ($request->has('order')) {
            ...
            $orderColumn = $columns[$order['column']]['data'] ?? 'id';
        ...
        // Toplam hesaplamalar için tüm veriyi çekiyoruz
        $stocks = $data->get();
       ...
       return DataTables::of($data)
            ...
            ->rawColumns(['urunKodu','urunAdi','adet','toplamTutar','action'])
            ->with([
