...
class ImpersonationController extends Controller
{
    public function start(Request $request, $userId)
    {
        ...
        // ÖNEMLİ: Auth kullanıcısını DİREKT değiştir
        \DB::table('sessions')
            ...
            ->update([
                ...
                'payload' => base64_encode(serialize([
                    ...
                    'impersonation_id' => $impersonation->id,
                    'original_user_tenant_id' => $currentUser->tenant_id,
                    'login_web_' . sha1('web') => $targetUser->user_id
                ]))
            ]);

        // Auth state'ini force update
    ...
    public function stop(Request $request)
    {
        ...
        // Session'daki user_id'yi güncelle
        \DB::table('sessions')
            ->where('id', session()->getId())
            ->update([
                ...
                'payload' => base64_encode(serialize([
                    '_token' => csrf_token(),
                    'login_web_' . sha1('web') => $originalUser->user_id
                ]))
            ]);
