...
class ServicesController extends Controller
{
    ...
    public function ServiceStageQuestionShow($tenant_id ,$asamaid, $serviceid) {
        ...
        $yonlendirilenPersonelId = $servisSec->pid; // Servisi alan teknisyen

        // Stokları getir
        if (auth()->user()->hasRole('Patron')) {
            // Patron: sadece yönlendirilen teknisyenin stoklarını görür
            $stoklar = PersonelStock::where('firma_id', $firma->id)
                        ->where('pid', $yonlendirilenPersonelId)
                        ->orderBy('id', 'asc')
                        ->get();
        } else {
            // Diğer kullanıcılar kendi stoklarını görür
            $stoklar = PersonelStock::where('firma_id', $firma->id)
