   ...
   public function paymentCallback(Request $request)
{
    try {
        ...
        if (!isset($postData['merchant_oid']) || !isset($postData['status']) || !isset($postData['hash'])) {
            Log::error('Missing required callback fields', $postData);
        ...
        if ($postData['status'] == 'success') {
            ...
            // Session verilerinin varlığını kontrol et (callback'te session olmayabilir)
            $billingData = session('subscription.billing');
            $planData = session('subscription.plan');

            if (!$billingData || !$planData) {
                Log::warning('Session data not available in callback, trying to reconstruct from payment');
                
                // Session verileri yoksa payment'tan yeniden oluştur
                $this->reconstructSessionFromPayment($payment);
            ...
            if ($billingData && $planData) {
                ...
                Log::info('Starting subscription completion from callback');
                $subscription = $this->completeSubscription($payment);
                
                if ($subscription) {
                    Log::info('Subscription completed successfully from callback', [
                        'subscription_id' => $subscription->id,
                        'tenant_id' => $payment->tenant_id
                    ]);
                } else {
                    Log::error('Subscription completion returned null');
                }
            } else {
                Log::error('Cannot complete subscription: session data still missing after reconstruction');
            }
...
/**
 * Payment kaydından session verilerini yeniden oluştur
 */
private function reconstructSessionFromPayment(SubscriptionPayment $payment)
{
    try {
        Log::info('Reconstructing session from payment', ['payment_id' => $payment->id]);

        $gatewayResponse = json_decode($payment->gateway_response, true);
        
        if (isset($gatewayResponse['order_data'])) {
            $orderData = $gatewayResponse['order_data'];
            
            // Tenant'ı bul
            $tenant = Tenant::find($payment->tenant_id);
            if (!$tenant) {
                throw new \Exception('Tenant not found: ' . $payment->tenant_id);
            }

            // Plan'ı bul - order_data'dan çıkarabilir veya payment amount'tan tahmin edebiliriz
            // Önce active planları kontrol et
            $plans = SubscriptionPlan::active()->get();
            $matchingPlan = null;
            
            foreach ($plans as $plan) {
                $planTotalWithTax = $plan->price * 1.20;
                if (abs($planTotalWithTax - $payment->amount) < 0.01) {
                    $matchingPlan = $plan;
                    break;
                }
            }
            ...
            if (!$matchingPlan) {
                throw new \Exception('Cannot determine plan from payment amount: ' . $payment->amount);
            }

            // Minimal billing data oluştur
            $billingData = [
                'billing_type' => 'bireysel',
                'first_name' => $orderData['user_name'] ?? 'Unknown',
                'last_name' => '',
            ...
            session([
                'subscription.plan' => $matchingPlan->toArray(),
                'subscription.billing' => $billingData,
                'subscription.tenant_id' => $payment->tenant_id,
                'subscription.planid' => $matchingPlan->id,
                'subscription.payment_id' => $payment->id
            ]);

            Log::info('Session reconstructed successfully', [
                'plan_id' => $matchingPlan->id,
                'tenant_id' => $payment->tenant_id
            ]);

        } else {
            throw new \Exception('No order_data found in gateway_response');
        }

    } catch (\Exception $e) {
        Log::error('Session reconstruction failed', [
            'error' => $e->getMessage(),
            'payment_id' => $payment->id
        ]);
    }
    ...
    if ($paymentId) {
        ...
        if ($payment && $payment->status == 'completed') {
            session()->forget('subscription');
                ...
                return redirect()->route('secure.home', $tenantId)
