...
class StockController extends Controller
{
     ...
     public function AllStocks($tenant_id, Request $request){
                    if ($request->ajax()) {
                        $query = Stock::with(['raf', 'marka', 'cihaz','haraketler'])
...
foreach ($stocks as $stock) {
    $giris = $stock->hareketler->where('islem', 1)->sum('adet');
    $cikis = $stock->hareketler->where('islem', 0)->sum('adet');
    $adet = $giris - $cikis;
    $fiyatBirim = $stock->fiyatBirim ?? 0;
...
->addColumn('adet', function ($stock) {
    $giris = $stock->hareketler->where('islem', 1)->sum('adet'); // giriş
    $cikis = $stock->hareketler->where('islem', 0)->sum('adet'); // çıkış
    return $giris - $cikis;
})
...
->addColumn('toplamTutar', function ($stock) {
    ...
    if ($adet > 0 && $stock->fiyatBirim) {
        return number_format($adet * $stock->fiyatBirim, 2) . ' ₺';
    }
    return '-';
})
