...
{
    try {
        // Teknisyenleri al (role_id = 2 teknisyen)
...
{
    // Servis aşama cevaplarından tarihleri al
    ...
    // sc.cevap'ın date formatında olmasını bekliyoruz.
    // Eğer sc.cevap D/M/Y formatında saklanıyorsa ve $tarihListesi Y-m-d ise bu sorun yaratır.
    // Bu durumda sc.cevap'ı da Y-m-d'ye dönüştürmek gerekebilir.
    // Ancak en doğrusu, getDatesFromRange'in döndürdüğü format ile sc.cevap'ın formatının eşleşmesini sağlamaktır.
    // Diyelim ki sc.cevap 'DD/MM/YYYY' formatında saklanıyor. O zaman tarihListesi de o formatta olmalı.
    
    // Eğer $tarihListesi Y-m-d formatında ise ve sc.cevap D/M/Y ise, aşağıdaki dönüşümü yapmalısınız
    // ->whereRaw("STR_TO_DATE(sc.cevap, '%d/%m/%Y') IN (?)", [array_map(function($date) { return Carbon::createFromFormat('Y-m-d', $date)->format('d/m/Y'); }, $tarihListesi)])
    // Ya da en basitinden sc.cevap'ı da Y-m-d formatında alıp whereIn kullanın.

    $query->whereIn('sc.cevap', $tarihListesi); // Bu kısım eğer formatlar uyuşmuyorsa hatalı olacaktır.
...
private function getTechnicianServices($teknisyenId, $servisPlanlar)
{
    ...
    // Teknisyenin servislerini getir (Grup tipi sorular ve teknisyen ID cevabı eşleşiyor)
    $servisler = DB::table('service_stage_answers as sc')
        ->leftJoin('stage_questions as ss', 'ss.id', '=', 'sc.soruid')
        ->where('sc.cevap', $teknisyenId) // Teknisyen ID eşleşiyor
        ->where('ss.cevapTuru', 'LIKE', '%Grup%') // Cevap türü 'Grup' olanlar (örneğin: "Teknik Grup", "Yetkili Grup" vb.)
        ->whereIn('sc.planid', $servisPlanlar) // İlgili planlara ait servisler
        ->pluck('sc.servisid') // Yalnızca servisid değerleri
        ->unique()
        ->toArray();
...
private function calculateComplaintServices($teknisyenId, $servisler)
{
    ...
    foreach ($sikayetciler as $servis) {
        ...
        if ($ilkServis && $sonSikayet) {
            $ilkTarihCarbon = Carbon::parse($ilkServis->tarih);
            $sikayetTarihCarbon = Carbon::parse($sonSikayet->tarih);
            
            // Şikayet tarihi, ilk servisin tarihinden sonra ise say
            if ($sikayetTarihCarbon->greaterThan($ilkTarihCarbon)) {
                $sikayetciSay++;
            }
        }
    }
...
{
    // Kasa hareketlerinden alınan ücreti hesapla
    $paraToplam = DB::table('kasa_hareketleri')
...
{
    // Tarih aralığını Y-m-d formatında alalım
    $tarihBaslangic = Carbon::createFromFormat('Y-m-d', $tarih1)->startOfDay();
    $tarihBitis = Carbon::createFromFormat('Y-m-d', $tarih2)->endOfDay();
    
    $teklifToplam = DB::table('service_stage_answers as sc')
        ->leftJoin('service_planning as sp', 'sp.planid', '=', 'sc.planid') // planid üzerinden join yapılmalı
        ->whereIn('sc.soruid', [350, 351, 352, 353, 354, 355, 356])
        ->whereIn('sc.servisid', $servisler)
        ->where('sp.pid', $teknisyenId)
        // Buradaki tarih karşılaştırmasını düzenle
        ->whereBetween('sp.tarih', [$tarihBaslangic, $tarihBitis])
        ->sum('sc.cevap');
