...
class ServiceBatchPlanningController extends Controller
{
     ...
     public function getServicePlanForm(Request $request, $tenant_id)
    {
        ...
        if (empty($servisIds)) {
            return response('-1', 400); // Bad Request
        }
        ...
        foreach ($idList as $serviceId) {
            ...
            if (!$service) {
                return response('-1', 403); // Forbidden
            }
        }
        ...
        $questions = StageQuestion::where('asama', $gidenDurum)
                                        ->where('firma_id', $tenant_id) // Soru modelinizde firma_id var mı? Yoksa ekleyin veya kaldırın.
                                        ->orderBy('sira', 'ASC')
                                        ->get();
        ...
        if ($defaultDate->dayOfWeek === Carbon::SATURDAY) { // Cumartesi ise +2 gün
            $defaultDate->addDays(2);
        } else { // Diğer günler +1 gün
            $defaultDate->addDay();
        }
        $defaultDateFormatted = $defaultDate->format('d/m/Y');
