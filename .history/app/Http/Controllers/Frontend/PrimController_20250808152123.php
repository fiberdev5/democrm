...
class PrimController extends Controller
{
    ...
    private function operatorPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        foreach ($gunler as $gun) {
            ...
            $validServisler = DB::table('service_plannings as sp')
            ->join('services as s', 's.id', '=', 'sp.servisid')
            ->select('sp.servisid', 'sp.created_at', 'sp.gidenIslem', 's.kayitAlan', 's.id')
            ->where('s.kayitAlan', $personelId)
            ->where('sp.gidenIslem', 255) // Biten servisler
            ->whereBetween('sp.created_at', [$gunBaslangic, $gunBitis])
            ->whereNotIn('sp.servisid', function ($query) use ($gunBaslangic, $gunBitis) {
                // O gün içinde iptal (244) kaydı olan servislerin ID'lerini hariç tut
                $query->select('servisid')
                    ->from('service_plannings')
                    ->where('gidenIslem', 244)
                    ->whereBetween('created_at', [$gunBaslangic, $gunBitis]);
            })
            ->distinct() // Aynı servisid birden fazla gelmesin
            ->get();
            

            $gunlukServisSayisi = count($validServisler);
        ...
        foreach ($gunler as $gun) {
            ...
            if ($gunlukTeklifToplami >= $gunlukSinir) {
                ...
                $primSonuclari[] = [
                    ...
                    'prim_orani' => $primOrani,
                    'gunluk_sinir' => $gunlukSinir,
                    'servis_sayisi' => count($servisIdler)
                ];
            }
        }
    ...
    private function atolyeUstasiPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        foreach ($gunler as $gun) {
            ...
            // O gün teslimata hazır işlemini seçenler
            ...
            $gunlukTamamlanmaSayisi = count($gunlukTamamlananlar);

            // Günlük sınırı aştı mı?
            if ($gunlukTamamlanmaSayisi >= $gunlukSinir) {
                $primTutari = ($gunlukTamamlanmaSayisi * $primOrani) / 100;
    ...
    public function index($tenant_id)
    {
        $personeller = User::where('tenant_id', $tenant_id)
            ...
            ->whereDoesntHave('roles', function ($query) {
                $query->whereIn('name', ['Bayi', 'Depocu', 'Patron']);
            })
            ->orderBy('name')
            ->get();
    ...
    public function hesapla(Request $request)
    {
        ...
        try {
            $data = $request->only(['personel_id', 'tarih1', 'tarih2']);
     ...
     */
    public function ayarlar($tenant_id)
    {
        $primAyarlari = DB::table('tenant_prims')
            ->where('firma_id', $tenant_id)
            ->get();
        ...
        return view('frontend.secure.prim.ayarlar', compact('primAyarlari', 'firma'));
    }

    /**
     * Prim ayarlarını kaydet
     */
    public function ayarlariKaydet(Request $request, $tenant_id)
    {

        try {
            DB::table('tenant_prims')->updateOrInsert(
                [
                    'firma_id' => $tenant_id,
                ],
                [
            ...
            return response()->json([
                'success' => true,
                'message' => 'Prim ayarları başarıyla kaydedildi.'
            ]);

        } catch (\Exception $e) {
            \Log::error('Prim ayarları kaydetme hatası: ' . $e->getMessage());

            return response()->json([
                'success' => false,
                'message' => 'Prim ayarları kaydedilirken hata oluştu.'
            ], 500);
        }
    }
