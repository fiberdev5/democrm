...
public function getSurveyStatisticsData(Request $request, $tenant_id)
{
   ...
   $completedServicesQuery = ServicePlanning::select('service_plannings.servisid')
    ->where('service_plannings.gidenIslem', 255)
    ->whereBetween('service_plannings.updated_at', [$startDateTime, $endDateTime])
    ->where('service_plannings.firma_id', $tenant_id);

if ($deviceTypeId) {
    $completedServicesQuery->join('services', 'service_plannings.servisid', '=', 'services.id')
        ->where('services.cihazTur', $deviceTypeId);
}

$completedServices = array_unique(
    $completedServicesQuery->pluck('service_plannings.servisid')->toArray()
);
