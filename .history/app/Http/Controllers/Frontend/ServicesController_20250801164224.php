...
$kullanilanKonsinyeArray = [];
 ...
 if (($cevap->soru === 'Konsinye Cihaz') && !empty($cevap->cevap) && str_contains($cevap->cevap, '---')) {
    if (!empty($cevap->cevap) && str_contains($cevap->cevap, '---')) {
        $satirlar = explode(',', $cevap->cevap);

        foreach ($satirlar as $satir) {
            $parcalar = explode('---', trim($satir));
            if (count($parcalar) === 2) {
                $stokId = (int) $parcalar[0];
                $adet = (int) $parcalar[1];
                $stok = Stock::find($stokId);

                if ($stok) {
                    $kullanilanKonsinyeArray[] = $stok->urunAdi . " ({$adet} adet)";
                }
            }
        }
    }
