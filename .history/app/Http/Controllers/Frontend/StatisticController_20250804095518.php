...
///////////////////////////////////////////////////////Technician Statistics///////////////////////////////////////////////////////////////////
 /**
     ...
     */
public function calculateTechnicianStats($userId, $dateRange, $deviceType, $tenantId)
{
    // Kullanıcı bilgisi
    ...
    // Teknisyenin ait olduğu grup ID'leri
    $groupIds = DB::table('role_group_users')
    ...
    // Grup etiketlerini oluştur (örn: "[Grup-5]")
    $groupTags = array_map(function ($id) {
        return '[Grup-' . $id . ']';
    }, $groupIds);
    ...
    // Grup etiketine sahip servis cevaplarından servisID'leri al
    $serviceIds = ServiceStageAnswer::join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ...
        ->where(function ($query) use ($groupTags) {
            foreach ($groupTags as $tag) {
                $query->orWhere('service_stage_answers.cevap', 'like', '%' . $tag . '%');
            }
        })
        ->pluck('service_stage_answers.servisid');

    // İstatistikleri topla
    $stats = ServicePlanning::whereIn('id', $serviceIds)
        ->selectRaw('
            COUNT(*) as toplam,
            SUM(CASE WHEN durum = "Tamamlandı" THEN 1 ELSE 0 END) as tamamlanan,
            SUM(CASE WHEN durum != "Tamamlandı" THEN 1 ELSE 0 END) as tamamlanmayan
        ')
        ->first();

    return [
        'toplam' => $stats->toplam ?? 0,
        'tamamlanan' => $stats->tamamlanan ?? 0,
        'tamamlanmayan' => $stats->tamamlanmayan ?? 0,
    ];
}
