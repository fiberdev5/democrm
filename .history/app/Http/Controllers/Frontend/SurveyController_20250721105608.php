...
{
    $servisTeknisyen = Service::with('Teknisyen')->findOrFail($servisId);
    ...
    return view('frontend.secure.surveys.survey_form', [
        ...
        'personeller' => $personeller,
         'servisTeknisyen' => $servis->teknisyen, 
    ]);
}
    ...
    public function SurveyStore(Request $request, $tenant_id, $servisId)
    {
        ...
        try {
            ...
            if ($ekleyenUser->roles()->where('id', 259)->exists()) {
                // Ekleyen bayi, anket personel alanına personel user_id atanabilir (isteğe bağlı)
                // Örneğin personel alanı boş bırakılabilir veya $request'ten alınabilir
                $anket->personel = $request->input('personel') ?? null;
                // ekleyen bayi olarak kalır
            } else {
                // Ekleyen personel ise anket personel alanı ekleyen kişi olabilir veya istenirse $request'ten
                   $anket->personel = $request->input('personel') ?? $servis->personel ?? Auth::id();
            }
