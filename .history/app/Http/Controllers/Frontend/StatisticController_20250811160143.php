...
public function CashStatistics()
{
    ...
    $kasaHareketleri = CashTransaction::whereBetween('created_at', [$startDate, $endDate])
        ->where('odemeYonu', '1')
        ->get();
    ...
    foreach ($odemeTurleri as $odemeTuru) {
        $gider = CashTransaction::whereBetween('created_at', [$startDate, $endDate])
            ->where('odemeYonu', '2')
...
public function giderTabloGetir(Request $request)
{
    ...
    foreach ($odemeTuruAll as $key => $value) {
         $odemeTurSec = PaymentType::where('odemeTuru', $key)->first();
