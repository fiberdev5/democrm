  ...
  public function AllStocks($tenant_id, Request $request)
    {
        ...
        if ($user->tenant->id != $tenant_id) {
            return redirect()->route('giris')->with('message', 'Yetkisiz erişim.');
        }

        $firma = Tenant::findOrFail($tenant_id);

        if ($request->ajax()) {
            $data = Stock::where('firma_id', $firma->id);

            if ($request->filled('raf')) {
                $data->where('raf', $request->raf);
            }
            if ($request->filled('marka')) {
                $data->where('marka', $request->marka);
            }
            if ($request->filled('cihaz')) {
                $data->where('cihaz', $request->cihaz);
            }
            if ($request->filled('personel')) {
                $data->where('personel', $request->personel);
            }

            // Arama filtresi
            $data->filter(function ($query) use ($request) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('marka_cihaz', function($row){
                    return $row->marka.' / '.$row->cihaz;
                })
                ->addColumn('raf_adi', function($row){
                    return $row->raf ?? '-';
                })
                ->addColumn('toplamTutar', function($row){
                    return number_format($row->adet * $row->fiyat, 2, ',', '.').' TL';
                })
                ->addColumn('action', function($row){
                    $edit = '<a href="javascript:void(0);" class="btn btn-warning btn-sm editStock" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editStockModal"><i class="fas fa-edit"></i></a>';
                    $delete = '<a href="'.route('delete.stock', [$row->firma_id, $row->id]).'" class="btn btn-danger btn-sm" id="delete"><i class="fas fa-trash-alt"></i></a>';
                    return $edit . ' ' . $delete;
                })
                ->rawColumns(['action'])
                ->with([
                    'toplamAdet' => $data->sum('adet'),
                    'toplamFiyat' => number_format($data->sum(function($r){ return $r->adet * $r->fiyat; }), 2, ',', '.').' TL'
                ])
                ->make(true);
        }

        $rafListesi = \App\Models\Raf::where('firma_id', $firma->id)->get();
        $markalar = \App\Models\Marka::where('firma_id', $firma->id)->get();
        $cihazlar = \App\Models\Cihaz::where('firma_id', $firma->id)->get();
        $personeller = \App\Models\User::where('tenant_id', $firma->id)->get();

        return view('frontend.secure.stocks.all_stocks', compact('firma', 'rafListesi', 'markalar', 'cihazlar', 'personeller'));
    }
