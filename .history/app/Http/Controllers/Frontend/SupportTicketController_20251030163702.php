    ...
    public function store(Request $request, $tenant_id)
    {
        ...
        if (empty($token)) {
            return redirect()->back()
                ->with('error', 'Geçersiz form token! Lütfen sayfayı yenileyin.')
                ->withInput();
        }
        // Bu token daha önce kullanıldı mı kontrol et
        $cacheKey = 'supportticket_form_token_' . $token;
        
        if (Cache::has($cacheKey)) {
            return redirect()->back()
                ->with('error', 'Bu form zaten gönderildi! Lütfen bekleyin veya sayfayı yenileyin.')
                ->withInput();
        }
