...
if ($request->ajax()) {
    ...
    \Log::info("Query build time: " . ($queryTime * 1000) . "ms");

    // Yetki kontrolü
    $yetkiStart = microtime(true);
    if ($user = auth()->user()) {
        if ($user->can('Kendi Servislerini Görebilir')) {
            $servisIDleri = $this->getYetkiliServisIDleri($user, $firma->id);
            \Log::info("Yetkili servis sayısı: " . count($servisIDleri));
            $data->whereIn('id', $servisIDleri);
        }
    }
    $yetkiTime = microtime(true) - $yetkiStart;
    \Log::info("Yetki kontrolü: " . ($yetkiTime * 1000) . "ms");

    // Filtreler
    $filtreStart = microtime(true);
    ...
    $filtreTime = microtime(true) - $filtreStart;
    \Log::info("Filtreler: " . ($filtreTime * 1000) . "ms");

    // DataTable oluşturma
    $datatableStart = microtime(true);
    ...
    $datatableTime = microtime(true) - $datatableStart;
    \Log::info("DataTable render: " . ($datatableTime * 1000) . "ms");
    
    // Toplam
    $queries = \DB::getQueryLog();
    $totalTime = microtime(true) - $startTime;
    
    \Log::info("===== SERVIS SAYFASI ÖZET =====");
    \Log::info("Toplam Sorgu Sayısı: " . count($queries));
    \Log::info("Toplam Süre: " . ($totalTime * 1000) . "ms");
    
    // En yavaş 5 sorguyu logla
    usort($queries, function($a, $b) {
        return $b['time'] <=> $a['time'];
    });
    
    \Log::info("=== EN YAVAŞ 5 SORGU ===");
    foreach (array_slice($queries, 0, 5) as $index => $query) {
        \Log::warning("Yavaş Sorgu #" . ($index + 1) . ": " . $query['time'] . "ms");
        \Log::warning("SQL: " . $query['query']);
