    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($soru->cevapTuru == '[Parca]') {
                // Mevcut parça güncelleme mantığınız
                // Frontend'den gelen 'parca[soruId][personel_stock]' değerini kullanın.
                // Bu değer zaten "ID---Adet, ID---Adet" formatında gelecektir.
                $selectedPartsString = $request->input('parca')[$cevap->id]['personel_stock'] ?? null;
                $cevap->cevap = $selectedPartsString;
                // updateParcaSelection metodunuzun bu stringi doğru şekilde işlemesi veya kaldırılması gerekebilir.
                // Eğer updateParcaSelection stok hareketlerini yönetiyorsa ve $selectedPartsString'i işliyorsa bırakın,
                // aksi halde doğrudan $cevap->cevap'a atama yapın.
                $this->updateParcaSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
            } elseif ($soru->cevapTuru == '[Konsinye Cihaz]') {
    // Frontend'den gelen 'consignment_device_X' ve 'consignment_quantity_X' gibi input isimlerini kullanıyoruz.
    $consignmentDeviceId = null;
    $consignmentQuantity = null;
    $consignmentDeviceName = null;

    // Frontend'den gönderilen hidden input'un adını kullanıyoruz: "soru" . $plan->id
    $combinedConsignmentValue = $request->input('soru' . $cevap->id);
    
    // Cihaz adını al
    $parcaData = $request->input('parca.' . $cevap->id, []);
    $consignmentDeviceName = $parcaData['consignment_name'] ?? null;

    if ($combinedConsignmentValue && str_starts_with($combinedConsignmentValue, 'C_')) {
        $parts = explode('---', substr($combinedConsignmentValue, 2));
        if (count($parts) === 2) {
            $consignmentDeviceId = $parts[0];
            $consignmentQuantity = $parts[1];
        }
    }

    if ($consignmentDeviceId && $consignmentQuantity !== null && $consignmentDeviceName) {
        // İstenen string formatını oluştur - Cihaz adını da dahil et
        $cevap->cevap = "{$consignmentDeviceName}: C_{$consignmentDeviceId}---{$consignmentQuantity}";
    } else {
        $cevap->cevap = null; // Veya seçim yapılmazsa uygulamanızın mantığına göre işlem yapın
    }
            } else {
                // Diğer cevap türleri
