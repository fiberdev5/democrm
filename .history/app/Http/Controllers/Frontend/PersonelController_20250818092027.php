...
public function UpdateDealer(Request $request, $tenant_id, $id)
{
    ...
    // Silinecek belgeleri işle
    if ($request->has('deletedDocuments') && !empty($request->deletedDocuments)) {
        $silinecekBelgeler = json_decode($request->deletedDocuments, true);
        
        foreach ($silinecekBelgeler as $index) {
            if (isset($mevcutBelgeler[$index])) {
                // Dosyayı fiziksel olarak sil
                if (Storage::disk('local')->exists($mevcutBelgeler[$index])) {
                    Storage::disk('local')->delete($mevcutBelgeler[$index]);
                }
                // Array'den kaldır
                unset($mevcutBelgeler[$index]);
            }
        }
    ...
    // Yeni belgeler yüklenmişse
    if ($request->hasFile('belgePdf')) {
        $mevcutBelgeSayisi = count($mevcutBelgeler);
        $yeniDosyalar = $request->file('belgePdf');
        $eklenebilecekSayi = 2 - $mevcutBelgeSayisi;
        
        // Maksimum eklenebilecek kadar dosya al
        $files = array_slice($yeniDosyalar, 0, $eklenebilecekSayi);
    ...
    // Belgeleri kaydet
    $bayi->belgePdf = !empty($mevcutBelgeler) ? json_encode($mevcutBelgeler) : null;
