...
public function DeleteStockAction($tenant_id, $id)
{
    ...
    if ($stockAction->islem == 2) {
        $notification = [
            'message' => 'Serviste konsinye kullanılmış bir parçayı silemezsiniz. Silmek için servis içerisinden işlem yapmanız gerekmektedir.',
            'alert-type' => 'warning'
        ];
        return redirect()->back()->with($notification);
    }
...
public function DeleteConsignmentStockAction($tenant_id, $id)
{
   ...
   if (in_array($stockAction->islem, [2, 4])) {
    $messages = [
        2 => 'Serviste kullanılmış bir parçayı silemezsiniz...',
        4 => 'Müşteriden iade hareketi silinemez...',
    ];

    return redirect()->back()->with([
        'message' => $messages[$stockAction->islem],
        'alert-type' => 'warning'
    ]);
}
    if ($stockAction->islem == 1) {
