 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        $consignmentDevices = Stock::where('firma_id', $tenant_id)
            ->where('urunKategori', 3) // urunKategori 3'ün konsinye cihazlara karşılık geldiğinden emin olun
            ->get();

        // Toplam konsinye cihaz adedini sıfırla
        $toplamConsignmentAdedi = 0; // Bu satır eklendi
        ...
        foreach ($consignmentDevices as $device) {
            ...
            $cikisAdet = StockAction::where('stokId', $device->id)
                ...
                ->sum('adet');
            $device->current_stock_quantity = $girisAdet - $cikisAdet;
            $toplamConsignmentAdedi += $device->current_stock_quantity; // Toplama işlemi yapılıyor
            ...
            $approvedConsignmentAnswer = $planCevaplar->first(function ($cevap) use ($device) {
                ...
                if ($cevap->soru?->cevapTuru == '[Konsinye Cihaz]') {
                    ...
                    // 'C_ID' formatını değil, doğrudan device ID'sini kontrol ediyoruz.
