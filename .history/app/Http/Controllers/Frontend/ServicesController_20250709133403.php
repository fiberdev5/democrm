...
{
    // 1. Konsinye stok mu kontrolü (güvenlik)
    $stok = Stock::where('id', $stokId)->first();
    if (!$stok || $stok->urunKategori != 3) {
        throw new \Exception("Bu stok bir konsinye cihaz değildir.");
    }

    // 2. Mevcut konsinye stok miktarını hesapla
    $girilen = StockAction::where('stokId', $stokId)
        ->where('firma_id', $tenantId)
        ->where('islem', 1) // giriş işlemi
        ->sum('adet');

    $cikan = StockAction::where('stokId', $stokId)
        ->where('firma_id', $tenantId)
        ->where('islem', 2) // çıkış işlemi
        ->sum('adet');

    $kalanStok = $girilen - $cikan;

    // 3. Stok yeterli mi kontrolü
    if ($adet > $kalanStok) {
        throw new \Exception("Yetersiz konsinye stok: İstenilen {$adet}, mevcut {$kalanStok}.");
    }

    // 4. StokAction ile çıkış işlemini kaydet
    StockAction::create([
        'firma_id' => $tenantId,
        'kid' => auth()->id(),
        'stokId' => $stokId,
