...
/**
     * Kasa İstatistikleri sayfasını gösterir.
     *
    ...
    /**
     * Kasa İstatistikleri için veri getirir.
     *
     * @param \Illuminate\Http\Request $request
     * @return \Illuminate\View\View
     */
    public function GetKasaStatistics(Request $request)
    {
        // Varsayılan olarak ayın ilk gününden bugüne kadar olan veriyi alır.
        ...
        // Gelirler için veriyi çek
        $gelirler = CashTransaction::where('islemTarihi', '>=', $startDate)
            ->where('islemTarihi', '<=', $endDate)
            ->where('odemeYonu', '1')
            ->select('odemeSekli', DB::raw('SUM(fiyat) as toplamFiyat'))
            ->groupBy('odemeSekli')
            ->get();
        ...
        foreach ($gelirler as $row) {
            if ($row->odemeSekli == "7") {
                $nakit = round($row->toplamFiyat);
            } elseif ($row->odemeSekli == "8") {
                $eft = round($row->toplamFiyat);
            } elseif ($row->odemeSekli == "9") {
                $kart = round($row->toplamFiyat);
            }
        }
        $toplamGelir = $nakit + $eft + $kart;

        // Ödeme şekillerini ve gelir verilerini hazırlar
        $odemeSekilleri = PaymentMethod::orderBy('id', 'asc')->get();
        $odemeSekliAll = $odemeSekilleri->pluck('odeme_sekli')->map(fn($item) => '"' . $item . '"')->implode(',');
        $gelirData = [
            'nakit' => $nakit,
            'eft' => $eft,
            'kart' => $kart,
        ...
        // Giderler için veriyi çek ve ödeme türlerine göre gruplandır.
        $odemeTurleri = PaymentType::orderBy('tur', 'asc')->get();
        $giderVerileri = CashTransaction::where('islemTarihi', '>=', $startDate)
            ->where('islemTarihi', '<=', $endDate)
            ->where('odemeYonu', '2')
            ->select('odemeTuru', DB::raw('SUM(fiyat) as toplamFiyat'))
            ->groupBy('odemeTuru')
            ->get();

        $odemeTuruAll = [];
        $odemeTurleriMapping = $odemeTurleri->keyBy('id');

        foreach ($giderVerileri as $row) {
            $turAdi = $odemeTurleriMapping->get($row->odemeTuru)?->tur;
            if ($turAdi) {
                $odemeTuruAll[$turAdi] = round($row->toplamFiyat);
            }
        }

        // Toplam gideri ve diğer verileri hazırlar.
        $toplamGider = array_sum($odemeTuruAll);
        ...
        $giderler = implode(",", array_values($odemeTuruAll));

        // Veriyi görüntüye gönderir
        return view('backend.kasa_istatistik.kasa-istatistik', compact(
            'nakit', 'eft', 'kart', 'toplamGelir', 'odemeSekliAll', 
            'odemeTuruAll', 'toplamGider', 'odemeTuruSonuc', 'odemeTuruRenkler', 'giderler'
        ));
    }
