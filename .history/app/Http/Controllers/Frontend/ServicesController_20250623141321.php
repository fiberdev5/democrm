...
class ServicesController extends Controller
{
    ...
    public function ServiceMoneyActions($tenant_id, $service_id) {
        ...
        $servisParaHareketleri = ServiceMoneyAction::where('firma_id', $firma->id)->where('servisid',$servis->id)->get();

        return view('frontend.secure.all_services.service_money_actions.service_money_actions', compact('firma', 'servis','servisParaHareketleri'));
    }
    ...
    public function StoreServiceIncome($tenant_id, Request $request) {
        ...
        if (auth()->user()->hasRole('Patron') && $request->input('tarih')) {
            $tarih = Carbon::parse($request->input('tarih'));
        }
