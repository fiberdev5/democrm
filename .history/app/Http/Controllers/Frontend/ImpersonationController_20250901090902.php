...
public function start(Request $request, $userId)
    {
        ...
        // Session bilgilerini ayarla - SUPER ADMIN KONTROLÜ EKLENDİ
        session([
            'impersonator_id' => $currentUser->user_id,
            'impersonated_user_id' => $targetUser->user_id,
            'impersonation_id' => $impersonation->id,
            'impersonation_started_at' => now(),
            'original_user_is_super_admin' => $currentUser->isSuperAdmin() // BURASI EKLENDİ
        ]);
    ...
    public function stop(Request $request)
    {
        ...
        $originalUserIsSuperAdmin = session('original_user_is_super_admin', false);
        ...
        // Session'ları ÖNCELİKLE temizle - bu önemli!
        session()->forget([
            'impersonator_id',
            'impersonated_user_id', 
            'impersonation_id',
            'impersonation_started_at',
            'original_user_is_super_admin'
        ]);

        // Orijinal kullanıcıya geri dön
        $originalUser = User::find($impersonatorId);
        if ($originalUser) {
            // Session'ları temizledikten SONRA auth işlemlerini yap
            Auth::logout();
        ...
        // YÖNLENDIRME - Basit URL kullan
        if ($originalUserIsSuperAdmin) {
            $redirectUrl = '/super-admin/tenants';
        } else {
            $redirectUrl = "/{$originalUser->tenant_id}/secure";
        }
