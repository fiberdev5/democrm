    ...
    private function applyDashBoardFilters($query, Request $request): void
{
    if ($request->has('dashboard_filter')) {
        ...
        // Status group kontrolü
        if ($request->has('status_group')) {
            
            // İPTAL EDİLEN SERVİSLER - updated_at kullan
            if ($request->status_group === 'cancelled') {
                $query->whereIn('servisDurum', $cancelled_statuses);
                
                // Tarih filtresi varsa updated_at'e göre filtrele
                if ($request->has('dashboard_istatistik_tarih1') && $request->has('dashboard_istatistik_tarih2')) {
                    $query->whereBetween(
                        DB::raw('DATE(updated_at)'), 
                        [$request->dashboard_istatistik_tarih1, $request->dashboard_istatistik_tarih2]
                    );
                }
            } 
            // İŞLEMDE OLAN SERVİSLER - updated_at kullan
            ...
            // DİĞER DURUMLAR - kayitTarihi kullan
            else {
                if ($request->has('dashboard_istatistik_tarih1') && $request->has('dashboard_istatistik_tarih2')) {
        ...
        // Status group yoksa normal kayitTarihi filtresi
        else {
            if ($request->has('dashboard_istatistik_tarih1') && $request->has('dashboard_istatistik_tarih2')) {
                $query->whereBetween('kayitTarihi', [$request->dashboard_istatistik_tarih1, $request->dashboard_istatistik_tarih2]);
            }
