...
class SubscriptionController extends Controller
{
    ...
    public function completePayment(Request $request, $tenant_id, $planid)
    {
        ...
        try {
            ...
            if (isset($billingData['first_name'])) {
                $tenant->name = $billingData['first_name'];
            }
            if (isset($billingData['email'])) {
                $tenant->eposta = $billingData['email'];
            }
            if (isset($billingData['phone'])) {
                $tenant->tel1 = $billingData['phone'];
            }
            if (isset($billingData['il'])) {
                $tenant->il = $billingData['il'];
            }
            if (isset($billingData['ilce'])) {
                $tenant->ilce = $billingData['ilce'];
            }
            if (isset($billingData['address'])) {
                $tenant->adres = $billingData['address'];
            }
            $tenant->save();
