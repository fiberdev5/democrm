...
class InvoicesController extends Controller
{
    ...
    // Servis ID'sine göre arama yap (LIKE kullanarak benzer sonuçları getir)
    $veriler = DB::table('services')
        ...
        ->where(function($query) use ($servisId) {
            $query->where('services.id', 'LIKE', '%' . $servisId . '%')
                  ->orWhere('services.servis_no', 'LIKE', '%' . $servisId . '%'); // Eğer servis_no alanınız varsa
        })
        ->select(
            'services.id',
            'services.musteri_id',
        ...
        ->limit(10) // Maksimum 10 sonuç getir
        ->get();
