...
public function DeleteConsignmentStockAction($tenant_id, $id)
{
    ...
    // DÜZELTME: stokId kullanılmalı, $id değil
    $stokHareketSayisi = StockAction::where('stokId', $stockAction->stokId)->count();
    ...
    // Stok hareketine ait işlem türü kontrolü: islem 2 veya islem 4 ise silinemez
    if (in_array($stockAction->islem, [2, 4])) {
        $notification = [
            'message' => 'Bu tür bir stok hareketi silinemez. İşlem yapmak için ilgili servis/iade kaydı üzerinden düzenleme yapmanız gerekmektedir.',
            'alert-type' => 'warning'
        ];
        return redirect()->back()->with($notification);
    }
