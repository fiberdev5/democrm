...
{
    // Varsayılan olarak mevcut ayın ilk gününden son güne kadar olan verileri alır
    $startDate = date("Y-m-01 00:00:00");
    $endDate = date("Y-m-d 23:59:59");

    // Gelir verilerini hesaplar
    $kasaHareketleri = CashTransaction::
        whereBetween('islemTarihi', [$startDate, $endDate])
        ->where('odemeYonu', '1')
        ->get();

    $nakit = $kasaHareketleri->where('odemeSekli', '7')->sum('fiyat');
    $eft = $kasaHareketleri->where('odemeSekli', '8')->sum('fiyat');
    $kart = $kasaHareketleri->where('odemeSekli', '9')->sum('fiyat');
    ...
    // Gider verilerini hesaplar
    $odemeTurleri = PaymentType::orderBy('tur', 'asc')->get();
    $odemeTuruAll = [];

    foreach ($odemeTurleri as $odemeTuru) {
        $gider = CashTransaction::
            whereBetween('islemTarihi', [$startDate, $endDate])
            ->where('odemeYonu', '2')
            ->where('odemeTuru', $odemeTuru->id)
            ->sum('fiyat');
        $odemeTuruAll[$odemeTuru->tur] = round($gider);
    }
    
    $giderlerToplam = array_sum($odemeTuruAll);

    // Grafik verileri için JSON formatına dönüştürme
    $odemeTuruSonuc = json_encode(array_keys($odemeTuruAll));
    $odemeTuruRenkler = json_encode($odemeTurleri->pluck('renk')->toArray());
    $giderler = rtrim(implode(",", array_values($odemeTuruAll)), ",");
...
public function kasaFilteredData(Request $request, $tenant_id)
{
    ...
    $kasaHareketleri = CashTransaction::where('tenant_id', $tenant_id)
                                        ->whereBetween('islemTarihi', [$startDate, $endDate])
                                        ->where('odemeYonu', '1')
                                        ->get();
...
public function giderTabloGetir(Request $request, $tenant_id)
{
    ...
    $giderHareketleri = CashTransaction::where('tenant_id', $tenant_id)
        ->whereBetween('islemTarihi', [$startDate, $endDate])
        ->where('odemeYonu', '2')
        ->get();

    $odemeTuruAll = array();
    $odemeTurleri = PaymentType::where('tenant_id', $tenant_id)->orderBy('tur','asc')->get();
    ...
    $odemeTuruAll = [];
    foreach($odemeTurleri as $odemeTuru){
        $gelir = CashTransaction::where('tenant_id', $tenant_id)
                ->where('islemTarihi', '>=', $startDate)
    ...
    $html = "<ul>";
    foreach ($odemeTuruAll as $key => $value) {
        $odemeTurSec = PaymentType::where('tenant_id', $tenant_id)->where('tur', $key)->first();
        $html .= '<li class="gider"><div class="renk" style="background:'.$odemeTurSec["renk"].'"></div><div class="adi">'.$key.'</div><div class="para">'.number_format($value, 0, ',', '.').'TL</div></li>';
    }
...
public function giderGrafikGetir(Request $request, $tenant_id)
{
    ...
    $getTable = CashTransaction::where('tenant_id', $tenant_id)
                                ->whereBetween('islemTarihi', [$startDate, $endDate])
