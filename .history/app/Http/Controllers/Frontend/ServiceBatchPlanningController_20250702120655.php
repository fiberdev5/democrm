    ...
    public function getServiceList(Request $request, $tenant_id)
{
    ...
    if (!empty($persID)) {
        ...
        $selectedDates = ServiceStageAnswer::where('cevap', $date)
            ->where('firma_id', $tenant_id)
            ->where('soruid', 48)
            ->pluck('planid');
   ...
   if ($request->ajax() && !empty($persID)) {
        return view('frontend.secure.all_services.service_batch_planning.personnel_services_partial', 
            compact('services', 'firma', 'persID', 'personeller'));
    }
