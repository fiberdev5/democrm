...
class InvoicesController extends Controller
{
   ...
   public function StoreInvoice(Request $request, $tenant_id){
    // Debug için - geçici
    \Log::info('Gelen fatura verisi:', [
        'firma_id' => $request->firma_id,
        'payment_types' => $request->payment_type,
        'payment_ids' => $request->payment_id,
        'aciklama' => $request->aciklama,
        'toplam' => $request->toplam,
    ]);


    // Debug - form verilerini yazdır
    dd([
        'firma_id' => $request->firma_id,
        'payment_types' => $request->payment_type,
        'payment_ids' => $request->payment_id,
    ...
    if ($request->payment_type && $request->payment_id) {
        foreach ($request->payment_type as $index => $type) {
            $paymentDetails[] = [
                ...
                'amount' => $request->tutar[$index] ?? null,
            ];
        }
    }

    // super_admin_invoices tablosuna kaydet
    $invoice = DB::table('super_admin_invoices')->insertGetId([
        'firma_id' => $request->firma_id, // Form'dan gelen firma ID
        'payment_type' => $request->payment_type[0] ?? null, // İlk ödeme tipi
        'odeme_id' => $request->payment_id[0] ?? null, // İlk ödeme ID'si
        ...
        'faturaPdf' => $save_url,
        'payment_details' => json_encode($paymentDetails), // Çoklu ödeme detayları JSON olarak
        'created_at' => now(),
        'updated_at' => now(),
    ]);
    ...
    if($invoice){
        ...
        // Log kaydı
        ActivityLogger::logInvoiceCreated($invoice, $request->faturaNumarasi, $customerName);
        ...
        $notification = array(
            'message' => 'Fatura Başarıyla Eklendi (' . count($aciklama) . ' ürün, ' . count($paymentDetails) . ' ödeme)',
            'alert-type' => 'success'
        );
