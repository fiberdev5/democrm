...
public function StoreStockAction(Request $request, $tenant_id) {
    ...
    if ($request->islem == 4) {
        $mevcutStok = \App\Models\StockAction::where('stokId', $stokId)
            ...
            ->selectRaw("SUM(CASE WHEN islem = 1 THEN adet ELSE 0 END) as giren, SUM(CASE WHEN islem = 4 THEN adet ELSE 0 END) as cikan")
            ->first();
    ...
    try {
        ...
        // Personel'e gönderme işlemi ise önce PersonelStock oluştur
        if ($request->islem == 2) {
            ...
            // PersonelStock ID'sini StockAction'a ekle
            $stockAction->perStokId = $personelStok->id;
            $stockAction->personel  = $personel_id;
        }
        
        // StockAction'ı kaydet
        if (!$stockAction->save()) {
            throw new \Exception('StockAction kaydedilemedi!');
        }
        ...
        \DB::rollback();
        \Log::error('StockAction kayıt hatası: ' . $e->getMessage());
