...
public function stop(Request $request)
{
    ...
    $originalUserTenantId = null; // Initialize this variable
    ...
    if ($originalUser) {
        ...
        $originalUserTenantId = $originalUser->tenant_id; // Get the tenant_id
    }
    ...
    // Determine the redirect URL based on the original user's role
    $redirectUrl = route('secure.home', ['tenant_id' => $originalUserTenantId]); // Default to regular home

    if ($originalUser && $originalUser->hasRole('Super Admin')) { // Assuming 'superadmin' is the role name
        $redirectUrl = route('super.admin.tenants');
    }
    ...
    if ($request->expectsJson()) {
        return response()->json([
            'success' => true,
            'message' => 'Kendi hesabınıza geri döndünüz.',
            'redirect_url' => $redirectUrl // Use the determined URL here
        ]);
    }

    return redirect($redirectUrl)->with($notification); // Use the determined URL here
}
