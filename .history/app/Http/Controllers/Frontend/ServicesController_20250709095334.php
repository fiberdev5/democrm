...
private function useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId,$soruId)
{
    ...
    StockAction::create([
        ...
        'kid' => auth()->id(),
        'stokId' => $stokId,
        'islem' => 2, // Bu işlemi stoktan düşüş olarak yorumladığınızdan emin olun
        'servisid' => $servisId,
        'adet' => $adet,
        'planId' => $planId,
        'created_at' => now(),
    ...
    Log::info("Consignment Device kullanımı: stokId={$stokId}, adet={$adet}, islem=2");

    // Konsinye cihazın stok miktarını güncelle (azalt)
    $consignmentStock = Stock::where('id', $stokId)->first();
    if ($consignmentStock) {
        $consignmentStock->decrement('adet', $adet); // Adeti belirtilen miktar kadar azalt
        $consignmentStock->save(); // Değişikliği kaydet
        Log::info("Stok Güncelleme: stokId={$stokId}, yeni_adet={$consignmentStock->adet}");
    }

    // Konsinye cihazın ürün adını al (bu kısım zaten var)
