...
public function UpdateServicePlan(Request $request, $tenant_id)
{
    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            $selectedConsignment = $request->input($soruKey);
            
            if ($selectedConsignment) {
                // küçük harf gelirse düzelt
                if (str_starts_with(strtolower($selectedConsignment), 'c_')) {
                    $selectedConsignment = 'C_' . substr($selectedConsignment, 2);
                }

                $cevap->cevap = $selectedConsignment;
            } else {
                $cevap->cevap = null;
            }
        } else {
