...
class ServicesController extends Controller
{
    ...
    public function ServiceStageQuestionShow($tenant_id ,$asamaid, $serviceid) {
        $firma = Tenant::where('id', $tenant_id)->first();
        if (!$firma) {
        dd('Hata: Belirtilen tenant_id için firma bulunamadı: ' . $tenant_id);
    }
    $loggedInUserId = auth()->user()->user_id ?? 'N/A (Kullanıcı giriş yapmamış veya user_id eksik)';
    dd('Firma ID: ' . $firma->id, 'Giriş Yapan Kullanıcı ID: ' . $loggedInUserId);
    ...
    private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    {
        ...
        $perStok = PersonelStocks::where('user_id', $sonPlan->user_id)
            ->where('stok_id', $stokId)
            ->first();

        if ($perStok) {
            PersonelStocks::where('id', $perStok->id)
                ->update([
    ...
    private function parcaKullan($stokId, $adet, $servisId, $planId, $tenantId)
    {
        ...
        $perStok = PersonelStock::where('user_id', auth()->id())
            ->where('stok_id', $stokId)
            ->first();
