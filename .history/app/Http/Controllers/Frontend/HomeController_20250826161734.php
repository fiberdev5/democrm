...
private function getStockAlerts($tenant_id)
{
    ...
    try {
        // Her bir ürünün güncel stoğunu hesapla - En son hareket tarihine göre
        $currentStock = DB::table('stock_actions')
            ->select(
                'stokId',
                DB::raw('SUM(adet) as current_stock'),
                DB::raw('MAX(created_at) as last_action_date')
            )
            ->where('firma_id', $tenant_id)
            ->groupBy('stokId')
            ->having('current_stock', '<=', $low_level); // Sadece düşük stokluları getir
        ...
        $products = DB::table('stocks as s')
            ...
            ->where('s.firma_id', $tenant_id)
            ->where('s.urunKategori', '!=', 3) // Konsinye kategorisini hariç tut (kategori ID 3 varsayarak)
            ->where('sa.current_stock', '>', 0) // Stoku sıfır olanları hariç tut
            ->select(
                's.id',
                's.urunAdi',
                's.urunKodu',
