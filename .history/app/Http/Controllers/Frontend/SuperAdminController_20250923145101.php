...
private function getOtherFilesBreakdown($tenant)
{
    // Mevcut private fonksiyonları çağır
    $supportCount = $this->getSupportAttachmentsCount($tenant);
    $dealerCount = $this->getDealerDocumentsCount($tenant);  
    $invoiceCount = $this->getInvoiceDocumentsCount($tenant);
    
    $totalCount = $supportCount + $dealerCount + $invoiceCount;
    $totalSize = $tenant->getOtherFilesSize(); // Mevcut fonksiyon
...
private function getSupportAttachmentsCount($tenant)
{
    $count = 0;
    $userIds = \DB::table('tb_user')->where('tenant_id', $tenant->id)->pluck('user_id');
...
private function getInvoiceDocumentsCount($tenant)
{
