...
private function applyDashBoardFilters($query, Request $request): void
{
    if ($request->has('dashboard_filter')) {
        ...
        // Statü grubuna göre tarih filtresi
        if ($request->has('dashboard_istatistik_tarih1') && $request->has('dashboard_istatistik_tarih2')) {
            ...
            // Eğer iptal edilenler için filtreleme yapılıyorsa, GÜNCELLEME tarihini baz al
            if ($request->status_group === 'cancelled') {
                $query->whereBetween(DB::raw('DATE(updated_at)'), [$startDate, $endDate]);
            } 
            // Diğer durumlar için KAYIT tarihini baz al
            else {
                $query->whereBetween('kayitTarihi', [$startDate, $endDate]);
            }
        }

        // Statü filtresi (iptal, işlemde vb.)
