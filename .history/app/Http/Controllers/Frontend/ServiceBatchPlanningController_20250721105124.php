...
{
    Log::info('getServiceList fonksiyonu başladı.');
    ...
    Log::info('İstek Parametreleri:', ['request_all' => $request->all()]);
    Log::info('Çekilen persID:', ['persID' => $persID]);
    ...
    if (!empty($persID)) {
        Log::info('persID dolu, personel özel filtreleme uygulanıyor.');
        // Personnel specific logic - bugün için atanan servisleri getir
        $today = Carbon::today()->format('Y-m-d');
        Log::info('Bugünün tarihi (personel filtresi için):', ['today' => $today]);
        
        $selectedDates = ServiceStageAnswer::where('cevap', $today)
            ->where('firma_id', $tenant_id)
            ->where('soruid', 48)
            ->pluck('planid');
        Log::info('selectedDates (planidler):', ['count' => $selectedDates->count(), 'dates' => $selectedDates->toArray()]);
        ...
        $serviceAnswers = ServiceStageAnswer::where('cevap', $persID)
            ...
            ->pluck('servisid');
        Log::info('serviceAnswers (servisidler):', ['count' => $serviceAnswers->count(), 'services' => $serviceAnswers->toArray()]);

        if ($serviceAnswers->isEmpty()) {
            Log::info('serviceAnswers boş, sorgu 1=0 olarak ayarlandı.');
            $query->whereRaw('1 = 0'); // Hiçbir sonuç döndürmemek için
        } else {
            Log::info('serviceAnswers dolu, ID bazlı filtre uygulanıyor.');
            $query->whereIn('id', $serviceAnswers);
        }
        
    } else {
        Log::info('persID boş, genel filtreleme uygulanıyor.');
        ...
        if (!empty($statuses) && $statuses !== '0') {
            ...
            Log::info('Genel filtre: servisDurum eklendi.', ['status' => $statuses]);
        }
    ...
    // *** BURASI ÇOK KRİTİK: Final SQL sorgusunu ve bağlamalarını burada loglayın ***
    Log::info('Final SQL Sorgusu:', ['sql' => $query->toSql(), 'bindings' => $query->getBindings()]);

    // Sonuçları getir
    $services = $query->orderByDesc('id')->get();
    Log::info('Son Servis Sorgu Sonucu:', ['count' => $services->count(), 'first_service_id' => $services->first()->id ?? 'N/A']);
