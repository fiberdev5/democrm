...
class StatisticController extends Controller
{
    ...
    {
        $days = $request->get('days', 7); // Varsayılan 7 gün
    ...
    {
        $type = $request->get('type', 'today'); // today, 7days, 15days, 30days
        ...
        switch ($type) {
            case 'today':
                if ($date) {
                    $targetDate = Carbon::createFromFormat('d/m/Y', $date);
                    $query->whereDate('kayitTarihi', $targetDate->format('Y-m-d'));
                } else {
                    $query->whereDate('kayitTarihi', Carbon::today());
                }
                break;
            case '7days':
                $query->whereBetween('kayitTarihi', [
        ...
        foreach ($validServices as $service) {
            $hour = (int) Carbon::parse($service->kayitTarihi)->format('H');
            $hourlyData[$hour]++;
        }
