    ...
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                $soruKey = $cevap->id;
                if ($request->has("soru.$soruKey")) {
                    $yeniCevap = $request->input("soru.$soruKey");
                    // Eğer parça seçimi varsa, checkbox'ları işle
                    if ($yeniCevap == 'Parca') {
                       $parcaCevap = $this->updateParcaSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid);
