...
public function getTechnicianDetailData(Request $request, $tenant_id)
{
    try {
        ...
        $planlar = DB::table('service_plannings')
        ->where('pid', $pid)
        ->get();

        $teknisyen_id = $request->input('pid'); // veya doğrudan $pid
...
private function calculateDailyGraphData($servisler, $tarihListesi, $teknisyenId)
{
    ...
    foreach ($tarihListesi as $tarih) {
        ...
        // Gelir
        $gelir = DB::table('kasa_hareketleri')
                   ->whereIn('servis', $gunlukServisler)
                   ->where('odemeYonu', '1')
                   ->where('personel', $teknisyenId)
                   ->whereBetween('islemTarihi', [$tarihYmd . ' 00:00:00', $tarihYmd . ' 23:59:59'])
                   ->sum('fiyat');
        $gelirData[] = $gelir ?: 0;
    }
