...
public function dashboard()
{
    ...
    // Destek talepleri istatistikleri (AdminSupportController'dan)
    $supportStats = [
...
private function getChartData()
{
    ...
    for ($i = 6; $i >= 0; $i--) {
        ...
        // O gün aktif olan kullanıcı sayısı (örnek olarak login tablosundan)
        // Eğer login tablosuz yoksa, sadece o güne kadar toplam aktif kullanıcı sayısını alabilirsiniz
    ...
    class="btn btn-sm btn-outline-warning rounded-pill editTenant mobilBtn me-1" 
    data-bs-toggle="modal" data-bs-target="#editTenantModal" 
    title="Düzenle"><i class="fas fa-edit"></i></a>';
                    
                    $usersButton = '<button class="btn btn-sm btn-outline-primary rounded-pill 
                    mobilBtn view-tenant-users me-1" data-tenant-id="'
                    .$row->id.'" title="Kullanıcıları Görüntüle">
                    ...
                    if ($row->status == 1) {
                        ...
                        if ($tenantOwner) {
                            $impersonateButton = '<button class="btn btn-outline-primary rounded-pill mobilBtn impersonate-tenant-owner me-1" 
