...
{
    $otherPaths = [
        'documents' => storage_path("app/public/documents/firma_{$tenant->firma_slug}"),
        'reports' => storage_path("app/public/reports/firma_{$tenant->firma_slug}"),
        'attachments' => storage_path("app/public/support_attachments/firma_{$tenant->firma_slug}")
    ];
    ...
    foreach ($otherPaths as $type => $path) {
        if (is_dir($path)) {
            ...
            $count = $this->countFilesInDirectory($path);
