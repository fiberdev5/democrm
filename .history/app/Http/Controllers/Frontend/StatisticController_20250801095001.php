...
public function getSurveyStatisticsData(Request $request, $tenant_id)
{
    ...
    $completedServicesQuery = ServicePlanning::select(
        ...
        'service_plannings.pid as personel_id',
        'tb_user.name as adsoyad'
    )
    ->leftJoin('tb_user', 'service_plannings.pid', '=', 'tb_user.user_id')
    ...
    $surveyedServicesQuery = Survey::select('surveys.servisid', 'surveys.personel')
    ...
    $surveyedServices = $surveyedServicesQuery->get();

    // Personel bazında istatistikleri grupla
    $groupedStats = [];
    
    // Önce tamamlanan servisleri personel bazında grupla
    foreach ($completedServices as $service) {
        $personelId = $service->personel_id;
        if (!isset($groupedStats[$personelId])) {
            $groupedStats[$personelId] = [
        ...
        $groupedStats[$personelId]['tamamlanan_servisler'][] = $service->servisid;
    }

    // Sonra anket yapılan servisleri ekle
    foreach ($surveyedServices as $survey) {
        $personelId = $survey->personel;
        if (isset($groupedStats[$personelId])) {
            $groupedStats[$personelId]['anket_yapilan_servisler'][] = $survey->servisid;
        }
    }

    // Her personel için özet bilgileri hesapla
    foreach ($groupedStats as &$stat) {
        $stat['tamamlanan_servis_sayisi'] = count($stat['tamamlanan_servisler']);
        $stat['anket_yapilan_servis_sayisi'] = count($stat['anket_yapilan_servisler']);
        $stat['anket_orani'] = $stat['tamamlanan_servis_sayisi'] > 0 
            ? round(($stat['anket_yapilan_servis_sayisi'] / $stat['tamamlanan_servis_sayisi']) * 100, 1) 
            : 0;
    }

    // Toplam sayıları hesapla
    $totalCompletedServices = $completedServices->count();
    $totalSurveyedServices = $surveyedServices->count();
