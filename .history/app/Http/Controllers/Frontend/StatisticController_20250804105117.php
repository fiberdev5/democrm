   ...
   private function calculateTechnicianStats($userId, $dateRange, $deviceType, $tenantId)
{
    ...
    \Log::info("Plan IDs for user $userId:", $planIds);

    if (empty($planIds)) {
        \Log::info("No plan IDs found for user $userId");
        return $this->getEmptyStats();
    }

    // Grup soruları ID'lerini dinamik olarak al
    $groupQuestionIds = DB::table('stage_questions')
        ->where('cevapTuru', 'like', '%Grup%')
        ->pluck('id')
        ->toArray();

    \Log::info('Group question IDs found:', $groupQuestionIds);

    // Bu teknisyenin servislerini bul - hem user_id hem de string olarak kontrol et
    $serviceIds = ServiceStageAnswer::join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ...
        ->whereIn('service_stage_answers.soruid', $groupQuestionIds)
        ->pluck('service_stage_answers.servisid')
        ->unique()
        ->toArray();

    \Log::info("Service IDs found for user $userId:", $serviceIds);

    if (empty($serviceIds)) {
        // Alternatif arama - cevap alanında kullanıcı ID'si geçen tüm kayıtlar
        $serviceIds = ServiceStageAnswer::whereIn('planid', $planIds)
            ->where(function($query) use ($userId) {
                $query->where('cevap', $userId)
                      ->orWhere('cevap', 'like', '%[Grup-' . $userId . ']%')
        ...
        \Log::info("Alternative service IDs found for user $userId:", $serviceIds);
    }
