        ...
        if ($request->ajax()) {           
            ...
            if ($request->filterType && $request->filters) {
                ...
                switch ($request->filterType) {
                    ...
                    break;
                    case 'gelen-cagrilar':
                        // 💡 Not: $data zaten Service::query() olarak yukarıda tanımlıysa, burada override ediyoruz.
                        $t1 = Carbon::parse($filters['cagri_tarih1'])->startOfDay();
                        $t2 = Carbon::parse($filters['cagri_tarih2'])->endOfDay();

                        $data = IncomingCall::query()
                            ...
                            ->select([
                                ...
                                'kayitAlan',
                                'marka',
                                \DB::raw("NULL as asama_id"),
                                \DB::raw("NULL as m_adi"),
                            ]);
                    break;
                }
            }
