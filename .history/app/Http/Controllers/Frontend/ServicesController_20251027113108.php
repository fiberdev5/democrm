    ...
    public function SaveServicePlan(Request $request, $tenant_id) {
        ...
        try {
           ...
           $stokHatasiVar = $this->stokKontrolEt($request, $gelenIslem);
            if ($stokHatasiVar !== null) { // Eğer bir hata mesajı döndürdüyse
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($yeniCevap == 'Parca') {
                    ...
                    if (!$request->has('stok_degisiklik_yok')) {
                        $parcaCevap = $this->processParcaSelection($request, $tenant_id, $cevap->cevap);
                        $cevap->cevap = $parcaCevap;
                    }
                    // Değişiklik yoksa mevcut cevap korunur
                }
                // Konsinye Cihaz seçimi
                elseif ($yeniCevap == 'Konsinye Cihaz') {
                    // Sadece değişiklik varsa işle
                    if (!$request->has('konsinye_degisiklik_yok')) {
                        $konsinyeCevap = $this->processKonsinyeSelection($request, $tenant_id, $cevap->cevap);
...
private function processParcaSelection(Request $request, $tenant_id, $eskiCevap)
{
   $stokCevap = [];
    $planid = $request->input('planid');
    $servisPlan = ServicePlanning::where('id', $planid)
    ...
    // ÖNEMLİ: Önce bu plan için eski stok hareketlerini sil/geri al
    $this->eskiParcaHareketleriniGeriAl($planid, $eskiCevap, $tenant_id);
...
private function processKonsinyeSelection(Request $request, $tenant_id, $eskiCevap)
{
    ...
    // ÖNEMLİ: Önce bu plan için eski konsinye hareketlerini sil/geri al
    $this->eskiKonsinyeHareketleriniGeriAl($planid, $eskiCevap, $tenant_id);
    ...
    return implode(', ', $konsinyeCevap);
}
/**
 * Eski parça hareketlerini geri al (stok ve kasa hareketlerini iptal et)
 */
private function eskiParcaHareketleriniGeriAl($planId, $eskiCevap, $tenantId)
{
    if (empty($eskiCevap)) {
        return; // Eski cevap yoksa yapılacak bir şey yok
    }

    // Eski cevaptan parça ID'lerini çıkar
    $eskiParcalar = explode(', ', $eskiCevap);
    
    foreach ($eskiParcalar as $parcaItem) {
        list($stokId, $adet) = array_pad(explode('---', $parcaItem), 2, 0);
        
        if ($stokId && $adet) {
            // Bu plan için kaydedilmiş stok hareketlerini bul
            $stokHareketler = \App\Models\StockAction::where('firma_id', $tenantId)
                ->where('planId', $planId)
                ->where('stokId', $stokId)
                ->get();

            foreach ($stokHareketler as $hareket) {
                \Log::info("Parça hareketi geri alınıyor: StokID={$stokId}, Adet={$hareket->adet}, HareketID={$hareket->id}");
                
                // 1. Personel Stoğuna Geri Ekle (eğer kullanım kaydıysa)
                if ($hareket->islem == 2) {
                    $personelStok = \App\Models\PersonelStock::where('pid', $hareket->kid)
                        ->where('stokid', $stokId)
                        ->first();

                    if ($personelStok) {
                        // Kayıt varsa adedi artır
                        $personelStok->adet += $hareket->adet;
                        $personelStok->save();
                        \Log::info("Personel stoğu artırıldı: PersonelID={$hareket->kid}, StokID={$stokId}, EklenenAdet={$hareket->adet}");
                    } else {
                        // Kayıt yoksa yeni kayıt oluştur
                        \App\Models\PersonelStock::create([
                            'pid' => $hareket->kid,
                            'stokid' => $stokId,
                            'adet' => $hareket->adet,
                            'firma_id' => $tenantId,
                        ]);
                        \Log::info("Yeni personel stok kaydı oluşturuldu: PersonelID={$hareket->kid}, StokID={$stokId}, Adet={$hareket->adet}");
                    }
                }
                
                // 2. İlgili Kasa Hareketlerini Sil
                $kasaHareketleri = \App\Models\CashTransaction::where('firma_id', $tenantId)
                    ->where('stokIslem', $hareket->id)
                    ->get();

                foreach ($kasaHareketleri as $kasaHareket) {
                    \Log::info("Kasa hareketi siliniyor: CashTransactionID={$kasaHareket->id}");
                    $kasaHareket->delete();
                }

                // 3. İlgili Servis Para Hareketlerini Sil
                $servisParaHareketleri = \App\Models\ServiceMoneyAction::where('firma_id', $tenantId)
                    ->where('stokIslem', $hareket->id)
                    ->get();

                foreach ($servisParaHareketleri as $servisParaHareket) {
                    \Log::info("Servis para hareketi siliniyor: ServiceMoneyActionID={$servisParaHareket->id}");
                    $servisParaHareket->delete();
                }
                
                // 4. Stok Hareket Kaydını Sil
                $hareket->delete();
                \Log::info("Stok hareketi silindi: StockActionID={$hareket->id}");
            }
        }
    }
}
...
/**
 * Eski konsinye hareketlerini geri al
 */
private function eskiKonsinyeHareketleriniGeriAl($planId, $eskiCevap, $tenantId)
{
    if (empty($eskiCevap)) {
        return;
    }

    $eskiKonsinyeler = explode(', ', $eskiCevap);
    
    foreach ($eskiKonsinyeler as $konsinyeItem) {
        list($stokId, $adet) = array_pad(explode('---', $konsinyeItem), 2, 0);
        
        if ($stokId && $adet) {
            // Bu plan için kaydedilmiş konsinye hareketlerini bul
            $konsinyeHareketler = \App\Models\StockAction::where('firma_id', $tenantId)
                ->where('planId', $planId)
                ->where('stokId', $stokId)
                ->get();

            foreach ($konsinyeHareketler as $hareket) {
                \Log::info("Konsinye hareketi geri alınıyor: StokID={$stokId}, Adet={$hareket->adet}, HareketID={$hareket->id}");
                
                // 1. Konsinye Stoğunu Geri Ekle (current_stock_quantity)
                if ($hareket->islem == 2) {
                    $stok = \App\Models\Stock::find($stokId);
                    if ($stok) {
                        $stok->current_stock_quantity += $hareket->adet;
                        $stok->save();
                        \Log::info("Konsinye stok miktarı artırıldı: StokID={$stokId}, EklenenAdet={$hareket->adet}, YeniMiktar={$stok->current_stock_quantity}");
                    }
                }
                ...
                foreach ($kasaHareketleri as $kasaHareket) {
                    \Log::info("Konsinye kasa hareketi siliniyor: CashTransactionID={$kasaHareket->id}");
                    $kasaHareket->delete();
                }
                ...
                foreach ($servisParaHareketleri as $servisParaHareket) {
                    \Log::info("Konsinye servis para hareketi siliniyor: ServiceMoneyActionID={$servisParaHareket->id}");
                    $servisParaHareket->delete();
                }
                
                // 4. Konsinye Hareket Kaydını Sil
                $hareket->delete();
                \Log::info("Konsinye stok hareketi silindi: StockActionID={$hareket->id}");
            }
        }
    }
}
