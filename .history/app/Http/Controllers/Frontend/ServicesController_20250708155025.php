...
private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem)
{
                ...
                if ($cevap == "Parca") {
                    ...
                    foreach ($request->input("konsinye_cihaz.{$soruId}") as $konsinyeId => $value) {
                        ...
                        if ($adet > 0) { // Sadece adet girildiyse işlem yap
                            // Stok adı bilgisine artık burada ihtiyacımız yok, sadece ID ve adet kaydediyoruz.
                            // $consignmentStock = Stock::where('id', $konsinyeId)->first();
                            // $consignmentName = $consignmentStock ? ($consignmentStock->urun_adi ?? 'N/A') : 'N/A';

                            // Konsinye cihaz kullanımı için özel fonksiyonu çağır
                            $this->useConsignmentDevice($konsinyeId, $adet, $servisId, $planId, $tenantId);
    ...
    private function useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId)
    {
        StockAction::create([
            ...
            'kid' => auth()->id(),
            'stokId' => $stokId,
            'islem' => 2, // Konsinye cihazlar için de "Serviste Kullanım" (İşlem kodu 2)
            'servisid' => $servisId,
            'adet' => $adet,
            'planId' => $planId,
            'created_at' => now(),
        ...
        private function useConsignmentDevice($stokId, $adet, $servisId, $planId, $tenantId)
{
    ...
    $consignmentStock = App\Models\Stock::where('id', $stokId)->first();
    ...
    ServiceStageAnswer::create([
        ...
        'servisid' => $servisId,
        'planid' => $planId,
        'soruid' => $soruId, // Bu satırın nereden geldiğini kontrol etmeliyiz, soruCevaplariniIsle'den gelmeli
        'cevap' => "{$stokId}---{$adet}---{$consignmentName}", // İsmi de ekledik
        'kid' => auth()->id(), // Auth()->user()->user_id yerine auth()->id() kullanıldı
        'created_at' => now(),
        'updated_at' => now()
    ]);
