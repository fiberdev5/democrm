...
class InvoicesController extends Controller
{
    ...
    public function StoreInvoice(Request $request, $tenant_id)
    {
        ...
        try {
            ...
            $invoice = Invoice::create([
                ...
                'faturaPdf' => $save_url,
                'faturaDurumu' => 'draft', // ✅ Başlangıçta draft
            ]);
    ...
    'faturaAciklama' => $request->faturaAciklama, // Formda varsa
    'indirim' => $invoice->indirim,
    'odemeDurum' => $invoice->odemeDurum,
    'toplamTutar' => $invoice->genelToplam,
    
    // --- KRİTİK EKLEMELER ---
    'tevkifatOrani' => $request->tevkifatOrani ?? '2', // Örn: 2 (2/10 için)
    // Not: Request'ten gelen ID ise gerçek kodu bulmanız lazım, direkt kod geliyorsa aynen gönderin.
    'tevkifatKodu' => $request->tevkifatKoduText ?? '601', // Örn: 601, 602 vb. (Selectbox'tan text olarak almak veya DB'den çekmek gerekir)
    
    'kdvTutar' => $request->kdvTutar, // Eğer 0 ise istisna kontrolü yapılır
    'kdvKodu' => $request->kdvKoduText ?? '301', // KDV İstisna Kodu (Örn: 301)
    'kdvAciklama' => $request->kdvAciklama,
    'faturaNotu' => 'Sistem tarafından otomatik oluşturuldu.',
    // -------------------------
    
    'customer' => $customerData,
    'items' => array_map(function($item) use ($request) {
        // Ürün bazlı KDV oranını da ekliyoruz
        $item['kdv_orani'] = $request->kdvTutar ?? 20; 
        return $item;
    }, $invoiceProducts)
];
