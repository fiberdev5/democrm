...
public function getIlceStatisticsData(Request $request)
{
    $from = $request->input('from_date');
    $to = $request->input('to_date');

   $data = DB::table('services')
    ->join('customer', 'services.musteri_id', '=', 'customer.id')
    ->join('ilceler', 'customer.ilce', '=', 'ilceler.id') // <--- bunu ekle
    ->select('ilceler.ilce_adi as ilce', DB::raw('COUNT(services.id) as toplam'), 'ilceler.id as ilce_id')
    ->whereBetween('services.kayitTarihi', [$from, $to])
    ->groupBy('customer.ilce', 'ilceler.ilce_adi', 'ilceler.id') // group by tüm select alanlarını kapsamalı
    ->get();


    return DataTables::of($data)
        ->addColumn('ilce_id', function ($row) {
            return $row->ilce;
        })
        ->make(true);
}
