...
public function searchBrands(Request $request): JsonResponse
{
    try {
        ...
        $query = Brand::where('tenant_id', $this->tenant_id)
                     ->where('marka', 'LIKE', '%' . $term . '%')
                     ->orderBy('marka');
        
        // Eğer sayfalama gerekiyorsa
        if ($page > 1) {
            $query->skip(($page - 1) * $perPage);
        }
        
        $brands = $query->take($perPage)->get();
...
public function searchDevices(Request $request): JsonResponse
{
    try {
        ...
        $query = Device::where('tenant_id', $this->tenant_id)
                      ->where('cihaz', 'LIKE', '%' . $term . '%')
                      ->orderBy('cihaz');
        ...
        $devices = $query->take($perPage)->get();
        ...
        \Log::error('Device search error: ' . $e->getMessage());
        return response()->json(['error' => 'Arama sırasında hata oluştu.'], 500);
    }
}

/**
 * Kategori arama - Select2 için
 */
public function searchCategories(Request $request): JsonResponse
{
    try {
        ...
        $query = Category::where('tenant_id', $this->tenant_id)
                        ->where('kategori', 'LIKE', '%' . $term . '%')
                        ->orderBy('kategori');
        ...
        $categories = $query->take($perPage)->get();
...
/**
 * Raf arama - Select2 için
 */
public function searchShelves(Request $request): JsonResponse
{
    try {
        $term = $request->get('q', '');
        $page = $request->get('page', 1);
        ...
        $query = Shelf::where('tenant_id', $this->tenant_id)
                     ->where('raf_adi', 'LIKE', '%' . $term . '%')
                     ->orderBy('raf_adi');
        ...
        $shelves = $query->take($perPage)->get();
        ...
        \Log::error('Shelf search error: ' . $e->getMessage());
        return response()->json(['error' => 'Arama sırasında hata oluştu.'], 500);
    }
}
//Marka ekleme
