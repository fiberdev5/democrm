...
class ImpersonationController extends Controller
{
    ...
    {
        // 1. Gerekli session bilgilerini kontrol et ve al
        if (!Session::has('impersonator_id') || !Session::has('impersonated_user_id') || !Session::has('impersonation_id')) {
            // Eğer session'da impersonation bilgisi yoksa, zaten impersonate edilmiyor demektir.
            // Güvenli bir yere yönlendir ve bilgilendir.
            $notification = [
                'message' => 'Aktif impersonation oturumu bulunamadı.',
                'alert-type' => 'info'
            ];
            // 'giris' rotası ön yüz login rotanız ise buraya yönlendirin.
            return redirect()->route('giris')->with($notification); 
        }

        $impersonatorId = Session::get('impersonator_id');
        $impersonationRecordId = Session::get('impersonation_id'); // Kayıt ID'sini kullanıyoruz

        // 2. Impersonation kaydını veritabanında sonlandır
        ...
        // 3. Orijinal kullanıcıyı bul
        $originalUser = User::find($impersonatorId);

        // 4. Session'daki impersonation verilerini temizle. Bu adım Auth::login() öncesi kritik!
        Session::forget([
            'impersonator_id',
        ...
        if ($originalUser) {
            // 5. Laravel'in varsayılan guard'ını kullanarak orijinal kullanıcıyı oturum aç
            // Auth::login() metodu otomatik olarak mevcut oturumu sonlandırır ve yeni kullanıcıyı oturum açar.
            // Bu, redirect loop'u tetikleyebilecek Auth::logout() çağrısını ortadan kaldırır.
            Auth::guard()->login($originalUser);
            
            // 6. Spatie Permission cache'ini temizle
            app(PermissionRegistrar::class)->forgetCachedPermissions();
            
            // 7. Kullanıcı rollerini ve izinlerini yeniden yükle (eğer lazy loading değilse)
            ...
            // 9. Eğer orijinal kullanıcı bulunamazsa, güvenlik için tamamen oturumu kapat
            Auth::logout();
