...
class CustomerController extends Controller
{
  public function AllCustomer($tenant_id, Request $request) {
    ...
    if ($request->ajax()) {
        // İZİN KONTROLÜ - Doğru izin adlarıyla
        $user = auth()->user();
        $canView = $user->can('Müşterileri Görebilir');      // İzin ID: 22
        $canEdit = $user->can('Müşteri Düzenleyebilir');     // İzin ID: 24
        $canDelete = $user->can('Müşteri Silebilir');        // İzin ID: 25
...
{
    return '<a class="t-link editCustomer address idWrap" href="javascript:void(0);" 
               data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editCustomerModal">
               '.$row->id.'
            </a>';
}
...
{
    return '<a class="t-link editCustomer address" href="javascript:void(0);" 
               data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editCustomerModal">
               <div class="mobileTitle">Ad Soyad:</div>'.e($row->adSoyad).'
            </a>';
}
...
private function colTel($row): string
{
    ...
    return '<a class="t-link editCustomer" href="javascript:void(0);" 
               data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editCustomerModal">
               <div class="mobileTitle">Telefon:</div>'.e($telefon).'
            </a>';
}

private function colAddress($row): string
{
    // Static cache ile N+1 problemini çöz
    static $ilCache = [];
    static $ilceCache = [];
    
    if ($row->il && !isset($ilCache[$row->il])) {
        $ilCache[$row->il] = \DB::table('ils')->where('id', $row->il)->value('name');
    }
    
    if ($row->ilce && !isset($ilceCache[$row->ilce])) {
        $ilceCache[$row->ilce] = \DB::table('ilces')->where('id', $row->ilce)->value('ilceName');
    }
    
    $il = $ilCache[$row->il] ?? '';
    $ilce = $ilceCache[$row->ilce] ?? '';
    
    $address = '';
    if (!empty($row->adres) && !empty($il) && !empty($ilce)) {
        $address = e($row->adres) . ' ' . e($il) . ' / ' . e($ilce);
    }
    
    return '<a class="t-link editCustomer address" href="javascript:void(0);" 
               data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editCustomerModal">
               <div class="mobileTitle">Adres:</div>'.$address.'
            </a>';
}

private function colActions($row, $canView, $canEdit, $canDelete): string
{
    ...
    if ($canView) {
        $viewButton = '<a href="javascript:void(0);" data-bs-id="'.$row->id.'" 
                          class="btn btn-outline-primary btn-sm editCustomer mobilBtn mbuton2" 
                          data-bs-toggle="modal" data-bs-target="#editCustomerModal" title="Görüntüle">
    ...
    if ($canEdit) {
        $editButton = '<a href="javascript:void(0);" data-bs-id="'.$row->id.'" 
                          class="btn btn-outline-warning btn-sm editCustomer mobilBtn mbuton1" 
                          data-bs-toggle="modal" data-bs-target="#editCustomerModal" title="Düzenle">
    ...
    if ($canDelete) {
        $deleteButton = '<a href="'.$deleteUrl.'" 
                            class="btn btn-outline-danger btn-sm mobilBtn mbuton3" 
                            id="delete" title="Sil">
