   ...
   public function paymentSuccess(Request $request)
{
    ...
    // Test amaçlı - Hipcall çağrılarını göster
    ...
    // Müşteri senkronizasyon sayfası
public function hipcallSyncContacts($tenant_id)
{
    $tenant = Tenant::findOrFail($tenant_id);
    ...
    return view('frontend.secure.integrations.hipcall-sync', compact('tenant', 'customers'));
}

// Seçili müşterileri senkronize et
public function syncSelectedContacts(Request $request, $tenant_id)
{
    try {
        ...
        if ($customers->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'Seçili müşteriler bulunamadı'
            ]);
        }
        
        // Hipcall'a gönder
        $hipcallService = new \App\Services\HipcallService($tenant_id);
        $results = $hipcallService->syncMultipleContacts($customers, $tenant_id);
        
        $message = "{$results['success']} müşteri başarıyla gönderildi";
        if ($results['failed'] > 0) {
            $message .= ", {$results['failed']} müşteri gönderilemedi";
        }
        ...
        Log::error('Sync selected contacts error', [
            'error' => $e->getMessage()
        ]);
...
public function syncAllContacts($tenant_id)
{
    try {
        $customers = \DB::table('customers')
            ->where('firma_id', $tenant_id)
        ...
        if ($customers->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'Telefon numarası olan müşteri bulunamadı'
            ]);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => "{$results['success']} / {$results['total']} müşteri gönderildi",
            'results' => $results
        ]);
        ...
        return response()->json([
            'success' => false,
            'message' => 'Toplu senkronizasyon hatası: ' . $e->getMessage()
        ], 500);
    }
}

// Tek müşteri gönder
public function syncSingleContact(Request $request, $tenant_id, $customer_id)
{
    try {
        ...
        $hipcallService = new \App\Services\HipcallService($tenant_id);
        $result = $hipcallService->syncContact($customer);
