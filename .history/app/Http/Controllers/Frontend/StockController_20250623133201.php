...
public function StoreStock(Request $request, $tenant_id){
        // 1. Aynı ürün kodu kontrolü
$existingCode = Stock::where('firma_id', $tenant_id)
    ->where('urunKodu', $request->urunKodu)
    ->first();

    // Aynı ürün kodu kontrolü
    $existingCode = Stock::where('firma_id', $tenant_id)
        ->where('urunKodu', $request->urunKodu)
        ->first();
    ...
    if ($existingCode) {
        return redirect()->back()->withInput()->with([
            'message' => 'Bu ürün kodu zaten kayıtlı: "' . $existingCode->urunAdi . '"',
            'alert-type' => 'warning'
        ]);
    }

    // AJAX ile aynı ürün adı kontrolü
    if ($request->ajax()) {
        ...
        if ($existingName) {
            return response()->json([
                'status' => 'exists',
                'stock_id' => $existingName->id,
                'message' => 'Bu ürün adıyla zaten kayıtlı bir ürün var. Ürün düzenleme alanına yönlendiriliyorsunuz: "' . $existingName->urunAdi . '"'
            ]);
        }
    }
    ...
    $stock = Stock::create([
        ...
        'urunKategori' => $request->urunKategori,
    ...
    return redirect()->route('stocks', $tenant_id)->with([
