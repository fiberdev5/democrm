            ...
            if ($request->filterType && $request->filters) {
                ...
                switch ($request->filterType) {
                    ...
                    case 'yapilananketler':
                        ...
                        $data->whereHas('surveys', function($query) use ($filters, $tarih1, $tarih2) {
                        $query->whereBetween('created_at', [$tarih1, $tarih2]);
                         // Personel filtresi (kayitAlan, yani servisi kaydeden personel)
                        if (!empty($filters['yapilan_personel']) && $filters['yapilan_personel'] != '0') {
                            $data->where('kayitAlan', $filters['yapilan_personel']);
                        }
                        ...
                        // Yalnızca anketleri olan servisleri getir
                        $data->whereHas('surveys', function($query) {
                            // surveys tablosunda o servise ait kayıt olup olmadığını kontrol eder
                            // Örneğin, survey tablosunda 'service_id' alanı varsa
                            // $query->where('service_id', '!=', null); veya sadece exists kontrolü
                        });
                        break;
