...
{
    try {
        // Teknisyenleri al (role name = 'Teknisyen')
...
{
    // Servis aşama cevaplarından tarihleri al - DÜZELTME: Doğru tablo adları
    ...
    // DÜZELTME: Doğru soru ID'leri - eski koddan alındı
...
private function getTechnicianServices($teknisyenId, $servisPlanlar)
{
    ...
    // DÜZELTME: Teknisyenin servislerini getir - doğru tablo adları ve mantık
    $servisler = DB::table('service_stage_answers as sc')
        ->leftJoin('stage_questions as ss', 'ss.id', '=', 'sc.soruid')
        ->where('sc.cevap', $teknisyenId) // Teknisyen ID eşleşiyor
        ->where('ss.cevapTuru', 'LIKE', '%Grup%') // Cevap türü 'Grup' olanlar
...
private function calculateComplaintServices($teknisyenId, $servisler)
{
    ...
    // DÜZELTME: Doğru tablo adı service_planning yerine service_plannings olabilir
    $sikayetciler = DB::table('service_plannings') // veya service_planning
...
{
    // DÜZELTME: Doğru tablo adı ve sütun adları
    $paraToplam = DB::table('kasa_hareketleri') // veya cash_movements
                    ->whereIn('servis', $servisler)
                    ->where('odemeYonu', '1') // 1 = Gelen ödeme
...
private function calculateOfferedAmount($teknisyenId, $servisler, $tarih1, $tarih2)
{
    ...
    // DÜZELTME: Doğru tablo adları ve join
    $teklifToplam = DB::table('service_stage_answers as sc')
                      ->leftJoin('service_plannings as sp', 'sp.id', '=', 'sc.planid') // Doğru tablo adı
                      ->whereIn('sc.soruid', [350, 351, 352, 353, 354, 355, 356]) // Teklif soru ID'leri
                      ->whereIn('sc.servisid', $servisler)
                      ->where('sp.pid', $teknisyenId)
                      ->whereIn('sp.tarih', $tarihAraligi)
                      ->sum('sc.cevap');
