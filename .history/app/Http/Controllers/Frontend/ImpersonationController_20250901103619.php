...
{
    Log::info('Impersonation stop metodu başladı.');
    Log::info('Session has impersonator_id: ' . session()->has('impersonator_id'));
    Log::info('Session has impersonated_user_id: ' . session()->has('impersonated_user_id'));

    if (!session()->has('impersonator_id') || !session()->has('impersonated_user_id')) {
        Log::warning('Aktif impersonation session bulunamadı.');
        return response()->json([
            'success' => false,
    ...
    Log::info('Impersonator ID: ' . $impersonatorId . ', Impersonation ID: ' . $impersonationId);
    ...
    if ($impersonationId) {
        ...
        if ($impersonation) {
            ...
            Log::info('Impersonation kaydı sonlandırıldı: ' . $impersonationId);
        } else {
            Log::warning('Impersonation kaydı bulunamadı: ' . $impersonationId);
        }
    }
    ...
    if ($originalUser) {
        Log::info('Orijinal kullanıcı bulundu: ' . $originalUser->name . ' (ID: ' . $originalUser->user_id . ')');
        Auth::logout();
        Log::info('Mevcut kullanıcı oturumu kapatıldı.');
        Auth::login($originalUser);
        Log::info('Orijinal kullanıcı olarak oturum açıldı: ' . Auth::user()->name);
        
        app()[\Spatie\Permission\PermissionRegistrar::class]->forgetCachedPermissions();
        $originalUser->load('roles', 'permissions');
        Log::info('Spatie permission cache temizlendi ve roller yüklendi.');
    } else {
        Log::error('Orijinal kullanıcı bulunamadı (ID: ' . $impersonatorId . ').');
    }
    ...
    Log::info('Impersonation sessionları temizlendi.');
    Log::info('Mevcut Auth::user() (sonrasındaki): ' . (Auth::check() ? Auth::user()->name : 'Yok'));
    ...
    Log::info('Yönlendirme yapılıyor: ' . route('secure.home', ['tenant_id' => $originalUser->tenant_id]));
