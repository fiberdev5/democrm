...
class StatisticController extends Controller
{
    ...
    private function getOperatorStats($servisIds)
    {
       ...
       r  return Service::whereIn('services.id', $servisIds)
                 ->join('tb_users', 'services.kayitAlan', '=', 'tb_users.user_id')
                 ->select('tb_users.name', DB::raw('count(*) as sayi'))
                 ->groupBy('tb_users.name')
                 ->orderBy('sayi', 'desc')
                 ->get();
    }
