...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function UpdateServicePlan(Request $request, $tenant_id)
    {
        ...
        try {
            // DD 1: Request'ten gelen tüm veriyi kontrol edelim
            // Formu gönderdikten sonra bu dd() çıktısını inceleyin.
            // 'soruX' (X = ServiceStageAnswer ID) anahtarının ve değerinin ne olduğunu kontrol edin.
            dd($request->all()); 
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($soru->cevapTuru == '[Parca]') {
                    ...
                    // DD 2: Konsinye cihaz için request'ten gelen değeri kontrol edelim
                    // Bu dd() çıktısı "C_ID---ADET" formatında olmalı.
                    // Eğer "Buzdolabı" veya "Konsinye Cihaz" gibi bir şey geliyorsa, sorun frontend'de olabilir.
                    dd('Konsinye Cihaz Cevabı:', $selectedConsignmentValue); 

                    if ($selectedConsignmentValue) { // Sadece doluysa kaydet
                        $cevap->cevap = $selectedConsignmentValue;
                    } else {
                        $cevap->cevap = null; // Veya boşaltmak isterseniz
                    }
