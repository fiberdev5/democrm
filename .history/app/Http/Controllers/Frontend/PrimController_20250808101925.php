...
class PrimController extends Controller
{
    ...
    private function teknisyenPrimHesapla($personelId, $tarih1, $tarih2)
    {
        ...
        $firmaId = auth()->user()->tenant_id;
        // Tenant için prim oranı ve sınırı çekiliyor
    $primAyar = DB::table('tenant_prims')
        ->where('firma_id', $firmaId)
        ->where('role', 'Teknisyen')
        ->first();
    ...
    $primOrani = floatval($primAyar->prim_orani); // Örn: 5
    $gunlukLimit = floatval($primAyar->gunluk_tutar_limiti); // Örn: 10000

    // Personelin servis planlarını al
    $planlamalar = DB::table('service_plannings')
        ->where('pid', $personelId)
        ->whereBetween('created_at', [$tarih1, $tarih2])
        ->pluck('id');

    if ($planlamalar->isEmpty()) {
        return [];
    }

    // Teklif içeren yanıtlar (cevapText = [Teklif])
    $teklifCevaplar = DB::table('service_stage_answers')
        ->join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ->whereIn('planid', $planlamalar)
        ->where('stage_questions.cevapText', '[Teklif]')
        ->where('service_stage_answers.cevap', '>', 0)
        ->select(
            DB::raw('DATE(service_stage_answers.created_at) as tarih'),
            DB::raw('SUM(service_stage_answers.cevap) as toplam_teklif')
        )
        ->groupBy(DB::raw('DATE(service_stage_answers.created_at)'))
        ->get();

    if ($teklifCevaplar->isEmpty()) {
        return [];
    }

    $sonuclar = [];

    foreach ($teklifCevaplar as $gunluk) {
        $tutar = floatval($gunluk->toplam_teklif);
        if ($tutar >= $gunlukLimit) {
            $primTutar = ($tutar * $primOrani) / 100;

            $sonuclar[] = [
                'tarih' => $gunluk->tarih,
                'toplam_teklif' => $tutar,
                'prim_orani' => $primOrani,
                'prim_tutar' => round($primTutar, 2),
            ];
        }
    }
    ...
    public function hesapla(Request $request)
    {
        ...
        try {
            ...
            $personel = User::where('user_id', $request->personel_id)
                ->first();
    ...
    public function detay(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'servis_id' => 'required|exists:services,id'
        ]);
        ...
        try {
            $servisDetay = DB::table('services')
                ...
                ->where('services.id', $request->servis_id)
                ->first();

            $servisPlanlama = DB::table('service_plannings')
                ->join('tb_user', 'service_plannings.pid', '=', 'tb_user.user_id')
                ->select(
                    'service_plannings.*',
                    'tb_user.name as personel_adi'
                )
                ->where('service_plannings.servisid', $request->servis_id)
                ->orderBy('service_plannings.created_at')
                ->get();
