...
class StockController extends Controller
{
     ...
     public function AllStocks($tenant_id, Request $request){
        ...
        // 1) Personel id filtreleme (hareketlerde personel var mı)
        if ($request->filled('personel_id')) {
            $personelId = $request->personel_id;
            $query->whereHas('hareketler', function ($q) use ($personelId) {
                $q->where('personel', $personelId);
            });
        }

        // 2) Raf filtreleme (urunDepo)
        if ($request->filled('raf_id')) {
            $query->where('urunDepo', $request->raf_id);
        }

        // 3) Marka filtreleme (stok_marka)
        if ($request->filled('marka_id')) {
            $query->where('stok_marka', $request->marka_id);
        }

        // 4) Cihaz filtreleme (stok_cihaz)
        if ($request->filled('cihaz_id')) {
            $query->where('stok_cihaz', $request->cihaz_id);
        }

        // 5) Stok durumu filtreleme
        if ($request->filled('stok_durum')) {
            $stokDurum = $request->stok_durum;
            $query->where(function($q) use ($stokDurum) {
                $q->whereRaw("(SELECT
                                COALESCE(SUM(CASE WHEN islem = 1 THEN adet ELSE 0 END), 0) - 
                                COALESCE(SUM(CASE WHEN islem IN (2, 3) THEN adet ELSE 0 END), 0)
                              FROM stock_actions WHERE stokId = stocks.id) " . ($stokDurum == 1 ? " > 0" : " = 0"));
            });
        }
        ...
        foreach ($stocks as $stock) {
            // Her stok için toplam adet hesapla (giriş - çıkış)
...
public function barkodPdf($tenant_id, $id)
{
    ...
    $pdf = Pdf::loadView('frontend.secure.stocks.stocks_barkod', compact('stock'))
              ->setPaper('A4', 'portrait'); // dikey A4 çıktısı
