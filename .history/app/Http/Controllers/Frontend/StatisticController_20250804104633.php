   ...
   private function getPlanIds($dateRange, $deviceType = null)
{
    ...
    for ($date = $start; $date <= $end; $date->addDay()) {
        $dates[] = $date->format('d/m/Y'); // Çift semicolon kaldırıldı
    }

    // Debug için tarihleri kontrol et
    \Log::info('Generated dates for plan search:', $dates);

    // Stage_questions tablosundan tarih sorusu ID'lerini dinamik olarak al
    ...
    if (empty($dateQuestionIds)) {
        \Log::warning('No date questions found in stage_questions table');
        return [];
    }
    ...
    $query = ServiceStageAnswer::join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ...
        ->whereIn('service_stage_answers.soruid', $dateQuestionIds); // Dinamik ID'ler

    if ($deviceType) {
        $query->where('services.cihazTur', $deviceType);
    }

    $planIds = $query->pluck('service_stage_answers.planid')->toArray();
    
    \Log::info('Plan IDs found:', $planIds);
