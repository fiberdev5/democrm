...
public function EditStock($tenant_id, $id) {
    ...
    $toplamGiris = StockAction::where('stokId', $id)->where('islem', 1)->sum('adet');
    $toplamCikis = StockAction::where('stokId', $id)->whereIn('islem', [2, 3])->sum('adet');
    $stokAdedi = $toplamGiris - $toplamCikis;

    $html = view('frontend.secure.stocks.edit_stock', compact('firma', 'stock', 'rafListesi', 'markalar', 'cihazlar', 'stokAdedi'))->render();
