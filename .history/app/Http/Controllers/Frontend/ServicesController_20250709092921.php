    ...
    foreach ($request->all() as $key => $value) {
        ...
        if (strpos($key, 'soru') !== false && $value == "Parca") {
            foreach ($request->all() as $stokKey => $stokValue) {
                if (strpos($stokKey, 'stokCheck') !== false) {
                    ...
                    foreach ($stokHareketleri as $hareket) {
                        if ($hareket->islem == "1") {
                            $toplam += $hareket->adet;
                        } elseif (in_array($hareket->islem, ["2", "3"])) {
                            $toplam -= $hareket->adet;
                        }
                    }
            ...
            if ($gelenIslem == "238") {
                ...
                if (!$stokSecildi) {
                    return "STOKHATA: Parça Teslim Ederken Stok Seçmeniz Zorunludur.";
                }
            }
        }

 // === KONSİNYE CİHAZ KONTROLÜ ===
if (strpos($key, 'soru') !== false && $value == "Konsinye Cihaz") {
    foreach ($request->all() as $stokKey => $stokValue) {
        if (strpos($stokKey, 'konsinye_cihaz') !== false) {
            $stokId = (int) filter_var($stokKey, FILTER_SANITIZE_NUMBER_INT);
            $adet = abs($request->input("konsinye_adet{$stokId}", 0));

            $stok = Stock::find($stokId);
            if (!$stok) return "STOKHATA: Konsinye stok bulunamadı (ID: $stokId)";
            if ((int)$stok->kategori !== 3) return "STOKHATA: Seçilen ürün konsinye cihaz değildir: " . $stok->urun_adi;

            // Buradaki 'giren' kısmı kaldırıldı veya 0 olarak varsayıldı
            // $giren = StockAction::where('stokId', $stokId)
            //     ->where('islem', 1)
            //     ->sum('adet');

            $giren = 0; // İşlem 1'i dikkate almamak için giren stoğu 0 olarak varsaydık

            $kullanilan = StockAction::where('stokId', $stokId)
                ->where('islem', 2)
                ->sum('adet');

            $kalan = $giren - $kullanilan; // Bu durumda $kalan = -$kullanilan olacak

            // Stok kontrol mantığı, artık sadece kullanılan (islem=2) miktarına bakacak
            // Örneğin: Eğer kullanılan adet 5 ise ve talep 3 ise, -5 < -3 olduğu için sorun yok.
            // Ama talep 6 ise, -5 <= -6 (false) olduğu için hata verecek.
            // Bu mantıkta $kalan <= 0 kontrolü $kullanilan >= 0 iken her zaman doğru olacaktır.
            // Asıl kontrol $adet > abs($kalan) veya $adet > (toplam giriş - toplam çıkış) olmalıydı.
            // Sadece işlem 2 ile azalsın istendiği için aslında 'kalan' mantığı değişiyor.
            // Bu durumda 'kalan' yerine 'toplam işlem 2 ile azalan miktar' diyebiliriz.

            // Yeni durumda kontrolü şöyle yapabiliriz:
            // Eğer $kullanilan (yani işlem 2 ile çıkılan toplam miktar) 0 ise ve $adet > 0 ise yetersizdir.
            // Veya sadece $kullanilan miktarının, istenen $adet'ten büyük veya eşit olup olmadığını kontrol edebiliriz.
            // Ancak bu durumda, hiç "giren" miktar olmadığı için her zaman yetersiz olacaktır.
            // Konsinye takibinde "giren" miktar da önemli olmalı.

            // Eğer amacınız 'konsinye verilen toplam cihaz adedi', 'kullanılan' olarak kabul edilsin ise,
            // $kalan = -$kullanilan olacak ve kontrol $adet > abs($kalan) olmalı.
            // Yani talep edilen adet, halihazırda işlem 2 ile çıkılan adetten fazla olamaz.
            if ($adet > abs($kalan)) { // abs($kalan) aslında $kullanilan değerine eşit
                 return "STOKHATA: " . mb_convert_case($stok->urun_adi, MB_CASE_TITLE, "UTF-8") . " Konsinye Stok Yetersizdir (Sadece İşlem 2 Azalışı Esas Alınmıştır).";
            }
        }
    }

    if ($gelenIslem == "238") {
        $konsinyeSecildi = false;
        foreach ($request->all() as $key => $value) {
            if (strpos($key, 'konsinye_cihaz') !== false) {
                $konsinyeSecildi = true;
                break;
            }
        }

        if (!$konsinyeSecildi) {
            return "STOKHATA: Konsinye Teslim Ederken Cihaz Seçmeniz Zorunludur.";
        }
            }
        }
...
{
    // StockAction kaydını oluştur
    StockAction::create([
        'firma_id' => $tenantId,
        'kid' => auth()->id(),
        'stokId' => $stokId,
        'islem' => 2, // Konsinye cihazlar için "Serviste Kullanım" (İşlem kodu 2)
        'servisid' => $servisId,
        'adet' => $adet,
        'planId' => $planId,
        'created_at' => now(),
    ...
    // Konsinye cihazın ürün adını al
    $consignmentStock = Stock::where('id', $stokId)->first();
    $consignmentName = $consignmentStock ? ($consignmentStock->urunAdi ?? 'N/A') : 'N/A';
