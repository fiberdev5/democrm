...
public function CashStatistics($tenant_id) {
    ...
    // Ödeme şekillerini dinamik olarak çek (firma_id'si NULL veya firma_id olanlar)
    ...
    // Kasa hareketlerini çek (odemeYonu = 1 gelirler)
    $kasaHareketleri = CashTransaction::where('firma_id', $firma_id)
        ->whereBetween('created_at', [$startDate, $endDate])
        ->where('odemeYonu', 1)
        ->get();

    // Ödeme şekline göre tutarları sıfırla
    $tutarlar = [];
    foreach ($odemeSekilleri as $odemeSekli) {
        $tutarlar[$odemeSekli->odemeSekli] = 0;
    }
    ...
    // Kasa hareketlerini ödeme şekline göre topla
    foreach ($kasaHareketleri as $row) {
        $key = $row->odemeSekli;
        if (isset($tutarlar[$key])) {
            $tutarlar[$key] += $row->fiyat;
        }
    }

    // Tutarları yuvarla ve toplam geliri hesapla
    foreach ($tutarlar as $key => $value) {
        $tutarlar[$key] = round($value);
    }
    $gelirler = array_sum($tutarlar);

    // Ödeme türlerini getir ve giderleri hesapla (odemeYonu = 2 giderler)
    ...
    // Ödeme şekillerini JSON string olarak hazırla (örnek: "Nakit","EFT / Havale","Kredi Kartı")
    $odemeSekliAll = '"' . implode('","', $odemeSekilleri->pluck('odemeSekli')->toArray()) . '"';

    return view('frontend.secure.statistics.cash_statistics', compact(
        'tenant_id', 'tutarlar', 'gelirler', 'odemeSekliAll',
