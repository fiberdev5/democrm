...
public function AllStocks($tenant_id, Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        // Personel filtresi - stock_actions tablosu üzerinden JOIN ile
        if ($request->filled('personel')) {
            // Debug için log ekleyelim
            \Log::info('Personel filtresi aktif: ' . $request->personel);
            
            $query->join('stock_actions', function($join) use ($request) {
                $join->on('stock_actions.stokId', '=', 'stocks.id')
                     ->where('stock_actions.islem', 3)
                     ->where(function($q) use ($request) {
                         $q->where('stock_actions.pid', $request->personel)
                           ->orWhere('stock_actions.personel', $request->personel);
                     });
            })->distinct();
        }
