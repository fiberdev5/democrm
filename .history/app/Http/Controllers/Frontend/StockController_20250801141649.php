...
public function AllStocks($tenant_id, Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        // Personel filtresi - stock_actions tablosu üzerinden
        if ($request->filled('personel')) {
            $query->whereExists(function ($subQuery) use ($request) {
                $subQuery->select(\DB::raw(1))
                    ->from('stock_actions')
                    ->whereRaw('stock_actions.stokId = stocks.id')
                    ->where('stock_actions.pid', $request->personel)
                    ->where('stock_actions.islem', 3); // personele gönderme işlemi
            });
        }
        ...
        return DataTables::of($query)
            ...
            ->addColumn('personel_bilgisi', function($row) {
                // Hangi personellere gönderilmiş göstermek isterseniz
                $personeller = \App\Models\StockAction::where('stokId', $row->id)
                    ->where('islem', 3)
                    ->join('users', 'users.id', '=', 'stock_actions.pid')
                    ->pluck('users.name')
                    ->unique()
                    ->implode(', ');
                return $personeller ?: '-';
            ...
            ->rawColumns(['urunKodu', 'urunAdi', 'adet', 'toplamTutar', 'raf_adi', 'marka_cihaz', 'personel_bilgisi', 'created_at', 'action'])
            ->with([
