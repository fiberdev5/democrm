...
public function StoreInvoice(Request $request, $tenant_id){
    $validateData = $request->validate([
        'document'=> 'max:2000',
        'payment_type.*' => 'nullable',  // Çoklu ödeme validation
        'payment_id.*' => 'nullable',
        'aciklama.*' => 'nullable',
        'miktar.*' => 'nullable',
        'fiyat.*' => 'nullable',
    ...
    // Dosya yükleme kısmı aynı kalacak...
    $document = $request->file('document');
    
    // Storage kontrolü
    if ($document && !$firma->canUploadFile($document->getSize())) {
        // ... storage kontrol kodu aynı
    }
    ...
    // ÖNEMLİ: Müşteri ID'sini firma_id'den alacağız
    $firma_id_from_form = $request->firma_id; // Frontend'den gelen firma ID'si

    $invoice = Invoice::create([
        'firma_id' => $tenant_id, // Super admin'in tenant_id'si
        'servisid' => $request->servisid,
        'musteriid' => $firma_id_from_form, // Form'dan gelen firma ID'si müşteri olarak
    ...
    if($invoice){
        // Müşteri bilgisini al - tenant tablosundan
        $customer = Tenant::find($firma_id_from_form);
        ...
        // 2. Ürünleri kaydet - ÇOKLU ÖDEME İÇİN GÜNCELLENDİ
        $aciklama = $request->aciklama ?? [];
        $miktar = $request->miktar ?? [];
        $fiyat = $request->fiyat ?? [];
        $tutar = $request->tutar ?? [];
        
        // Çoklu ödeme bilgilerini de saklayabilirsiniz
        ...
        // Ödeme bilgilerini kaydetmek isterseniz ayrı bir tablo oluşturabilirsiniz
        // InvoicePayments tablosu gibi...
        
        $notification = array(
            'message' => 'Fatura Başarıyla Eklendi (' . count($aciklama) . ' ürün)',
            'alert-type' => 'success'
        );
