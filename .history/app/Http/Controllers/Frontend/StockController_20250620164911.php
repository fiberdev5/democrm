...
public function StokActions(Request $request, $tenant_id, $stock_id)
{
    $stock = Stock::with(['marka', 'cihaz', 'raf'])
        ->where('firma_id', $tenant_id)
        ->findOrFail($stock_id);

    $query = StockAction::select('stock_actions.*', 'stock_suppliers.tedarikci', 'tb_user.name')
    ...
    // islem filtresi var mı kontrol et
    if ($request->has('islem') && in_array($request->islem, ['0', '1', '2', '3'])) {
        if ($request->islem != '0') {
            $query->where('stock_actions.islem', $request->islem);
        }
        // 0 ise tüm hareketler gösterilecek, filtre uygulanmaz
    }

    $stokHareketleri = $query->get();
