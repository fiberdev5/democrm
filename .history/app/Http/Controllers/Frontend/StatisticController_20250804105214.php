    ...
    public function TechnicianStatisticsData(Request $request, $tenant_id)
{
    ...
    \Log::info('Technician stats request params:', [
        'tenant_id' => $tenant_id,
        'from_date' => $fromDate,
        'to_date' => $toDate,
    ...
    \Log::info('Technicians found:', $technicians->pluck('user_id', 'name')->toArray());

    $data = [];
    
    foreach ($technicians as $technician) {
        \Log::info("Processing technician: {$technician->name} (ID: {$technician->user_id})");
    ...
    \Log::info('Final technician data:', $data);

    return DataTables::of(collect($data))->make(true);
}
public function debugTechnicianStats(Request $request, $tenant_id)
{
    $userId = $request->get('user_id', 6); // Test için user_id 6
    ...
    // 2. Plan ID'leri
    $planIds = $this->getPlanIds($dateRange);
    $debug['plan_ids'] = $planIds;
    $debug['plan_ids_count'] = count($planIds);
    
    // 3. Stage questions
    $dateQuestions = DB::table('stage_questions')
        ->where('cevapTuru', 'like', '%Tarih%')
        ->get();
    $debug['date_questions'] = $dateQuestions;
    
    $groupQuestions = DB::table('stage_questions')
        ->where('cevapTuru', 'like', '%Grup%')
        ->get();
    $debug['group_questions'] = $groupQuestions;
    
    // 4. Service stage answers
    $stageAnswers = ServiceStageAnswer::whereIn('planid', $planIds)
        ->where('cevap', 'like', '%' . $userId . '%')
        ->get();
    $debug['stage_answers'] = $stageAnswers;
    ...
    public function TechnicianStatisticsDetail(Request $request, $tenant_id)
    {
        ...
        // Grafik verileri
        $chartData = $this->getChartData2($userId, $dateRange, $deviceType, $tenant_id);
...
{
    \Log::info("Starting calculation for user: $userId");
    
    // Plan ID'lerini al
    $planIds = $this->getPlanIds($dateRange, $deviceType);

    if (empty($planIds)) {
        \Log::info("No plan IDs found for user $userId, returning empty stats");
        return $this->getEmptyStats();
    }

    \Log::info("Plan IDs found for user $userId:", $planIds);

    // Teknisyenin servis ID'lerini bul - birden fazla yöntemle ara
    $serviceIds = collect();

    // Yöntem 1: Grup sorularında user ID'si geçen kayıtlar
    $method1Ids = DB::table('service_stage_answers')
        ...
        ->where(function($query) use ($userId) {
            $query->where('service_stage_answers.cevap', $userId)
                  ->orWhere('service_stage_answers.cevap', 'like', '%' . $userId . '%')
                  ->orWhere('service_stage_answers.cevap', 'like', '%[Grup-' . $userId . ']%')
                  ->orWhere('service_stage_answers.cevap', 'like', '%Grup-' . $userId . '%');
        })
        ->pluck('service_stage_answers.servisid');

    $serviceIds = $serviceIds->merge($method1Ids);

    // Yöntem 2: Service plannings tablosunda pid ile eşleşen kayıtlar
    $method2Ids = DB::table('service_plannings')
        ->whereIn('planid', $planIds)
        ->where('pid', $userId)
        ->pluck('servisid');

    $serviceIds = $serviceIds->merge($method2Ids);

    // Yöntem 3: Doğrudan cevap alanında user ID geçen kayıtlar
    $method3Ids = DB::table('service_stage_answers')
        ->whereIn('planid', $planIds)
        ->where(function($query) use ($userId) {
            $query->where('cevap', $userId)
                  ->orWhere('cevap', 'like', '%' . $userId . '%');
        })
        ->pluck('servisid');

    $serviceIds = $serviceIds->merge($method3Ids);

    // Benzersiz servis ID'lerini al
    $serviceIds = $serviceIds->unique()->values()->toArray();

    \Log::info("Service IDs found for user $userId:", [
        'method1_count' => $method1Ids->count(),
        'method2_count' => $method2Ids->count(), 
        'method3_count' => $method3Ids->count(),
        'total_unique' => count($serviceIds),
        'service_ids' => $serviceIds
    ]);

    if (empty($serviceIds)) {
        \Log::info("No service IDs found for user $userId");
        return $this->getEmptyStats();
    }
    ...
    $stats['tamamlanan_servis'] = DB::table('services')
    ...
    // Alınan ücret
    $stats['alinan_ucret'] = DB::table('cash_transactions')
        ->whereIn('servis', $serviceIds)
        ->where('odemeYonu', 1)
    ...
    \Log::info("Final stats for user $userId:", $stats);
...
{
    // Tarih formatını düzelt - veritabanında hangi formatta saklandığını kontrol et
    $dates = [];
    ...
    for ($date = $start; $date <= $end; $date->addDay()) {
        ...
        $dates[] = $date->format('d/m/Y');     // 01/07/2025
        $dates[] = $date->format('Y-m-d');     // 2025-07-01  
        $dates[] = $date->format('d-m-Y');     // 01-07-2025
        $dates[] = $date->format('d.m.Y');     // 01.07.2025
    }

    \Log::info('All date formats generated:', $dates);

    // Stage_questions tablosundan tarih sorusu ID'lerini al
    $dateQuestionIds = DB::table('stage_questions')
        ->where('cevapTuru', 'like', '%Tarih%')
        ->pluck('id')
        ->toArray();

    \Log::info('Date question IDs found:', $dateQuestionIds);

    if (empty($dateQuestionIds)) {
        return [];
    }

    // Ana sorgu - service_stage_answers tablosunda tarihleri ara
    $query = DB::table('service_stage_answers')
        ->join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ->where('stage_questions.cevapTuru', 'like', '%Tarih%')
        ->whereIn('service_stage_answers.soruid', $dateQuestionIds)
        ->whereIn('service_stage_answers.cevap', $dates);

    // Cihaz türü filtresi varsa
    if ($deviceType) {
        $query->join('services', 'service_stage_answers.servisid', '=', 'services.id')
              ->where('services.cihazTur', $deviceType);
    }

    $planIds = $query->pluck('service_stage_answers.planid')->unique()->toArray();
    
    \Log::info('Plan IDs found:', ['count' => count($planIds), 'ids' => $planIds]);
    
    // Eğer plan ID bulunamazsa, alternatif arama yap
    if (empty($planIds)) {
        \Log::info('No plans found with date search, trying alternative approach...');
        
        // Alternatif: created_at tarihine göre plan ID'lerini bul
        $alternativePlanIds = DB::table('service_stage_answers')
            ->whereBetween('created_at', [
                Carbon::parse($dateRange['start'])->startOfDay(),
                Carbon::parse($dateRange['end'])->endOfDay()
            ])
        ...
        \Log::info('Alternative plan IDs found:', ['count' => count($alternativePlanIds), 'ids' => $alternativePlanIds]);
        
        return $alternativePlanIds;
    }
    ...
    private function getComplaintServiceCount($userId, $serviceIds)
{
    ...
    // GROUP BY hatasını düzeltmek için distinct servisid kullan
    $complaintServiceIds = DB::table('service_plannings')
        ->whereIn('servisid', $serviceIds)
        ->where('gidenIslem', 254)
        ->distinct()
        ->pluck('servisid')
        ->toArray();

    foreach ($complaintServiceIds as $serviceId) {
        $firstService = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('pid', $userId)
            ->orderBy('created_at', 'ASC')
            ->first();
        
        $lastComplaint = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('gidenIslem', 254)
        ...
        if ($firstService && $lastComplaint) {
            $firstDate = strtotime(date('Y-m-d', strtotime($firstService->created_at)));
            $lastDate = strtotime(date('Y-m-d', strtotime($lastComplaint->created_at)));
   ...
   private function getCancelServiceCount($userId, $serviceIds)
{
    ...
    $cancelServiceIds = DB::table('service_plannings')
        ->whereIn('servisid', $serviceIds)
        ->where('gidenIslem', 244)
        ->distinct()
        ->pluck('servisid')
    ...
    foreach ($cancelServiceIds as $serviceId) {
        $firstService = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('pid', $userId)
            ->orderBy('created_at', 'ASC')
        ...
        $lastCancel = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('gidenIslem', 244)
            ->orderBy('created_at', 'DESC')
            ->first();
        ...
        if ($firstService && $lastCancel) {
            $firstDate = strtotime(date('Y-m-d', strtotime($firstService->created_at)));
            $lastDate = strtotime(date('Y-m-d', strtotime($lastCancel->created_at)));
    ...
    private function getCallBackServiceCount($userId, $serviceIds)
{
    ...
    $callbackServiceIds = DB::table('service_plannings')
        ->whereIn('servisid', $serviceIds)
    ...
    foreach ($callbackServiceIds as $serviceId) {
        $firstService = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('pid', $userId)
            ->orderBy('created_at', 'ASC')
        ...
        $lastCallback = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('gidenIslem', 247)
            ->orderBy('created_at', 'DESC')
            ->first();
        ...
        if ($firstService && $lastCallback) {
            $firstDate = strtotime(date('Y-m-d', strtotime($firstService->created_at)));
            $lastDate = strtotime(date('Y-m-d', strtotime($lastCallback->created_at)));
            
            if ($lastDate >= $firstDate - 86400) { // 1 gün tolerans
                $count++;
            }
        }
    }
   ...
   private function getPriceDisagreementCount($userId, $serviceIds)
{
    ...
    $priceIssueServiceIds = DB::table('service_plannings')
        ->whereIn('servisid', $serviceIds)
    ...
    foreach ($priceIssueServiceIds as $serviceId) {
        $firstService = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('pid', $userId)
            ->orderBy('created_at', 'ASC')
        ...
        $lastPriceIssue = DB::table('service_plannings')
            ->where('servisid', $serviceId)
            ->where('gidenIslem', 241)
            ->orderBy('created_at', 'DESC')
            ->first();
        ...
        if ($firstService && $lastPriceIssue) {
            $firstDate = strtotime(date('Y-m-d', strtotime($firstService->created_at)));
            $lastDate = strtotime(date('Y-m-d', strtotime($lastPriceIssue->created_at)));
