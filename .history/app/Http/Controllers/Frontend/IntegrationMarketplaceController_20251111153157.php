...
public function showHipcallContacts($tenant_id)
{
    try {
        ...
        Log::info('Hipcall Contacts Raw Data', [
            'total' => count($result['contacts'] ?? []),
            'first_contact' => $result['contacts'][0] ?? null,
            'sample_contacts' => array_slice($result['contacts'] ?? [], 0, 3)
        ]);
        ...
        if ($result['success']) {
            ...
            foreach ($contacts as &$contact) {
                // DEBUG: Her contact'ın tüm field'larını göster
                Log::debug('Contact Fields', array_keys($contact));
                ...
                // Nested olabilir mi kontrol et
                if (!$phone && isset($contact['phones']) && is_array($contact['phones'])) {
                ...
                if ($phone) {
                    ...
                    $contact['display_phone'] = $phone; // Görüntüleme için
                } else {
                    $contact['exists_in_serbis'] = false;
                    $contact['display_phone'] = null;
                }
            }
        ...
        return view('frontend.secure.integrations.hipcall-contacts', [
            'tenant' => $tenant ?? Tenant::find($tenant_id),
            'contacts' => [],
            'total' => 0,
            'success' => false,
...
public function getContactDetail(Request $request, $tenant_id, $contact_id)
{
    try {
        $hipcallService = new \App\Services\HipcallService($tenant_id);
        $result = $hipcallService->getContactDetail($contact_id);
        
        if ($result['success']) {
            $contact = $result['data'];
            
            // SerbisERP'de var mı kontrol et
            ...
            return response()->json([
                'success' => true,
                'contact' => $contact,
                'exists_in_serbis' => $exists
            ]);
        }
        
        return response()->json([
            'success' => false,
            'message' => 'Detay çekilemedi'
        ]);
