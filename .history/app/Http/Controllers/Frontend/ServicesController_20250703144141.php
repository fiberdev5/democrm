...
// Personel stok ekle/güncelle
    ...
    private function parcaKullan($stokId, $adet, $servisId, $planId, $tenantId)
    {
        ...
        // Stok düşme mantığı: Konsinye cihaz mı, normal parça mı?
        if ($stok->urunKategori == 3) {
            // Konsinye Cihaz: Doğrudan ana stoktan düş
            if ($stok->adet >= $adet) {
                $stok->adet -= $adet;
                $stok->save();
            }
        } else {
            // Normal Parça: Personel stoğundan düş
            $perStok = PersonelStock::where('pid', auth()->user()->user_id)
                ->where('stokid', $stokId)
                ->first();

            if ($perStok) {
                if ($perStok->adet >= $adet) {
                    $perStok->adet -= $adet;
                    $perStok->save();
                }
            }
...
//         }
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        // Konsinye cihazları çek (urunKategori = 3 olan stoklar)
        $consignmentDevices = Stock::where('firma_id', $tenant_id)
            ->where('urunKategori', 3)
            ->orderBy('urunAdi', 'ASC') // Veya başka bir sıralama ölçütü
            ->get();

        // Her bir konsinye cihazın güncel stok miktarını hesapla
        foreach ($consignmentDevices as $device) {
            $toplamGiris = StockAction::where('stokId', $device->id)
                ->whereIn('islem', [1, 4]) // Alış ve Müşteriden Geri Alma
                ->sum('adet');

            $toplamCikis = StockAction::where('stokId', $device->id)
                ->whereIn('islem', [2, 3]) // Serviste Kullanım ve Teslim Edildi (personel stoğuna veya başka bir yere)
                ->sum('adet');

            $device->current_stock_quantity = $toplamGiris - $toplamCikis;
        }

        // Sadece mevcut adedi 0'dan büyük olanları filtrele (opsiyonel ama mantıklı)
        $consignmentDevices = $consignmentDevices->filter(function($device) {
            return $device->current_stock_quantity > 0;
        });

        // Toplam konsinye cihaz adedini hesapla (sadece adet > 0 olanlar için)
        $toplamConsignmentAdedi = $consignmentDevices->sum('current_stock_quantity');
        ...
        try {
            ...
            foreach ($planCevaplar as $cevap) {
                ...
                if ($request->has($soruKey)) {
                    ...
                    if ($yeniCevap == 'Parca') {
                        // Normal personel stokları için parça seçimi
                        ...
                        // Konsinye cihaz seçimi için ek mantık
                        $selectedConsignmentValue = $request->input("parca.{$cevap->soruid}.consignment_select");
                        
                        if ($selectedConsignmentValue && str_starts_with($selectedConsignmentValue, 'C_')) {
                            $consignmentStokId = (int) substr($selectedConsignmentValue, 2); // 'C_' ön ekini kaldır ve ID'yi al
                            $consignmentAdet = $request->input("adet.{$cevap->soruid}.selected_consignment_quantity");

                            if ($consignmentStokId > 0 && $consignmentAdet > 0) {
                                // parcaKullan fonksiyonunu çağırarak konsinye cihazı kaydet
                                $this->parcaKullan($consignmentStokId, $consignmentAdet, $servisPlan->servisid, $planid, $tenant_id);

                                // Cevap olarak konsinye cihaz bilgisini kaydet
                                $selectedConsignmentDevice = Stock::find($consignmentStokId);
                                if ($selectedConsignmentDevice) {
                                    $cevap->cevap = "Konsinye Cihaz: " . $selectedConsignmentDevice->urunAdi . " (Adet: " . $consignmentAdet . ")";
                                } else {
                                    $cevap->cevap = "Konsinye Cihaz (ID: " . $consignmentStokId . ", Adet: " . $consignmentAdet . ")";
                                }
                            } else {
                                // Seçim yapıldı ama adet geçersizse hata veya uyarı verilebilir
                                return response()->json([
                                    'success' => false,
                                    'error' => 'Konsinye cihaz seçildi ancak geçerli adet belirtilmedi.'
                                ], 400);
                            }
                        } else {
                            // Konsinye cihaz cevabı olmasına rağmen seçim yapılmamışsa
                            $cevap->cevap = "Konsinye Cihaz Seçilmedi";
                        }
                    } else {
                        // Diğer cevap türleri
