 ...
 // Helper Methods
    ...
    public function StoreCustomer($tenant_id, Request $request) {
        ...
        $customer = Customer::create([
            ...
            // 2. Verimor entegrasyonunu çalıştır
    try {
        // Telefon numarası var mı diye kontrol et (boşsa gönderme)
        if (!empty($customer->tel1)) {
            $verimorService = new VerimorSantralService($tenant_id);
            $result = $verimorService->addCallerId($customer->adSoyad, $customer->tel1);

            // 3. (Opsiyonel ama Önerilir) Sonucu kontrol et ve logla
            if (!$result['success']) {
                Log::warning('Verimor rehberine kayıt başarısız oldu.', [
        ...
        // Servis sınıfında bir hata olursa sistemin çökmesini engelle
        Log::error('Verimor servisi çağrılırken kritik bir hata oluştu.', [
            'customer_id' => $customer->id,
