...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    {
        ...
        // Eğer bu konsinye cihazsa (kategori 3)
        if ($stok && $stok->urunKategori == 3) {
            // Konsinye cihaz iade işlemi yap
            $this->konsinyeCihazIadeEt($stokId, $adet, $servisId, $planId, $tenantId);
            return; // konsinye cihazlar için aşağıdaki işlemleri yapma
        }
 ...
 private function konsinyeCihazIadeEt($stokId, $adet, $servisId, $planId, $tenantId)
{
    // Önceki planı bul
    $sonPlan = ServicePlanning::where('servisid', $servisId)
        ->orderBy('id', 'desc')
    ...
    // 1. Önce bu konsinye cihazın gerçekten daha önce bu serviste kullanılıp kullanılmadığını kontrol et
    $konsinyeKullanim = StockAction::where('firma_id', $tenantId)
        ->where('stokId', $stokId)
        ->where('servisid', $servisId)
        ->where('islem', 2) // Serviste kullanım
        ->sum('adet');

    if ($konsinyeKullanim < $adet) {
        throw new \Exception("Bu konsinye cihazdan bu serviste sadece {$konsinyeKullanim} adet kullanılmış. {$adet} adet iade edilemez.");
    }

    // 2. Önceki aynı planId için iade kayıtlarını sil (güncelleme durumunda)
    StockAction::where('firma_id', $tenantId)
        ->where('stokId', $stokId)
        ->where('planId', $planId)
        ->where('islem', 4) // iade işlemi
        ->delete();

    // 3. Konsinye cihaz iade işlemi için StockAction kaydı oluştur
    StockAction::create([
        'firma_id' => $tenantId,
        'stokId' => $stokId,
        'islem' => 4, // Müşteriden İade (Konsinye için giriş)
        'adet' => $adet,
        'servisid' => $servisId,
        'fiyat' => 0,
        'fiyatBirim' => 1,
        'planId' => $planId,
        'personel' => $sonPlan->pid ?? $sonPlan->kid ?? auth()->id(),
        'kid' => auth()->id(),
        'depo' => 1,
        'created_at' => now(),
        'updated_at' => now()
    ...
    // 4. ServiceStageAnswer kaydı oluştur
    $kid = auth()->user()->user_id;
    ServiceStageAnswer::create([
        'firma_id' => $tenantId,
        'servisid' => $servisId,
        'planid' => $planId,
        'soruid' => $this->getCurrentSoruId(), // Bu değeri parametre olarak geçmeniz gerekebilir
        'cevap' => "{$stokId}---{$adet}", 
        'kid' => $kid,
        'created_at' => now(),
        'updated_at' => now()
    ...
    // 5. Log kaydı (isteğe bağlı)
    Log::info("Konsinye cihaz iade edildi", [
        'stok_id' => $stokId,
        'adet' => $adet,
        'servis_id' => $servisId,
        'plan_id' => $planId,
...
private function konsinyeCihazGeriAl($stokId, $adet, $servisId, $planId, $tenantId, $soruId)
{
    // Stok kaydını al
    $stok = Stock::where('id', $stokId)->first();
    ...
    // Bu konsinye cihazın bu serviste kullanılıp kullanılmadığını kontrol et
    $konsinyeKullanim = StockAction::where('firma_id', $tenantId)
        ->where('stokId', $stokId)
        ->where('servisid', $servisId)
        ->where('islem', 2) // Serviste kullanım
    ...
    // Daha önce iade edilen miktarı hesapla
    $konsinyeIade = StockAction::where('firma_id', $tenantId)
        ->where('stokId', $stokId)
        ->where('servisid', $servisId)
        ->where('islem', 4) // İade işlemi
        ->sum('adet');

    // Geri alınabilir maksimum miktarı hesapla
    $geriAlinabilirMiktar = $konsinyeKullanim - $konsinyeIade;

    if ($adet > $geriAlinabilirMiktar) {
        throw new \Exception("Bu konsinye cihazdan bu serviste sadece {$geriAlinabilirMiktar} adet geri alınabilir. {$adet} adet istendi.");
    }

    // Önceki aynı planId için iade kayıtlarını sil (güncelleme durumunda)
    StockAction::where('firma_id', $tenantId)
        ->where('stokId', $stokId)
        ->where('planId', $planId)
        ->where('islem', 4) // iade işlemi
    ...
    // Konsinye cihaz iade işlemi için StockAction kaydı oluştur
    StockAction::create([
        'firma_id' => $tenantId,
        'kid' => auth()->id(),
        'stokId' => $stokId,
