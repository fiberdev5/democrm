...
if ($isBayi) {
    // Bayi: formdan gelen personel ID'yi al (veya servis kayıtlarından)
    if ($request->filled('personel')) {
        $personelIdToAssign = (int)$request->input('personel');
    } elseif ($personelYapilanServis) {
        $personelIdToAssign = (int)$personelYapilanServis->cevap;
    }
} else {
    // Merkez personeli: kendi ID'sini kullan
    $personelIdToAssign = $user->id;
}
