...
class InvoicesController extends Controller
{
    ...
    'odemeDurum' => $invoice->odemeDurum, // ✅ Eklendi
    'toplamTutar' => $invoice->genelToplam, // ✅ Eklendi
    'genelToplam' => $invoice->genelToplam, // ✅ Eklendi
    'kasaId' => $request->input('kasa') ?? 1, // ✅ Eklendi
    ...
    $invoice->update([
        'faturaDurumu' => 'sent',
        'integration_invoice_id' => $result['invoice_number'],
    ...
    if ($result['payment_added'] ?? false) {
        $integrationMessage = ' Fatura Paraşüt\'e gönderildi, mahsuplaştırıldı ve PDF oluşturuldu!';
    } elseif ($result['pdf_path']) {
        $integrationMessage = ' Fatura Paraşüt\'e gönderildi ve PDF oluşturuldu!';
    } else {
        $integrationMessage = ' Fatura Paraşüt\'e gönderildi. PDF için lütfen Paraşüt\'te mahsuplaştırma yapın.';
    }
    
    Log::info('Fatura entegrasyona başarıyla gönderildi', [
        'invoice_id' => $invoice->id,
        'integration_invoice_id' => $result['invoice_id'],
        'payment_added' => $result['payment_added'] ?? false, // ✅ Eklendi
