...
public function TechnicianStatisticData(Request $request, $tenant_id)
{
    $startDate = \Carbon\Carbon::createFromFormat('d/m/Y', $request->tarih1)->startOfDay();
    $endDate   = \Carbon\Carbon::createFromFormat('d/m/Y', $request->tarih2)->endOfDay();
    $cihazTur  = $request->cihazTur;

    // Teknisyen listesi (grup 244: teknisyen)
    ...
    foreach ($teknisyenler as $personel) {
        $servisler = Service::where('personel', $personel->user_id)
            ->where('tenant_id', $tenant_id)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->when($cihazTur, function ($query) use ($cihazTur) {
                return $query->where('cihazTur', $cihazTur);
            })
            ->get();

        $servisCount = $servisler->count();
        $iptal = $servisler->where('servisDurum', 23)->count();
        $tamam = $servisler->where('servisDurum', 22)->count();
        $beklemede = $servisler->whereNotIn('servisDurum', [22, 23])->count();
        $sikayet = $servisler->where('sikayet', 1)->count();
        $odeme = $servisler->where('odemeDurumu', 1)->count();
        $teklif = $servisler->where('teklifDurumu', 1)->count();

        $result[] = [
            'adsoyad'     => $personel->name . ' ' . $personel->surname,
            'servis'      => $servisCount,
            'tamam'       => $tamam,
            'iptal'       => $iptal,
            'beklemede'   => $beklemede,
