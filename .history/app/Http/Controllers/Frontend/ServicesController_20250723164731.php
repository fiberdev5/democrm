...
class ServicesController extends Controller
{
    ...
    if (!empty($filters['anketi_yapilmayan_personel']) && $filters['anketi_yapilmayan_personel'] != '0') {
        $personelId = $filters['anketi_yapilmayan_personel'];
        $data->whereDoesntHave('surveys', function ($query) use ($personelId) {
            $query->where('personel', $personelId);
        });
    } else {
        $data->whereDoesntHave('surveys');
    }

    if (!empty($filters['bayiler']) && $filters['bayiler'] != '0') {
        $bayiId = $filters['bayiler'];
        $data->where('bayi', $bayiId);
    }
