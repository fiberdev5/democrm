     ...
     private function stokKontrolEt(Request $request, $gelenIslem)
{
    foreach ($request->input('parca', []) as $soruId => $parcalar) {
        foreach ($parcalar as $idWithPrefix => $value) {
            // Konsinye cihaz kontrolü
            if (strpos($idWithPrefix, 'C_') === 0) {
                $konsinyeId = (int) str_replace('C_', '', $idWithPrefix);
                $adet = abs($request->input("adet.{$soruId}.C_{$konsinyeId}", 0));

                $device = Stock::where('id', $konsinyeId)->where('urunKategori', 3)->first();
                if (!$device) {
                    return "STOKHATA: Konsinye Cihaz bulunamadı (ID: {$konsinyeId}).";
                }

                // Konsinye cihaz için anlık stok hesapla
                $girisAdet = StockAction::where('stokId', $konsinyeId)
                                         ->whereIn('islem', [1, 4])
                                         ->sum('adet');
                $cikisAdet = StockAction::where('stokId', $konsinyeId)
                                         ->where('islem', 2)
                                         ->sum('adet');
                $currentStock = $girisAdet - $cikisAdet;

                if ($currentStock <= 0 || $adet > $currentStock) {
                    return "STOKHATA: " . mb_convert_case($device->name ?? $device->urunAdi, MB_CASE_TITLE, "UTF-8") . " (Konsinye Cihaz) Stok Adeti Yetersizdir.";
                }
            } 
            // Normal personel stok kontrolü
            else {
                $stokId = (int) $idWithPrefix; // Bu doğrudan stok ID'si olmalı
                $adet = abs($request->input("adet.{$soruId}.{$stokId}", 0));

                $stok = Stock::where('id', $stokId)->first(); // Bu Stock modeliniz olmalı
                if (!$stok) {
                    return "STOKHATA: Stok ürünü bulunamadı (ID: {$stokId}).";
                }

                // Personel stoğu (PersonelStock tablosundan)
                $personelStok = PersonelStock::where('stokid', $stokId)
                                             ->where('pid', auth()->user()->user_id)
                                             ->first();
                
                if (!$personelStok || $personelStok->adet <= 0 || $adet > $personelStok->adet) {
                    return "STOKHATA: " . mb_convert_case($stok->urunAdi ?? $stok->urun_adi, MB_CASE_TITLE, "UTF-8") . " (Personel Stoğu) Adeti Yetersizdir.";
                }
            }
        }
    }
    ...
    // Bu kontrolü, hem personel stoğu hem de konsinye cihazlar için geçerli olacak şekilde güncelleyelim.
    if ($gelenIslem['cevapTuru'] == "[Parca Teslim Et]") { // gelenIslem artık bir nesne, id yerine cevapTuru kullanıyoruz.
        $stokSecildi = false;
