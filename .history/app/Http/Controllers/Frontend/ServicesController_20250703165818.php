...
private function soruCevaplariniIsle(Request $request, $servisId, $planId, $tenantId, $gelenIslem)
{
    if ($request->has('soru')) {
        foreach ($request->input('soru') as $soruId => $cevap) {
            if ($cevap == "Parca") {
                // Parça işlemleri (mevcut haliyle kalabilir)
                ...
                $consignmentName = $request->input("parca.{$soruId}.consignment_name"); // Yeni eklenen kısım: Cihaz adını al
                ...
                if ($selectedConsignment && strpos($selectedConsignment, 'C_') === 0) {
                    ...
                    ServiceStageAnswer::create([
                        ...
                        'planid' => $planId,
                        'soruid' => $soruId,
                        // JSON formatını güncelliyoruz: cihazin_adi anahtarını ekledik
                        'cevap' => json_encode([
                            'type' => 'consignment_device',
                            'stok_id' => $stokId,
                            'adet' => $adet,
                            'cihazin_adi' => $consignmentName // Adı buraya kaydet
                        ]),
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                ...
                // Diğer cevap türleri (mevcut haliyle kalabilir)
                ServiceStageAnswer::create([
                    'firma_id' => $tenantId,
                    'servisid' => $servisId,
                    'planid' => $planId,
