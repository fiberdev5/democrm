...
class ServicesController extends Controller
{
    public function AllServices($tenant_id, Request $request) {
        ...
        if ($request->ajax()) {           
            $data = Service::with(['musteri', 'markaCihaz', 'turCihaz', 'asamalar'])
    ...
    if ($request->filterType && $request->filters) {
        ...
        switch ($request->filterType) {
            case 'operator':
                ...
                if (!empty($filters['operator_tarih1']) && !empty($filters['operator_tarih2'])) {
                    $data->whereBetween('created_at', [$filters['operator_tarih1'], $filters['operator_tarih2']]);
                }
                break;
        ...
        return view('frontend.secure.all_services.services', compact('services', 'device_brands', 'device_types', 'service_stages','firma', 'service_resources','states','operators'));
    }
