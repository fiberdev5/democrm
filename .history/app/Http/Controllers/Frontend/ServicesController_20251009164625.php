...
if ($request->ajax()) {
    ...
    return DataTables::of($data)
        ...
        ->filter(function ($instance) use ($request) {
                ...
                if (!empty($search['value'] ?? null)) {
                    $searchValue = $search['value'];
                    $instance->where(function ($w) use ($searchValue) {
                        $w->where('id', 'LIKE', "%$searchValue%")
                        ->orWhereHas('musteri', function ($q) use ($searchValue) {
                            $q->where('adSoyad', 'LIKE', "%$searchValue%")
                                ->orWhere('tel1', 'LIKE', "%$searchValue%");
                        });
                    });
                }
            }, true)
...
{
    // 🔥 ARAMA YAPILIYORSA TARİH FİLTRESİ UYGULAMA
    $search = $request->input('search');
    if (!empty($search['value'] ?? null)) {
        return; // Arama varsa, direkt çık - tüm kayıtlarda ara
    }
