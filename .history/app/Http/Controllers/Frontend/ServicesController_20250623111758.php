...
class ServicesController extends Controller
{
    ...
    private function getServisDetay($tenant_id, $servisId)
    {
        ...
        foreach ($paraIslemler as $paraIslem) {
            ...
            $personel = User::where('tenant_id', $tenant_id)->where('id', $paraIslem->pid)->first();
    ...
    public function ServiceMoneyActions($tenant_id, $service_id) {
        ...
        return view('frontend.secure.all_services.service_money_actions.service_money_actions', compact('firma', 'servis'));
    }
    ...
    public function StoreServiceIncome($tenant_id, Request $request) {
        // Temel verileri al
        $servisid = trim($request->input('servisid'));
        ...
        if (auth()->user()->hasRole('Patron')) {
            $tarih = Carbon::createFromFormat('Y-m-d H:i:s', $request->input('tarih') . ' ' . now()->format('H:i:s'));
        ...
        $sonuc = ServiceMoneyAction::where('firma_id', $tenant_id)->insertGetId($data);

        if ($sonuc) {
            // kasa_hareketleri için veri hazırlığı
            $kasaData = [
                'kid'          => session('kid'),
                'islemTarihi'  => session('grup') == '242' ? $tarih : now(),
                'odemeYonu'    => 1,
                'odemeSekli'   => trim($request->input('odemeSekli')),
                'odemeDurum'   => trim($request->input('odemeDurum')),
                'fiyat'        => $fiyat,
                'fiyatBirim'   => 1,
                'aciklama'     => trim($request->input('aciklama')),
                'marka'        => trim($request->input('markaid')),
                'cihaz'        => trim($request->input('cihazid')),
                'servis'       => $servisid,
                'servisIslem'  => $sonuc,
            ];
            ...
            $odemeTuru = DB::table('odeme_turleri')->where('servis', 1)->first();
            if ($odemeTuru) {
                $kasaData['odemeTuru'] = $odemeTuru->id;
            }

            // kasa_hareketleri tablosuna ekle
            $kasaID = DB::table('kasa_hareketleri')->insertGetId($kasaData);
    ...
    public function AddServiceExpense($tenant_id, $service_id) {
        ...
        return view('frontend.secure.all_services.service_money_actions.add_service_expense', compact('firma', 'servis'));
    }
