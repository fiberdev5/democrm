...
// Ürün adı kontrolü
$existingStockByName = Stock::where('firma_id', $tenant_id)
                           ->where('urunAdi', $request->urunAdi)
                           ->first();

if ($existingStockByName) {
    // Yönlendirme: route parametre isimleri route tanımıyla aynı olmalı
    return redirect()->route('edit.stock', [
        'tenant_id' => $tenant_id,
        'id' => $existingStockByName->id  // burası 'id' olmalı
    ])->with([
        'message' => 'Bu ürün adı zaten mevcut. Var olan ürün sayfasına yönlendiriliyorsunuz.',
        'alert-type' => 'info'
    ]);
}
