...
public function UpdateInvoice(Request $request) {
    ...
    try {
        ...
        // Mevcut fatura PDF yolunu al (eğer yeni dosya yüklenmemişse aynı kalacak)
        $currentInvoicePath = $invoice->faturaPdf;
        ...
        // ÖNEMLİ: Önce tüm eski ödeme kayıtlarını temizle
        foreach ($oldPaymentDetails as $oldPayment) {
            $this->clearPaymentInvoicePath($oldPayment['payment_type'], $oldPayment['payment_id']);
        }

        // Sonra yeni ödeme kayıtlarını güncelle
        foreach ($paymentDetails as $paymentDetail) {
            $this->updatePaymentInvoicePath(
                $paymentDetail['payment_type'], 
                $paymentDetail['payment_id'], 
                $currentInvoicePath // Mevcut fatura PDF'ini kullan
            );
        }
...
// Geliştirilmiş clearPaymentInvoicePath method'u
private function clearPaymentInvoicePath($paymentType, $paymentId)
{
    try {
        if ($paymentType === 'subscription') {
            ...
            if ($payment) {
                // Debug için log ekleyebilirsin
                \Log::info("Subscription payment $paymentId invoice_path temizlendi: " . $payment->invoice_path);
                $payment->update(['invoice_path' => null]);
            }
        } elseif ($paymentType === 'storage') {
            $payment = StoragePurchase::find($paymentId);
            if ($payment) {
                // Debug için log ekleyebilirsin
                \Log::info("Storage payment $paymentId invoice_path temizlendi: " . $payment->invoice_path);
                $payment->update(['invoice_path' => null]);
            }
        }
    } catch (\Exception $e) {
        \Log::error("clearPaymentInvoicePath error: " . $e->getMessage());
    }
}

// Geliştirilmiş updatePaymentInvoicePath method'u
private function updatePaymentInvoicePath($paymentType, $paymentId, $invoicePath)
{
    try {
        if ($paymentType === 'subscription') {
            $payment = SubscriptionPayment::find($paymentId);
            if ($payment) {
                \Log::info("Subscription payment $paymentId yeni invoice_path: " . $invoicePath);
                $payment->update(['invoice_path' => $invoicePath]);
            }
        } elseif ($paymentType === 'storage') {
            $payment = StoragePurchase::find($paymentId);
            if ($payment) {
                \Log::info("Storage payment $paymentId yeni invoice_path: " . $invoicePath);
                $payment->update(['invoice_path' => $invoicePath]);
            }
        }
    } catch (\Exception $e) {
        \Log::error("updatePaymentInvoicePath error: " . $e->getMessage());
    }
}
