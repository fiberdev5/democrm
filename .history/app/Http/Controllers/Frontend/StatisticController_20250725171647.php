...
class StatisticController extends Controller
{
   ...
   private function getFilteredStatistics($request, $tenant_id, $servisKaynaklari)
{
    // 1. Sadece Patron (1) ve Operatör (5) rolündekileri çek
    $personeller = User::where('tenant_id', $tenant_id)
        ->whereNull('ayrilmaTarihi') // sadece aktif olanlar
        ->whereHas('roles', function ($query) {
            $query->whereIn('id', [1, 5]); // 1 = Patron, 5 = Operatör
        })
        ->orderBy('name')
        ->get();
