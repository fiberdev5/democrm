...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        // Konsinye cihazların toplam adedini hesapla (Bu kısım front-end'de direk kullanılmadığı için gereksiz olabilir)
        $toplamConsignmentAdedi = 0;
    ...
    /**
     * Konsinye cihaz stoğunu günceller (stok hareketi oluşturur).
     * Log çağrıları kaldırılmıştır.
     */
    ...
    // Mevcut updateParcaSelection ve getUpdatedParcaAnswer fonksiyonları aynen kalabilir
    private function updateParcaSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
    {
        ...
        foreach ($request->all() as $key => $value) {
            if (strpos($key, 'stokCheck') !== false) {
                ...
                if ($servisPlan && $servisPlan->gelenIslem == "238") {
                    $this->parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId);
                } else {
                    $this->parcaKullan($stokId, $adet, $servisId, $planId, $tenantId);
                }
