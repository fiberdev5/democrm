...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        // Yeni yaklaşım: Hem PersonelStock ilişkisine sahip olanları
        // hem de doğrudan Stock tablosunda pid'si olanları getir.
        $stoklar = Stock::where('firma_id', $tenant_id)
                        ->where(function($query) use ($personel_id) {
                            $query->whereHas('personelStoklar', function($q) use ($personel_id) {
                                $q->where('pid', $personel_id);
                            })->orWhere('pid', $personel_id); // Doğrudan Stock tablosundaki pid'yi kontrol et
                        })
                        ->orderBy('id', 'ASC')
                        ->get();
    }

    // Toplam personel stok adedini hesapla
    // NOT: Bu hesaplama, yukarıdaki $stoklar koleksiyonundan doğru şekilde yapılmalı
    // Veya PersonelStock tablosundan ayrı olarak toplanmalıdır.
    // Eğer $stoklar sadece Stock modellerini içeriyorsa, adedi manuel olarak eklemeniz gerekir.
    // Şimdilik PersonelStock tablosundan toplamayı tercih edelim, daha güvenli.
    $toplamPersonelStokAdedi = PersonelStock::where('firma_id', $tenant_id)
                                            ->where('pid', $personel_id)
                                            ->sum('adet');

    // Eğer Stock tablosundaki pid'den gelen stoklar için de adet kontrolü yapmanız gerekiyorsa,
    // Stock modelinize 'adet' sütununu ekleyebilir veya stok hareketlerinden hesaplayabilirsiniz.
    // Şu anki Stock tablonuzda 'adet' sütunu yok gibi görünüyor.
    // Bu durumda, sadece PersonelStock tablosundaki adetler üzerinden toplama yapmak en mantıklısı.
