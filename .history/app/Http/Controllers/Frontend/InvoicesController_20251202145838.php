    ...
    public function AllInvoice(Request $request, $tenant_id) {
        ...
        // herhangi bir tarih kısıtlaması uygulanmaz, bu da tüm kayıtlarda arama yapılmasını sağlar.
            ...
            ->addColumn('actions', function($row){ 
                ...
                $tenant = Tenant::find($row->firma_id);
                
                // Paraşüt entegrasyonunu kontrol et (Integration modelinden)
                $parasutIntegration = Integration::where('slug', 'parasut')->first();
                $hasParasutIntegration = $tenant && $parasutIntegration && $tenant->hasIntegration($parasutIntegration->id);
                
                // Paraşüt entegrasyonu varsa Resmileştir butonu, yoksa PDF görüntüleme butonu
                if($hasParasutIntegration) {
                    $earsivButton = '<a href="javascript:void(0);" data-invoice-id="'.$row->id.'" class="btn btn-outline-primary btn-sm formalizeInvoice mobilBtn mbuton1" title="Resmileştir"><i class="fas fa-paper-plane"></i> </a>';
                } else {
