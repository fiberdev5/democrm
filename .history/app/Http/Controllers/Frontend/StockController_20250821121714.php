...
public function StoreStockAction(Request $request, $tenant_id)
{
    ...
    if ($request->islem == 3) {
        ...
        $detayText = '-';
        if ($request->islem == 1 && $request->filled('tedarikci')) {
            $tedarikci = \App\Models\StockSupplier::find($request->tedarikci);
            $detayText = $tedarikci ? $tedarikci->tedarikci : '';
        } elseif ($request->islem == 3 && $request->filled('personel')) {
            $personel = \App\Models\User::find($request->personel);
            $detayText = $personel ? $personel->name : '';
        }

        // Frontend'e gönderilecek veriyi oluştur
        $yeniHareketVerisi = [
            'id' => $stockAction->id,
            'islem' => $stockAction->islem,
            'islem_adi' => $stockAction->islem == 1 ? 'Alış' : ($stockAction->islem == 3 ? 'Personel Depo' : '-'),
            'tarih' => $stockAction->created_at->format('d/m/Y'),
            'adet' => $stockAction->adet,
            'fiyat' => $stockAction->islem == 1 && $stockAction->fiyat > 0 ? number_format($stockAction->fiyat, 2, '.', '') . ' TL' : '-',
            'detay' => $detayText
        ];
        
        // REDIRECT YERİNE JSON CEVABI DÖNÜYORUZ
        return response()->json([
            'status' => 'success',
            'message' => 'Stok hareketi başarıyla eklendi.',
            'data' => $yeniHareketVerisi // Yeni veriyi frontend'e gönder
        ]);
    }
 catch (\Exception $e) {
        return response()->json([
