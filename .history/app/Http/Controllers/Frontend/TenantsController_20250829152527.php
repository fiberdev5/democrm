...
class TenantsController extends Controller
{
               ...
               ->addColumn('action', function($row){
                    ...
                    if ($row->status == 1) {
                        ...
                        if ($tenantOwner) {
                            ...
                            // Patron yoksa, Müdür rolündeki kullanıcıyı bul
                            $manager = \App\Models\User::where('tenant_id', $row->id)
                                                    ->whereHas('roles', function($query) {
                                                        $query->where('name', 'Müdür');
                                                    })
                                                    ->first();
                            
                            if ($manager) {
                                $impersonateButton = '<button class="btn btn-info btn-sm mobilBtn impersonate-tenant-owner me-1" 
                                                            data-tenant-id="'.$row->id.'" 
                                                            data-owner-id="'.$manager->user_id.'"
                                                            data-owner-name="'.$manager->name.'"
                                                            data-company-name="'.$row->firma_adi.'"
                                                            title="Firma Müdürü Olarak Giriş Yap">
    ...
    public function EditTenant($tenant_id, $id) {
        ...
        $topServisSayisi = $servisler = Service::where('firma_id', $id) 
            ->where('durum', 1)
            ->count();
        return view('frontend.secure.super_admin.edit_tenants', compact('tenant','countries','firma', 'periodStats','topServisSayisi'));
