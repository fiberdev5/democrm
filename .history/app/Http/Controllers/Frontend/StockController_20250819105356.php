...
public function AllStocks($tenant_id, Request $request)
{
 ...
 public function EditStock($tenant_id, $id) {
    ...
    $sonTedarikciler =StockSupplier::where('firma_id', $tenant_id) // firma_id'ye göre filtrelemek daha doğru olur
    ...
    $sonPersoneller =User::where('tenant_id', $tenant_id)
                                        ->latest('created_at') // Veya created_at, modelinize bağlı
                                        ->take(5)
                                        ->get();
...
public function searchSuppliers(Request $request, $tenant_id)
{
    ...
    $data =StockSupplier::where('firma_id', $tenant_id)
                                     ...
                                     ->latest()
                                     ->take(5) // Arama sonucunda en fazla 10 sonuç göster
...
public function deletePhoto(Request $request, $tenant_id)
{
    try {
        ...
        // DEĞİŞEN VE DOĞRU OLAN KISIM BURASI
        // public_path() ve unlink() yerine Storage fasadını kullanıyoruz.
        // Veritabanındaki yolu doğrudan Storage::delete metoduna veriyoruz.
        if (Storage::disk('public')->exists($photo->resimyol)) {
            Storage::disk('public')->delete($photo->resimyol);
        }

        // Veritabanından fotoğraf kaydını sil
        $photo->delete();
        ...
        // Hata ayıklama için loglama eklemek çok faydalıdır.
        \Log::error('Fotoğraf silme hatası: ' . $e->getMessage());

        return response()->json([
            'message' => 'Fotoğraf silme işlemi sırasında bir hata oluştu.',
            'alert_type' => 'danger'
        ], 500);
    }
}
