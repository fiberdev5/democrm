...
{
    $dates = explode('---', $request->personelTabloGetir);
...
private function getServicePlansForDateRange($dateRange, $tenant_id, $deviceTypeId = null)
{
    ...
    $query = ServiceStageAnswer::select([
            ...
            'service_stage_answers.service_id',
            'service_stage_answers.plan_id',
            'service_stage_answers.question_id',
            'service_stage_answers.answer',
            'stage_questions.answer as question_answer',
            'services.device_type_id'
        ])
        ->leftJoin('stage_questions', 'stage_questions.id', '=', 'service_stage_answers.question_id')
        ->leftJoin('services', 'services.id', '=', 'service_stage_answers.service_id')
        ->where('stage_questions.answer', 'LIKE', '%Tarih%')
        ->where('services.tenant_id', $tenant_id)
        ->whereIn('service_stage_answers.answer', $dateConditions);

    if ($deviceTypeId) {
        $query->where('services.device_type_id', $deviceTypeId);
    }

    $results = $query->get();

    // Sadece belirli soru ID'lerini filtrele (eski sistemdeki soru ID'leri yerine yeni sistem mantığı)
    $validQuestionIds = [296, 326, 369, 306, 337, 341]; // Bu ID'leri yeni sisteme göre güncellemelisiniz
    
    return $results->where('question_id', 'in', $validQuestionIds)->pluck('plan_id')->toArray();
}
