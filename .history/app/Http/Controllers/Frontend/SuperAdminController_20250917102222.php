   ...
   public function allTenants(Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        $data = Tenant::with(['ils', 'ilces', 'currentSubscription.plansubs'])
        ...
        return DataTables::of($data)
            ...
            ->addColumn('plan', function($row) {
                ...
                if ($row->currentSubscription && $row->currentSubscription->plansubs) {
                    $planName = $row->currentSubscription->plansubs->plan_name ?? 'Plan';
                    
                    // Plan durumuna göre badge rengi
                    if ($row->subscription_status === 'active') {
                        $badgeClass = 'bg-success';
                    } elseif ($row->subscription_status === 'trial') {
                        $badgeClass = 'bg-info';
                        $planName .= ' (Deneme)';
                    } elseif ($row->subscription_status === 'expired') {
                        $badgeClass = 'bg-danger';
                        $planName .= ' (Süresi Dolmuş)';
                    } elseif ($row->subscription_status === 'suspended') {
                        $badgeClass = 'bg-warning';
                        $planName .= ' (Askıya Alınmış)';
                    }
                ...
                return '<a class="t-link editTenant" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editTenantModal">
                           ...
                           <span class="badge '.$badgeClass.'">'.$planInfo.'</span>
                        </a>';
            })
            ->addColumn('plan_end_date', function($row) {
                ...
                // Eğer aktif abonelik varsa, abonelik bitiş tarihini kontrol et
                if ($row->currentSubscription && $row->currentSubscription->ends_at) {
                ...
                return '<a class="t-link editTenant" href="javascript:void(0);" data-bs-id="'.$row->id.'" data-bs-toggle="modal" data-bs-target="#editTenantModal">
                           ...
                           <span class="'.$textClass.'">'.$endDate.'</span>
                        </a>';
            })
            ...
            ->rawColumns(['id','name','plan','plan_end_date','durum','action'])
            ->make(true);
    }
