...
public function CashStatistics($tenant_id) {
    ...
    // 2. Firma ID'yi farklı yollardan almaya çalış
    $firma_id = null;
    
    // Yöntem 1: User'dan
    if (isset($user->firma_id) && !empty($user->firma_id)) {
        $firma_id = $user->firma_id;
        \Log::info('Firma ID from user: ' . $firma_id);
    }
    
    // Yöntem 2: Session'dan
    if (empty($firma_id) && session()->has('firma_id')) {
        $firma_id = session('firma_id');
        \Log::info('Firma ID from session: ' . $firma_id);
    }
    
    // Yöntem 3: Tenant ID'den
    if (empty($firma_id) && !empty($tenant_id)) {
        $firma_id = $tenant_id;
        \Log::info('Using tenant_id as firma_id: ' . $firma_id);
    }
    
    
    
    // 3. Bu firma_id ile veri var mı kontrol et
    $firmaRecords = CashTransaction::where('firma_id', $firma_id)->get();
    \Log::info('Records for firma_id ' . $firma_id . ': ' . $firmaRecords->count());
    
    if ($firmaRecords->count() > 0) {
        \Log::info('Sample records for firma_id ' . $firma_id . ':', $firmaRecords->take(2)->toArray());
    }
    
    // 4. Eğer hala veri yoksa, tüm firma_id'leri listele
    if ($firmaRecords->count() == 0) {
        $allFirmaIds = CashTransaction::distinct()->pluck('firma_id')->toArray();
        \Log::info('Available firma_id values in database:', $allFirmaIds);
        
        // İlk mevcut firma_id'yi kullan
        if (!empty($allFirmaIds)) {
            $firma_id = $allFirmaIds[0];
            \Log::info('Using first available firma_id: ' . $firma_id);
        }
    }
    
    // Mevcut ay için kasa hareketlerini getir
