    ...
    {  
        ...
        if (!empty($persID)) {
            ...
            $selectedDates = ServiceStageAnswer::where('cevap', $today)
                ...
                ->where('soruid', 48)
                ->pluck('planid');
            Log::info('selectedDates:', ['count' => $selectedDates->count(), 'dates' => $selectedDates->toArray()]);
            $serviceAnswers = ServiceStageAnswer::where('cevap', $persID)
                ...
                ->pluck('servisid');
                Log::info('serviceAnswers:', ['count' => $serviceAnswers->count(), 'services' => $serviceAnswers->toArray()]);
            ...
            try {
                ...
                // İsteğe bağlı: log veya hata mesajı
                // Log::warning('Tarih format hatası: '.$e->getMessage());
            }
        }
