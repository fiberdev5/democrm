    ...
    public function SurveyStore(Request $request, $tenant_id, $servisId)
    {
        ...
        try {
            ...
            if ($personelYapilanServis) {
                // Eğer personel bilgisi ServiceStageAnswer'da varsa, onu kullan.
                // Hangi sütunda tutulduğuna göre `cevap` veya `cevapText` seçimi yapın.
                // Genellikle ID'ler `cevap` sütununda tutulur.
                $personelIdToAssign = $personelYapilanServis->cevap; 
                // Eğer cevap string gelirse, integer'a cast etmek faydalı olabilir: (int)$personelYapilanServis->cevap
            }
            ...
            // Atama yapmadan önce $personelIdToAssign'in hala null olup olmadığını kontrol edebilirsiniz.
            // Eğer hala null ise ve zorunlu ise hata döndürülebilir veya bir varsayılan atanabilir.
            if (is_null($personelIdToAssign)) {
                // Bu durum, ne ServiceStageAnswer'da ne de formda personel bulunamadığı anlamına gelir.
                // Duruma göre bir hata mesajı döndürebilir veya Auth::id() gibi bir fallback kullanabilirsiniz.
                // Şu an için bir hata döndürmek en güvenlisidir.
                return response()->json(['success' => false, 'error' => 'Anketi yapılan personel belirlenemedi.'], 400);
            }
