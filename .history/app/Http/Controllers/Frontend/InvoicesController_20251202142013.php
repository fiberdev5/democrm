...
class InvoicesController extends Controller
{
    ...
    // Firma için Paraşüt entegrasyonunu kontrol et
    $hasParasutIntegration = \App\Models\Integration::where('firma_id', $row->firma_id)
        ->where('integration_type', 'parasut')
        ->where('is_active', 1)
        ->exists();
    
    // Paraşüt entegrasyonu varsa Resmileştir butonu, yoksa PDF görüntüleme butonu
    if($hasParasutIntegration) {
        $earsivButton = '<a href="javascript:void(0);" data-invoice-id="'.$row->id.'" class="btn btn-outline-success btn-sm formalizeInvoice mobilBtn mbuton1" title="Paraşütte Resmileştir"><i class="fas fa-file-invoice"></i> Resmileştir</a>';
    } else {
        $earsivButton = '<a href="'.asset($row->faturaPdf).'" target="_blank" class="btn btn-outline-primary btn-sm mobilBtn mbuton1" title="Faturayı görüntüle"><i class="fas fa-paper-plane"></i></a>'; 
    }
