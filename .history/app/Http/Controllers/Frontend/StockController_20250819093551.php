...
public function searchSuppliers(Request $request, $tenant_id)
{
    ...
    $query = StockSupplier::where('firma_id', $tenant_id);
    
    if (!empty($search)) {
        $query->where('tedarikci', 'LIKE', '%' . $search . '%');
    }
    
    $total = $query->count();
    $suppliers = $query->orderBy('tedarikci')
...
public function searchPersonel(Request $request, $tenant_id)
{
    $search = $request->get('q', '');
    $page = $request->get('page', 1);
    $perPage = 30;
    ...
    $total = $query->count();
    $personel = $query->orderBy('name')
                     ->offset(($page - 1) * $perPage)
                     ->limit($perPage)
                     ->get();
    
    $items = $personel->map(function($person) {
        return [
            'id' => $person->user_id,
            'text' => $person->name
        ];
 ...
 public function EditStock($tenant_id, $id) {
    ...
    // 3. FOTOĞRAFLARI ÇEK
    $photos = stock_photos::where('kid', $tenant_id) // kid yerine firma_id daha mantıklı olabilir, modelinize bağlı
                          ->where('stock_id', $id)
                          ->latest()
                          ->limit(1) // Sadece 1 fotoğraf
                          ->get();

    //SON 5 TEDARİKÇİ VE PERSONEL (Select2 için başlangıç verileri)
    $recentSuppliers = StockSupplier::where('firma_id', $tenant_id)
                                   ->orderBy('created_at', 'desc')
                                   ->limit(5)
                                   ->get();
    
    $recentPersonel = User::where('tenant_id', $tenant_id)
                         ->orderBy('created_at', 'desc')
                         ->limit(5)
                         ->get();
    ...
    $html = view('frontend.secure.stocks.edit_stock', compact('firma', 'stock', 'rafListesi', 'markalar',
        ...
        'hareketler',  
        'photos'  ,'tenant_id' ,'recentSuppliers', 'recentPersonel'))->render();
...
public function getPhotos($tenant_id, $stock_id)
{
    $photos = stock_photos::where('kid', $tenant_id)
                        ...
                        ->latest()
                        ->limit(1) // Sadece 1 fotoğraf getir
                        ->get();
...
public function uploadPhoto(Request $request, $tenant_id)
{
    $request->validate([
        ...
        // Mevcut fotoğraf kontrolü - sadece 1 fotoğraf izin ver
        $existingPhotos = stock_photos::where('kid', $tenant_id)
        ...
        if ($existingPhotos > 0) {
            return response()->json([
                'success' => false,
                'message' => 'Bu ürün için zaten bir fotoğraf mevcut. Önce mevcut fotoğrafı silmeniz gerekiyor.',
                'alert_type' => 'warning'
            ], 400);
        }
...
public function replacePhoto(Request $request, $tenant_id)
{
    try {
        $request->validate([
            'resim' => 'required|file|mimes:jpg,jpeg,png|max:5120',
        ...
        $existingPhoto = stock_photos::where('kid', $tenant_id)
                                    ->where('stock_id', $stockId)
                                    ->first();
        
        if ($existingPhoto) {
            // Storage'dan sil
            if (Storage::disk('public')->exists($existingPhoto->resimyol)) {
                Storage::disk('public')->delete($existingPhoto->resimyol);
            }
            
            // Eski yöntemle de kontrol et
            $dosyaYolu = public_path('storage/' . $existingPhoto->resimyol);
            if (file_exists($dosyaYolu)) {
                unlink($dosyaYolu);
            }
        ...
        // Yeni fotoğrafı yükle (uploadPhoto metodunu kullan)
        return $this->uploadPhoto($request, $tenant_id);
        
    } catch (\Exception $e) {
        return response()->json([
            'success' => false,
            'message' => 'Fotoğraf değiştirme işlemi sırasında hata oluştu: ' . $e->getMessage(),
            'alert_type' => 'danger'
        ], 500);
    }
}

// Stok fotoğrafı var mı kontrolü
public function hasPhoto($tenant_id, $stock_id)
{
    $hasPhoto = stock_photos::where('kid', $tenant_id)
                           ->where('stock_id', $stock_id)
                           ->exists();
