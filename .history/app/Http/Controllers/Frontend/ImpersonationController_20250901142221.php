...
public function start(Request $request, $userId)
{
    ...
    // ÖNEMLİ: Session bilgilerini ÖNCE ayarla
    session([
        'impersonator_id' => $currentUser->user_id,
    ...
    // Target user'ı fresh olarak yükle
    $targetUser = $targetUser->fresh(['roles', 'permissions', 'tenant']);

    // Auth işlemleri - DİKKAT: Session'dan SONRA yapın
    try {
        // Mevcut auth'u temizle ama session'ı koruyarak
        Auth::logout();
        
        // Session'ı yeniden başlat ama impersonation verilerini koru
        $impersonationData = [
        ...
        // Target user olarak login ol
         Auth::loginUsingId($targetUser->user_id, false);
        
        // Impersonation session'larını geri yükle
        ...
        return response()->json([
            'success' => false,
            'message' => 'Impersonation başlatılamadı: ' . $e->getMessage()
        ], 500);
    }

    $redirectUrl = route('secure.home', ['tenant_id' => $targetUser->tenant_id]);
...
public function stop(Request $request)
{
    if (!session()->has('impersonator_id') || !session()->has('impersonated_user_id')) {
        ...
        return redirect()->route('super.admin.dashboard')
                         ->with('error', 'Aktif impersonation session bulunamadı.');
    }

    $impersonatorId = session('impersonator_id');
    $impersonationRecordId = session('impersonation_id');

    // Impersonation kaydını sonlandır
    if ($impersonationRecordId) {
        UserImpersonation::find($impersonationRecordId)?->update([
            'ended_at' => now()
        ]);
    }

    // Session'ları ÖNCE temizle
    $sessionData = [
        'impersonator_id' => session('impersonator_id'),
        'impersonated_user_id' => session('impersonated_user_id'),
        'impersonation_id' => session('impersonation_id'),
    ...
    if ($originalUser) {
        ...
        // Kullanıcı verilerini fresh olarak yükle
        $originalUser->refresh();
        $originalUser->load('roles', 'permissions', 'tenant');
    }
