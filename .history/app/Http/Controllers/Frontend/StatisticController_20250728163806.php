...
public function getTechnicianStatisticsData(Request $request, $tenant_id)
{
    try {
        ...
        // Tarih aralığındaki tüm tarihleri al (Y-m-d formatında)
        $tarihListesi = $this->getDatesFromRange($tarih1, $tarih2, 'Y-m-d');
...
{
    \Log::info('getServicePlansForDates başlatıldı:', [
        'tarih_listesi' => $tarihListesi,
        'tenant_id' => $tenant_id,
        'cihaz_tur' => $cihazTur
    ]);

    if (empty($tarihListesi)) {
        \Log::info('Tarih listesi boş, boş array döndürülüyor');
        return [];
    }
    ...
    // Tarih karşılaştırması için her iki formatı da dene
   
    $query->where(function($q) use ($tarihListesi) {
        foreach ($tarihListesi as $tarih) {
            // Y-m-d formatını dene (örneğin '2025-07-28')
            $q->orWhere('sc.cevap', 'like', $tarih . '%'); // Saat bilgisi olsa bile eşleşir

            // d/m/Y formatını dene (örneğin '28/07/2025')
            try {
                $tarihDMY = Carbon::createFromFormat('Y-m-d', $tarih)->format('d/m/Y');
                $q->orWhere('sc.cevap', 'like', $tarihDMY . '%'); // Saat bilgisi olsa bile eşleşir
            } catch (\Exception $e) {
                // Dönüşüm hatası olursa dikkate alma
            }
        }
    });
    ...
    \Log::info('Servis plan sorgusu sonucu:', [
        'kullanilan_tarih_listesi_ymd' => $tarihListesi,
        'kullanilan_tarih_listesi_dmy' => $tarihListesiDMY ?? [],
        'bulunan_planlar' => count($seciliTarihler),
        'plan_ids' => array_slice($seciliTarihler, 0, 10) // İlk 10 ID'yi göster
    ]);
...
private function getTechnicianServices($teknisyenId, $servisPlanlar)
{
    ...
    \Log::info('Teknisyen servisleri:', [
        ...
        'servis_ids' => array_slice($servisler, 0, 10) // İlk 10 servis ID'sini göster
    ]);

    return $servisler;
}

// Veritabanında hangi tarih formatının kullanıldığını kontrol etmek için debug fonksiyonu
public function debugDateFormats($tenant_id)
{
    $sampleDates = DB::table('service_stage_answers as sc')
        ->leftJoin('stage_questions as ss', 'ss.id', '=', 'sc.soruid')
        ->leftJoin('services', 'services.id', '=', 'sc.servisid')
        ->where('ss.cevapTuru', 'LIKE', '%Tarih%')
        ->where('services.firma_id', $tenant_id)
    ...
    \Log::info('Veritabanındaki örnek tarih formatları:', $sampleDates);
    
    return response()->json([
        'sample_dates' => $sampleDates,
        'message' => 'Örnek tarih formatları logda görüntülendi'
    ]);
}
        ...
        try {
            if (Schema::hasTable($tablo)) {
                $teklifToplam = DB::table('service_stage_answers as sc')
                    ...
                    ->whereBetween('sp.tarih', [$tarih1 . ' 00:00:00', $tarih2 . ' 23:59:59'])
                    ->where('sc.cevap', '!=', '')
                    ->where('sc.cevap', '!=', '0')
