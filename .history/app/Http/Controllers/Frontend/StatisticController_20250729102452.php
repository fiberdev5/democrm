  ...
  public function OperatorStatistics(Request $request,$tenant_id)
    {
        ...
        return view('frontend.secure.statistics.operator_statistics', compact('tarih1', 'tarih2','$tenant_id'));
    }
     public function getOperatorStatisticsData(Request $request)
    {
        // CSRF token'ı dışındaki tüm verileri al
        $input = $request->except('_token');

        $tarihler = explode("---", $input["optTabloGetir"]);
        ...
        try {
            ...
            // Hatalı tarih formatı durumunda varsayılan değerler
            $tarih1 = Carbon::now()->startOfDay();
            $tarih2 = Carbon::now()->endOfDay();
        }

        $getTable = Service::query()
            ->where('durum', 1)
        ...
        // Bu sefer tablo içeriğini doğrudan blade view'inden render edip döndürüyoruz.
        // Bu, Ajax isteği sonucunda #operatorStatisticsTable div'ine yerleştirilecek.
        return view('frontend.secure.statistics._operator-statistics-table-content', compact('getTable', 'tarih1Str', 'tarih2Str'))->render();
    }
