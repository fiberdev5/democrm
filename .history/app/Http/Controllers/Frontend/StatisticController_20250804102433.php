...
private function getPlanIds($dateRange, $deviceType = null)
{
    ...
    $query = ServiceStageAnswer::join('stage_questions', 'service_stage_answers.soruid', '=', 'stage_questions.id')
        ...
        // Burada soruid değerlerini, tablonuzdaki gerçek değerlerle değiştirin
        ->whereIn('service_stage_answers.soruid', [35, 40, 48, 57]); // Örnek olarak tablodaki değerler kullanıldı.
...
{
    // Öncelikle, şikayet kaydı olan tüm servis ID'lerini tek bir sorguyla alalım.
    $complaintServiceIds = ServicePlanning::whereIn('servisid', $serviceIds)
        ->where('gidenIslem', 254)
        // Sadece 'servisid' sütununu seçiyoruz, bu GROUP BY hatasını giderir.
        ->groupBy('servisid')
        ->pluck('servisid');

    // Eğer şikayet kaydı bulunamazsa, direkt 0 dönelim.
    if ($complaintServiceIds->isEmpty()) {
        return 0;
    }

    // Şimdi, her bir şikayetli servis için ilk servis kaydını ve son şikayet kaydını bulalım.
    $firstServices = ServicePlanning::whereIn('servisid', $complaintServiceIds)
        ->where('pid', $userId)
    ...
    $lastComplaints = ServicePlanning::whereIn('servisid', $complaintServiceIds)
        ->where('gidenIslem', 254)
    ...
    foreach ($complaintServiceIds as $servisid) {
        $firstService = $firstServices->get($servisid);
        $lastComplaint = $lastComplaints->get($servisid);

        if ($firstService && $lastComplaint) {
            // Carbon nesnelerini doğrudan karşılaştırabilirsiniz, formatlamaya gerek yok.
            // Bu, aynı günde olsalar bile saat ve dakika farkını doğru şekilde değerlendirir.
            if ($lastComplaint->created_at > $firstService->created_at) {
                $count++;
            }
        }
    }
