...
class ServicesController extends Controller
{
    public function AllServices($tenant_id, Request $request) {
        ...
        if ($request->ajax()) {           
            ...
            $filterType = $request->input('filterType');      // 'operator', 'technician', ...
            $f          = $request->input('filters', []);     // = aktif form verileri

            /* === Operatör Arama === */
            if ($filterType === 'operator') {
                if (!empty($f['operator_pers']) && $f['operator_pers'] !== '0')
                    $data->where('kid', $f['operator_pers']);

                if (!empty($f['operator_tarih1']) && !empty($f['operator_tarih2']))
                    $data->whereBetween('created_at', [$f['operator_tarih1'], $f['operator_tarih2']]);
            }
