...
public function AllServices($tenant_id, Request $request)
{
    ...
    // ✅ 1. YETKİ KONTROLÜNÜ BAŞTA BİR KEZ YAP
    $user = auth()->user();
    $canDeleteServices = $user->can('Servisleri Silebilir');
    $canViewOwnServices = $user->can('Kendi Servislerini Görebilir');

    // View için veri (değişmeden)
    ...
    if ($request->ajax()) {
        ...
        // ✅ 2. YETKİ DEĞİŞKENLERİNİ CLOSURE İÇİNE AKTAR
        return DataTables::of($data)
            ...
            ->addColumn('action', function($row) use ($canDeleteServices) {
                // ✅ 3. ARTIK VERİTABANI SORGUSU YOK!
                return $this->colActions($row, $canDeleteServices);
            })
...
private function colActions($row, $canDelete): string
{
    ...
    $deleteButton = '';
    // ✅ ARTIK HER SATIR İÇİN Auth::user()->can() ÇAĞRILMIYOR!
    if ($canDelete) {
