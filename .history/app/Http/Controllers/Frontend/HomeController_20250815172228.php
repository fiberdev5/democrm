...
class HomeController extends Controller
{
    ...
    private function getDailyChartData($period,$tenant_id)
    {
        ...
        ->join('tb_user', 'services.user_id', '=', 'tb_user.user_id') // İki tabloyu birleştiriyoruz
        ->select(
            'kayitTarihi as date',
            DB::raw('COUNT(*) as count')
        )
        ->whereBetween('kayitTarihi', [$startDate->format('Y-m-d'), $endDate->format('Y-m-d')])
        ->where('services.durum', '!=', 0) // Silinmeyenler
        ->where('tb_user.tenant_id', $tenant_id) // Filtreyi tb_user tablosundan yapıyoruz
        ->groupBy('kayitTarihi')
        ->orderBy('kayitTarihi')
        ->get();
    ...
    private function getHourlyChartData($period,$tenant_id)
    {
        ...
        ->join('tb_user', 'services.user_id', '=', 'tb_user.user_id') // İki tabloyu birleştiriyoruz
        ->select(
            DB::raw('HOUR(services.created_at) as hour'),
            DB::raw('COUNT(*) as count')
        )
        ->whereBetween(DB::raw('DATE(services.created_at)'), [$startDate->format('Y-m-d'), $endDate->format('Y-m-d')])
        ->where('services.durum', '!=', 0)
        ->where('tb_user.tenant_id', $tenant_id) // Filtreyi tb_user tablosundan yapıyoruz
        ->whereNotNull('services.created_at')
        ->groupBy(DB::raw('HOUR(services.created_at)'))
        ->orderBy('hour')
        ->get();
