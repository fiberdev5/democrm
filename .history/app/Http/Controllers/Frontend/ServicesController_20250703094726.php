...
class ServicesController extends Controller
{
    ...
    public function SaveServicePlan(Request $request, $tenant_id) {
        ...
        try {
            ...
            // Sadece gelen işlem ID'sini gönderiyoruz.
            $stokHatasiVar = $this->stokKontrolEt($request, $gelenIslemArray['id']);
            if ($stokHatasiVar) {
            ...
            foreach ($request->input('soru') as $questionId => $answerValue) {
                ...
                if ($stageQuestion) {
                    if ($stageQuestion->cevapTuru == "[Parca]") {
                        ...
                        // Diğer soru türleri için ServiceStageAnswer kaydı oluşturma (eğer böyle bir modeliniz varsa).
                        // Bu kısım sizin mevcut `ServiceStageAnswer` modelinize ve yapınıza bağlıdır.
                        // Eğer bu kısım yoksa veya farklı bir işlem yapıyorsanız, bu `else` bloğunu buna göre düzenleyebilirsiniz.
                        ...
                        ServiceStageAnswer::create([
                            ...
                            'planid' => $planId,
                            'soruid' => $questionId,
                            'cevap' => is_array($answerValue) ? json_encode($answerValue) : $answerValue,
                            'created_at' => now(),
                            'updated_at' => now()
                        ]);
                        */
