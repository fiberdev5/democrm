...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
  ...
  public function EditServicePlan($tenant_id, $planid) {
        ...
        // urunKategori'si 3 olan ve firma ile eşleşen stokları çek
        $consignmentDevices = Stock::where('firma_id', $firma->id)
                                   ->where('urunKategori', 3) 
                                   ->get();
        
        $toplamConsignmentAdedi = 0; // Toplam konsinye adedini sıfırla
        // `consignmentDevices` koleksiyonu doğrudan görünüme gönderilecek,
        // ancak her bir cihaz için `current_stock_quantity`'yi hesaplayacağız.
        ...
        foreach ($consignmentDevices as $device) {
            ...
            // Sadece pozitif stokları toplama dahil et ve `calculatedConsignmentDevices`'a ekle
            if ($device->current_stock_quantity > 0) {
                $toplamConsignmentAdedi += $device->current_stock_quantity;
                $calculatedConsignmentDevices->push($device); // Bu, view'e gidecek olan listemiz
            } else if ($servisPlan->cevap && str_contains($servisPlan->cevap, 'C_' . $device->id . '---')) {
                // Eğer stok 0 veya negatifse ancak mevcut planda bu cihaz seçiliyse, yine de listeye ekle
                // Böylece, seçili olan cihaz listede görünmeye devam eder, ancak mevcut adedi 0 gösterilir.
