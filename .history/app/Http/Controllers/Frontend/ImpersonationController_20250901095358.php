...
public function stop(Request $request)
{
    ...
    $impersonationId = session('impersonated_id');
    ...
    // Session'ları ÖNCE temizle (ÖNEMLİ!)
    session()->forget([
        'impersonator_id',
        ...
        'original_user_id',
        'impersonation_middleware_processed' // Middleware işaretlemesini de temizle
    ]);

    // Gerçek impersonator kullanıcıyı al (session'dan)
    $impersonatorUser = User::find($impersonatorId);
    
    if ($impersonatorUser) {
        // Sadece Auth::setUser kullan, logout/login yapma
        Auth::setUser($impersonatorUser);
        
        // Spatie Permission cache'ini temizle
        app()[\Spatie\Permission\PermissionRegistrar::class]->forgetCachedPermissions();
        
        // Kullanıcı rollerini yeniden yükle
        $impersonatorUser->load('roles', 'permissions');
    }

    if ($request->expectsJson()) {
        // İmpersonator super admin ise super admin dashboard'a, değilse normal home'a yönlendir
        $redirectUrl = $impersonatorUser && $impersonatorUser->isSuperAdmin() 
            ? route('super.admin.dashboard', ['tenant_id' => $impersonatorUser->tenant_id])
            : route('secure.home', ['tenant_id' => $impersonatorUser->tenant_id]);
    ...
    // İmpersonator super admin ise super admin dashboard'a, değilse normal home'a yönlendir
    $redirectRoute = $impersonatorUser && $impersonatorUser->isSuperAdmin() 
        ? 'super.admin.dashboard'
        : 'secure.home';
        
    return redirect()->route($redirectRoute, ['tenant_id' => $impersonatorUser->tenant_id])->with($notification);
}
