...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        $consignmentDevices = Stock::where('firma_id', $firma->id)
            ->where('urunKategori', 3) // urunKategori 3'ün konsinye cihazları temsil ettiğini varsayalım
            ->get();
        
        $toplamConsignmentAdedi = 0; // Toplam konsinye adedini sıfırla
        $detailedConsignmentStocks = collect(); // Görünüme göndermek için detaylı konsinye stok listesi
        ...
        foreach ($consignmentDevices as $device) {
            ...
            // Her bir cihazın anlık stok miktarını hesapla ve modele ata
            // Not: current_stock_quantity'yi negatif olmaması için max(0, ...) kullanıldı
            $device->current_stock_quantity = max(0, $girisAdet - $cikisAdet); 
            
            // Bu kısım değişti: Sadece pozitif stokları toplamaya dahil et,
            // ancak detailedConsignmentStocks'a tümünü (sıfır stoklu olanları da) ekle.
            // Böylece Blade'de her zaman stok miktarı bulunabilir.
            if ($device->current_stock_quantity > 0) {
                $toplamConsignmentAdedi += $device->current_stock_quantity;
            }
            $detailedConsignmentStocks->push($device); // Tüm cihazları (stoklu veya stoksuz) detaylı listeye ekle
        }
