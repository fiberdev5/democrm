...
private function parcaIslemleriniYap(Request $request, $servisId, $planId, $tenantId, $soruId, $gelenIslem)
{
 ...
 private function parcaTeslimEt($stokId, $adet, $servisId, $planId, $tenantId)
    ...
    public function EditServicePlan($tenant_id, $planid) {
        ...
        //Konsinye cihaz stoklar
         $consignmentStocks = Stock::where('firma_id', $firma->id)
            ->where('urunKategori', 3) 
            ->get();

        $detailedConsignmentStocks = $consignmentStocks->map(function ($stock) {
        
            $girisAdet = StockAction::where('stokId', $stock->id)
                ->whereIn('islem', [1, 4])
                ->sum('adet');
           
            $cikisAdet = StockAction::where('stokId', $stock->id)
                ->where('islem', 2)
                ->sum('adet');
            $stock->current_stock_quantity = max(0, $girisAdet - $cikisAdet);

            return $stock;
        })->filter(function ($stock) {
            
            return $stock->current_stock_quantity >= 0;
        });
    ...
    try {
        ...
        foreach ($planCevaplar as $cevap) {
            ...
            if ($soru->cevapTuru == '[Parca]') {
                ...
                // Konsinye Cihaz seçimi için yeni helper fonksiyonunu çağır
                $this->updateConsignmentDeviceSelection($request, $tenant_id, $servisPlan->servisid, $planid, $cevap->soruid, $cevap->id);
                // Helper fonksiyon veritabanında güncellemeyi yaptığı için
                // burada $cevap objesinin kendisini doğrudan güncellemenize gerek kalmayabilir.
                // Eğer $cevap objesinin içindeki 'cevap' değerinin döngünün sonunda
                // güncel halini yansıtmasını istiyorsanız aşağıdaki satırı kullanabilirsiniz.
                $updatedConsignmentAnswer = ServiceStageAnswer::where('id', $cevap->id)->first();
                if ($updatedConsignmentAnswer) {
                    $cevap->cevap = $updatedConsignmentAnswer->cevap;
                }
            } else {
                // Diğer cevap türleri
...
private function updateConsignmentDeviceSelection(Request $request, $tenantId, $servisId, $planId, $soruId)
{
    // Mevcut konsinye cihaz cevaplarını sil
    ServiceStageAnswer::where('planid', $planId)
        ->where('soruid', $soruId)
        ->where('firma_id', $tenantId)
        ->delete();

    $consignmentCevaplar = []; // Konsinye cihaz cevaplarını tutmak için dizi

    // İstekteki tüm inputları gez, konsinye cihaz seçimlerini bul
    // Konsinye cihaz inputlarının adlandırma kuralına göre bu kısmı ayarlamanız gerekebilir.
    // Örneğin, "konsinyeCihazCheck123" veya "soruX" gibi.
    // Eğer her soru için sadece bir konsinye cihaz seçimi varsa, döngüye gerek kalmayabilir.
    // Ancak parçalar gibi birden fazla seçim olabileceği varsayımıyla devam edelim.
    foreach ($request->all() as $key => $value) {
        // Burada konsinye cihaz inputlarını nasıl tanımladığınıza göre bir kontrol yapmalısınız.
        // Örneğin, 'konsinyeCihazCheck' ile başlayan inputlar varsa:
        if (strpos($key, 'konsinyeCihazCheck') !== false) {
            $deviceId = (int) filter_var($key, FILTER_SANITIZE_NUMBER_INT);
            // Konsinye cihaz için adet gibi bir değer varsa onu da alabilirsiniz.
            // $adet = abs($request->input("konsinyeCihazAdet{$deviceId}", 1));
            // $consignmentCevaplar[] = "{$deviceId}---{$adet}";

            $consignmentCevaplar[] = "{$deviceId}"; // Sadece ID'yi kaydediyoruz

            // Konsinye cihazla ilgili ek bir işlem varsa (örneğin, cihazı servise atama, durumunu güncelleme)
            // burada çağırabilirsiniz.
            // Örneğin: $this->assignConsignmentDevice($deviceId, $servisId, $planId, $tenantId);
        }
        // Eğer konsinye cihaz cevabı sadece 'soruID' şeklinde tek bir inputtan geliyorsa
        // ve bu inputun değeri doğrudan cihaz ID'si ise:
        if (strpos($key, 'soru') === 0 && is_numeric(substr($key, 4))) {
             $cevapIdFromKey = (int) substr($key, 4);
             // Eğer bu 'soru' inputu gerçekten bu $soruId'ye aitse ve bir konsinye cihaz ID'si içeriyorsa
             // ve bu $cevapIdFromKey, döngüdeki $cevap->id ile eşleşiyorsa:
             // Bu kontrolü daha spesifik hale getirmek gerekebilir.
             // Şu anki yapıda $cevap->id'ye göre input adı oluşturulduğu için,
             // bu fonksiyonun çağrıldığı yerdeki $cevap->id'ye karşılık gelen inputu almalıyız.
             // Bu yüzden, `UpdateServicePlan` içindeki `$cevap->id`'yi parametre olarak alıp,
             // `soru` . $cevap->id şeklindeki inputu doğrudan okuyabiliriz.
             // Aşağıdaki gibi bir yaklaşım daha doğru olacaktır:
             // if ($cevapIdFromKey == $cevapIdParametresi) { $consignmentCevaplar[] = $value; }
        }
    }

    // `UpdateServicePlan` içinde `updateConsignmentDeviceSelection`'ı çağırırken
    // `$cevap->id`'yi parametre olarak geçirelim.
    // Bu sayede, konsinye cihazın input adını `soru` . $cevapId şeklinde doğrudan alabiliriz.
    $targetCevapInputKey = 'soru' . $request->input('cevapIdParam'); // Varsayımsal olarak cevapId'yi request'ten alıyoruz
    // Veya fonksiyon parametresi olarak doğrudan $cevapId'yi alabiliriz.
    // private function updateConsignmentDeviceSelection(Request $request, $tenantId, $servisId, $planId, $soruId, $cevapId)

    // Eğer konsinye cihaz cevabı sadece tek bir inputtan geliyorsa (birden fazla seçim değilse):
    $selectedConsignment = $request->input('soru' . $request->input('currentCevapId')); // Varsayımsal input adı

    if ($selectedConsignment) {
        $consignmentCevaplar[] = $selectedConsignment;
        // Konsinye cihazla ilgili ek bir işlem varsa (örneğin, cihazı servise atama, durumunu güncelleme)
        // burada çağırabilirsiniz.
        // Örneğin: $this->assignConsignmentDevice($selectedConsignment, $servisId, $planId, $tenantId);
    }


    // Yeni konsinye cihaz cevaplarını kaydet
    if (!empty($consignmentCevaplar)) {
        ServiceStageAnswer::create([
            'firma_id' => $tenantId,
            'kid' => auth()->user()->user_id, // Güncelleme yapan kullanıcı ID'si
            'servisid' => $servisId,
            'planid' => $planId,
            'soruid' => $soruId,
            'cevap' => implode(', ', $consignmentCevaplar), // Cevapları virgülle ayırarak kaydet
            'created_at' => now(),
            'updated_at' => now()
        ]);
    }
