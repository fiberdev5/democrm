...
class PrimController extends Controller
{
    public function hesaplaPrim($data)
    {
        ...
        if ($personel->hasAnyRole(['Operator'])) {
        ...
        if ($personel->hasAnyRole(['Teknisyen'])) {
    ...
    private function operatorPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        $gunlukSinir = $primAyarlari->daily_limit; // Günlük kaç servis
        $primOrani = $primAyarlari->commission_rate; // Prim oranı (%)
        ...
        foreach ($gunler as $gun) {
            ...
            if ($gunlukServisSayisi >= $gunlukSinir) {
                $primTutari = ($gunlukServisSayisi * $primOrani) / 100;
    ...
    private function teknisyenPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        $gunlukSinir = $primAyarlari->daily_limit; // Günlük tutar sınırı (örn: 10000 TL)
        $primOrani = $primAyarlari->commission_rate; // Prim oranı (%)
    ...
    private function atolyeUstasiPrimHesapla($personelId, $tarih1, $tarih2, $tenantId)
    {
        ...
        $gunlukSinir = $primAyarlari->daily_limit; // Günlük tamamlanan servis sayısı
        $primOrani = $primAyarlari->commission_rate; // Prim oranı
