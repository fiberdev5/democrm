    ...
    public function getSurveyStatisticsData(Request $request, $tenant_id)
{
   ...
   $groupedSurveys = [];
// Cihaz türünü almak için services tablosuna join yapılmalı
$surveysQuery = Survey::select(
    'surveys.ekleyen',
    'tb_user.name as adsoyad',
    'surveys.servisid',
    'surveys.created_at as tarih',
    'services.cihazTur' // EKLENDİ
)
->leftJoin('tb_user', 'surveys.ekleyen', '=', 'tb_user.user_id')
->leftJoin('services', 'surveys.servisid', '=', 'services.id') // Cihaz türü için JOIN
->whereBetween('surveys.created_at', [$startDateTime, $endDateTime])
->where('surveys.firma_id', $tenant_id);

if ($deviceTypeId) {
    $surveysQuery->where('services.cihazTur', $deviceTypeId);
}
...
foreach ($surveys as $survey) {
    ...
    $groupedSurveys[$survey->ekleyen]['servisler'][] = [
        'id' => $survey->servisid,
        'cihazTur' => $survey->cihazTur
    ];
}


    // Tamamlanan servisleri getir (gidenIslem=255 olanlar)
    $completedServicesQuery = ServicePlanning::select('servisid')
