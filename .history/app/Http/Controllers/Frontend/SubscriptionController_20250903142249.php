...
class SubscriptionController extends Controller
{
    ...
    /**
     * Paytr ödeme işlemini başlat - Düzeltilmiş versiyon
     */
    public function initiatePayment(Request $request, $tenant_id, $planid)
    {
        try {
            ...
            $orderId = 'SUB' . $tenant_id . '_' . $planid . '_' . time();
            
            // KDV dahil toplam tutarı hesapla
            $totalAmount = $plan->price * 1.20;

            // Sepet bilgilerini hazırla - PayTR formatında
            $basket = [
                [
            ...
            $userAddress = trim(($billingData['address'] ?? '') . ' ' . 
                                $billingData['neighborhood'] ?? '' . ' ' . 
                                $billingData['ilce'] . '/' . $billingData['il']);
            ...
            $orderData = [
                ...
                'amount' => $totalAmount,
                'user_name' => trim($billingData['first_name'] . ' ' . ($billingData['last_name'] ?? '')),
                'user_address' => $userAddress,
                'user_phone' => $billingData['phone'],
                'basket' => $basket,
                'success_url' => 'https://a4b86c57d745.ngrok-free.app/payment/success',
                'fail_url' => 'https://a4b86c57d745.ngrok-free.app/payment/fail',
            ];
    ...
    /**
     * Paytr callback (ödeme bildirimi) - Düzeltilmiş versiyon
     */
    ...
    /**
     * Ödeme başarı sayfası - Düzeltilmiş versiyon
     */
    public function paymentSuccess($tenant_id, $planid)
    {
        Log::info('Payment success page accessed', ['tenant_id' => $tenant_id, 'planid' => $planid]);
    ...
    /**
     * Ödeme başarısız sayfası - Session tabanlı düzeltilmiş versiyon
     */
    ...
    /**
     * Ödeme durumu kontrol et (AJAX) - Düzeltilmiş versiyon
     */
    public function checkPaymentStatus(Request $request)
    {
        ...
        // PayTR'den durum sorgula
        if ($payment->status === 'pending') {
            $statusResult = $this->paytrService->checkOrderStatus($payment->payment_id);
