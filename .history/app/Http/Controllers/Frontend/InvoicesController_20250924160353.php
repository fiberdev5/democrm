...
class InvoicesController extends Controller
{
    ...
    // Çoklu ödeme bilgilerini JSON olarak kaydet
    $paymentDetails = [];
    ...
    $invoice = Invoice::create([
        ...
        'faturaPdf' => $save_url,
        'payment_details' => json_encode($paymentDetails), // Çoklu ödeme detayları
    ]);
    ...
    if($invoice){
        ...
        // 3. Çoklu ödeme durumlarını güncelle
        if (!empty($paymentDetails)) {
            foreach ($paymentDetails as $paymentDetail) {
                $this->markPaymentAsInvoiced(
...
private function convertToDecimal($value)
{
   ...
   public function UpdateInvoice(Request $request, $tenant_id) {
    ...
    / 4. Yeni helper metodlar ekleyin
/**
 * Ödemeyi faturalı olarak işaretle
 */
private function markPaymentAsInvoiced($paymentId, $paymentType, $invoiceId)
{
    switch ($paymentType) {
        case 'subscription':
            // Subscription payments tablosunda güncelleme
        ...
        case 'service':
            // Service payments tablosunda güncelleme
        ...
        case 'other':
            // Other payments tablosunda güncelleme
...
/**
 * Ödemeyi serbest bırak (faturasız hale getir)
 */
private function markPaymentAsFree($paymentId, $paymentType)
{
    switch ($paymentType) {
        case 'subscription':
            DB::table('subscription_payments')
...
// 5. Tamamlanmış ödemeleri getiren route metodunu ekleyin
public function getCompletedPayments(Request $request)
{
    ...
    // Subscription payments - tamamlanmış ve faturası oluşturulmamış olanları getir
    $subscriptionPayments = DB::table('subscription_payments')
        ...
        ->where('subscription_payments.tenant_id', $tenantId)
        ->where('subscription_payments.status', 'completed')
        ->where('subscription_payments.is_invoiced', false)
        ->select([
            ...
            DB::raw('"subscription" as type'),
            DB::raw('CONCAT("Abonelik Ödemesi - ", subscriptions.name) as description')
        ])
        ->get();

    // Service payments - tamamlanmış ve faturası oluşturulmamış olanları getir
    $servicePayments = DB::table('service_payments')
        ->leftJoin('services', 'service_payments.service_id', '=', 'services.id')
        ->where('service_payments.tenant_id', $tenantId)
        ->where('service_payments.status', 'completed')
        ->where('service_payments.is_invoiced', false)
        ->select([
            'service_payments.id',
            'service_payments.amount',
            'service_payments.currency',
            'service_payments.paid_at',
            'service_payments.payment_method',
            DB::raw('"service" as type'),
            DB::raw('CONCAT("Servis Ödemesi - ", services.service_name) as description')
        ])
        ->get();

    // Other payments - tamamlanmış ve faturası oluşturulmamış olanları getir
    $otherPayments = DB::table('other_payments')
