...
public function sendCard($callId, $cardData)
{
    ...
    try {
        ...
        $response = Http::timeout(10)
            ...
            ->post($this->baseUrl . "/calls/{$callId}/cards", [
                'card' => $cardData
            ]);
        ...
        Log::error('Hipcall sendCard error', [
            'call_id' => $callId,
            'error' => $e->getMessage()
        ]);
...
{
    $customerUrl = $baseUrl . "/{$tenantId}/musteri/duzenle/{$customer->id}";
    
    $card = [
        [
            'type' => 'title',
            'text' => '🏢 SerbisERP Müşteri Bilgisi',
            'link' => $customerUrl
        ],
        [
            'type' => 'shortText',
            'label' => 'Müşteri Adı',
            'text' => ($customer->adSoyad ?? '') ,
            'link' => $customerUrl
        ]
    ];
    
    // Firma varsa ekle
    if (!empty($customer->company->name)) {
        $card[] = [
            'type' => 'shortText',
            'label' => 'Firma',
    ...
    // Son servis tarihi varsa
    $lastService = DB::table('services')
        ->where('firma_id', $tenantId)
        ->where('musteri_id', $customer->id)
    ...
    if ($lastService) {
        $card[] = [
            'type' => 'shortText',
            'label' => 'Son Servis',
            'text' => \Carbon\Carbon::parse($lastService->created_at)->format('d.m.Y')
        ];
    }
