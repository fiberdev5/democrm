...
public function addContact($fullName, $phoneNumber)
{
    ...
    try {
        ...
        $responseBody = $response->body(); // Cevabın body'sini bir değişkene alalım

        Log::info('Verimor Contact API Response', ['status' => $response->status(), 'body' => $responseBody]);

        if ($response->status() == 200) {
            // ---- YENİ VE AKILLI KONTROL BAŞLANGICI ----
            ...
            // 1. Durum: Cevap JSON ve içinde 'id' anahtarı var mı?
            if (is_array($data) && isset($data['id'])) {
                $verimorId = $data['id'];
            } 
            // 2. Durum: Cevap JSON değil, sadece bir sayı mı? (Düz ID)
