   ...
   public function createInvoice(array $invoiceData): array
{
    try {
        ...
        Log::info('🔷 Ürün senkronizasyonu tamamlandı', [
        ...
        foreach ($invoiceData['items'] as $index => $item) {
            ...
            // ✅ KRITIK: Ürün ID'si varsa SADECE product, yoksa SADECE description
            if (!empty($productId)) {
                ...
                Log::info("  → Product relationship eklendi", ['product_id' => $productId]);
            } else {
                // Ürün yoksa description zorunlu
                $detail['attributes']['description'] = $item['aciklama'] ?? 'Hizmet';
                Log::info("  → Description eklendi", ['description' => $detail['attributes']['description']]);
            }
        ...
        // 4. Final kontrol
        if (empty($details)) {
            throw new Exception('Fatura detayları boş olamaz!');
        }

        Log::info('🟢 Fatura detayları hazırlandı', [
            'details_count' => count($details),
            'details' => $details // DETAYLI LOG
        ]);
