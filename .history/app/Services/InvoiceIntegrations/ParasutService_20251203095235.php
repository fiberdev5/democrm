...
public function checkCustomerEInvoiceStatus(string $vkn): array
{
    try {
        ...
        // ✅ Inbox alias bilgisini al
        $inboxAlias = null;
        if ($isEInvoiceUser) {
            $inboxAlias = $response['data'][0]['attributes']['alias'] ?? null;
        }
        ...
        return [
            ...
            'inbox_alias' => $inboxAlias, // ✅ Bunu controller'a gönderiyoruz
...
public function getEInvoiceInboxAlias(string $vkn): ?string
{
    try {
        Log::info('E-Fatura inbox alias alınıyor', ['vkn' => $vkn]);
        ...
        if (!empty($response['data']) && count($response['data']) > 0) {
            $alias = $response['data'][0]['attributes']['alias'] ?? null;
            
            Log::info('E-Fatura inbox alias bulundu', [
                'vkn' => $vkn,
                'alias' => $alias
            ]);
        ...
        Log::error('E-Fatura inbox alias alma hatası: ' . $e->getMessage());
        return null;
    }
}
...
public function formalizeAsEInvoice(string $invoiceId, string $vkn, string $scenario = 'basic'): array
{
    try {
        ...
        // ✅ VKN ile inbox alias al
        $inboxAlias = $this->getEInvoiceInboxAlias($vkn);
        
        if (!$inboxAlias) {
            throw new Exception("VKN '{$vkn}' için e-Fatura gelen kutusu bulunamadı. Müşteri e-Fatura mükellefi olmayabilir.");
        }
        
        // ✅ Attributes'u doğru şekilde hazırla
        $attributes = [
            'scenario' => $scenario, // 'basic' veya 'commercial'
            'to' => $inboxAlias, // ✅ ZORUNLU: Alıcının e-Fatura gelen kutusu
        ];
...
public function checkFormalizationStatus(string $invoiceId): array
{
    try {
        ...
        return [
            ...
            'pdf_url' => $pdfUrl,
            'invoice_number' => $officialInvoiceNumber // ⭐ Bunu controller'a gönderiyoruz
        ];
