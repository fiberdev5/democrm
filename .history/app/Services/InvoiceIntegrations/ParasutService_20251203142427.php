    ...
    protected function syncProducts(array $items): array
{
    ...
    foreach ($items as $key => $item) {
        try {
            ...
            // ❌ ESKİ: null ekliyorduk (HATALI)
            // $syncedProducts[] = null;
            
            // ✅ YENİ: Exception fırlat ve işlemi durdur
            Log::error('Ürün senkronizasyon hatası - İşlem durduruluyor', [
            ...
            throw new Exception(
                "Ürün senkronizasyonu başarısız: '{$item['aciklama']}' ürünü Paraşüt'e eklenemedi. " .
                "Hata: " . $e->getMessage()
            );
        }
    }
...
foreach ($invoiceData['items'] as $index => $item) {
    ...
    // ✅ Ürün ID'si varsa ekle, yoksa sadece description ile devam et
    if (!empty($productIds[$index])) {
        ...
        Log::info('Fatura satırına ürün ID eklendi', [
            'product_id' => $productIds[$index],
            'description' => $item['aciklama']
        ]);
    } else {
        // Ürün ID'si yoksa sadece description kullanılacak
        Log::warning('Fatura satırı ürün ID\'si olmadan oluşturuluyor', [
            'description' => $item['aciklama'],
            'index' => $index
        ]);
    }
