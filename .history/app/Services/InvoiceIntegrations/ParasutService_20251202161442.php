...
public function checkCustomerEInvoiceStatus(string $vkn): array
{
    try {
        ...
        // ✅ Doğru endpoint: GET /{company_id}/e_invoice_inboxes?filter[vkn]={vkn}
        $response = $this->makeRequest('get', 'e_invoice_inboxes', [
            'filter[vkn]' => $vkn,
        ]);
        
        // Eğer data array'i boş değilse ve en az 1 kayıt varsa → e-Fatura mükellefi
...
public function formalizeAsEInvoice(string $invoiceId, string $vkn, string $scenario = 'basic'): array
    {
        try {
            ...
            // Payload hazırlığı
            $attributes = [
                'scenario' => $scenario, // 'basic' (Temel) veya 'commercial' (Ticari)
                'vkn' => $vkn, // Zorunlu alan
            ];
...
{
    try {
        // 1. Faturayı ve ilişkili e-belge bilgisini al
        // 'include=e_document' ekleyerek ilişkiyi garantiye alıyoruz
        $response = $this->makeRequest('get', "sales_invoices/{$invoiceId}", [
            'include' => 'e_document'
        ]);
        
        // e-document ilişkisini kontrol et
        $eDocumentId = null;
        $eDocumentType = null;
        
        // İlişki verisi 'data' içinde veya 'included' içinde olabilir, standart kontrol:
        if (isset($response['data']['relationships']['e_document']['data'])) {
            $eDocumentData = $response['data']['relationships']['e_document']['data'];
            $eDocumentId = $eDocumentData['id'] ?? null;
            $eDocumentType = $eDocumentData['type'] ?? null; // 'e_invoices' veya 'e_archives'
        }
        
        if (!$eDocumentId || !$eDocumentType) {
            return [
                'success' => true,
                'formalized' => false,
                'status' => null,
                'type' => null,
                'message' => 'e-Belge henüz oluşmamış.'
            ];
        }
        
        // 2. e-document detaylarını al (Durumunu öğrenmek için)
        ...
        $errors = $eDocResponse['data']['attributes']['errors'] ?? [];
        
        // ✅ GÜNCELLENMİŞ STATUS HARİTASI
        $statusMap = [
            // Bekleme Durumları
            'waiting_for_approval' => 'pending',
            'queued'               => 'pending',
            'running'              => 'pending',
            'unsent'               => 'pending', // Gönderilmedi (henüz)
            
            // Başarılı Durumlar
            'approved'             => 'sent', // Onaylandı
            'sent'                 => 'sent', // Gönderildi
            'printed'              => 'sent', // Yazdırıldı (e-Arşiv için genelde bu döner)
            'signed'               => 'sent', // İmzalandı (e-Arşiv için bu da dönebilir)
        ...
        // Yerel statüyü belirle
        $localStatus = $statusMap[$eDocStatus] ?? 'pending';

        // ⭐ EKSTRA KONTROL: Eğer statü haritada yoksa ama PDF oluşmuşsa, işlem başarılıdır.
        if ($localStatus === 'pending' && !empty($pdfUrl)) {
            $localStatus = 'sent';
            Log::info('Statü pending görünüyor ama PDF var, sent olarak işaretlendi.', ['status' => $eDocStatus]);
        }
        
        Log::info('Resmileştirme durumu kontrol edildi', [
            'invoice_id' => $invoiceId,
            'e_document_id' => $eDocumentId,
            'type' => $type,
            'parasut_status' => $eDocStatus,
        ...
        return [
            'success' => true,
            'formalized' => true,
            'status' => $localStatus, // 'pending', 'sent', 'error'
