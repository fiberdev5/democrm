...
public function checkFormalizationStatus(string $invoiceId): array
{
    try {
        // Faturayı getir
        $response = $this->makeRequest('get', "/v4/{$this->companyId}/sales_invoices/{$invoiceId}", [
            'include' => 'e_document'
        ]);
        ...
        if (!isset($response['data']['relationships']['e_document']['data'])) {
             return [
                ...
                'status' => null, 
                'type' => null,
                'invoice_number' => $officialInvoiceNumber // Numarayı yine de dönelim
            ];
        }
        ...
        $statusMap = [
            ...
            'approved' => 'sent',
            'sent' => 'sent',
            'printed' => 'sent', // e-Arşiv için önemli
            'signed' => 'sent',
            'rejected' => 'error',
            'failed' => 'error',
            'unknown' => 'pending'
