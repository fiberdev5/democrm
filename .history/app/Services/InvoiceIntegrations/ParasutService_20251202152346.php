...
public function formalizeAsEInvoice(string $invoiceId): array
{
    try {
        ...
        // ✅ Doğru endpoint: POST /{company_id}/e_invoices
        $response = $this->makeRequest('post', "e_invoices", [
            'data' => [
                'id' => $invoiceId, // ✅ Fatura ID'si
                'type' => 'e_invoices',
                'attributes' => [
                    'vkn' => null, // Müşteri VKN - opsiyonel
                    'to' => null   // Alıcı email - opsiyonel
                ],
                'relationships' => [
                    'sales_invoice' => [
                        'data' => [
...
public function formalizeAsEArchive(string $invoiceId): array
{
    try {
        ...
        // ✅ Doğru endpoint: POST /{company_id}/e_archives
        $response = $this->makeRequest('post', "e_archives", [
            'data' => [
                'id' => $invoiceId, // ✅ Fatura ID'si eklenmiş
                'type' => 'e_archives',
                'attributes' => [
                    'internet_sale' => [
                        'url' => config('app.url'),
                ...
                'relationships' => [ // ✅ Relationships eklenmiş
                    'sales_invoice' => [
                        'data' => [
                            'id' => $invoiceId,
                            'type' => 'sales_invoices'
...
public function checkFormalizationStatus(string $invoiceId): array
{
    try {
        ...
        return [
            'success' => true,
            'formalized' => true,
            'status' => $statusMap[$eDocStatus] ?? 'pending',
