...
/**
 * Müşteri bilgilerini Paraşüt'te güncelle
 */
public function updateContact(string $contactId, array $customerData): array
{
    try {
        Log::info('Paraşüt müşteri güncelleniyor', [
            'contact_id' => $contactId,
            'name' => $customerData['adSoyad'] ?? null
        ]);
        ...
        if (!empty($customerData['email'])) {
            $parasutData['data']['attributes']['email'] = $customerData['email'];
        }

        if (!empty($customerData['tel1'])) {
            $parasutData['data']['attributes']['phone'] = $customerData['tel1'];
        }

        if (!empty($customerData['vergiNo'])) {
            $parasutData['data']['attributes']['tax_number'] = $customerData['vergiNo'];
        }

        if (!empty($customerData['tcNo']) && empty($customerData['vergiNo'])) {
            // Bireysel müşteri için TC No
            $parasutData['data']['attributes']['tax_number'] = $customerData['tcNo'];
        }

        if (!empty($customerData['vergiDairesi'])) {
            $parasutData['data']['attributes']['tax_office'] = $customerData['vergiDairesi'];
        }

        if (!empty($customerData['adres'])) {
            $parasutData['data']['attributes']['address'] = $customerData['adres'];
        }

        if (!empty($customerData['il'])) {
            $parasutData['data']['attributes']['city'] = $customerData['il'];
        }

        if (!empty($customerData['ilce'])) {
            $parasutData['data']['attributes']['district'] = $customerData['ilce'];
        }
 ...
 */
public function getContactIdByCustomer($customer): ?string
{
    try {
        // Eğer parasut_contact_id sütunu varsa direkt al
        if (!empty($customer->parasut_contact_id)) {
            return $customer->parasut_contact_id;
        }

        // Yoksa müşteriyi ara
        $existingCustomer = $this->findCustomer([
        ...
        if ($existingCustomer) {
            ...
            $customer->update(['parasut_contact_id' => $existingCustomer['id']]);
        ...
        Log::error('Paraşüt contact ID bulunamadı: ' . $e->getMessage());
        return null;
    }
}
/**
