...
public function checkFormalizationStatus(string $invoiceId, bool $getPdfUrl = true): array
{
    try {
        ...
        if ($localStatus === 'sent' && $getPdfUrl) {
            Log::info('PDF URL alınmaya başlanıyor', [
                'invoice_id' => $invoiceId,
                'e_document_id' => $eDocumentId,
                'e_document_type' => $eDocumentType
            ]);
            ...
            Log::info('waitAndGetPdfUrl sonucu', [
                'result' => $pdfResult
            ]);
            ...
            Log::info('PDF alma koşulu SAĞLANMADI - PDF alınmayacak', [
                'reason' => $localStatus !== 'sent' ? "Status 'sent' değil, şu an: {$localStatus}" : "getPdfUrl false"
            ]);
        }
