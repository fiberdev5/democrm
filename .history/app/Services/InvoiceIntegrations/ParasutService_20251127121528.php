...
class ParasutService implements InvoiceIntegrationInterface
{
    ...
    {
        // VKN veya TCKN ile ara
        $taxNumber = $customerData['vergiNo'] ?? $customerData['tcNo'];
        
        if ($taxNumber) {
            $existing = $this->makeRequest('get', 'contacts', ['filter[tax_number]' => $taxNumber]);
            if (!empty($existing['data'])) {
                Log::info('Müşteri bulundu', ['id' => $existing['data'][0]['id']]);
                return $existing['data'][0]; // Direkt Paraşüt objesini döndür (içinde 'id' var)
            }
        }
        ...
        $payload = [
            'data' => [
                ...
                'attributes' => [
                    ...
                    'tax_office' => $customerData['vergiDairesi'] ?? '',
                    'email' => $customerData['email'] ?? '',
                    'address' => $customerData['adres'] ?? '',
                    'city' => $customerData['il'] ?? '',
                    'district' => $customerData['ilce'] ?? '',
                    'account_type' => 'customer',
                    'phone' => $customerData['tel1'] ?? '',
                ]
            ]
        ];

        $response = $this->makeRequest('post', 'contacts', $payload);
        Log::info('Yeni müşteri oluşturuldu', ['id' => $response['data']['id']]);
     ...
     */
     public function syncProduct(string $name): array
    {
        // İsim ile ara
        ...
        $payload = [
            'data' => [
                ...
                'attributes' => [
                    'name' => $name,
                    'vat_rate' => 20 // Varsayılan KDV (Faturada ezilecek)
                ]
            ]
        ];
