...
class ParasutService implements InvoiceIntegrationInterface
{
        ...
        try {
            ...
            $parasutInvoiceData = [
                'data' => [
                    ...
                    'attributes' => [
                        'item_type' => 'invoice',
                        'description' => 'Servis Faturası',
                        'issue_date' => date('Y-m-d') ?? $invoiceData['faturaTarihi'],
                        'due_date' => date('Y-m-d', strtotime('+30 days')) ?? $invoiceData['faturaTarihi'],
                        'currency' => 'TRL',
                        'withholding_rate' => 0,
    ...
    /**
     * Faturaya ödeme ekle (mahsuplaştırma)
     */
    protected function addPayment(string $invoiceId, array $paymentData): bool
    {
        try {
            Log::info('Mahsuplaştırma yapılıyor', [
                'invoice_id' => $invoiceId,
            ...
            $data = [
                'data' => [
                    'type' => 'payments',
                    'attributes' => [
                        'account_id' => (int) ($paymentData['account_id'] ?? 1),
            ...
            Log::info('Mahsuplaştırma başarılı', [
                'invoice_id' => $invoiceId,
            ...
            Log::error('Mahsuplaştırma hatası: ' . $e->getMessage(), [
                'invoice_id' => $invoiceId
            ]);
            return false;
        }
