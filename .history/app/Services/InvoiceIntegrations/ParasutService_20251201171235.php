...
public function addPayment(string $invoiceId, array $paymentData): array
{
    try {
        ...
        return [
            'success' => true,
            'payment_id' => $response['data']['id'], // ⭐ Bu ID'yi kullanacağız
            'data' => $response['data']
        ];
...
public function deleteInvoicePayment($invoiceId, $paymentId)
{
    try {
        ...
        $url = "{$this->baseUrl}/v4/sales_invoices/{$invoiceId}?include=payments.transaction";
        ...
        if (isset($data['included'])) {
            foreach ($data['included'] as $item) {
                if ($item['type'] === 'payments' && $item['id'] == $paymentId) {
        ...
        if (!$transactionId) {
            ...
            return [
                'success' => false,
                'message' => 'Tahsilat işlemi bulunamadı'
            ];
        }

        // Transaction'ı sil
        $deleteUrl = "{$this->baseUrl}/v4/transactions/{$transactionId}";
