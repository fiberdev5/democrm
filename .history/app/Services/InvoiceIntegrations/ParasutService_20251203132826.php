...
protected function getOrCreateServiceProduct(): ?string
{
    try {
        // Cache'den kontrol et
        $cacheKey = 'parasut_service_product_' . $this->tenantId . '_' . $this->companyId;
        ...
        $productData = [
            'data' => [
                ...
                'attributes' => [
                    ...
                    'vat_rate' => 20,
                    'unit' => 'Adet',
                    'list_price' => 0 // Fiyat faturada dinamik olacak
                ]
            ]
        ];
        
        $createResponse = $this->makeRequest('post', 'products', $productData);
        $productId = $createResponse['data']['id'];
        
        Cache::put($cacheKey, $productId, now()->addDays(30));
        
        Log::info('Genel hizmet ürünü oluşturuldu', ['product_id' => $productId]);
        
        return $productId;
        
    } catch (Exception $e) {
        Log::error('Genel hizmet ürünü oluşturulamadı: ' . $e->getMessage());
        return null;
    }
}

/**
 * ✅ Fatura detaylarını hazırla - YENİ VERSİYON
 */
public function createInvoice(array $invoiceData): array
{
    try {
        ...
        // 2. Genel hizmet ürününü al/oluştur
        $serviceProductId = $this->getOrCreateServiceProduct();
        
        if (!$serviceProductId) {
            throw new Exception('Hizmet ürünü oluşturulamadı');
        }
        
        Log::info('✅ Genel hizmet ürünü kullanılacak', ['product_id' => $serviceProductId]);

        // 3. Fatura detaylarını hazırla - HER ITEM İÇİN AYNI ÜRÜNÜ KULLAN
        $details = [];
        foreach ($invoiceData['items'] as $index => $item) {
            $detail = [
                ...
                'attributes' => [
                    ...
                    'description' => $item['aciklama'] ?? 'Hizmet', // ✅ Description ekle
                ],
                'relationships' => [
                    'product' => [
                        'data' => [
                            'id' => $serviceProductId, // ✅ Hep aynı ürün
                            'type' => 'products'
                        ]
                    ]
                ]
        ...
        $parasutInvoiceData = [
            'data' => [
                ...
                'attributes' => [
                    'item_type' => 'invoice',
                    'description' => $invoiceData['faturaAciklama'] ?? 'Servis Faturası',
        ...
        Log::info('📤 Paraşüt API\'ye gönderiliyor', [
            'full_payload' => json_encode($parasutInvoiceData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)
        ]);
        ...
        return [
            ...
            'customer_action' => $customerSync['action'],
            'data' => $response['data']
        ];
...
public function checkFormalizationStatus(string $invoiceId, bool $getPdfUrl = true): array
{
    try {
        ...
        // ⭐ YENİ: Paraşüt'teki güncel fatura numarasını al
        ...
        $pdfUrl = $eDocResponse['data']['attributes']['pdf_url'] ?? null;
        
        // Status haritası (Önceki cevaptaki güncel harita)
        $statusMap = [
       ...
       $pdfUrl = null;
        ...
        if ($localStatus === 'sent' && $getPdfUrl) {
            Log::info('Fatura resmileştirildi, PDF URL alınıyor', [
                'invoice_id' => $invoiceId,
                'e_document_id' => $eDocumentId,
                'e_document_type' => $eDocumentType
            ]);
            
            // PDF URL'ini bekleyerek al
            ...
            if ($pdfResult['success']) {
                ...
                Log::info('PDF URL başarıyla alındı', [
                    'invoice_id' => $invoiceId,
                    'pdf_url' => substr($pdfUrl, 0, 50) . '...',
                    'expires_at' => $pdfExpiresAt
                ]);
            } else {
                Log::warning('PDF URL alınamadı', [
                    'invoice_id' => $invoiceId,
                    'error' => $pdfResult['error'] ?? 'Bilinmeyen hata'
                ]);
            }
        }
        ...
        return [
            ...
            'pdf_url' => $pdfUrl, // ✅ Paraşüt PDF URL'i
            'pdf_expires_at' => $pdfExpiresAt, // ✅ URL'in son kullanma tarihi
            'invoice_number' => $officialInvoiceNumber,
            'e_document_id' => $eDocumentId
        ];
...
public function getEArchivePdfUrl(string $eArchiveId): array
{
    try {
        ...
        $response = Http::withToken($this->accessToken)
            ->accept('application/json')
            ->get("{$this->baseUrl}/e_archives/{$eArchiveId}/pdf");
...
public function getEInvoicePdfUrl(string $eInvoiceId): array
{
    try {
        ...
        $response = Http::withToken($this->accessToken)
            ->accept('application/json')
            ->get("{$this->baseUrl}/e_invoices/{$eInvoiceId}/pdf");
