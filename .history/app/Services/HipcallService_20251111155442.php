...
public function getAllContacts()
{
    ...
    do {
        ...
        if (!$response->successful()) {
            Log::error('Contacts liste çekme başarısız', [
                'status' => $response->status()
            ]);
            break;
        }
        ...
        foreach ($contactsList as $contact) {
            ...
            if ($contactId) {
                ...
                if ($detailResponse->successful()) {
                    ...
                    // Detay data'sı contact field'ı içinde olabilir
                    $fullContact = $detailData['contact'] ?? $detailData['data'] ?? $detailData;
                    $allContacts[] = $fullContact;
                } else {
                    // Detay çekilemezse listedeki veriyi kullan
                    $allContacts[] = $contact;
                }
