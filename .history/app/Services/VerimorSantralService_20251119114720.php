...
public function addCallerId($name, $number)
{
    ...
    // Telefon numarasındaki tüm boşluk, parantez, tire gibi karakterleri temizle
    $cleanedNumber = preg_replace('/\D/', '', $number);

    // Numaranın başındaki 0'ı veya +90'ı temizleyip 90 ile başlatalım
    // Eğer numara 0 ile başlıyorsa (örn: 0532...), ilk 0'ı at.
    if (substr($cleanedNumber, 0, 1) === '0') {
        $cleanedNumber = substr($cleanedNumber, 1);
    }
    // Eğer numara zaten 90 ile başlıyorsa (örn: 90532...), dokunma.
    // Değilse (örn: 532...), başına 90 ekle.
    ...
    try {
        ...
        // ------------ DÜZELTİLEN YER ------------
        // URL'e '/api/v2' eklendi.
        $response = Http::timeout(10)
            ->withHeaders(['Content-Type' => 'application/json'])
            ->post($this->apiUrl . '/api/v2/caller_ids', [ // <--- DÜZELTME BURADA
                'key' => $this->apiKey,
                'name' => $name,
                'number' => $cleanedNumber
            ]);
