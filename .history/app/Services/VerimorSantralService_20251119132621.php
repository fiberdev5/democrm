...
public function addCallerId($name, $number)
{
    ...
    // Telefon numarasındaki tüm gereksiz karakterleri temizle
    $cleanedNumber = preg_replace('/\D/', '', $number);

    // Numaranın başındaki 0'ı at
    if (substr($cleanedNumber, 0, 1) === '0') {
        $cleanedNumber = substr($cleanedNumber, 1);
    }
   
    // Numaranın başında 90 yoksa ekle
    if (substr($cleanedNumber, 0, 2) !== '90') {
        $cleanedNumber = '90' . $cleanedNumber;
    }

    try {
        Log::info('Verimor: Yeni Caller ID ekleniyor (Form Data olarak)', [
            'name' => $name,
            'number' => $cleanedNumber,
            'url' => 'https://api.bulutsantralim.net/caller_ids'
        ]);

        // ------------ TEK DEĞİŞİKLİK BURADA ------------
        // withHeaders satırı kaldırıldı. Laravel artık veriyi form-urlencoded olarak gönderecek.
        $response = Http::timeout(10)
            ->asForm() // Laravel'e bunun bir form gönderimi olduğunu açıkça belirtelim
            ->post('https://api.bulutsantralim.net/caller_ids', [ 
                'key' => $this->apiKey,
                'name' => $name,
                'number' => $cleanedNumber
            ]);
        ...
        if ($response->successful()) {
            return [
                ...
                'data' => $response->json() // Dönen cevap JSON olabilir
            ];
        }
...
public function addContact($fullName, $phoneNumber)
{
    ...
    $surname = array_pop($nameParts); // Son kelimeyi soyad olarak al
    $name = implode(' ', $nameParts); // Geri kalanları ad olarak al
    
    // Eğer ad boşsa (tek kelimelik isim girildiyse), adı soyad ile aynı yap
    if (empty($name)) {
        $name = $surname;
    }
    ...
    if (substr($cleanedNumber, 0, 1) !== '0') {
        $cleanedNumber = '0' . $cleanedNumber; // API'nin istediği 05... formatı
    }
    
    try {
        // API anahtarını URL'e ekliyoruz
        $apiUrlWithKey = 'https://api.bulutsantralim.net/contacts?key=' . $this->apiKey;
        ...
        Log::info('Verimor: Yeni Contact ekleniyor', [
            'url' => $apiUrlWithKey,
            'data' => $contactData
        ]);

        $response = Http::timeout(10)
            ->asForm() // Veriyi form olarak gönderiyoruz
            ->post($apiUrlWithKey, $contactData);

        Log::info('Verimor Contact API Response', [
            'status' => $response->status(),
            'body' => $response->body()
        ]);

        $responseBody = $response->body();
        // Dokümanda başarılı istek için 200 döneceği belirtilmiş.
        if ($response->status() == 200) {
            $data = json_decode($responseBody, true);
            // Başarılı yanıtta kişinin ID'si döner.
            if (isset($data['id'])) {
                 return [
                    'success' => true,
                    'message' => 'Kişi başarıyla rehbere eklendi.',
