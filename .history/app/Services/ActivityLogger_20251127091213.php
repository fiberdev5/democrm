...
class ActivityLogger
{
    public static function log($action, $description, $options = [])
    {
        try {
            ...
            // Eğer user yoksa (webhook/callback durumunda) ve tenant_id verilmişse,
            // o tenant'ın sahibini veya bir kullanıcısını bulmaya çalış
            if (!$user && isset($options['tenant_id'])) {
                $tenant = \App\Models\Tenant::find($options['tenant_id']);
                if ($tenant) {
                    // Tenant'ın ilk kullanıcısını al (genellikle patron/yönetici)
