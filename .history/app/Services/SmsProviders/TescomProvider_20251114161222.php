...
class TescomProvider implements SmsProviderInterface
{
     ...
     public function sendBulkSms(array $phones, string $message): array
    {
        try {
            ...
            if ($response->status() === 401) {
                ...
                return [
                    'success' => false,
                    'message' => 'Tescom API kimlik doğrulama hatası. Kullanıcı adı veya şifreniz yanlış. Lütfen entegrasyon ayarlarınızı kontrol edin.',
            ...
            if ($response->status() === 403) {
                return [
                    'success' => false,
                    'message' => 'Tescom API erişim izniniz yok. Hesabınızın aktif olduğundan emin olun.',
    ...
    protected function formatPhoneNumbers(array $phones): array
    {
        ...
        foreach ($phones as $phone) {
            ...
            // Geçerli Türkiye telefon numarası mı kontrol et (11 haneli ve 0 ile başlamalı)
    ...
    public function testConnection(): array
    {
        try {
            ...
            if (empty($username) || empty($password)) {
                return [
                    'success' => false,
                    'message' => 'API kullanıcı adı veya şifre eksik'
                ];
            }
            
            // Basit bir balance veya status sorgulama endpoint'i varsa kullan
            // Yoksa küçük bir test SMS gönder
            $response = Http::withBasicAuth($username, $password)
                ->timeout(10)
                ->get($this->apiUrl . '/account/balance'); // Bu endpoint Tescom API'de varsa
            
            if ($response->status() === 401) {
                return [
                    'success' => false,
                    'message' => 'Kimlik doğrulama başarısız. Kullanıcı adı veya şifre yanlış.'
                ];
            }
            ...
            if ($response->successful()) {
                return [
                    'success' => true,
                    'message' => 'Tescom API bağlantısı başarılı'
                ];
            }
            
            return [
                'success' => false,
                'message' => 'API bağlantısı test edilemedi'
            ];
