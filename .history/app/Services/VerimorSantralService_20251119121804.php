...
public function addCallerId($name, $number)
{
    ...
    try {
        Log::info('Verimor: Yeni Caller ID ekleniyor', [
            ...
            'url' => 'https://api.bulutsantralim.net/caller_ids' // Gideceği adres
        ]);

        $response = Http::timeout(10)
            ->withHeaders(['Content-Type' => 'application/json'])
            // ------------ DEĞİŞİKLİK BURADA ------------
            // URL doğrudan elle girildi.
        ...
        // Verimor'dan dönen bazı hatalar 200 (OK) status kodu ile gelebilir,
        // bu yüzden body'yi de kontrol etmekte fayda var.
        $responseBody = $response->body();
        if ($response->successful() && !str_contains(strtolower($responseBody), 'error')) {
            return [
                'success' => true,
