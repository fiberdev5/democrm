    ...
    public function getFeatureLimit($feature)
{
    ...
    // Önce limits sütununu kontrol et
    if (!empty($plan->limits) && isset($plan->limits[$feature])) {
        return $plan->limits[$feature];
    }
    
    // Sonra features sütununu kontrol et (eski sistem için)
    if (!empty($plan->features) && isset($plan->features[$feature])) {
...
// Alternatif olarak dealers için özel bir method
public function getDealerLimit()
{
    $subscription = $this->currentSubscription;
    if (!$subscription || !$subscription->plan) {
        return 0;
    ...
    // Limits sütunundaki dealers değerini direkt al
    return $plan->limits['dealers'] ?? 0;
}

// canAccessDealers methodunu da güncelleyin
public function canAccessDealers()
{
    // Deneme sürecindeyse bayi sayısını kontrol et
    if ($this->isOnTrial()) {
        return ($this->bayiSayisi ?? 0) > 0;
    }
    
    // Aktif abonelik varsa plan limitini kontrol et
    if ($this->hasActiveSubscription()) {
        return $this->getDealerLimit() > 0;
    }
    
    // Aktif abonelik yoksa erişim yok
    return false;
}
