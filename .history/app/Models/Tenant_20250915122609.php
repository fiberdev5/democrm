...
private function getStockPhotosSize()
{
    ...
    */
    ...
    $stockPath = storage_path("app/public/stocks/firma_{$this->firma_slug}");
    if (is_dir($stockPath)) {
 ...
 */
/**
 * Diğer dosyaların toplam boyutunu hesaplar (Tenant.php model'inde)
 * Ticket resimlerini de dahil eder
 * 
 * @return int bytes cinsinden
 */
private function getOtherFilesSize()
{
    ...
    // Diğer dosya türleri için path'ler - ticket resimleri dahil
    $otherPaths = [
        ...
        storage_path("app/public/support_attachments/firma_{$firmSlug}"), // Ticket resimleri
    ];
...
/**
 * Storage kullanım detaylarını döndürür (güncellenmiş versiyon)
 * 
 * @return array
 */
public function getStorageUsageDetails()
{
    $servicePhotosSize = $this->getServicePhotosSize();
    $stockPhotosSize = $this->getStockPhotosSize(); 
    $otherFilesSize = $this->getOtherFilesSize(); // Artık ticket resimlerini de içeriyor
    
    $totalSize = $servicePhotosSize + $stockPhotosSize + $otherFilesSize;
    
    // Ticket resimleri için özel hesaplama
    $firmSlug = $this->firma_slug ?? $this->id;
    $ticketAttachmentsSize = 0;
    $supportAttachmentsPath = storage_path("app/public/support_attachments/firma_{$firmSlug}");
    
    if (is_dir($supportAttachmentsPath)) {
        $ticketAttachmentsSize = $this->calculateDirectorySize($supportAttachmentsPath);
    }
    
    return [
        'service_photos' => [
            'size_bytes' => $servicePhotosSize,
            'size_gb' => round($servicePhotosSize / (1024 * 1024 * 1024), 4),
            'count' => ServicePhoto::where('firma_id', $this->id)->count()
        ],
        'stock_photos' => [
            'size_bytes' => $stockPhotosSize,
            'size_gb' => round($stockPhotosSize / (1024 * 1024 * 1024), 4),
            // 'count' => StockPhoto::where('firma_id', $this->id)->count()
        ],
        'ticket_attachments' => [
            'size_bytes' => $ticketAttachmentsSize,
            'size_gb' => round($ticketAttachmentsSize / (1024 * 1024 * 1024), 4),
            'ticket_count' => $this->getTicketAttachmentsCount()
        ],
        'other_files' => [
            'size_bytes' => $otherFilesSize - $ticketAttachmentsSize, // Ticket'ları çıkar çünkü ayrı gösteriyoruz
            'size_gb' => round(($otherFilesSize - $ticketAttachmentsSize) / (1024 * 1024 * 1024), 4)
        ],
        'total' => [
            'size_bytes' => $totalSize,
            'size_gb' => round($totalSize / (1024 * 1024 * 1024), 4)
        ]
    ];
}

/**
 * Ticket attachments sayısını hesaplar
 * 
 * @return int
 */
private function getTicketAttachmentsCount()
{
    $firmSlug = $this->firma_slug ?? $this->id;
    $supportAttachmentsPath = storage_path("app/public/support_attachments/firma_{$firmSlug}");
    ...
    try {
        // Ticket klasörlerini say
        $ticketFolders = glob($supportAttachmentsPath . '/ticket_*', GLOB_ONLYDIR);
        
        foreach ($ticketFolders as $folder) {
            $totalFiles += $this->countFilesInDirectory($folder);
        }
        
    } catch (\Exception $e) {
        \Log::warning('Ticket attachments count error', ['error' => $e->getMessage()]);
    }
...
/**
 * Klasördeki dosya sayısını hesaplar (helper method)
 * 
