...
public function getStorageInfo()
{
    ...
    $percentage = $totalLimit == 0 ? 100 : round(($currentUsage / $totalLimit) * 100, 1);

     $currentUsageBytes = $currentUsage * 1024 * 1024 * 1024; // GB to bytes
    $totalLimitBytes = $totalLimit * 1024 * 1024 * 1024; // GB to bytes
    
    $percentage = $totalLimitBytes == 0 ? 100 : round(($currentUsageBytes / $totalLimitBytes) * 100, 1);
    
    return [
        // Bu satırı ekleyin - blade template bunu kullanıyor
        'used_bytes' => $currentUsage * 1024 * 1024 * 1024, // GB'yi byte'a çevir
        
        'current_usage_gb' => $currentUsage,
        'current_usage_formatted' => $this->formatBytes($currentUsage * 1024 * 1024 * 1024, 2, true),
