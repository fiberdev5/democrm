...
private function getDealerDocumentsSize()
{
    ...
    // dealers-documents klasörü
    $dealerDocsPath = storage_path("app/public/dealers-documents/firma_{$this->firma_slug}");
    if (is_dir($dealerDocsPath)) {
        $totalSize += $this->calculateDirectorySize($dealerDocsPath);
    }
...
public function getStorageInfo()
{
    ...
    // Yüzde hesaplaması - eğer formatlanmış değerde gerçek veri varsa onu kullan
    $percentage = 0;
    if ($totalLimit > 0) {
        // Eğer formatlanmış değer "0 B" değilse, gerçek kullanım var demektir
        if ($formattedUsage !== '0 B' && $formattedUsage !== '< 10 MB') {
            // currentUsage değeri doğru ise onu kullan
            $percentage = round(($currentUsage / $totalLimit) * 100, 1);
        } else {
            // Alternatif hesaplama - dosya sayısı varsa yaklaşık hesap
            $servicePhotoCount = ServicePhoto::where('firma_id', $this->id)->count();
            if ($servicePhotoCount > 0) {
                // Ortalama 100KB per fotoğraf varsayımı ile
                $estimatedUsageGB = ($servicePhotoCount * 100 * 1024) / (1024 * 1024 * 1024);
                $percentage = round(($estimatedUsageGB / $totalLimit) * 100, 1);
                $formattedUsage = $this->formatBytes($servicePhotoCount * 100 * 1024, 2, true);
            }
        }
    }
...
public function getTotalStorageLimit()
{
    ...
    public function getAllPayments()
    {
        ...
        return $subscriptionPayments->concat($storagePurchases)
