...
Route::get('/parasut/debug', function () {
    return response()->json([
        'client_id' => config('parasut.client_id'),
        'client_secret' => config('parasut.client_secret') ? 'Var (gizli)' : 'YOK!',
        'username' => config('parasut.username'),
        'company_id' => config('parasut.company_id'),
        'api_url' => config('parasut.api_url'),
    ]);
});
Route::get('/parasut/raw-test', function () {
    ...
    return response()->json([
        ...
        'config_check' => [
            ...
            'company_id' => config('parasut.company_id'),
            'username' => config('parasut.username'),
            'has_password' => !empty(config('parasut.password')),
            'has_secret' => !empty(config('parasut.client_secret')),
        ]
    ]);
});

Route::get('/parasut/show-credentials', function () {
    return response()->json([
        'client_id' => config('parasut.client_id'),
        'client_id_length' => strlen(config('parasut.client_id')),
        'client_secret_length' => strlen(config('parasut.client_secret')),
        'has_spaces_in_id' => str_contains(config('parasut.client_id'), ' ') ? 'EVET - SORUN!' : 'Hayır',
        'has_quotes_in_id' => (str_contains(config('parasut.client_id'), '"') || str_contains(config('parasut.client_id'), "'")) ? 'EVET - SORUN!' : 'Hayır',
    ]);
});
