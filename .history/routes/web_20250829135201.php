...
// Mevcut tenant route'larını güncelle (SuperAdmin kontrolü ekle)
Route::middleware(['auth'])->group(function () {
    
    // Tenant yönetimi - artık SuperAdmin de erişebilir
   Route::get('/{tenant_id}/firmalarimiz', [TenantsController::class, 'AllTenants'])
         ->name('all.tenants')
         ->middleware([function ($request, $next) { // <- Burada bir array içine alındı
             $user = Auth::user();
             
             // Super Admin ise geçmesine izin ver
             if ($user->isSuperAdmin()) {
                 return $next($request);
             }
             
             // Normal kullanıcı ise tenant kontrolü yap
             ...
             return $next($request);
         }]); // <- Array burada kapatıldı
    
    Route::get('/{tenant_id}/firma-duzenle/{id}', [TenantsController::class, 'EditTenant'])
         ->name('edit.tenant')
         ->middleware([function ($request, $next) { // <- Burada da array içine alındı
             $user = Auth::user();
