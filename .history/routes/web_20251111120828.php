...
Route::group(['prefix' => '{tenant_id}', 'middleware' => ['auth','checkTenantId','check.subscription','redirect_after_login','check.tenant.status']], function () {
    ...
    // });
        ...
        Route::controller(IntegrationMarketplaceController::class)->group(function() { 
            ...
            Route::get('/integrations/hipcall/sync-contacts', 'hipcallSyncContacts')
                ->name('tenant.integrations.hipcall.sync-contacts');
            
            // Seçili müşterileri gönder
            Route::post('/integrations/hipcall/sync-selected','syncSelectedContacts')
                ->name('tenant.integrations.hipcall.sync-selected');
            
            // Tüm müşterileri gönder
            Route::post('/integrations/hipcall/sync-all','syncAllContacts')
                ->name('tenant.integrations.hipcall.sync-all');
            
            // Tek müşteri gönder
            Route::post('/integrations/hipcall/sync-single/{customer_id}','syncSingleContact')
                ->name('tenant.integrations.hipcall.sync-single');
