...
Route::group(['prefix' => '{tenant_id}', 'middleware' => ['auth','checkTenantId','check.subscription','redirect_after_login','check.tenant.status']], function () {
    ...
    // Entegrasyon Detayı
    Route::get('/entegrasyonlar/{slug}', 'show')->name('tenant.integrations.show');
    
    // Entegrasyonu Aktifleştir/Deaktive Et
    Route::get('/entegrasyonlar/{id}/aktif-et', 'activate')->name('tenant.integrations.activate');
    Route::get('/entegrasyonlar/{id}/pasif-yap', 'deactivate')->name('tenant.integrations.deactivate');
    
    // Firmamın Entegrasyonları
    Route::get('/entegrasyonlarim','myIntegrations')->name('tenant.integrations.my');
    
    // Entegrasyon Ayarları
    Route::get('/entegrasyonlar/{id}/ayarlar','settings')->name('tenant.integrations.settings');
    Route::post('/entegrasyonlar/{id}/ayarlar','saveSettings')->name('tenant.integrations.settings.save');
});
