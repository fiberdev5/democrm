...
use App\Http\Controllers\Frontend\SuperAdminController;SupportTicketController
use App\Http\Controllers\Frontend\SupportTicketController;
use App\Http\Controllers\Frontend\SuperAdmin\SupportTicketController;
...
Route::middleware(['auth'])->prefix('{tenant_id}')->group(function () {
    Route::resource('support', SupportTicketController::class)->only(['index', 'create', 'store', 'show']);
...
// Super Admin destek yönetimi
Route::middleware(['auth'])->prefix('super-admin')->name('super.admin.')->group(function () {
    Route::get('support/dashboard', [\App\Http\Controllers\SuperAdmin\SupportTicketController::class, 'dashboard'])->name('support.dashboard');
    Route::resource('support', \App\Http\Controllers\SuperAdmin\SupportTicketController::class)->only(['index', 'show']);
    Route::post('support/{ticket}/reply', [\App\Http\Controllers\SuperAdmin\SupportTicketController::class, 'reply'])->name('support.reply');
    Route::patch('support/{ticket}/close', [\App\Http\Controllers\SuperAdmin\SupportTicketController::class, 'close'])->name('support.close');
    Route::patch('support/{ticket}/reopen', [\App\Http\Controllers\SuperAdmin\SupportTicketController::class, 'reopen'])->name('support.reopen');
    Route::get('support/{ticket}/download/{fileName}', [\App\Http\Controllers\SuperAdmin\SupportTicketController::class, 'downloadAttachment'])->name('support.download');
});
//İmpersonation 
...
// Route::get('/{tenant_id}/firmalarimiz', [TenantsController::class, 'AllTenants'])
//     ->name('all.tenants')
//     ->middleware('checkTenantId');

// Route::get('/{tenant_id}/firma-duzenle/{id}', [TenantsController::class, 'EditTenant'])
//     ->name('edit.tenant')
//     ->middleware('checkTenantId');
