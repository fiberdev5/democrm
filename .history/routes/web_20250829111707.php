...
// Super Admin Grubunun Rotaları (Middleware ile yetki kontrolü yapılacak)
Route::prefix('super-admin')->middleware(['auth', 'role:Super Admin'])->group(function () {
    Route::get('/tenants', [SuperAdminTenantController::class, 'index'])->name('super_admin.tenants.index');
    Route::get('/tenants/{tenant_id}/users', [ImpersonationController::class, 'getUsersForImpersonation'])->name('super_admin.impersonation.users');
    // Diğer Super Admin işlemleri buraya eklenebilir (tenant oluşturma, silme vb.)
});
