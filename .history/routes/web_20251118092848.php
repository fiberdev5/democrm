...
Route::group(['prefix' => '{tenant_id}', 'middleware' => ['auth','checkTenantId','check.subscription','redirect_after_login','check.tenant.status']], function () {
    ...
    Route::post('/integrations/hipcall/refresh-contacts','refreshHipcallContacts')
        ...
        // ============================================
    // VERIMOR SANTRAL (WEB TELEFONU)
    // ============================================
    
    // Web telefonu ana sayfa
    Route::get('/integrations/verimor-santral/webphone', 'showVerimorWebphone')
        ->name('tenant.integrations.verimor-santral.webphone');
    
    // AJAX - Token yenile
    Route::post('/integrations/verimor-santral/refresh-token', 'refreshVerimorToken')
        ->name('tenant.integrations.verimor-santral.refresh-token');
    
    // AJAX - iframe HTML al (modal için)
    Route::get('/integrations/verimor-santral/get-iframe', 'getVerimorIframe')
        ->name('tenant.integrations.verimor-santral.get-iframe');
    
    // Bağlantı testi
    Route::get('/integrations/verimor-santral/test-connection', 'testVerimorConnection')
        ->name('tenant.integrations.verimor-santral.test-connection');
});
