...
class SubscriptionController extends Controller
{
   ...
   public function success(Request $request)
    {
        // QNB’nin gönderdiği tüm parametreler burada gelir
        $response = $request->all();

        // 3D secure kontrolü
        if ($response['3DStatus'] == "1" || $response['3DStatus'] == "2" || $response['3DStatus'] == "3" || $response['3DStatus'] == "4") {
            // ✅ 3D doğrulama başarılı sayılır
            // Tenant / Subscription kayıtlarını güncelle
            // $request->session()->forget('subscription');

            // Kullanıcıyı GET success ekranına yönlendir
            return redirect()->route('subscription.show.success')->with('success', 'Ödeme başarılı, aboneliğiniz aktif!');
        }

        // ❌ doğrulama başarısız
        return redirect()->route('subscription.fail')->with('error', '3D doğrulama başarısız.');
    }
    ...
    {
        return view('frontend.secure.subscription.fail');
    }
