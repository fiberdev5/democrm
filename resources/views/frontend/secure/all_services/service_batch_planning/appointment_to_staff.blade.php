<form method="POST" id="personeltopluPlanKaydet" action="{{route('update.personel.batch', $tenant_id)}}" style="padding:0 10px;">
   @csrf 
    ...
    <div class="row form-group">
      ...
      <div class="col-lg-8">
        ...
        @elseif ($question->cevapTuru == "[Tarih]")
          <input type="date" name="soru{{ $question->id }}" class="form-control datepicker" value="{{ $defaultDateFormatted }}" style="background:#fff;" required>
...
<script>
  $(document).on('submit', '#personeltopluPlanKaydet', function(e) {
    e.preventDefault();
    var form = $(this);
