<form method="post" id="addResource" action="{{ route('store.service.resource', $firma->id)}}" enctype="multipart/form-data" class="needs-validation" novalidate>
    @csrf
     <input type="hidden" name="form_token" id="formTokenResource" value="">
    <div class="row mb-3">
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              var newRow = `<tr>
                ...
                <td>
                  <a href="javascript:void(0);" data-bs-id="${response.id}" class="btn-outline-primary btn-sm editServiceResource mobilBtn mbuton1" data-bs-toggle="modal" data-bs-target="#editServiceResourceModal" title="Göster"><i class="fas fa-eye"></i> <span> Düzenle</span></a>
                  <a href="javascript:void(0);" data-bs-id="${response.id}" class="btn btn-outline-warning btn-sm editServiceResource mobilBtn mbuton1" data-bs-toggle="modal" data-bs-target="#editServiceResourceModal" title="Düzenle"><i class="fas fa-edit"></i> <span> Düzenle</span></a>
                  <a href="javascript:void(0);"  class="btn btn-outline-danger btn-sm mobilBtn deleteServiceResource" data-bs-id="${response.id}" title="Sil"><i class="fas fa-trash-alt"></i> <span> Sil</span></a>
                </td>
              </tr>`;
  ...
  $(document).ready(function() {
      ...
      $('#formTokenResource').val(generateToken());
      ...
      $('#addResource').on('submit', function() {
          ...
          setTimeout(function() {
              ...
              $('#addResource input[type="submit"]').prop('disabled', false);
          }, 3000);
      });
  });
  </script>