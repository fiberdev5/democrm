        ...
        if (this.checkValidity() === false) {
          ...
          $.ajax({
            ...
            success: function(response) {
              alert("Stok kategorisi başarıyla güncellendi");
              var rowToUpdate = $('#datatableStockCategory tbody tr[data-id="' + response.id + '"]');
              rowToUpdate.find('td:eq(0)').html(`<a class="t-link editStockCategory" href="javascript:void(0);" data-bs-id="${response.id}" data-bs-toggle="modal" data-bs-target="#editStockCategoryModal"><div class="mobileTitle">Kategori:</div>${response.kategori}</a>`);    
