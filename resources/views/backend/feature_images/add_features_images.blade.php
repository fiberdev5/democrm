      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            ...
            <h4 class="card-title">Özellik Resimleri Ekleme</h4><br>
            <form method="post" action="{{ route('store.features.image')}}" enctype="multipart/form-data" class="needs-validation" novalidate>   <!--buradaki enctype veritabanına resim yüklenmesine yarıyor -->
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <select name="feature" class="form-select" required>
                    ...
                    <option value="{{ $item->id }}" {{ $item->id == $feature->id ? 'selected' : ''}}>{{ $item->title }}</option>
                    @endforeach
                  </select>
